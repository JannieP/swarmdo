...
| `swarmdo cycles` | — | Find circular import dependencies in the repo, `madge --circular` / dpdm / skott style — the cycles that cause temporal-dead-zone crashes, `undefined` exports at module-eval time, and brittle init order. Detection is provably correct via Tarjan's strongly-connected-components over `codegraph`'s import graph: every SCC with ≥2 files is a group of mutually-importing modules, and a file that imports itself is a one-node cycle. The SCC scan is iterative (no recursion → no stack overflow on deep graphs), O(V+E), and deterministic (sorted). `--ci` exits 1 when any cycle exists so you can gate a build/PR; `--format json` gives `{count, cycles, selfLoops}`. Also MCP tool `cycles` so an agent checks after wiring up modules, before it commits. Pure engine → fixture-tested (2-node, 3-node, self-loop, disjoint cycles, diamond-DAG no-false-positive) |
