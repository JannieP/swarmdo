...
| `swarmdo comms` | `mailbox` | Cross-session agent mailbox (multiplayer swarms): `send -t <session> -m "…"` (`-t all` broadcasts), `inbox` (`-u` unread, `--mark-read`), `read <id>`. Sessions on the same repo share `.swarmdo/comms/`; name yourself with `$SWARMDO_SESSION` |
