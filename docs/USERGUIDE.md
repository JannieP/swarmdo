...
| `swarmdo hooks recipe` | `recipes` | Install ready-made Claude Code hooks (`notify-done` → Stop, `notify-input` → Notification, `memory-inject` + `comms-inbox` → UserPromptSubmit); dry-run by default, `--apply` writes, idempotent additive merge; targets `settings.local.json` (`--shared` / `--global` to retarget) |
