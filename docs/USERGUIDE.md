...
| `swarmdo comms` | `mailbox` | Cross-session agent mailbox (multiplayer swarms): `send -t <session> -m "…"` (`-t all` broadcasts), `inbox` (`-u` unread, `--mark-read`, `--hook`), `read <id>`, `watch` (live-tail). Sessions on the same repo share `.swarmdo/comms/`; name yourself with `$SWARMDO_SESSION`. `inbox --hook` emits unread mail as `UserPromptSubmit` context and marks it read — wire it with `hooks recipe comms-inbox --apply` so messages surface without polling. Also exposed as MCP tools `comms_send` / `comms_inbox` for in-Claude use |
