...
| `swarmdo orchestrate verify/panel` | `orch` | Deterministic multi-agent **verify + vote**: fan out N parallel skeptics (`verify`) or diversified attempts + majority vote (`panel`) on cheap models (`--model`), schema-validated; `--strict` gates CI, `--demo` runs offline with no key. Also MCP tools `orchestrate_verify`/`orchestrate_panel`. See [guide](orchestration-user-guide.md) |
