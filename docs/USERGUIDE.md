...
| `swarmdo changelog` | `notes`, `release-notes` | Release notes from conventional commits; default range is `<lastTag>..HEAD`; `--from/--to/--version/--out/--all/--no-links`. Recognizes git's default `Revert "…"` subject (not just `revert:`) and surfaces it in the Reverts section with the reverted commit's scope, instead of dropping it into the hidden bucket |
