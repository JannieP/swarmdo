...
|---------|---------|-------------|
| `swarmdo usage` | `cost` | Claude Code spend analytics from local transcripts: `daily`, `monthly`, `models`, `projects`, `sessions`, live 5-hour `blocks` burn, `errors` (tool-failure analytics), `cache` (prompt-cache efficiency + $ saved), `diff` (period comparison — day/week/month presets or explicit `--a/--b` ranges, per-model movers), `reflect` (wrapped-style retrospective over `--period 1m\|3m\|6m\|12m`: totals, busiest day, top models by cost share, longest active-day streak, cost trend, cache read share). Cache-write cost is TTL-aware: 1-hour-TTL writes are priced at 2× base input (not the 1.25× 5-minute rate), read from each transcript's `cache_creation` breakdown |
