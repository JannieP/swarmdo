...
| `swarmdo codegraph` | `cg` | Queryable index of exported symbols: `index [path]` scans TS/JS (skips node_modules/dist/.d.ts) and persists `.swarm/codegraph.json`; `query <name>` (`--fuzzy`, `--kind`), `file <path>` lists a file's exports, `stats` summarises. Answers "where is X defined / what does file Y export" without grep+read. MVP: exported top-level symbols (call-graph edges + watch are follow-ups). Also available as MCP tools — `codegraph_index`/`codegraph_query`/`codegraph_file`/`codegraph_stats` (lean profile, so on by default) — so agents query the symbol map in-session instead of shelling out |
