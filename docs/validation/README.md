...
Three-layer regression-protection stack used by swarmdo's CI to catch the regression classes that traditional unit tests miss. Adopt the same stack in any project — the toolkit is project-agnostic and ships in `plugins/swarmdo-core/scripts/witness/`.

> **External-friendly version** (no swarmdo-specific paths): [agentic-validation-system gist](https://gist.the upstream project (see NOTICE)). This document is the in-repo home with cross-references to swarmdo's own implementation.
...
|---|---|---|---|
| `#1867` | `@swarmdo/memory` had `better-sqlite3` as a hard dep + static import | CI ran on Node 20 where prebuilds existed, so the static import evaluated fine | `npm install swarmdo@latest` failed on Node 26 with `node-gyp` errors |
| `#1862` | `swarmdo-core` plugin's `hooks.json` called `--format true` (not a real flag) | No CI test invoked the plugin's `hooks.json` against the CLI with realistic stdin | Every Write/Edit tool use printed `[ERROR] Invalid value for --format: true` |
...
|---|---|---|
| Install smoke | `v3/@swarmdo/memory/scripts/smoke-no-bsqlite.mjs` | `smoke-install-no-bsqlite` (`v3-ci.yml`) |
| Hook smoke | `plugins/swarmdo-core/scripts/test-hooks.mjs` | `plugin-hooks-smoke` (`v3-ci.yml`) |

**Install smoke** — packs `@swarmdo/memory`, installs the tarball into `/tmp/smoke` with `--omit=optional` (simulates "native better-sqlite3 build failed" on Node 26 without prebuilds), asserts the package loads, runtime auto-falls-back to RVF/sql.js, round-trip works. Catches *any* form of "install fails when an optional native dep can't build."

**Hook smoke** — reads each PostToolUse hook from `plugins/swarmdo-core/hooks/hooks.json`, pipes synthetic Claude-Code-style JSON to it, asserts both exit code 0 *and* that the recorded value matches the input. Negative assertions like `expect(stdout).not.toContain('Recording outcome for: true')` are critical — a naive `contains: 'true'` test would have spuriously passed against the broken CLI because the recorded value happened to be `"true"`.
...
SEED=$(echo -n "$GITSHA:swarmdo-witness/v1" | sha256sum | head -c 64)
...
node plugins/swarmdo-core/scripts/witness/history.mjs \
...
All scripts live in `plugins/swarmdo-core/scripts/witness/`. Project-agnostic — the only runtime dep is `@noble/ed25519`. Adopt by copying the directory into your repo.
...
| File | Purpose |
|---|---|
| `plugins/swarmdo-core/skills/witness/SKILL.md` | Workflow guide + anti-patterns |
| `plugins/swarmdo-core/commands/witness.md` | Slash command |
...
```bash
node plugins/swarmdo-core/scripts/witness/init.mjs --root .
...
In swarmdo: `node scripts/regen-witness.mjs` is a thin wrapper that hard-codes swarmdo's paths.
...
These are the specific traps that hit swarmdo's GitHub Actions during the 2026-05-08 work and that adopters will hit too. The fixes are small once you know to look for them; the failure modes are subtle when you don't.
...
**Fix:** `verify.mjs` and `lib.mjs` probe paths now include the workspace packages that *do* declare `@noble/ed25519` directly. In swarmdo:

```js
const probes = [
  repoRoot,
  join(repoRoot, 'v3'),
  join(repoRoot, 'v3/@swarmdo/cli'),                         // declares ed25519
  join(repoRoot, 'v3/@swarmdo/plugin-agent-federation'),     // declares ed25519
];
```
...
There are two ways to invoke the verifier: the bundled CLI subcommand (`swarmdo verify`) and the standalone plugin script (`plugins/swarmdo-core/scripts/witness/verify.mjs`). They produce identical output.
...
- run: node plugins/swarmdo-core/scripts/witness/verify.mjs --manifest verification.md.json --json
```
...
- name: Pack memory tarball (pnpm rewrites workspace:* → versions)
  working-directory: v3/@swarmdo/memory
  run: |
...
- **No CLI required for adopters.** The standalone scripts depend only on `@noble/ed25519` (~15KB minified). Copy `plugins/swarmdo-core/scripts/witness/` into your project, install one package, run.
...
- [witness-fixes.json](../../witness-fixes.json) — swarmdo's project-specific NEW_FIXES
...
- [Public gist](https://gist.the upstream project (see NOTICE)) — external-friendly version
