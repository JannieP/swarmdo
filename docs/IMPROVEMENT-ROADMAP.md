...
**Pitch**: The scheduled verification run reports that 95 artifacts referenced in the signed witness manifest are absent from the working tree. The Ed25519 signature is valid — the manifest itself has not been tampered with — but downstream tooling and enterprise users who run `rufflo verify` in CI will see a failing check on every source-only checkout. This erodes the trust story that ADR-103 was meant to establish.
...
**Pitch**: The SOTA benchmark drive covers 9 of 10 milestones. M5 ("real-model integration quality") is blocked on a stale API key. Without M5, the comparator cannot measure response quality, hallucination rate, or tool-call accuracy against LangGraph/AutoGen/CrewAI in real-LLM mode — the dimension competitors will emphasize most. The PR #2124 results (rufflo wins 3 of 5 on coordination throughput, memory search latency, safety gate coverage, security posture, and plugin ecosystem breadth) are all structure-and-plumbing metrics. M5 is where a skeptical reader will look.
...
## Item 6 — `rufflo` branding cleanup: eliminate `rufflo` strings from CLI output (#1861, #1858)

**Pitch**: `npx rufflo doctor --fix` and the MCP server registration display `rufflo` branding. For users who installed via `npx rufflo`, this is confusing. For enterprise buyers evaluating the `rufflo` product separately from the open-source `rufflo` project, it creates a support and positioning problem. This is a known-low-complexity issue with no architectural dependencies.

**Effort**: S (string replacement in CLI output templates, doctor command, and MCP server name; scope: `v3/@rufflo/cli/src/commands/doctor.ts`, `bin/mcp-server.js`, and the branding strings emitted by `hooks statusline`)
...
**Pitch**: `@noble/ed25519` is pinned at `^2.1.0` across the monorepo. Upstream is at 3.1.0. The v3 release changed async-by-default behavior and renamed `ed.etc.sha512Sync` (which is used in `plugins/rufflo-core/scripts/witness/verify.mjs`). If left unaddressed, npm 11.x's arborist may resolve a transitive dep to v3 and break `rufflo verify` silently — the same class of breakage as the `optionalDependencies` + `peerDependencies` overlap that produced the `npx Invalid Version` pattern (#1147, #2018). The fix is an explicit API-compatibility audit and either a pin to `^2.x` with `overrides`, or a migration to the v3 API.
...
**Pitch**: `rufflo-graph-intelligence@0.1.0-alpha.1` is not in the marketplace's `official` array (`v3/@rufflo/cli/src/plugins/store/discovery.ts`). `rufflo-knowledge-graph@v0.2.0` is listed but not `featured`. With ADR-130 landing in 3.9.0, these plugins become core infrastructure for the unified graph backend. They should be promoted to `official` / `featured` before 3.9.0 ships, and `rufflo-graph-intelligence` should bump to `0.2.0` (stable, not alpha) with the Phase 4 adapter changes. A broader plugin health audit should also flag: (a) `rufflo-iot-cognitum` — no visible test coverage, no ADR; (b) `rufflo-market-data` — depends on external data feeds not exercised in CI; (c) `plugins/rufflo-agent` — unclear if it is superseded by ADR-115/ADR-129's rvagent integration or still carries independent functionality.
...
- **`rufflo-iot-cognitum` health**: insufficient test coverage visible; needs a dedicated audit pass before declaring production-ready.
