# Swarmdo on Raspberry Pi / edge

Swarmdo runs on a Raspberry Pi (and other ARM/edge boards). Nothing special to
...
|-----------------|-----|---------|
| Pi 5 / Pi 4 (4–8GB) | ≥1GB | ✅ full Swarmdo, local embedder |
...
```bash
npx -y swarmdo mcp start --tools-profile lean
```
...
Pi (or another box on your LAN) and route Swarmdo to it:

```bash
export SWARMDO_PROVIDER=ollama
export OLLAMA_BASE_URL=http://localhost:11434   # or http://<lan-host>:11434
npx -y swarmdo task dispatch                       # executes queued tasks locally
```

`swarmdo doctor -c edge` then reports **offline-capable**. Memory, search, and
...
`@swarmdo/plugin-iot-cognitum` plugin, which pairs the local memory + dispatch
loop with device I/O.

## Footprint tips

- `--tools-profile lean` is the single biggest win on constrained RAM.
- Keep `.swarmdo/` on fast storage (USB SSD beats SD card for the HNSW index).
