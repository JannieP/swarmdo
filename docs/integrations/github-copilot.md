# Swarmdo + GitHub Copilot

Expose Swarmdo's MCP tools (vector memory, `agent_run`, `task dispatch`, embeddings,
hooks, swarm) to GitHub Copilot. Both supported surfaces are **MCP-based** —
Swarmdo already ships an MCP server (`swarmdo mcp start`), so this is configuration,
not new code.

> **Why not a Copilot Extension / GitHub App (`@swarmdo` in chat)?** Copilot
   ...
   ```jsonc
   {
     "servers": {
       "swarmdo": {
         "type": "stdio",
         "command": "npx",
         "args": ["-y", "swarmdo", "mcp", "start", "--tools-profile", "lean"],
         "env": { "ANTHROPIC_API_KEY": "${input:swarmdo_anthropic_key}" }
       }
     },
     "inputs": [
       { "id": "swarmdo_anthropic_key", "type": "promptString",
         "description": "Anthropic API key for Swarmdo agent execution", "password": true }
     ]
   }
   ```
...
3. The `swarmdo` tools appear in the tool picker. Try: *"search my Swarmdo memory
...
`balanced` or `full` if you need the rest. (See `swarmdo mcp start --help`.)
...
the cloud agent call Swarmdo's memory/search/dispatch tools inside its loop.
...
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | npx -y swarmdo mcp start --tools-profile lean
