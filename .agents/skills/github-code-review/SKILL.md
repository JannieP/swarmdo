...
gh pr view 123 --json files,diff | npx swarmdo-swarm github review-init --pr 123
...
npx swarmdo-swarm github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
SECURITY_RESULTS=$(npx swarmdo-swarm github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
npx swarmdo-swarm github review-performance \
  --pr 123 \
...
```bash
# Architecture review
npx swarmdo-swarm github review-architecture \
  --pr 123 \
...
npx swarmdo-swarm github review-style \
  --pr 123 \
...
createServer((req, res) => {
  if (req.url === '$github-webhook') {
    ...
    if (event.comment && event.comment.body.startsWith('$swarm')) {
      ...
      execSync(`npx swarmdo-swarm github handle-comment --pr ${event.issue.number} --command "${command}"`);
    }
    ...
    steps:
      ...
      - name: Run Review Swarm
        run: |
          ...
          # Run swarm review
          REVIEW_OUTPUT=$(npx swarmdo-swarm github review-all \
            --pr $PR_NUM \
            --pr-data "$PR_DATA" \
            --diff "$PR_DIFF" \
...
COMMENTS=$(npx swarmdo-swarm github review-comment \
  --pr 123 \
  --diff "$PR_DIFF" \
  --files "$PR_FILES" \
  --style "constructive" \
...
npx swarmdo-swarm github review-comments \
  --pr 123 \
...
npx swarmdo-swarm github quality-gates \
  --define '{
...
npx swarmdo-swarm github review-metrics \
  --period 30d \
...
npx swarmdo-swarm github review-context \
  --pr 123 \
  --load-related-prs \
  --analyze-impact \
...
npx swarmdo-swarm github review-learn \
...
npx swarmdo-swarm github review-train \
  --learn-patterns \
  --adapt-to-style \
  --improve-accuracy
```
...
npx swarmdo-swarm github review-batch \
  --prs "123,124,125" \
  --check-consistency \
  --verify-integration \
  --combined-impact
...
npx swarmdo-swarm github register-agent \
  ...
  swarm-review:
    ...
    steps:
      - name: Run Swarm Review
        run: |
          npx swarmdo-swarm github review-all \
...
npx swarmdo-swarm github review-dashboard \
  --real-time \
...
npx swarmdo-swarm github review-report \
  --format "markdown" \
...
```bash
# Check swarm status
npx swarmdo-swarm swarm-status

# Verify GitHub CLI authentication
gh auth status

# Re-initialize swarm
npx swarmdo-swarm github review-init --pr 123 --force
```
...
npx swarmdo-swarm github review-comments --pr 123 --batch
```
...
npx swarmdo-swarm github review-init --pr 123 --incremental

# Reduce agent count
npx swarmdo-swarm github review-init --pr 123 --agents "security,style" --max-agents 3

# Enable parallel processing
npx swarmdo-swarm github review-init --pr 123 --parallel --cache-results
```
...
- [RUV Swarm Guide](https:/$github.com$upstream$swarmdo-swarm)
- [Swarmdo Integration](https:/$github.com$upstream$swarmdo)
