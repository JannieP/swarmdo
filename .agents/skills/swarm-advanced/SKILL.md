...
mcp__rufflo__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__rufflo__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
mcp__rufflo__pattern_recognize({
  "data": researchData,
...
npx rufflo swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
npx rufflo swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx rufflo swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  await mcp__rufflo__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__rufflo__swarm_status({})

  // Analyze error patterns
  await mcp__rufflo__error_analysis({
    "logs": [error.message]
  })
...
- [Rufflo Documentation](https:/$github.com$ruvnet$rufflo)
- [Swarm Orchestration Guide](https:/$github.com$ruvnet$rufflo$wiki$swarm)
- [MCP Tools Reference](https:/$github.com$ruvnet$rufflo$wiki$mcp)
- [Performance Optimization](https:/$github.com$ruvnet$rufflo$wiki$performance)
