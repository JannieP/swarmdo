...
hooks:
  pre_task: |
    ...
    npx swarmdo-swarm hook pre-task --mode release-swarm --init-swarm
  post_edit: |
    echo "🔄 Synchronizing release swarm state and validating changes..."
    npx swarmdo-swarm hook post-edit --mode release-swarm --sync-swarm
  post_task: |
    echo "🎯 Release swarm task completed. Coordinating final deployment..."
    npx swarmdo-swarm hook post-task --mode release-swarm --finalize-release
  notification: |
    echo "📡 Broadcasting release completion across all swarm agents..."
    npx swarmdo-swarm hook notification --mode release-swarm --broadcast
---
...
npx swarmdo-swarm github release-plan \
  --commits "$COMMITS" \
...
npx swarmdo-swarm github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo$compare/${LAST_TAG}...HEAD \
  ...
  npx swarmdo-swarm github generate-changelog)
...
npx swarmdo-swarm github release-create \
  --version "2.0.0" \
  --changelog "$CHANGELOG" \
  --build-artifacts \
...
CHANGELOG=$(npx swarmdo-swarm github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
...
npx swarmdo-swarm github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
npx swarmdo-swarm github release-build \
...
```bash
# Pre-release testing
npx swarmdo-swarm github release-test \
...
npx swarmdo-swarm github release-deploy \
...
npx swarmdo-swarm github multi-release \
...
npx swarmdo-swarm github hotfix \
  --issue 789 \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx swarmdo-swarm github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx swarmdo-swarm github release-changelog \
            --format markdown)
          ...
          npx swarmdo-swarm github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx swarmdo-swarm github release-publish \
            --platforms all
...
npx swarmdo-swarm github cd-pipeline \
...
npx swarmdo-swarm github release-validate \
  --checks "
    version-conflicts,
...
npx swarmdo-swarm github compat-test \
...
```bash
# Security validation
npx swarmdo-swarm github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts
...
npx swarmdo-swarm github release-monitor \
  --version v2.0.0 \
...
npx swarmdo-swarm github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%"
...
npx swarmdo-swarm github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx swarmdo-swarm github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx swarmdo-swarm github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx swarmdo-swarm github docker-release \
...
npx swarmdo-swarm github mobile-release \
...
```bash
# Emergency hotfix
npx swarmdo-swarm github emergency-release \
  --severity critical \
...
```bash
# Immediate rollback
npx swarmdo-swarm github rollback \
  --to-version v1.9.9 \
