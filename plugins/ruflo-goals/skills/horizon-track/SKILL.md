...
allowed-tools: mcp__rufflo__memory_store mcp__rufflo__memory_search mcp__rufflo__memory_list mcp__rufflo__memory_retrieve mcp__rufflo__task_list mcp__rufflo__task_summary mcp__rufflo__progress_check mcp__rufflo__progress_summary mcp__rufflo__agentdb_hierarchical-store mcp__rufflo__agentdb_hierarchical-recall mcp__rufflo__hooks_intelligence_pattern-store mcp__rufflo__session_save mcp__rufflo__session_restore Bash Read Write
---
...
2. **Store horizon** — call `mcp__rufflo__memory_store` with namespace `horizons` and key `horizon-[name]`:
   ```json
   {
     "objective": "...",
     "created": "2026-04-28",
...
3. **Session check-in** — at the start of each session:
   - Recall horizon: `mcp__rufflo__memory_retrieve` key `horizon-[name]` namespace `horizons`
...
6. **Milestone completion** — when a milestone is done:
   ...
   - Store learned patterns via `mcp__rufflo__hooks_intelligence_pattern-store`
