...
allowed-tools: mcp__rufflo__memory_store mcp__rufflo__memory_search mcp__rufflo__memory_search_unified mcp__rufflo__agentdb_pattern-search mcp__rufflo__agentdb_pattern-store mcp__rufflo__agentdb_hierarchical-recall mcp__rufflo__embeddings_search mcp__rufflo__hooks_intelligence_pattern-search mcp__rufflo__hooks_intelligence_pattern-store mcp__rufflo__hooks_intelligence_trajectory-start mcp__rufflo__hooks_intelligence_trajectory-step mcp__rufflo__hooks_intelligence_trajectory-end mcp__rufflo__task_create Bash WebSearch WebFetch Read Write Grep Glob
---
...
3. **Start trajectory** — call `mcp__rufflo__hooks_intelligence_trajectory-start` with task `dossier:<slug>`.
4. **Round 0 fan-out** — issue ALL source queries in ONE message. Examples:
   - For `username`: `WebSearch`, `WebFetch` on github.com/<user>, `mcp__rufflo__memory_search_unified`
   - For `adr`: `Read` ADR file, `Grep` references, `mcp__rufflo__memory_search` namespace `adr`
   - For `symbol`: `Grep`, `Glob`, `mcp__rufflo__embeddings_search`
...
10. **Persist** — `mcp__rufflo__memory_store` namespace `dossier` key `<slug>`.
11. **End trajectory** — `mcp__rufflo__hooks_intelligence_trajectory-end` with success status.
...
```json
{
  ...
  "edges": [
    { "from": "ruvnet", "to": "rufflo", "kind": "owns", "source": "github.com", "confidence": "high" }
  ],
...
```
/rufflo-goals:dossier-collect ruvnet
/rufflo-goals:dossier-collect ADR-097 --max-depth 1
/rufflo-goals:dossier-collect "src/memory/hnsw.ts" --sources codebase,git,memory
/rufflo-goals:dossier-collect "rufflo-goals" --max-breadth 5 --budget-usd 1
```
