...
argument-hint: "<query>"
allowed-tools: mcp__rufflo__agentdb_semantic-route mcp__rufflo__agentdb_hierarchical-recall mcp__rufflo__agentdb_hierarchical-store mcp__rufflo__agentdb_context-synthesize mcp__rufflo__agentdb_causal-edge mcp__rufflo__agentdb_pattern-search mcp__rufflo__agentdb_pattern-store mcp__rufflo__agentdb_controllers mcp__rufflo__agentdb_health mcp__rufflo__agentdb_batch mcp__rufflo__agentdb_feedback mcp__rufflo__agentdb_consolidate mcp__rufflo__agentdb_session-start mcp__rufflo__agentdb_session-end Bash
---
...
1. **Check health** — `mcp__rufflo__agentdb_health`. Sanity-check `available: true`.
2. **Start session** — `mcp__rufflo__agentdb_session-start` if not already active.
3. **Store knowledge** — `mcp__rufflo__agentdb_hierarchical-store` for structured tier-keyed data (tiers: `working|episodic|semantic`).
4. **Recall knowledge** — `mcp__rufflo__agentdb_hierarchical-recall` with a query.
5. **Search patterns** — `mcp__rufflo__agentdb_pattern-search` for learned patterns (ReasoningBank-routed).
6. **Synthesize context** — `mcp__rufflo__agentdb_context-synthesize` to combine multiple memories.
7. **Build causal graph** — `mcp__rufflo__agentdb_causal-edge` to link related knowledge.

## Available controller groups

Call `mcp__rufflo__agentdb_controllers` to list the runtime registry. Functional categories surfaced via the 15 MCP tools:
...
```bash
npx @rufflo/cli@latest memory search --query "your query" --namespace patterns
npx @rufflo/cli@latest memory store --key "key" --value "value" --namespace patterns
