...
argument-hint: ""
allowed-tools: Bash(npx *) mcp__rufflo__swarm_status mcp__rufflo__swarm_health Monitor
---
Use the Monitor tool to stream swarm events in real time instead of polling:

Run via Monitor: `npx @rufflo/cli@latest swarm watch --stream`
...
For one-shot status, use MCP: `mcp__rufflo__swarm_status` or `mcp__rufflo__swarm_health`.
