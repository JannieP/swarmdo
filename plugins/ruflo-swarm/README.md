...
- **Monitor Streams**: Real-time swarm status via `Monitor("npx @rufflo/cli@latest swarm watch --stream")`
...
- **Verification:** `bash plugins/rufflo-swarm/scripts/smoke.sh` is the contract.
...
Sources: `v3/@rufflo/cli/src/mcp-tools/swarm-tools.ts:71, 145, 208, 270` and `agent-tools.ts:182, 287, 319, 356, 395, 451, 573, 651`.
...
This plugin owns the `swarm-state` AgentDB namespace (kebab-case, follows the convention from [rufflo-agentdb ADR-0001 §"Namespace convention"](../rufflo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — rufflo-swarm plugin contract (12-tool MCP surface, anti-drift defaults, Monitor streaming, smoke as contract)](./docs/adrs/0001-swarm-contract.md)

## Related Plugins

- `rufflo-agentdb` — namespace convention owner
- `rufflo-autopilot` — owns the 270s cache-aware /loop heartbeat for long-running swarms
- `rufflo-intelligence` — `hooks_route` powers swarm agent recommendation per task
