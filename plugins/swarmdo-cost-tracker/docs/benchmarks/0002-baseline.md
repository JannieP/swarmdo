...
| commands/swarmdo-cost.md                     |  2,802 |   370 |    46 |
...
| 3 | `dist/token-optimizer.js` built  | `ls v3/node_modules/@swarmdo/integration/dist/`         | present ✓ | none |
| 4 | sibling contract honored both ways | `grep "cost-tracker" swarmdo-loop-workers/README.md`        | declared at lines 46, 55 ✓ | none |
| 5 | Skill's claimed import path      | `import("@swarmdo/integration/dist/token-optimizer.js")`| FAILED — Node resolver doubled `.js.js` via the `./*` exports rule | use canonical export `@swarmdo/integration/token-optimizer` |
...
| 7 | Booster signal under published CLI | `npx @swarmdo/cli@latest hooks route --task "var to const"` | router used semantic-VectorDb path; **no `[AGENT_BOOSTER_AVAILABLE]` emitted** | added "sparse signal" caveat to the skill — the partition is a lower bound on Tier 1 eligibility |
...
```bash
( cd v3 && node ../plugins/swarmdo-cost-tracker/scripts/bench.mjs )
```
...
`BENCH_LLM_BASELINE=1` (Gemini via OpenAI shim) and `BENCH_ANTHROPIC=1` (Sonnet 4.6 + Opus 4.7) drive the same corpus. API keys pulled from the GCP secrets the deployed swarmvocal Cloud Run service uses (`GOOGLE_AI_API_KEY`, `ANTHROPIC_API_KEY`).
...
Method to refresh: `( cd v3 && BENCH_LLM_BASELINE=1 BENCH_ANTHROPIC=1 node ../plugins/swarmdo-cost-tracker/scripts/bench.mjs )`.
...
```bash
cd plugins/swarmdo-cost-tracker
