...
// hoist `@swarmdo/integration` into `v3/node_modules/`, so resolving
...
//      (script lives at <swarmdo>/plugins/swarmdo-cost-tracker/scripts/, the
//       integration package at <swarmdo>/v3/@swarmdo/integration/)
//
...
function tryResolveFrom(anchorPath) {
  try {
    ...
    return req.resolve('@swarmdo/integration/token-optimizer');
  } catch {
    return null;
  }
}
...
function tryAbsolutePath() {
  // Walk up from this script looking for `v3/@swarmdo/integration/dist/token-optimizer.js`.
  // Handles the marketplace layout (script at plugins/swarmdo-cost-tracker/scripts/) and the
  // dev-repo layout. Caps depth at 6 to avoid runaway traversal.
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = join(dir, 'v3', '@swarmdo', 'integration', 'dist', 'token-optimizer.js');
...
async function main() {
  ...
  for (const resolved of candidates) {
    try {
      ...
      if (!existsSync(abs)) {
        lastErr = new Error(`token-optimizer.js missing at ${abs} — run \`cd v3/@swarmdo/integration && pnpm run build\` (#1930 step 1)`);
        continue;
      }
  ...
  if (!mod) {
    const reason = lastErr
      ...
      : 'Cannot find module @swarmdo/integration/token-optimizer';
    const out = {
    ...
    console.log('  - absolute path walk from this script (looking for v3/@swarmdo/integration/dist/token-optimizer.js)');
    console.log('');
    console.log('If `@swarmdo/integration` is installed but `dist/` is missing, run:');
    console.log('  cd v3/@swarmdo/integration && pnpm run build');
    return;
  }
