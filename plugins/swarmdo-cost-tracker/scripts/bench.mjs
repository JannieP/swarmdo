#!/usr/bin/env node
// Verified-corpus benchmark for swarmdo-cost-tracker.
...
//   BENCH_LLM_BASE_URL=... -- override (default: deployed swarmvocal Gemini OpenAI shim)
...
async function main() {
  ...
  try {
    ...
    console.error('  ( cd v3 && node ../plugins/swarmdo-cost-tracker/scripts/bench.mjs )');
    process.exit(2);
  }
...
async function runLlmBaseline(cases) {
  ...
  if (!apiKey) {
    // Try to pull from gcloud (deployed swarmvocal uses GOOGLE_AI_API_KEY)
    try {
