#!/usr/bin/env node
/**
 * swarmdo-hook.cjs — cross-platform Windows shim for cost-tracker's Stop hook (#2132)
 *
 ...
 *
 * On Windows, `swarmdo init` writes a `.claude/settings.json` that overrides
 * the Stop hook to invoke this shim instead — `node swarmdo-hook.cjs` —
 ...
 *
 * Usage: node swarmdo-hook.cjs [hook-args ignored]
 */
