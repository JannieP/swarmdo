...
```bash
claude --plugin-dir plugins/swarmdo-cost-tracker
```
...
|---|---|---|
| **Agent Booster bypass** (Tier 1, $0, WASM) | `hooks_route` emits `[AGENT_BOOSTER_AVAILABLE]` (CLI: `npx @swarmdo/cli@latest hooks route --task ...`) | `cost-booster-route` skill |
| **Token optimizer / compact context** | `getTokenOptimizer().getCompactContext()` from `@swarmdo/integration` (uses `agentic-flow` when present) | `cost-compact-context` skill |
| **Model-outcome feedback loop** | `hooks_model-outcome` (typed equivalent of legacy `routing-outcomes`) | `cost-optimize` skill step 8 |
| **Optimize + benchmark loop workers** | `hooks_worker-status --worker optimize / --worker benchmark` (declared by swarmdo-loop-workers) | `cost workers` command + `cost-analyst` agent |
...
## Federation budget circuit breaker pairing (swarmdo 3.6.25+)
...
- **Verification:** `bash plugins/swarmdo-cost-tracker/scripts/smoke.sh` is the contract.
...
Both follow the kebab-case `<plugin-stem>-<intent>` convention from [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md). Both are accessed via the `memory_*` tool family which routes by namespace.
...
- [`ADR-0001` — swarmdo-cost-tracker plugin contract (namespace-routing fix, federation budget pairing, smoke as contract)](./docs/adrs/0001-cost-tracker-contract.md)
...
- `swarmdo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmdo-observability` -- Token usage metrics collected via observability instrumentation
- `swarmdo-neural-trader` -- PnL tracking and cost-adjusted return calculation
- `swarmdo-federation` -- Budget circuit breaker on outbound federation_send (ADR-097)
