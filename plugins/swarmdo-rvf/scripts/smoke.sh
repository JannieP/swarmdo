...
step "6. RVF cross-reference to swarmdo-browser sessions"
F="$ROOT/README.md"
grep -q "swarmdo-browser" "$F" \
  && grep -qE "session.*RVF|RVF.*session" "$F" \
  && ok || bad "browser-RVF cross-reference missing"

step "7. RVF tooling cross-reference to swarmdo-swarmvector"
F="$ROOT/README.md"
grep -q "swarmdo-swarmvector" "$F" \
  && grep -qE "swarmvector rvf|rvf tooling|RVF tooling" "$F" \
  && ok || bad "swarmvector RVF tooling cross-reference missing"
