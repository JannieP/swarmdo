...
- **Encryption at rest** (opt-in via `RUFFLO_ENCRYPT_AT_REST=1`) — AES-256-GCM with magic-byte (`RFE1`) backward-compat sniff. Reports involving memory.db / sessions / terminal-history exfiltration should account for the gate state (`rufflo doctor -c encryption`).
