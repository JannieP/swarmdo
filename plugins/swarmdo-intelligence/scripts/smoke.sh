#!/usr/bin/env bash
# Structural smoke test for swarmdo-intelligence v0.3.0.
...
miss=""
for t in swarmllm_sona_create swarmllm_sona_adapt swarmllm_microlora_create swarmllm_microlora_adapt; do
  grep -rq "$t" "$ROOT" --include='*.md' || miss="$miss $t"
done
[[ -z "$miss" ]] && ok || bad "undocumented:$miss"
...
# 10. Pluralization gotcha referenced (defers to swarmdo-agentdb)
...
step "13. Compatibility section pins @swarmdo/cli to v3.6"
if grep -qE "@swarmdo/cli.*v3\.6|v3\.6.*swarmdo/cli" "$ROOT/README.md"; then
  ok
else
