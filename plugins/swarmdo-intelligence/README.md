# swarmdo-intelligence

User-facing surface for Swarmdo's self-learning system. Wraps **29 intelligence-related MCP tools** across four families into discoverable skills, commands, and the canonical 4-step pipeline (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE). Coordinates with `swarmdo-agentdb` (namespace convention), `swarmdo-swarmvector` (trajectory recording substrate), and `swarmdo-browser` (consumes trajectory hooks for session replay).

> **Status:** ADR-0001 implemented. Plugin v0.3.0 targets `@swarmdo/cli` v3.6.x.
...
- **Verification:** `bash plugins/swarmdo-intelligence/scripts/smoke.sh` is the contract.

## Tool inventory

| Family | Count | Source |
|--------|------:|--------|
| `neural_*` | 6 | `v3/@swarmdo/cli/src/mcp-tools/neural-tools.ts:195, 312, 413, 539, 651, 706` |
| `hooks_intelligence_*` (incl. dispatcher + reset) | 10 | `v3/@swarmdo/cli/src/mcp-tools/hooks-tools.ts:2093, 2226, 2296, 2355, 2404, 2556, 2634, 2741, 2952, 3027` |
...
| `swarmllm_sona_*` + `swarmllm_microlora_*` | 4 | `v3/@swarmdo/cli/src/mcp-tools/swarmllm-tools.ts:142, 169, 192, 222` |
| **Total** | **29** | — |
...
| **DISTILL** | Extract the key learnings via LoRA / SONA adaptation | `swarmllm_sona_adapt`, `swarmllm_microlora_adapt`, `neural_train`, `hooks_intelligence_learn` |
| **CONSOLIDATE** | Prevent catastrophic forgetting via EWC++ | `agentdb_consolidate`, `swarmllm_microlora_adapt --consolidate`, `neural_compress` |
...
| Trajectory hooks (swarmvector substrate) | `intelligence_trajectory-*` | Recorded by `swarmdo-swarmvector`; consumed by this plugin's pattern-store |

> **Pluralization gotcha:** ReasoningBank fallback writes to `pattern` (singular). The `pretrain` hook writes to `patterns` (plural). They are *different* namespaces. See `swarmdo-agentdb` ADR-0001 §"Namespace convention" for the canonical contract.

## Namespace coordination with swarmdo-agentdb

This plugin defers to [swarmdo-agentdb ADR-0001](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md) for namespace conventions. Three reserved namespaces are read by the intelligence pipeline:
...
3. **For SONA / MicroLoRA adapters specifically**, call `swarmllm_microlora_adapt` with the `--consolidate` flag to apply Elastic Weight Consolidation on the adapter's weight deltas. This prevents catastrophic forgetting when the adapter is trained on a new domain.
...
- [`ADR-0001` — Optimize swarmdo-intelligence (surface completeness, 4-step pipeline, IPFS transfer, namespace coordination)](./docs/adrs/0001-intelligence-surface-completeness.md)

## Related Plugins

- `swarmdo-agentdb` — substrate for HNSW + namespace contract; `agentdb_pattern-*` is this plugin's storage backend
- `swarmdo-swarmvector` — trajectory hooks substrate; `intelligence_trajectory-*` calls land in swarmvector's persisted trajectories
- `swarmdo-browser` — consumes trajectory hooks for session replay (ADR-0001 there)
- `swarmdo-daa` — Dynamic Agentic Architecture; cognitive patterns feed routing as inputs
