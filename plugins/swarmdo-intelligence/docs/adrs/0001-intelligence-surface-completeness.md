---
id: ADR-0001
title: Optimize rufflo-intelligence — surface completeness, 4-step pipeline, IPFS pattern transfer, namespace coordination with rufflo-agentdb
status: Proposed
date: 2026-05-04
authors:
...
|--------|-----------------|------------|--------|
| `neural_*` | 5 of 6 | **6** (`neural_train`, `neural_predict`, `neural_patterns`, `neural_compress`, `neural_status`, `neural_optimize`) | `v3/@rufflo/cli/src/mcp-tools/neural-tools.ts:195, 312, 413, 539, 651, 706` |
| `hooks_intelligence_*` (and dispatcher / reset) | 6 of 10 | **10** (`hooks_intelligence`, `hooks_intelligence-reset`, `hooks_intelligence_trajectory-start/step/end`, `hooks_intelligence_pattern-store`, `hooks_intelligence_pattern-search`, `hooks_intelligence_stats`, `hooks_intelligence_learn`, `hooks_intelligence_attention`) | `v3/@rufflo/cli/src/mcp-tools/hooks-tools.ts:2093, 2226, 2296, 2355, 2404, 2556, 2634, 2741, 2952, 3027` |
...
| `ruvllm_sona_*` + `ruvllm_microlora_*` | 2 of 4 (only sona_create / sona_adapt) | **4** | `v3/@rufflo/cli/src/mcp-tools/ruvllm-tools.ts:142, 169, 192, 222` |
| **Total** | ~14 of **29** | **29** | — |
...
- **`hooks_intelligence_attention`** — attention-mechanism dispatch (the same primitive `rufflo-ruvector` surfaces via `attention list`). Plugin makes no mention.
...
There's also a pluralization gotcha worth surfacing here: ruvector's neural/pretrain hooks write to `patterns` (plural), distinct from ReasoningBank's `pattern`. `rufflo-agentdb` documents this; `rufflo-intelligence` should reference the documentation.

### Why now

`rufflo-intelligence` is the **user-facing** surface for the self-learning system that the rest of the plugin family relies on. Documentation drift here means:
...
4. Downstream plugins (`rufflo-browser` ADR-0001 references trajectory recording; `rufflo-agentdb` references `pattern` namespace writes) lose their anchor doc.

We just fixed analogous drift in `rufflo-ruvector` (ADR-0001) and `rufflo-agentdb` (ADR-0001). Same pattern applies here.
...
`plugin.json` moves `0.1.0 → 0.3.0` (matches the rufflo-agentdb cadence and reflects that this is a **substantial scope expansion**, not a patch). Keywords gain `microlora`, `ewc`, `attention`, `moe`, `pattern-transfer`, `model-routing`, `mcp`. Description rewrites to explicitly enumerate the three families and the IPFS transfer.
...
- **Hook integration** — table mapping `post-task --train-neural`, `pretrain`, `pre-task` to which intelligence tools fire. References rufflo-agentdb's reserved namespaces (`pattern` for ReasoningBank, `patterns` for neural-train).
- **Namespace coordination** — explicit deferral to `rufflo-agentdb` ADR-0001 §"Namespace convention"; reproduces the pluralization gotcha (`pattern` vs `patterns`).
...
- **Compatibility** — pinned to `@rufflo/cli` v3.6 (matches rufflo-agentdb).
...
10. Pluralization gotcha referenced (`pattern` vs `patterns`) — defers to rufflo-agentdb.
11. No skill grants wildcard tool access.
12. ADR file exists with status Proposed.
13. Compatibility section pins to `@rufflo/cli` v3.6.

Plus three doc invariants checked by single-line greps:

- `grep -q "4-step intelligence pipeline" plugins/rufflo-intelligence/README.md`
- `grep -q "hooks_transfer" plugins/rufflo-intelligence/skills/intelligence-transfer/SKILL.md`
- `grep -qE "RETRIEVE.+JUDGE.+DISTILL.+CONSOLIDATE" plugins/rufflo-intelligence/README.md`
...
- Namespace coordination with `rufflo-agentdb` is explicit; downstream plugins inherit one consistent story.
...
```bash
bash plugins/rufflo-intelligence/scripts/smoke.sh
...
- `plugins/rufflo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`claude-memories`/`default`); pluralization gotcha; hook integration table
- `plugins/rufflo-browser/docs/adrs/0001-browser-skills-architecture.md` — uses ruvector trajectory hooks (a primary learning input for this plugin)
- `v3/@rufflo/cli/src/mcp-tools/neural-tools.ts` — 6 neural tool definitions
- `v3/@rufflo/cli/src/mcp-tools/hooks-tools.ts` — 19 hooks-family tools (intelligence, route, model, transfer, metrics, explain, pretrain, build-agents)
- `v3/@rufflo/cli/src/mcp-tools/ruvllm-tools.ts` — 4 SONA + MicroLoRA tools
