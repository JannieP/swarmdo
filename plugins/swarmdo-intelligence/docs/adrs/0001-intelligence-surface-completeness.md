---
id: ADR-0001
title: Optimize swarmdo-intelligence — surface completeness, 4-step pipeline, IPFS pattern transfer, namespace coordination with swarmdo-agentdb
status: Proposed
date: 2026-05-04
authors:
...
- `agents/intelligence-specialist.md:15-19` — names four tool prefixes (`neural_*`, `hooks_intelligence_*`, `hooks_route`/`hooks_model-route`, `swarmllm_sona_*`)
...
|--------|-----------------|------------|--------|
| `neural_*` | 5 of 6 | **6** (`neural_train`, `neural_predict`, `neural_patterns`, `neural_compress`, `neural_status`, `neural_optimize`) | `v3/@swarmdo/cli/src/mcp-tools/neural-tools.ts:195, 312, 413, 539, 651, 706` |
| `hooks_intelligence_*` (and dispatcher / reset) | 6 of 10 | **10** (`hooks_intelligence`, `hooks_intelligence-reset`, `hooks_intelligence_trajectory-start/step/end`, `hooks_intelligence_pattern-store`, `hooks_intelligence_pattern-search`, `hooks_intelligence_stats`, `hooks_intelligence_learn`, `hooks_intelligence_attention`) | `v3/@swarmdo/cli/src/mcp-tools/hooks-tools.ts:2093, 2226, 2296, 2355, 2404, 2556, 2634, 2741, 2952, 3027` |
...
| `swarmllm_sona_*` + `swarmllm_microlora_*` | 2 of 4 (only sona_create / sona_adapt) | **4** | `v3/@swarmdo/cli/src/mcp-tools/swarmllm-tools.ts:142, 169, 192, 222` |
| **Total** | ~14 of **29** | **29** | — |
...
- **`hooks_intelligence_attention`** — attention-mechanism dispatch (the same primitive `swarmdo-swarmvector` surfaces via `attention list`). Plugin makes no mention.
...
- **`swarmllm_microlora_create` / `swarmllm_microlora_adapt`** — MicroLoRA adaptation. Plugin only mentions SONA.
- **`neural_compress`** — neural-pattern compression for storage efficiency.

### What's claimed but unsurfaced

`README.md:20` claims **"EWC++ consolidation"** as a feature. The plugin nowhere says how to invoke it. The actual EWC++ semantic lives in `agentdb_consolidate` and `swarmllm_microlora_adapt --consolidate` (per the swarmvector adaptive embedder). The plugin should map the feature claim to the tool that delivers it.
...
`swarmdo-agentdb` ADR-0001 (just landed) introduced a namespace convention with three reserved namespaces: `pattern` (ReasoningBank fallback), `claude-memories` (Claude Code bridge), `default`. **The intelligence pipeline writes to `pattern`** (via `agentdb_pattern-store` and the `hooks post-task --train-neural` path). The plugin should explicitly cite the convention so consumers don't reinvent it.

There's also a pluralization gotcha worth surfacing here: swarmvector's neural/pretrain hooks write to `patterns` (plural), distinct from ReasoningBank's `pattern`. `swarmdo-agentdb` documents this; `swarmdo-intelligence` should reference the documentation.

### Why now

`swarmdo-intelligence` is the **user-facing** surface for the self-learning system that the rest of the plugin family relies on. Documentation drift here means:
...
4. Downstream plugins (`swarmdo-browser` ADR-0001 references trajectory recording; `swarmdo-agentdb` references `pattern` namespace writes) lose their anchor doc.

We just fixed analogous drift in `swarmdo-swarmvector` (ADR-0001) and `swarmdo-agentdb` (ADR-0001). Same pattern applies here.
...
`plugin.json` moves `0.1.0 → 0.3.0` (matches the swarmdo-agentdb cadence and reflects that this is a **substantial scope expansion**, not a patch). Keywords gain `microlora`, `ewc`, `attention`, `moe`, `pattern-transfer`, `model-routing`, `mcp`. Description rewrites to explicitly enumerate the three families and the IPFS transfer.
...
- **Hook integration** — table mapping `post-task --train-neural`, `pretrain`, `pre-task` to which intelligence tools fire. References swarmdo-agentdb's reserved namespaces (`pattern` for ReasoningBank, `patterns` for neural-train).
- **Namespace coordination** — explicit deferral to `swarmdo-agentdb` ADR-0001 §"Namespace convention"; reproduces the pluralization gotcha (`pattern` vs `patterns`).
- **EWC++ explanation** — no longer just a feature claim. Maps to `agentdb_consolidate` and `swarmllm_microlora_adapt --consolidate`.
- **MoE explanation** — `hooks_intelligence` modes (`balanced` / `sona` / `moe` / `hnsw`) enumerated.
- **Compatibility** — pinned to `@swarmdo/cli` v3.6 (matches swarmdo-agentdb).
...
- `neural-train/SKILL.md` — adds **MicroLoRA** section (`swarmllm_microlora_create` / `_adapt`) with EWC++ consolidation. Adds `neural_compress` for pattern compaction.
...
10. Pluralization gotcha referenced (`pattern` vs `patterns`) — defers to swarmdo-agentdb.
11. No skill grants wildcard tool access.
12. ADR file exists with status Proposed.
13. Compatibility section pins to `@swarmdo/cli` v3.6.

Plus three doc invariants checked by single-line greps:

- `grep -q "4-step intelligence pipeline" plugins/swarmdo-intelligence/README.md`
- `grep -q "hooks_transfer" plugins/swarmdo-intelligence/skills/intelligence-transfer/SKILL.md`
- `grep -qE "RETRIEVE.+JUDGE.+DISTILL.+CONSOLIDATE" plugins/swarmdo-intelligence/README.md`
...
- Namespace coordination with `swarmdo-agentdb` is explicit; downstream plugins inherit one consistent story.
...
```bash
bash plugins/swarmdo-intelligence/scripts/smoke.sh
...
- `plugins/swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`claude-memories`/`default`); pluralization gotcha; hook integration table
- `plugins/swarmdo-browser/docs/adrs/0001-browser-skills-architecture.md` — uses swarmvector trajectory hooks (a primary learning input for this plugin)
- `v3/@swarmdo/cli/src/mcp-tools/neural-tools.ts` — 6 neural tool definitions
- `v3/@swarmdo/cli/src/mcp-tools/hooks-tools.ts` — 19 hooks-family tools (intelligence, route, model, transfer, metrics, explain, pretrain, build-agents)
- `v3/@swarmdo/cli/src/mcp-tools/swarmllm-tools.ts` — 4 SONA + MicroLoRA tools
