...
allowed-tools: Bash(npx * npm *) mcp__swarmdo__memory_store Read
---
Check dependencies for CVEs and outdated packages:

```bash
npx @swarmdo/cli@latest security cve --check
npx @swarmdo/cli@latest security audit --include-dev
npm audit --json
```
...
Auto-fix: `npx @swarmdo/cli@latest security cve --fix`

For continuous monitoring, dispatch via MCP:
`mcp__swarmdo__hooks_worker-dispatch({ trigger: "audit" })`
