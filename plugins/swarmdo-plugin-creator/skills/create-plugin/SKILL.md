...
allowed-tools: mcp__swarmdo__transfer_plugin-info mcp__swarmdo__transfer_plugin-search mcp__swarmdo__transfer_store-search Bash Read Write Edit
---
...
2. **Check for conflicts** — call `mcp__swarmdo__transfer_plugin-search` to ensure the name isn't taken
...
5. **Generate SKILL.md files** with proper frontmatter:
   ...
   allowed-tools: mcp__swarmdo__tool1 mcp__swarmdo__tool2 Bash
   ---
   ```
...
8. **Generate README.md** with install instructions, features, commands, skills, AND the canonical plugin-contract sections:
   - **Compatibility** — pin to `@swarmdo/cli` v3.6 major+minor
   - **Namespace coordination** — claim a kebab-case `<plugin-stem>-<intent>` namespace; defer to swarmdo-agentdb ADR-0001 §"Namespace convention"
...
11. **Update marketplace.json** if adding to the swarmdo marketplace.
...
Browse available tools: `mcp__swarmdo__transfer_plugin-info`
