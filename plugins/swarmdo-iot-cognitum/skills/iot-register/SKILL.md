...
allowed-tools: Bash(npx *) mcp__swarmdo__memory_store Read
...
2. `npx -y -p @swarmdo/plugin-iot-cognitum@latest cognitum-iot register ENDPOINT`
3. If pairing token provided: `npx -y -p @swarmdo/plugin-iot-cognitum@latest cognitum-iot pair DEVICE_ID`
4. Show device status: `npx -y -p @swarmdo/plugin-iot-cognitum@latest cognitum-iot status DEVICE_ID`

Store registration event:
`mcp__swarmdo__memory_store({ key: "iot-register-DEVICEID", value: "Registered at ENDPOINT", namespace: "iot-devices" })`
