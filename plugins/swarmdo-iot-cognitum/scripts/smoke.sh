...
step "8. README pins @swarmdo/cli to v3.6"
grep -qE "@swarmdo/cli.*v3\.6|v3\.6.*swarmdo/cli" "$ROOT/README.md" \
  && ok || bad "v3.6 pin missing"

step "9. README defers to swarmdo-agentdb namespace convention"
grep -q "swarmdo-agentdb" "$ROOT/README.md" \
  && grep -qE "Namespace convention|namespace convention" "$ROOT/README.md" \
  && ok || bad "namespace coordination block incomplete"

step "10. swarmdo-federation trust-model cross-reference present"
F="$ROOT/README.md"
grep -q "swarmdo-federation" "$F" \
