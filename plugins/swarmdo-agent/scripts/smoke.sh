...
step "1. plugin.json is swarmdo-agent 0.2.0 with both-runtime keywords"
P="$ROOT/.claude-plugin/plugin.json"
v=$(grep -E '"version"' "$P" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
n=$(grep -E '"name"' "$P" | grep -oE 'swarmdo-agent' | head -1)
if [[ "$v" != "0.2.0" ]]; then bad "expected version 0.2.0, got '$v'";
elif [[ "$n" != "swarmdo-agent" ]]; then bad "expected name swarmdo-agent";
else
  miss=""
  # local WASM runtime (rvagent) + cloud runtime (managed-agents) keywords
  for k in mcp rvagent-wasm swarmllm-wasm managed-agents anthropic; do
...
step "4. README pins @swarmdo/cli to v3.6"
grep -qE "@swarmdo/cli.*v3\.6|v3\.6.*swarmdo/cli" "$ROOT/README.md" \
  && ok || bad "v3.6 pin missing"

step "5. README defers to swarmdo-agentdb namespace convention"
...
step "7. ADR-070 cross-reference present (rvagent-wasm + swarmllm-wasm integration)"
F="$ROOT/README.md"
miss=""
grep -q "ADR-070" "$F" || miss="$miss adr-ref"
grep -q "rvagent-wasm" "$F" || miss="$miss rvagent-wasm"
grep -q "swarmllm-wasm" "$F" || miss="$miss swarmllm-wasm"
