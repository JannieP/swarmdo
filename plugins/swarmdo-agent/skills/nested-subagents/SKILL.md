...
| Swarmdo default (`pre-task` hook) | 4 levels — one-level guard band, configurable in `swarmdo.config.json` |
| Strict-mode env var | `SWARMDO_STRICT_NESTING=true` to enforce the swarmdo cap |
...
- **Maps cleanly onto swarmdo's existing orchestrators** — `swarmdo-sparc:sparc-orchestrator` (5 phases ≈ 5 levels), `swarmdo-goals:dossier-investigator` (recursive entity expansion), `v3-queen-coordinator` (hierarchical-mesh top).
...
- **Agent**: `swarmdo-agent:nested-coordinator` — the orchestrator
