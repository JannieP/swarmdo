...
```bash
claude --plugin-dir plugins/swarmdo-rag-memory
```
...
The AgentDB SQLite blob written by this plugin (`.swarm/memory.db`) supports opt-in AES-256-GCM encryption at rest per [ADR-096](../../v3/docs/adr/ADR-096-encryption-at-rest.md). When `SWARMDO_ENCRYPT_AT_REST=1` and `SWARMDO_ENCRYPTION_KEY` is set:
...
Verify gate state with `swarmdo doctor -c encryption`. Off by default; flipping it on doesn't require a migration step (legacy plaintext bytes are sniffed on read; first write after enable rewrites the DB encrypted).
...
```bash
# CLI
npx @swarmdo/cli@latest memory search --query "auth patterns" --smart --limit 10

# MCP
mcp__swarmdo__memory_search({ query: "auth patterns", smart: true, limit: 10 })
```
...
# Via CLI:
npx @swarmdo/cli@latest memory search --query "auth security" --limit 5
```
...
## Integration with swarmvector

When `swarmdo-swarmvector` is also loaded, rag-memory delegates to swarmvector's backend for advanced features:
...
- **Verification:** `bash plugins/swarmdo-rag-memory/scripts/smoke.sh` is the contract.

## Namespace coordination — claude-memories consumer

This plugin is the **canonical user-facing consumer** of the `claude-memories` reserved namespace defined in [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md). The auto-import flow:

```
Claude Code SessionStart hook
  → memory_import_claude (MCP)
  → claude-memories namespace (reserved, swarmdo-agentdb owned)
...
- [`ADR-0001` — swarmdo-rag-memory plugin contract (claude-memories reserved-namespace consumer, smoke as contract)](./docs/adrs/0001-rag-memory-contract.md)

## Related Plugins

- `swarmdo-agentdb` — Full AgentDB controller bridge (15 `agentdb_*` MCP tools); namespace convention owner; owns the `claude-memories` reserved namespace
- `swarmdo-swarmvector` — Advanced vector operations (FlashAttention-3, Graph RAG, hybrid search)
- `swarmdo-rvf` — Portable RVF memory format for cross-machine export/import
- `swarmdo-knowledge-graph` — Entity extraction and graph traversal over memory
