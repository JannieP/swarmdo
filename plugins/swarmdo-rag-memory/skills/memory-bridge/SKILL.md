...
allowed-tools: Bash Read mcp__swarmdo__memory_import_claude mcp__swarmdo__memory_bridge_status mcp__swarmdo__memory_search_unified
---
...
1. **Check bridge health**:
   `mcp__swarmdo__memory_bridge_status({})`
   Verify: Claude files count, AgentDB entries, SONA state, connection status.

2. **Import memories**:
   - Current project: `mcp__swarmdo__memory_import_claude({})`
   - All projects: `mcp__swarmdo__memory_import_claude({ allProjects: true })`
...
5. **Test unified search**:
   `mcp__swarmdo__memory_search_unified({ query: "test query", limit: 3 })`
...
When `swarmdo-swarmvector` is loaded, bridged memories are also indexed by swarmvector for:
