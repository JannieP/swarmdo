...
description: SOTA RAG memory specialist — hybrid search (sparse+dense), Graph RAG multi-hop retrieval, MMR diversity reranking, smart consolidation, swarmvector integration
model: sonnet
---
...
### Retrieval via swarmvector (when available)

```bash
# Hybrid search (sparse + dense)
npx swarmvector search "query" --hybrid --limit 10

# Graph RAG (multi-hop)
npx swarmvector search "query" --graph-rag --limit 10

# Brain knowledge search
npx swarmvector brain search "query"

# RAG context retrieval (MCP)
# hooks_rag_context({ query: "topic", limit: 5 })
```

### Retrieval via swarmdo CLI

```bash
# Dense semantic search
npx @swarmdo/cli@latest memory search --query "QUERY" --namespace NAMESPACE --limit 10

# Store with metadata
npx @swarmdo/cli@latest memory store --key "KEY" --value "VALUE" --namespace NAMESPACE

# List and audit
npx @swarmdo/cli@latest memory list --namespace NAMESPACE --limit 20

# Consolidated search across all namespaces
npx @swarmdo/cli@latest memory search --query "QUERY" --limit 10
```
...
- **swarmdo-agentdb**: Full AgentDB backend with HNSW vector_indexes table
- **swarmdo-swarmvector**: FlashAttention-3, Graph RAG, hybrid search, DiskANN
- **swarmdo-rvf**: Portable RVF format for cross-machine memory export/import
- **swarmdo-knowledge-graph**: Entity-relationship graphs over memory entries
- **swarmdo-intelligence**: SONA trajectory learning from retrieval patterns
