...
```bash
npx @swarmdo/cli@latest memory search --query "$ARGUMENTS" --limit 5
```

For richer results when swarmvector is available:
```bash
npx swarmvector search "$ARGUMENTS" --hybrid --limit 5
```
...
```bash
npx @swarmdo/cli@latest memory list --limit 10
```
