...
Competitive ruliology for Swarmdo swarms — **arenas**, **tournaments**, and **adaptive
co-evolution** of program strategies. Implements the first executable slice of Swarmdo
...
## MCP tools (`swarmdo-arena/mcp-tools`)
...
Full run artifacts are written to `.swarmdo/arena/<runId>.json` (exact replay). Each tool result
also carries an `agentdb` payload so the command layer can store a searchable summary via
`mcp__swarmdo__memory_store` (namespace `arena`) — the local stand-in for the SwarmVector data
layer (ADR-196/197), enabling queries like *"tournaments where grim dominated"*.

## Scope

v1 is intentionally Swarmdo-only and core-untouched:
...
- **Out (tracked elsewhere):** LLM-agent strategies + distillation (ADR-151), execution
  sandboxing/resource governance (ADR-153), dashboard UI (ADR-149/154), and the full SwarmVector
