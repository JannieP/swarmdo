...
The MCP tools persist full artifacts to `.swarmdo/arena/<runId>.json` (exact replay). Each tool
result also includes an `agentdb` payload — use it to store a searchable summary in AgentDB so
runs are queryable later (the local stand-in for the SwarmVector data layer, ADR-196/197):
...
Later: `mcp__swarmdo__memory_search({ namespace: "arena", query: "tournaments where grim dominated" })`.
