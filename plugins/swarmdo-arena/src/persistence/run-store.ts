// Run persistence. A competition run is a reproducible artifact (Swarmdo ADR-147; the local
// stand-in for SwarmVector ADR-197's payoff/fitness/evolution storage).
//
// Two backends ship here:
//   - FileRunStore     — writes full artifacts to `.swarmdo/arena/<runId>.json` (exact replay).
//   - InMemoryRunStore — for tests / ephemeral use.
// AgentDB persistence is handled at the COMMAND layer (Swarmdo convention: plugin TS persists
// artifacts; commands call `mcp__swarmdo__memory_store` with `agentdbRecord(run)`).
...
export class FileRunStore implements RunStore {
  constructor(private readonly baseDir = join(process.cwd(), '.swarmdo', 'arena')) {}
...
/**
 * Shape a run for AgentDB persistence via `mcp__swarmdo__memory_store`.
 * The command layer calls the MCP tool with this payload so runs become semantically
 * searchable ("tournaments where grim dominated") and feed the SwarmVector data layer later.
 *
 * `kind`, `game`, and `seed` are lifted to top-level fields (parallel to `tags`)
 * so SwarmVector ADR-197 indexers can filter without re-parsing `value`. `value`
