# ADR-0001: swarmdo-arena plugin contract

**Status**: Proposed
**Date**: 2026-06-07
**Plugin**: swarmdo-arena

## Context

`swarmdo-arena` implements the first executable slice of the competitive-ruliology initiative
(Swarmdo ADR-147 competitive arena/tournament modes, ADR-148 co-evolution, ADR-150 visualization
...
the SwarmVector data layer) can depend on stable shapes.
...
Every tool returns the Swarmdo envelope `{ success: boolean; result?: T; error?: { message } }`
...
- TS code persists full artifacts via `RunStore` (`FileRunStore` → `.swarmdo/arena/<runId>.json`;
  `InMemoryRunStore` for tests). This is the local stand-in for SwarmVector ADR-197.
- AgentDB persistence is performed at the **command/agent layer** (Swarmdo convention): each tool
  result carries an `agentdb` payload `{ namespace:"arena", key, value, tags }` for
  `mcp__swarmdo__memory_store`, making runs semantically searchable.
...
- **Positive**: stable, typed contract; zero runtime deps beyond Zod; engine is pure and
  unit-tested; no Swarmdo-core changes; clean seam for the SwarmVector data layer and the dashboard.
...
- **Neutral / Risks**: the AgentDB summary schema (`agentdb.value`) is unversioned in v1;
  formalize with the SwarmVector integration contract (ADR-152 ↔ ADR-200) before cross-repo use.
...
- Swarmdo ADR-147 (arena/tournament), ADR-148 (co-evolution), ADR-149 (dashboard), ADR-150 (viz split)
- SwarmVector ADR-196 (strategy graphs), ADR-197 (payoff/fitness storage)
