...
[[ -f "$ROOT/commands/swarmdo-loop.md" ]] || miss="$miss missing-loop-cmd"
[[ -f "$ROOT/commands/swarmdo-schedule.md" ]] || miss="$miss missing-schedule-cmd"
[[ -z "$miss" ]] && ok || bad "$miss"
...
step "9. swarmdo-autopilot 270s heartbeat contract cross-reference"
F="$ROOT/README.md"
grep -q "swarmdo-autopilot" "$F" \
...
grep -q "swarmdo-docs" "$F" || miss="$miss docs-attribution"
grep -q "swarmdo-testgen" "$F" || miss="$miss testgen-attribution"
[[ -z "$miss" ]] && ok || bad "$miss"
