 ...
 *
 * `swarmdo-rag-memory` ships Graph-RAG multi-hop retrieval. This adapter
...
export function ragMemoryGraphId(namespace?: string): string {
  return namespace
    ? `swarmdo-rag-memory:chunks:${namespace}`
    : 'swarmdo-rag-memory:chunks:default';
}

export class RagMemoryAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'swarmdo-rag-memory';
