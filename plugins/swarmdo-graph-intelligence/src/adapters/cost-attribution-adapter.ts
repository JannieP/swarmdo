 ...
 *
 * `swarmdo-cost-tracker` records token usage per session in a causation graph:
...
export function costAttributionGraphId(sessionId?: string): string {
  return sessionId ? `swarmdo-cost-tracker:causation:${sessionId}` : 'swarmdo-cost-tracker:causation:global';
}

export class CostAttributionAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'swarmdo-cost-tracker';
