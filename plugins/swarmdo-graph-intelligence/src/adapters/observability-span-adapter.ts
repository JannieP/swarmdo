 ...
 *
 * `swarmdo-observability` records spans (OpenTelemetry-style) with parent links
...
export function observabilityGraphId(traceId: string): string {
  return `swarmdo-observability:trace:${traceId}`;
}

export class ObservabilitySpanAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'swarmdo-observability';
