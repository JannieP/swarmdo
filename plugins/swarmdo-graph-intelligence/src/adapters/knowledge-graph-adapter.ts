 ...
 *
 * `swarmdo-knowledge-graph` builds an entity-relation graph via kg-extract.
...
export const KNOWLEDGE_GRAPH_ID = 'swarmdo-knowledge-graph:entities';

export class KnowledgeGraphAdapter implements SublinearAdapter {
  readonly graphId = KNOWLEDGE_GRAPH_ID;
  readonly ownerPlugin = 'swarmdo-knowledge-graph';
...
export class GraphEdgesSource implements KnowledgeGraphSource {
  ...
  async listEdges(): Promise<readonly KGEdge[]> {
    try {
      // Lazy import to avoid hard-coupling the plugin to @swarmdo/cli at compile time.
      // The import paths are resolved at runtime only; TypeScript cannot type-check them
      // from this plugin's compilation context (no package-level dependency on @swarmdo/cli).
      ...
      const mod: GraphEdgeWriterModule = await import('@swarmdo/cli/src/memory/graph-edge-writer.js')
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore — fallback to local dist path in mono-repo context
        .catch(() => import('../../../../../v3/@swarmdo/cli/dist/src/memory/graph-edge-writer.js'));
