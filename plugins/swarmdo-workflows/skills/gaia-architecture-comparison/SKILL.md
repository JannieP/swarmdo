...
description: Side-by-side comparison of swarmdo vs HAL vs other GAIA harnesses — capability gaps, design decisions, and improvement roadmap
argument-hint: "[--focus=tools|routing|memory|cost]"
allowed-tools: Bash Read mcp__swarmdo__memory_search mcp__swarmdo__memory_store
---

# GAIA Architecture Comparison Skill

Compare swarmdo's GAIA benchmark harness against the Princeton HAL reference
...
| Dimension | swarmdo | HAL reference | Gap |
|-----------|-------|--------------|-----|
...
| Self-consistency | voting.ts (Track A) | Not in reference | swarmdo advantage |
| Hardness routing | predictor.ts (Track Q) | Not in reference | swarmdo advantage |
| Memory | AgentDB HNSW | None | swarmdo advantage |
...
```bash
npx @swarmdo/cli@latest memory search \
