...
allowed-tools: mcp__swarmdo__workflow_execute mcp__swarmdo__workflow_run mcp__swarmdo__workflow_pause mcp__swarmdo__workflow_resume mcp__swarmdo__workflow_cancel mcp__swarmdo__workflow_status Workflow Read Bash
---
...
1. **Execute** — call `mcp__swarmdo__workflow_execute` or `mcp__swarmdo__workflow_run` with the workflow ID
2. **Monitor** — call `mcp__swarmdo__workflow_status` to check progress and step outcomes
3. **Pause** — call `mcp__swarmdo__workflow_pause` to halt at the current step
4. **Resume** — call `mcp__swarmdo__workflow_resume` to continue from where paused
5. **Cancel** — call `mcp__swarmdo__workflow_cancel` to abort the workflow
