...
allowed-tools: Bash Read mcp__swarmdo__memory_search mcp__swarmdo__memory_store mcp__swarmdo__agentdb_pattern_search mcp__swarmdo__agentdb_pattern_store
---
...
RESULTS=~/.cache/swarmdo/gaia/results-latest.json
node -e "
...
```bash
node v3/@swarmdo/cli/bin/cli.js gaia-bench run \
  --level 1 --limit 1 \
  --task-id $TASK_ID \
...
```bash
node -e "
  const { createDefaultToolCatalogue } = require('./v3/@swarmdo/cli/src/benchmarks/gaia-tools/index.js');
