...
- **Verification:** `bash plugins/swarmdo-workflows/scripts/smoke.sh` is the contract.

## MCP surface (10 tools)

All defined at `v3/@swarmdo/cli/src/mcp-tools/workflow-tools.ts`:
...
Workflow({ name: 'plugin-contract-audit', args: 'swarmdo-agentdb' })  // pass args (the script's `args` global)
...
This plugin owns the `workflows-state` AgentDB namespace (kebab-case, follows the convention from [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — swarmdo-workflows plugin contract (10-tool MCP surface, lifecycle state machine, smoke as contract)](./docs/adrs/0001-workflows-contract.md)
...
- `swarmdo-loop-workers` — sibling automation surface (loops are recurring; workflows are stateful pipelines)
- `swarmdo-sparc` — SPARC phase transitions can be modeled as workflows
