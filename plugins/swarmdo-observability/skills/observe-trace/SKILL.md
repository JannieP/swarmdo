...
allowed-tools: mcp__swarmdo__memory_search mcp__swarmdo__memory_list mcp__swarmdo__agentdb_semantic-route mcp__swarmdo__agentdb_context-synthesize mcp__swarmdo__agentdb_pattern-search Bash
---
...
1. **Collect spans** -- call `mcp__swarmdo__memory_search --namespace observability` (or `memory_list`) to retrieve all spans matching the `<task-id>`. The `memory_*` tool family routes by namespace; `agentdb_hierarchical-*` does NOT (it routes by tier `working|episodic|semantic`), so use `memory_*` here. See [swarmdo-agentdb ADR-0001 §"Namespace convention"](../../../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md).
...
5. **Synthesize** -- call `mcp__swarmdo__agentdb_context-synthesize` to combine span metadata into a narrative summary of the execution flow
...
```bash
npx @swarmdo/cli@latest memory search --query "trace spans for task TASK_ID" --namespace observability
```
