...
```bash
claude --plugin-dir plugins/swarmdo-observability
```
...
- **Verification:** `bash plugins/swarmdo-observability/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `observability` AgentDB namespace (base-name exception per [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md), same precedent as `federation` and `migrations`). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmdo-observability/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmdo-observability plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-observability-contract.md)

## Related Plugins

- `swarmdo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmdo-cost-tracker` -- Token usage metrics feed into cost attribution
- `swarmdo-iot-cognitum` -- Reuses Z-score anomaly detection for telemetry patterns
- `swarmdo-market-data` -- Data feed health and ingestion latency monitoring
