...
- `mcp__swarmdo__agentdb_hierarchical-store` -- store trace spans and log entries
- `mcp__swarmdo__agentdb_hierarchical-recall` -- recall traces by traceId or correlationId
- `mcp__swarmdo__agentdb_pattern-store` -- store anomaly patterns for future detection
- `mcp__swarmdo__agentdb_pattern-search` -- search for similar anomaly patterns
- `mcp__swarmdo__agentdb_semantic-route` -- route observability queries to relevant data
- `mcp__swarmdo__agentdb_context-synthesize` -- synthesize context from multiple trace spans
...
npx @swarmdo/cli@latest neural train --pattern-type observability --epochs 10
```
...
```bash
npx @swarmdo/cli@latest memory store --namespace observability --key "trace-TRACE_ID" --value "TRACE_SUMMARY_JSON"
npx @swarmdo/cli@latest memory store --namespace observability-patterns --key "anomaly-ANOMALY_TYPE" --value "ANOMALY_SIGNATURE_JSON"
npx @swarmdo/cli@latest memory search --query "latency spikes in authentication flow" --namespace observability
```

### Related Plugins

- **swarmdo-iot-cognitum**: Reuses Z-score anomaly detection for telemetry pattern analysis
- **swarmdo-loop-workers**: Background workers produce telemetry that this plugin correlates
- **swarmdo-swarm**: Agent swarm activity generates the traces and metrics this plugin collects
- **swarmdo-cost-tracker**: Token usage metrics feed into cost attribution and budget monitoring
