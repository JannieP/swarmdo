---
id: ADR-0001
title: rufflo-swarm plugin contract — pinning, namespace coordination, MCP surface (4 swarm_* + 8 agent_*), Monitor streaming, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`rufflo-swarm` (v0.1.0) — multi-agent swarm coordination, Monitor streams, worktree isolation. 2 agents (`coordinator`, `architect`), 2 skills (`swarm-init`, `monitor-stream`), 2 commands (`/swarm`, `/watch`).

Wraps **12 MCP tools** across two families:

| Family | Count | Source |
|--------|-------|--------|
| `swarm_*` | 4 (`init`, `status`, `shutdown`, `health`) | `v3/@rufflo/cli/src/mcp-tools/swarm-tools.ts:71, 145, 208, 270` |
| `agent_*` | 8 (`spawn`, `execute`, `terminate`, `status`, `list`, `pool`, `health`, `update`) | `v3/@rufflo/cli/src/mcp-tools/agent-tools.ts:182, 287, 319, 356, 395, 451, 573, 651` |
...
```bash
bash plugins/rufflo-swarm/scripts/smoke.sh
...
- `plugins/rufflo-autopilot/docs/adrs/0001-autopilot-contract.md` — 270s cache-aware /loop heartbeat for swarm coordination
- `plugins/rufflo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — `hooks_route` powers swarm agent recommendation
- `v3/@rufflo/cli/src/mcp-tools/swarm-tools.ts` — 4 `swarm_*` tools
- `v3/@rufflo/cli/src/mcp-tools/agent-tools.ts` — 8 `agent_*` tools

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/rufflo-swarm/`. Contract elements implemented: all 4 `swarm_*` + 8 `agent_*` MCP tools (12 total) covered; Monitor streaming (`/watch`) skill shipped; namespace `swarm-state` claimed; smoke-as-contract gate defined in `scripts/smoke.sh` (11 checks).
