...
Phase 1 enforces at the **send** side. Phase 2 (peer state machine: ACTIVE / SUSPENDED / EVICTED) and Phase 3 (`swarmdo-cost-tracker` integration for unified spend reporting) ship in follow-up releases.
...
- `@swarmdo/security` (cryptographic primitives)

## Compatibility

- **CLI:** pinned to `@swarmdo/cli` v3.6 major+minor.
- **Federation runtime:** `@swarmdo/plugin-agent-federation` (resolved via `npx -y -p`).
- **Verification:** `bash plugins/swarmdo-federation/scripts/smoke.sh` is the contract.

## Alignment with the canonical 3-gate pattern

Federation's "PII Pipeline" feature is a richer specialization of the canonical 3-gate pattern owned by [swarmdo-aidefence ADR-0001](../swarmdo-aidefence/docs/adrs/0001-aidefence-contract.md). The mapping:
...
This plugin owns the `federation` AgentDB namespace. This is the documented exception to the kebab-case `<plugin-stem>-<intent>` rule: when a plugin's name *is* the intent, the namespace can match the plugin stem. See [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — swarmdo-federation plugin contract (3-gate alignment, ADR-097 budget integration, namespace coordination, smoke as contract)](./docs/adrs/0001-federation-contract.md)
