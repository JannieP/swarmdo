...
**init**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation init`
**join**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation join ENDPOINT`
**leave**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation leave`
**peers**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation peers`
**send**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation send NODE_ID MSG_TYPE PAYLOAD [--max-hops N] [--max-tokens N] [--max-usd N] [--hop-count N] [--spent-tokens N] [--spent-usd N]`
**status**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation status`
**audit**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation audit --compliance MODE --since DATE`
**trust**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation trust NODE_ID --review`
**config**: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation config --pii-policy PATH`
