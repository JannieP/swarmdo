...
allowed-tools: Bash(npx *) mcp__swarmdo__memory_store mcp__swarmdo__hooks_post-task Read Write
...
Steps:
1. `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation init`
2. If a compliance mode is specified, configure it: `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation config --compliance MODE`
