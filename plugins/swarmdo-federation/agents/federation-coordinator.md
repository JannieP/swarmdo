...
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation init` -- generate keypair, create config
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation join <endpoint>` -- connect to peer
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation peers` -- list peers with trust levels
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation status` -- health dashboard
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation audit --compliance hipaa` -- audit logs
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation trust <node-id> --review` -- trust breakdown
- `npx -y -p @swarmdo/plugin-agent-federation@latest swarmdo-federation send <node-id> <msg-type> <payload> [--max-hops N] [--max-tokens N] [--max-usd N]` -- delegate with budget guardrails
...
```bash
npx @swarmdo/cli@latest memory store --namespace federation --key "peer-NODEID" --value "TRUST_HISTORY"
```
