# AIDefence Wiring Sketch — swarmdo-neural-trader
...
|------|-----------------------------------|-------------------------------------|---------------|
| 1    | `--symbol $TICKER` input          | `mcp__swarmdo__aidefence_is_safe` | Reject — refuse to invoke neural-trader; alert team lead |
| 2    | `fetchLiveBars` response body     | `mcp__swarmdo__aidefence_has_pii` | Redact PII placeholders; record in session manifest |
| 3    | LLM prompt body (pre-`neural_predict`) | `mcp__swarmdo__aidefence_is_safe` | Quarantine to `findings.md`; don't reach model |
| 4    | AgentDB store value               | `mcp__swarmdo__aidefence_scan`    | Block high-entropy tokens that look like leaked credentials |

These are exactly the four gates `swarmdo-browser` and `swarmdo-federation`
...
1. **`agents/market-analyst.md`** — add a "PII & injection gates" section
   mirroring `plugins/swarmdo-browser/agents/browser-agent.md:79-81`. Include
   the four-gate workflow above.
2. **`agents/market-analyst.md` allowed-tools** — add
   `mcp__swarmdo__aidefence_is_safe`,
   `mcp__swarmdo__aidefence_has_pii`,
   `mcp__swarmdo__aidefence_scan` to the frontmatter so the agent
   has the capability.
3. **`README.md`** — add a "Safety" section like
   `plugins/swarmdo-browser/README.md:74-78`.
...
- `plugins/swarmdo-federation/README.md:74-82` — existing four-gate pattern
- `plugins/swarmdo-browser/agents/browser-agent.md:79-81` — agent-prompt
  gate example
