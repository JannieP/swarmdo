# Security Audit — swarmdo-neural-trader (2026-05-20)
...
```
node scripts/audit-supply-chain.mjs 2>&1 | grep -A5 'swarmdo-neural-trader\|plugins/swarmdo-neural-trader' || echo "no plugin-specific findings"
```
...
grep -rn 'process.env\|API_KEY\|SECRET' \
  plugins/swarmdo-neural-trader/skills/ \
  plugins/swarmdo-neural-trader/src/
```
...
| src/sublinear-adapter.mjs:22, 34-35             | `SWARMDO_SUBLINEAR_NATIVE`    | DOCUMENTED — feature flag, no value |
| src/sublinear-adapter.ts:32, 118, 136-137       | `SWARMDO_SUBLINEAR_NATIVE`    | DOCUMENTED — TS mirror of the above |
...
```
grep -rn 'eval\|new Function' plugins/swarmdo-neural-trader/ | grep -v '\.md:'
```
...
```
grep -rn 'spawn\|execSync\|spawnSync' plugins/swarmdo-neural-trader/
```
...
`npx @swarmdo/cli` via documented memory-store commands. Both go
...
- AIDefence MCP tools to call: `aidefence_has_pii`, `aidefence_scan`,
  `aidefence_is_safe` (same three gates the `swarmdo-browser` and
  `swarmdo-federation` plugins already use).
...
- `plugins/swarmdo-federation/README.md` — existing AIDefence wiring (reference pattern)
- `plugins/swarmdo-browser/agents/browser-agent.md` — existing AIDefence gates (reference pattern)
