...
allowed-tools: Bash Read mcp__swarmdo__memory_retrieve mcp__swarmdo__memory_store mcp__swarmdo-sublinear__page-rank-entry
...
- The same call site picks up the full native-WASM PageRank from `mcp__swarmdo-sublinear__page-rank-entry` once that tool is registered in the runtime — until then, the local power-iteration kernel ships in `signed-attribution.mjs` and produces the same ordering (seeded mulberry32).
...
4. **Run single-entry PageRank** — preferred path when `mcp__swarmdo-sublinear__page-rank-entry` is registered:
   ```text
   mcp__swarmdo-sublinear__page-rank-entry({
     nodes: GRAPH_NODES,
     edges: GRAPH_EDGES,
     sourceIndex: 0,
     damping: 0.85,
   ...
   ```
   The local fallback (`localSingleEntryPageRank` in `plugins/swarmdo-neural-trader/src/signed-attribution.mjs`) runs ~30 LOC of seeded power-iteration when the MCP tool is not available — same math, same result up to floating-point tolerance, same ordering for the same seed (the Phase 6 smoke asserts this).
...
6. **Sign the artifact** (reuses the Phase 4 signing primitives — same Ed25519 + canonicalization):
   ...
   - Resolve the witness signing key — same lookup order as Phase 4:
     1. `SWARMDO_WITNESS_KEY_PATH` env var — JSON file with `{ "privateKey": "<hex>" }`.
     2. `verification/witness-key.json` (the ADR-103 default path).
   - If a key resolves: `signAttributionArtifact(body, privateKeyHex)` from `plugins/swarmdo-neural-trader/src/signed-attribution.mjs`.
   - If NEITHER path resolves: log `"[WARN] swarmdo-neural-trader: no witness signing key found — storing attribution artifact in UNSIGNED degraded mode. Regulator filings will reject UNSIGNED artifacts."` and store the body unsigned. NEVER silently fall back.
...
```ts
import { verifyAttributionArtifact } from 'plugins/swarmdo-neural-trader/src/signed-attribution.mjs';
...
- `plugins/swarmdo-neural-trader/src/signed-attribution.ts` (the typed contract)
- `plugins/swarmdo-neural-trader/src/signed-attribution.mjs` (the runtime mirror)
