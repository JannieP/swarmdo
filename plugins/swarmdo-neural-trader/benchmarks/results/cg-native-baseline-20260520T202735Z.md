...
probes `mcp__swarmdo-sublinear__solve` on `globalThis` and honours
`SWARMDO_SUBLINEAR_NATIVE=1` as a manual override. When the native path
...
The full 40-60× speedup headline requires the `swarmdo-sublinear` plugin
to be registered AND its MCP tool to be mounted into the adapter's
runtime via the harness — that happens on a live swarmdo daemon. CI
exercises that path.
...
mounted `mcp__swarmdo-sublinear__solve` onto `globalThis`. CI exercises
that path; this run does not.

## Wiring summary (what changed in #55)

1. `SublinearAdapter.detectSublinearTool()` — new public probe that
   checks both `globalThis['mcp__swarmdo-sublinear__solve']` and the
   `SWARMDO_SUBLINEAR_NATIVE=1` env-var override. Legacy `isMcpAvailable()`
...
6. Smoke gained contract checks for `detectSublinearTool`,
   `SWARMDO_SUBLINEAR_NATIVE`, and the new `method` / `solver` fields.
