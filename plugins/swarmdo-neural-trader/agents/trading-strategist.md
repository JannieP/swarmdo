...
```bash
npx @swarmdo/cli@latest memory store --namespace trading-strategies --key "strategy-NAME" --value "CONFIG_JSON"
npx @swarmdo/cli@latest memory search --query "momentum strategies Sharpe > 1.5" --namespace trading-strategies
```
...
```bash
npx @swarmdo/cli@latest neural train --pattern-type trading-strategy --epochs 20
npx @swarmdo/cli@latest neural predict --input "current market: high volatility, upward drift"
```

### Related Plugins

- **swarmdo-market-data**: OHLCV ingestion and candlestick pattern detection
...
Message schemas: `RegimeVerdict`, `SignalProposal`, `RiskDecision` in `plugins/swarmdo-neural-trader/src/pipeline-messages.ts`.
