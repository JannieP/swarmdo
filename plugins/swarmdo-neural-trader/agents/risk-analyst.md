...
```bash
npx @swarmdo/cli@latest memory store --namespace trading-risk --key "risk-PORTFOLIO_ID" --value "RISK_METRICS_JSON"
npx @swarmdo/cli@latest memory search --query "high correlation drawdown event" --namespace trading-risk
```

### Related Plugins

- **swarmdo-observability**: Real-time risk dashboards and alerting
- **swarmdo-cost-tracker**: PnL tracking and fee attribution
- **swarmdo-agentdb**: Historical risk event storage for pattern matching
...
Message schemas: `SignalProposal`, `RiskDecision` in `plugins/swarmdo-neural-trader/src/pipeline-messages.ts`.
