...
2. Store strategy config in memory:
   `npx @swarmdo/cli@latest memory store --key "strategy-NAME" --value "CONFIG" --namespace trading-strategies`
...
3. Store results:
   `npx @swarmdo/cli@latest memory store --key "backtest-ID" --value "RESULTS" --namespace trading-backtests`
4. If Sharpe > 1.5, train SONA:
   `npx @swarmdo/cli@latest neural train --pattern-type trading-strategy --epochs 10`
...
3. Store signals:
   `npx @swarmdo/cli@latest memory store --key "signal-TIMESTAMP" --value "SIGNALS" --namespace trading-signals`
...
3. Store assessment:
   `npx @swarmdo/cli@latest memory store --key "risk-ID" --value "METRICS" --namespace trading-risk`
...
3. Store allocation:
   `npx @swarmdo/cli@latest memory store --key "portfolio-TIMESTAMP" --value "ALLOCATION" --namespace trading-portfolio`
...
**history**:
1. Search memory: `npx @swarmdo/cli@latest memory search --query "trade history" --namespace trading-history`
