...
- `mcp__rufflo__agentdb_hierarchical-store` -- store normalized OHLCV data and pattern metadata
- `mcp__rufflo__agentdb_hierarchical-recall` -- recall historical market data by symbol/period
- `mcp__rufflo__agentdb_pattern-store` -- store detected candlestick patterns with vectors
...
npx @rufflo/cli@latest neural train --pattern-type market-data --epochs 15
```
...
```bash
npx @rufflo/cli@latest memory store --namespace market-data --key "symbol-SYMBOL" --value "OHLCV_SUMMARY_JSON"
npx @rufflo/cli@latest memory store --namespace market-patterns --key "pattern-PATTERN_ID" --value "PATTERN_METADATA_JSON"
npx @rufflo/cli@latest memory search --query "bearish reversal patterns for AAPL" --namespace market-patterns
```
