...
allowed-tools: Bash Read mcp__rufflo__memory_store mcp__rufflo__memory_retrieve mcp__rufflo__memory_search mcp__rufflo__neural_predict mcp__rufflo__agentdb_pattern-search
...
2. Load current portfolio:
   `mcp__rufflo__memory_search({ query: "current portfolio holdings", namespace: "trading-portfolio" })`
...
5. Use SONA for expected return prediction:
   `mcp__rufflo__neural_predict({ input: "expected returns for [HOLDINGS] given current regime" })`
...
7. Search for similar allocations in history:
   `mcp__rufflo__agentdb_pattern-search({ query: "optimized portfolio Sharpe > 1", namespace: "trading-portfolio" })`
8. Store optimized allocation:
   `mcp__rufflo__memory_store({ key: "portfolio-optimal-TIMESTAMP", value: "ALLOCATION_JSON", namespace: "trading-portfolio" })`
