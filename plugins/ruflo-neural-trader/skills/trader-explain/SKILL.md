...
allowed-tools: Bash Read mcp__rufflo__memory_retrieve mcp__rufflo__memory_store mcp__rufflo-sublinear__page-rank-entry
...
- The same call site picks up the full native-WASM PageRank from `mcp__rufflo-sublinear__page-rank-entry` once that tool is registered in the runtime — until then, the local power-iteration kernel ships in `signed-attribution.mjs` and produces the same ordering (seeded mulberry32).

Steps:

1. **Retrieve the signal** from the canonical `trading-signals` namespace (ADR-126 Phase 1 + Phase 2 lifecycle):
   ```text
   mcp__rufflo__memory_retrieve({
     key: "SIGNAL_ID",
...
4. **Run single-entry PageRank** — preferred path when `mcp__rufflo-sublinear__page-rank-entry` is registered:
   ```text
   mcp__rufflo-sublinear__page-rank-entry({
     nodes: GRAPH_NODES,
     edges: GRAPH_EDGES,
     sourceIndex: 0,
     damping: 0.85,
   ...
   ```
   The local fallback (`localSingleEntryPageRank` in `plugins/rufflo-neural-trader/src/signed-attribution.mjs`) runs ~30 LOC of seeded power-iteration when the MCP tool is not available — same math, same result up to floating-point tolerance, same ordering for the same seed (the Phase 6 smoke asserts this).
...
6. **Sign the artifact** (reuses the Phase 4 signing primitives — same Ed25519 + canonicalization):
   ...
   - If a key resolves: `signAttributionArtifact(body, privateKeyHex)` from `plugins/rufflo-neural-trader/src/signed-attribution.mjs`.
   - If NEITHER path resolves: log `"[WARN] rufflo-neural-trader: no witness signing key found — storing attribution artifact in UNSIGNED degraded mode. Regulator filings will reject UNSIGNED artifacts."` and store the body unsigned. NEVER silently fall back.
...
```ts
import { verifyAttributionArtifact } from 'plugins/rufflo-neural-trader/src/signed-attribution.mjs';
...
- `plugins/rufflo-neural-trader/src/signed-attribution.ts` (the typed contract)
- `plugins/rufflo-neural-trader/src/signed-attribution.mjs` (the runtime mirror)
