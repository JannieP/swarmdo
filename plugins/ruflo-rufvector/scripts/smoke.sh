#!/usr/bin/env bash
# Smoke test for rufflo-rufvector plugin against rufvector@0.2.25.
# Exits non-zero if any contracted CLI surface is missing or behaves
# differently from documented. Run after `npm install rufvector@0.2.25`
# (or rely on the npx fetch).
set -u
PIN="rufvector@0.2.25"
PASS=0
FAIL=0
WORKDIR="$(mktemp -d -t rufvector-smoke.XXXXXX)"
