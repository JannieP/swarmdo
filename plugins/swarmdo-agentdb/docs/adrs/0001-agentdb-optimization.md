---
id: ADR-0001
title: Optimize swarmdo-agentdb — accurate surface, quantization opt-ins, controller-managed namespacing, smoke-as-contract
status: Proposed
date: 2026-05-04
authors:
  - planner (Claude Code)
...
### Today's `swarmdo-agentdb`

`swarmdo-agentdb` is a thin documentation wrapper around three MCP tool families exposed by `@swarmdo/cli`: `agentdb_*` (controller bridge), `embeddings_*` (SwarmVector ONNX engine), and `swarmllm_hnsw_*` (WASM-backed pattern router). The plugin ships six files:

- `.claude-plugin/plugin.json:2` — `name: "swarmdo-agentdb"`, `version: "0.2.0"`, keywords `agentdb, swarmvector, hnsw, embeddings, vector-search`.
...
- `agents/agentdb-specialist.md:15` — header `MCP Tools (19 Controllers)` then enumerates eight `agentdb_*` tool prefixes plus `embeddings_*` and `swarmllm_hnsw_*` as bullet groups.
...
- `skills/vector-search/SKILL.md:5` — `allowed-tools` enumerates seven `embeddings_*` and three `swarmllm_hnsw_*` tools; `SKILL.md:46–50` is a performance table claiming `HNSW (n=10,000) ~4.7x faster`.
...
|---|---|---|---|
| `agentdb_*` MCP tools | "19 controllers" → implies tools | **15 tools** | `v3/@swarmdo/cli/src/mcp-tools/agentdb-tools.ts:629–645` (`agentdbTools` export array) |
| `embeddings_*` MCP tools | 7 enumerated in skill | **10 tools** | `v3/@swarmdo/cli/src/mcp-tools/embeddings-tools.ts:159, 260, 328, 418, 520, 717, 835, 910, 926, 970` |
| `swarmllm_hnsw_*` MCP tools | 3 enumerated | **3 tools** (correct) but capacity is **~11 patterns**, not 10,000 | `v3/@swarmdo/cli/src/mcp-tools/swarmllm-tools.ts:57–58` ("Max ~11 patterns (v2.0.1 limit)") |
| Controllers in `ControllerRegistry` | "19" | **29** names across 6 init levels: 13 AgentDB-layer + 16 CLI-layer | `v3/@swarmdo/memory/src/controller-registry.ts:34–73` (type `ControllerName`) and `:160–174` (`INIT_LEVELS`) |
| RaBitQ 1-bit quantization | Not documented | 32× compression, two-phase Hamming-prefilter + exact-rerank pipeline | `embeddings_rabitq_build`, `_search`, `_status` at `embeddings-tools.ts:910, 926, 970`; impl at `v3/@swarmdo/cli/src/memory/rabitq-index.ts` |
| HNSW tuning (`efSearch`, `efConstruction`, `M`) | Not surfaced in any skill | `efSearch` accepted as a constructor param on `swarmllm_hnsw_create`; `efConstruction` defaulted to 200 in the lite index; `M` not exposed via MCP | `swarmllm-tools.ts:64`, `v3/@swarmdo/memory/src/hnsw-index.ts:537` |
...
| `agentdb` npm dep version | Not pinned in plugin | `agentdb: ^3.0.0-alpha.11` in CLI's `package.json:120`; `pnpm-lock.yaml` resolves multiple versions (1.6.1, 2.0.0-alpha.3.4/3.7, 3.0.0-alpha.10/11) | `v3/@swarmdo/cli/package.json:120`; `v3/node_modules/.pnpm/` |
...
3. **Namespacing convention.** Every consumer plugin (`swarmdo-browser` defines `browser-sessions / browser-selectors / browser-templates / browser-cookies` in its ADR-0001 §3; `swarmdo-rag-memory` references `claude-memories / patterns / tasks / solutions`; `swarmdo-intelligence` writes to `pattern`) reinvents namespace naming. There is no contract from `swarmdo-agentdb` about how namespaces should be named, what they should contain, or how they are GC'd. The `agentdb_*` tools mostly do not even take a namespace parameter — they route to controllers (`reasoningBank`, `hierarchicalMemory`, `causalGraph`) — but the CLI fallback `memory_store` and `embeddings_search` *do* take namespace strings, so the surface is mixed and undocumented.
4. **Token-efficiency path.** The repo ships `getCompactContext` on the `TokenOptimizer` (`v3/@swarmdo/integration/src/token-optimizer.ts:109`), and `agentdb_context-synthesize` exists for the same goal at the MCP layer. Neither is surfaced by the plugin as a "use this when you want compact retrieved context for an LLM call" workflow.

### Why now

`swarmdo-agentdb` is the substrate plugin — `swarmdo-browser` (ADR-0001) explicitly depends on AgentDB namespaces and controller-managed sessions; `swarmdo-rag-memory` and `swarmdo-intelligence` route through the same controllers; the whole "memory-as-substrate" story in `CLAUDE.md` rests on it. Documentation drift in this plugin is contagious — every downstream plugin inherits the wrong tool count, the wrong controller count, and the missing quantization story. We just fixed the same class of drift in `swarmdo-swarmvector` (ADR-0001 there); the same pattern applies here.

## Decision

We propose six changes to `swarmdo-agentdb`. None requires modifying AgentDB itself; all are plugin-local edits plus one new smoke script.
...
`efSearch` is passed via `swarmllm_hnsw_create` (`swarmllm-tools.ts:64`). `M` is not currently MCP-exposed; we document it as a registry-level setting with a forward reference: "raise as a follow-up ADR if `M` should be MCP-tunable."

We also fix the speed claim. The 12,500× number is for the embeddings/HNSW path inside `@swarmdo/memory`, not for the WASM `swarmllm_hnsw_*` tools — those are capped at ~11 patterns (`swarmllm-tools.ts:58`). The skill currently lumps them together. The new copy reads: "`embeddings_search` uses the `@swarmdo/memory` HNSW index (large-scale, 150–12,500×). `swarmllm_hnsw_*` is a separate WASM-backed router for ≤11 high-priority patterns — useful for hot routes, not for corpus search."

### 4. Define a namespace convention contract for downstream plugins

Add a new section to `README.md` titled **"Namespace convention"** that swarmdo-agentdb owns and downstream plugins consume. The contract:

- **Naming**: `<plugin-stem>-<intent>` kebab-case. Examples: `browser-sessions`, `browser-selectors`, `browser-cookies` (swarmdo-browser), `claude-memories` (swarmdo-rag-memory's bridge), `pattern` (swarmdo-intelligence ReasoningBank fallback).
...
- **GC posture**: swarmdo-agentdb does not GC namespaces. Consumer plugins that want lifecycle (e.g., `browser-sessions` after `purge`) own their own deletion via `memory_delete` + `agentdb_consolidate`. Document this so consumers don't expect cleanup we don't provide.
...
- **Causal-edge graph-node backend** (`agentdb-tools.ts:267–290`, ADR-087): `agentdb_causal-edge` tries the native `@swarmvector/graph-node` backend first, then the bridge. Document the `_graphNodeBackend: true` field so consumers can branch on it.
...
### 6. Smoke test as the contract (mirrors swarmvector ADR-0001 §5)

Add `scripts/smoke.sh` (file does not exist yet — first one in this plugin). It runs against any environment that has `npx @swarmdo/cli@latest` available with MCP enabled. Each check is a one-liner around `mcp tool call ... --json` + `jq`. The contract is "10 passed, 0 failed".
...
5. The 3 documented `swarmllm_hnsw_*` tools are all callable: `swarmllm_hnsw_create` (dim=384, max=11), `swarmllm_hnsw_add`, `swarmllm_hnsw_route`.
...
Following swarmvector ADR-0001:

- Add a "Compatibility" subsection to README.md that states: "Plugin v0.3.x targets `@swarmdo/cli` v3.6.x with bundled `agentdb@^3.0.0-alpha.11`. The plugin is documentation-only and does not pin via package.json; the smoke contract is the verification mechanism."
...
- RaBitQ goes from invisible to a documented quantization workflow — downstream plugins handling large corpora (`swarmdo-rag-memory`, future `swarmdo-knowledge-graph` integrations) get a 32× memory-reduction story by reference, not by re-discovery.
- HNSW becomes tunable instead of a magic number. Consumer plugins choose an operating point.
- A namespace convention exists. `swarmdo-browser`'s ADR §3 already implements it ad hoc; this ADR formalizes it so the next plugin author doesn't re-derive it.
...
```bash
bash plugins/swarmdo-agentdb/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

Plus three documentation invariants checked by a one-line `grep` each:

- `! grep -rn "19 AgentDB controllers\|all 19 controllers\|19 Controllers" plugins/swarmdo-agentdb/` — no occurrence remains.
- `grep -q "embeddings_rabitq_build" plugins/swarmdo-agentdb/skills/vector-search/SKILL.md` — quantization workflow is documented.
- `grep -q "Namespace convention" plugins/swarmdo-agentdb/README.md` — namespace contract section exists.

## Related

- `plugins/swarmdo-swarmvector/docs/adrs/0001-pin-swarmvector-0.2.25.md` — pinning + smoke-as-contract precedent (this ADR mirrors §5 and §6).
- `plugins/swarmdo-browser/docs/adrs/0001-browser-skills-architecture.md` — `browser-*` namespace family that motivated §4 (namespace convention contract).
...
- `v3/@swarmdo/cli/src/mcp-tools/agentdb-tools.ts` — 15 `agentdb_*` tool definitions (canonical surface for §6 check 3).
- `v3/@swarmdo/cli/src/mcp-tools/embeddings-tools.ts` — 10 `embeddings_*` tool definitions including RaBitQ trio at `:910–981`.
- `v3/@swarmdo/cli/src/mcp-tools/swarmllm-tools.ts` — 3 `swarmllm_hnsw_*` tools, ~11-pattern WASM cap at `:58`.
- `v3/@swarmdo/memory/src/controller-registry.ts:34–73` — `ControllerName` union (canonical 29-name list).
- `v3/@swarmdo/memory/src/controller-registry.ts:160–174` — `INIT_LEVELS` (canonical dependency-ordered grouping).
- `v3/@swarmdo/cli/src/memory/rabitq-index.ts` — RaBitQ implementation referenced by the quantization workflow in §2.
- `v3/@swarmdo/integration/src/token-optimizer.ts:109` — `getCompactContext` token-efficiency path (deferred: §6 covers `agentdb_context-synthesize` only; the integration-layer optimizer is a separate ADR if surfaced).
