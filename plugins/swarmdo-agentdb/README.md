...
The substrate plugin for Swarmdo memory. Wraps three CLI MCP families — `agentdb_*` (controller bridge, 15 tools), `embeddings_*` (SwarmVector ONNX engine, 10 tools), and `swarmllm_hnsw_*` (WASM-backed pattern router, 3 tools) — into discoverable skills and commands. Other plugins (`swarmdo-browser`, `swarmdo-rag-memory`, `swarmdo-intelligence`) compose this substrate; this plugin owns the namespace convention and the smoke contract for the substrate as a whole.

> **Status:** ADR-0001 implemented. Plugin v0.3.0 targets `@swarmdo/cli` v3.6.x with bundled `agentdb@^3.0.0-alpha.11`. The smoke contract (10 checks) is the verification mechanism — see [docs/adrs/0001-agentdb-optimization.md](./docs/adrs/0001-agentdb-optimization.md).
...
- **CLI:** pinned to `@swarmdo/cli` v3.6 major+minor. Patch bumps within v3.6 are expected to be no-op.
...
- **Verification:** the bundled smoke script is the source of truth (`bash plugins/swarmdo-agentdb/scripts/smoke.sh`). If smoke passes against your CLI version, the plugin's contract holds.
...
- **SwarmVector embeddings**: 10 `embeddings_*` MCP tools — 384-dim ONNX (all-MiniLM-L6-v2), HNSW search, hyperbolic (Poincare), neural substrate, and **RaBitQ 1-bit quantization (32× memory reduction)**.
- **HNSW pattern router**: 3 `swarmllm_hnsw_*` tools (WASM-backed, ≤11 high-priority patterns — distinct from the large-scale embeddings HNSW path).
...
The "controller count" reported anywhere in this plugin is **whatever the runtime tool reports**. The canonical list of names is the `ControllerName` union at `v3/@swarmdo/memory/src/controller-registry.ts:34-73` (29 names across 6 init levels). Inspect at runtime:
...
`graphAdapter` is currently disabled pending an external graph-DB connection (tracked in ADR-095). Other Level-2/3 security controllers (`mutationGuard`, `attestationLog`, `gnnService`, `rvfOptimizer`, `guardedVectorBackend`) were activated by ADR-095 G7 in swarmdo 3.6.23+.
...
| `rvfOptimizer` | SwarmVector format compaction — quantizes + dedupes vector blocks before persistence. | `agentdb/dist/src/optimizations/RVFOptimizer.js` |
...
| `GuardedVectorBackend` | Wraps the existing vectorBackend with `mutationGuard` + `attestationLog`. | `agentdb/dist/src/backends/swarmvector/GuardedVectorBackend.js` |

## Commands

- `/agentdb` — AgentDB health, controller status, session management
- `/embeddings` — SwarmVector embedding engine status and operations
...
| Plugin | Namespaces |
|---|---|
| `swarmdo-browser` | `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies` |
| `swarmdo-rag-memory` | (uses bridge target `claude-memories`) |
| `swarmdo-intelligence` | (uses fallback target `pattern`) |
...
| `trajectory-begin/step/end` (swarmvector hooks) | swarmvector substrate (separate plugin) | sona/agentdb namespaces handled by `swarmdo-swarmvector` | See `plugins/swarmdo-swarmvector/docs/adrs/0001-pin-swarmvector-0.2.25.md` |
...
`agentdb_causal-edge` tries the native `@swarmvector/graph-node` backend first; on failure, falls back to the bridge. The response includes `_graphNodeBackend: true` when the native backend handled the call. Source: `agentdb-tools.ts:267-290`.

### Bridge unavailable

When `bridgeHealthCheck()` returns null (the `@swarmdo/memory` package is not installed or `controller-registry.ts` is missing), every `agentdb_*` handler returns:

```json
{
  "success": false,
  "error": "AgentDB bridge not available — @swarmdo/memory not installed... Use memory_store/memory_search tools instead."
}
```
...
- [`ADR-0001` — Optimize swarmdo-agentdb (accurate surface, RaBitQ, namespacing, smoke contract)](./docs/adrs/0001-agentdb-optimization.md)

## Related Plugins

- `swarmdo-rag-memory` — simple store/search/recall interface; consumes the `claude-memories` reserved namespace
- `swarmdo-intelligence` — SONA neural patterns; consumes the `pattern` reserved namespace via ReasoningBank
- `swarmdo-browser` — composes the namespace convention for `browser-sessions/-selectors/-templates/-cookies` (ADR-0001 §3 there)
- `swarmdo-swarmvector` — pinned swarmvector CLI; sibling substrate plugin
