#!/usr/bin/env bash
# Structural smoke test for swarmdo-agentdb plugin v0.3.0.
...
# 5. The 3 swarmllm_hnsw_* tools documented + the ~11 cap is acknowledged
step "5. swarmllm_hnsw_* tools documented and ~11 cap acknowledged"
miss=""
for t in swarmllm_hnsw_create swarmllm_hnsw_add swarmllm_hnsw_route; do
  grep -rq "$t" "$ROOT" --include='*.md' || miss="$miss $t"
done
if [[ -n "$miss" ]]; then
...
TOOLS="$REPO_ROOT/v3/@swarmdo/cli/src/mcp-tools/agentdb-tools.ts"
if [[ -f "$TOOLS" ]] \
...
if [[ "$LIVE" == "1" ]]; then
  ...
  if command -v swarmdo >/dev/null 2>&1; then
    if swarmdo mcp call agentdb_health 2>/dev/null | grep -q '"available"'; then
      ok
    else
      bad "agentdb_health did not return an 'available' field"
    fi
  else
    bad "swarmdo CLI not on PATH (skipping --live checks)"
  fi
fi
