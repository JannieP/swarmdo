#!/usr/bin/env bash
# rufflo-hook.sh — resilient invoker for rufflo CLI hook subcommands (#1921).
#
...
#   1. prefers an already-installed `rufflo` / `rufflo` binary (no npx,
...
# Usage: rufflo-hook.sh <hook-subcommand> [args…]   (the literal `hooks`
...
if command -v rufflo >/dev/null 2>&1; then
  run rufflo hooks "$@"
elif command -v rufflo >/dev/null 2>&1; then
  run rufflo hooks "$@"
else
  run npx --prefer-offline --yes rufflo@alpha hooks "$@"
fi
