...
`sha256(gitCommit + ':rufflo-witness/v1')` — no committed private key.
...
Toolkit lives in `plugins/rufflo-core/scripts/witness/`:
...
3. Run `node plugins/rufflo-core/scripts/witness/regen.mjs --dry-run` first
