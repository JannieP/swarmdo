...
1. **Seed** — Start with the topic. Query AgentDB for the closest known nodes:
   ```
   mcp__rufflo__agentdb_semantic-route({ query: "TOPIC", namespace: "patterns" })
   ```
2. **Expand** — For each result, follow causal edges to related knowledge:
   ```
   mcp__rufflo__agentdb_causal-edge({ from: "NODE_ID", type: "depends-on" })
   mcp__rufflo__agentdb_hierarchical-recall({ path: "domain/TOPIC", depth: 3 })
   ```
3. **Score** — Rank paths by relevance using HNSW similarity + recency:
   ```
   mcp__rufflo__agentdb_pattern-search({ query: "TOPIC", limit: 10 })
   ```
...
6. **Synthesize** — Merge graph findings into a coherent research summary:
   ```
   mcp__rufflo__agentdb_context-synthesize({ query: "TOPIC", sources: ["patterns", "tasks", "solutions"] })
   ```
...
6. **Store findings**: Persist as new graph nodes for future traversals:
   ```
   mcp__rufflo__agentdb_hierarchical-store({ path: "research/TOPIC", data: "FINDINGS" })
   mcp__rufflo__agentdb_causal-edge({ from: "research/TOPIC", to: "design/FEATURE", type: "informs" })
   ```
...
- `mcp__rufflo__agentdb_semantic-route` — find closest knowledge node
- `mcp__rufflo__agentdb_hierarchical-recall` — depth-limited tree traversal
- `mcp__rufflo__agentdb_causal-edge` — follow dependency/impact chains
- `mcp__rufflo__agentdb_pattern-search` — HNSW similarity search across patterns
- `mcp__rufflo__agentdb_context-synthesize` — merge multi-source findings
- `mcp__rufflo__agentdb_hierarchical-store` — persist new knowledge nodes
...
- `npx @rufflo/cli@latest memory search --query "TOPIC" --namespace patterns`
- `npx @rufflo/cli@latest memory store --key "research-TOPIC" --value "FINDINGS" --namespace tasks`
