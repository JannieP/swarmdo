...
|---|---|---|
| **Nested sub-agents** (you) | Deep delegation where each level discovers more work. Context window of any single agent would otherwise fill. | 5 levels (Anthropic API), rufflo default 4 (one-level guard band) |
...
| `mcp__rufflo__wasm_agent_*` | Untrusted code execution in WASM sandbox | n/a (different mechanism) |
...
```bash
npx @rufflo/cli@latest memory search --query "<problem shape>" --namespace nested-patterns --limit 5
```

After completion, store the tree shape and what worked:

```bash
npx @rufflo/cli@latest memory store --namespace nested-patterns \
