...
```bash
# File inputs
npx swarmdo metaharness similarity --a a.json --b b.json

# Memory inputs (records persisted by oia-audit.mjs)
npx swarmdo metaharness similarity --a-key harness-X --b-key harness-Y

# Per-dimension breakdown (used by ADR-151 §3.2 Recommender)
npx swarmdo metaharness similarity --a a.json --b b.json --per-dimension

# Alert when too-dissimilar (used by ADR-151 §3.3 Drift Detection)
npx swarmdo metaharness similarity --a a.json --b b.json --alert-below 0.5
```
...
MCP tool: `mcp__swarmdo__metaharness_similarity` (registered in `v3/@swarmdo/cli/src/mcp-tools/metaharness-tools.ts`)
