...
Surfaces the upstream `metaharness score` CLI as a swarmdo skill. Use when
Claude Code needs to assess whether a repo is ready for harness adoption
before recommending the user run `npx swarmdo init` or `harness-mint`.
...
## Phase-0 baseline (swarmdo's own scorecard, measured 2026-06-16)
...
Swarmdo passes its own readiness check. `memoryUsefulness: 40` is the
...
```bash
node plugins/swarmdo-metaharness/scripts/score.mjs --alert-on-fit-below 70 --format json
```
...
and exits 0. Swarmdo continues to function — this is the architectural
