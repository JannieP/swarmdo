...
async function main() {
  ...
  const distPath = resolve(SCRIPTS_DIR, '..', '..', '..',
    'v3', '@swarmdo', 'cli', 'dist', 'src', 'mcp-tools', 'metaharness-tools.js');
  ...
  if (!existsSync(distPath)) {
    ...
    console.log(`  cd v3/@swarmdo/cli && npm run build`);
    console.log('');
  ...
  for (const tool of tools) {
    ...
    // `npx @swarmdo/cli@latest memory list` invocation pays both
  ...
  if (scanTool) {
    // Run against swarmdo itself — guaranteed to produce at least the
  ...
  if (trendTool) {
    ...
    if (driftTool) {
      ...
      // through. baselineFile has no findings; current swarmdo audit has
