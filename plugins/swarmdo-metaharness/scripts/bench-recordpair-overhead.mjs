...
// recordPair() fires only when SWARMDO_ROUTER_PARALLEL_LOG=1.
...
function main() {
  ...
  delete process.env.SWARMDO_ROUTER_PARALLEL_LOG;  // ensure OFF
  const envCheckOff = bench(
    'iter-12 env check (FLAG OFF — default path)',
    () => { if (process.env.SWARMDO_ROUTER_PARALLEL_LOG === '1') { /* unreached */ } },
    iters,
  );
  ...
  process.env.SWARMDO_ROUTER_PARALLEL_LOG = '1';
  const envCheckOn = bench(
    'iter-12 env check (FLAG ON — branch taken, NO recordPair)',
    () => { if (process.env.SWARMDO_ROUTER_PARALLEL_LOG === '1') { /* branch taken */ } },
    iters,
  );
  delete process.env.SWARMDO_ROUTER_PARALLEL_LOG;
  ...
  if (ARGS.maxOverheadNs !== null) {
    ...
    if (overhead > ARGS.maxOverheadNs) {
      ...
      console.log(`  Inspect v3/@swarmdo/cli/src/swarmvector/model-router.ts route() for new work in the default path.`);
      process.exit(1);
    } else {
