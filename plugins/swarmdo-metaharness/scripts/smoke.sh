#!/usr/bin/env bash
# Structural smoke test for swarmdo-metaharness v0.1.0 (ADR-150 Phase 1).
set -u
...
if [[ "$v" != "0.1.0" ]]; then
  ...
  miss=""
  for k in swarmdo metaharness harness scorecard genome mcp-scan threat-model router adr-150 adr-148 adr-149 optional-dependency graceful-degradation subprocess phase-1-mvp; do
...
F="$ROOT/commands/swarmdo-metaharness.md"
miss=""
...
F="$ROOT/../../v3/@swarmdo/cli/src/commands/metaharness.ts"
miss=""
...
LOADER="$ROOT/../../v3/@swarmdo/cli/src/commands/index.ts"
grep -q "metaharness: () => import" "$LOADER" 2>/dev/null || miss="$miss not-registered-in-loader"
[[ -z "$miss" ]] && ok || bad "$miss"

step "16. swarmdo wrapper has metaharness in optionalDependencies (architectural constraint #2)"
F="$ROOT/../../swarmdo/package.json"
node -e "
...
" 2>/dev/null && ok || bad "swarmdo wrapper missing metaharness optionalDep"
...
EXPECTED_JOBS=(
  "score"            # iter 1 — score harness against swarmdo
...
#   - `npm install swarmdo` → uses swarmdo/package.json
#   - `npm install @swarmdo/cli` → uses cli/package.json
...
SWARMDOPJ="$ROOT/../../swarmdo/package.json"
CLIPJ="$ROOT/../../v3/@swarmdo/cli/package.json"
for pkg in "metaharness" "@metaharness/router" "@metaharness/kernel"; do
  ...
  SWARMDO_PIN=$(node -e "console.log(JSON.parse(require('fs').readFileSync('$SWARMDOPJ')).optionalDependencies?.['$pkg'] || '')" 2>/dev/null)
  ...
  if [[ -z "$ROOT_PIN" && -z "$SWARMDO_PIN" && -z "$CLI_PIN" ]]; then
    continue
  fi
  # Compare non-empty pins — they must all match.
  PINS=$(echo "$ROOT_PIN $SWARMDO_PIN $CLI_PIN" | tr ' ' '\n' | grep -v '^$' | sort -u | wc -l | tr -d ' ')
  if [[ "$PINS" != "1" ]]; then
    miss="$miss ${pkg}-pin-drift:root=${ROOT_PIN},swarmdo=${SWARMDO_PIN},cli=${CLI_PIN}"
  fi
done
[[ -z "$miss" ]] && ok || bad "$miss"
...
#   plugins/swarmdo-metaharness/skills/harness-foo/SKILL.md
...
DISP="$ROOT/../../v3/@swarmdo/cli/src/commands/metaharness.ts"
WRAPPER="$ROOT/../../v3/@swarmdo/cli/src/mcp-tools/metaharness-tools.ts"
...
# SUBCOMMANDS map should appear as `npx swarmdo metaharness <X>` for
# discoverability.
DISP="$ROOT/../../v3/@swarmdo/cli/src/commands/metaharness.ts"
CMD="$ROOT/../../CLAUDE.md"
...
for k in $KEYS; do
  COUNT=$((COUNT + 1))
  grep -qE "npx swarmdo metaharness ${k}([ \\\\]|$)" "$CMD" 2>/dev/null \
...
for t in $TOOLS; do
  COUNT=$((COUNT + 1))
  # Look for `mcp__swarmdo__metaharness_X` (the agent-facing name form)
  grep -q "mcp__swarmdo__${t}" "$CMD" 2>/dev/null \
...
# but forgets the map (or vice-versa), `swarmdo metaharness X` errors at
...
# Runtime: tripwire passes on swarmdo's own audit
node "$F" >/dev/null 2>&1 || miss="$miss tripwire-fails-locally"
[[ -z "$miss" ]] && ok || bad "$miss"
...
# (real swarmdo audit has 1 INFO finding; baseline file with no findings → introduced=1)
BC=$(mktemp)
...
# as a swarmdo skill or MCP tool. It clones arbitrary git URLs which is too
# powerful to expose to agents. (Upstream supports it; swarmdo deliberately
...
# static import of @metaharness/* breaks the "swarmdo works without the dep"
# rule. Scan the CLI source tree for stray static imports.
CLI_SRC="$ROOT/../../v3/@swarmdo/cli/src"
...
miss=""
DOC="$ROOT/../../v3/@swarmdo/cli/src/commands/doctor.ts"
...
CMD="$ROOT/../../CLAUDE.md"
grep -q "mcp__swarmdo__metaharness_drift_from_history" "$CMD" 2>/dev/null || miss="$miss claude-md-no-mcp"
grep -q "swarmdo metaharness drift-from-history" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
step "17z8. swarmdo doctor checks the integration layer, not just upstream (iter 45)"
miss=""
DOC="$ROOT/../../v3/@swarmdo/cli/src/commands/doctor.ts"
...
CMD="$ROOT/../../CLAUDE.md"
grep -q "mcp__swarmdo__metaharness_similarity" "$CMD" 2>/dev/null || miss="$miss claude-md-no-mcp-tool"
grep -q "swarmdo metaharness similarity" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
# MCP tool registered
MCP="$ROOT/../../v3/@swarmdo/cli/src/mcp-tools/metaharness-tools.ts"
...
grep -q "SWARMDO_ROUTER_PARALLEL_LOG === '1'" "$F" || miss="$miss no-flag-literal"
...
grep -q "Swarmdo remains operational if every MetaHarness package is removed" "$F" || miss="$miss no-constraint-quote"
# All 4 rules documented
grep -q "no-metaharness-smoke.yml" "$F" || miss="$miss no-ci-gate-ref"
# Command surface + tool surface enumerated
grep -q "npx swarmdo metaharness score" "$F" || miss="$miss no-cli-example"
grep -q "mcp__swarmdo__metaharness_" "$F" || miss="$miss no-mcp-tool-list"
# Routing + parallel-log integration both mentioned
grep -q "SWARMDO_ROUTER_NEURAL\|SWARMDO_ROUTER_PARALLEL_LOG" "$F" || miss="$miss no-routing-flags"
# 3-criteria gate
...
F="$ROOT/../../v3/@swarmdo/cli/src/mcp-tools/metaharness-tools.ts"
miss=""
...
INIT="$ROOT/../../v3/@swarmdo/cli/src/commands/init.ts"
HOOKS="$ROOT/../../v3/@swarmdo/cli/src/commands/hooks.ts"
miss=""
...
F="$ROOT/../../v3/@swarmdo/cli/src/commands/doctor.ts"
miss=""
[[ -f "$F" ]] || miss="$miss missing-file"
...
F="$ROOT/../../v3/@swarmdo/cli/src/swarmvector/model-router.ts"
miss=""
[[ -f "$F" ]] || miss="$miss missing-file"
...
grep -q "SWARMDO_ROUTER_PARALLEL_LOG === '1'" "$F" || miss="$miss no-env-gate-in-router"
# Call site present
...
F="$ROOT/../../v3/@swarmdo/cli/src/swarmvector/router-parallel-recorder.ts"
miss=""
[[ -f "$F" ]] || miss="$miss missing-file"
# Architectural constraint #2: env-gated optional behavior
grep -q "SWARMDO_ROUTER_PARALLEL_LOG" "$F" || miss="$miss no-env-gate"
...
F="$ROOT/../../v3/@swarmdo/cli/src/plugins/store/types.ts"
miss=""
