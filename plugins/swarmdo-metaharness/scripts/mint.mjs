...
//     a temp dir under /tmp/swarmdo-mint-<timestamp>/ if not given — never the
//     project root (swarmdo behavioral rule "never save to root folder").
...
//     confirmation" pattern in swarmdo's CLAUDE.md.
//
// USAGE
...
function safetyChecks() {
  ...
  if (!ARGS.target) {
    ARGS.target = resolve(tmpdir(), `swarmdo-mint-${Date.now()}-${ARGS.name}`);
  }
