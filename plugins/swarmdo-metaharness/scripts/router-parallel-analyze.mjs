  ...
  if (!existsSync(ARGS.input)) {
    if (ARGS.format === 'json') {
      console.log(JSON.stringify({
        ...
        input: ARGS.input,
        hint: 'Enable parallel logging (SWARMDO_ROUTER_PARALLEL_LOG=1) and run a workload first.',
      }, null, 2));
    } else {
      ...
      console.log('');
      console.log('Enable parallel logging by setting `SWARMDO_ROUTER_PARALLEL_LOG=1`');
