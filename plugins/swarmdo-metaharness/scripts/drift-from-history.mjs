...
//   $ npx swarmdo metaharness audit-list --format json
//   $ # ... pick a key by hand from the listing
//   $ npx swarmdo metaharness oia-audit --format json > /tmp/curr.json
//   $ npx swarmdo metaharness audit-trend --baseline-key X --current-file /tmp/curr.json
// Now it's:
...
async function main() {
  ...
  if (records.length === 0) {
    ...
    emitAndExit({
      ...
      hint: 'Run `swarmdo metaharness oia-audit` at least once to seed history.',
    }, 2);
  }
