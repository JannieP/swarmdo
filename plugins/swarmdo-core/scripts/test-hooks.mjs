 ...
 *   node plugins/swarmdo-core/scripts/test-hooks.mjs <path-to-cli-binary>
 *
...
//   - npx fallthrough:     "npx --yes @swarmdo/cli@latest"
const cliInvoke = process.argv[2];
if (!cliInvoke) {
  console.error('Usage: node test-hooks.mjs "<cli-invocation-string>"');
  console.error('Examples:');
  console.error('  node test-hooks.mjs "node $PWD/v3/@swarmdo/cli/bin/cli.js"');
  console.error('  node test-hooks.mjs "npx --yes @swarmdo/cli@latest"');
  process.exit(2);
}
...
const findHook = (matcher) => {
  ...
  return hit.hooks[0].command
    ...
    .replace(/"\$\{CLAUDE_PLUGIN_ROOT\}\/scripts\/swarmdo-hook\.sh"/g, `${cliInvoke} hooks`)
