#!/usr/bin/env node
/**
 * Regression guard for upstream/swarmdo#1863 (and the crash class it represents).
 *
...
const CLI = resolve(REPO_ROOT, 'v3/@swarmdo/cli/bin/cli.js');

if (!existsSync(CLI)) {
  console.error(`FAIL: ${CLI} not found — run \`npm --prefix v3/@swarmdo/cli run build\` first`);
  process.exit(1);
}
