#!/usr/bin/env node
/**
 * Regression guard for upstream/swarmdo#1889.
 *
...
const DIST = resolve(REPO_ROOT, 'v3/@swarmdo/cli/dist/src/mcp-tools/agentdb-tools.js');

if (!existsSync(DIST)) {
  console.error(`FAIL: ${DIST} not found — run \`npm --prefix v3/@swarmdo/cli run build\` first`);
  process.exit(1);
}
