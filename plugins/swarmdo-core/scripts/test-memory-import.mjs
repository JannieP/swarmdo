#!/usr/bin/env node
/**
 * Regression guard for upstream/swarmdo#1883 + #1884.
 *
...
const DIST = resolve(REPO_ROOT, 'v3/@swarmdo/cli/dist/src/mcp-tools/memory-tools.js');
...
if (!existsSync(DIST)) {
  fail(`${DIST} not found — run \`npm --prefix v3/@swarmdo/cli run build\` first`);
  process.exit(1);
}
...
const posixCwd = '/Users/alice/projects/swarmdo';
const posixCandidates = deriveCandidates(posixCwd);
if (posixCandidates.has('-Users-alice-projects-swarmdo')) {
