...
step "2. .mcp.json registers a 'swarmdo' MCP server"
F="$ROOT/.mcp.json"
if [[ -f "$F" ]] && grep -q '"swarmdo"' "$F" && grep -q '"command"' "$F"; then
  ok
else
  bad ".mcp.json missing or no swarmdo server registration"
fi
...
miss=""
for s in init-project swarmdo-doctor discover-plugins; do
...
n=$(grep -oE 'swarmdo-[a-z-]+' "$F" | sort -u | wc -l | tr -d ' ')
if [[ $n -ge 25 ]]; then
  ok
else
  bad "expected ≥25 distinct swarmdo-* references, got $n"
fi

step "6. README pins @swarmdo/cli to v3.6"
...
step "9. /swarmdo-status command invokes doctor + status"
F="$ROOT/commands/swarmdo-status.md"
if grep -q "doctor" "$F" && grep -q "status" "$F"; then
  ok
else
  bad "swarmdo-status command missing doctor/status invocation"
fi
