 ...
 *
 * @module plugins/swarmdo-core/scripts/witness/lib
 */
...
/**
 * Try to load the binary RVF backend (@swarmvector/rvf-node) from the
...
function loadRvfNode(probeRoots) {
  const expanded = [
    ...
    ...probeRoots.flatMap(r => [
      join(r, 'v3/@swarmdo/cli'),
      join(r, 'v3/@swarmdo/memory'),
    ]),
  ];
  for (const root of expanded) {
    try {
      const req = createRequire(join(root, 'noop.js'));
      const mod = req('@swarmvector/rvf-node');
...
// install roots so the toolkit works in swarmdo's monorepo layout *and*
...
function loadEd25519(probeRoots) {
  ...
  const expanded = [
    ...
    ...probeRoots.flatMap(r => [
      join(r, 'v3/@swarmdo/cli'),
      join(r, 'v3/@swarmdo/plugin-agent-federation'),
    ]),
  ];
  let lastErr;
  for (const root of expanded) {
...
export function regenerate(opts) {
  ...
  const manifest = {
    schema: existing?.manifest?.schema ?? 'swarmdo-witness/v1',
    issuedAt,
    gitCommit,
    branch,
    os: osDir(),
  ...
  const seed = createHash('sha256').update(gitCommit + ':swarmdo-witness/v1').digest();
  ...
  const witness = {
    ...
    integrity: {
      ...
      seedDerivation: "sha256(gitCommit + ':swarmdo-witness/v1')",
    },
  };
