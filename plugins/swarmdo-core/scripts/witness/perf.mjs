 ...
 * ────────────────────
 *   install_pack          how long `pnpm pack @swarmdo/memory` takes
 *   install_no_optional   how long `npm install <tarball> --omit=optional` takes
 *   memory_load           how long `import('@swarmdo/memory')` takes
 ...
 *
 * Usage:
 *   node plugins/swarmdo-core/scripts/witness/perf.mjs \
...
const runners = {
  install_pack: () => bench(() => {
    ...
    execFileSync('pnpm', ['pack', '--pack-destination', tmpdir()], {
      cwd: join(REPO_ROOT, 'v3/@swarmdo/memory'),
      stdio: 'pipe',
    });
  }),
  ...
  memory_load: () => {
    return bench(() => {
      ...
      spawnSync(process.execPath, ['-e', "import('@swarmdo/memory').then(()=>{}).catch(e=>{process.exit(1)})"], {
        cwd: REPO_ROOT,
        stdio: 'pipe',
      });
    });
  ...
  memory_round_trip: () => {
    ...
    const ms = bench(() => {
      const r = spawnSync(process.execPath, ['-e', `
        const m = await import('@swarmdo/memory');
