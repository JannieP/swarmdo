 ...
 *
 * Project-agnostic — works without swarmdo CLI being installed.
 *
 * Usage:
...
async function verifySignature(witness, repoRoot) {
  ...
  const probes = [
    repoRoot,
    join(repoRoot, 'v3'),
    join(repoRoot, 'v3/@swarmdo/cli'),
    join(repoRoot, 'v3/@swarmdo/plugin-agent-federation'),
  ];
  ...
  if (!ed) {
    // swarmdo#1880 — the scheduled 12h verification has bounced off this
