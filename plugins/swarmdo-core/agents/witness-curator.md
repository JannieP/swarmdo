...
`sha256(gitCommit + ':swarmdo-witness/v1')` — no committed private key.
...
Toolkit lives in `plugins/swarmdo-core/scripts/witness/`:
...
3. Run `node plugins/swarmdo-core/scripts/witness/regen.mjs --dry-run` first
