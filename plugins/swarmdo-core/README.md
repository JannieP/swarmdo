...
- **Hooks**: PreToolUse / PostToolUse / PreCompact / UserPromptSubmit / SessionEnd wired to swarmdo's auto-routing + learning loop. The session-end pipeline (summary, state persistence, metrics export) runs on `SessionEnd` — once at termination, covering abnormal end — not `Stop`, which fires after every turn. **Compaction survival:** `PreCompact` snapshots the working set (recent edits, uncommitted changes, branch) via `compact-snapshot write`, and the next `UserPromptSubmit` re-injects it via `compact-snapshot read` (consume-once), so the model re-grounds after compaction instead of re-exploring. Defined at `plugins/swarmdo-core/hooks/hooks.json` so the per-plugin loader picks them up on `/plugin install swarmdo-core@swarmdo` (per-plugin layout — fixes #1748 Issue 1; the marketplace-root copy at `.claude-plugin/hooks/hooks.json` is preserved for `claude --plugin-dir <repo-root>` users).
