---
id: ADR-0001
title: swarmdo-core plugin contract — pinning, MCP server contract, plugin-catalog discovery, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`swarmdo-core` is the **foundation plugin**. Every other plugin (`swarmdo-swarmvector`, `swarmdo-agentdb`, `swarmdo-browser`, `swarmdo-intelligence`, `swarmdo-adr`, `swarmdo-aidefence`, `swarmdo-autopilot`, plus 25 others) depends on the MCP server it registers via `.mcp.json` and the orchestration patterns it documents.

Today's plugin (v0.1.0):

- `.claude-plugin/plugin.json:4` — `version: "0.1.0"`, keywords `mcp, orchestration, claude-code`
- `.mcp.json` — registers `swarmdo` MCP server via `npx -y @swarmdo/cli@latest`
- `agents/` — 3 generalists (`coder`, `researcher`, `reviewer`)
- `skills/` — 3 first-run helpers (`init-project`, `swarmdo-doctor`, `discover-plugins`)
- `commands/swarmdo-status.md` — system status one-liner
...
2. **No smoke test.**
3. **No Compatibility section** pinning to `@swarmdo/cli` v3.6.
...
- **Compatibility** — pin to `@swarmdo/cli` v3.6. Note the `npx -y @swarmdo/cli@latest` invocation in `.mcp.json` is the dynamic resolver; smoke verifies the resolved version.
- **MCP server contract** — the registered `swarmdo` MCP server exposes 300+ tools across families: `memory_*`, `agentdb_*`, `embeddings_*`, `swarmllm_*`, `hooks_*`, `aidefence_*`, `neural_*`, `autopilot_*`, `browser_*`, `agent_*`, `swarm_*`, `system_*`, etc. Runtime truth via `mcp tool call mcp_status`.
...
2. `.mcp.json` exists and registers a `swarmdo` MCP server.
3. All 3 agents present (`coder`, `researcher`, `reviewer`) with valid frontmatter.
4. All 3 skills present (`init-project`, `swarmdo-doctor`, `discover-plugins`) with valid frontmatter.
...
9. `commands/swarmdo-status.md` invokes `doctor` and `status`.
...
- `plugins/swarmdo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — 4-step pipeline
- `plugins/swarmdo-adr/docs/adrs/0001-adr-plugin-pattern.md`
- `plugins/swarmdo-aidefence/docs/adrs/0001-aidefence-contract.md` — 3-gate pattern
- `plugins/swarmdo-autopilot/docs/adrs/0001-autopilot-contract.md` — 270s cache-aware /loop
- `v3/@swarmdo/cli/` — the MCP server source backing this plugin

## Implementation status

Plugin version v0.2.1 shipped and listed in marketplace.json. Source exists at `plugins/swarmdo-core/`. Contract elements implemented: `.mcp.json` registers `swarmdo` server via `npx -y @swarmdo/cli@latest`; plugin-catalog discovery skill present; 3 generalist agents shipped; smoke-as-contract gate defined in `scripts/smoke.sh`.
