---
id: ADR-0001
title: Adopt a session-as-skill architecture for swarmdo-browser, backed by RVF, swarmvector trajectories, and AgentDB
status: Proposed
date: 2026-05-04
authors:
  - planner (Claude Code)
tags: [plugin, browser, playwright, rvf, swarmvector, agentdb, aidefence, mcp, skills]
---

## Context

### Today's `swarmdo-browser`

The current plugin (v0.1.0) is a thin wrapper around 23 Playwright-backed MCP tools (`mcp__swarmdo__browser_*`). Surface inventory:
...
- `commands/swarmdo-browser.md` — one slash command that lists/screenshots/closes sessions via `browser_session-list`
...
What Browserbase does *not* offer that we need: persistent semantic memory of selectors and page structures, PII/prompt-injection scanning of scraped content, federated session sharing, or learning-from-trajectory beyond the file-based `strategy.md` loop. Those are exactly the gaps Swarmdo's existing subsystems already fill.

### Why act now

`swarmdo-browser` is the most-used surface for any agent that needs the open web, and it is the weakest at translating a one-off run into reusable knowledge. Adopting the Browserbase architectural shape — primitive + named sessions + traces + outer/inner loop — while wiring each artifact into Swarmdo's own substrates (RVF, swarmvector trajectories, AgentDB, AIDefence) gives us replayability, learning, and safety without leaving Playwright behind and without depending on Browserbase's hosted backend.

## Decision

We propose to refactor `swarmdo-browser` around a **session-as-skill** architecture. Each browser session is a first-class, replayable, auditable RVF container with a recorded swarmvector trajectory; skills are thin compositions over a stable Playwright primitive; the marketplace shape mirrors Browserbase's so the plugin can be split later if the surface grows.

### 1. Session model — RVF as the session container

Every browser session opened by swarmdo-browser is allocated an RVF container at session start (`rvf create --dimension 384 <session-name>.rvf`) and committed at session end (`rvf compact && rvf export`). The container holds:

| Slot | Producer | Notes |
|------|----------|-------|
| `manifest.yaml` | session-start hook | URL, viewport, profile, runner (local Playwright vs remote), parent-session lineage |
| `trajectory.ndjson` | swarmvector `hooks trajectory-step` per action | One line per click/nav/extract, with selector, ref, screenshot path, ms-since-start |
...
### 2. Trajectory recording — swarmvector hooks as the action log

We bind every browser MCP tool call to swarmvector trajectory hooks via the existing `pre-task`/`post-task` and a new `browser_*` interceptor:

```
session-start
  → swarmvector hooks trajectory-begin --session-id <rvf-id> --task <human-task>
each action (click, fill, eval, snapshot, screenshot, navigate)
  → swarmvector hooks trajectory-step --session-id <rvf-id> \
       --action <tool> --args <json> --selector <sel> --result <ok|fail>
session-end
  → swarmvector hooks trajectory-end --session-id <rvf-id> --verdict <pass|fail|partial>
```
...
|-----------|-----|-------|---------|
| `browser-sessions` | `<rvf-id>` | manifest summary + verdict + tags | session index for `/swarmdo-browser ls --query "logged into stripe"` |
...
Borrowed pattern: the *idea* of a deliberate sync boundary from `cookie-sync/SKILL.md`. Browserbase has no PII/injection layer; this is a Swarmdo-native addition.
...
### 6. Commands — `/swarmdo-browser` as a verb dispatcher

The single `commands/swarmdo-browser.md` is rewritten to dispatch by subcommand:

```
/swarmdo-browser ls                      # list sessions (RVF-indexed) with filters
/swarmdo-browser show <session-id>       # show manifest + trajectory + verdict
/swarmdo-browser replay <session-id>     # invoke browser-replay
/swarmdo-browser export <session-id>     # rvf export → tar.zst, optionally federated
/swarmdo-browser fork <session-id>       # rvf derive → new session with shared lineage
/swarmdo-browser purge <session-id>      # destroy session, keep redacted manifest
/swarmdo-browser doctor                  # check Playwright, MCP, AgentDB, AIDefence wiring
```
...
- `commands/swarmdo-browser.md` is replaced with the verb dispatcher in §6. Old behavior (list + screenshot + close) maps to `ls` + `show` + `purge`.
- `.claude-plugin/plugin.json` bumps to `0.2.0` with new keywords (`rvf`, `replay`, `trajectory`).
- A `marketplace.json` mirror is **not** introduced yet — swarmdo's plugin marketplace is centralized at the repo root. We will revisit only if the plugin grows beyond ~8 skills (Browserbase's split happened at 10).

### 9. Pinning and contract

Following the precedent of ADR-0001 in `swarmdo-swarmvector`:

- Pin Playwright to a specific minor version in the agent doc and in any helper scripts under `scripts/`.
- Pin the `swarmvector` CLI invocations to `swarmvector@0.2.25` to match the swarmvector plugin's pin (so trajectory hooks behave identically across plugins).
...
- AgentDB and AIDefence become hard dependencies of `swarmdo-browser`. Today the plugin runs against a stock `swarmdo` install; under this proposal it requires the AgentDB controllers and AIDefence to be initialized. This must be enforced by `swarmdo-browser doctor` and by `init-project` updates.
...
7. **Doctor:** `/swarmdo-browser doctor` returns non-zero on each of: missing AgentDB controller, AIDefence not initialized, swarmvector CLI not pinned, Playwright version drift.

A `scripts/smoke.sh` materializes these as 7 numbered tests; the contract is "7 passed, 0 failed".

## Related

- `plugins/swarmdo-swarmvector/docs/adrs/0001-pin-swarmvector-0.2.25.md` — pinning precedent and smoke-test-as-contract pattern.
...
- `https://github.com/browserbase/skills/blob/main/.claude-plugin/marketplace.json` — multi-plugin marketplace shape (deferred for swarmdo-browser).
- Playwright (`https://playwright.dev/`) — the underlying runner; pinning precedent applies.
- Swarmdo subsystems referenced: AgentDB controllers, RVF (`rvf` CLI), swarmvector hooks (`trajectory-*`, `pattern-*`), AIDefence (`aidefence_has_pii`, `aidefence_is_safe`, `aidefence_scan`), Federation (zero-trust cross-installation sharing).
