...
allowed-tools: mcp__swarmdo__browser_open mcp__swarmdo__browser_close mcp__swarmdo__browser_get-text mcp__swarmdo__browser_get-value mcp__swarmdo__browser_eval mcp__swarmdo__browser_snapshot mcp__swarmdo__browser_screenshot mcp__swarmdo__browser_scroll mcp__swarmdo__browser_wait mcp__swarmdo__browser_click mcp__swarmdo__aidefence_has_pii mcp__swarmdo__aidefence_is_safe mcp__swarmdo__aidefence_scan Bash Read Write
---
...
3. **Choose a path**:
   - **Template path** (`--template <name>`): retrieve from AgentDB and apply.
     ```bash
     npx -y @swarmdo/cli@latest memory retrieve --namespace browser-templates --key "<name>"
     ```
...
4. **AIDefence pre-storage**: every extracted string passes the PII gate.
   ```bash
   # Pseudocode — mcp__swarmdo__aidefence_has_pii returns true/false per string.
...
6. **Persist the template** if `--save-template <name>` was passed:
   ```bash
   npx -y @swarmdo/cli@latest memory store --namespace browser-templates \
