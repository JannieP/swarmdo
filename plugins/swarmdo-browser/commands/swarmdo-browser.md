...
Usage: /swarmdo-browser <verb> [args]
...
1. **ls [--query "<text>"] [--host <host>] [--verdict pass|fail|partial]**
   ...
   ```bash
   npx -y @swarmdo/cli@latest memory list --namespace browser-sessions
   # or with semantic filter:
   npx -y @swarmdo/cli@latest memory search --namespace browser-sessions --query "QUERY"
   ```
   For active (live) sessions, also call `mcp__swarmdo__browser_session-list`.

2. **show `<session-id>`**
   Print the session manifest + last 20 trajectory entries + verdict.
   ```bash
   npx -y swarmvector@0.2.25 rvf status <session-id>.rvf
   npx -y swarmvector@0.2.25 rvf segments <session-id>.rvf
   ```
...
4. **export `<session-id>` [-o <path>] [--federate]**
   ```bash
   npx -y swarmvector@0.2.25 rvf export <session-id>.rvf -o <path>
   ```
   With `--federate`, also push via the `swarmdo-federation` plugin.

5. **fork `<session-id>` [--name <new-name>]**
   ```bash
   npx -y swarmvector@0.2.25 rvf derive <session-id>.rvf <new-name>.rvf
   ```
...
7. **doctor**
   ...
   - Playwright runner reachable (`mcp__swarmdo__browser_open` against `about:blank`)
   - AgentDB controllers initialized (`mcp__swarmdo__agentdb_health`)
   - AIDefence loaded (`mcp__swarmdo__aidefence_stats`)
   - swarmvector pinned to 0.2.25 (`npx -y swarmvector@0.2.25 --version`)
