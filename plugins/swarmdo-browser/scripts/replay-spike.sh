...
#   - swarmvector@0.2.25 reachable (npx fetches if missing)
#   - the new browser_session_record / _end / _replay MCP tools wired
#     (v3/@swarmdo/cli/src/mcp-tools/browser-session-tools.ts)
#   - network access
#
# Run from a TTY:
...
site_record_replay() {
  ...
  # #2015: swarmvector@0.2.25's rvf create needs --dimension and does NOT
  # support --kind (unknown option). Strip --kind, keep --dimension 384.
  if ! npx -y swarmvector@0.2.25 rvf create "$rvf" --dimension 384 >/dev/null 2>&1; then
    echo "SKIP:$label rvf-create-failed" ; return 2
  fi
  if ! npx -y swarmvector@0.2.25 hooks trajectory-begin --session-id "$sid" --task "spike-$label" >/dev/null 2>&1; then
  ...
  if ! agent-browser --session "$sid" --json open "$url" >"$RESULTS_DIR/$label.open.json" 2>"$RESULTS_DIR/$label.open.err"; then
    if ! npx --yes agent-browser --session "$sid" --json open "$url" >"$RESULTS_DIR/$label.open.json" 2>"$RESULTS_DIR/$label.open.err"; then
      ...
      npx -y swarmvector@0.2.25 hooks trajectory-end --session-id "$sid" --verdict fail >/dev/null 2>&1 || true
      return 2
    fi
  fi

  npx -y swarmvector@0.2.25 hooks trajectory-step --session-id "$sid" \
  ...
  npx -y swarmvector@0.2.25 hooks trajectory-end --session-id "$sid" --verdict pass >/dev/null 2>&1 || true
  npx -y swarmvector@0.2.25 rvf compact "$rvf" >/dev/null 2>&1 || true

  # 2. Replay: derive a child container, re-run the same step sequence
  local rsid="${sid}-replay"
  local rrvf="$RESULTS_DIR/${rsid}.rvf"
  if ! npx -y swarmvector@0.2.25 rvf derive "$rvf" "$rrvf" >/dev/null 2>&1; then
