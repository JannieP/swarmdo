#!/usr/bin/env bash
# Structural smoke test for swarmdo-browser plugin v0.2.0.
...
step "/swarmdo-browser command covers ls/show/replay/export/fork/purge/doctor"
missing=""
for v in 'ls' 'show' 'replay' 'export' 'fork' 'purge' 'doctor'; do
  grep -qE "\\*\\*$v\b" "$ROOT/commands/swarmdo-browser.md" || missing="$missing $v"
done
...
# 8. Agent uses swarmvector trajectory hooks
step "browser-agent wires swarmvector trajectory-begin/step/end"
missing=""
...
TOOLS_FILE="$ROOT/../../v3/@swarmdo/cli/src/mcp-tools/browser-session-tools.ts"
if [[ ! -f "$TOOLS_FILE" ]]; then
  ...
  if [[ -n "$missing" ]]; then
    ...
    grep -q 'browserSessionTools' "$ROOT/../../v3/@swarmdo/cli/src/mcp-client.ts" \
