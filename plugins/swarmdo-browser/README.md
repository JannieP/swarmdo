...
Session-as-skill browser automation. Playwright-backed via 23 `mcp__swarmdo__browser_*` tools, with each session captured as a first-class **RVF cognitive container** holding manifest + trajectory + screenshots + sanitized cookies + findings, indexed in AgentDB and gated by AIDefence.
...
├── trajectory.ndjson     # one line per action via swarmvector hooks trajectory-step
...
`/swarmdo-browser` is a verb dispatcher:

```bash
/swarmdo-browser ls [--query <text>]      # list sessions, AgentDB-indexed
/swarmdo-browser show <session-id>        # manifest + trajectory + verdict
/swarmdo-browser replay <session-id>      # re-drive trajectory
/swarmdo-browser export <session-id>      # rvf export → tar.zst
/swarmdo-browser fork <session-id>        # rvf derive → new lineage-tracked session
/swarmdo-browser purge <session-id>       # destroy, keep redacted manifest
/swarmdo-browser doctor                   # check Playwright, MCP, AgentDB, AIDefence
```
...
|-----------|-----|-------|---------|
| `browser-sessions` | `<rvf-id>` | manifest summary + verdict + tags | session index for `/swarmdo-browser ls` |
...
23 existing `mcp__swarmdo__browser_*` tools (interaction primitive) **+ 5 new `browser_session_*` lifecycle tools (implemented in v0.2.0)**:

| Tool | Purpose |
|------|---------|
| `browser_session_record` | RVF allocate + swarmvector `trajectory-begin` + `agent-browser open`. Returns session id + rvf path. |
...
Implementation: [`v3/@swarmdo/cli/src/mcp-tools/browser-session-tools.ts`](../../v3/@swarmdo/cli/src/mcp-tools/browser-session-tools.ts), registered in `mcp-client.ts`. Each handler shells out to the pinned `swarmvector@0.2.25` CLI for trajectory + RVF, the existing `agent-browser` CLI for browser actions, and the bridged `swarmdo memory` for AgentDB. Missing dependencies degrade with structured `success: false` errors instead of crashing.
...
```bash
bash plugins/swarmdo-browser/scripts/smoke.sh
...
```bash
bash plugins/swarmdo-browser/scripts/replay-spike.sh
```
...
The spike requires `agent-browser` (or `npx --yes agent-browser`), `swarmvector@0.2.25` (auto-fetched via `npx`), and network access. It is **not** part of the smoke test — running it is a deliberate audit step.

## Architecture Decisions

- [`ADR-0001` — Adopt session-as-skill architecture for swarmdo-browser](./docs/adrs/0001-browser-skills-architecture.md)

## Related Plugins

- `swarmdo-swarmvector` — trajectory hooks, SONA pattern distillation, MCP tools
- `swarmdo-agentdb` — controllers backing `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies`
- `swarmdo-aidefence` — PII / prompt-injection gates
- `swarmdo-federation` — cross-installation session sharing via RVF export
