...
/plugin install swarmdo-goals@swarmdo
```
...
- **Dossier investigation**: Recursive parallel fan-out across all swarmdo sources for seed-driven investigation (ADR-099)
...
- **Verification:** `bash plugins/swarmdo-goals/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin uses six AgentDB namespaces. They predate the namespace convention from [swarmdo-agentdb ADR-0001](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md). [ADR-0001](./docs/adrs/0001-goals-contract.md) documents the legacy-vs-canonical mapping and the forward path:

| Legacy (current) | Canonical (forward) | Status |
|------------------|---------------------|--------|
| `adr` | `adr-patterns` (owned by swarmdo-adr) | Defer to canonical owner — don't write here from this plugin |
...
- [`ADR-0001` — swarmdo-goals plugin contract (legacy-vs-canonical namespaces, GOAP/dossier workflow contract)](./docs/adrs/0001-goals-contract.md)
