...
- `mcp__swarmdo__memory_store` / `memory_search` — store and retrieve plans in `goap-plans` namespace
- `mcp__swarmdo__task_create` / `task_update` — create and track plan steps as tasks
- `mcp__swarmdo__hooks_intelligence_trajectory-start` / `trajectory-step` / `trajectory-end` — record execution trajectories for learning
- `mcp__swarmdo__neural_predict` — predict optimal approaches based on learned patterns
- `mcp__swarmdo__workflow_create` / `workflow_execute` — codify repeatable plans as workflows
