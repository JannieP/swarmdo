...
```bash
claude --plugin-dir plugins/swarmdo-migrations
```
...
- **Verification:** `bash plugins/swarmdo-migrations/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `migrations` AgentDB namespace (kebab-case is implicit when the plugin name is the intent — same documented exception as `federation`). Follows the convention from [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — swarmdo-migrations plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-migrations-contract.md)

## Related Plugins

- `swarmdo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmdo-adr` -- Document schema change decisions as ADRs
- `swarmdo-ddd` -- Align migration boundaries with aggregate roots
- `swarmdo-observability` -- Track migration execution duration and failure rates
