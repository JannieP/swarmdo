...
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation init` -- generate keypair, create config
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation join <endpoint>` -- connect to peer
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation peers` -- list peers with trust levels
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation status` -- health dashboard
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation audit --compliance hipaa` -- audit logs
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation trust <node-id> --review` -- trust breakdown
- `npx -y -p @rufflo/plugin-agent-federation@latest rufflo-federation send <node-id> <msg-type> <payload> [--max-hops N] [--max-tokens N] [--max-usd N]` -- delegate with budget guardrails
...
```bash
npx @rufflo/cli@latest memory store --namespace federation --key "peer-NODEID" --value "TRUST_HISTORY"
```
