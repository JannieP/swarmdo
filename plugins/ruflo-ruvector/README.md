...
Wraps the `ruvector` npm package as a Rufflo plugin, providing vector embedding, semantic search, code-graph clustering, hyperbolic projection, self-learning hooks, and SONA / Brain diagnostics. ruvector's Rust backend delivers sub-millisecond queries and 52,000+ inserts/sec.
...
```bash
claude --plugin-dir plugins/rufflo-ruvector
```
...
- [`ADR-0001` — Pin rufflo-ruvector to ruvector@0.2.25 with optional add-ons](./docs/adrs/0001-pin-ruvector-0.2.25.md)
...
- `rufflo-rag-memory` — Simple semantic search via ruvector
