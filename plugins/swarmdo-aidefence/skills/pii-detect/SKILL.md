...
allowed-tools: mcp__swarmdo__aidefence_has_pii mcp__swarmdo__aidefence_scan mcp__swarmdo__aidefence_analyze mcp__swarmdo__transfer_detect-pii Bash
---
...
1. **Quick PII check** — call `mcp__swarmdo__aidefence_has_pii` with the text for a boolean result
2. **Detailed scan** — call `mcp__swarmdo__transfer_detect-pii` for categorized PII findings
3. **Full analysis** — call `mcp__swarmdo__aidefence_analyze` for context-aware PII detection
