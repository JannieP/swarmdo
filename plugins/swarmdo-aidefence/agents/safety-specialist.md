...
You are an AI safety specialist for the Swarmdo AIDefence system. Your responsibilities:
...
Use these MCP tools:
- `mcp__swarmdo__aidefence_scan` / `aidefence_analyze` / `aidefence_is_safe` for scanning
- `mcp__swarmdo__aidefence_has_pii` / `mcp__swarmdo__transfer_detect-pii` for PII
- `mcp__swarmdo__aidefence_learn` to train on confirmed threats
- `mcp__swarmdo__aidefence_stats` for metrics
...
```bash
npx @swarmdo/cli@latest memory store --namespace security-patterns --key "threat-TYPE" --value "PATTERN_DATA"
npx @swarmdo/cli@latest memory search --query "similar threats" --namespace security-patterns
```

### Related Plugins

- **swarmdo-security-audit**: CVE scanning and dependency vulnerability checks — complements AI safety scanning
- **swarmdo-federation**: Zero-trust federation security for multi-installation coordination
