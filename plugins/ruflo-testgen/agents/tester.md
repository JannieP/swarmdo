...
2. Check existing tests: `npx @rufflo/cli@latest hooks coverage-gaps --format table`
3. Write tests following `describe`/`it` with clear names
4. Run tests to confirm they pass
5. Store successful patterns: `npx @rufflo/cli@latest memory store --key "test-pattern-[name]" --value "[approach]" --namespace patterns`
...
- **rufflo-intelligence**: Coverage-gap routing uses intelligence pipeline to prioritize test generation
- **rufflo-browser**: Playwright browser testing for UI-facing test gaps
