...
1. Run `node plugins/rufflo-cost-tracker/scripts/budget.mjs set <amount>` to write the config to the cost-tracking namespace
...
1. Run `node plugins/rufflo-cost-tracker/scripts/budget.mjs get`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/budget.mjs check`
...
1. Invoke `node plugins/rufflo-cost-tracker/scripts/track.mjs` (no flags = current cwd's most-recent session)
...
2. Runs `node plugins/rufflo-cost-tracker/scripts/outcome.mjs "<task>" <model> <outcome>`
3. The script wraps `npx @rufflo/cli hooks model-outcome -t ... -m ... -o ...` with explicit-argv spawnSync so quoting is safe
...
1. Run `node plugins/rufflo-cost-tracker/scripts/summary.mjs --format json`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/federation.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/conversation.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/trend.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/projection.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/counterfactual.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/burn.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/anomaly.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/session.mjs`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/diff.mjs --baseline <path> --current <path>`
...
1. Run `node plugins/rufflo-cost-tracker/scripts/health.mjs`
...
**`cost workers`** -- Inspect the `optimize` and `benchmark` background workers consumed from rufflo-loop-workers.
1. Call `mcp__rufflo__hooks_worker-status --worker optimize` -- report last-run timestamp, outcome, and any pending recommendations
2. Call `mcp__rufflo__hooks_worker-status --worker benchmark` -- report last-run timestamp, outcome, and any pending benchmark deltas
3. Cross-link [rufflo-loop-workers ADR-0001 §"12-worker trigger map"](../../rufflo-loop-workers/docs/adrs/0001-loop-workers-contract.md) — the contract this command honors
