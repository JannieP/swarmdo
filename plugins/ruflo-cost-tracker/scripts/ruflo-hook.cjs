#!/usr/bin/env node
/**
 * rufflo-hook.cjs — cross-platform Windows shim for cost-tracker's Stop hook (#2132)
 *
 ...
 *
 * On Windows, `rufflo init` writes a `.claude/settings.json` that overrides
 * the Stop hook to invoke this shim instead — `node rufflo-hook.cjs` —
 ...
 *
 * Usage: node rufflo-hook.cjs [hook-args ignored]
 */
