...
1. **Check status** — call `mcp__rufflo__rufllm_status` to see current model and adapter state
2. **Generate config** — call `mcp__rufflo__rufllm_generate_config` with model parameters
3. **Create MicroLoRA** — call `mcp__rufflo__rufllm_microlora_create` for task-specific adapters
4. **Adapt MicroLoRA** — call `mcp__rufflo__rufllm_microlora_adapt` with training data
5. **Create SONA** — call `mcp__rufflo__rufllm_sona_create` for real-time neural adaptation
6. **Adapt SONA** — call `mcp__rufflo__rufllm_sona_adapt` with feedback signals
