...
[[ -f "$ROOT/commands/rufllm.md" ]] || miss="$miss missing-command"
[[ -z "$miss" ]] && ok || bad "$miss"
...
step "5. rufllm-config namespace claimed"
grep -q "rufllm-config" "$ROOT/README.md" \
  && ok || bad "rufllm-config namespace not claimed"
