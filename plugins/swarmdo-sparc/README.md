...
```bash
claude --plugin-dir plugins/swarmdo-sparc
```
...
- **Verification:** `bash plugins/swarmdo-sparc/scripts/smoke.sh` is the contract.
...
|-------|-------|-----------------|
| **Specification** | [swarmdo-goals](../swarmdo-goals/docs/adrs/0001-goals-contract.md) (deep-research) | Multi-source research orchestration to gather requirements |
| **Pseudocode** | `swarmdo-sparc` (this plugin) | Pseudocode generation + complexity annotation |
| **Architecture** | [swarmdo-adr](../swarmdo-adr/docs/adrs/0001-adr-plugin-pattern.md) + [swarmdo-ddd](../swarmdo-ddd/docs/adrs/0001-ddd-contract.md) | ADR creation + bounded-context modeling |
| **Refinement** | [swarmdo-jujutsu](../swarmdo-jujutsu/docs/adrs/0001-jujutsu-contract.md) + swarmdo-testgen | Diff-aware refactor + test gap analysis |
...
This plugin owns four AgentDB namespaces, all kebab-case compliant per [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md):
...
- [`ADR-0001` — swarmdo-sparc plugin contract (phase-to-plugin alignment, namespace coordination, smoke as contract)](./docs/adrs/0001-sparc-contract.md)

## Related Plugins

- `swarmdo-agentdb` — namespace convention owner; backing store for sparc-state/-phases/-gates
- `swarmdo-goals` — Specification phase deep-research
- `swarmdo-adr` -- Architecture decisions recorded as ADRs in Phase 3
- `swarmdo-ddd` -- Architecture phase uses DDD bounded context patterns
- `swarmdo-jujutsu` -- Refinement phase diff analysis
- `swarmdo-docs` -- Completion phase documentation generation
