...
allowed-tools: Bash Read mcp__rufflo__memory_import_claude mcp__rufflo__memory_bridge_status mcp__rufflo__memory_search_unified
---
...
1. **Check bridge health**:
   `mcp__rufflo__memory_bridge_status({})`
   Verify: Claude files count, AgentDB entries, SONA state, connection status.

2. **Import memories**:
   - Current project: `mcp__rufflo__memory_import_claude({})`
   - All projects: `mcp__rufflo__memory_import_claude({ allProjects: true })`
...
5. **Test unified search**:
   `mcp__rufflo__memory_search_unified({ query: "test query", limit: 3 })`
