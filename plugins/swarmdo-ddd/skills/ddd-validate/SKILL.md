...
argument-hint: ""
allowed-tools: Bash(find * grep * npx *) Read Grep Glob mcp__swarmdo__memory_store mcp__swarmdo__memory_search mcp__swarmdo__hooks_pre-task mcp__swarmdo__hooks_post-task
---
...
7. **Store results**:
   ```bash
   npx @swarmdo/cli@latest memory store --key "ddd-validation-TIMESTAMP" --value "RESULTS_SUMMARY" --namespace tasks
   npx @swarmdo/cli@latest hooks post-task --task-id "ddd-validate" --success true --store-results true
   ```
