...
```bash
claude --plugin-dir plugins/swarmdo-ddd
```
...
- **Verification:** `bash plugins/swarmdo-ddd/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `ddd-patterns` AgentDB namespace (kebab-case, follows the convention from [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — swarmdo-ddd plugin contract](./docs/adrs/0001-ddd-contract.md)

## Related Plugins

- `swarmdo-agentdb` — namespace convention owner; backing store for the domain graph
- `swarmdo-adr` -- Document domain decisions as Architecture Decision Records
- `swarmdo-sparc` -- Architecture phase leverages DDD bounded context patterns
- `swarmdo-migrations` -- Align migration boundaries with aggregate roots
