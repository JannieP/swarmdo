...
/plugin install rufflo-rvf@rufflo
```
...
Confirm the gate state with `rufflo doctor -c encryption`.
...
- **Verification:** `bash plugins/rufflo-rvf/scripts/smoke.sh` is the contract.
...
This plugin owns the `rvf-sessions` AgentDB namespace (kebab-case, follows the convention from [rufflo-agentdb ADR-0001 §"Namespace convention"](../rufflo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — rufflo-rvf plugin contract (cross-plugin RVF ownership table, namespace coordination, smoke as contract)](./docs/adrs/0001-rvf-contract.md)
