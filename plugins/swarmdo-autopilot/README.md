...
Combines Swarmdo's 10 autopilot MCP tools with Claude Code's native `/loop` + `ScheduleWakeup` for persistent, cache-aware task completion loops.
...
All 10 are wired in `v3/@swarmdo/cli/src/mcp-tools/autopilot-tools.ts`.

## Compatibility

- **CLI:** pinned to `@swarmdo/cli` v3.6 major+minor.
- **MCP surface:** the 10 tools above.
- **Verification:** `bash plugins/swarmdo-autopilot/scripts/smoke.sh` is the contract.
...
This plugin owns the `autopilot-patterns` AgentDB namespace (kebab-case, follows the convention from [swarmdo-agentdb ADR-0001 §"Namespace convention"](../swarmdo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.

`autopilot_learn` writes to this namespace via `agentdb_pattern-store` semantics — see [swarmdo-intelligence ADR-0001](../swarmdo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md) for the 4-step pipeline this feeds (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE).
...
- [`ADR-0001` — swarmdo-autopilot plugin contract](./docs/adrs/0001-autopilot-contract.md)
