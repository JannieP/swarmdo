#!/usr/bin/env node
/**
 * Smoke test for ADR-131 / upstream/swarmdo#2149 — ToolOutputGuardrail wired up
 * end-to-end through the published `@swarmdo/security` build output.
 *
...
const SECURITY_DIST = join(REPO_ROOT, 'v3/@swarmdo/security/dist/index.js');

if (!existsSync(SECURITY_DIST)) {
  console.error(`[smoke] FAIL: built artifact missing at ${SECURITY_DIST}`);
  console.error('[smoke] Run `cd v3/@swarmdo/security && npm run build` first.');
  process.exit(1);
}
...
if (failed > 0) {
  ...
  console.error('Reference: ADR-131, upstream/swarmdo#2149');
  process.exit(1);
}
