#!/usr/bin/env node
/**
 * Smoke test for upstream/swarmdo#2132 — Windows end-to-end hook execution.
 *
...
// Step 1: Generate settings.json via swarmdo init
const tmpDir = mkdtempSync(join(tmpdir(), 'swarmdo-smoke-hook-exec-'));
...
if (postEditCmd) {
  ...
  const env = {
    ...
    // swarmdo@latest` — that takes 30+s on a cold CI runner and exceeds
