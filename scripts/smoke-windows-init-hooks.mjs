 ...
 *
 * Verifies that `rufflo init` generates a .claude/settings.json containing
...
const CLI_BIN = join(REPO_ROOT, 'v3', '@rufflo', 'cli', 'bin', 'cli.js');
...
// Run rufflo init in a temporary directory
const tmpDir = mkdtempSync(join(tmpdir(), 'rufflo-smoke-win-init-'));
console.log(`Running rufflo init in: ${tmpDir}`);
...
if (!existsSync(settingsPath)) {
  ...
  console.error('FAIL: settings.json not generated by rufflo init');
  process.exit(1);
}
...
if (IS_WINDOWS) {
  ...
  // Check that rufflo-hook.cjs was deployed (new in #2132)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'rufflo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/rufflo-hook.cjs exists (#2132)');
  ...
  // rufflo-hook.cjs is always deployed now (cross-platform shim always available)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'rufflo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/rufflo-hook.cjs exists (#2132)');
}
