...
const CLI_BIN = join(REPO_ROOT, 'v3', '@swarmdo', 'cli', 'bin', 'cli.js');
...
// Run swarmdo init in a temporary directory
const tmpDir = mkdtempSync(join(tmpdir(), 'swarmdo-smoke-win-init-'));
console.log(`Running swarmdo init in: ${tmpDir}`);
...
if (!existsSync(settingsPath)) {
  ...
  console.error('FAIL: settings.json not generated by swarmdo init');
  process.exit(1);
}
...
if (IS_WINDOWS) {
  ...
  // Check that swarmdo-hook.cjs was deployed (new in #2132)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'swarmdo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/swarmdo-hook.cjs exists (#2132)');
  ...
  // swarmdo-hook.cjs is always deployed now (cross-platform shim always available)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'swarmdo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/swarmdo-hook.cjs exists (#2132)');
}
