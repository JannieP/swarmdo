...
// ============================================================================
// SwarmVector Config Tests
// ============================================================================

describe('getSwarmVectorConfig', () => {
  it('should return null when no env vars set', () => {
    // Save and clear env vars
    const saved = { ...process.env };
    delete process.env.SWARMVECTOR_HOST;
    delete process.env.SWARMVECTOR_DATABASE;
    delete process.env.SWARMVECTOR_USER;
    delete process.env.PGHOST;
    delete process.env.PGDATABASE;
    delete process.env.PGUSER;

    const config = getSwarmVectorConfig();
  ...
  it('should parse config from SWARMVECTOR_* env vars', () => {
    const saved = { ...process.env };
    process.env.SWARMVECTOR_HOST = 'pg.example.com';
    process.env.SWARMVECTOR_PORT = '5433';
    process.env.SWARMVECTOR_DATABASE = 'claude_flow';
    process.env.SWARMVECTOR_USER = 'admin';
    process.env.SWARMVECTOR_PASSWORD = 'secret123';
    process.env.SWARMVECTOR_SSL = 'true';
    ...
    // Cleanup
    delete process.env.SWARMVECTOR_HOST;
    delete process.env.SWARMVECTOR_PORT;
    delete process.env.SWARMVECTOR_DATABASE;
    delete process.env.SWARMVECTOR_USER;
    delete process.env.SWARMVECTOR_PASSWORD;
    delete process.env.SWARMVECTOR_SSL;
    Object.assign(process.env, saved);
  });
...
// ============================================================================
// SwarmVectorBackend Class Tests (mock-based, no real PostgreSQL)
// ============================================================================

describe('SwarmVectorBackend', () => {
  it('should be exported and constructable', () => {
    assert.ok(SwarmVectorBackend);
    const backend = new SwarmVectorBackend({
...
describe('resolveBackend priority', () => {
  ...
  it('should not resolve swarmvector when env vars are absent', () => {
    const config = getSwarmVectorConfig();
    assert.equal(config, null);
  });
});
...
describe('autoOptimize', () => {
  it('should prune stale entries during optimization', async () => {
    ...
    assert.equal(result.synced, 0); // No SwarmVector configured
