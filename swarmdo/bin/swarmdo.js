#!/usr/bin/env node
// Swarmdo CLI - thin wrapper around @swarmdo/cli with swarmdo branding
...
// downstream @swarmdo/cli dist eagerly loads swarmvector + a 23 MB ONNX
...
// Walk up from swarmdo/bin/ to find @swarmdo/cli in node_modules
function findCliPath() {
  let dir = resolve(__dirname, '..');
  for (let i = 0; i < 10; i++) {
    const candidate = join(dir, 'node_modules', '@swarmdo', 'cli', 'bin', 'cli.js');
...
const cliBase = pkgDir
  ? join(pkgDir, 'node_modules', '@swarmdo', 'cli')
  : resolve(__dirname, '../../v3/@swarmdo/cli');
...
if (isMCPMode) {
  ...
  // CLI mode: use swarmdo branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'swarmdo',
    description: 'Swarmdo - AI Agent Orchestration Platform',
  });
  cli.run()
    .then(() => {
