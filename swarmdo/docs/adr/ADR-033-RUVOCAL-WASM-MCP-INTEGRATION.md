...
- Misc: `Modal.svelte`, `NavMenu.svelte`, `RuffloUniverse.svelte`, `Switch.svelte`, `WelcomeModal.svelte`, `Logo.svelte`, `mcpExamples.ts`, `.gitignore`, `rvf.manifest.json`

**Local-only (must preserve):**
- `mcp-bridge/index.js` — local MCP bridge implementation (absent in upstream)
- `src/routes/api/v2/debug/` — debug routes used by rufflo
- `stub/@reflink/reflink/index.js` — reflink stub
- `.env` — populated local environment
- `package-lock.json` — local lockfile
- All rufflo-specific docs/CLAUDE.md guidance

`package.json` is identical between local and upstream — no dependency changes required.

## Decision

We will pull the upstream improvements into `rufflo/src/ruvocal/` on a dedicated feature branch (`feat/ruvocal-wasm-mcp-integration`) using a **directory-level overlay strategy** rather than a Git merge, because the local snapshot has no shared history with the upstream repository.
...
3. **Verification** — `npm install`, `npm run check`, `npm run build`, then local docker compose + `rufflo-browser` smoke test.
...
- No changes to `rufflo/src/chat-ui/` (the thin HF base-image wrapper) in this ADR.
...
- Changes to `Settings.ts` / `Tool.ts` types could ripple into rufflo packages that import from `src/ruvocal`. Mitigation: run `npm run check` before merging.
...
- [ ] `rufflo-browser` smoke test: load the home page, open the gallery panel, send a message through a non-WASM model, confirm no console errors.
...
Cloud Run deployment is **out of scope for this PR** but the path is staged in `rufflo/src/ruvocal/cloudbuild.yaml`. Two infrastructure prerequisites must be satisfied before the first deploy:
...
The thin `rufflo/src/chat-ui/Dockerfile` wrapper (FROM `ghcr.io/huggingface/chat-ui-db:latest`) is **unsuitable** for deploying this integration — it can only patch the upstream HF base image with a few static files; it cannot include compiled WASM source. The full ruvocal Dockerfile build is required.
...
**Fix:** `rufflo/src/ruvocal/.gcloudignore` (commit `e3b74f606`). All `/api/v2/*` routes now serve correctly. Verified via `gsutil cp` of the build's source tarball that all `+server.ts` files for the models subtree are present after the fix.
...
- **Help icon** in sidebar opens "Rufflo Capabilities" modal (`src/lib/components/RuffloHelpModal.svelte`) with all 6 tool groups, quick-start, tips, and resource links.
- **8 Rufflo-themed example prompts** on the welcome screen (`src/lib/constants/{mcpExamples,routerExamples}.ts`) replacing HF defaults.
- **`PUBLIC_APP_NAME=Rufflo`** brands the title bar, sidebar, welcome modal, and PWA manifest.
...
| **P3** | **Help-modal copy fix** | Quick Start section says "default: Gemini 2.5 Flash". Update to "default: Claude Sonnet 4.6" in `src/lib/components/RuffloHelpModal.svelte`. |
...
- Local target: `rufflo/src/ruvocal/`
...
- Cloud Build configs: `rufflo/src/ruvocal/cloudbuild.yaml`, `rufflo/src/ruvocal/mcp-bridge/cloudbuild.yaml`
- `.gcloudignore` (CRITICAL — see resolved root cause): `rufflo/src/ruvocal/.gcloudignore`
- Web Worker: `rufflo/src/ruvocal/src/lib/wasm/{wasm.worker.ts,workerClient.ts}`
- Help modal: `rufflo/src/ruvocal/src/lib/components/RuffloHelpModal.svelte`
- Live URLs: https://flo.ruv.io/ · https://rufflo.ruv.io/ · https://ruvocal.ruv.io/
