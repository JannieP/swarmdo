...
class StdioMcpClient {
  ...
  _send(method, params) {
    return new Promise((resolve, reject) => {
      ...
      // initialize is the cold-start gate for backends like swarmdo/swarmvector
      // which boot a full swarmdo / swarmvector kernel — on Cloud Run with
...
Tools prefixed with backend name (e.g., \`swarmdo__agent_spawn\`). Always use the full prefixed name.
...
- **guidance** — Get help on tool groups or specific tool usage.
  ...
  \`{"topic": "overview"}\` or \`{"topic": "tool", "tool_name": "swarmdo__agent_spawn"}\`
...
7. **Code task** → \`swarmvector__hooks_route\` + \`swarmdo__agent_spawn\`
8. **Remember something** → \`swarmdo__memory_store\` / \`swarmdo__memory_search\`
...
async function handleAutopilot(req, res, provider, body) {
  ...
  const priorityPrefixes = ['swarmvector__hooks', 'swarmvector__memory', 'swarmdo__memory', 'swarmdo__agent'];
