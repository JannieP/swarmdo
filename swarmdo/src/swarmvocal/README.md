...
## What SwarmVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five Swarmdo server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **Swarmdo Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
# Curated Swarmdo model list (optional — defaults to /models from the base URL)
TASK_MODEL=qwen/qwen3.6-max-preview
PUBLIC_APP_NAME=Swarmdo
...
```bash
docker build -t swarmvocal --build-arg INCLUDE_DB=true .
docker run -p 3000:3000 \
  -e OPENAI_BASE_URL=https://openrouter.ai/api/v1 \
  -e OPENAI_API_KEY=sk-or-v1-... \
  -v swarmvocal-data:/data \
  swarmvocal
```
...
- **Local container** — `docker run -d -p 27017:27017 --name mongo-swarmvocal mongo:latest` then `MONGODB_URL=mongodb://localhost:27017`.

`MONGODB_DB_NAME` defaults to `chat-ui` (kept for upstream compatibility); change per environment.

## MCP Tools (the Swarmdo difference)

SwarmVocal calls tools exposed by Model Context Protocol servers and feeds results back to the model via OpenAI function calling. Configure trusted servers via env, let users add their own, and the router auto-selects tools-capable models when needed.

```env
MCP_SERVERS=[
  {"name":"Swarmdo Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"Swarmdo Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
...
SwarmVocal can do server-side smart routing using [katanemo/Arch-Router-1.5B](https://huggingface.co/katanemo/Arch-Router-1.5B) without a separate router service. Selecting "Omni" in the model picker:
...
- **Capabilities modal** — `src/lib/components/SwarmdoHelpModal.svelte`
- **Dynamic follow-ups** — tool-call-aware suggested next prompts in `ChatWindow.svelte`

For deeper internals, see [`CLAUDE.md`](./CLAUDE.md) (Claude Code agent guide) and [ADR-033](../../docs/adr/ADR-033-SWARMVOCAL-WASM-MCP-INTEGRATION.md).
