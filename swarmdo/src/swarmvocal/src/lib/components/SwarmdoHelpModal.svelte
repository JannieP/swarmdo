<script lang="ts">
	...
	const groups: Group[] = [
		{
			...
			tools: [
				{ name: "swarmdo__memory_store", desc: "Save a key/value into a namespace." },
				{ name: "swarmdo__memory_search", desc: "Semantic vector search over stored memories." },
				{ name: "swarmdo__memory_retrieve", desc: "Fetch a specific entry by key + namespace." },
				{ name: "swarmdo__memory_list", desc: "List entries in a namespace." },
				{ name: "swarmdo__embeddings_compare", desc: "Compare two pieces of text by semantic similarity." },
				{ name: "swarmdo__agentdb_*", desc: "Lower-level AgentDB controllers (route, consolidate, hierarchical)." },
			],
		},
		{
			id: "agents",
			...
			tools: [
				{ name: "swarmdo__agent_spawn", desc: "Create a new specialized agent (coder, tester, reviewer, security-auditor, …)." },
				{ name: "swarmdo__swarm_init", desc: "Initialize a swarm with topology + strategy + max agents." },
				{ name: "swarmdo__hive-mind_*", desc: "Queen-led Byzantine fault-tolerant collective." },
				{ name: "swarmdo__task_create / task_assign / task_status", desc: "Full task lifecycle." },
				{ name: "swarmdo__agent_list / agent_status", desc: "Inspect active agents and their state." },
			],
		},
		{
			id: "intelligence",
			...
			emoji: "✨",
			intro: "Pattern learning, model routing, code analysis, and trajectory tracking via SwarmVector. Tools here improve future task execution by recording what worked.",
			tools: [
				{ name: "swarmvector__hooks_route", desc: "Pick the optimal agent type for a task." },
				{ name: "swarmvector__hooks_remember / recall", desc: "Cross-session key/value memory shared with agents." },
				{ name: "swarmvector__hooks_trajectory_begin / step / end", desc: "Record multi-step task execution so the system learns from the run." },
				{ name: "swarmvector__hooks_security_scan", desc: "Scan code for vulnerabilities." },
				{ name: "swarmvector__hooks_rag_context", desc: "Get retrieval-augmented context for a query." },
				{ name: "swarmvector__hooks_swarm_recommend", desc: "Recommend swarm topology + agent mix for a task." },
			],
		},
		{
			id: "devtools",
			...
			tools: [
				{ name: "swarmdo__system_status", desc: "Overall system health overview." },
				{ name: "swarmdo__performance_metrics / bottleneck / report", desc: "Detailed performance and hotspot analysis." },
				{ name: "swarmdo__analyze_diff*", desc: "Risk-score, classify, and suggest reviewers for a code diff." },
				{ name: "swarmdo__github_repo_analyze / pr_manage / issue_track", desc: "Repository metrics and PR/issue ops." },
				{ name: "swarmdo__terminal_execute / terminal_create", desc: "Run shell commands and manage sessions." },
				{ name: "swarmdo__progress_*", desc: "Implementation progress tracking across long-horizon work." },
			],
		},
		{
			id: "core",
	...
	const tips = [
		...
		'**AUTO toggle**: green/lit means tool chains continue automatically. Gray means Swarmdo stops after each tool result so you can inspect before continuing.',
		...
		'**Trajectory mode** — for long multi-step work, prefix with "Use `swarmvector__hooks_trajectory_begin`" so the system learns from the run for future routing.',
		'**Tool name prefixes** — `swarmdo__` is the agent/memory backend, `swarmvector__` is the intelligence layer. Always use the full prefixed name when invoking explicitly.',
	];

	const examplePrompts = [
		'"Use swarmdo__memory_store namespace=prefs key=editor value=vim then swarmdo__memory_retrieve to confirm."',
		'"Run swarmdo__system_status, swarmdo__performance_metrics, and swarmdo__memory_list in parallel and summarize."',
	...
	const stack: { name: string; role: string }[] = [
		...
		{ name: "swarmdo backend (npx swarmdo mcp start)", role: "Agents, memory, swarm, devtools (~158 tools)" },
		{ name: "swarmvector backend (npx swarmvector mcp start)", role: "Intelligence, routing, trajectories (~49 tools)" },
		...
		{ name: "Cloud Run (us-central1)", role: "Both swarmvocal chat-ui and mcp-bridge services with min-instance=1 warm" },
	];
	...
	<div class="flex max-h-[88vh] w-full flex-col overflow-hidden rounded-2xl bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100">
		<header class="flex items-start justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
			<div class="flex items-start gap-3">
				...
				<div>
					<h2 class="text-lg font-semibold">Swarmdo Capabilities</h2>
		...
		<div class="flex-1 overflow-y-auto px-6 py-4">
			<section class="mb-6">
				...
				<ol class="list-decimal space-y-1.5 pl-5 text-sm">
					...
					<li>Swarmdo decides which tools to call. Watch the streaming tool-call cards below your message — multiple cards = parallel execution.</li>
			...
			<section class="mb-6">
				...
				<p class="mb-2 text-xs text-gray-600 dark:text-gray-400">Swarmdo's web UI is a fork of the open-source HuggingFace chat-ui (SvelteKit) wired to a custom MCP bridge that fans out to two backend kernels. Everything runs on Google Cloud Run.</p>
				<div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
					<table class="w-full text-xs">
						<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
							{#each stack as layer}
