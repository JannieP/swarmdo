/**
 * PostgreSQL adapter for SwarmVocal — drop-in replacement for MongoDB collections.
 *
...
export function getPool(): pg.Pool {
	if (!pool) {
		const connectionString =
			...
			"postgresql://swarmvocal:swarmvocal@localhost:5432/swarmvocal";
		pool = new Pool({
			connectionString,
			max: 20,
...
export class PostgresCollection<T extends Record<string, unknown>> {
	...
	// SwarmVector extension: semantic search via pgvector
