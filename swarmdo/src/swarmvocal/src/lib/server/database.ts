/**
 * SwarmVocal Database — self-contained RVF document store.
 *
...
export class Database {
	...
	private async init() {
		...
		const dbPath = join(dbFolder, "swarmvocal.rvf.json");

		console.log(`[SwarmVocal] Database: ${dbPath}`);
		initRvfStore(dbPath);
		this.initialized = true;

		// Flush to disk on exit
		onExit(async () => {
			console.log("[SwarmVocal] Flushing database to disk");
			flushToDisk();
		});
	}
