...
const TOOL_GROUPS = {
  ...
  // --- Intelligence (swarmvector) ---
  intelligence: {
    enabled: process.env.MCP_GROUP_INTELLIGENCE !== "false",
    description: "Self-learning intelligence — routing, memory, pattern training (swarmvector)",
    source: "swarmvector",
    prefixes: ["hooks_"],
  },

  // --- Agents & Orchestration (swarmdo) ---
  agents: {
    enabled: process.env.MCP_GROUP_AGENTS !== "false",
    description: "Agent lifecycle, swarm coordination, task management, workflows (swarmdo)",
    source: "swarmdo",
    prefixes: ["agent_", "swarm_", "task_", "session_", "hive-mind_", "workflow_", "coordination_"],
  },

  // --- Memory & Knowledge (swarmdo) ---
  memory: {
    enabled: process.env.MCP_GROUP_MEMORY !== "false",
    description: "Vector memory, AgentDB, embeddings, semantic search (swarmdo)",
    source: "swarmdo",
  ...
  devtools: {
    ...
    description: "Hooks, code analysis, performance profiling, GitHub integration (swarmdo)",
    source: "swarmdo",
    prefixes: ["hooks_", "analyze_", "performance_", "github_", "terminal_", "config_", "system_", "progress_"],
  },

  // --- Security & Safety (swarmdo) ---
  security: {
    enabled: process.env.MCP_GROUP_SECURITY === "true",
    description: "AI defence, PII detection, claims management, pattern transfer (swarmdo)",
    source: "swarmdo",
    prefixes: ["aidefence_", "claims_", "transfer_"],
  },

  // --- Browser Automation (swarmdo) ---
  browser: {
    enabled: process.env.MCP_GROUP_BROWSER === "true",
    description: "Headless browser control — navigate, click, fill, screenshot (swarmdo)",
    source: "swarmdo",
    prefixes: ["browser_"],
  },
  ...
  neural: {
    ...
    description: "Neural network training, DAA autonomous agents, cognitive patterns (swarmdo)",
    source: "swarmdo",
...
const BACKEND_DEFS = [
  { name: "swarmvector",       command: "npx", args: ["-y", "swarmvector", "mcp", "start"],   groups: ["intelligence"] },
  { name: "swarmdo",          command: "npx", args: ["-y", "swarmdo", "mcp", "start"],      groups: ["agents", "memory", "devtools", "security", "browser", "neural"] },
...
4. Call \`guidance\` with a specific group name to learn how to use that group's tools`,
  ...
  const groupGuides = {
    intelligence: `# Intelligence Group (swarmvector)

Self-learning intelligence tools for routing and vector memory.

## Key Tools
- **swarmvector__hooks_route** — Route a task to the best agent type. Call with a task description.
- **swarmvector__hooks_remember** — Store context/knowledge in vector memory for later recall.
- **swarmvector__hooks_recall** — Search vector memory semantically. Good for finding past context.
- **swarmvector__hooks_pretrain** — Bootstrap intelligence from a code repository.
- **swarmvector__hooks_build_agents** — Generate optimized agent configurations.
- **swarmvector__hooks_stats** — Get intelligence statistics and learning metrics.
...
- To recall previously stored patterns or solutions`,
    ...
    agents: `# Agents & Orchestration Group (swarmdo)

Multi-agent lifecycle management, swarm coordination, and task workflows.

## Key Tools
- **swarmdo__agent_spawn** — Create a new agent with specific capabilities
- **swarmdo__agent_list** — List all active agents
- **swarmdo__swarm_init** — Initialize a swarm with a topology (mesh, hierarchical, ring, star)
- **swarmdo__task_create** — Create and assign tasks
- **swarmdo__workflow_create** — Define multi-step workflows
- **swarmdo__workflow_execute** — Execute a workflow
- **swarmdo__hive-mind_init** — Start collective intelligence coordination
- **swarmdo__coordination_orchestrate** — Multi-agent coordination
...
- Distributed task management`,
    ...
    memory: `# Memory & Knowledge Group (swarmdo)

Vector storage, semantic search, AgentDB pattern learning, and embeddings.

## Key Tools
- **swarmdo__memory_store** — Store a value with vector embedding for semantic search
- **swarmdo__memory_search** — Semantic search across stored memories (HNSW-indexed)
- **swarmdo__memory_list** — List stored memory entries
- **swarmdo__agentdb_pattern-store** — Store a reasoning pattern for learning
- **swarmdo__agentdb_pattern-search** — Search for similar reasoning patterns
- **swarmdo__agentdb_context-synthesize** — Synthesize context from stored memories
- **swarmdo__embeddings_generate** — Generate vector embeddings for text
- **swarmdo__embeddings_search** — Semantic similarity search
...
- Building semantic search over custom data`,
    ...
    devtools: `# Dev Tools Group (swarmdo)

Code analysis, performance profiling, GitHub integration, and terminal access.

## Key Tools
- **swarmdo__analyze_diff** — Analyze git diff for risk and change classification
- **swarmdo__performance_benchmark** — Run performance benchmarks
- **swarmdo__performance_bottleneck** — Detect performance bottlenecks
- **swarmdo__github_repo_analyze** — Analyze a GitHub repository
- **swarmdo__github_pr_manage** — Manage pull requests
- **swarmdo__terminal_execute** — Execute commands in a terminal session
...
- GitHub repository management`,
    ...
    security: `# Security & Safety Group (swarmdo)

AI defence, PII detection, and claims-based authorization.

## Key Tools
- **swarmdo__aidefence_scan** — Scan text for AI manipulation attempts
- **swarmdo__aidefence_has_pii** — Check for PII (emails, phones, SSNs)
- **swarmdo__aidefence_is_safe** — Quick safety check on input
- **swarmdo__claims_claim** — Claim an issue for work
- **swarmdo__claims_board** — Visual board of all claims
...
- Work item management across agents`,
    ...
    browser: `# Browser Automation Group (swarmdo)

Headless browser control for web interaction and testing.

## Key Tools
- **swarmdo__browser_open** — Navigate to a URL
- **swarmdo__browser_click** — Click elements by reference
- **swarmdo__browser_fill** — Fill form inputs
- **swarmdo__browser_screenshot** — Capture page screenshots
- **swarmdo__browser_snapshot** — Get accessibility tree for AI parsing
- **swarmdo__browser_eval** — Execute JavaScript in page context
...
- Form filling and web interaction`,
    ...
    neural: `# Neural & DAA Group (swarmdo)

Neural network operations and Decentralized Autonomous Agents.

## Key Tools
- **swarmdo__neural_train** — Train a neural model
- **swarmdo__neural_predict** — Make predictions
- **swarmdo__daa_agent_create** — Create an autonomous agent
- **swarmdo__daa_workflow_create** — Create autonomous workflows
- **swarmdo__daa_knowledge_share** — Share knowledge between agents
...
Your tools are organized into groups. Each tool name is prefixed with its backend (e.g., \`swarmdo__agent_spawn\`, \`swarmvector__hooks_route\`). Always use the full prefixed name when calling tools.
...
- **guidance** — Get help on any tool group, specific tool usage, or capabilities overview.
  ...
  For specific tool help: \`guidance(topic='tool', tool_name='swarmdo__agent_spawn')\`

## Group 2: Intelligence & Learning (swarmvector)
...
- **swarmvector__hooks_route** — Route a task to the optimal agent type. Call this FIRST for complex tasks.
  \`{"task": "describe what needs to be done", "context": ["relevant info"]}\`
  Returns ranked agent recommendations with confidence scores.
- **swarmvector__hooks_remember** — Store a key-value pair in persistent memory for cross-session recall.
  \`{"key": "pattern-name", "value": "what to remember", "namespace": "patterns"}\`
- **swarmvector__hooks_recall** — Retrieve a previously stored memory by key.
- **swarmvector__hooks_suggest_context** — Get contextual suggestions based on current work.
- **swarmvector__hooks_swarm_recommend** — Get swarm topology recommendation for a task type.
- **swarmvector__hooks_capabilities** — List all intelligence system capabilities.

### Code Analysis:
- **swarmvector__hooks_ast_analyze** — Analyze code structure (AST) of a file.
- **swarmvector__hooks_ast_complexity** — Get complexity metrics for code.
- **swarmvector__hooks_security_scan** — Scan code for security vulnerabilities.
- **swarmvector__hooks_diff_analyze** — Analyze a code diff for risk and impact.
- **swarmvector__hooks_diff_similar** — Find similar past diffs/changes.

### Trajectory Learning (for multi-step tasks):
- **swarmvector__hooks_trajectory_begin** — Start tracking a multi-step task for learning.
- **swarmvector__hooks_trajectory_step** — Record a step in the current trajectory.
- **swarmvector__hooks_trajectory_end** — End trajectory, triggering pattern extraction.

### Memory & Compression:
- **swarmvector__hooks_compress** — Compress/summarize long text for efficient storage.
- **swarmvector__hooks_rag_context** — Get RAG context for a query from stored knowledge.
- **swarmvector__hooks_learn** — Force the system to learn from provided examples.
- **swarmvector__hooks_batch_learn** — Learn from multiple examples at once.
- **swarmvector__hooks_stats** — View learning statistics and metrics.
- **swarmvector__hooks_doctor** — Run diagnostics on the intelligence system.

## Group 3: Agents & Orchestration (swarmdo)
Spawn agents, coordinate swarms, manage tasks and workflows. ${TOOL_GROUPS.agents.enabled ? "ACTIVE" : "DISABLED"}

### Agent Lifecycle:
- **swarmdo__agent_spawn** — Create a new specialized agent.
...
- **swarmdo__agent_status** — Check an agent's current state. \`{"agentId": "agent-xxx"}\`
- **swarmdo__agent_list** — List all active agents with their states.
- **swarmdo__agent_terminate** — Stop an agent. \`{"agentId": "agent-xxx"}\`
- **swarmdo__agent_health** — Health check across all agents.
- **swarmdo__agent_pool** — View the agent pool and available capacity.

### Swarm Coordination:
- **swarmdo__swarm_init** — Initialize a multi-agent swarm.
...
- **swarmdo__swarm_status** — Get swarm health, topology, and agent states.
- **swarmdo__swarm_health** — Detailed health metrics for the swarm.
- **swarmdo__swarm_shutdown** — Tear down a swarm and all its agents.

### Task Management:
- **swarmdo__task_create** — Create a tracked task.
  \`{"description": "what needs to be done", "priority": "low|normal|high|critical"}\`
- **swarmdo__task_status** — Check task progress. \`{"taskId": "task-xxx"}\`
- **swarmdo__task_list** — List all tasks with their statuses.
- **swarmdo__task_complete** — Mark a task as done. \`{"taskId": "task-xxx"}\`
- **swarmdo__task_update** — Update task details, status, or assignment.
- **swarmdo__task_cancel** — Cancel a task.

### Workflow Orchestration:
- **swarmdo__workflow_create** — Define a multi-step workflow with dependencies.
- **swarmdo__workflow_execute** — Run a workflow. \`{"workflowId": "wf-xxx"}\`
- **swarmdo__workflow_status** — Check workflow progress.
- **swarmdo__workflow_template** — Use a pre-built workflow template.
- **swarmdo__workflow_pause** / **swarmdo__workflow_resume** — Control workflow execution.

### Hive-Mind (Distributed Consensus):
- **swarmdo__hive-mind_init** — Start distributed consensus system.
- **swarmdo__hive-mind_spawn** — Add an agent to the hive.
- **swarmdo__hive-mind_consensus** — Run consensus vote across agents.
- **swarmdo__hive-mind_broadcast** — Send message to all hive agents.
- **swarmdo__hive-mind_memory** — Access shared hive memory.

### Coordination:
- **swarmdo__coordination_topology** — View/change coordination topology.
- **swarmdo__coordination_load_balance** — Distribute work across agents.
- **swarmdo__coordination_orchestrate** — Orchestrate complex multi-agent tasks.
- **swarmdo__coordination_sync** — Synchronize state across agents.

### Session Management:
- **swarmdo__session_save** — Save current session state.
- **swarmdo__session_restore** — Restore a previous session.
- **swarmdo__session_list** — List available sessions.

## Group 4: Memory & Knowledge (swarmdo)
Persistent memory, vector search, embeddings, and pattern storage. ${TOOL_GROUPS.memory.enabled ? "ACTIVE" : "DISABLED"}

### Memory Operations:
- **swarmdo__memory_store** — Store data in persistent memory.
  \`{"key": "my-key", "value": "data to store", "namespace": "default", "tags": ["tag1"]}\`
- **swarmdo__memory_retrieve** — Get stored data by key. \`{"key": "my-key"}\`
- **swarmdo__memory_search** — Semantic vector search across stored memories.
  \`{"query": "what to search for", "limit": 5, "namespace": "default"}\`
- **swarmdo__memory_list** — List all stored keys in a namespace.
- **swarmdo__memory_delete** — Remove a stored memory.
- **swarmdo__memory_stats** — View memory usage statistics.

### Embeddings:
- **swarmdo__embeddings_generate** — Generate vector embeddings for text.
- **swarmdo__embeddings_compare** — Compare semantic similarity of two texts.
- **swarmdo__embeddings_search** — Search embeddings database by similarity.
- **swarmdo__embeddings_neural** — Generate neural embeddings.
- **swarmdo__embeddings_hyperbolic** — Generate hyperbolic embeddings for hierarchical data.

### AgentDB (Advanced Pattern Storage):
- **swarmdo__agentdb_pattern-store** — Store a learned pattern with metadata.
  \`{"pattern": "description", "category": "code|debug|architecture", "confidence": 0.9}\`
- **swarmdo__agentdb_pattern-search** — Search patterns by similarity.
- **swarmdo__agentdb_route** — Route a query to the most relevant stored pattern.
- **swarmdo__agentdb_feedback** — Provide feedback on a pattern (reinforcement learning).
- **swarmdo__agentdb_context-synthesize** — Synthesize context from multiple sources.
- **swarmdo__agentdb_semantic-route** — Semantic routing based on stored knowledge.
- **swarmdo__agentdb_consolidate** — Consolidate and deduplicate stored patterns.
- **swarmdo__agentdb_batch** — Batch operations on patterns.
- **swarmdo__agentdb_session-start** / **swarmdo__agentdb_session-end** — Session tracking.
- **swarmdo__agentdb_hierarchical-store** / **swarmdo__agentdb_hierarchical-recall** — Hierarchical memory.

## Group 5: Dev Tools & Analysis (swarmdo)
Performance, system health, GitHub integration, code analysis, terminal. ${TOOL_GROUPS.devtools.enabled ? "ACTIVE" : "DISABLED"}

### System & Performance:
- **swarmdo__system_status** — System health overview.
- **swarmdo__system_metrics** — Detailed performance metrics.
- **swarmdo__system_health** — Health check across all subsystems.
- **swarmdo__performance_report** — Generate performance report.
- **swarmdo__performance_bottleneck** — Identify performance bottlenecks.
- **swarmdo__performance_benchmark** — Run benchmarks.
- **swarmdo__performance_optimize** — Get optimization recommendations.
- **swarmdo__performance_profile** — Profile specific operations.

### Code Analysis:
- **swarmdo__analyze_diff** — Analyze a code diff.
- **swarmdo__analyze_diff-risk** — Assess risk level of changes.
- **swarmdo__analyze_diff-classify** — Classify type of changes (feature, bugfix, refactor).
- **swarmdo__analyze_diff-reviewers** — Suggest code reviewers.
- **swarmdo__analyze_file-risk** — Assess risk of a specific file.

### GitHub Integration:
- **swarmdo__github_repo_analyze** — Analyze a GitHub repository.
  \`{"repo": "owner/repo", "analysis_type": "code_quality|performance|security"}\`
- **swarmdo__github_pr_manage** — Manage pull requests (create, review, merge).
- **swarmdo__github_issue_track** — Track and manage issues.
- **swarmdo__github_workflow** — Manage GitHub Actions workflows.
- **swarmdo__github_metrics** — Repository metrics and insights.

### Terminal Access:
- **swarmdo__terminal_create** — Create a terminal session.
- **swarmdo__terminal_execute** — Execute a command. \`{"command": "ls -la"}\`
- **swarmdo__terminal_list** — List active terminals.
- **swarmdo__terminal_history** — View command history.

### Development Hooks:
- **swarmdo__hooks_pre-task** / **swarmdo__hooks_post-task** — Task lifecycle hooks for learning.
- **swarmdo__hooks_pre-edit** / **swarmdo__hooks_post-edit** — File edit hooks.
- **swarmdo__hooks_session-start** / **swarmdo__hooks_session-end** — Session lifecycle.
- **swarmdo__hooks_worker-dispatch** — Dispatch background workers.
  Workers: \`optimize\`, \`audit\`, \`testgaps\`, \`document\`, \`map\`, \`deepdive\`, \`benchmark\`
- **swarmdo__hooks_model-route** — Route to optimal AI model for a task.
- **swarmdo__hooks_explain** — Explain a routing or intelligence decision.

### Configuration:
- **swarmdo__config_get** / **swarmdo__config_set** / **swarmdo__config_list** — Manage settings.

### Progress Tracking:
- **swarmdo__progress_check** — Check implementation progress.
- **swarmdo__progress_summary** — Summarize overall progress.
...
3. **Code task** ("write", "fix", "implement") → \`swarmvector__hooks_route\` to find best approach, then \`swarmdo__agent_spawn\`
4. **Analysis request** ("analyze", "review", "audit") → spawn reviewer/security agents + analysis tools
5. **Multi-step project** → \`swarmdo__task_create\` for tracking, \`swarmdo__swarm_init\` for coordination
6. **Memory/recall** ("remember", "save", "what did we...") → \`swarmdo__memory_store\` / \`swarmdo__memory_search\`
7. **System question** ("what tools", "help") → \`guidance(topic='overview')\`
8. **Performance concern** → \`swarmdo__performance_bottleneck\` + \`swarmdo__performance_optimize\`
9. **GitHub task** → \`swarmdo__github_*\` tools
...
### Code Implementation
\`swarmvector__hooks_route\` → \`swarmdo__agent_spawn(coder)\` → track with \`swarmdo__task_create\` → report

### Multi-Agent Analysis
\`swarmdo__swarm_init(hierarchical)\` → spawn agents → coordinate → synthesize results

### Learning & Memory
\`swarmdo__memory_search\` (check existing) → do work → \`swarmdo__memory_store\` (save results) → \`swarmvector__hooks_learn\`
...
- The prefixes "swarmdo__" or "swarmvector__" — just describe what you're doing naturally
