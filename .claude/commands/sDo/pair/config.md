...
Main configuration file: `.swarmdo/pair-config.json`
...
swarmdo pair config reset verification
```
...
### Project-Specific
`.swarmdo/pair-config.json` in project root

### User-Specific
`~/.swarmdo/pair-config.json`

### Global
`/etc/swarmdo/pair-config.json`
...
swarmdo pair config validate

# Test configuration
swarmdo pair config test
```
...
### From Version 1.x
```bash
swarmdo pair config migrate --from 1.x
```

### Export/Import
```bash
# Export current config
swarmdo pair config export > my-config.json

# Import config
swarmdo pair config import my-config.json
```
