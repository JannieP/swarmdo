...
```bash
swarmdo pair --switch <session-id>
```

### Merge Sessions
```bash
swarmdo pair --merge <session-1> <session-2>
```
...
### Manual Recovery
```bash
swarmdo pair --recover [--point <timestamp>]
```

### Crash Recovery
```bash
swarmdo pair --crash-recovery
```
...
### Share with Team
```bash
swarmdo pair --share <session-id> \
  --team <team-id>
```

### Export for Review
```bash
swarmdo pair --export-review <session-id>
```

### Create Learning Material
```bash
swarmdo pair --create-tutorial <session-id>
```
