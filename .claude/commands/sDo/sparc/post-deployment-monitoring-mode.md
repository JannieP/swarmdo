...
npx swarmdo sparc run post-deployment-monitoring-mode "monitor production metrics"

# For alpha features
npx swarmdo@alpha sparc run post-deployment-monitoring-mode "monitor production metrics"

# With namespace
npx swarmdo sparc run post-deployment-monitoring-mode "your task" --namespace post-deployment-monitoring-mode

# Non-interactive mode
npx swarmdo sparc run post-deployment-monitoring-mode "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If swarmdo is installed locally
./swarmdo sparc run post-deployment-monitoring-mode "monitor production metrics"
```
...
```bash
# Store mode-specific context
npx swarmdo memory store "post-deployment-monitoring-mode_context" "important decisions" --namespace post-deployment-monitoring-mode

# Query previous work
npx swarmdo memory query "post-deployment-monitoring-mode" --limit 5
```
