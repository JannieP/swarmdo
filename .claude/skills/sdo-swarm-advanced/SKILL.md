...
npx swarmdo swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
npx swarmdo swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx swarmdo swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  ...
  // Check swarm health
  const status = await mcp__swarmdo__swarm_status({})

  // Analyze error patterns
  await mcp__swarmdo__error_analysis({
    "logs": [error.message]
  })
