---
name: sdo-browser
description: Web browser automation with AI-optimized snapshots for swarmdo agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @swarmdo/cli memory store --namespace browser-patterns --key "login-flow" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @swarmdo/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @swarmdo/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @swarmdo/cli hooks post-task --task-id "browse-1" --success true
```
