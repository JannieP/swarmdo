...
npx swarmdo hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx swarmdo hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx swarmdo hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx swarmdo hook pre-edit [options]
...
Examples:
  npx swarmdo hook pre-edit --file "src/auth/login.js"
  npx swarmdo hook pre-edit -f "config/db.js" --validate-syntax
  npx swarmdo hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx swarmdo hook pre-bash --command <cmd>
...
Examples:
  npx swarmdo hook pre-bash -c "rm -rf /tmp/cache"
  npx swarmdo hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx swarmdo hook pre-task [options]
...
Examples:
  npx swarmdo hook pre-task --description "Implement user authentication"
  npx swarmdo hook pre-task -d "Continue API dev" --load-memory
  npx swarmdo hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx swarmdo hook pre-search --query <query>
...
Examples:
  npx swarmdo hook pre-search -q "authentication middleware"
```
...
```bash
npx swarmdo hook post-edit [options]
...
Examples:
  npx swarmdo hook post-edit --file "src/components/Button.jsx"
  npx swarmdo hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx swarmdo hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx swarmdo hook post-bash --command <cmd>
...
Examples:
  npx swarmdo hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx swarmdo hook post-task [options]
...
Examples:
  npx swarmdo hook post-task --task-id "auth-implementation"
  npx swarmdo hook post-task -t "api-refactor" --analyze-performance
  npx swarmdo hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx swarmdo hook post-search --query <query> --results <path>
...
Examples:
  npx swarmdo hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx swarmdo hook mcp-initialized --swarm-id <id>
...
```bash
npx swarmdo hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx swarmdo hook task-orchestrated --task-id <id>
...
```bash
npx swarmdo hook neural-trained --pattern <name>
...
```bash
npx swarmdo hook memory-sync --namespace <ns>
...
```bash
npx swarmdo hook session-start --session-id <id>
...
```bash
npx swarmdo hook session-restore --session-id <id>
...
Examples:
  npx swarmdo hook session-restore --session-id "swarm-20241019"
  npx swarmdo hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx swarmdo hook session-end [options]
...
Examples:
  npx swarmdo hook session-end --session-id "dev-session-2024"
  npx swarmdo hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx swarmdo hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx swarmdo hook notify --message <msg>
...
Examples:
  npx swarmdo hook notify -m "Task completed" --level info
  npx swarmdo hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx swarmdo hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx swarmdo hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx swarmdo hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx swarmdo hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx swarmdo hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx swarmdo hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx swarmdo hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__swarmdo__memory_persist {
  sessionId: "dev-2024"
}
...
for FILE in $FILES; do
  ...
  npx swarmdo hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx swarmdo hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx swarmdo hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx swarmdo metrics score --format json | jq -r '.truth_score')
...
npx swarmdo hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx swarmdo hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx swarmdo hook session-restore \
...
npx swarmdo hook session-start --session-id "fullstack-feature"
...
# Backend work
npx swarmdo hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx swarmdo hook post-edit \
  --file "api/profile.js" \
...
npx swarmdo hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx swarmdo hook post-edit \
...
npx swarmdo hook session-start --session-id "debug-memory-leak"
...
npx swarmdo hook pre-search --query "EventEmitter"
# ... search executes ...
npx swarmdo hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx swarmdo hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx swarmdo hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx swarmdo hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx swarmdo hook post-edit \
    --file "$file" \
...
```bash
# Enable debug output
export SWARMDO_DEBUG=true

# Test specific hook with verbose output
npx swarmdo hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .swarmdo/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx swarmdo hook validate-config
```
...
- `npx swarmdo init --hooks` - Initialize hooks system
- `npx swarmdo hook --list` - List available hooks
- `npx swarmdo hook --test <hook>` - Test specific hook
- `npx swarmdo memory usage` - Manage memory
- `npx swarmdo agent spawn` - Spawn agents
- `npx swarmdo swarm init` - Initialize swarm
