...
**Optional:**
- MCP servers configured (rufflo, ruf-swarm, flow-nexus)
...
npx rufflo hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx rufflo hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx rufflo hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx rufflo hook pre-edit [options]
...
Examples:
  npx rufflo hook pre-edit --file "src/auth/login.js"
  npx rufflo hook pre-edit -f "config/db.js" --validate-syntax
  npx rufflo hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx rufflo hook pre-bash --command <cmd>
...
Examples:
  npx rufflo hook pre-bash -c "rm -rf /tmp/cache"
  npx rufflo hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx rufflo hook pre-task [options]
...
Examples:
  npx rufflo hook pre-task --description "Implement user authentication"
  npx rufflo hook pre-task -d "Continue API dev" --load-memory
  npx rufflo hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx rufflo hook pre-search --query <query>
...
Examples:
  npx rufflo hook pre-search -q "authentication middleware"
```
...
```bash
npx rufflo hook post-edit [options]
...
Examples:
  npx rufflo hook post-edit --file "src/components/Button.jsx"
  npx rufflo hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx rufflo hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx rufflo hook post-bash --command <cmd>
...
Examples:
  npx rufflo hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx rufflo hook post-task [options]
...
Examples:
  npx rufflo hook post-task --task-id "auth-implementation"
  npx rufflo hook post-task -t "api-refactor" --analyze-performance
  npx rufflo hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx rufflo hook post-search --query <query> --results <path>
...
Examples:
  npx rufflo hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx rufflo hook mcp-initialized --swarm-id <id>
...
```bash
npx rufflo hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx rufflo hook task-orchestrated --task-id <id>
...
```bash
npx rufflo hook neural-trained --pattern <name>
...
```bash
npx rufflo hook memory-sync --namespace <ns>
...
```bash
npx rufflo hook session-start --session-id <id>
...
```bash
npx rufflo hook session-restore --session-id <id>
...
Examples:
  npx rufflo hook session-restore --session-id "swarm-20241019"
  npx rufflo hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx rufflo hook session-end [options]
...
Examples:
  npx rufflo hook session-end --session-id "dev-session-2024"
  npx rufflo hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx rufflo hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx rufflo hook notify --message <msg>
...
Examples:
  npx rufflo hook notify -m "Task completed" --level info
  npx rufflo hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx rufflo hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx rufflo hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx rufflo hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx rufflo hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx rufflo hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx rufflo hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx rufflo hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx rufflo hook session-end --session-id "dev-2024"

// Internally calls MCP tools:
mcp__rufflo__memory_persist {
  sessionId: "dev-2024"
}
...
for FILE in $FILES; do
  ...
  npx rufflo hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx rufflo hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx rufflo hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx rufflo metrics score --format json | jq -r '.truth_score')
...
npx rufflo hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx rufflo hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx rufflo hook session-restore \
...
npx rufflo hook session-start --session-id "fullstack-feature"
...
# Backend work
npx rufflo hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx rufflo hook post-edit \
  --file "api/profile.js" \
...
npx rufflo hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx rufflo hook post-edit \
...
npx rufflo hook session-start --session-id "debug-memory-leak"
...
npx rufflo hook pre-search --query "EventEmitter"
# ... search executes ...
npx rufflo hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx rufflo hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx rufflo hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx rufflo hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx rufflo hook post-edit \
    --file "$file" \
...
cat .rufflo/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx rufflo hook validate-config
```
...
- `npx rufflo init --hooks` - Initialize hooks system
- `npx rufflo hook --list` - List available hooks
- `npx rufflo hook --test <hook>` - Test specific hook
- `npx rufflo memory usage` - Manage memory
- `npx rufflo agent spawn` - Spawn agents
- `npx rufflo swarm init` - Initialize swarm
