 ...
 *
 * The bash shim (swarmdo-hook.sh) works on Mac/Linux but fails on native
 ...
 *
 * Mac/Linux continue to use swarmdo-hook.sh via the plugin hooks.json files
 * (unchanged). On Windows, swarmdo init writes a .claude/settings.json that
 * overrides those entries with node-based equivalents pointing here.
 *
 * Behaviour mirrors swarmdo-hook.sh:
 *   1. Reads hook JSON payload from stdin.
 *   2. Prefers a locally installed `swarmdo` or `swarmdo` binary.
 *   3. Falls back to `npx --prefer-offline swarmdo@latest`.
 *   4. Always exits 0 — hook subcommands are best-effort telemetry.
 *   5. Swallows all stderr — nothing should surface to Claude Code.
 *
 * Usage: node swarmdo-hook.cjs <hook-subcommand> [args...]
 *   e.g. node swarmdo-hook.cjs post-edit --file "x.ts" --train-patterns
 */
...
/** Build the argv for the swarmdo/swarmdo/npx invocation */
function buildArgs(subcommand, extraArgs) {
  // The `hooks` word is prepended here, matching swarmdo-hook.sh convention.
...
function main() {
  ...
  // Priority 1: locally installed swarmdo binary
  if (commandExists('swarmdo')) {
    invokeHook('swarmdo', [], hookArgs, stdinData);
    done();
  }

  // Priority 2: locally installed swarmdo binary
  if (commandExists('swarmdo')) {
    invokeHook('swarmdo', [], hookArgs, stdinData);
    done();
  }
  ...
  //
  // SKIP this when SWARMDO_HOOK_SKIP_NPX=1 — used by CI smokes that test
  ...
  if (process.env.SWARMDO_HOOK_SKIP_NPX !== '1') {
