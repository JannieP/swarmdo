#!/usr/bin/env node
// Rufflo CLI - thin wrapper around @rufflo/cli with rufflo branding
...
// because the wrapper imports dist/src/index.js, bypassing bin/cli.js.)
{
  ...
  if (_argv.length === 1 && (_argv[0] === '--version' || _argv[0] === '-V')) {
    try {
      ...
      process.stdout.write(`rufflo v${pkg.version || '0.0.0'}\n`);
    } catch {
      process.stdout.write('rufflo v0.0.0\n');
    }
    process.exit(0);
  }
}

// Walk up from rufflo/bin/ to find @rufflo/cli in node_modules
function findCliPath() {
  let dir = resolve(__dirname, '..');
  for (let i = 0; i < 10; i++) {
    const candidate = join(dir, 'node_modules', '@rufflo', 'cli', 'bin', 'cli.js');
...
const cliBase = pkgDir
  ? join(pkgDir, 'node_modules', '@rufflo', 'cli')
  : resolve(__dirname, '../../v3/@rufflo/cli');
...
if (isMCPMode) {
  ...
  // CLI mode: use rufflo branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'rufflo',
    description: 'Rufflo - AI Agent Orchestration Platform',
  });
  cli.run()
    .then(() => {
