<script lang="ts">
	...
	const groups: Group[] = [
		{
			...
			tools: [
				{ name: "rufflo__memory_store", desc: "Save a key/value into a namespace." },
				{ name: "rufflo__memory_search", desc: "Semantic vector search over stored memories." },
				{ name: "rufflo__memory_retrieve", desc: "Fetch a specific entry by key + namespace." },
				{ name: "rufflo__memory_list", desc: "List entries in a namespace." },
				{ name: "rufflo__embeddings_compare", desc: "Compare two pieces of text by semantic similarity." },
				{ name: "rufflo__agentdb_*", desc: "Lower-level AgentDB controllers (route, consolidate, hierarchical)." },
			],
		},
		{
			id: "agents",
			...
			tools: [
				{ name: "rufflo__agent_spawn", desc: "Create a new specialized agent (coder, tester, reviewer, security-auditor, …)." },
				{ name: "rufflo__swarm_init", desc: "Initialize a swarm with topology + strategy + max agents." },
				{ name: "rufflo__hive-mind_*", desc: "Queen-led Byzantine fault-tolerant collective." },
				{ name: "rufflo__task_create / task_assign / task_status", desc: "Full task lifecycle." },
				{ name: "rufflo__agent_list / agent_status", desc: "Inspect active agents and their state." },
			],
		},
		{
			id: "intelligence",
			...
			emoji: "✨",
			intro: "Pattern learning, model routing, code analysis, and trajectory tracking via RufVector. Tools here improve future task execution by recording what worked.",
			tools: [
				{ name: "rufvector__hooks_route", desc: "Pick the optimal agent type for a task." },
				{ name: "rufvector__hooks_remember / recall", desc: "Cross-session key/value memory shared with agents." },
				{ name: "rufvector__hooks_trajectory_begin / step / end", desc: "Record multi-step task execution so the system learns from the run." },
				{ name: "rufvector__hooks_security_scan", desc: "Scan code for vulnerabilities." },
				{ name: "rufvector__hooks_rag_context", desc: "Get retrieval-augmented context for a query." },
				{ name: "rufvector__hooks_swarm_recommend", desc: "Recommend swarm topology + agent mix for a task." },
			],
		},
		{
			id: "devtools",
			...
			tools: [
				{ name: "rufflo__system_status", desc: "Overall system health overview." },
				{ name: "rufflo__performance_metrics / bottleneck / report", desc: "Detailed performance and hotspot analysis." },
				{ name: "rufflo__analyze_diff*", desc: "Risk-score, classify, and suggest reviewers for a code diff." },
				{ name: "rufflo__github_repo_analyze / pr_manage / issue_track", desc: "Repository metrics and PR/issue ops." },
				{ name: "rufflo__terminal_execute / terminal_create", desc: "Run shell commands and manage sessions." },
				{ name: "rufflo__progress_*", desc: "Implementation progress tracking across long-horizon work." },
			],
		},
		{
			id: "core",
	...
	const tips = [
		...
		'**AUTO toggle**: green/lit means tool chains continue automatically. Gray means Rufflo stops after each tool result so you can inspect before continuing.',
		...
		'**Trajectory mode** — for long multi-step work, prefix with "Use `rufvector__hooks_trajectory_begin`" so the system learns from the run for future routing.',
		'**Tool name prefixes** — `rufflo__` is the agent/memory backend, `rufvector__` is the intelligence layer. Always use the full prefixed name when invoking explicitly.',
	];

	const examplePrompts = [
		'"Use rufflo__memory_store namespace=prefs key=editor value=vim then rufflo__memory_retrieve to confirm."',
		'"Run rufflo__system_status, rufflo__performance_metrics, and rufflo__memory_list in parallel and summarize."',
		'"Spawn a 5-agent hierarchical swarm (architect, coder, tester, reviewer, security-auditor) for a Python→TypeScript refactor."',
		'"Use rufvector__hooks_route on the task: add OAuth to a SvelteKit API. Then spawn the recommended agent."',
		'"Search Rufflo memory for prior decisions about authentication, then web_research recent OAuth2 best practices, in parallel."',
	...
	const stack: { name: string; role: string }[] = [
		...
		{ name: "rufflo backend (npx rufflo mcp start)", role: "Agents, memory, swarm, devtools (~158 tools)" },
		{ name: "rufvector backend (npx rufvector mcp start)", role: "Intelligence, routing, trajectories (~49 tools)" },
		...
		{ name: "Cloud Run (us-central1)", role: "Both rufvocal chat-ui and mcp-bridge services with min-instance=1 warm" },
	];

	const domains: { url: string; note: string }[] = [
		{ url: "flo.ruv.io", note: "Primary — shortest URL, recommended" },
		{ url: "rufflo.ruv.io", note: "Brand alias" },
		{ url: "rufvocal.ruv.io", note: "Original alias (matches the upstream chat-ui fork name)" },
		{ url: "rufvocal-875130704813.us-central1.run.app", note: "Raw Cloud Run URL — always available, no DNS dependency" },
	];
	...
	<div class="flex max-h-[88vh] w-full flex-col overflow-hidden rounded-2xl bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100">
		<header class="flex items-start justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
			<div class="flex items-start gap-3">
				...
				<div>
					<h2 class="text-lg font-semibold">Rufflo Capabilities</h2>
		...
		<div class="flex-1 overflow-y-auto px-6 py-4">
			<section class="mb-6">
				...
				<ol class="list-decimal space-y-1.5 pl-5 text-sm">
					...
					<li>Rufflo decides which tools to call. Watch the streaming tool-call cards below your message — multiple cards = parallel execution.</li>
			...
			<section class="mb-6">
				...
				<div class="mt-3 rounded-lg border border-emerald-300/40 bg-emerald-50/50 px-3 py-2 text-xs dark:border-emerald-700/40 dark:bg-emerald-900/10">
					...
					<p class="mt-1 text-emerald-900/80 dark:text-emerald-200/80">Rufflo speaks any OpenAI-compatible endpoint: vLLM, Ollama, LM Studio, Together, Groq, or self-hosted. Native support for <a href="https://github.com/ruvnet/RufVector/tree/main/examples/rufLLM" target="_blank" rel="noopener" class="underline">rufLLM</a> (lives in <code>ruvnet/RuVector/examples/ruvLLM</code>) — Rufflo's self-improving local model layer with MicroLoRA adapters that learn from your trajectories via SONA. Pair cloud + local, or run fully offline.</p>
				</div>
			</section>
			...
			<section class="mb-6">
				...
				<p class="mb-2 text-xs text-gray-600 dark:text-gray-400">Rufflo's web UI is a fork of the open-source HuggingFace chat-ui (SvelteKit) wired to a custom MCP bridge that fans out to two backend kernels. Everything runs on Google Cloud Run.</p>
				<div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
					<table class="w-full text-xs">
						<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
							{#each stack as layer}
			...
			<section>
				...
				<ul class="space-y-1 text-sm">
					...
					<li>
						<a href="https://github.com/ruvnet/ruflo/blob/main/rufflo/docs/adr/ADR-033-RUFVOCAL-WASM-MCP-INTEGRATION.md" target="_blank" rel="noopener" class="text-blue-600 hover:underline dark:text-blue-400">
					...
					<li>
						<a href="https://github.com/ruvnet/rufvector" target="_blank" rel="noopener" class="text-blue-600 hover:underline dark:text-blue-400">
							ruvnet/rufvector — intelligence layer →
						</a>
					</li>
				</ul>
			</section>
