/**
 * PostgreSQL adapter for RufVocal — drop-in replacement for MongoDB collections.
 *
...
export function getPool(): pg.Pool {
	if (!pool) {
		const connectionString =
			...
			"postgresql://rufvocal:rufvocal@localhost:5432/rufvocal";
		pool = new Pool({
			connectionString,
			max: 20,
...
export class PostgresCollection<T extends Record<string, unknown>> {
	...
	// RufVector extension: semantic search via pgvector
