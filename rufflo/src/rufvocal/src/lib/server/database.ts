/**
 * RufVocal Database — self-contained RVF document store.
 *
...
export class Database {
	...
	private async init() {
		...
		const dbPath = join(dbFolder, "rufvocal.rvf.json");

		console.log(`[RufVocal] Database: ${dbPath}`);
		initRvfStore(dbPath);
		this.initialized = true;

		// Flush to disk on exit
		onExit(async () => {
			console.log("[RufVocal] Flushing database to disk");
			flushToDisk();
		});
	}
