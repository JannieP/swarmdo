# RufVocal — Rufflo Web UI

> Rufflo's multi-model AI chat with built-in Model Context Protocol (MCP) tool calling. Live at [**flo.ruv.io**](https://flo.ruv.io/).
...
RufVocal is the SvelteKit web app that lets you chat with Qwen, Claude, Gemini, or OpenAI while [Rufflo](https://github.com/ruvnet/ruflo) invokes the same ~210 MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — all directly from chat. No install, no API key needed to try the hosted demo.

It started as a fork of the [HuggingFace chat-ui](https://github.com/huggingface/chat-ui) v0.20.0 and has been extended with a WASM-MCP integration layer, parallel tool execution, an in-browser tool gallery, and a "Rufflo Capabilities" tour modal. See [ADR-033](../../docs/adr/ADR-033-RUFVOCAL-WASM-MCP-INTEGRATION.md) for the architecture.

## What RufVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five Rufflo server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **Rufflo Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
| 🦾 **rufLLM ready** | Native support for [rufLLM](https://github.com/ruvnet/RufVector/tree/main/examples/rufLLM) — Rufflo's self-improving local model layer |
...
# Curated Rufflo model list (optional — defaults to /models from the base URL)
TASK_MODEL=qwen/qwen3.6-max-preview
PUBLIC_APP_NAME=Rufflo
...
```bash
docker build -t rufvocal --build-arg INCLUDE_DB=true .
docker run -p 3000:3000 \
  -e OPENAI_BASE_URL=https://openrouter.ai/api/v1 \
  -e OPENAI_API_KEY=sk-or-v1-... \
  -v rufvocal-data:/data \
  rufvocal
```

### Google Cloud Run (production-style)

`cloudbuild.yaml` does a multi-stage build with `INCLUDE_DB=true`, pushes to Artifact Registry, and deploys to Cloud Run. Hosted demo at `flo.ruv.io` runs from this exact pipeline. See [`cloudbuild.yaml`](./cloudbuild.yaml) and the deploy notes in [ADR-033](../../docs/adr/ADR-033-RUFVOCAL-WASM-MCP-INTEGRATION.md).
...
- **Local container** — `docker run -d -p 27017:27017 --name mongo-rufvocal mongo:latest` then `MONGODB_URL=mongodb://localhost:27017`.

`MONGODB_DB_NAME` defaults to `chat-ui` (kept for upstream compatibility); change per environment.

## MCP Tools (the Rufflo difference)

RufVocal calls tools exposed by Model Context Protocol servers and feeds results back to the model via OpenAI function calling. Configure trusted servers via env, let users add their own, and the router auto-selects tools-capable models when needed.

```env
MCP_SERVERS=[
  {"name":"Rufflo Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"Rufflo Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
  {"name":"Rufflo Agents","url":"https://mcp-bridge-...run.app/mcp/agents","transport":"sse"},
  {"name":"Rufflo Memory","url":"https://mcp-bridge-...run.app/mcp/memory","transport":"sse"},
  {"name":"Rufflo DevTools","url":"https://mcp-bridge-...run.app/mcp/devtools","transport":"sse"},
...
RufVocal can do server-side smart routing using [katanemo/Arch-Router-1.5B](https://huggingface.co/katanemo/Arch-Router-1.5B) without a separate router service. Selecting "Omni" in the model picker:
...
- **Capabilities modal** — `src/lib/components/RuffloHelpModal.svelte`
- **Dynamic follow-ups** — tool-call-aware suggested next prompts in `ChatWindow.svelte`

For deeper internals, see [`CLAUDE.md`](./CLAUDE.md) (Claude Code agent guide) and [ADR-033](../../docs/adr/ADR-033-RUFVOCAL-WASM-MCP-INTEGRATION.md).

## Related

- 🏠 **Parent project** — [Rufflo](https://github.com/ruvnet/ruflo)
- 🎯 **Goal Planner UI** — [goal.ruv.io](https://goal.ruv.io/) · [/agents](https://goal.ruv.io/agents)
- 📖 **ADR-033** — RufVocal/WASM-MCP integration architecture
- 📋 **Roadmap** — [issue #1689](https://github.com/ruvnet/ruflo/issues/1689)
- 🍴 **Upstream** — [huggingface/chat-ui](https://github.com/huggingface/chat-ui) (RufVocal is forked from v0.20.0)

## License

MIT — same as the parent [Rufflo](https://github.com/ruvnet/ruflo) project.
