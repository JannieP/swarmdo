...
const TOOL_GROUPS = {
  ...
  // --- Agents & Orchestration (rufflo) ---
  agents: {
    enabled: process.env.MCP_GROUP_AGENTS !== "false",
    description: "Agent lifecycle, swarm coordination, task management, workflows (rufflo)",
    source: "rufflo",
    prefixes: ["agent_", "swarm_", "task_", "session_", "hive-mind_", "workflow_", "coordination_"],
  },

  // --- Memory & Knowledge (rufflo) ---
  memory: {
    enabled: process.env.MCP_GROUP_MEMORY !== "false",
    description: "Vector memory, AgentDB, embeddings, semantic search (rufflo)",
    source: "rufflo",
  ...
  devtools: {
    ...
    description: "Hooks, code analysis, performance profiling, GitHub integration (rufflo)",
    source: "rufflo",
    prefixes: ["hooks_", "analyze_", "performance_", "github_", "terminal_", "config_", "system_", "progress_"],
  },

  // --- Security & Safety (rufflo) ---
  security: {
    enabled: process.env.MCP_GROUP_SECURITY === "true",
    description: "AI defence, PII detection, claims management, pattern transfer (rufflo)",
    source: "rufflo",
    prefixes: ["aidefence_", "claims_", "transfer_"],
  },

  // --- Browser Automation (rufflo) ---
  browser: {
    enabled: process.env.MCP_GROUP_BROWSER === "true",
    description: "Headless browser control — navigate, click, fill, screenshot (rufflo)",
    source: "rufflo",
    prefixes: ["browser_"],
  },
  ...
  neural: {
    ...
    description: "Neural network training, DAA autonomous agents, cognitive patterns (rufflo)",
    source: "rufflo",
...
- To recall previously stored patterns or solutions`,
    ...
    agents: `# Agents & Orchestration Group (rufflo)

Multi-agent lifecycle management, swarm coordination, and task workflows.

## Key Tools
- **rufflo__agent_spawn** — Create a new agent with specific capabilities
- **rufflo__agent_list** — List all active agents
- **rufflo__swarm_init** — Initialize a swarm with a topology (mesh, hierarchical, ring, star)
- **rufflo__task_create** — Create and assign tasks
- **rufflo__workflow_create** — Define multi-step workflows
- **rufflo__workflow_execute** — Execute a workflow
- **rufflo__hive-mind_init** — Start collective intelligence coordination
- **rufflo__coordination_orchestrate** — Multi-agent coordination
...
- Distributed task management`,
    ...
    memory: `# Memory & Knowledge Group (rufflo)

Vector storage, semantic search, AgentDB pattern learning, and embeddings.

## Key Tools
- **rufflo__memory_store** — Store a value with vector embedding for semantic search
- **rufflo__memory_search** — Semantic search across stored memories (HNSW-indexed)
- **rufflo__memory_list** — List stored memory entries
- **rufflo__agentdb_pattern-store** — Store a reasoning pattern for learning
- **rufflo__agentdb_pattern-search** — Search for similar reasoning patterns
- **rufflo__agentdb_context-synthesize** — Synthesize context from stored memories
- **rufflo__embeddings_generate** — Generate vector embeddings for text
- **rufflo__embeddings_search** — Semantic similarity search
...
- Building semantic search over custom data`,
    ...
    devtools: `# Dev Tools Group (rufflo)

Code analysis, performance profiling, GitHub integration, and terminal access.

## Key Tools
- **rufflo__analyze_diff** — Analyze git diff for risk and change classification
- **rufflo__performance_benchmark** — Run performance benchmarks
- **rufflo__performance_bottleneck** — Detect performance bottlenecks
- **rufflo__github_repo_analyze** — Analyze a GitHub repository
- **rufflo__github_pr_manage** — Manage pull requests
- **rufflo__terminal_execute** — Execute commands in a terminal session
...
- GitHub repository management`,
    ...
    security: `# Security & Safety Group (rufflo)

AI defence, PII detection, and claims-based authorization.

## Key Tools
- **rufflo__aidefence_scan** — Scan text for AI manipulation attempts
- **rufflo__aidefence_has_pii** — Check for PII (emails, phones, SSNs)
- **rufflo__aidefence_is_safe** — Quick safety check on input
- **rufflo__claims_claim** — Claim an issue for work
- **rufflo__claims_board** — Visual board of all claims
...
- Work item management across agents`,
    ...
    browser: `# Browser Automation Group (rufflo)

Headless browser control for web interaction and testing.

## Key Tools
- **rufflo__browser_open** — Navigate to a URL
- **rufflo__browser_click** — Click elements by reference
- **rufflo__browser_fill** — Fill form inputs
- **rufflo__browser_screenshot** — Capture page screenshots
- **rufflo__browser_snapshot** — Get accessibility tree for AI parsing
- **rufflo__browser_eval** — Execute JavaScript in page context
...
- Form filling and web interaction`,
    ...
    neural: `# Neural & DAA Group (rufflo)

Neural network operations and Decentralized Autonomous Agents.

## Key Tools
- **rufflo__neural_train** — Train a neural model
- **rufflo__neural_predict** — Make predictions
- **rufflo__daa_agent_create** — Create an autonomous agent
- **rufflo__daa_workflow_create** — Create autonomous workflows
- **rufflo__daa_knowledge_share** — Share knowledge between agents
...
Your tools are organized into groups. Each tool name is prefixed with its backend (e.g., \`rufflo__agent_spawn\`, \`rufvector__hooks_route\`). Always use the full prefixed name when calling tools.
...
- **guidance** — Get help on any tool group, specific tool usage, or capabilities overview.
  ...
  For specific tool help: \`guidance(topic='tool', tool_name='rufflo__agent_spawn')\`
...
### Agent Lifecycle:
- **rufflo__agent_spawn** — Create a new specialized agent.
...
- **rufflo__agent_status** — Check an agent's current state. \`{"agentId": "agent-xxx"}\`
- **rufflo__agent_list** — List all active agents with their states.
- **rufflo__agent_terminate** — Stop an agent. \`{"agentId": "agent-xxx"}\`
- **rufflo__agent_health** — Health check across all agents.
- **rufflo__agent_pool** — View the agent pool and available capacity.

### Swarm Coordination:
- **rufflo__swarm_init** — Initialize a multi-agent swarm.
...
- **rufflo__swarm_status** — Get swarm health, topology, and agent states.
- **rufflo__swarm_health** — Detailed health metrics for the swarm.
- **rufflo__swarm_shutdown** — Tear down a swarm and all its agents.

### Task Management:
- **rufflo__task_create** — Create a tracked task.
  \`{"description": "what needs to be done", "priority": "low|normal|high|critical"}\`
- **rufflo__task_status** — Check task progress. \`{"taskId": "task-xxx"}\`
- **rufflo__task_list** — List all tasks with their statuses.
- **rufflo__task_complete** — Mark a task as done. \`{"taskId": "task-xxx"}\`
- **rufflo__task_update** — Update task details, status, or assignment.
- **rufflo__task_cancel** — Cancel a task.

### Workflow Orchestration:
- **rufflo__workflow_create** — Define a multi-step workflow with dependencies.
- **rufflo__workflow_execute** — Run a workflow. \`{"workflowId": "wf-xxx"}\`
- **rufflo__workflow_status** — Check workflow progress.
- **rufflo__workflow_template** — Use a pre-built workflow template.
- **rufflo__workflow_pause** / **rufflo__workflow_resume** — Control workflow execution.

### Hive-Mind (Distributed Consensus):
- **rufflo__hive-mind_init** — Start distributed consensus system.
- **rufflo__hive-mind_spawn** — Add an agent to the hive.
- **rufflo__hive-mind_consensus** — Run consensus vote across agents.
- **rufflo__hive-mind_broadcast** — Send message to all hive agents.
- **rufflo__hive-mind_memory** — Access shared hive memory.

### Coordination:
- **rufflo__coordination_topology** — View/change coordination topology.
- **rufflo__coordination_load_balance** — Distribute work across agents.
- **rufflo__coordination_orchestrate** — Orchestrate complex multi-agent tasks.
- **rufflo__coordination_sync** — Synchronize state across agents.

### Session Management:
- **rufflo__session_save** — Save current session state.
- **rufflo__session_restore** — Restore a previous session.
- **rufflo__session_list** — List available sessions.

## Group 4: Memory & Knowledge (rufflo)
Persistent memory, vector search, embeddings, and pattern storage. ${TOOL_GROUPS.memory.enabled ? "ACTIVE" : "DISABLED"}

### Memory Operations:
- **rufflo__memory_store** — Store data in persistent memory.
  \`{"key": "my-key", "value": "data to store", "namespace": "default", "tags": ["tag1"]}\`
- **rufflo__memory_retrieve** — Get stored data by key. \`{"key": "my-key"}\`
- **rufflo__memory_search** — Semantic vector search across stored memories.
  \`{"query": "what to search for", "limit": 5, "namespace": "default"}\`
- **rufflo__memory_list** — List all stored keys in a namespace.
- **rufflo__memory_delete** — Remove a stored memory.
- **rufflo__memory_stats** — View memory usage statistics.

### Embeddings:
- **rufflo__embeddings_generate** — Generate vector embeddings for text.
- **rufflo__embeddings_compare** — Compare semantic similarity of two texts.
- **rufflo__embeddings_search** — Search embeddings database by similarity.
- **rufflo__embeddings_neural** — Generate neural embeddings.
- **rufflo__embeddings_hyperbolic** — Generate hyperbolic embeddings for hierarchical data.

### AgentDB (Advanced Pattern Storage):
- **rufflo__agentdb_pattern-store** — Store a learned pattern with metadata.
  \`{"pattern": "description", "category": "code|debug|architecture", "confidence": 0.9}\`
- **rufflo__agentdb_pattern-search** — Search patterns by similarity.
- **rufflo__agentdb_route** — Route a query to the most relevant stored pattern.
- **rufflo__agentdb_feedback** — Provide feedback on a pattern (reinforcement learning).
- **rufflo__agentdb_context-synthesize** — Synthesize context from multiple sources.
- **rufflo__agentdb_semantic-route** — Semantic routing based on stored knowledge.
- **rufflo__agentdb_consolidate** — Consolidate and deduplicate stored patterns.
- **rufflo__agentdb_batch** — Batch operations on patterns.
- **rufflo__agentdb_session-start** / **rufflo__agentdb_session-end** — Session tracking.
- **rufflo__agentdb_hierarchical-store** / **rufflo__agentdb_hierarchical-recall** — Hierarchical memory.

## Group 5: Dev Tools & Analysis (rufflo)
Performance, system health, GitHub integration, code analysis, terminal. ${TOOL_GROUPS.devtools.enabled ? "ACTIVE" : "DISABLED"}

### System & Performance:
- **rufflo__system_status** — System health overview.
- **rufflo__system_metrics** — Detailed performance metrics.
- **rufflo__system_health** — Health check across all subsystems.
- **rufflo__performance_report** — Generate performance report.
- **rufflo__performance_bottleneck** — Identify performance bottlenecks.
- **rufflo__performance_benchmark** — Run benchmarks.
- **rufflo__performance_optimize** — Get optimization recommendations.
- **rufflo__performance_profile** — Profile specific operations.

### Code Analysis:
- **rufflo__analyze_diff** — Analyze a code diff.
- **rufflo__analyze_diff-risk** — Assess risk level of changes.
- **rufflo__analyze_diff-classify** — Classify type of changes (feature, bugfix, refactor).
- **rufflo__analyze_diff-reviewers** — Suggest code reviewers.
- **rufflo__analyze_file-risk** — Assess risk of a specific file.

### GitHub Integration:
- **rufflo__github_repo_analyze** — Analyze a GitHub repository.
  \`{"repo": "owner/repo", "analysis_type": "code_quality|performance|security"}\`
- **rufflo__github_pr_manage** — Manage pull requests (create, review, merge).
- **rufflo__github_issue_track** — Track and manage issues.
- **rufflo__github_workflow** — Manage GitHub Actions workflows.
- **rufflo__github_metrics** — Repository metrics and insights.

### Terminal Access:
- **rufflo__terminal_create** — Create a terminal session.
- **rufflo__terminal_execute** — Execute a command. \`{"command": "ls -la"}\`
- **rufflo__terminal_list** — List active terminals.
- **rufflo__terminal_history** — View command history.

### Development Hooks:
- **rufflo__hooks_pre-task** / **rufflo__hooks_post-task** — Task lifecycle hooks for learning.
- **rufflo__hooks_pre-edit** / **rufflo__hooks_post-edit** — File edit hooks.
- **rufflo__hooks_session-start** / **rufflo__hooks_session-end** — Session lifecycle.
- **rufflo__hooks_worker-dispatch** — Dispatch background workers.
  Workers: \`optimize\`, \`audit\`, \`testgaps\`, \`document\`, \`map\`, \`deepdive\`, \`benchmark\`
- **rufflo__hooks_model-route** — Route to optimal AI model for a task.
- **rufflo__hooks_explain** — Explain a routing or intelligence decision.

### Configuration:
- **rufflo__config_get** / **rufflo__config_set** / **rufflo__config_list** — Manage settings.

### Progress Tracking:
- **rufflo__progress_check** — Check implementation progress.
- **rufflo__progress_summary** — Summarize overall progress.
...
3. **Code task** ("write", "fix", "implement") → \`rufvector__hooks_route\` to find best approach, then \`rufflo__agent_spawn\`
4. **Analysis request** ("analyze", "review", "audit") → spawn reviewer/security agents + analysis tools
5. **Multi-step project** → \`rufflo__task_create\` for tracking, \`rufflo__swarm_init\` for coordination
6. **Memory/recall** ("remember", "save", "what did we...") → \`rufflo__memory_store\` / \`rufflo__memory_search\`
7. **System question** ("what tools", "help") → \`guidance(topic='overview')\`
8. **Performance concern** → \`rufflo__performance_bottleneck\` + \`rufflo__performance_optimize\`
9. **GitHub task** → \`rufflo__github_*\` tools
