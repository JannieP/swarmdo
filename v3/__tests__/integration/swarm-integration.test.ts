...
describe('Swarm Integration Tests', () => {
  ...
  // swarmdo#1872 — fixed: SwarmCoordinator.executeTask now wraps the
  ...
  // swarmdo#1872 — fixed: scaleAgents({count:N}) now interprets count as
  ...
  it('should handle dynamic agent scaling', async () => {
    ...
    // Scale up: base-agent is itself a coder, so a target total of 3
    // coders means 2 new spawns — 3 agents overall, not 1+3.
