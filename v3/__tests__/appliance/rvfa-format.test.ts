 ...
 * Run: npx vitest run __tests__/appliance/rvfa-format.test.ts
 */
...
function addTestSections(writer: RvfaWriter): void {
  ...
  writer.addSection('swarmdo', Buffer.from('swarmdo-payload'), { compression: 'none' });
}
...
describe('createDefaultHeader', () => {
  ...
  it('cloud profile does not include swarmllm capability', () => {
    assert.ok(!createDefaultHeader('cloud').capabilities.includes('swarmllm'));
  });

  it('offline profile includes swarmllm capability', () => {
    assert.ok(createDefaultHeader('offline').capabilities.includes('swarmllm'));
  });
});
...
describe('RvfaWriter', () => {
  ...
  it('section offsets are correct and non-overlapping', () => {
    ...
    writer.addSection('swarmdo', Buffer.from('ccccccc'), { compression: 'none' });
...
describe('RvfaReader', () => {
  ...
  it('handles multiple sections', () => {
    ...
    writer.addSection('swarmdo', Buffer.from('f'), { compression: 'none' });
