...
const DB_CONFIG_KEY: &str = "__swarmvector_db_config__";
...
impl VectorStorage {
    ...
    pub fn new<P: AsRef<Path>>(path: P, dimensions: usize) -> Result<Self> {
        ...
        if let Some(parent) = path_ref.parent() {
            if !parent.as_os_str().is_empty() && !parent.exists() {
                std::fs::create_dir_all(parent).map_err(|e| {
                    SwarmvectorError::InvalidPath(format!("Failed to create directory: {}", e))
                })?;
            }
        }
        ...
        let path_buf = if path_ref.is_absolute() {
            ...
            std::env::current_dir()
                .map_err(|e| SwarmvectorError::InvalidPath(format!("Failed to get cwd: {}", e)))?
                .join(path_ref)
        };
                        ...
                        match component {
                            std::path::Component::ParentDir => {
                                if !normalized.pop() || !normalized.starts_with(&cwd) {
                                    return Err(SwarmvectorError::InvalidPath(
    ...
    pub fn insert(&self, entry: &VectorEntry) -> Result<VectorId> {
        if entry.vector.len() != self.dimensions {
            return Err(SwarmvectorError::DimensionMismatch {
