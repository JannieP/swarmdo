...
use swarmvector_core::types::{DistanceMetric, HnswConfig, QuantizationConfig};
use swarmvector_core::vector_db::VectorDB;
...
impl Collection {
    ...
    pub fn new(name: String, config: CollectionConfig, storage_path: String) -> Result<Self> {
        ...
        let db_options = swarmvector_core::types::DbOptions {
...
mod tests {
    use super::*;
    use swarmvector_core::types::HnswConfig;
    ...
    fn test_collection_new_with_valid_config() {
        let temp = std::env::temp_dir().join("swarmvector_test_coll_new_valid");
    ...
    fn test_collection_new_rejects_zero_dimensions() {
        let temp = std::env::temp_dir().join("swarmvector_test_coll_new_zero");
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn test_collection_stats_on_empty() {
        let temp = std::env::temp_dir().join("swarmvector_test_coll_stats_empty");
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn test_collection_touch_updates_timestamp() {
        let temp = std::env::temp_dir().join("swarmvector_test_coll_touch");
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn test_collection_debug_format() {
        let temp = std::env::temp_dir().join("swarmvector_test_coll_debug");
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
