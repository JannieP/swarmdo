...
lazy_static! {
    ...
    pub static ref SEARCH_REQUESTS_TOTAL: CounterVec = register_counter_vec!(
        Opts::new("swarmvector_search_requests_total", "Total search requests"),
        &["collection", "status"]
    ).unwrap();

    pub static ref SEARCH_LATENCY_SECONDS: HistogramVec = register_histogram_vec!(
        "swarmvector_search_latency_seconds",
    ...
    pub static ref INSERT_REQUESTS_TOTAL: CounterVec = register_counter_vec!(
        Opts::new("swarmvector_insert_requests_total", "Total insert requests"),
        &["collection", "status"]
    ).unwrap();

    pub static ref INSERT_LATENCY_SECONDS: HistogramVec = register_histogram_vec!(
        "swarmvector_insert_latency_seconds",
    ...
    pub static ref VECTORS_INSERTED_TOTAL: CounterVec = register_counter_vec!(
        Opts::new("swarmvector_vectors_inserted_total", "Total vectors inserted"),
        &["collection"]
    ).unwrap();

    // Delete metrics
    pub static ref DELETE_REQUESTS_TOTAL: CounterVec = register_counter_vec!(
        Opts::new("swarmvector_delete_requests_total", "Total delete requests"),
        &["collection", "status"]
    ).unwrap();

    // Collection metrics
    pub static ref VECTORS_TOTAL: GaugeVec = register_gauge_vec!(
        Opts::new("swarmvector_vectors_total", "Total vectors stored"),
        &["collection"]
    ).unwrap();

    pub static ref COLLECTIONS_TOTAL: Gauge = register_gauge!(
        Opts::new("swarmvector_collections_total", "Total number of collections")
    ).unwrap();

    // System metrics
    pub static ref MEMORY_USAGE_BYTES: Gauge = register_gauge!(
        Opts::new("swarmvector_memory_usage_bytes", "Memory usage in bytes")
    ).unwrap();

    pub static ref UPTIME_SECONDS: Counter = register_counter!(
        Opts::new("swarmvector_uptime_seconds", "Uptime in seconds")
    ).unwrap();
}
...
mod tests {
    ...
    fn test_gather_metrics() {
        ...
        assert!(metrics.contains("swarmvector"));
    }
