...
Core engine powering Swarmvector's intelligent request distribution, model selection, and sub-millisecond vector similarity search. Combines advanced indexing algorithms with SIMD-optimized distance calculations for maximum performance.

## 🎯 Overview

Router Core is the foundation of Swarmvector's vector database capabilities, providing:
...
Or use the full swarmvector package:
...
Router Core integrates seamlessly with the main Swarmvector database:

```rust
use swarmvector_core::VectorDB as MainDB;
...
// Or use main swarmvector-core for full features
...
- **`swarmvector-core`**: Full-featured vector database (superset of router-core)
- **`swarmvector-node`**: Node.js bindings via NAPI-RS
- **`swarmvector-wasm`**: WebAssembly bindings for browsers
