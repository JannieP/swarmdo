//! Node.js bindings for Swarmvector via NAPI-RS
//!
...
// Import new crates
use swarmvector_collections::CollectionManager as CoreCollectionManager;
use swarmvector_filter::FilterExpression;
use swarmvector_metrics::{gather_metrics, HealthChecker, HealthStatus};
    ...
    fn from(options: JsDbOptions) -> Self {
        DbOptions {
            ...
            storage_path: options
                .storage_path
                .unwrap_or_else(|| "./vector.db".to_string()),
...
/// Get the version of the Swarmvector library
#[napi]
...
#[napi]
pub fn hello() -> String {
    "Hello from Swarmvector Node.js bindings!".to_string()
}
...
impl From<JsCollectionConfig> for swarmvector_collections::CollectionConfig {
    fn from(config: JsCollectionConfig) -> Self {
        swarmvector_collections::CollectionConfig {
...
impl From<swarmvector_collections::CollectionStats> for JsCollectionStats {
    fn from(stats: swarmvector_collections::CollectionStats) -> Self {
        JsCollectionStats {
...
impl CollectionManager {
    ...
    pub async fn create_collection(&self, name: String, config: JsCollectionConfig) -> Result<()> {
        let core_config: swarmvector_collections::CollectionConfig = config.into();
