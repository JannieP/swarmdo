...
[![npm version](https://img.shields.io/npm/v/swarmvector.svg)](https://www.npmjs.com/package/swarmvector)
...
**Swarmvector Node.js eliminates these limitations.**
...
```
Operation              Swarmvector    Pure JS     Speedup
────────────────────────────────────────────────────────
...
Browser Support         Via WASM    Use swarmvector-wasm package
...
```bash
node -e "const {version} = require('swarmvector'); console.log('Swarmvector v' + version())"
```
...
```typescript
import { VectorDB, JsDbOptions, JsSearchQuery } from 'swarmvector';
...
Get the Swarmvector library version.

```typescript
import { version } from 'swarmvector';
console.log(`Swarmvector v${version()}`);
```
...
| Feature | Swarmvector | Pure JS | Python (Faiss) | Pinecone |
|---------|----------|---------|----------------|----------|
...
**Error**: `Cannot find module 'swarmvector'`
...
# Benchmark performance
cargo bench -p swarmvector-bench
```

## 📖 Documentation

- **[Main Documentation](../../docs/README.md)** - Complete Swarmvector documentation
...
- **[swarmvector-core](../swarmvector-core)** - Core Rust implementation
...
- **[swarmvector-bench](../swarmvector-bench)** - Benchmarking suite
