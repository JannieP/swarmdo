...
impl LlmBackend for MockBackend {
    ...
    fn generate(&self, prompt: &str, _params: GenerateParams) -> Result<String> {
        if !self.loaded {
            return Err(rufllm::RufLLMError::Backend("Model not loaded".to_string()));
        }
    ...
    ) -> Result<Box<dyn Iterator<Item = Result<rufllm::backends::GeneratedToken>> + Send + '_>>
    {
        if !self.loaded {
            return Err(rufllm::RufLLMError::Backend("Model not loaded".to_string()));
        }
        ...
        let tokens = vec![
            rufllm::backends::GeneratedToken {
                id: 1,
