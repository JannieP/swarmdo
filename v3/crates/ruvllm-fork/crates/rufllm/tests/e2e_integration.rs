...
/// Create a test RufLLM configuration
fn create_test_config(storage_path: &str) -> RufLLMConfig {
    RufLLMConfig {
...
async fn test_full_inference_pipeline() {
    ...
    // let engine = RufLLMEngine::new(config).unwrap();
...
fn test_engine_creation() {
    ...
    let result = RufLLMEngine::new(config);
...
async fn test_concurrent_requests() {
    let temp_dir = create_test_dir();
    let config = create_test_config(temp_dir.path().to_str().unwrap());
    let engine = Arc::new(RufLLMEngine::new(config).unwrap());
