...
//! cargo test --package rufllm ruvltra_tests
//!
//! # Run with ANE support (Apple Silicon only)
//! cargo test --package rufllm --features coreml ruvltra_tests
//!
//! # Run with full feature set
//! cargo test --package rufllm --all-features ruvltra_tests
//! ```

use rufllm::backends::{
    AneCapabilities, ComputeUnits, ModelArchitecture, ModelConfig, Quantization,
};
use rufllm::gguf::quantization::{dequantize_tensor, GgufQuantType, QuantizedTensor};
use rufllm::kernels::ane_ops::{
