...
//! End-to-end Integration Tests for RufLLM
//!
...
//! ```bash
//! cargo test -p rufllm --test e2e_integration_test
//! ```
//!
//! ### With a real model file:
//! ```bash
//! TEST_MODEL_PATH=/path/to/model.gguf cargo test -p rufllm --test e2e_integration_test -- --ignored
//! ```
//!
//! ### Run specific test with model:
//! ```bash
//! TEST_MODEL_PATH=/path/to/model.gguf cargo test -p rufllm --test e2e_integration_test test_real_model_generation -- --ignored
//! ```
...
impl LlmBackend for MockLlmBackend {
    ...
    fn generate(&self, prompt: &str, params: GenerateParams) -> Result<String> {
        if !self.model_loaded.load(Ordering::SeqCst) {
            return Err(RufLLMError::Config("Model not loaded".to_string()));
        }
    ...
    fn model_info(&self) -> Option<rufllm::backends::ModelInfo> {
        if self.is_model_loaded() {
            Some(rufllm::backends::ModelInfo {
