...
//! cargo test --package rufllm ruvltra_e2e
//!
//! # Run only tests that don't require model files
//! cargo test --package rufllm ruvltra_e2e -- --skip model_required
//!
//! # Run with full features on Apple Silicon
//! cargo test --package rufllm --features coreml,hybrid-ane ruvltra_e2e
//! ```

use rufllm::backends::{GenerateParams, LlmBackend, ModelArchitecture, ModelConfig, Quantization};
use rufllm::error::{Result, RufLLMError};
use rufllm::gguf::quantization::GgufQuantType;
use rufllm::kernels::is_ane_available;
