...
impl PageBlock {
    ...
    pub fn append(
        ...
        if keys.len() != values.len() {
            return Err(RufLLMError::PagedAttention(
        ...
        if end_offset > self.keys.len() {
            return Err(RufLLMError::PagedAttention("Block overflow".to_string()));
        }
...
impl PageTable {
    ...
    pub fn allocate_block(&self, sequence_id: &str) -> Result<usize> {
        ...
        let block_id = block_id
            .ok_or_else(|| RufLLMError::OutOfMemory("No free blocks available".to_string()))?;
    ...
    pub fn free_block(&self, block_id: usize) -> Result<()> {
        ...
        if block_id >= blocks.len() {
            return Err(RufLLMError::PagedAttention(format!(
                "Invalid block ID: {}",
                block_id
            )));
        }
...
impl PagedAttention {
    ...
    pub fn forward(&self, query: &[f32], sequence_id: &str, scale: f32) -> Result<Vec<f32>> {
        let blocks = self.page_table.get_blocks(sequence_id).ok_or_else(|| {
            RufLLMError::PagedAttention(format!("Sequence not found: {}", sequence_id))
        })?;
