...
impl MetalPipelines {
    ...
    fn create_pipeline(
        ...
        let function = library.get_function(function_name, None).map_err(|e| {
            RufLLMError::Backend(format!("Failed to get function '{}': {}", function_name, e))
        })?;
...
impl PipelineCache {
    ...
    pub fn get_or_compile(
        ...
        let library = self
            ...
            .map_err(|e| RufLLMError::Backend(format!("Shader compilation failed: {}", e)))?;

        let function = library
            .get_function(function_name, None)
            .map_err(|e| RufLLMError::Backend(format!("Function not found: {}", e)))?;

        let pipeline = self
            .device
            .new_compute_pipeline_state_with_function(&function)
            .map_err(|e| RufLLMError::Backend(format!("Pipeline creation failed: {}", e)))?;
