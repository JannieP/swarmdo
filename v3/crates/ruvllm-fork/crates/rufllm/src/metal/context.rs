...
impl MetalContext {
    ...
    pub fn new(config: MetalConfig) -> Result<Self> {
        let device = Device::system_default()
            .ok_or_else(|| RufLLMError::Backend("No Metal device found".to_string()))?;
        ...
        let library = device
            ...
            .map_err(|e| RufLLMError::Backend(format!("Failed to compile shaders: {}", e)))?;
    ...
    pub fn fused_swiglu(&self, gate: &[f32], up: &[f32]) -> Result<Vec<f32>> {
        let pipeline = self.pipelines.fused_swiglu.as_ref().ok_or_else(|| {
            RufLLMError::Backend("Fused SwiGLU not available on this device".to_string())
        })?;
    ...
    pub fn int4_gemv(
        ...
        let pipeline = self
            ...
            .ok_or_else(|| {
                RufLLMError::Backend("INT4 GEMV not available on this device".to_string())
            })?;
    ...
    pub fn yarn_attention(
        ...
        let pipeline = self.pipelines.yarn_attention.as_ref().ok_or_else(|| {
            RufLLMError::Backend("YaRN attention not available on this device".to_string())
        })?;
