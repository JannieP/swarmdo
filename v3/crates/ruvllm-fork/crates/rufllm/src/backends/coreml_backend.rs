...
//!
//! ```rust,ignore
//! use rufllm::backends::CoreMLBackend;
//!
    ...
    impl CoreMLModelHandle {
        ...
        pub fn load(path: &Path, compute_units: ComputeUnits) -> Result<Self> {
            ...
            let url = NSURL::from_file_path(path).ok_or_else(|| {
                RufLLMError::CoreML(format!("Invalid model path: {}", path.display()))
            })?;
        ...
        pub fn predict(&self, input_name: &str, token_ids: &[i32]) -> Result<Vec<f32>> {
            ...
            unsafe {
                ...
                let output_name = self
                    .output_names
                    .first()
                    .ok_or_else(|| RufLLMError::CoreML("No output features found".to_string()))?;
                ...
                let output_value = MLFeatureProvider::featureValueForName(&*output, &output_key)
                    .ok_or_else(|| {
                        RufLLMError::CoreML(format!("Output feature '{}' not found", output_name))
                    })?;

                // Get the multi-array from the output
                let output_array = output_value.multiArrayValue().ok_or_else(|| {
                    RufLLMError::CoreML("Output is not a multi-array".to_string())
                })?;
        ...
        pub fn get_embeddings(
            ...
            unsafe {
                ...
                let output_name = embedding_name.ok_or_else(|| {
                    RufLLMError::CoreML("No embedding output found in model".to_string())
                })?;
                ...
                let output_value = MLFeatureProvider::featureValueForName(&*output, &output_key)
                    .ok_or_else(|| {
                        RufLLMError::CoreML(format!("Embedding output '{}' not found", output_name))
                    })?;

                let output_array = output_value.multiArrayValue().ok_or_else(|| {
                    RufLLMError::CoreML("Embedding output is not a multi-array".to_string())
                })?;
...
impl<'a> CoreMLStreamIterator<'a> {
    ...
    fn sample_token(&self, logits: &[f32]) -> Result<u32> {
        ...
        if logits.is_empty() {
            return Err(RufLLMError::Generation("Empty logits".to_string()));
        }
...
impl CoreMLBackend {
    ...
    pub fn new() -> Result<Self> {
        ...
        std::fs::create_dir_all(&cache_dir).map_err(|e| {
            RufLLMError::Storage(format!("Failed to create Core ML cache directory: {}", e))
        })?;
    ...
    fn convert_to_coreml(&self, _gguf_path: &Path, _output_path: &Path) -> Result<()> {
        ...
        Err(RufLLMError::NotImplemented(
        ...
        match extension {
            Some("mlmodelc") => {
                ...
                if !model_mil.exists() && !coreml_data.exists() && !weights.exists() {
                    return Err(RufLLMError::CoreML(format!(
            ...
            Some("mlmodel") => {
                ...
                if !path.is_file() {
                    return Err(RufLLMError::CoreML(".mlmodel should be a file".to_string()));
                }
            }
            Some("mlpackage") => {
...
impl LlmBackend for CoreMLBackend {
    ...
    fn generate(&self, prompt: &str, params: GenerateParams) -> Result<String> {
        if !self.loaded {
            return Err(RufLLMError::InvalidOperation("No model loaded".to_string()));
        }

        #[cfg(all(target_os = "macos", target_arch = "aarch64", feature = "candle"))]
        {
            let model_handle = self.model_handle.as_ref().ok_or_else(|| {
                RufLLMError::InvalidOperation("Model handle not initialized".to_string())
            })?;
