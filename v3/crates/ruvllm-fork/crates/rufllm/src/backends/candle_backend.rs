...
//!
//! ```rust,ignore
//! use rufllm::backends::CandleBackend;
//! use rufllm::tokenizer::{ChatMessage, ChatTemplate};
//!
...
mod candle_impl {
    ...
    ///
    /// For new code, prefer using `RuvTokenizer` directly via `rufllm::tokenizer`.
    ...
    impl Tokenizer for CandleTokenizer {
        fn encode(&self, text: &str) -> Result<Vec<u32>> {
            let encoding = self
                .inner
                .encode(text, false)
                .map_err(|e| RufLLMError::Tokenization(format!("Tokenization failed: {}", e)))?;
        ...
        fn decode(&self, tokens: &[u32]) -> Result<String> {
            self.inner
                .decode(tokens, true)
                .map_err(|e| RufLLMError::Tokenization(format!("Decoding failed: {}", e)))
        }
    ...
    impl CandleBackend {
        ...
        pub fn new() -> Result<Self> {
            ...
            std::fs::create_dir_all(&cache_dir).map_err(|e| {
                RufLLMError::Storage(format!("Failed to create cache directory: {}", e))
            })?;
        ...
        pub fn apply_chat_template(&self, messages: &[ChatMessage]) -> Result<String> {
            let tokenizer = self
                .ruv_tokenizer
                .as_ref()
                .ok_or_else(|| RufLLMError::InvalidOperation("No tokenizer loaded".to_string()))?;
                ...
                DeviceType::Cuda(device_id) => {
                    #[cfg(feature = "cuda")]
                    {
                        Device::new_cuda(device_id).map_err(|e| {
                            RufLLMError::Backend(format!("Failed to initialize CUDA device: {}", e))
                        })
                    }
        ...
        pub fn load_from_hub(&mut self, model_id: &str, config: &ModelConfig) -> Result<()> {
            ...
            let api = Api::new().map_err(|e| {
                RufLLMError::Storage(format!("Failed to initialize HuggingFace API: {}", e))
            })?;
            ...
            let tokenizer_path = repo.get("tokenizer.json").map_err(|e| {
                RufLLMError::NotFound(format!("Tokenizer not found for {}: {}", model_id, e))
            })?;
            ...
            let config_path = repo.get("config.json").map_err(|e| {
                RufLLMError::NotFound(format!("Config not found for {}: {}", model_id, e))
            })?;
        ...
        fn get_safetensors_files(&self, repo: &hf_hub::api::sync::ApiRepo) -> Result<Vec<PathBuf>> {
            ...
            if let Ok(index_path) = repo.get("model.safetensors.index.json") {
                let index_str = std::fs::read_to_string(&index_path).map_err(|e| {
                    RufLLMError::Storage(format!("Failed to read safetensors index: {}", e))
                })?;
        ...
        pub fn load_tokenizer(&mut self, path: &Path) -> Result<()> {
            ...
            let tokenizer = HfTokenizer::from_file(path)
                .map_err(|e| RufLLMError::Storage(format!("Failed to load tokenizer: {}", e)))?;
        ...
        pub fn load_gguf(&mut self, path: &Path, config: &ModelConfig) -> Result<()> {
            ...
            let mut file = std::fs::File::open(path)
                .map_err(|e| RufLLMError::Storage(format!("Failed to open GGUF file: {}", e)))?;

            // Read GGUF content
            let gguf_content = gguf_file::Content::read(&mut file)
                .map_err(|e| RufLLMError::Storage(format!("Failed to read GGUF file: {}", e)))?;
            ...
            let model_weights =
                qlama::ModelWeights::from_gguf(gguf_content, &mut file, &self.device).map_err(
                    |e| RufLLMError::Model(format!("Failed to load GGUF weights: {}", e)),
                )?;
        ...
        pub fn load_safetensors(
            ...
            let config_str = std::fs::read_to_string(config_path)
                .map_err(|e| RufLLMError::Storage(format!("Failed to read config: {}", e)))?;
            ...
            let vb = unsafe {
                VarBuilder::from_mmaped_safetensors(weights_files, dtype, &self.device)
                    .map_err(|e| RufLLMError::Model(format!("Failed to load safetensors: {}", e)))?
            };
            ...
            let inner = match config.architecture {
                ModelArchitecture::Mistral => {
                    ...
                    let model = mistral_model::Model::new(&mistral_config, vb).map_err(|e| {
                        RufLLMError::Model(format!("Failed to create Mistral model: {}", e))
                    })?;
                ...
                ModelArchitecture::Llama => {
                    ...
                    let model = llama_model::Llama::load(vb, &llama_config).map_err(|e| {
                        RufLLMError::Model(format!("Failed to create Llama model: {}", e))
                    })?;

                    // Create KV cache for the Llama model
                    let cache = llama_model::Cache::new(true, dtype, &llama_config, &self.device)
                        .map_err(|e| {
                        RufLLMError::Model(format!("Failed to create Llama cache: {}", e))
                    })?;
                ...
                _ => {
                    return Err(RufLLMError::Config(format!(
        ...
        fn sample_token(
            ...
            let logits = if logits.dims().len() == 3 {
                logits.squeeze(0).map_err(|e| {
                    RufLLMError::Generation(format!("Failed to squeeze logits: {}", e))
                })?
            } else {
                logits.clone()
            };
            ...
            let last_logits = if logits.dims().len() == 2 {
                let seq_len = logits.dim(0).map_err(|e| {
                    RufLLMError::Generation(format!("Failed to get seq_len: {}", e))
                })?;
                logits.i(seq_len - 1).map_err(|e| {
                    RufLLMError::Generation(format!("Failed to get last logits: {}", e))
                })?
            } else {
                logits
            };
            ...
            let mut logits_vec: Vec<f32> = last_logits
                .to_vec1()
                .map_err(|e| RufLLMError::Generation(format!("Failed to convert logits: {}", e)))?;
            ...
            let filtered_tensor =
                Tensor::from_vec(filtered_logits, indexed_logits.len(), &self.device)
                    .map_err(|e| RufLLMError::Generation(e.to_string()))?;
            ...
            let sampled_idx = logits_processor
                ...
                .map_err(|e| RufLLMError::Generation(format!("Sampling failed: {}", e)))?;
        ...
        fn mock_stream(&self, prompt: &str, params: &GenerateParams) -> Result<TokenStream> {
            ...
            {
                "Here's an example function:\n\n```rust\nfn hello() {\n    println!(\"Hello from RufLLM!\");\n}\n```"
            } else {
                "I understand your request. This is a streaming response from RufLLM mock mode."
            };
    ...
    impl LlmBackend for CandleBackend {
        fn load_model(&mut self, model_id: &str, config: ModelConfig) -> Result<()> {
            ...
            if path.exists() {
                ...
                if path.extension().map_or(false, |e| e == "gguf") {
                    ...
                    if !config_file.exists() {
                        return Err(RufLLMError::NotFound(format!(
                    ...
                    if weights_files.is_empty() {
                        return Err(RufLLMError::NotFound(
            ...
            {
                Err(RufLLMError::NotFound(format!(
        ...
        fn generate(&self, prompt: &str, params: GenerateParams) -> Result<String> {
            ...
            if prompt_len >= max_ctx {
                return Err(RufLLMError::Generation(format!(
        ...
        fn generate_stream(
            ...
            let iter = stream.filter_map(|event_result| match event_result {
                ...
                Ok(StreamEvent::Error(msg)) => Some(Err(RufLLMError::Generation(msg))),
                Err(e) => Some(Err(e)),
            });
...
fn estimate_gguf_memory(path: &Path) -> Result<usize> {
    let metadata = std::fs::metadata(path)
        .map_err(|e| RufLLMError::Storage(format!("Failed to read file metadata: {}", e)))?;
...
mod tests {
    ...
    fn test_get_cache_dir() {
        let cache_dir = get_cache_dir();
        assert!(cache_dir.to_string_lossy().contains("rufllm"));
    }
