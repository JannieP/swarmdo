...
//!
//! ```rust,ignore
//! use rufllm::backends::{MistralBackend, MistralConfig};
//!
...
impl MistralBackend {
    ...
    pub fn load_xlora_adapter(&self, name: &str, path: &Path) -> Result<()> {
        let manager = self
            .xlora_manager
            .as_ref()
            .ok_or_else(|| RufLLMError::Config("X-LoRA not configured".to_string()))?;
    ...
    pub fn apply_isq(&mut self) -> Result<()> {
        if !self.is_model_loaded() {
            return Err(RufLLMError::InvalidOperation(
        ...
        let _isq_config = self
            ...
            .isq
            .as_ref()
            .ok_or_else(|| RufLLMError::Config("ISQ not configured".to_string()))?;
    ...
    fn generate_with_mistral_rs(
        ...
        model
            .get_sender()
            .map_err(|e| RufLLMError::Compute(format!("Failed to get model sender: {}", e)))?
            .blocking_send(request)
            .map_err(|e| RufLLMError::Compute(format!("Failed to send request to model: {}", e)))?;

        // Wait for response
        let response = rx
            .recv()
            .map_err(|e| RufLLMError::Compute(format!("Failed to receive response: {}", e)))?;
        ...
        match response {
            ...
            Response::InternalError(e) => {
                Err(RufLLMError::Compute(format!("Model internal error: {}", e)))
            }
            Response::ValidationError(e) => {
                Err(RufLLMError::Config(format!("Validation error: {}", e)))
            }
            Response::ModelError(msg, _) => {
                Err(RufLLMError::Compute(format!("Model error: {}", msg)))
            }
            _ => Err(RufLLMError::Compute("Unexpected response type".to_string())),
        }
    }
