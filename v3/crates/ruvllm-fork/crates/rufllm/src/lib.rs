//! # RufLLM - LLM Serving Runtime with Rufvector Integration
//!
//! RufLLM is an edge-focused LLM serving runtime designed for portable, high-performance
//! inference across heterogeneous hardware. It integrates with Rufvector for intelligent
//! memory capabilities, enabling continuous self-improvement through SONA learning.
//!
//! ## Architecture
//!
//! RufLLM uses Rufvector as a unified memory layer with three distinct roles:
//!
...
//! - [`PolicyStore`]: Rufvector-backed policy storage with semantic search
...
//!
//! ```rust,ignore
//! use rufllm::{RufLLMConfig, RufLLMEngine};
//!
//! // Create engine with default configuration
//! let config = RufLLMConfig::default();
//! let engine = RufLLMEngine::new(config)?;
//!
//! // Create a session
...
pub use error::{Result, RufLLMError};
pub use gguf::{
    GgufFile,
    GgufHeader,
...
// Rufvector integration exports (unified entry point for all Rufvector capabilities)
pub use capabilities::{
    gate_feature, gate_feature_or, RufvectorCapabilities, ATTENTION_AVAILABLE, GNN_AVAILABLE,
    GRAPH_AVAILABLE, HNSW_AVAILABLE, PARALLEL_AVAILABLE, SIMD_AVAILABLE, SONA_AVAILABLE,
};
pub use rufvector_integration::{
    IndexStats,
    IntegrationConfig,
    IntegrationStats,
    // Intelligence layer
...
/// RufLLM engine configuration.
///
/// This configuration struct controls all aspects of the RufLLM engine,
...
///
/// ```rust,ignore
/// use rufllm::{RufLLMConfig, PagedAttentionConfig, KvCacheConfig};
///
/// let config = RufLLMConfig {
///     storage_path: "/var/rufllm".to_string(),
///     max_sessions: 500,
///     embedding_dim: 1024,
///     ..Default::default()
/// };
...
#[derive(Debug, Clone)]
pub struct RufLLMConfig {
    /// Path to Rufvector storage
...
impl Default for RufLLMConfig {
    fn default() -> Self {
        Self {
            storage_path: ".rufllm".to_string(),
...
/// Main RufLLM engine for LLM inference with intelligent memory.
///
/// The `RufLLMEngine` is the primary entry point for RufLLM, providing:
///
/// - **Session Management**: Create and manage user sessions with state persistence
/// - **Policy Storage**: Rufvector-backed semantic search for runtime policies
...
///
/// ```rust,ignore
/// use rufllm::{RufLLMEngine, RufLLMConfig};
///
...
/// +-------------------+     +-------------------+
/// ```
pub struct RufLLMEngine {
    /// Configuration
    config: RufLLMConfig,
    /// Policy store backed by Rufvector
    policy_store: PolicyStore,
    /// Session manager
    session_manager: SessionManager,
    /// Session index backed by Rufvector
...
impl RufLLMEngine {
    /// Create a new RufLLM engine with the given configuration.
    ///
    ...
    /// use rufllm::{RufLLMEngine, RufLLMConfig};
    ///
    /// let engine = RufLLMEngine::new(RufLLMConfig::default())?;
    /// ```
    pub fn new(config: RufLLMConfig) -> Result<Self> {
    ...
    /// Rufvector for semantic retrieval.
    ///
    /// # Arguments
    ///
    ...
    pub fn create_session(&self, user_id: Option<&str>) -> Result<Session> {
        ...
        // Index the session in Rufvector
    ...
    ///
    /// ```rust,ignore
    /// use rufllm::{WitnessEntry, LatencyBreakdown, RoutingDecision};
    ///
