//! Policy Memory Store
//!
//! Stores learned policies and thresholds in Rufvector for semantic search
...
use rufvector_core::{AgenticDB, SearchQuery, VectorEntry};
...
/// Policy entry stored in Rufvector
...
impl PolicyStore {
    ...
    pub fn new(storage_path: &str, embedding_dim: usize) -> Result<Self> {
        ...
        let db = AgenticDB::new(options).map_err(|e| RufLLMError::Storage(e.to_string()))?;
    ...
    pub fn store(&self, entry: PolicyEntry) -> Result<Uuid> {
        ...
        self.db
            .insert(vector_entry)
            .map_err(|e| RufLLMError::Storage(e.to_string()))?;
