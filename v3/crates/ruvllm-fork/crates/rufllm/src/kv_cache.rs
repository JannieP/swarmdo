...
impl TwoTierKvCache {
    ...
    pub fn append(&self, keys: &[f32], values: &[f32]) -> Result<()> {
        ...
        if keys.len() != values.len() {
            return Err(RufLLMError::KvCache(
    ...
    pub fn append(&self, keys: &[f32], values: &[f32]) -> Result<()> {
        ...
        while !remaining_keys.is_empty() {
            ...
            if need_new_block {
                ...
                .ok_or_else(|| {
                    RufLLMError::OutOfMemory("Failed to allocate KV block from pool".to_string())
                })?;
...
///
/// ```rust,ignore
/// use rufllm::kv_cache::{TurboQuantKvCache, TurboQuantKvCacheConfig};
///
