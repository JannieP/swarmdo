...
//!
//! `rufllm` reaches the GPU through Candle's abstraction.  Candle does not
    ...
    COMPILED_PTX
        .get_or_try_init(|| {
            compile_ptx(ACT_KERNEL_SRC)
                .map_err(|e| RufLLMError::Model(format!("nvrtc compile act_fused: {e}")))
        })
        .cloned()
}
...
fn load_module(ctx: &Arc<CudaContext>) -> Result<Arc<CudaModule>> {
    ...
    ctx.load_module(ptx)
        .map_err(|e| RufLLMError::Model(format!("cudarc load_module: {e}")))
}
...
impl FusedActKernel {
    ...
    pub fn new(n: usize) -> Result<Self> {
        let ctx = CudaContext::new(0)
            .map_err(|e| RufLLMError::Model(format!("cudarc CudaContext::new: {e}")))?;
        ...
        let cum = stream
            ...
            .map_err(|e| RufLLMError::Model(format!("htod cum: {e}")))?;
        let not_halted = stream
            .clone_htod(vec![1.0f32; n].as_slice())
            .map_err(|e| RufLLMError::Model(format!("htod not_halted: {e}")))?;
        let depth = stream
            .clone_htod(vec![0.0f32; n].as_slice())
            .map_err(|e| RufLLMError::Model(format!("htod depth: {e}")))?;
        let w_out = stream
            .clone_htod(vec![0.0f32; n].as_slice())
            .map_err(|e| RufLLMError::Model(format!("htod w_out: {e}")))?;
    ...
    pub fn step(
        ...
        let p_flat = p_tensor
            .flatten_all()
            .map_err(|e| RufLLMError::Model(format!("flatten p: {e}")))?;
        ...
        match p_flat.dtype() {
            DType::F32 => {
                let p_host: Vec<f32> = p_flat
                    .to_vec1()
                    .map_err(|e| RufLLMError::Model(format!("to_vec1 f32: {e}")))?;
                let p_dev = self
                    .stream
                    .clone_htod(p_host.as_slice())
                    .map_err(|e| RufLLMError::Model(format!("htod p_f32: {e}")))?;
                let f = self
                    .module
                    .load_function("act_fused_step_f32")
                    .map_err(|e| RufLLMError::Model(format!("load_function f32: {e}")))?;
                unsafe {
                    self.stream
                        .launch_builder(&f)
                        .arg(&p_dev)
                        ...
                        .arg(&step_f32)
                        .launch(cfg)
                        .map_err(|e| RufLLMError::Model(format!("launch f32: {e}")))?;
                }
            }
            ...
            DType::BF16 => {
                let p_host: Vec<half::bf16> = p_flat
                    .to_vec1()
                    .map_err(|e| RufLLMError::Model(format!("to_vec1 bf16: {e}")))?;
                let p_u16: Vec<u16> = p_host.iter().map(|x| x.to_bits()).collect();
                let p_dev = self
                    .stream
                    .clone_htod(p_u16.as_slice())
                    .map_err(|e| RufLLMError::Model(format!("htod p_bf16: {e}")))?;
                let f = self
                    .module
                    .load_function("act_fused_step_bf16")
                    .map_err(|e| RufLLMError::Model(format!("load_function bf16: {e}")))?;
                unsafe {
                    self.stream
                        .launch_builder(&f)
                        .arg(&p_dev)
                        ...
                        .arg(&step_f32)
                        .launch(cfg)
                        .map_err(|e| RufLLMError::Model(format!("launch bf16: {e}")))?;
                }
            }
        ...
        let w_host = self
            ...
            .map_err(|e| RufLLMError::Model(format!("dtoh w_out: {e}")))?;
        let w_cpu = Tensor::from_slice(&w_host, (b, seq, 1), &Device::Cpu)
            .map_err(|e| RufLLMError::Model(format!("from_slice w: {e}")))?;
        w_cpu
            .to_device(p_tensor.device())
            .map_err(|e| RufLLMError::Model(format!("to_device w: {e}")))
    }
    ...
    pub fn all_halted(&self) -> Result<bool> {
        let v = self
            ...
            .map_err(|e| RufLLMError::Model(format!("dtoh not_halted: {e}")))?;
    ...
    pub fn depths(&self) -> Result<Vec<usize>> {
        let v = self
            ...
            .map_err(|e| RufLLMError::Model(format!("dtoh depth: {e}")))?;
...
pub unsafe fn with_tensor_f32_ptr<R, F: FnOnce(u64) -> R>(tensor: &Tensor, f: F) -> Result<R> {
    ...
    let cuda_dev = tensor
        .device()
        .as_cuda_device()
        .map_err(|e| RufLLMError::Model(format!("not CUDA: {e}")))?;
    let stream = cuda_dev.cuda_stream();

    let (storage, layout) = tensor.storage_and_layout();
    let Storage::Cuda(ref cs) = *storage else {
        return Err(RufLLMError::Model("tensor not on CUDA device".into()));
    };
    let slice = cs
        .as_cuda_slice::<f32>()
        .map_err(|e| RufLLMError::Model(format!("dtype: {e}")))?;
...
impl FusedActZeroCopy {
    ...
    pub fn new(n: usize, device: &candle_core::Device) -> Result<Self> {
        ...
        let w_candle = candle_core::Tensor::zeros((n,), DType::F32, device)
            .map_err(|e| RufLLMError::Model(format!("w_candle alloc: {e}")))?;
        ...
        match p_tensor.dtype() {
            DType::F32 => {
                let f = self
                    ...
                    .module
                    .load_function("act_fused_step_f32")
                    .map_err(|e| RufLLMError::Model(format!("load_function: {e}")))?;
                ...
                .map_err(|e| RufLLMError::Model(format!("zero-copy launch: {e}")))?
                .map_err(|e| RufLLMError::Model(format!("inner launch: {e}")))?
                .map_err(|e| RufLLMError::Model(format!("kernel: {e}")))?;
            }
            other => {
                return Err(RufLLMError::Model(format!(
