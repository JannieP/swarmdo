...
impl RdtConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_loops == 0 {
            return Err(RufLLMError::Config("RDT: max_loops must be >= 1".into()));
        }
...
impl From<RdtCompatibilityError> for RufLLMError {
    fn from(e: RdtCompatibilityError) -> Self {
        RufLLMError::Model(e.to_string())
    }
}
...
mod candle_impl {
    ...
    impl RdtModel {
        ...
        pub fn generate(
            ...
            if prompt_ids.is_empty() {
                return Err(RufLLMError::Generation("empty prompt".into()));
            }
    ...
    fn cand(e: candle_core::Error) -> RufLLMError {
        RufLLMError::Model(format!("candle (rdt): {e}"))
    }
}
