...
//!
//! RuvLTRA (Rufvector Ultra-Lightweight Transformer Runtime Architecture) is an
...
//!
//! ```rust,ignore
//! use rufllm::models::ruvltra::{RuvLtraConfig, RuvLtraModel, AneOptimization};
//!
...
impl RuvLtraModel {
    ...
    pub fn forward(
        ...
        let lm_weights = if self.tie_word_embeddings {
            ...
            self.lm_head
                .as_ref()
                .ok_or_else(|| RufLLMError::InvalidOperation("No LM head weights".to_string()))?
        };
