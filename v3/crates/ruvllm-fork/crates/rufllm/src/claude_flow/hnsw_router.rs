...
//!
//! Provides 150x faster pattern search for task routing using rufvector-core's HNSW index.
...
use rufvector_core::types::{DistanceMetric, HnswConfig, SearchResult};
...
///
/// Uses rufvector-core's HNSW index for 150x faster pattern search compared to
...
impl HnswRouter {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>> {
        ...
        bincode::serde::encode_to_vec(&state, bincode::config::standard())
            .map_err(|e| RufLLMError::Serialization(e.to_string()))
    }
