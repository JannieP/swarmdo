...
//!
//! ```rust,ignore
//! use rufllm::gguf::{GgufLoader, LoadConfig, ModelInitializer};
//! use std::path::Path;
//!
//! // Load GGUF file
...
impl WeightTensor {
    ...
    pub fn from_loaded(tensor: &LoadedTensor) -> Result<Self> {
        if let Some(ref data) = tensor.data_f32 {
            ...
            Err(RufLLMError::Model("Tensor has no data".to_string()))
        }
    }
}
...
impl ModelInitializer {
    ...
    pub fn new(weights: LoadedWeights) -> Result<Self> {
        let architecture = weights
            .architecture()
            .ok_or_else(|| RufLLMError::Model("Cannot determine model architecture".to_string()))?;
    ...
    fn load_tensor(&self, name: &str) -> Result<WeightTensor> {
        ...
        Err(RufLLMError::NotFound(format!("Tensor not found: {}", name)))
    }
