...
//!
//! ```rust,ignore
//! use rufllm::gguf::{GgufLoader, LoadProgress, LoadConfig};
//! use std::path::Path;
//!
...
impl GgufLoader {
    ...
    pub fn load_weights(&self) -> Result<LoadedWeights> {
        ...
        let mapper = self.mapper.as_ref().ok_or_else(|| {
            RufLLMError::Model("Unknown architecture, cannot map tensor names".to_string())
        })?;
...
impl StreamingLoader {
    ...
    pub fn load_embeddings(&self) -> Result<Vec<LoadedTensor>> {
        let mapper = self
            ...
            .mapper
            .as_ref()
            .ok_or_else(|| RufLLMError::Model("Unknown architecture".to_string()))?;
