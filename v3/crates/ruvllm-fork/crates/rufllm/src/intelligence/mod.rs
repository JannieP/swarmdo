...
//! to feed quality signals into RufLLM's learning loops (SONA, embedding
...
//!
//! ```rust,ignore
//! use rufllm::intelligence::{IntelligenceLoader, FileSignalProvider};
...
/// Trait for external systems that supply quality signals to RufLLM.
///
...
/// ```rust,ignore
/// use rufllm::intelligence::FileSignalProvider;
/// use std::path::PathBuf;
///
...
/// Custom provider:
/// ```rust,ignore
/// use rufllm::intelligence::{IntelligenceProvider, QualitySignal, ProviderQualityWeights};
/// use rufllm::error::Result;
///
...
impl IntelligenceProvider for FileSignalProvider {
    ...
    fn load_signals(&self) -> Result<Vec<QualitySignal>> {
        ...
        if metadata.len() > MAX_SIGNAL_FILE_SIZE {
            return Err(crate::error::RufLLMError::InvalidOperation(format!(
        ...
        let signals: Vec<QualitySignal> = serde_json::from_reader(reader).map_err(|e| {
            crate::error::RufLLMError::Serialization(format!(
...
mod tests {
    ...
    impl IntelligenceProvider for FailingProvider {
        ...
        fn load_signals(&self) -> Result<Vec<QualitySignal>> {
            Err(crate::error::RufLLMError::Serialization(
