...
impl TurboQuantCompressor {
    ...
    pub fn new(config: TurboQuantConfig) -> Result<Self> {
        ...
        if block_size == 0 || (block_size & (block_size - 1)) != 0 {
            return Err(RufLLMError::Quantization(format!(
    ...
    pub fn compress_batch(&self, vectors: &[&[f32]]) -> Result<TurboQuantized> {
        if vectors.is_empty() {
            return Err(RufLLMError::Quantization(
...
impl TurboQuantCacheTier {
    ...
    pub fn get(&self, index: usize) -> Result<(Vec<f32>, Vec<f32>, usize)> {
        let pair = self.pairs.get(index).ok_or_else(|| {
            RufLLMError::Quantization(format!("KV pair index {} out of range", index))
        })?;
...
// ============================================================================
// Embedding Store for RufVector Integration
// ============================================================================

/// TurboQuant-compressed embedding store for RufVector integration.
///
...
/// other RufVector geometric operations.
///
/// ## Key property
///
...
impl TurboQuantEmbeddingStore {
    ...
    pub fn get(&self, id: u64) -> Result<Vec<f32>> {
        let index = self
            ...
            .iter()
            .position(|&i| i == id)
            .ok_or_else(|| RufLLMError::Quantization(format!("Embedding ID {} not found", id)))?;

        let compressed = self
            .compressed
            .as_ref()
            .ok_or_else(|| RufLLMError::Quantization("Store is empty".to_string()))?;
