//! Rufvector Integration Layer
//!
//! This module provides a unified integration layer for all Rufvector capabilities:
//!
//! - **rufvector-core**: HNSW index, vector storage, similarity search
//! - **rufvector-attention**: Flash Attention for efficient inference
//! - **rufvector-graph**: Knowledge graph for relationship learning
//! - **rufvector-gnn**: Graph neural networks for complex reasoning
//! - **rufvector-sona**: SONA (Self-Optimizing Neural Architecture) learning
//!
//! ## Architecture
//!
//! ```text
//! +---------------------+
//! | RufvectorIntegration |
//! |                     |
//! | +-------+ +-------+ |     +---------------+
...
//!
//! ```rust,ignore
//! use rufllm::rufvector_integration::{
//!     RufvectorIntegration, IntegrationConfig, UnifiedIndex
//! };
//!
//! // Detect capabilities and create integration
//! let config = IntegrationConfig::default();
//! let integration = RufvectorIntegration::new(config)?;
//!
...
use crate::capabilities::{
    RufvectorCapabilities, ATTENTION_AVAILABLE, GNN_AVAILABLE, GRAPH_AVAILABLE, HNSW_AVAILABLE,
    SONA_AVAILABLE,
};
...
use rufvector_core::index::hnsw::HnswIndex;
use rufvector_core::index::VectorIndex;
use rufvector_core::types::{DistanceMetric, HnswConfig, VectorId};
use rufvector_sona::{LearnedPattern, PatternConfig, ReasoningBank};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::Arc;

/// Configuration for the Rufvector integration layer
...
impl Default for IntegrationConfig {
    fn default() -> Self {
        let caps = RufvectorCapabilities::detect();
...
impl UnifiedIndex {
    ...
    pub fn new(config: IntegrationConfig) -> Result<Self> {
        ...
        .map_err(|e| RufLLMError::Rufvector(e.to_string()))?;
...
impl IntelligenceLayer {
    ...
    pub fn learn_from_outcome(
        ...
        let trajectory = Trajectory {
            ...
            session_id: "rufvector-integration".to_string(),
...
/// Main Rufvector integration entry point
///
/// This struct provides the single entry point for all Rufvector capabilities
/// in RuvLTRA.
pub struct RufvectorIntegration {
    /// Detected capabilities
    capabilities: RufvectorCapabilities,
...
impl RufvectorIntegration {
    /// Create a new Rufvector integration
    ///
    ...
    ///
    /// ```rust,ignore
    /// use rufllm::rufvector_integration::{RufvectorIntegration, IntegrationConfig};
    ///
    /// let config = IntegrationConfig::default();
    /// let integration = RufvectorIntegration::new(config)?;
    ///
    ...
    pub fn capabilities(&self) -> &RufvectorCapabilities {
        &self.capabilities
    }
    ...
    pub fn compute_attention(&self, query: &[f32], keys: &[&[f32]], values: &[&[f32]]) -> Vec<f32> {
        use rufvector_attention::{traits::Attention, ScaledDotProductAttention};
...
pub struct IntegrationStats {
    /// Detected capabilities
    pub capabilities: RufvectorCapabilities,
...
mod tests {
    ...
    #[test]
    fn test_rufvector_integration() {
        let config = IntegrationConfig {
            embedding_dim: 128,
            ..Default::default()
        };
        let integration = RufvectorIntegration::new(config).unwrap();
