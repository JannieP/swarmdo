//! Tokenizer Integration for RufLLM
//!
...
//!
//! ```rust,ignore
//! use rufllm::tokenizer::{RuvTokenizer, ChatMessage, Role};
//!
    ...
    impl RuvTokenizer {
        ...
        pub fn from_file(path: &Path) -> Result<Self> {
            let inner = HfTokenizer::from_file(path).map_err(|e| {
                RufLLMError::Tokenization(format!("Failed to load tokenizer: {}", e))
            })?;
        ...
        fn load_special_tokens_from_config(&mut self, path: &Path) -> Result<()> {
            let config_str = std::fs::read_to_string(path).map_err(|e| {
                RufLLMError::Storage(format!("Failed to read tokenizer config: {}", e))
            })?;
        ...
        pub fn encode(&self, text: &str) -> Result<Vec<u32>> {
            let encoding = self
                .inner
                .encode(text, false)
                .map_err(|e| RufLLMError::Tokenization(format!("Encoding failed: {}", e)))?;
        ...
        pub fn decode_stream(&mut self, token: u32) -> Result<Option<String>> {
            ...
            let token_text = self
                .inner
                .decode(&[token], false)
                .map_err(|e| RufLLMError::Tokenization(format!("Stream decode failed: {}", e)))?;
        ...
        pub fn apply_chat_template(&self, messages: &[ChatMessage]) -> Result<String> {
            let template = self
                .chat_template
                .as_ref()
                .ok_or_else(|| RufLLMError::Config("No chat template configured".to_string()))?;
        ...
        pub fn encode_batch(&self, texts: &[&str]) -> Result<Vec<Vec<u32>>> {
            let encodings = self
                ...
                .map_err(|e| RufLLMError::Tokenization(format!("Batch encoding failed: {}", e)))?;
