...
//!
//! ```rust,ignore
//! use rufllm::qat::{LoraQatConfig, LoraQatLayer, LoraQatTrainer};
//!
...
impl LoraQatConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.rank == 0 {
            return Err(RufLLMError::Config("LoRA rank must be > 0".to_string()));
        }
        if self.alpha <= 0.0 {
            return Err(RufLLMError::Config("LoRA alpha must be > 0".to_string()));
        }
