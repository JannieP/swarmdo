...
impl AdapterMerger {
    ...
    pub fn merge(
        ...
        if adapters.is_empty() {
            return Err(RufLLMError::Config("No adapters to merge".to_string()));
        }
    ...
    fn merge_average(
        ...
        for module in &output_config.target_modules {
            let merged_adapter = merged
                .get_adapter(module)
                .ok_or_else(|| RufLLMError::NotFound(format!("Module {:?} not found", module)))?;
...
impl HotSwapManager {
    ...
    pub fn swap(&mut self) -> Result<()> {
        if self.swapping {
            return Err(RufLLMError::Config("Swap already in progress".to_string()));
        }
