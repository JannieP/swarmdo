...
impl AdapterRegistry {
    ...
    pub fn register(
        ...
        if self.name_index.contains_key(&name) {
            return Err(RufLLMError::Adapter(format!(
    ...
    pub fn set_active(&self, id: Uuid) -> Result<()> {
        if !self.adapters.contains_key(&id) {
            return Err(RufLLMError::NotFound(format!("Adapter {} not found", id)));
        }
    ...
    pub fn set_active_by_name(&self, name: &str) -> Result<()> {
        let id = self
            ...
            .get(name)
            .map(|r| *r)
            .ok_or_else(|| RufLLMError::NotFound(format!("Adapter '{}' not found", name)))?;
        self.set_active(id)
    }
