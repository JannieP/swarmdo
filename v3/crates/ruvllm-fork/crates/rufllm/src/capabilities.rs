//! Rufvector Capabilities Detection
//!
//! Auto-detection and graceful fallback for Rufvector features.
//! This module provides compile-time and runtime detection of available features.
//!
//! ## Available Feature Flags
//!
//! - `HNSW_AVAILABLE`: HNSW index from rufvector-core
//! - `ATTENTION_AVAILABLE`: Flash Attention from rufvector-attention
//! - `GRAPH_AVAILABLE`: Knowledge graph from rufvector-graph
//! - `GNN_AVAILABLE`: Graph neural networks from rufvector-gnn
//! - `SONA_AVAILABLE`: SONA learning from rufvector-sona
//!
...
pub const HNSW_AVAILABLE: bool = true; // Always available via rufvector-core
...
pub const SONA_AVAILABLE: bool = true; // Always available via rufvector-sona
...
static CAPABILITIES: OnceLock<RufvectorCapabilities> = OnceLock::new();

/// Rufvector capabilities flags
///
/// Indicates which Rufvector features are available at runtime.
/// Use `detect()` to get the current capabilities.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct RufvectorCapabilities {
...
impl Default for RufvectorCapabilities {
    fn default() -> Self {
        Self::detect()
    }
}

impl RufvectorCapabilities {
    /// Detect available Rufvector capabilities
    ///
    ...
    ///
    /// ```rust,ignore
    /// use rufllm::capabilities::RufvectorCapabilities;
    ///
    ...
    fn probe_quantization_level() -> u8 {
        // rufvector-core always provides scalar quantization
    ...
    fn probe_performance() -> (usize, u64) {
        // These are estimated based on benchmarks from rufvector-core
        if SIMD_AVAILABLE && PARALLEL_AVAILABLE {
...
mod tests {
    ...
    fn test_capabilities_cached() {
        let caps1 = RufvectorCapabilities::detect();
        let caps2 = RufvectorCapabilities::cached();
    ...
    fn test_capabilities_summary() {
        ...
        assert!(summary.contains("Rufvector"));
