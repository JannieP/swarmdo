...
///
/// ```rust,ignore
/// use rufllm::bitnet::PtBitnetConfig;
///
...
///
/// ```rust,ignore
/// use rufllm::bitnet::absmean_ternary;
///
...
///
/// ```rust,ignore
/// use rufllm::bitnet::{quantize_tensor, PtBitnetConfig};
///
...
pub fn quantize_tensor(
    ...
    let num_blocks = total_elements.checked_add(block_size - 1).ok_or_else(|| {
        RufLLMError::Model("Integer overflow in block count calculation".to_string())
    })? / block_size;
