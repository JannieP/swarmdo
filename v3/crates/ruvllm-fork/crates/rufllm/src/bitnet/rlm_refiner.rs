...
impl RlmRefiner {
    ...
    pub fn new(config: RlmRefinerConfig, num_expert_layers: usize) -> Result<Self> {
        ...
        let contrastive = ContrastiveTrainer::new(contrastive_config)
            .map_err(|e| RufLLMError::Config(format!("ContrastiveTrainer init: {}", e)))?;
    ...
    pub fn refine_step(
        ...
        let lora = self.lora_adapters.get(&expert_idx).ok_or_else(|| {
            RufLLMError::InvalidOperation(format!("No LoRA adapter for expert {}", expert_idx))
        })?;
    ...
    pub fn repair_router<P: AsRef<Path>>(&mut self, triplet_path: P) -> Result<f64> {
        let count = self
            ...
            .map_err(|e| RufLLMError::Config(format!("Load triplets: {}", e)))?;
        ...
        let result = self
            ...
            .map_err(|e| RufLLMError::InvalidOperation(format!("Router repair failed: {}", e)))?;
