...
//! cargo run -p rufllm --example benchmark_model --release -- --model ./test_models/tinyllama.gguf
//!
//! # With custom parameters
//! cargo run -p rufllm --example benchmark_model --release -- \
//!     --model ./model.gguf \
//!     --warmup 5 \
//!     --iterations 20 \
//!     --max-tokens 100
...
fn main() {
    ...
    if !config.model_path.exists() {
        ...
        eprintln!("  cargo run -p rufllm --example download_test_model -- --model tinyllama");
        std::process::exit(1);
    }
    ...
    if !config.json_output {
        println!("RufLLM Model Benchmark");
        println!("======================");
        println!();
...
fn print_help() {
    ...
    println!();
    println!("USAGE:");
    println!("    cargo run -p rufllm --example benchmark_model --release -- [OPTIONS] <MODEL>");
    println!();
    println!("ARGUMENTS:");
    ...
    println!("    cargo run -p rufllm --example benchmark_model --release -- ./model.gguf");
    println!();
    println!("    # Custom configuration");
    println!("    cargo run -p rufllm --example benchmark_model --release -- \\");
...
fn run_real_benchmark(
    ...
    use rufllm::{CandleBackend, GenerateParams, LlmBackend, ModelConfig};
