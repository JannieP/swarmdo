...
//!
//! This utility downloads small, quantized models suitable for testing RufLLM.
...
//! cargo run -p rufllm --example download_test_model -- --model ruvltra-small
//!
//! # Download RuvLTRA Medium
//! cargo run -p rufllm --example download_test_model -- --model ruvltra-medium
//!
//! # Download TinyLlama (legacy)
//! cargo run -p rufllm --example download_test_model -- --model tinyllama
//!
//! # Download to custom directory
//! cargo run -p rufllm --example download_test_model -- --model ruvltra-small --output ./my_models
//!
//! # List available models
//! cargo run -p rufllm --example download_test_model -- --list
//! ```
//!
//! ## Available Models
//!
...
//! - `RUFLLM_MODELS_DIR`: Default output directory for models

use rufllm::hub::{default_cache_dir, DownloadConfig, ModelDownloader, RuvLtraRegistry};
use std::env;
...
fn main() {
    ...
    let output_dir = output_dir
        .or_else(|| env::var("RUFLLM_MODELS_DIR").ok().map(PathBuf::from))
    ...
    match download_model(model.url, &output_path, model.size_mb) {
        Ok(()) => {
            ...
            println!(
                "  TEST_MODEL_PATH={} cargo test -p rufllm --test real_model_test -- --ignored",
                output_path.display()
            );
        }
        Err(e) => {
...
fn print_help() {
    println!("RufLLM Test Model Downloader");
    println!();
    println!("USAGE:");
    println!("    cargo run -p rufllm --example download_test_model -- [OPTIONS] <MODEL>");
    println!();
    println!("ARGUMENTS:");
    ...
    println!("    RUFLLM_MODELS_DIR     Default output directory");
    println!();
    println!("EXAMPLES:");
    println!("    # Download TinyLlama (recommended for quick tests)");
    println!("    cargo run -p rufllm --example download_test_model -- tinyllama");
    println!();
    println!("    # Download to custom directory");
    println!("    cargo run -p rufllm --example download_test_model -- -m qwen-0.5b -o ./models");
}
...
fn download_ruvltra_model(
    model_info: &rufllm::hub::ModelInfo,
    output_dir: Option<PathBuf>,
    force: bool,
) {
    use rufllm::hub::DownloadConfig;
    ...
    match downloader.download(model_info, None) {
        Ok(path) => {
            ...
            println!("  cargo test -p rufllm --test real_model_test -- --ignored");
        }
        Err(e) => {
            eprintln!("\nDownload failed: {}", e);
            eprintln!("\nTroubleshooting:");
...
fn list_ruvltra_models() {
    use rufllm::hub::RuvLtraRegistry;
