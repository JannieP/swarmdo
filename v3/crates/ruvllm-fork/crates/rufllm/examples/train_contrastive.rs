...
//!   --triplets ~/.rufllm/training/ruvltra-finetuned/triplets.jsonl \
//!   --epochs 20 \
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut triplets_path =
        PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| ".".to_string()))
            .join(".rufllm/training/ruvltra-finetuned/triplets.jsonl");
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!();
                println!("Options:");
                println!("  -t, --triplets <PATH>  Path to triplets.jsonl (default: ~/.rufllm/training/ruvltra-finetuned/triplets.jsonl)");
    ...
    if !triplets_path.exists() {
        ...
        println!("  node npm/packages/rufllm/scripts/training/contrastive-finetune.js");
        println!();
...
/// Simplified trainer for example (uses the actual rufllm training module when available)
