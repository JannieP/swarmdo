...
//! Run CPU benchmarks:
//! ```bash
//! cargo bench -p rufllm --features candle --bench recurrent_depth_bench
//! ```
//!
//! Run GPU (CUDA) benchmarks:
//! ```bash
//! cargo bench -p rufllm --features candle,cuda --bench recurrent_depth_bench
//! ```
...
mod candle_bench {
    ...
    use rufllm::models::openmythos::{MythosCache, MythosConfig, OpenMythos};
    use rufllm::models::rdt::{RdtConfig, RdtModel};
    ...
    fn mythos_cfg() -> MythosConfig {
        MythosConfig {
            ...
            prelude_layers: 2,
            coda_layers: 2,
            attn_type: rufllm::models::openmythos::AttnType::Gqa,
            kv_lora_rank: 128,
            q_lora_rank: 256,
            qk_rope_head_dim: 32,
            qk_nope_head_dim: 64,
    ...
    pub fn bench_mythos_forward_mla_cpu(c: &mut Criterion) {
        ...
        cfg.attn_type = rufllm::models::openmythos::AttnType::Mla;
    ...
    pub fn bench_mythos_decode_sweep_cpu(c: &mut Criterion) {
        use rufllm::models::sampling::SamplingConfig;
    ...
    //
    // Run with:
    //   cargo bench -p rufllm --features candle,cuda,fused-act --bench recurrent_depth_bench
    // -----------------------------------------------------------------------
    ...
    pub fn bench_act_fused_vs_tensor_f32(c: &mut Criterion) {
        use rufllm::models::openmythos::act_kernel::FusedActKernel;
