//! RufLLM ESP32 — Tiny Agents on Heterogeneous SoCs
//!
//! Implements ADR-165: each ESP32 chip runs **one tiny-agent role** drawn from
//! the rufllm/rufvector primitive surface defined in `lib.rs`. This is the
...
const HELP_TEXT: &str = "rufllm-esp32 tiny-agent (ADR-165). commands:\n\
...
fn main() -> anyhow::Result<()> {
    ...
    let variant = match option_env!("RUFLLM_VARIANT") {
        Some(s) => parse_variant(s).unwrap_or(Esp32Variant::Esp32S3),
        None => Esp32Variant::Esp32S3,
    };
    let role = match option_env!("RUFLLM_ROLE") {
        Some(s) => Role::parse(s).unwrap_or_else(|| Role::default_for(variant)),
        None => Role::default_for(variant),
    };
    let chip_id = ChipId(option_env!("RUFLLM_CHIP_ID").and_then(|s| s.parse().ok()).unwrap_or(0));

    use std::io::Write as _;
    let mut err = std::io::stderr();
    let _ = writeln!(err);
    let _ = writeln!(err, "=== rufllm-esp32 tiny-agent (ADR-165) ===");
...
fn main() {
    ...
    // Allow `RUFLLM_VARIANT` / `RUFLLM_ROLE` to drive host smoke tests.
    let variant = std::env::var("RUFLLM_VARIANT")
        .ok()
        .and_then(|s| parse_variant(&s))
        .unwrap_or(Esp32Variant::Esp32S3);
    let role = std::env::var("RUFLLM_ROLE")
        .ok()
        .and_then(|s| Role::parse(&s))
        .unwrap_or_else(|| Role::default_for(variant));
    let chip_id = ChipId(std::env::var("RUFLLM_CHIP_ID")
        .ok().and_then(|s| s.parse().ok()).unwrap_or(0));

    let mut agent = TinyAgent::new(variant, role, chip_id);

    println!("=== rufllm-esp32 tiny-agent (ADR-165) — host-test ===");
...
pub fn wasm_init() -> String {
    "rufllm-esp32 tiny-agent (ADR-165) WASM shim".to_string()
}
...
fn main() {
    eprintln!("rufllm-esp32 tiny-agent (ADR-165)");
