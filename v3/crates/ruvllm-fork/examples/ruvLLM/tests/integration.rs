//! Integration tests for RufLLM
//!
//! Tests the complete pipeline from request to response.

use rufllm::types::{EdgeType, Feedback, MemoryEdge, MemoryNode, NodeType};
use rufllm::{Config, Request, RufLLM};
...
fn test_config() -> Config {
    ...
    let db_path = format!("/tmp/rufllm_test_{}.db", id);
    Config::builder()
        .db_path(&db_path)
        .embedding_dim(128)
...
async fn test_basic_query() {
    ...
    let llm = RufLLM::new(config).await.unwrap();
...
async fn test_concurrent_queries() {
    let config = test_config();
    let llm = std::sync::Arc::new(RufLLM::new(config).await.unwrap());
...
mod memory_integration {
    use super::*;
    use rufllm::config::MemoryConfig;
    use rufllm::memory::MemoryService;
...
mod router_integration {
    use super::*;
    use rufllm::config::RouterConfig;
    use rufllm::router::FastGRNNRouter;
    use rufllm::types::RouterSample;
...
mod attention_integration {
    use super::*;
    use rufllm::attention::GraphAttentionEngine;
    use rufllm::config::EmbeddingConfig;
    use rufllm::memory::SubGraph;
...
mod embedding_integration {
    use super::*;
    use rufllm::config::EmbeddingConfig;
    use rufllm::embedding::{EmbeddingService, PoolingStrategy};
...
mod compression_integration {
    use super::*;
    use rufllm::compression::CompressionService;
    use rufllm::config::MemoryConfig;
    use rufllm::memory::MemoryService;
