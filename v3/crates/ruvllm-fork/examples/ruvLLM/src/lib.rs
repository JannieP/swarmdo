//! # RufLLM - Self-Learning LLM
//!
//! A self-learning language model system integrating LFM2 with Rufvector.
//!
//! ## Architecture
//!
...
//! - **Rufvector** (Living memory): Adaptive synaptic mesh that learns continuously
...
//! - Every interaction writes to rufvector (Q&A, context, outcome)
...
//!         .build()?;
//!
//!     let llm = RufLLM::new(config).await?;
//!
...
//!
//! Version 2.0 integrates the `rufllm` crate for optimized inference:
//!
...
// =============================================================================
// Re-exports from rufllm for optimized kernels and backends
// =============================================================================

/// Optimized NEON/SIMD kernels from rufllm.
///
...
pub mod kernels {
    pub use rufllm_lib::kernels::*;
}

/// LLM inference backends (Candle, mistral-rs).
pub mod backends {
    pub use rufllm_lib::backends::*;
}

/// Two-tier KV cache with FP16 + quantized storage.
pub mod kv_cache {
    pub use rufllm_lib::kv_cache::*;
}

/// Memory pool and arena allocators for inference.
pub mod memory_pool {
    pub use rufllm_lib::memory_pool::*;
}

/// Speculative decoding for faster generation.
pub mod speculative {
    pub use rufllm_lib::speculative::*;
}

/// LoRA adapter management and composition.
pub mod lora {
    pub use rufllm_lib::lora::*;
}

// Re-export key types from rufllm at crate root
pub use rufllm_lib::{
    RufLLMConfig as IntegrationConfig,
    RufLLMEngine as IntegrationEngine,
