//! RufLLM HuggingFace Export Binary
//!
//! Export learned SONA patterns, LoRA weights, and preference pairs to HuggingFace.

use anyhow::Result;
use rufvector_sona::{HuggingFaceExporter, PretrainPipeline, SonaConfig, SonaEngine};
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("rufllm=info".parse().unwrap()),
        )
        .init();
...
fn print_usage() {
    println!(
        r#"
RufLLM HuggingFace Export Tool

USAGE:
    rufllm-export <COMMAND> [OPTIONS]
...
EXAMPLES:
    # Export LoRA weights
    rufllm-export safetensors ./exports/lora

    # Export all artifacts
    rufllm-export all ./exports

    # Push to HuggingFace Hub
    rufllm-export push username/my-sona-model

    # Generate pretraining script
    rufllm-export pretrain ./exports

ENVIRONMENT:
    HF_TOKEN                    HuggingFace API token (required for push)
    RUFLLM_DIM                  Hidden dimension (default: 256)
    RUFLLM_PATTERNS             Pattern clusters (default: 100)
"#
    );
}

fn create_demo_engine() -> SonaEngine {
    let dim = std::env::var("RUFLLM_DIM")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(256);

    let clusters = std::env::var("RUFLLM_PATTERNS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(100);
...
fn push_to_hub(args: &[String]) -> Result<()> {
    if args.is_empty() {
        error!("Usage: rufllm-export push <repo_id>");
        return Ok(());
    }
