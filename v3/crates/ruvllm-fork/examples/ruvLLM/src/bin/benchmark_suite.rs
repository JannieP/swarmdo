...
//!
//! Compares RufLLM against state-of-the-art systems and tracks
//! self-learning improvement over time.

use rufllm::{Config, Feedback, Result, RufLLM};
...
async fn benchmark_latency(llm: &RufLLM, config: &BenchmarkConfig) -> Result<BenchmarkMetrics> {
...
async fn benchmark_self_learning(config: &BenchmarkConfig) -> Result<Vec<LearningMetrics>> {
    ...
    // Create RufLLM with learning enabled
    ...
    let llm = RufLLM::new(llm_config).await?;
...
fn print_comparison_table(metrics: &BenchmarkMetrics, baselines: &SOTABaselines) {
    ...
    println!("╠════════════════════════════════════════════════════════════════════════════════╣");
    println!(
        "║ \x1b[32mRufLLM (This)          │ {:>8.2} │ {:>8.2} │ {:>8.2} │ {:>19.0}x\x1b[0m ║",
    ...
    println!("╠════════════════════════════════════════════════════════════════════════════════╣");
    println!(
        "║ \x1b[32mRufLLM (CPU Only)      │ {:>11.1} │ {:>38.0}x\x1b[0m ║",
...
fn print_capability_benchmarks() {
    ...
    println!("╠════════════════════════════════════════════════════════════════════════════════════════╣");
    println!("║ \x1b[33mRufLLM (Mock LFM2)   │    N/A*   │    N/A*   │  N/A* │  N/A* │    N/A    │ ~350M-2.6B\x1b[0m ║");
    println!("╠════════════════════════════════════════════════════════════════════════════════════════╣");
    println!("║ * RufLLM uses mock inference. Production deployment requires LFM2/llama.cpp backend.   ║");
...
/// Print RufLLM-specific advantages
fn print_rufllm_advantages() {
    println!("\n╔════════════════════════════════════════════════════════════════════════════════════════╗");
    println!("║                      RufLLM ARCHITECTURAL ADVANTAGES                                    ║");
    println!("╠════════════════════════════════════════════════════════════════════════════════════════╣");
    println!("║                                                                                        ║");
    println!("║  RufLLM is NOT a replacement for large foundation models - it's an AUGMENTATION LAYER  ║");
    ...
    println!("║  │    • RufLLM: Writes successful Q&A pairs to memory, improves over time          │   ║");
    println!("║  ├─────────────────────────────────────────────────────────────────────────────────┤   ║");
    ...
    println!("║  └─────────────────────────────────────────────────────────────────────────────────┘   ║");
    println!("║                                                                                        ║");
    println!("║  DEPLOYMENT: RufLLM wraps ANY LLM backend (llama.cpp, vLLM, OpenAI API, Ollama)        ║");
    println!(
...
fn print_feature_comparison() {
    ...
    println!("╠════════════════════════════════════════════════════════════════════════════════════════╣");
    println!(
        "║ Feature                    │ GPT-4o │ Claude │ Gemini │ RAG   │ vLLM │ RufLLM         ║"
    );
    println!("╠════════════════════════════════════════════════════════════════════════════════════════╣");
...
fn print_quality_comparison(avg_quality: f64, baselines: &SOTABaselines) {
    ...
    println!(
        "║ \x1b[32mRufLLM (after learning)         │ {:>12.1}% │ Adaptive + learning\x1b[0m    ║",
        avg_quality * 100.0
    );
    println!("╠═══════════════════════════════════════════════════════════════════════════╣");
    println!(
...
async fn main() -> Result<()> {
    println!("╔═══════════════════════════════════════════════════════════════════════════╗");
    println!("║           RufLLM Comprehensive Benchmark Suite v1.0                       ║");
    println!("║     Self-Learning LLM with LFM2 + Rufvector + FastGRNN                     ║");
    println!("╚═══════════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    let llm = std::sync::Arc::new(RufLLM::new(llm_config).await?);
    ...
    println!("║  RufLLM adds memory, routing, and learning ON TOP of any LLM.                  ║");
    println!("║                                                                                ║");
    println!("╚════════════════════════════════════════════════════════════════════════════════╝");
