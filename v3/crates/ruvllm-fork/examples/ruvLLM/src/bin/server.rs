//! RufLLM HTTP Server Binary
//!
//! REST API server for RufLLM inference.
...
async fn main() -> rufllm::Result<()> {
    ...
    println!("🚀 Initializing RufLLM...");
    let llm = RufLLM::new(config).await?;
    println!("✅ RufLLM initialized!");
...
fn main() {
    eprintln!("Error: rufllm-server requires the 'server' feature");
    eprintln!("Build with: cargo build --features server --bin rufllm-server");
    std::process::exit(1);
}
