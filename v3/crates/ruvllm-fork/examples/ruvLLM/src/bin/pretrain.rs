...
fn main() {
    println!("╔═══════════════════════════════════════════════════════════════════════════╗");
    println!("║           RufLLM Pretraining & Optimization Pipeline                       ║");
    ...
    let rufllm_results: Vec<_> = all_results
        .iter()
        .filter(|r| r.model_name.starts_with("RufLLM"))
        .collect();

    if let (Some(tiny), Some(medium)) = (rufllm_results.first(), rufllm_results.last()) {
        println!("║ RufLLM Scaling Analysis:                                                             ║");
...
fn create_baseline(
    ...
    memory_mb: f64,
    ppl: Option<f64>,
) -> rufllm::training::BenchmarkResults {
    rufllm::training::BenchmarkResults {
