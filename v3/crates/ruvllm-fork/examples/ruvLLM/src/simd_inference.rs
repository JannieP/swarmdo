...
//!
//! This module now integrates with `rufllm_lib::kernels` for optimized operations:
...
//!
//! ```rust,ignore
//! use rufllm::kernels::{flash_attention_neon, gemv_neon, gemm_neon};
//! use rufllm::simd_inference::SimdOps;
//!
...
// Import optimized kernels from rufllm when available on aarch64
#[cfg(target_arch = "aarch64")]
use rufllm_lib::kernels::{
...
impl SimdOps {
    // =========================================================================
    // Optimized operations using rufllm kernels (v2.0)
    // =========================================================================
    ...
    /// `rufllm_lib::kernels` on Apple Silicon, with automatic fallback
    ...
    pub fn attention(query: &[f32], key: &[f32], value: &[f32], scale: f32, causal: bool) -> Vec<f32> {
        #[cfg(target_arch = "aarch64")]
        {
            // Use optimized Flash Attention 2 from rufllm
    ...
    ///
    /// Uses the 12-row micro-kernel from `rufllm_lib` on aarch64.
    ...
    ///
    /// Uses vectorized sum-of-squares and normalization from `rufllm_lib`.
    #[inline]
        ...
        if self.is_demo_model {
            ...
            let response = format!(
                "[RufLLM Demo Mode]\n\
                 ...
                 3. Or use RufLLM with a trained checkpoint\n\n\
