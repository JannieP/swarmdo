...
/// Main RufLLM system orchestrator
pub struct RufLLM {
    /// Configuration
    config: Config,
    /// Embedding service
...
impl RufLLM {
    ...
    pub async fn new(config: Config) -> Result<Self> {
        tracing::info!("Initializing RufLLM v{}", crate::VERSION);
    ...
    pub async fn shutdown(&self) -> Result<()> {
        tracing::info!("Shutting down RufLLM");
        ...
        tracing::info!("RufLLM shutdown complete");
        Ok(())
    }
}
...
impl Metrics {
    fn new() -> Self {
        ...
        static REQUEST_COUNTER: Lazy<prometheus::IntCounter> = Lazy::new(|| {
            prometheus::register_int_counter!("rufllm_requests_total", "Total number of requests")
                .unwrap()
        });

        static LATENCY_HISTOGRAM: Lazy<prometheus::Histogram> = Lazy::new(|| {
            prometheus::register_histogram!(
                "rufllm_request_latency_seconds",
        ...
        static QUALITY_GAUGE: Lazy<prometheus::Gauge> = Lazy::new(|| {
            prometheus::register_gauge!("rufllm_quality_score", "Average quality score").unwrap()
        });
