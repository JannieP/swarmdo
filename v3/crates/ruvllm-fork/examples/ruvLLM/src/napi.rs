//! N-API bindings for RufLLM
//!
//! Provides Node.js bindings for the RufLLM self-learning LLM orchestrator.
//!
//! ## v2.0 Features
//!
...
//!
//! ```javascript
//! const { RufLLMEngine } = require('@rufvector/rufllm');
//!
//! // Create engine with parallel inference
//! const engine = new RufLLMEngine({
//!   useParallel: true,
//!   useMetal: false,
//!   quantization: 'q4k',
//! });
...
use rufllm_lib::kernels::is_neon_available;
use rufllm_lib::memory_pool::{MemoryManager, MemoryManagerConfig, MemoryManagerStats};

/// RufLLM Configuration for Node.js
#[napi(object)]
#[derive(Clone, Debug)]
pub struct JsRufLLMConfig {
...
impl Default for JsRufLLMConfig {
    fn default() -> Self {
        Self {
...
#[napi]
impl RufLLMEngine {
    /// Create a new RufLLM engine with default configuration
    #[napi(constructor)]
    pub fn new(config: Option<JsRufLLMConfig>) -> Result<Self> {
    ...
    #[napi]
    pub fn stats(&self) -> JsRufLLMStats {
