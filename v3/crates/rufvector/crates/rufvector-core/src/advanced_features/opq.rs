...
impl OPQConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.codebook_size > 256 {
            return Err(RufvectorError::InvalidParameter(format!(
                "Codebook size {} exceeds u8 max 256",
                self.codebook_size
            )));
        }
        if self.num_subspaces == 0 {
            return Err(RufvectorError::InvalidParameter(
...
impl OPQIndex {
    ...
    fn check_dim(&self, len: usize) -> Result<()> {
        if len != self.dimensions {
            Err(RufvectorError::DimensionMismatch {
    ...
    cbs.iter()
        ...
        .map(|(s, cb)| {
            ...
            cb.iter()
                ...
                .map(|(i, _)| i as u8)
                .ok_or_else(|| RufvectorError::Internal("Empty codebook".into()))
        })
        .collect()
}
