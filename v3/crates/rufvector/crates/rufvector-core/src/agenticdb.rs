...
use crate::error::{Result, RufvectorError};
use crate::types::*;
...
impl AgenticDB {
    ...
    /// ```rust,no_run
    /// use rufvector_core::{AgenticDB, ApiEmbedding};
    /// use rufvector_core::types::DbOptions;
    /// use std::sync::Arc;
    ///
    ...
    /// # {
    /// use rufvector_core::{AgenticDB, CandleEmbedding};
    /// use rufvector_core::types::DbOptions;
    /// use std::sync::Arc;
    ///
    /// let mut options = DbOptions::default();
    ...
    pub fn with_embedding_provider(
        ...
        if options.dimensions != embedding_provider.dimensions() {
            return Err(RufvectorError::InvalidDimension(format!(
    ...
    pub fn store_episode(
        ...
        let write_txn = self.db.begin_write()?;
        {
            ...
            let json = serde_json::to_vec(&episode)
                .map_err(|e| RufvectorError::SerializationError(e.to_string()))?;
    ...
    pub fn add_experience(
        ...
        let data = table
            .get(session_id)?
            .ok_or_else(|| RufvectorError::VectorNotFound(session_id.to_string()))?;
...
impl<'a> PolicyMemoryStore<'a> {
    ...
    /// other metadata.
    ///
    /// Returns [`RufvectorError::VectorNotFound`] if no policy with `policy_id`
    ...
    pub fn update_q_value(&self, policy_id: &str, new_q_value: f64) -> Result<()> {
        ...
        let mut entry = self
            ...
            .vector_db
            .get(&key)?
            .ok_or_else(|| crate::error::RufvectorError::VectorNotFound(key.clone()))?;
