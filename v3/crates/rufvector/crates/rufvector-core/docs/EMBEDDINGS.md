...
This guide explains how to use real text embeddings with AgenticDB in rufvector-core.
...
```rust
use rufvector_core::{AgenticDB, types::DbOptions};
...
```rust
use rufvector_core::{AgenticDB, ApiEmbedding, types::DbOptions};
use std::sync::Arc;
...
```rust
use rufvector_core::embeddings::EmbeddingProvider;
use rufvector_core::error::Result;
...
```toml
[dependencies]
rufvector-core = { version = "0.1", features = ["real-embeddings"] }
```
...
```rust
use rufvector_core::error::RufvectorError;

match AgenticDB::with_embedding_provider(options, provider) {
    Ok(db) => {
        // Use db
    }
    Err(RufvectorError::InvalidDimension(msg)) => {
        eprintln!("Dimension mismatch: {}", msg);
    }
    Err(RufvectorError::ModelLoadError(msg)) => {
        eprintln!("Failed to load model: {}", msg);
    }
    Err(RufvectorError::ModelInferenceError(msg)) => {
