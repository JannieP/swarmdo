...
//! (InfoNCE over the ogbn-arxiv citation graph) using `rufvector-gnn`'s own optimizer
//! and loss; the index is the shipping `rufvector-diskann` Vamana, driven through its
...
//!
//! Run: cargo run --release -p rufvector-gnn --example diskann_real_trajectory -- [N] [EPOCHS]

use ndarray::Array2;
use rand::{rngs::StdRng, Rng, SeedableRng};
use rufvector_diskann::distance::{l2_squared, FlatVectors};
use rufvector_diskann::{DriftingIndex, RebuildPolicy};
use rufvector_gnn::training::{info_nce_loss, Optimizer, OptimizerType};
