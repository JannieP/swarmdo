//! Node.js bindings for Rufvector GNN via NAPI-RS
//!
//! This module provides JavaScript bindings for the Rufvector GNN library,
...
use rufvector_gnn::{
    ...
    layer::RufvectorLayer as RustRufvectorLayer,
    search::{
...
impl RufvectorLayer {
    ...
    /// # Example
    /// ```javascript
    /// const layer = new RufvectorLayer(128, 256, 4, 0.1);
    /// ```
    #[napi(constructor)]
    pub fn new(input_dim: u32, hidden_dim: u32, heads: u32, dropout: f64) -> Result<Self> {
        let inner = RustRufvectorLayer::new(
            input_dim as usize,
            hidden_dim as usize,
            heads as usize,
            dropout as f32,
    ...
    pub fn from_json(json: String) -> Result<Self> {
        let inner: RustRufvectorLayer = serde_json::from_str(&json).map_err(|e| {
            Error::new(
                Status::GenericFailure,
...
/// const layer1 = new RufvectorLayer(2, 2, 1, 0.0);
...
pub fn hierarchical_forward(
    ...
    let gnn_layers: Vec<RustRufvectorLayer> = gnn_layers_json
        .iter()
        .map(|json| {
...
#[napi]
pub fn init() -> String {
    "Rufvector GNN Node.js bindings initialized".to_string()
}
