...
[![Crates.io](https://img.shields.io/crates/v/rufvector-attention.svg)](https://crates.io/crates/rufvector-attention)
[![Documentation](https://docs.rs/rufvector-attention/badge.svg)](https://docs.rs/rufvector-attention)
...
```bash
cargo add rufvector-attention
```

Attention is the core operation in transformers, vector search, and graph neural networks, but most libraries give you one or two flavors and call it done. `rufvector-attention` ships 46 mechanisms spanning standard dot-product, sparse (Flash, linear, local-global), geometric (hyperbolic, mixed-curvature), graph (GAT, RoPE), and mixture-of-experts -- all SIMD-accelerated with quantization support. Pick the right attention for your data shape instead of forcing everything through softmax(QK^T/sqrt(d))V.

| | rufvector-attention | PyTorch `nn.MultiheadAttention` | FlashAttention (standalone) | xFormers |
|---|---|---|---|---|
...
> Part of the [RufVector](https://github.com/ruvnet/rufvector) ecosystem -- the self-learning vector database with graph intelligence.
...
```rust
use rufvector_attention::{SlicedWassersteinAttention, SlicedWassersteinConfig};
...
```rust
use rufvector_attention::{FisherMetric, FisherConfig, NaturalGradient, NaturalGradientConfig};
...
```rust
use rufvector_attention::{InformationBottleneck, IBConfig, KLDivergence, DiagonalGaussian};
...
```toml
[dependencies]
rufvector-attention = { version = "0.1", features = ["simd", "wasm"] }
```
...
```rust
use rufvector_attention::sdk::presets::*;
...
- [API Documentation](https://docs.rs/rufvector-attention) - Full API reference
...
```bibtex
@software{rufvector_attention,
  title = {rufvector-attention: Advanced Attention Mechanisms for Vector Search},
  author = {rufvector contributors},
  year = {2025},
  url = {https://github.com/ruvnet/rufvector}
}
```

## Related Projects

- [rufvector](../rufvector) - Core vector search engine
- [rufvector-graph](../rufvector-graph) - Graph neural networks
- [rufvector-gnn](../rufvector-gnn) - Geometric neural networks
