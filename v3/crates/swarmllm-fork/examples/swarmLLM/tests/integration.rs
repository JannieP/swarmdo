//! Integration tests for SwarmLLM
//!
//! Tests the complete pipeline from request to response.

use swarmllm::types::{EdgeType, Feedback, MemoryEdge, MemoryNode, NodeType};
use swarmllm::{Config, Request, SwarmLLM};
...
fn test_config() -> Config {
    ...
    let db_path = format!("/tmp/swarmllm_test_{}.db", id);
    Config::builder()
        .db_path(&db_path)
        .embedding_dim(128)
...
async fn test_basic_query() {
    ...
    let llm = SwarmLLM::new(config).await.unwrap();
...
async fn test_concurrent_queries() {
    let config = test_config();
    let llm = std::sync::Arc::new(SwarmLLM::new(config).await.unwrap());
...
mod memory_integration {
    use super::*;
    use swarmllm::config::MemoryConfig;
    use swarmllm::memory::MemoryService;
...
mod router_integration {
    use super::*;
    use swarmllm::config::RouterConfig;
    use swarmllm::router::FastGRNNRouter;
    use swarmllm::types::RouterSample;
...
mod attention_integration {
    use super::*;
    use swarmllm::attention::GraphAttentionEngine;
    use swarmllm::config::EmbeddingConfig;
    use swarmllm::memory::SubGraph;
...
mod embedding_integration {
    use super::*;
    use swarmllm::config::EmbeddingConfig;
    use swarmllm::embedding::{EmbeddingService, PoolingStrategy};
...
mod compression_integration {
    use super::*;
    use swarmllm::compression::CompressionService;
    use swarmllm::config::MemoryConfig;
    use swarmllm::memory::MemoryService;
