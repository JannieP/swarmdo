//! SwarmLLM ESP32 — Tiny Agents on Heterogeneous SoCs
//!
//! Implements ADR-165: each ESP32 chip runs **one tiny-agent role** drawn from
//! the swarmllm/swarmvector primitive surface defined in `lib.rs`. This is the
...
const HELP_TEXT: &str = "swarmllm-esp32 tiny-agent (ADR-165). commands:\n\
...
fn main() -> anyhow::Result<()> {
    ...
    let variant = match option_env!("SWARMLLM_VARIANT") {
        Some(s) => parse_variant(s).unwrap_or(Esp32Variant::Esp32S3),
        None => Esp32Variant::Esp32S3,
    };
    let role = match option_env!("SWARMLLM_ROLE") {
        Some(s) => Role::parse(s).unwrap_or_else(|| Role::default_for(variant)),
        None => Role::default_for(variant),
    };
    let chip_id = ChipId(option_env!("SWARMLLM_CHIP_ID").and_then(|s| s.parse().ok()).unwrap_or(0));

    use std::io::Write as _;
    let mut err = std::io::stderr();
    let _ = writeln!(err);
    let _ = writeln!(err, "=== swarmllm-esp32 tiny-agent (ADR-165) ===");
...
fn main() {
    ...
    // Allow `SWARMLLM_VARIANT` / `SWARMLLM_ROLE` to drive host smoke tests.
    let variant = std::env::var("SWARMLLM_VARIANT")
        .ok()
        .and_then(|s| parse_variant(&s))
        .unwrap_or(Esp32Variant::Esp32S3);
    let role = std::env::var("SWARMLLM_ROLE")
        .ok()
        .and_then(|s| Role::parse(&s))
        .unwrap_or_else(|| Role::default_for(variant));
    let chip_id = ChipId(std::env::var("SWARMLLM_CHIP_ID")
        .ok().and_then(|s| s.parse().ok()).unwrap_or(0));

    let mut agent = TinyAgent::new(variant, role, chip_id);

    println!("=== swarmllm-esp32 tiny-agent (ADR-165) — host-test ===");
...
pub fn wasm_init() -> String {
    "swarmllm-esp32 tiny-agent (ADR-165) WASM shim".to_string()
}
...
fn main() {
    eprintln!("swarmllm-esp32 tiny-agent (ADR-165)");
