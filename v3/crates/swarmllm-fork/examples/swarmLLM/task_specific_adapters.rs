...
//! Run with:
//! ```bash
//! cargo run --example task_specific_adapters --features swarmllm
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let coder_output = coder_lora.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let security_output = security_lora.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let merged_output = merged.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let weighted_output = weighted_merged.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let slerp_output = slerp_merged.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    if let Some(active) = swap_manager.active() {
        let output = active.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let ties_output = ties_merged.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let baseline = coder_lora.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    let adapted = coder_lora.forward(&test_input, &swarmllm::lora::TargetModule::QProj);
    ...
    println!("\n  Loading adapter...");
    let loaded_lora = swarmllm::lora::MicroLoRA::load(save_path)?;
    println!("    ✓ Loaded");
