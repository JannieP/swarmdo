...
//!
//! This module now integrates with `swarmllm_lib::kernels` for optimized operations:
...
//!
//! ```rust,ignore
//! use swarmllm::kernels::{flash_attention_neon, gemv_neon, gemm_neon};
//! use swarmllm::simd_inference::SimdOps;
//!
...
// Import optimized kernels from swarmllm when available on aarch64
#[cfg(target_arch = "aarch64")]
use swarmllm_lib::kernels::{
...
impl SimdOps {
    // =========================================================================
    // Optimized operations using swarmllm kernels (v2.0)
    // =========================================================================
    ...
    /// `swarmllm_lib::kernels` on Apple Silicon, with automatic fallback
    ...
    pub fn attention(query: &[f32], key: &[f32], value: &[f32], scale: f32, causal: bool) -> Vec<f32> {
        #[cfg(target_arch = "aarch64")]
        {
            // Use optimized Flash Attention 2 from swarmllm
    ...
    ///
    /// Uses the 12-row micro-kernel from `swarmllm_lib` on aarch64.
    ...
    ///
    /// Uses vectorized sum-of-squares and normalization from `swarmllm_lib`.
    #[inline]
        ...
        if self.is_demo_model {
            ...
            let response = format!(
                "[SwarmLLM Demo Mode]\n\
                 ...
                 3. Or use SwarmLLM with a trained checkpoint\n\n\
