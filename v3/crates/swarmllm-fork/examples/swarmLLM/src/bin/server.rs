//! SwarmLLM HTTP Server Binary
//!
//! REST API server for SwarmLLM inference.
...
async fn main() -> swarmllm::Result<()> {
    ...
    println!("🚀 Initializing SwarmLLM...");
    let llm = SwarmLLM::new(config).await?;
    println!("✅ SwarmLLM initialized!");
...
fn main() {
    eprintln!("Error: swarmllm-server requires the 'server' feature");
    eprintln!("Build with: cargo build --features server --bin swarmllm-server");
    std::process::exit(1);
}
