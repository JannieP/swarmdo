...
fn main() {
    println!("╔═══════════════════════════════════════════════════════════════════════════╗");
    println!("║           SwarmLLM Pretraining & Optimization Pipeline                       ║");
    ...
    let swarmllm_results: Vec<_> = all_results
        .iter()
        .filter(|r| r.model_name.starts_with("SwarmLLM"))
        .collect();

    if let (Some(tiny), Some(medium)) = (swarmllm_results.first(), swarmllm_results.last()) {
        println!("║ SwarmLLM Scaling Analysis:                                                             ║");
...
fn create_baseline(
    ...
    memory_mb: f64,
    ppl: Option<f64>,
) -> swarmllm::training::BenchmarkResults {
    swarmllm::training::BenchmarkResults {
