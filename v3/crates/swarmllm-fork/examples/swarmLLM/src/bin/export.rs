//! SwarmLLM HuggingFace Export Binary
//!
//! Export learned SONA patterns, LoRA weights, and preference pairs to HuggingFace.

use anyhow::Result;
use swarmvector_sona::{HuggingFaceExporter, PretrainPipeline, SonaConfig, SonaEngine};
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("swarmllm=info".parse().unwrap()),
        )
        .init();
...
fn print_usage() {
    println!(
        r#"
SwarmLLM HuggingFace Export Tool

USAGE:
    swarmllm-export <COMMAND> [OPTIONS]
...
EXAMPLES:
    ...
    swarmllm-export push username/my-sona-model
...
ENVIRONMENT:
    ...
    SWARMLLM_DIM                  Hidden dimension (default: 256)
    SWARMLLM_PATTERNS             Pattern clusters (default: 100)
"#
    );
}

fn create_demo_engine() -> SonaEngine {
    let dim = std::env::var("SWARMLLM_DIM")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(256);

    let clusters = std::env::var("SWARMLLM_PATTERNS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(100);
...
fn push_to_hub(args: &[String]) -> Result<()> {
    if args.is_empty() {
        error!("Usage: swarmllm-export push <repo_id>");
        return Ok(());
    }
