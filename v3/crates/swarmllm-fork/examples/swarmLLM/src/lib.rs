...
//! - **Swarmvector** (Living memory): Adaptive synaptic mesh that learns continuously
...
//! - Every interaction writes to swarmvector (Q&A, context, outcome)
...
//!
//! Version 2.0 integrates the `swarmllm` crate for optimized inference:
//!
...
// =============================================================================
// Re-exports from swarmllm for optimized kernels and backends
// =============================================================================

/// Optimized NEON/SIMD kernels from swarmllm.
///
...
pub mod kernels {
    pub use swarmllm_lib::kernels::*;
}

/// LLM inference backends (Candle, mistral-rs).
pub mod backends {
    pub use swarmllm_lib::backends::*;
}

/// Two-tier KV cache with FP16 + quantized storage.
pub mod kv_cache {
    pub use swarmllm_lib::kv_cache::*;
}

/// Memory pool and arena allocators for inference.
pub mod memory_pool {
    pub use swarmllm_lib::memory_pool::*;
}

/// Speculative decoding for faster generation.
pub mod speculative {
    pub use swarmllm_lib::speculative::*;
}

/// LoRA adapter management and composition.
pub mod lora {
    pub use swarmllm_lib::lora::*;
}

// Re-export key types from swarmllm at crate root
pub use swarmllm_lib::{
    SwarmLLMConfig as IntegrationConfig,
    SwarmLLMEngine as IntegrationEngine,
