//! Main orchestrator for SwarmLLM
//!
...
/// Main SwarmLLM system orchestrator
pub struct SwarmLLM {
    /// Configuration
    config: Config,
    /// Embedding service
...
impl SwarmLLM {
    ...
    pub async fn new(config: Config) -> Result<Self> {
        tracing::info!("Initializing SwarmLLM v{}", crate::VERSION);
    ...
    pub async fn shutdown(&self) -> Result<()> {
        tracing::info!("Shutting down SwarmLLM");
        ...
        tracing::info!("SwarmLLM shutdown complete");
        Ok(())
    }
}
...
impl Metrics {
    fn new() -> Self {
        ...
        static REQUEST_COUNTER: Lazy<prometheus::IntCounter> = Lazy::new(|| {
            prometheus::register_int_counter!("swarmllm_requests_total", "Total number of requests")
                .unwrap()
        });

        static LATENCY_HISTOGRAM: Lazy<prometheus::Histogram> = Lazy::new(|| {
            prometheus::register_histogram!(
                "swarmllm_request_latency_seconds",
        ...
        static QUALITY_GAUGE: Lazy<prometheus::Gauge> = Lazy::new(|| {
            prometheus::register_gauge!("swarmllm_quality_score", "Average quality score").unwrap()
        });
