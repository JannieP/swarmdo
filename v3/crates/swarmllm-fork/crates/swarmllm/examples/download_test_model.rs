...
fn main() {
    ...
    let output_dir = output_dir
        .or_else(|| env::var("SWARMLLM_MODELS_DIR").ok().map(PathBuf::from))
    ...
    match download_model(model.url, &output_path, model.size_mb) {
        Ok(()) => {
            ...
            println!(
                "  TEST_MODEL_PATH={} cargo test -p swarmllm --test real_model_test -- --ignored",
                output_path.display()
            );
        }
        Err(e) => {
...
fn print_help() {
    println!("SwarmLLM Test Model Downloader");
    println!();
    println!("USAGE:");
    println!("    cargo run -p swarmllm --example download_test_model -- [OPTIONS] <MODEL>");
    println!();
    println!("ARGUMENTS:");
    ...
    println!("    SWARMLLM_MODELS_DIR     Default output directory");
    println!();
    println!("EXAMPLES:");
    println!("    # Download TinyLlama (recommended for quick tests)");
    println!("    cargo run -p swarmllm --example download_test_model -- tinyllama");
    println!();
    println!("    # Download to custom directory");
    println!("    cargo run -p swarmllm --example download_test_model -- -m qwen-0.5b -o ./models");
}
...
fn download_ruvltra_model(
    ...
    match downloader.download(model_info, None) {
        Ok(path) => {
            ...
            println!("  cargo test -p swarmllm --test real_model_test -- --ignored");
        }
        Err(e) => {
            eprintln!("\nDownload failed: {}", e);
            eprintln!("\nTroubleshooting:");
