...
impl LlmBackend for MockBackend {
    ...
    fn generate(&self, prompt: &str, _params: GenerateParams) -> Result<String> {
        if !self.loaded {
            return Err(swarmllm::SwarmLLMError::Backend("Model not loaded".to_string()));
        }
    ...
    ) -> Result<Box<dyn Iterator<Item = Result<swarmllm::backends::GeneratedToken>> + Send + '_>>
    {
        if !self.loaded {
            return Err(swarmllm::SwarmLLMError::Backend("Model not loaded".to_string()));
        }
        ...
        let tokens = vec![
            swarmllm::backends::GeneratedToken {
                id: 1,
