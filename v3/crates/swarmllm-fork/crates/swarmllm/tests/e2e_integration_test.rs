...
//! End-to-end Integration Tests for SwarmLLM
//!
...
//! ```bash
//! cargo test -p swarmllm --test e2e_integration_test
//! ```
//!
//! ### With a real model file:
//! ```bash
//! TEST_MODEL_PATH=/path/to/model.gguf cargo test -p swarmllm --test e2e_integration_test -- --ignored
//! ```
//!
//! ### Run specific test with model:
//! ```bash
//! TEST_MODEL_PATH=/path/to/model.gguf cargo test -p swarmllm --test e2e_integration_test test_real_model_generation -- --ignored
//! ```
...
impl LlmBackend for MockLlmBackend {
    ...
    fn generate(&self, prompt: &str, params: GenerateParams) -> Result<String> {
        if !self.model_loaded.load(Ordering::SeqCst) {
            return Err(SwarmLLMError::Config("Model not loaded".to_string()));
        }
    ...
    fn model_info(&self) -> Option<swarmllm::backends::ModelInfo> {
        if self.is_model_loaded() {
            Some(swarmllm::backends::ModelInfo {
