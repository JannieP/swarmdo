...
/// Create a test SwarmLLM configuration
fn create_test_config(storage_path: &str) -> SwarmLLMConfig {
    SwarmLLMConfig {
...
async fn test_full_inference_pipeline() {
    ...
    // let engine = SwarmLLMEngine::new(config).unwrap();
...
fn test_engine_creation() {
    ...
    let result = SwarmLLMEngine::new(config);
...
async fn test_concurrent_requests() {
    let temp_dir = create_test_dir();
    let config = create_test_config(temp_dir.path().to_str().unwrap());
    let engine = Arc::new(SwarmLLMEngine::new(config).unwrap());
