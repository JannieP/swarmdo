...
//! TEST_MODEL_PATH=./test_models/tinyllama.gguf cargo test -p swarmllm --test real_model_test -- --ignored
//!
//! # Run with default test_models directory
//! cargo test -p swarmllm --test real_model_test -- --ignored
//! ```
//!
...
pub fn skip_if_no_model(patterns: &[&str], model_name: &str) -> Option<PathBuf> {
    match find_test_model(patterns) {
        ...
        None => {
            ...
            println!(
                "     cargo run -p swarmllm --example download_test_model -- --model {}",
...
fn test_tinyllama_load() {
    ...
    // In a real implementation, you would use the SwarmLLM API
