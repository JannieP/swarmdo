...
//! to feed quality signals into SwarmLLM's learning loops (SONA, embedding
...
//!
//! ```rust,ignore
//! use swarmllm::intelligence::{IntelligenceLoader, FileSignalProvider};
...
/// Trait for external systems that supply quality signals to SwarmLLM.
///
...
/// ```rust,ignore
/// use swarmllm::intelligence::FileSignalProvider;
/// use std::path::PathBuf;
///
...
/// Custom provider:
/// ```rust,ignore
/// use swarmllm::intelligence::{IntelligenceProvider, QualitySignal, ProviderQualityWeights};
/// use swarmllm::error::Result;
///
...
impl IntelligenceProvider for FileSignalProvider {
    ...
    fn load_signals(&self) -> Result<Vec<QualitySignal>> {
        ...
        if metadata.len() > MAX_SIGNAL_FILE_SIZE {
            return Err(crate::error::SwarmLLMError::InvalidOperation(format!(
        ...
        let signals: Vec<QualitySignal> = serde_json::from_reader(reader).map_err(|e| {
            crate::error::SwarmLLMError::Serialization(format!(
...
mod tests {
    ...
    impl IntelligenceProvider for FailingProvider {
        ...
        fn load_signals(&self) -> Result<Vec<QualitySignal>> {
            Err(crate::error::SwarmLLMError::Serialization(
