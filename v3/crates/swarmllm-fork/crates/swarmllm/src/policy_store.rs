//! Policy Memory Store
//!
//! Stores learned policies and thresholds in Swarmvector for semantic search
...
use swarmvector_core::{AgenticDB, SearchQuery, VectorEntry};
...
/// Policy entry stored in Swarmvector
...
impl PolicyStore {
    ...
    pub fn new(storage_path: &str, embedding_dim: usize) -> Result<Self> {
        ...
        let db = AgenticDB::new(options).map_err(|e| SwarmLLMError::Storage(e.to_string()))?;
    ...
    pub fn store(&self, entry: PolicyEntry) -> Result<Uuid> {
        ...
        // Store in Swarmvector
        self.db
            .insert(vector_entry)
            .map_err(|e| SwarmLLMError::Storage(e.to_string()))?;
