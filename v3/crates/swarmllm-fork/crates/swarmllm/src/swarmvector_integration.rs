//! Swarmvector Integration Layer
//!
//! This module provides a unified integration layer for all Swarmvector capabilities:
//!
//! - **swarmvector-core**: HNSW index, vector storage, similarity search
//! - **swarmvector-attention**: Flash Attention for efficient inference
//! - **swarmvector-graph**: Knowledge graph for relationship learning
//! - **swarmvector-gnn**: Graph neural networks for complex reasoning
//! - **swarmvector-sona**: SONA (Self-Optimizing Neural Architecture) learning
//!
//! ## Architecture
//!
//! ```text
...
//!
//! ```rust,ignore
//! use swarmllm::swarmvector_integration::{
//!     SwarmvectorIntegration, IntegrationConfig, UnifiedIndex
//! };
//!
//! // Detect capabilities and create integration
//! let config = IntegrationConfig::default();
//! let integration = SwarmvectorIntegration::new(config)?;
//!
...
use crate::capabilities::{
    SwarmvectorCapabilities, ATTENTION_AVAILABLE, GNN_AVAILABLE, GRAPH_AVAILABLE, HNSW_AVAILABLE,
    SONA_AVAILABLE,
};
...
use swarmvector_core::index::hnsw::HnswIndex;
use swarmvector_core::index::VectorIndex;
use swarmvector_core::types::{DistanceMetric, HnswConfig, VectorId};
use swarmvector_sona::{LearnedPattern, PatternConfig, ReasoningBank};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::Arc;

/// Configuration for the Swarmvector integration layer
...
impl Default for IntegrationConfig {
    fn default() -> Self {
        let caps = SwarmvectorCapabilities::detect();
...
impl UnifiedIndex {
    ...
    pub fn new(config: IntegrationConfig) -> Result<Self> {
        ...
        .map_err(|e| SwarmLLMError::Swarmvector(e.to_string()))?;
...
impl IntelligenceLayer {
    ...
    pub fn learn_from_outcome(
        ...
        let trajectory = Trajectory {
            ...
            session_id: "swarmvector-integration".to_string(),
...
impl SwarmvectorIntegration {
    /// Create a new Swarmvector integration
    ///
    ...
    ///
    /// ```rust,ignore
    /// use swarmllm::swarmvector_integration::{SwarmvectorIntegration, IntegrationConfig};
    ///
    /// let config = IntegrationConfig::default();
    /// let integration = SwarmvectorIntegration::new(config)?;
    ///
    ...
    pub fn capabilities(&self) -> &SwarmvectorCapabilities {
        &self.capabilities
    }
    ...
    pub fn compute_attention(&self, query: &[f32], keys: &[&[f32]], values: &[&[f32]]) -> Vec<f32> {
        use swarmvector_attention::{traits::Attention, ScaledDotProductAttention};
...
pub struct IntegrationStats {
    /// Detected capabilities
    pub capabilities: SwarmvectorCapabilities,
...
mod tests {
    ...
    #[test]
    fn test_swarmvector_integration() {
        let config = IntegrationConfig {
            embedding_dim: 128,
            ..Default::default()
        };
        let integration = SwarmvectorIntegration::new(config).unwrap();
