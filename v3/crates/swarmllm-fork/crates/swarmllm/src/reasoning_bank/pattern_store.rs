...
//!
//! High-performance pattern storage using swarmvector-core's HNSW index
//! for fast similarity search (150x faster than brute force).

use crate::error::{Result, SwarmLLMError};
use chrono::{DateTime, Utc};
use parking_lot::RwLock;
use swarmvector_core::types::{DbOptions, HnswConfig};
use swarmvector_core::{DistanceMetric, SearchQuery, VectorDB, VectorEntry};
...
impl PatternStore {
    ...
    pub fn new(config: PatternStoreConfig) -> Result<Self> {
        ...
        let index = VectorDB::new(db_options)
            .map_err(|e| SwarmLLMError::Storage(format!("Failed to create HNSW index: {}", e)))?;
    ...
    pub fn store_pattern(&mut self, pattern: Pattern) -> Result<u64> {
        ...
        // Insert into HNSW index
        {
            ...
            index
                .insert(entry)
                .map_err(|e| SwarmLLMError::Storage(format!("Failed to insert into index: {}", e)))?;
        }
    ...
    pub fn search_similar(&self, query: &[f32], limit: usize) -> Result<Vec<PatternSearchResult>> {
        ...
        let results = {
            ...
            index
                .search(search_query)
                .map_err(|e| SwarmLLMError::Storage(format!("Search failed: {}", e)))?
        };
