...
//! | (SwarmLLM adapter)  |     | (PagedAttention)  |
//! +-------------------+     +-------------------+
//!         |                         |
//!         v                         v
//! +-------------------+     +-------------------+
...
//!
//! ```rust,ignore
//! use swarmllm::backends::{MistralBackend, MistralConfig};
//!
...
impl MistralBackend {
    ...
    pub fn load_xlora_adapter(&self, name: &str, path: &Path) -> Result<()> {
        let manager = self
            .xlora_manager
            .as_ref()
            .ok_or_else(|| SwarmLLMError::Config("X-LoRA not configured".to_string()))?;
    ...
    pub fn apply_isq(&mut self) -> Result<()> {
        if !self.is_model_loaded() {
            return Err(SwarmLLMError::InvalidOperation(
        ...
        let _isq_config = self
            ...
            .isq
            .as_ref()
            .ok_or_else(|| SwarmLLMError::Config("ISQ not configured".to_string()))?;
    ...
    fn generate_with_mistral_rs(
        ...
        model
            .get_sender()
            .map_err(|e| SwarmLLMError::Compute(format!("Failed to get model sender: {}", e)))?
            .blocking_send(request)
            .map_err(|e| SwarmLLMError::Compute(format!("Failed to send request to model: {}", e)))?;

        // Wait for response
        let response = rx
            .recv()
            .map_err(|e| SwarmLLMError::Compute(format!("Failed to receive response: {}", e)))?;
        ...
        match response {
            ...
            Response::InternalError(e) => {
                Err(SwarmLLMError::Compute(format!("Model internal error: {}", e)))
            }
            Response::ValidationError(e) => {
                Err(SwarmLLMError::Config(format!("Validation error: {}", e)))
            }
            Response::ModelError(msg, _) => {
                Err(SwarmLLMError::Compute(format!("Model error: {}", msg)))
            }
            _ => Err(SwarmLLMError::Compute("Unexpected response type".to_string())),
        }
    }
