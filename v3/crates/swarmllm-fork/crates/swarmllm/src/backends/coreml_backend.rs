...
//!
//! ```rust,ignore
//! use swarmllm::backends::CoreMLBackend;
//!
    ...
    impl CoreMLModelHandle {
        ...
        pub fn load(path: &Path, compute_units: ComputeUnits) -> Result<Self> {
            ...
            let url = NSURL::from_file_path(path).ok_or_else(|| {
                SwarmLLMError::CoreML(format!("Invalid model path: {}", path.display()))
            })?;
        ...
        pub fn predict(&self, input_name: &str, token_ids: &[i32]) -> Result<Vec<f32>> {
            ...
            unsafe {
                ...
                let output_name = self
                    .output_names
                    .first()
                    .ok_or_else(|| SwarmLLMError::CoreML("No output features found".to_string()))?;
                ...
                let output_value = MLFeatureProvider::featureValueForName(&*output, &output_key)
                    .ok_or_else(|| {
                        SwarmLLMError::CoreML(format!("Output feature '{}' not found", output_name))
                    })?;

                // Get the multi-array from the output
                let output_array = output_value.multiArrayValue().ok_or_else(|| {
                    SwarmLLMError::CoreML("Output is not a multi-array".to_string())
                })?;
        ...
        pub fn get_embeddings(
            ...
            unsafe {
                ...
                let output_name = embedding_name.ok_or_else(|| {
                    SwarmLLMError::CoreML("No embedding output found in model".to_string())
                })?;
                ...
                let output_value = MLFeatureProvider::featureValueForName(&*output, &output_key)
                    .ok_or_else(|| {
                        SwarmLLMError::CoreML(format!("Embedding output '{}' not found", output_name))
                    })?;

                let output_array = output_value.multiArrayValue().ok_or_else(|| {
                    SwarmLLMError::CoreML("Embedding output is not a multi-array".to_string())
                })?;
...
impl<'a> CoreMLStreamIterator<'a> {
    ...
    fn sample_token(&self, logits: &[f32]) -> Result<u32> {
        ...
        if logits.is_empty() {
            return Err(SwarmLLMError::Generation("Empty logits".to_string()));
        }
...
impl CoreMLBackend {
    ...
    pub fn new() -> Result<Self> {
        ...
        if !caps.available {
            return Err(SwarmLLMError::Config(
        ...
        std::fs::create_dir_all(&cache_dir).map_err(|e| {
            SwarmLLMError::Storage(format!("Failed to create Core ML cache directory: {}", e))
        })?;
    ...
    fn convert_to_coreml(&self, _gguf_path: &Path, _output_path: &Path) -> Result<()> {
        ...
        Err(SwarmLLMError::NotImplemented(
    ...
    fn validate_coreml_path(path: &Path) -> Result<()> {
        ...
        match extension {
            Some("mlmodelc") => {
                ...
                if !path.is_dir() {
                    return Err(SwarmLLMError::CoreML(
                ...
                if !model_mil.exists() && !coreml_data.exists() && !weights.exists() {
                    return Err(SwarmLLMError::CoreML(format!(
            ...
            Some("mlmodel") => {
                ...
                if !path.is_file() {
                    return Err(SwarmLLMError::CoreML(".mlmodel should be a file".to_string()));
                }
            }
            Some("mlpackage") => {
...
impl LlmBackend for CoreMLBackend {
    ...
    fn generate(&self, prompt: &str, params: GenerateParams) -> Result<String> {
        if !self.loaded {
            return Err(SwarmLLMError::InvalidOperation("No model loaded".to_string()));
        }

        #[cfg(all(target_os = "macos", target_arch = "aarch64", feature = "candle"))]
        {
            let model_handle = self.model_handle.as_ref().ok_or_else(|| {
                SwarmLLMError::InvalidOperation("Model handle not initialized".to_string())
            })?;

            let tokenizer = self.tokenizer.as_ref().ok_or_else(|| {
                SwarmLLMError::Config(
