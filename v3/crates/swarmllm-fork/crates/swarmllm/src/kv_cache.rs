...
impl TwoTierKvCache {
    ...
    pub fn append(&self, keys: &[f32], values: &[f32]) -> Result<()> {
        ...
        if keys.len() != values.len() {
            return Err(SwarmLLMError::KvCache(
    ...
    pub fn append(&self, keys: &[f32], values: &[f32]) -> Result<()> {
        ...
        while !remaining_keys.is_empty() {
            ...
            if need_new_block {
                ...
                .ok_or_else(|| {
                    SwarmLLMError::OutOfMemory("Failed to allocate KV block from pool".to_string())
                })?;
...
///
/// ```rust,ignore
/// use swarmllm::kv_cache::{TurboQuantKvCache, TurboQuantKvCacheConfig};
///
