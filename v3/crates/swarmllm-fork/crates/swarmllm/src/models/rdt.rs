...
impl RdtConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_loops == 0 {
            return Err(SwarmLLMError::Config("RDT: max_loops must be >= 1".into()));
        }
...
impl From<RdtCompatibilityError> for SwarmLLMError {
    fn from(e: RdtCompatibilityError) -> Self {
        SwarmLLMError::Model(e.to_string())
    }
}
...
mod candle_impl {
    ...
    impl RdtModel {
        ...
        pub fn generate(
            ...
            if prompt_ids.is_empty() {
                return Err(SwarmLLMError::Generation("empty prompt".into()));
            }
    ...
    fn cand(e: candle_core::Error) -> SwarmLLMError {
        SwarmLLMError::Model(format!("candle (rdt): {e}"))
    }
}
