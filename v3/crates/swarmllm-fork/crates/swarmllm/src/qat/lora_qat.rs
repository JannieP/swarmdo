...
//!
//! ```rust,ignore
//! use swarmllm::qat::{LoraQatConfig, LoraQatLayer, LoraQatTrainer};
//!
...
impl LoraQatConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.rank == 0 {
            return Err(SwarmLLMError::Config("LoRA rank must be > 0".to_string()));
        }
        if self.alpha <= 0.0 {
            return Err(SwarmLLMError::Config("LoRA alpha must be > 0".to_string()));
        }
