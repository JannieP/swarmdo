...
//!
//! ```rust,ignore
//! use swarmllm::qat::{CalibrationEngine, CalibrationConfig};
//!
...
impl CalibrationResult {
    ...
    pub fn save(&self, path: &str) -> Result<()> {
        let json = serde_json::to_string_pretty(self)
            .map_err(|e| SwarmLLMError::Model(format!("Serialization failed: {}", e)))?;
        std::fs::write(path, json)
            .map_err(|e| SwarmLLMError::Model(format!("Write failed: {}", e)))?;
        Ok(())
    }
    ...
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)
            .map_err(|e| SwarmLLMError::Model(format!("Read failed: {}", e)))?;
        let result = serde_json::from_str(&json)
            .map_err(|e| SwarmLLMError::Model(format!("Deserialization failed: {}", e)))?;
        Ok(result)
    }
...
impl CalibrationEngine {
    ...
    pub fn finalize(&mut self) -> Result<CalibrationResult> {
        if self.layer_stats.is_empty() {
            return Err(SwarmLLMError::Model(
