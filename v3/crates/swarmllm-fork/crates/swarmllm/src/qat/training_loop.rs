...
//!
//! ```rust,ignore
//! use swarmllm::qat::{QatTrainer, QatConfig, TrainingCallback};
//!
...
impl TrainingCheckpoint {
    ...
    pub fn save(&self, path: &str) -> Result<()> {
        let json = serde_json::to_string_pretty(self)
            .map_err(|e| SwarmLLMError::Model(format!("Checkpoint serialization failed: {}", e)))?;
        std::fs::write(path, json)
            .map_err(|e| SwarmLLMError::Model(format!("Checkpoint write failed: {}", e)))?;
        Ok(())
    }
    ...
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)
            .map_err(|e| SwarmLLMError::Model(format!("Checkpoint read failed: {}", e)))?;
        let checkpoint = serde_json::from_str(&json)
            .map_err(|e| SwarmLLMError::Model(format!("Checkpoint deserialization failed: {}", e)))?;
        Ok(checkpoint)
    }
}
...
impl TrainingResult {
    ...
    pub fn export(&self, path: &str) -> Result<()> {
        let json = serde_json::to_string_pretty(self)
            .map_err(|e| SwarmLLMError::Model(format!("Export serialization failed: {}", e)))?;
        std::fs::write(path, json)
            .map_err(|e| SwarmLLMError::Model(format!("Export write failed: {}", e)))?;
        Ok(())
    }
    ...
    pub fn load(path: &str) -> Result<Self> {
        let json = std::fs::read_to_string(path)
            .map_err(|e| SwarmLLMError::Model(format!("Load read failed: {}", e)))?;
        let result = serde_json::from_str(&json)
            .map_err(|e| SwarmLLMError::Model(format!("Load deserialization failed: {}", e)))?;
        Ok(result)
    }
