...
impl ServingEngine {
    ...
    fn generate_with_speculation(
        ...
        let tokenizer = self
            .model
            .tokenizer()
            .ok_or_else(|| SwarmLLMError::InvalidOperation("No tokenizer available".to_string()))?;
...
impl ServingEngine {
    ...
    pub async fn submit_async(&self, request: InferenceRequest) -> Result<GenerationResult> {
        ...
        // Wait for completion
        rx.await
            .map_err(|_| SwarmLLMError::Generation("Request cancelled".to_string()))
    }
