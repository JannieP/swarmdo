//! GGUF Model Format Loader for SwarmLLM
//!
...
//!
//! ```rust,ignore
//! use swarmllm::gguf::{GgufFile, GgufModelLoader};
//! use std::path::Path;
//!
//! // Load a GGUF file
...
//! ## Backend Integration
//!
//! The GGUF loader integrates seamlessly with SwarmLLM backends:
//!
//! ```rust,ignore
//! use swarmllm::backends::LlmBackend;
//! use std::path::Path;
//!
...
impl GgufFile {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        let file = File::open(path)
            .map_err(|e| SwarmLLMError::Model(format!("Failed to open GGUF file: {}", e)))?;
        ...
        let current_pos = reader
            .stream_position()
            .map_err(|e| SwarmLLMError::Model(format!("Failed to get stream position: {}", e)))?;
    ...
    pub fn open_mmap(path: &Path) -> Result<Self> {
        ...
        let file = File::open(path)
            .map_err(|e| SwarmLLMError::Model(format!("Failed to open file for mmap: {}", e)))?;

        let mmap = unsafe {
            memmap2::Mmap::map(&file)
                .map_err(|e| SwarmLLMError::Model(format!("Failed to memory map file: {}", e)))?
        };
    ...
    pub fn open_mmap(path: &Path) -> Result<Self> {
        ...
        let data = std::fs::read(path)
            .map_err(|e| SwarmLLMError::Model(format!("Failed to read file: {}", e)))?;
    ...
    fn read_tensor_bytes(&self, info: &TensorInfo) -> Result<Vec<u8>> {
        ...
        let mut file = File::open(&self.path)
            .map_err(|e| SwarmLLMError::Model(format!("Failed to open file: {}", e)))?;

        file.seek(SeekFrom::Start(self.data_offset + info.offset))
            .map_err(|e| SwarmLLMError::Model(format!("Failed to seek: {}", e)))?;

        let mut data = vec![0u8; info.byte_size()];
        file.read_exact(&mut data)
            .map_err(|e| SwarmLLMError::Model(format!("Failed to read tensor: {}", e)))?;
        ...
        F: FnMut(&[f32]) -> Result<()>,
    {
        ...
        while processed < num_elements {
            ...
            file.read_exact(&mut buffer[..byte_count])
                .map_err(|e| SwarmLLMError::Model(format!("Failed to read: {}", e)))?;
...
///
/// ```rust,ignore
/// use swarmllm::gguf::GgufModelLoader;
/// use std::path::Path;
///
