...
impl TryFrom<u32> for GgufValueType {
    ...
    fn try_from(value: u32) -> Result<Self> {
        match value {
            ...
            11 => Ok(Self::I64),
            12 => Ok(Self::F64),
            _ => Err(SwarmLLMError::Model(format!(
...
fn read_string<R: Read>(reader: &mut R) -> Result<String> {
    ...
    String::from_utf8(buf).map_err(|e| SwarmLLMError::Model(format!("Invalid UTF-8 string: {}", e)))
}

fn read_err(e: std::io::Error) -> SwarmLLMError {
    SwarmLLMError::Model(format!("Failed to read: {}", e))
}
