//! Swarmvector Capabilities Detection
//!
//! Auto-detection and graceful fallback for Swarmvector features.
//! This module provides compile-time and runtime detection of available features.
//!
//! ## Available Feature Flags
//!
//! - `HNSW_AVAILABLE`: HNSW index from swarmvector-core
//! - `ATTENTION_AVAILABLE`: Flash Attention from swarmvector-attention
//! - `GRAPH_AVAILABLE`: Knowledge graph from swarmvector-graph
//! - `GNN_AVAILABLE`: Graph neural networks from swarmvector-gnn
//! - `SONA_AVAILABLE`: SONA learning from swarmvector-sona
//!
...
pub const HNSW_AVAILABLE: bool = true; // Always available via swarmvector-core
...
pub const SONA_AVAILABLE: bool = true; // Always available via swarmvector-sona
...
static CAPABILITIES: OnceLock<SwarmvectorCapabilities> = OnceLock::new();

/// Swarmvector capabilities flags
///
/// Indicates which Swarmvector features are available at runtime.
/// Use `detect()` to get the current capabilities.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct SwarmvectorCapabilities {
...
impl Default for SwarmvectorCapabilities {
    fn default() -> Self {
        Self::detect()
    }
}

impl SwarmvectorCapabilities {
    /// Detect available Swarmvector capabilities
    ///
    ...
    ///
    /// ```rust,ignore
    /// use swarmllm::capabilities::SwarmvectorCapabilities;
    ///
    ...
    fn probe_quantization_level() -> u8 {
        // swarmvector-core always provides scalar quantization
    ...
    fn probe_performance() -> (usize, u64) {
        // These are estimated based on benchmarks from swarmvector-core
        if SIMD_AVAILABLE && PARALLEL_AVAILABLE {
...
mod tests {
    ...
    fn test_capabilities_cached() {
        let caps1 = SwarmvectorCapabilities::detect();
        let caps2 = SwarmvectorCapabilities::cached();
    ...
    fn test_capabilities_summary() {
        ...
        assert!(summary.contains("Swarmvector"));
