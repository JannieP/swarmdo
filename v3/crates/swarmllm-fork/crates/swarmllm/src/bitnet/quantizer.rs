...
///
/// ```rust,ignore
/// use swarmllm::bitnet::PtBitnetConfig;
///
...
///
/// ```rust,ignore
/// use swarmllm::bitnet::absmean_ternary;
///
...
///
/// ```rust,ignore
/// use swarmllm::bitnet::{quantize_tensor, PtBitnetConfig};
///
...
pub fn quantize_tensor(
    ...
    let num_blocks = total_elements.checked_add(block_size - 1).ok_or_else(|| {
        SwarmLLMError::Model("Integer overflow in block count calculation".to_string())
    })? / block_size;
