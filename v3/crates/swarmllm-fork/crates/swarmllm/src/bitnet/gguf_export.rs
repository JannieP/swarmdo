...
pub fn export_craftsman_model(path: &Path, tensors: HashMap<String, ExportTensor>) -> Result<()> {
    ...
    let file = std::fs::File::create(path)
        .map_err(|e| SwarmLLMError::Model(format!("Failed to create file: {}", e)))?;
...
pub fn validate_export(data: &[u8], expected_tensors: usize) -> Result<()> {
    ...
    if header.magic != GGUF_MAGIC {
        return Err(SwarmLLMError::Model("Invalid GGUF magic number".into()));
    }
...
fn io_err(e: io::Error) -> SwarmLLMError {
    SwarmLLMError::Model(format!("GGUF write error: {}", e))
}
