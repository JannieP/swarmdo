...
impl ResponseStreamer {
    ...
    pub async fn process_token(&mut self, text: String, quality_score: Option<f32>) -> Result<()> {
        ...
        self.sender
            ...
            .await
            .map_err(|e| SwarmLLMError::InvalidOperation(format!("Failed to send token: {}", e)))?;
    ...
    pub async fn complete(&mut self, usage: UsageStats, stop_reason: String) -> Result<()> {
        ...
        self.sender
            ...
            .await
            .map_err(|e| {
                SwarmLLMError::InvalidOperation(format!("Failed to send complete: {}", e))
            })?;
...
impl AgentCoordinator {
        ...
        F: FnOnce(&mut AgentContext),
    {
        ...
        let agent = agents
            .get_mut(agent_id)
            .ok_or_else(|| SwarmLLMError::NotFound(format!("Agent {} not found", agent_id)))?;
        f(agent);
        Ok(())
    }
