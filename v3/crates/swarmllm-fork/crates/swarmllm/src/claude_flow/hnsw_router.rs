...
//!
//! Provides 150x faster pattern search for task routing using swarmvector-core's HNSW index.
...
use swarmvector_core::types::{DistanceMetric, HnswConfig, SearchResult};
...
///
/// Uses swarmvector-core's HNSW index for 150x faster pattern search compared to
...
impl HnswRouter {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>> {
        ...
        bincode::serde::encode_to_vec(&state, bincode::config::standard())
            .map_err(|e| SwarmLLMError::Serialization(e.to_string()))
    }
