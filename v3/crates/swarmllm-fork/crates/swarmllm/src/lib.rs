//! # SwarmLLM - LLM Serving Runtime with Swarmvector Integration
//!
//! SwarmLLM is an edge-focused LLM serving runtime designed for portable, high-performance
//! inference across heterogeneous hardware. It integrates with Swarmvector for intelligent
//! memory capabilities, enabling continuous self-improvement through SONA learning.
//!
//! ## Architecture
//!
//! SwarmLLM uses Swarmvector as a unified memory layer with three distinct roles:
//!
...
//! - [`PolicyStore`]: Swarmvector-backed policy storage with semantic search
...
//!
//! ```rust,ignore
//! use swarmllm::{SwarmLLMConfig, SwarmLLMEngine};
//!
//! // Create engine with default configuration
//! let config = SwarmLLMConfig::default();
//! let engine = SwarmLLMEngine::new(config)?;
//!
//! // Create a session
...
pub mod reflection;
pub mod swarmvector_integration;
pub mod serving;
pub mod session;
...
pub use error::{Result, SwarmLLMError};
pub use gguf::{
    GgufFile,
    GgufHeader,
...
pub use capabilities::{
    gate_feature, gate_feature_or, SwarmvectorCapabilities, ATTENTION_AVAILABLE, GNN_AVAILABLE,
    GRAPH_AVAILABLE, HNSW_AVAILABLE, PARALLEL_AVAILABLE, SIMD_AVAILABLE, SONA_AVAILABLE,
};
pub use swarmvector_integration::{
    IndexStats,
    IntegrationConfig,
    IntegrationStats,
    // Intelligence layer
...
/// SwarmLLM engine configuration.
///
/// This configuration struct controls all aspects of the SwarmLLM engine,
...
///
/// ```rust,ignore
/// use swarmllm::{SwarmLLMConfig, PagedAttentionConfig, KvCacheConfig};
///
/// let config = SwarmLLMConfig {
///     storage_path: "/var/swarmllm".to_string(),
///     max_sessions: 500,
///     embedding_dim: 1024,
///     ..Default::default()
/// };
...
impl Default for SwarmLLMConfig {
    fn default() -> Self {
        Self {
            storage_path: ".swarmllm".to_string(),
...
/// Main SwarmLLM engine for LLM inference with intelligent memory.
///
/// The `SwarmLLMEngine` is the primary entry point for SwarmLLM, providing:
///
/// - **Session Management**: Create and manage user sessions with state persistence
/// - **Policy Storage**: Swarmvector-backed semantic search for runtime policies
...
///
/// ```rust,ignore
/// use swarmllm::{SwarmLLMEngine, SwarmLLMConfig};
///
...
/// +-------------------+     +-------------------+
/// ```
pub struct SwarmLLMEngine {
    /// Configuration
    config: SwarmLLMConfig,
    /// Policy store backed by Swarmvector
    policy_store: PolicyStore,
    /// Session manager
    session_manager: SessionManager,
    /// Session index backed by Swarmvector
...
impl SwarmLLMEngine {
    /// Create a new SwarmLLM engine with the given configuration.
    ///
    ...
    /// use swarmllm::{SwarmLLMEngine, SwarmLLMConfig};
    ///
    /// let engine = SwarmLLMEngine::new(SwarmLLMConfig::default())?;
    /// ```
    pub fn new(config: SwarmLLMConfig) -> Result<Self> {
    ...
    /// Swarmvector for semantic retrieval.
    ///
    /// # Arguments
    ///
    ...
    pub fn create_session(&self, user_id: Option<&str>) -> Result<Session> {
        ...
        // Index the session in Swarmvector
    ...
    ///
    /// ```rust,ignore
    /// use swarmllm::{WitnessEntry, LatencyBreakdown, RoutingDecision};
    ///
