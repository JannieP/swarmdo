...
///
/// ```rust,ignore
/// use swarmllm::quantize::security::WeightIntegrity;
///
...
impl WeightIntegrity {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let mse_bytes: [u8; 4] = bytes[64..68]
            .try_into()
            .map_err(|_| SwarmLLMError::Quantization("Invalid MSE bytes".to_string()))?;
...
pub fn validate_gguf_security(path: &Path) -> Result<GgufSecurityReport> {
    ...
    let file = File::open(path).map_err(|e| {
        SwarmLLMError::Io(std::io::Error::new(
            e.kind(),
...
pub fn validate_gguf_quick(path: &Path) -> Result<()> {
    ...
    if !report.is_valid {
        return Err(SwarmLLMError::Gguf(format!(
