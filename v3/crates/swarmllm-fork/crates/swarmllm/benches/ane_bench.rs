...
//! ```bash
//! cargo bench -p swarmllm --features coreml --bench ane_bench
//! ```
//!
//! Compare ANE vs Accelerate:
//! ```bash
//! cargo bench -p swarmllm --features coreml,accelerate --bench ane_bench
//! ```
//!
...
//! ```bash
//! sudo powermetrics --samplers tasks -i 100 | grep swarmllm
//! ```
...
fn bench_gemm_comparison(c: &mut Criterion) {
    ...
    for (m, k, n) in sizes {
        ...
        #[cfg(all(target_os = "macos", feature = "accelerate"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    swarmllm::kernels::accelerate::gemm_accelerate(
                        black_box(&a),
                        black_box(&b),
                        black_box(&mut c_out),
                        m,
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    swarmllm::kernels::ane_ops::matmul_ane(
                        black_box(&a),
                        black_box(&b),
                        black_box(&mut c_out),
                        m,
...
fn bench_batched_gemm_comparison(c: &mut Criterion) {
    ...
    for (batch_size, m, k, n) in configs {
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    swarmllm::kernels::ane_ops::batched_matmul_ane(
                        black_box(&a),
                        black_box(&b),
                        black_box(&mut c_out),
                        batch_size,
    ...
    for (batch_size, dim) in configs {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    ...
                    swarmllm::kernels::activations::batch_gelu(black_box(&mut x), dim);
                })
            });
        }
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::ane_ops::gelu_ane(black_box(&mut x), batch_size, dim);
                })
            });
        }
    }
    ...
    for (batch_size, dim) in configs {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::activations::batch_silu(black_box(&mut x), dim);
                })
            });
        }
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::ane_ops::silu_ane(black_box(&mut x), batch_size, dim);
                })
            });
        }
    }
    ...
    for (batch_size, dim) in configs {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::activations::batch_softmax(black_box(&mut x), dim);
                })
            });
        }
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::ane_ops::softmax_ane(black_box(&mut x), batch_size, dim);
                })
            });
        }
    }
    ...
    for (batch_size, dim) in configs {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::norm::batched_layer_norm_neon(
                        black_box(&mut x),
                        black_box(&weight),
                        black_box(&bias),
                        batch_size,
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::ane_ops::layer_norm_ane(
                        black_box(&mut x),
                        black_box(&weight),
                        black_box(&bias),
                        batch_size,
    ...
    for (batch_size, dim) in configs {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::norm::batched_rms_norm_neon(
                        black_box(&mut x),
                        black_box(&weight),
                        batch_size,
                        dim,
        ...
        #[cfg(all(target_os = "macos", feature = "coreml"))]
        {
            ...
            group.bench_function(id, |bencher| {
                bencher.iter(|| {
                    x.copy_from_slice(&x_orig);
                    swarmllm::kernels::ane_ops::rms_norm_ane(
                        black_box(&mut x),
                        black_box(&weight),
                        batch_size,
                        dim,
    ...
    // Auto-dispatch GELU
    {
        ...
        group.bench_function("gelu_auto", |bencher| {
            bencher.iter(|| {
                x.copy_from_slice(&x_orig);
                #[cfg(all(target_os = "macos", feature = "coreml"))]
                swarmllm::kernels::ane_ops::gelu_auto(black_box(&mut x), batch_size, dim);
    ...
    // Auto-dispatch SiLU
    {
        ...
        group.bench_function("silu_auto", |bencher| {
            bencher.iter(|| {
                x.copy_from_slice(&x_orig);
                #[cfg(all(target_os = "macos", feature = "coreml"))]
                swarmllm::kernels::ane_ops::silu_auto(black_box(&mut x), batch_size, dim);
                #[cfg(not(all(target_os = "macos", feature = "coreml")))]
                swarmllm::kernels::activations::batch_silu(black_box(&mut x), dim);
            })
        });
    ...
    // Auto-dispatch LayerNorm
    {
        ...
        group.bench_function("layernorm_auto", |bencher| {
            bencher.iter(|| {
                x.copy_from_slice(&x_orig);
                #[cfg(all(target_os = "macos", feature = "coreml"))]
                swarmllm::kernels::ane_ops::layer_norm_auto(
                    black_box(&mut x),
                    black_box(&weight),
                    black_box(&bias),
                    batch_size,
    ...
    #[cfg(target_arch = "aarch64")]
    {
        group.bench_function("neon", |bencher| {
            bencher.iter(|| {
                ...
                // SiLU activation
                swarmllm::kernels::activations::batch_silu(
                    black_box(&mut intermediate),
                    intermediate_dim,
                );
                // Down projection
    ...
    #[cfg(all(target_os = "macos", feature = "coreml"))]
    {
        group.bench_function("ane", |bencher| {
            bencher.iter(|| {
                ...
                // SiLU activation
                swarmllm::kernels::ane_ops::silu_ane(
                    black_box(&mut intermediate),
                    batch_size,
                    intermediate_dim,
                );
    ...
    for (batch, seq_len, hidden_dim, num_heads, head_dim, intermediate_dim) in configs {
        ...
        group.bench_function(id, |bencher| {
            bencher.iter(|| {
                ...
                swarmllm::kernels::ane_ops::matmul_auto(
                    &hidden,
                    &w_q,
                    &mut q,
                    batch * seq_len,
                ...
                swarmllm::kernels::ane_ops::silu_auto(
                    black_box(&mut intermediate),
                    batch * seq_len,
                    intermediate_dim,
                );
