# Cypher Query Language Parser for SwarmVector

A complete Cypher-compatible query language parser implementation for the SwarmVector graph database, built using the nom parser combinator library.
...
Traditional graph databases represent relationships as binary edges (one source, one target). SwarmVector's Cypher parser supports **hyperedges** - relationships connecting multiple nodes simultaneously.
