...
//! (InfoNCE over the ogbn-arxiv citation graph) using `swarmvector-gnn`'s own optimizer
//! and loss; the index is the shipping `swarmvector-diskann` Vamana, driven through its
...
//!
//! Run: cargo run --release -p swarmvector-gnn --example diskann_real_trajectory -- [N] [EPOCHS]

use ndarray::Array2;
use rand::{rngs::StdRng, Rng, SeedableRng};
use swarmvector_diskann::distance::{l2_squared, FlatVectors};
use swarmvector_diskann::{DriftingIndex, RebuildPolicy};
use swarmvector_gnn::training::{info_nce_loss, Optimizer, OptimizerType};
