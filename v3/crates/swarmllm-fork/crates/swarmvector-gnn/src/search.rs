use crate::layer::SwarmvectorLayer;
...
pub fn hierarchical_forward(
    ...
    for (embeddings, gnn_layer) in layer_embeddings.iter().zip(gnn_layers.iter()) {
...
mod tests {
    ...
    fn test_hierarchical_forward() {
        ...
        let gnn_layers = vec![
            SwarmvectorLayer::new(2, 2, 1, 0.0).unwrap(), // input_dim, hidden_dim, heads, dropout
        ];
