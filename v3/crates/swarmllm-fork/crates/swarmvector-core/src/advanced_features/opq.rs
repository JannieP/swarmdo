...
impl OPQConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.codebook_size > 256 {
            return Err(SwarmvectorError::InvalidParameter(format!(
                "Codebook size {} exceeds u8 max 256",
                self.codebook_size
            )));
        }
        if self.num_subspaces == 0 {
            return Err(SwarmvectorError::InvalidParameter(
...
impl OPQIndex {
    ...
    fn check_dim(&self, len: usize) -> Result<()> {
        if len != self.dimensions {
            Err(SwarmvectorError::DimensionMismatch {
    ...
    cbs.iter()
        ...
        .map(|(s, cb)| {
            ...
            cb.iter()
                ...
                .map(|(i, _)| i as u8)
                .ok_or_else(|| SwarmvectorError::Internal("Empty codebook".into()))
        })
        .collect()
}
