...
//!
//! ```rust,no_run
//! use swarmvector_core::embeddings::{EmbeddingProvider, HashEmbedding};
//!
...
//!
//! ```rust,ignore
//! use swarmvector_core::embeddings::{EmbeddingProvider, OnnxEmbedding};
//!
...
use crate::error::SwarmvectorError;
use std::sync::Arc;
...
pub mod candle {
    ...
    impl CandleEmbedding {
        ...
        /// # #[cfg(feature = "real-embeddings")]
        /// # {
        /// use swarmvector_core::embeddings::candle::CandleEmbedding;
        ///
        ...
        pub fn from_pretrained(model_id: &str, _use_gpu: bool) -> Result<Self> {
            Err(SwarmvectorError::ModelLoadError(format!(
    ...
    impl EmbeddingProvider for CandleEmbedding {
        fn embed(&self, _text: &str) -> Result<Vec<f32>> {
            Err(SwarmvectorError::ModelInferenceError(
...
/// # Example (OpenAI)
/// ```rust,no_run
/// use swarmvector_core::embeddings::{EmbeddingProvider, ApiEmbedding};
///
...
impl EmbeddingProvider for ApiEmbedding {
    fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let response = self
            ...
            .send()
            .map_err(|e| {
                SwarmvectorError::ModelInferenceError(format!("API request failed: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            return Err(SwarmvectorError::ModelInferenceError(format!(
                "API returned error {}: {}",
                status, error_text
            )));
        }
        ...
        let response_json: serde_json::Value = response.json().map_err(|e| {
            SwarmvectorError::ModelInferenceError(format!("Failed to parse response: {}", e))
        })?;
        ...
        let embedding = if let Some(data) = response_json.get("data") {
            ...
            data.as_array()
                ...
                .ok_or_else(|| {
                    SwarmvectorError::ModelInferenceError("Invalid OpenAI response format".to_string())
                })?
            ...
            embeddings
                ...
                .and_then(|emb| emb.as_array())
                .ok_or_else(|| {
                    SwarmvectorError::ModelInferenceError("Invalid Cohere response format".to_string())
                })?
        } else {
            return Err(SwarmvectorError::ModelInferenceError(
        ...
        let embedding_vec: Result<Vec<f32>> = embedding
            ...
            .map(|v| {
                v.as_f64().map(|f| f as f32).ok_or_else(|| {
                    SwarmvectorError::ModelInferenceError("Invalid embedding value".to_string())
                })
            })
            .collect();
    ...
    impl OnnxEmbedding {
        ...
        pub fn from_pretrained(model_id: &str) -> Result<Self> {
            let api = hf_hub::api::sync::Api::new().map_err(|e| {
                SwarmvectorError::ModelLoadError(format!("Failed to create HuggingFace API: {}", e))
            })?;
            ...
            let model_path = repo
                ...
                .map_err(|e| {
                    SwarmvectorError::ModelLoadError(format!(
        ...
        pub fn from_files(
            ...
            let session = Session::builder()
                ...
                .map_err(|e| {
                    SwarmvectorError::ModelLoadError(format!("Failed to set thread count: {}", e))
                })?
                .commit_from_file(model_path)
                .map_err(|e| {
                    SwarmvectorError::ModelLoadError(format!("Failed to load ONNX model: {}", e))
                })?;

            // Load tokenizer
            let tokenizer = Tokenizer::from_file(tokenizer_path).map_err(|e| {
                SwarmvectorError::ModelLoadError(format!("Failed to load tokenizer: {}", e))
            })?;
        ...
        fn embed(&self, text: &str) -> Result<Vec<f32>> {
            ...
            let encoding = {
                ...
                tokenizer.encode(text, true).map_err(|e| {
                    SwarmvectorError::ModelInferenceError(format!("Tokenization failed: {}", e))
                })?
            };
            ...
            let input_ids_tensor =
                Tensor::<i64>::from_array(([1, seq_len], input_ids.clone().into_boxed_slice()))
                    .map_err(|e| {
                        SwarmvectorError::ModelInferenceError(format!(
            ...
            let (output_data, output_shape_vec) = {
                ...
                let outputs = session
                    ...
                    .map_err(|e| {
                        SwarmvectorError::ModelInferenceError(format!("ONNX inference failed: {}", e))
                    })?;
