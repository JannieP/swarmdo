#!/usr/bin/env bash
# RufVector-Postgres Docker Test Script
...
cmd_build() {
    ...
    log_info "Building rufvector-postgres image..."
    DOCKER_BUILDKIT=1 docker build \
        ${build_args} \
        -f crates/rufvector-postgres/docker/Dockerfile \
        -t "rufvector-postgres:pg${PG_VERSION}" \
        --progress=plain \
        .
...
cmd_test() {
    ...
    while [ ${waited} -lt ${max_wait} ]; do
        if dc exec -T postgres pg_isready -U rufvector -d rufvector_test &>/dev/null; then
    ...
    # Collect results
    log_info "Test results available in: rufvector-test-results volume"
...
cmd_benchmark() {
    ...
    log_info "Benchmark results available in: rufvector-benchmark-results volume"
...
cmd_clean() {
    ...
    docker rmi rufvector-postgres:pg14 2>/dev/null || true
    docker rmi rufvector-postgres:pg15 2>/dev/null || true
    docker rmi rufvector-postgres:pg16 2>/dev/null || true
    docker rmi rufvector-postgres:pg17 2>/dev/null || true

    log_info "Removing volumes..."
    docker volume rm rufvector-postgres-data 2>/dev/null || true
    docker volume rm rufvector-cargo-cache 2>/dev/null || true
    docker volume rm rufvector-cargo-git 2>/dev/null || true
    docker volume rm rufvector-target-cache 2>/dev/null || true
    docker volume rm rufvector-test-results 2>/dev/null || true
    docker volume rm rufvector-benchmark-results 2>/dev/null || true
