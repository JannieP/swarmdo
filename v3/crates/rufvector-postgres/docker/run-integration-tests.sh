#!/usr/bin/env bash
# RufVector-Postgres Integration Test Runner
...
POSTGRES_CONTAINER="rufvector-postgres-integration"
TEST_RUNNER_CONTAINER="rufvector-integration-runner"
...
wait_for_postgres() {
    ...
    while [ ${attempt} -le ${max_attempts} ]; do
        if docker exec "${POSTGRES_CONTAINER}" pg_isready -U rufvector -d rufvector_test &>/dev/null; then
...
verify_extension() {
    log_info "Verifying RufVector extension..."

    docker exec "${POSTGRES_CONTAINER}" psql -U rufvector -d rufvector_test -c "
        SELECT rufvector_version();
        SELECT rufvector_simd_info();
    " || {
        log_error "Failed to verify RufVector extension"
        return 1
    }

    log_success "RufVector extension verified"
}

build_extension() {
    log_section "Building RufVector Extension"

    cd "${PROJECT_ROOT}"

    DOCKER_BUILDKIT=1 docker build \
        -f crates/rufvector-postgres/docker/Dockerfile \
        -t "rufvector-postgres:pg${PG_VERSION}-test" \
...
start_postgres() {
    ...
    docker run -d \
        ...
        -e POSTGRES_USER=rufvector \
        -e POSTGRES_PASSWORD=rufvector \
        -e POSTGRES_DB=rufvector_test \
        -p 5433:5432 \
        --health-cmd="pg_isready -U rufvector -d rufvector_test" \
        --health-interval=5s \
        --health-timeout=5s \
        --health-retries=10 \
        "rufvector-postgres:pg${PG_VERSION}-test"
...
setup_test_schema() {
    ...
    docker exec "${POSTGRES_CONTAINER}" psql -U rufvector -d rufvector_test << 'EOF'
...
-- Grant permissions
GRANT ALL ON ALL SCHEMAS IN DATABASE rufvector_test TO rufvector;
...
run_integration_tests() {
    ...
    export DATABASE_URL="postgresql://rufvector:rufvector@localhost:5433/rufvector_test"
...
EOF
    ...
    docker exec "${POSTGRES_CONTAINER}" psql -U rufvector -d rufvector_test \
...
generate_report() {
    ...
    cat > "${report_file}" << EOF
# RufVector Postgres Integration Test Report
...
main() {
    ...
    log_section "RufVector Integration Test Suite"
