#!/usr/bin/env bash
# RufVector-Postgres Test Runner
...
CONTAINER_NAME="rufvector-postgres-test"
IMAGE_NAME="rufvector-postgres:test"
POSTGRES_PORT="${POSTGRES_PORT:-5433}"
POSTGRES_USER="${POSTGRES_USER:-rufvector}"
POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-rufvector}"
POSTGRES_DB="${POSTGRES_DB:-rufvector_test}"
...
run_integration_tests() {
    ...
    docker exec -it "${CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -c "CREATE EXTENSION IF NOT EXISTS rufvector_postgres;" || {
...
Environment Variables:
    ...
    POSTGRES_USER          PostgreSQL user (default: rufvector)
    POSTGRES_PASSWORD      PostgreSQL password (default: rufvector)
    POSTGRES_DB            PostgreSQL database (default: rufvector_test)
...
main() {
    ...
    log_info "RufVector-Postgres Test Runner"
    log_info "Platform: ${PLATFORM}"
