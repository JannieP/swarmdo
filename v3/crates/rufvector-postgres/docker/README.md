...
docker pull rufvector/rufvector-postgres:latest

# Or specific PostgreSQL version
docker pull rufvector/rufvector-postgres:2.0.0-pg17
docker pull rufvector/rufvector-postgres:2.0.0-pg16
docker pull rufvector/rufvector-postgres:2.0.0-pg15
docker pull rufvector/rufvector-postgres:2.0.0-pg14
...
docker run -d \
  ...
  rufvector/rufvector-postgres:latest

# Connect and test
psql -h localhost -U rufvector -d rufvector_test -c "SELECT rufvector_version();"
```
...
| **SIMD** | AVX2/AVX512/NEON acceleration | `rufvector_simd_info()` |
...
| **Flash Attention** | Memory-efficient attention | `rufvector_flash_attention()` |
| **Multi-Head Attention** | Transformer attention | `rufvector_multi_head_attention()` |
...
- `-d, --database DB` - PostgreSQL database (default: rufvector_dev)
...
**Services:**
- `postgres` - PostgreSQL 16 with rufvector extension
...
POSTGRES_DB=rufvector_dev    # Database name
```
...
POSTGRES_USER=rufvector      # PostgreSQL user
POSTGRES_PASSWORD=rufvector  # PostgreSQL password
POSTGRES_DB=rufvector_test   # Test database name
...
./dev.sh psql
# DROP EXTENSION rufvector_postgres CASCADE;
# CREATE EXTENSION rufvector_postgres;
```
...
# View logs
docker logs rufvector-postgres-test
```
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: |
          cd crates/rufvector-postgres/docker
          ./run-tests.sh
```
...
- [RufVector Repository](https://github.com/ruvnet/rufvector)
