#!/usr/bin/env bash
# RufVector-Postgres Test Runner Script
...
cd /app/crates/rufvector-postgres 2>/dev/null || cd /app
...
<testsuites name="rufvector-postgres" tests="${TESTS_RUN}" failures="${TESTS_FAILED}" time="${DURATION}">
  <testsuite name="cargo-test" tests="${TESTS_RUN}" failures="${TESTS_FAILED}" time="${DURATION}">
    <testcase name="pgrx-tests" classname="rufvector_postgres" time="${DURATION}">
