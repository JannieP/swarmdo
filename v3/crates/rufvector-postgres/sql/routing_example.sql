...
SELECT rufvector_update_agent_metrics(
    'gpt-4',
    450.0,
    true,
    0.92
...
SELECT * FROM rufvector_find_agents_by_capability('writing', 5);
SELECT * FROM rufvector_find_agents_by_capability('fast', 5);

-- Example 15: Get detailed agent information
SELECT rufvector_get_agent('gpt-4') AS agent_details;
SELECT rufvector_get_agent('claude-3-opus') AS agent_details;

-- Example 16: View routing statistics
SELECT rufvector_routing_stats() AS stats;
...
SELECT
    ...
    COALESCE(
        (SELECT rufvector_route(r.embedding, 'cost', '{"max_cost": 0.01, "min_quality": 0.8}'::jsonb)),
        -- Fallback to balanced if no cheap option
        rufvector_route(r.embedding, 'balanced', '{"max_cost": 0.05}'::jsonb)
    ) AS routing_decision
FROM ai_requests r;
...
SELECT
    ...
    (rufvector_route(r.embedding, 'quality', '{"excluded_agents": ["claude-3-opus"]}'::jsonb))::jsonb->>'expected_quality' AS a_score,
    (rufvector_route(r.embedding, 'quality', '{"excluded_agents": ["gpt-4"]}'::jsonb))::jsonb->>'expected_quality' AS b_score
FROM ai_requests r
WHERE created_at > NOW() - INTERVAL '1 hour'
LIMIT 100;
...
UPDATE rufvector_list_agents()
SET is_active = false
WHERE success_rate < 0.80;

-- Example 23: Remove inactive agents
SELECT rufvector_remove_agent(name)
FROM rufvector_list_agents()
WHERE NOT is_active
