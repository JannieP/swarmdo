...
CREATE OPERATOR CLASS rufvector_ivfflat_l2_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <-> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 rufvector_l2_distance(vector, vector);

-- Inner product distance operator class
CREATE OPERATOR CLASS rufvector_ivfflat_ip_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <#> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 rufvector_ip_distance(vector, vector);

-- Cosine distance operator class
CREATE OPERATOR CLASS rufvector_ivfflat_cosine_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <=> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 rufvector_cosine_distance(vector, vector);

-- Helper function to get IVFFlat index statistics
CREATE OR REPLACE FUNCTION rufvector_ivfflat_stats(index_name text)
RETURNS TABLE(
    lists integer,
    probes integer,
    dimensions integer,
...
--
-- SELECT * FROM rufvector_ivfflat_stats('items_embedding_idx');
