# RufVector PostgreSQL Extension - Test Suite

## 📋 Overview

This directory contains the comprehensive test framework for rufvector-postgres, a high-performance PostgreSQL vector similarity search extension. The test suite consists of **9 test files** with **3,276 lines** of test code, providing extensive coverage across all components.

## 🗂️ Test Files

### 1. `unit_vector_tests.rs` (677 lines)
**Core RufVector type unit tests**
...
### By Component
- **Core Types** (45%): RufVector, HalfVec
...
fn test_sql_behavior() {
    let result = Spi::get_one::<f32>(
        "SELECT distance('[1,2,3]'::rufvector, '[4,5,6]'::rufvector)"
    );
