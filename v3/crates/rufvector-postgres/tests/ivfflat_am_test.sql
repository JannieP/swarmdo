...
SELECT setting FROM pg_settings WHERE name = 'rufvector.ivfflat_probes';
...
SELECT
    ...
    (
        SELECT COUNT(*)
        FROM test_ivfflat, query
        WHERE pg_catalog.set_config('rufvector.ivfflat_probes', '1', true) IS NOT NULL
        ORDER BY embedding <-> query.q
        LIMIT 10
    ) as result_count
UNION ALL
SELECT
    ...
    (
        SELECT COUNT(*)
        FROM test_ivfflat, query
        WHERE pg_catalog.set_config('rufvector.ivfflat_probes', '10', true) IS NOT NULL
        ORDER BY embedding <-> query.q
        LIMIT 10
    ) as result_count;
...
SELECT * FROM rufvector_ivfflat_stats('test_ivfflat_l2_idx');
