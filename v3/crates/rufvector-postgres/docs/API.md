# RufVector-Postgres API Reference

## Overview

Complete API reference for RufVector-Postgres extension, including SQL functions, operators, types, and GUC variables.
...
CREATE TABLE items (
    ...
    embedding rufvector(1536)  -- OpenAI ada-002 dimensions
);

INSERT INTO items (embedding) VALUES ('[1.0, 2.0, 3.0]');
INSERT INTO items (embedding) VALUES (ARRAY[1.0, 2.0, 3.0]::rufvector);
```
...
- 50% memory reduction vs `rufvector`
...
CREATE TABLE items (
    id SERIAL PRIMARY KEY,
    embedding halfvec(1536)  -- 3,080 bytes vs 6,152 for rufvector
);

-- Automatic conversion from rufvector
INSERT INTO items (embedding)
SELECT embedding::halfvec FROM rufvector_table;
```
...
```sql
rufvector_l2_distance(a rufvector, b rufvector) → float4
```
...
```sql
SELECT rufvector_l2_distance(
    '[1.0, 2.0, 3.0]'::rufvector,
    '[4.0, 5.0, 6.0]'::rufvector
);
-- Output: 5.196...
```

#### `rufvector_cosine_distance(a, b)`
...
```sql
rufvector_cosine_distance(a rufvector, b rufvector) → float4
```
...
```sql
SELECT rufvector_cosine_distance(
    '[1.0, 0.0]'::rufvector,
    '[0.0, 1.0]'::rufvector
);
...
```sql
rufvector_ip_distance(a rufvector, b rufvector) → float4
```
...
```sql
rufvector_l1_distance(a rufvector, b rufvector) → float4
```
...
```sql
rufvector_norm(v rufvector) → float4
```
...
```sql
SELECT rufvector_norm('[3.0, 4.0]'::rufvector);
-- Output: 5.0
```

#### `rufvector_normalize(v)`
...
```sql
rufvector_normalize(v rufvector) → rufvector
```
...
```sql
SELECT rufvector_normalize('[3.0, 4.0]'::rufvector);
-- Output: [0.6, 0.8]
```

### Index Maintenance Functions

#### `rufvector_index_stats(index_name)`
...
```sql
rufvector_index_stats(index_name text) → TABLE(
    index_name text,
...
#### `rufvector_index_maintenance(index_name)`
...
```sql
rufvector_index_maintenance(index_name text) → void
```
...
```sql
SELECT * FROM items
ORDER BY embedding <-> '[1.0, 2.0, 3.0]'::rufvector
LIMIT 10;
```
...
- `rufvector_l2_ops`: For `<->` operator
- `rufvector_ip_ops`: For `<#>` operator
- `rufvector_cosine_ops`: For `<=>` operator
...
```sql
CREATE INDEX ... USING ruhnsw (embedding rufvector_l2_ops);
SELECT ... ORDER BY embedding <-> query;
```
...
```sql
CREATE INDEX ... USING ruhnsw (embedding rufvector_ip_ops);
SELECT ... ORDER BY embedding <#> query;
```

### `rufvector_cosine_ops`
...
```sql
CREATE INDEX ... USING ruhnsw (embedding rufvector_cosine_ops);
SELECT ... ORDER BY embedding <=> query;
```
...
INSERT INTO documents (content, embedding) VALUES
    ('Document 1', '[0.1, 0.2, ...]'::rufvector),
    ('Document 2', '[0.3, 0.4, ...]'::rufvector);
...
-- Search
SELECT content, embedding <-> '[0.5, 0.6, ...]'::rufvector AS distance
FROM documents
ORDER BY distance
LIMIT 10;
```
...
BEGIN
    FOR ef_val IN 10, 20, 40, 80, 160 LOOP
        EXECUTE format('SET LOCAL rufvector.ef_search = %s', ef_val);
