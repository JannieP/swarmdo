...
This document describes the build system for the rufvector-postgres extension.
...
```bash
# Build image
docker build -t rufvector-postgres:latest -f crates/rufvector-postgres/Dockerfile .

# Run container
docker run -d \
  -e POSTGRES_PASSWORD=postgres \
  -p 5432:5432 \
  rufvector-postgres:latest
```
...
# Package location
ls target/release/rufvector-postgres-pg16/
```
...
```bash
# Copy files
sudo cp target/release/rufvector-postgres-pg16/usr/lib/postgresql/16/lib/*.so \
  /usr/lib/postgresql/16/lib/
sudo cp target/release/rufvector-postgres-pg16/usr/share/postgresql/16/extension/* \
  /usr/share/postgresql/16/extension/

# Verify installation
psql -c "CREATE EXTENSION rufvector;"
```
