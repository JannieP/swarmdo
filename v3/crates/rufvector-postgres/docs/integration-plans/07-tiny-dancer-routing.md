...
Integrate AI agent routing capabilities from `rufvector-tiny-dancer` into PostgreSQL, enabling intelligent request routing, model selection, and cost optimization directly in SQL.
...
SELECT * FROM rufvector_route_multi(
...
SELECT rufvector_create_route(
    name := 'customer_support',
    description := 'Handle customer support inquiries, complaints, and feedback',
    embedding := rufvector_embed('Customer support and help requests'),
...
SELECT rufvector_create_route(
    ...
    embedding := rufvector_embed('Technical documentation and API reference'),
    target_agent := 'docs-agent',
    priority := 2
);

-- Semantic route matching
SELECT * FROM rufvector_semantic_route(
...
SELECT * FROM rufvector_routing_analytics(
    time_range := '7 days',
    group_by := 'agent'
);
...
SELECT * FROM rufvector_optimize_budget(
...
SELECT * FROM rufvector_batch_route(
    requests := ARRAY[
        'Simple question 1',
...
SELECT request_id, rufvector_classify_request(content) AS classification
...
#[pg_extern]
fn rufvector_classify_request(request: &str) -> pgrx::JsonB {
...
#[pg_extern]
fn rufvector_routing_analytics(
...
```toml
[dependencies]
# Link to rufvector-tiny-dancer
rufvector-tiny-dancer-core = { path = "../rufvector-tiny-dancer-core", optional = true }
