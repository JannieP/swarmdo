...
SET poincare_embedding = rufvector_to_poincare(euclidean_embedding, curvature);
```
...
-- Poincaré distance
SELECT id, name,
       rufvector_poincare_distance(poincare_embedding, query_point) AS dist
...
SELECT id, name,
       rufvector_lorentz_distance(lorentz_embedding, query_point) AS dist
FROM hierarchical_nodes
ORDER BY dist
LIMIT 10;

-- Custom curvature
SELECT rufvector_hyperbolic_distance(
    a := point_a,
    b := point_b,
    model := 'poincare',
    curvature := -0.5
...
SELECT rufvector_mobius_add(point_a, point_b, curvature := -1.0);

-- Exponential map (tangent vector → manifold point)
SELECT rufvector_exp_map(base_point, tangent_vector, curvature := -1.0);

-- Logarithmic map (manifold point → tangent vector)
SELECT rufvector_log_map(base_point, target_point, curvature := -1.0);

-- Parallel transport (move vector along geodesic)
SELECT rufvector_parallel_transport(vector, from_point, to_point, curvature := -1.0);

-- Geodesic midpoint
SELECT rufvector_geodesic_midpoint(point_a, point_b);

-- Project Euclidean to hyperbolic
SELECT rufvector_project_to_hyperbolic(euclidean_vec, model := 'poincare');
```
...
CREATE INDEX ON hierarchical_nodes USING rufvector_hyperbolic (
...
fn rufvector_poincare_distance(a: Vec<f32>, b: Vec<f32>, curvature: default!(f32, -1.0)) -> f32 {
    let ball = PoincareBall::new(a.len(), curvature);
    ball.distance(&a, &b)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_mobius_add(a: Vec<f32>, b: Vec<f32>, curvature: default!(f32, -1.0)) -> Vec<f32> {
    let ball = PoincareBall::new(a.len(), curvature);
    ball.mobius_add(&a, &b)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_exp_map(base: Vec<f32>, tangent: Vec<f32>, curvature: default!(f32, -1.0)) -> Vec<f32> {
    let ball = PoincareBall::new(base.len(), curvature);
    ball.exp_map(&base, &tangent)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_log_map(base: Vec<f32>, target: Vec<f32>, curvature: default!(f32, -1.0)) -> Vec<f32> {
    let ball = PoincareBall::new(base.len(), curvature);
...
#[pg_extern(immutable, parallel_safe)]
fn rufvector_lorentz_distance(a: Vec<f32>, b: Vec<f32>, curvature: default!(f32, -1.0)) -> f32 {
    let model = LorentzModel::new(a.len(), curvature);
    model.distance(&a, &b)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_poincare_to_lorentz(poincare: Vec<f32>, curvature: default!(f32, -1.0)) -> Vec<f32> {
    let model = LorentzModel::new(poincare.len() + 1, curvature);
    model.from_poincare(&poincare, curvature)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_lorentz_to_poincare(lorentz: Vec<f32>) -> Vec<f32> {
...
#[pg_extern]
fn rufvector_hyperbolic_hnsw_handler(internal: Internal) -> Internal {
    // Index AM handler
}
```
...
#[pg_extern]
fn rufvector_batch_to_poincare(
    table_name: &str,
...
WHERE rufvector_poincare_distance(embedding, root_embedding) < subtree_radius
ORDER BY rufvector_poincare_distance(embedding, root_embedding);
```
...
SELECT entity_a.name, entity_b.name,
       rufvector_poincare_distance(entity_a.embedding, entity_b.embedding) AS distance
