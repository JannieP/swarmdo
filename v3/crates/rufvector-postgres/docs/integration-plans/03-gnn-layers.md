...
Integrate Graph Neural Network layers from `rufvector-gnn` into PostgreSQL, enabling graph-aware vector search, message passing, and neural graph queries directly in SQL.
...
│  │                    GNN Layer Registry                    │    │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────────┐  │    │
│  │  │  GCN  │ │GraphSAGE│ │  GAT  │ │  GIN  │ │ RufVector  │  │    │
│  │  └───┬───┘ └───┬───┘ └───┬───┘ └───┬───┘ └─────┬─────┘  │    │
│  └──────┼─────────┼─────────┼─────────┼───────────┼────────┘    │
│         └─────────┴─────────┴─────────┴───────────┘             │
│                              ▼                                   │
...
│   │   └── rufvector.rs     # Custom RufVector layer
...
CREATE INDEX ON nodes USING rufvector_gnn (
    embedding vector(256)
) WITH (
    edge_table = 'edges',
...
SELECT n.id, n.embedding,
       rufvector_gnn_score(n.embedding, query_vec, 'edges', 2) AS score
FROM nodes n
ORDER BY score DESC
LIMIT 10;
...
SELECT * FROM rufvector_subgraph_search(
    center_node := 42,
...
SELECT node_id,
       rufvector_gnn_classify(embedding, 'edges', model_name := 'node_classifier') AS class
FROM nodes;
```
...
SELECT rufvector_build_knn_graph(
    node_table := 'nodes',
    embedding_column := 'embedding',
...
SELECT rufvector_build_eps_graph(
    node_table := 'nodes',
    embedding_column := 'embedding',
...
### Phase 5: Custom RufVector Layer (Week 9-10)

```rust
// src/gnn/layers/rufvector.rs

/// RufVector's custom differentiable search layer
/// Combines HNSW navigation with learned message passing
pub struct RufVectorLayer {
    in_features: usize,
    out_features: usize,
    num_hops: usize,
...
#[pg_extern]
fn rufvector_differentiable_search(
    query: Vec<f32>,
    index_name: &str,
...
| RufVector | 10K | 100K | 256 | 25 | 55MB |

## Dependencies

```toml
[dependencies]
# Link to rufvector-gnn
rufvector-gnn = { path = "../rufvector-gnn", optional = true }
