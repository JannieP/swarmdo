...
Integrate graph database capabilities from `rufvector-graph` into PostgreSQL, enabling Cypher query language support, property graph operations, and vector-enhanced graph traversals directly in SQL.
...
-- Define node labels
SELECT rufvector_create_node_label('social_network', 'Person',
    properties := '{
        "name": "text",
        "age": "integer",
...
SELECT rufvector_create_node_label('social_network', 'Company',
    properties := '{
        "name": "text",
        "industry": "text",
        "embedding": "vector(768)"
...
-- Define edge types
SELECT rufvector_create_edge_type('social_network', 'KNOWS',
    properties := '{"since": "date", "strength": "float"}'
);

SELECT rufvector_create_edge_type('social_network', 'WORKS_AT',
...
SELECT * FROM rufvector_cypher('social_network', $$
...
-- Create nodes
SELECT rufvector_cypher('social_network', $$
...
SELECT * FROM rufvector_cypher('social_network', $$
    MATCH (p:Person)
    WHERE rufvector.similarity(p.embedding, $query) > 0.8
    RETURN p.name, p.age, rufvector.similarity(p.embedding, $query) AS similarity
...
SELECT * FROM rufvector_cypher('social_network', $$
    ...
    WHERE p.name = 'Alice'
    WITH friend, rufvector.similarity(friend.embedding, $query) AS sim
    WHERE sim > 0.7
...
SELECT * FROM rufvector_cypher('social_network', $$
    CALL rufvector.pagerank('Person', 'KNOWS', {
        dampingFactor: 0.85,
        iterations: 20,
...
SELECT * FROM rufvector_cypher('social_network', $$
    ...
    RETURN p, rufvector.pathEmbeddingDistance(p) AS distance
$$);
```
...
SELECT * FROM rufvector_cypher('social_network', $$
    CALL rufvector.louvain('Person', 'KNOWS', {resolution: 1.0})
...
SELECT * FROM rufvector_cypher('social_network', $$
    CALL rufvector.nodeSimilarity('Person', 'KNOWS', {
...
-- Centrality measures
SELECT * FROM rufvector_cypher('social_network', $$
    CALL rufvector.betweenness('Person', 'KNOWS')
    YIELD node, score
    RETURN node.name, score
    ORDER BY score DESC
    LIMIT 10
...
// Node label creation
#[pg_extern]
fn rufvector_create_node_label(
    graph_name: &str,
    label: &str,
...
#[pg_extern]
fn rufvector_graph_stats(graph_name: &str) -> pgrx::JsonB {
    let graph = get_graph(graph_name).expect("Graph not found");
...
- **RufVector-specific**:
  - `rufvector.similarity(embedding1, embedding2)`
  - `rufvector.distance(embedding1, embedding2, metric)`
  - `rufvector.knn(embedding, k)`
...
```toml
[dependencies]
# Link to rufvector-graph
rufvector-graph = { path = "../rufvector-graph", optional = true }
