...
SELECT id, content,
       rufvector_sparse_dot(sparse_embedding, query_sparse) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
...
SELECT id,
       rufvector_sparse_cosine(sparse_embedding, query_sparse) AS similarity
FROM documents
WHERE rufvector_sparse_cosine(sparse_embedding, query_sparse) > 0.5;
...
CREATE INDEX ON documents USING rufvector_sparse (
...
SELECT * FROM rufvector_hybrid_search(
...
fn rufvector_sparse_dot(a: PgSparseVec, b: PgSparseVec) -> f32 {
    sparse_dot(&a.0, &b.0)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_sparse_cosine(a: PgSparseVec, b: PgSparseVec) -> f32 {
    sparse_cosine(&a.0, &b.0)
}

#[pg_extern(immutable, parallel_safe)]
fn rufvector_sparse_euclidean(a: PgSparseVec, b: PgSparseVec) -> f32 {
...
#[pg_extern]
fn rufvector_sparse_handler(internal: Internal) -> Internal {
...
#[pg_extern]
fn rufvector_sparse_top_k(sparse: PgSparseVec, k: i32) -> PgSparseVec {
    PgSparseVec(sparse.0.top_k(k as usize))
}

#[pg_extern]
fn rufvector_sparse_prune(sparse: PgSparseVec, threshold: f32) -> PgSparseVec {
