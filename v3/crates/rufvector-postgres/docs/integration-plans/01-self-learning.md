...
Integrate adaptive learning capabilities into rufvector-postgres, enabling the database to learn from query patterns, optimize search strategies, and improve recall/precision over time.
...
SELECT rufvector_enable_learning('embeddings',
...
SELECT * FROM rufvector_learning_stats('embeddings');

-- Export learned patterns
SELECT rufvector_export_patterns('embeddings') AS patterns_json;

-- Import patterns from another instance
SELECT rufvector_import_patterns('embeddings', patterns_json);
```
...
SELECT rufvector_auto_tune('embeddings_idx',
    optimize_for := 'recall',  -- or 'latency', 'balanced'
    sample_queries := 1000
);

-- Get recommended index parameters
SELECT * FROM rufvector_recommend_params('embeddings');
```
...
SELECT rufvector_benchmark_recall(
...
SELECT rufvector_benchmark_latency(
    table_name := 'embeddings',
    num_queries := 10000,
    k := 10,
...
```toml
[dependencies]
# Existing rufvector crates (optional integration)
# rufvector-core = { path = "../rufvector-core", optional = true }
...
learning-distributed = ["learning", "rufvector-replication"]
```
