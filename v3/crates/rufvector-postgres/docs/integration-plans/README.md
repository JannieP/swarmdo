# RufVector-Postgres Integration Plans

Comprehensive implementation plans for integrating advanced capabilities into the rufvector-postgres PostgreSQL extension.

## Overview

These documents outline the roadmap to transform rufvector-postgres from a pgvector-compatible extension into a full-featured AI database with self-learning, attention mechanisms, GNN layers, and more.

## Current State

rufvector-postgres v0.1.0 includes:
...
```
rufvector-postgres
├── rufvector-core (shared types, SIMD)
├── rufvector-attention (optional)
├── rufvector-gnn (optional)
├── rufvector-graph (optional)
├── rufvector-tiny-dancer-core (optional)
└── External
    ├── pgrx (PostgreSQL FFI)
...
```sql
-- Enable features
SELECT rufvector_enable_feature('learning', table_name := 'embeddings');

-- Configuration via GUCs
SET rufvector.learning_rate = 0.01;
SET rufvector.attention_type = 'flash';

-- Feature-specific functions prefixed with rufvector_
SELECT rufvector_attention_score(a, b, 'scaled_dot');
SELECT rufvector_gnn_search(query, 'edges', num_hops := 2);
SELECT rufvector_route(request, optimize_for := 'cost');

-- Cypher queries via dedicated function
SELECT * FROM rufvector_cypher('graph_name', $$
