...
Integrate 39 attention mechanisms from `rufvector-attention` into PostgreSQL, enabling attention-weighted vector search, transformer-style queries, and neural reranking directly in SQL.
...
CREATE INDEX ON documents USING rufvector_attention (
...
SELECT id, content,
       rufvector_attention_score(embedding, query_vec, 'scaled_dot') AS score
FROM documents
ORDER BY score DESC
LIMIT 10;

-- Multi-head attention search
SELECT * FROM rufvector_mha_search(
...
FROM rufvector_cross_attention(
    query_table := 'queries',
...
SELECT id,
       rufvector_moe_route(embedding, num_experts := 8, top_k := 2) AS expert_weights
FROM documents;

-- Sliding window attention for long sequences
SELECT * FROM rufvector_sliding_attention(
...
#[pg_extern(immutable, parallel_safe)]
fn rufvector_hyperbolic_distance(
    a: Vec<f32>,
    b: Vec<f32>,
...
```toml
[dependencies]
# Link to rufvector-attention for implementations
rufvector-attention = { path = "../rufvector-attention", optional = true }
