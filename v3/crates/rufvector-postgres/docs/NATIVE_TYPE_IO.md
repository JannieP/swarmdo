# Native PostgreSQL Type I/O Functions for RufVector

## Overview

This document describes the native PostgreSQL type I/O functions implementation for the `RufVector` type, providing zero-copy access like pgvector.

## Implementation Summary

### Memory Layout

The `RufVector` type uses a pgvector-compatible varlena layout:
...
**Example:**
```sql
SELECT '[1.0, 2.0, 3.0]'::rufvector;
```

#### 2. `rufvector_out` - Text Output
...
#### 4. `rufvector_send` - Binary Output
...
unsafe fn from_varlena(varlena_ptr: *const pgrx::pg_sys::varlena) -> Self {
    ...
    RufVector { dimensions, data: data.to_vec() }
}
```
...
CREATE FUNCTION rufvector_in(cstring)
RETURNS rufvector
AS 'MODULE_PATHNAME', 'rufvector_in'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION rufvector_out(rufvector)
RETURNS cstring
AS 'MODULE_PATHNAME', 'rufvector_out'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION rufvector_recv(internal)
RETURNS rufvector
AS 'MODULE_PATHNAME', 'rufvector_recv'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION rufvector_send(rufvector)
RETURNS bytea
AS 'MODULE_PATHNAME', 'rufvector_send'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
- **Main Implementation**: `/home/user/rufvector/crates/rufvector-postgres/src/types/vector.rs`
