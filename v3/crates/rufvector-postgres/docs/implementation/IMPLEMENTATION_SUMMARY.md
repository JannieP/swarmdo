...
Complete implementation of IVFFlat (Inverted File with Flat quantization) as a PostgreSQL index access method for the rufvector extension. This provides native, high-performance approximate nearest neighbor (ANN) search directly integrated into PostgreSQL.
...
- **GUC Variables**: Configurable via `rufvector.ivfflat_probes`
...
|----------|---------|-------------|
| rufvector.ivfflat_probes | 1 | Number of lists to probe during search |
...
SET rufvector.ivfflat_probes = 20;
...
2. **Types Module**: Compatible with `RufVector` type
...
| Feature | rufvector IVFFlat | pgvector IVFFlat |
|---------|------------------|------------------|
