...
graph_embeddings AS (
    SELECT
        graph_id,
        rufvector_gnn_aggregate(embeddings, 'mean') as graph_embedding
    FROM node_embeddings
)
...
1. **Use Sampling for Large Graphs**:
   ...
   SELECT rufvector_graphsage_forward(..., 10);  -- Sample 10 neighbors
   ```

2. **Batch Processing**:
   ```sql
   -- Process multiple graphs at once
   SELECT rufvector_gnn_batch_forward(...);
   ```
...
SELECT CASE
    ...
    THEN NULL
    ELSE rufvector_gcn_forward(embeddings, src, dst, NULL, 64)
END;
...
SELECT rufvector_graphsage_forward(..., 10);  -- Limit neighbors
```
