...
RufVector-Postgres is designed with first-class support for Neon's serverless PostgreSQL platform. This guide covers deployment, configuration, and optimization for Neon environments.
...
| Neon Feature | RufVector Support | Notes |
|--------------|------------------|-------|
...
Neon compute nodes are ephemeral and may be replaced at any time. RufVector-Postgres handles this by:
...
Critical for scale-to-zero. RufVector-Postgres achieves sub-100ms initialization:
...
Neon compute instances have memory limits based on compute units (CU). RufVector-Postgres is memory-conscious:
...
SET rufvector.max_index_memory = '256MB';
...
Neon restricts background workers for resource management. RufVector-Postgres is designed without them:

```rust
// ❌ NOT USED: Background workers
// BackgroundWorker::register("rufvector_maintenance", ...);
...
Neon uses PgBouncer in **transaction mode** for connection pooling. RufVector-Postgres is fully compatible:
...
BEGIN;
SET LOCAL rufvector.ef_search = 100;  -- Transaction-local setting
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;

-- Next transaction (potentially different connection)
BEGIN;
SET LOCAL rufvector.ef_search = 200;  -- Different setting
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;
```
...
- RufVector requires custom extension approval
...
Body:
I would like to install the RufVector-Postgres extension for vector similarity search.

Details:
- Extension: rufvector-postgres
- Version: 0.1.19
...
Repository: https://github.com/ruvnet/rufvector
Documentation: https://github.com/ruvnet/rufvector/tree/main/crates/rufvector-postgres
...
1. **Shared Library** (`.so` file):
   ...
   # Artifact: target/release/rufvector-pg16/usr/lib/postgresql/16/lib/rufvector.so
   ```

2. **Control File** (`rufvector.control`):
   ```
   comment = 'High-performance vector similarity search'
   default_version = '0.1.19'
   module_pathname = '$libdir/rufvector'
   relocatable = true
   ```

3. **SQL Scripts**:
   - `rufvector--0.1.0.sql` (initial schema)
   - `rufvector--0.1.0--0.1.19.sql` (migration script)
...
-- RufVector provides migration path
-- (See MIGRATION.md)
```

## Migration from pgvector

RufVector-Postgres is API-compatible with pgvector. Migration is seamless:
...
-- Create new table with rufvector
...
INSERT INTO items_rufvector (id, content, embedding)
SELECT id, content, embedding::rufvector FROM items;
```
...
CREATE INDEX items_embedding_ruhnsw_idx ON items_rufvector
USING ruhnsw (embedding rufvector_l2_ops)
WITH (m = 32, ef_construction = 200);

-- Analyze for query planner
ANALYZE items_rufvector;
```
...
rufvector_results AS (
    SELECT id, embedding <-> '[...]'::rufvector AS dist
    FROM items_rufvector ORDER BY dist LIMIT 10
)
SELECT
    p.id AS pg_id,
    r.id AS ru_id,
...
SET rufvector.ef_search = 200;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;

-- Low latency (use for real-time queries)
SET rufvector.ef_search = 40;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;

-- Per-query tuning
SET LOCAL rufvector.ef_search = 100;
```
...
ALTER DATABASE dev_branch SET rufvector.ef_search = 20;

-- Staging branch: Balanced
ALTER DATABASE staging SET rufvector.ef_search = 100;

-- Production branch: High recall
ALTER DATABASE prod SET rufvector.ef_search = 200;
```
...
```sql
-- Index statistics
SELECT * FROM rufvector_index_stats();
...
```sql
-- Reduce index memory
SET rufvector.max_index_memory = '128MB';
...
ALTER DATABASE mydb SET rufvector.ef_search = 100;
```

## Support Resources

- **Neon Documentation**: https://neon.tech/docs
- **RufVector GitHub**: https://github.com/ruvnet/rufvector
- **RufVector Issues**: https://github.com/ruvnet/rufvector/issues
