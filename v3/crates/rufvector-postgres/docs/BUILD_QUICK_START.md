...
```bash
# Build Docker image
docker build -t rufvector-postgres:latest \
  -f crates/rufvector-postgres/Dockerfile .
...
# Test extension
docker exec -it <container> psql -U postgres -c "CREATE EXTENSION rufvector;"
```
...
```
target/release/rufvector-postgres-pg16/
├── usr/
