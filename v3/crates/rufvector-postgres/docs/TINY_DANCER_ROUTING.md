...
The Tiny Dancer Routing module is a neural-powered dynamic agent routing system for the rufvector-postgres PostgreSQL extension. It intelligently routes AI requests to the best available agent based on cost, latency, quality, and capability requirements.
...
```sql
-- Register agent
rufvector_register_agent(name, type, capabilities, cost, latency, quality)

-- Register with full config
rufvector_register_agent_full(config_jsonb)

-- Update metrics
rufvector_update_agent_metrics(name, latency_ms, success, quality)

-- Remove agent
rufvector_remove_agent(name)

-- Set active status
rufvector_set_agent_active(name, is_active)

-- Get agent details
rufvector_get_agent(name) -> jsonb

-- List all agents
rufvector_list_agents() -> table

-- Find by capability
rufvector_find_agents_by_capability(capability, limit) -> table
```
...
rufvector_routing_stats() -> jsonb

-- Clear all agents (testing only)
rufvector_clear_agents() -> boolean
```
...
SELECT
    r.id,
    (rufvector_route(r.embedding, 'balanced', NULL))::jsonb->>'agent_name' AS agent
FROM requests r
...
cd /workspaces/rufvector/crates/rufvector-postgres
cargo test routing::
```
...
```sql
-- 1. Register agents
SELECT rufvector_register_agent('gpt-4', 'llm', ARRAY['coding'], 0.03, 500.0, 0.95);
SELECT rufvector_register_agent('gpt-3.5', 'llm', ARRAY['general'], 0.002, 150.0, 0.75);
...
For issues, questions, or contributions, see the main rufvector-postgres repository.

## License

Same as rufvector-postgres (MIT/Apache-2.0 dual license)
