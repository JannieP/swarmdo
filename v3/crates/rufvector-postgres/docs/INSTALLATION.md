# RufVector-Postgres Installation Guide

## Overview

This guide covers installation of RufVector-Postgres on various platforms including standard PostgreSQL, Neon, Supabase, and containerized environments.
...
RufVector-Postgres supports PostgreSQL 14-18:
...
**Critical:** RufVector-Postgres requires pgrx **0.12.9 or higher**.
...
git clone https://github.com/ruvnet/rufvector.git
...
# target/release/rufvector-pg16/usr/share/postgresql/16/extension/
# target/release/rufvector-pg16/usr/lib/postgresql/16/lib/
```
...
sudo cp target/release/rufvector-pg16/usr/share/postgresql/16/extension/* \
    /usr/share/postgresql/16/extension/

sudo cp target/release/rufvector-pg16/usr/lib/postgresql/16/lib/* \
    /usr/lib/postgresql/16/lib/

# Set proper permissions
sudo chmod 644 /usr/share/postgresql/16/extension/rufvector*
sudo chmod 755 /usr/lib/postgresql/16/lib/rufvector.so
...
docker pull rufvector/postgres:16

# Run container
docker run -d \
    --name rufvector-postgres \
    -e POSTGRES_PASSWORD=mysecretpassword \
    -e POSTGRES_DB=vectordb \
    -p 5432:5432 \
    -v rufvector-data:/var/lib/postgresql/data \
    rufvector/postgres:16

# Connect and enable extension
docker exec -it rufvector-postgres psql -U postgres -d vectordb
```
...
```dockerfile
# Dockerfile for RufVector-Postgres
FROM postgres:16
...
COPY . /app/rufvector
WORKDIR /app/rufvector/crates/rufvector-postgres
...
RUN apt-get remove -y build-essential git curl && \
    ...
    rm -rf /usr/local/cargo/registry /app/rufvector

# Auto-enable extension on database creation
RUN echo "CREATE EXTENSION IF NOT EXISTS rufvector;" > /docker-entrypoint-initdb.d/init-rufvector.sql
...
```bash
# Build image
docker build -t rufvector-postgres:custom .
...
docker run -d \
    ...
    rufvector-postgres:custom

# Verify installation
docker exec -it rufvector-db psql -U postgres -d vectordb -c "SELECT rufvector_version();"
```
...
services:
  postgres:
    ...
    container_name: rufvector-postgres
    environment:
...
1. **Request Installation** (Scale Plan customers):
   ...
   Subject: Custom Extension Request - RufVector-Postgres
   Details:
   - PostgreSQL version: 16 (or your version)
   - Extension: rufvector-postgres v0.1.19
   - Use case: Vector similarity search
   ```

2. **Provide Artifacts**:
   - Pre-built `.so` files
   - Control file (`rufvector.control`)
   - SQL scripts (`rufvector--0.1.0.sql`)
...
-- Migrate to EC2 with RufVector when needed
...
```ini
# RufVector settings
shared_preload_libraries = 'rufvector'  # Optional, for background workers
...
SET rufvector.ef_search = 100;          -- Default: 40, Range: 1-1000

-- IVFFlat probes (higher = better recall, slower)
SET rufvector.probes = 10;              -- Default: 1, Range: 1-10000

-- Set globally in postgresql.conf:
ALTER SYSTEM SET rufvector.ef_search = 100;
ALTER SYSTEM SET rufvector.probes = 10;
...
BEGIN;
SET LOCAL rufvector.ef_search = 200;
SET LOCAL rufvector.probes = 20;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;

-- For low-latency queries
BEGIN;
SET LOCAL rufvector.ef_search = 20;
SET LOCAL rufvector.probes = 1;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;
```
...
SELECT * FROM pg_extension WHERE extname = 'rufvector';
-- Expected: extname=rufvector, extversion=0.1.19
...
SELECT id, embedding <-> '[1, 1, 1]'::rufvector AS distance
FROM test_vectors
ORDER BY distance
LIMIT 3;
...
CREATE TABLE items (
    id SERIAL PRIMARY KEY,
    embedding rufvector(128)
);

-- Insert sample data (10,000 vectors)
INSERT INTO items (embedding)
SELECT ('[' || array_to_string(array_agg(random()), ',') || ']')::rufvector
...
# Check library path
pg_config --pkglibdir
ls -la $(pg_config --pkglibdir)/rufvector*

# Expected output:
# -rwxr-xr-x ... rufvector.so

# Check extension path
pg_config --sharedir
ls -la $(pg_config --sharedir)/extension/rufvector*

# Expected output:
# -rw-r--r-- ... rufvector.control
# -rw-r--r-- ... rufvector--0.1.0.sql
...
```bash
sudo chmod 755 $(pg_config --pkglibdir)/rufvector.so
sudo chmod 644 $(pg_config --sharedir)/extension/rufvector*
sudo systemctl restart postgresql
```
...
-- Upgrade extension
ALTER EXTENSION rufvector UPDATE TO '0.1.20';
...
# Upgrade in database
psql -U postgres -d your_database -c "ALTER EXTENSION rufvector UPDATE;"
```
...
sudo rm $(pg_config --pkglibdir)/rufvector.so
sudo rm $(pg_config --sharedir)/extension/rufvector*
...
- **Documentation**: https://github.com/ruvnet/rufvector/tree/main/crates/rufvector-postgres/docs
- **Issues**: https://github.com/ruvnet/rufvector/issues
- **Discussions**: https://github.com/ruvnet/rufvector/discussions
