...
SELECT rufvector_register_agent(
    'gpt-4',                    -- name
    'llm',                      -- type
...
```sql
-- Simple registration
SELECT rufvector_register_agent(name, type, capabilities, cost, latency, quality);

-- Full configuration
SELECT rufvector_register_agent_full('{
    "name": "claude-3",
    "agent_type": "llm",
...
```sql
-- Cost-optimized
SELECT rufvector_route(emb, 'cost', NULL);

-- Quality-optimized
SELECT rufvector_route(emb, 'quality', NULL);

-- Latency-optimized
SELECT rufvector_route(emb, 'latency', NULL);

-- Balanced (default)
SELECT rufvector_route(emb, 'balanced', NULL);
```
...
```sql
-- Max cost
SELECT rufvector_route(emb, 'quality', '{"max_cost": 0.01}'::jsonb);

-- Max latency
SELECT rufvector_route(emb, 'balanced', '{"max_latency_ms": 500}'::jsonb);

-- Min quality
SELECT rufvector_route(emb, 'cost', '{"min_quality": 0.8}'::jsonb);

-- Required capability
SELECT rufvector_route(emb, 'balanced',
...
```sql
-- List all
SELECT * FROM rufvector_list_agents();

-- Get specific agent
SELECT rufvector_get_agent('gpt-4');

-- Find by capability
SELECT * FROM rufvector_find_agents_by_capability('coding', 5);

-- Update metrics
SELECT rufvector_update_agent_metrics('gpt-4', 450.0, true, 0.92);

-- Deactivate
SELECT rufvector_set_agent_active('gpt-4', false);

-- Remove
SELECT rufvector_remove_agent('old-agent');

-- Statistics
SELECT rufvector_routing_stats();
```
...
```sql
-- Get agent name
SELECT (rufvector_route(emb, 'balanced', NULL))::jsonb->>'agent_name';

-- Get cost
SELECT (rufvector_route(emb, 'cost', NULL))::jsonb->>'estimated_cost';
...
FROM (
    SELECT rufvector_route(emb, 'balanced', NULL) AS route
...
SELECT
    id,
    (rufvector_route(embedding, 'cost', '{"max_cost": 0.01}'::jsonb))::jsonb->>'agent_name' AS agent
FROM requests
...
SELECT * FROM rufvector_list_agents() WHERE is_active = true;

-- Relax constraints
SELECT rufvector_route(emb, 'balanced', '{"max_cost": 1.0}'::jsonb);
```
...
```sql
-- Check reasoning
SELECT (rufvector_route(emb, 'balanced', NULL))::jsonb->>'reasoning';

-- View alternatives
SELECT (rufvector_route(emb, 'balanced', NULL))::jsonb->'alternatives';
```
...
```sql
-- Verify registration
SELECT rufvector_get_agent('agent-name');

-- Check active status
SELECT is_active FROM rufvector_list_agents() WHERE name = 'agent-name';

-- Reactivate
SELECT rufvector_set_agent_active('agent-name', true);
```
...
1. **Always set constraints in production**
   ```sql
   SELECT rufvector_route(emb, 'balanced', '{"max_cost": 0.1}'::jsonb);
   ```

2. **Update metrics after each request**
   ```sql
   SELECT rufvector_update_agent_metrics(agent, latency, success, quality);
   ```
...
4. **Use capability filters**
   ```sql
   SELECT rufvector_route(emb, 'quality',
       '{"required_capabilities": ["coding"]}'::jsonb);
   ```
...
```sql
SELECT rufvector_route(emb, 'cost', '{"max_cost": 0.005}'::jsonb);
```

### Real-Time Chat (Latency-Optimized)
```sql
SELECT rufvector_route(emb, 'latency', '{"max_latency_ms": 200}'::jsonb);
```

### Critical Analysis (Quality-Optimized)
```sql
SELECT rufvector_route(emb, 'quality', '{"min_quality": 0.95}'::jsonb);
```
...
-- List active agents
SELECT name, capabilities FROM rufvector_list_agents() WHERE is_active;

-- Test simple route
SELECT rufvector_route(ARRAY[0.1]::float4[] || ARRAY(SELECT 0::float4 FROM generate_series(1,383)), 'balanced', NULL);

-- View agent details
SELECT jsonb_pretty(rufvector_get_agent('gpt-4'));

-- Clear and restart (testing only)
-- SELECT rufvector_clear_agents();
```
