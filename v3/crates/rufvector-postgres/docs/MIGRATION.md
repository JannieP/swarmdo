# Migration Guide from pgvector to RufVector-Postgres

## Overview

This guide provides step-by-step instructions for migrating from pgvector to RufVector-Postgres. RufVector-Postgres is designed as a **drop-in replacement** for pgvector with 100% SQL API compatibility and significant performance improvements.

## Key Benefits of Migration

| Feature | pgvector 0.8.0 | RufVector-Postgres | Improvement |
|---------|---------------|-------------------|-------------|
...
1. Install RufVector-Postgres alongside pgvector
2. Create parallel tables with RufVector types
3. Dual-write to both tables during transition
4. Validate RufVector results match pgvector
5. Switch reads to RufVector tables
...
1. Create complete RufVector environment (green)
2. Replicate data from pgvector (blue) to RufVector
...
### Step 1: Install RufVector-Postgres

See [INSTALLATION.md](./INSTALLATION.md) for detailed instructions.

```bash
# Install RufVector-Postgres extension
cd rufvector/crates/rufvector-postgres
cargo pgrx package --pg-config $(which pg_config)
sudo cp target/release/rufvector-pg16/usr/lib/postgresql/16/lib/* /usr/lib/postgresql/16/lib/
sudo cp target/release/rufvector-pg16/usr/share/postgresql/16/extension/* /usr/share/postgresql/16/extension/
...
SELECT rufvector_version();
-- Expected: 0.1.19

-- pgvector can coexist (for parallel deployment)
SELECT extname, extversion FROM pg_extension WHERE extname IN ('vector', 'rufvector');
```
...
| pgvector Type | RufVector Type | Notes |
|---------------|---------------|-------|
| `vector(n)` | `rufvector(n)` | Direct replacement |
...
-- Create RufVector table
CREATE TABLE items_rufvector (
    id INT PRIMARY KEY,
    content TEXT,
    metadata JSONB,
...
INSERT INTO items_rufvector (id, content, metadata, embedding, created_at)
SELECT id, content, metadata, embedding::rufvector, created_at
FROM items;

-- Verify row counts match
SELECT
    (SELECT COUNT(*) FROM items) AS pgvector_count,
    (SELECT COUNT(*) FROM items_rufvector) AS rufvector_count;
```
...
-- Create new table with rufvector type
CREATE TABLE items (
    id INT PRIMARY KEY,
    content TEXT,
    metadata JSONB,
...
| pgvector Index | RufVector Index | Notes |
|----------------|----------------|-------|
...
-- RufVector HNSW index (compatible parameters)
CREATE INDEX items_embedding_idx ON items_rufvector
USING ruhnsw (embedding rufvector_l2_ops)
WITH (m = 16, ef_construction = 64);
...
-- RufVector IVFFlat index
CREATE INDEX items_embedding_idx ON items_rufvector
USING ruivfflat (embedding rufvector_l2_ops)
WITH (lists = 100);
...
| pgvector | RufVector | Description |
|----------|----------|-------------|
...
-- RufVector query (identical syntax)
SELECT * FROM items_rufvector
ORDER BY embedding <-> '[0.1, 0.2, ...]'::rufvector
LIMIT 10;
```
...
```sql
-- Compare results between pgvector and RufVector
...
rufvector_results AS (
    SELECT id, embedding <-> '[...]'::rufvector AS distance
    FROM items_rufvector
    ORDER BY distance
    LIMIT 100
)
SELECT
...
FULL OUTER JOIN rufvector_results r ON p.id = r.id
...
-- Benchmark RufVector
\timing on
SELECT COUNT(*) FROM items_rufvector WHERE embedding <-> '[...]'::rufvector < 0.5;
-- Compare with pgvector baseline

EXPLAIN ANALYZE
SELECT * FROM items_rufvector
ORDER BY embedding <-> '[...]'::rufvector
LIMIT 10;
...
SELECT
    (SELECT COUNT(*) FROM items) AS pgvector_count,
    (SELECT COUNT(*) FROM items_rufvector) AS rufvector_count,
    (SELECT COUNT(*) FROM items) = (SELECT COUNT(*) FROM items_rufvector) AS counts_match;

-- Check for NULL vectors
SELECT COUNT(*) FROM items_rufvector WHERE embedding IS NULL;
...
// RufVector code (identical)
const result = await client.query(
    'SELECT * FROM items_rufvector ORDER BY embedding <-> $1 LIMIT 10',
    [queryVector]
);
```
...
-- (Update application to write only to items_rufvector)

-- Step 2: Sync any final changes (if dual-writing was used)
INSERT INTO items_rufvector (id, content, metadata, embedding, created_at)
SELECT id, content, metadata, embedding::rufvector, created_at
FROM items
WHERE id NOT IN (SELECT id FROM items_rufvector)
ON CONFLICT (id) DO NOTHING;

-- Step 3: Switch reads to RufVector table
-- (Update application queries from 'items' to 'items_rufvector')

-- Step 4: Rename tables for seamless transition
BEGIN;
ALTER TABLE items RENAME TO items_pgvector_old;
ALTER TABLE items_rufvector RENAME TO items;
COMMIT;
...
ALTER SYSTEM SET rufvector.ef_search = 100;  -- Higher = better recall
ALTER SYSTEM SET rufvector.probes = 10;      -- For IVFFlat indexes
SELECT pg_reload_conf();

-- Or set per-session
SET rufvector.ef_search = 200;  -- For high-recall queries
SET rufvector.ef_search = 40;   -- For low-latency queries
```
...
SELECT * FROM rufvector_index_stats('items_embedding_idx');
...
```
ERROR: cannot cast type vector to rufvector
```
...
```sql
-- Explicit conversion
INSERT INTO items_rufvector (embedding)
SELECT embedding::text::rufvector FROM items;
...
SHOW rufvector.ef_search;
-- Try increasing: SET rufvector.ef_search = 100;
```
...
WITH comparison AS (
    ...
    FROM pgvector_results p
    JOIN rufvector_results r ON p.id = r.id
)
SELECT
...
BEGIN;
ALTER TABLE items RENAME TO items_rufvector;
ALTER TABLE items_pgvector_old RENAME TO items;
COMMIT;

-- Drop RufVector extension (optional)
DROP EXTENSION rufvector CASCADE;
```
...
- [ ] Team trained on RufVector-specific features

## Schema Compatibility Notes

### Compatible SQL Functions

| pgvector | RufVector | Compatible |
|----------|----------|------------|
| `vector_dims(v)` | `rufvector_dims(v)` | ✓ |
| `vector_norm(v)` | `rufvector_norm(v)` | ✓ |
| `l2_distance(a, b)` | `rufvector_l2_distance(a, b)` | ✓ |
| `cosine_distance(a, b)` | `rufvector_cosine_distance(a, b)` | ✓ |
| `inner_product(a, b)` | `rufvector_ip_distance(a, b)` | ✓ |

### New Features in RufVector
...
SELECT rufvector_index_maintenance('items_embedding_idx');

-- Memory statistics
SELECT * FROM rufvector_memory_stats();
```
...
- **GitHub Issues**: https://github.com/ruvnet/rufvector/issues
- **Community Forum**: https://github.com/ruvnet/rufvector/discussions
...
- [ ] Install RufVector extension

## Migration
- [ ] Create RufVector tables
