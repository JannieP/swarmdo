# RufVector-Postgres Architecture

## Overview

RufVector-Postgres is a high-performance, drop-in replacement for the pgvector extension, built in Rust using the pgrx framework. It provides SIMD-optimized vector similarity search with advanced indexing algorithms, quantization support, and hybrid search capabilities.
...
5. **Production Ready**: Battle-tested algorithms from rufvector-core
...
#### `rufvector` - Primary Vector Type

**Varlena Memory Layout (Zero-Copy Design)**

```
┌─────────────────────────────────────────────────────────────────┐
│                    RufVector Varlena Layout                       │
├─────────────────────────────────────────────────────────────────┤
...
pub unsafe fn as_ptr(&self) -> *const f32 {
    // Skip varlena header (4 bytes) + RufVectorHeader (4 bytes)
    let base = self as *const _ as *const u8;
    base.add(VARHDRSZ + RufVectorHeader::SIZE) as *const f32
}
...
CREATE TABLE items (
    ...
    embedding rufvector(1536)  -- OpenAI embedding dimensions
);
...
- 50% memory savings vs rufvector
...
PostgreSQL automatically TOASTs values > ~2KB. RufVector handles this transparently:
...
- RufVector: ~512+ dimensions (2048+ bytes)
...
| Feature | pgvector 0.8.0 | RufVector-Postgres |
|---------|---------------|-------------------|
...
RufVector-Postgres is fully thread-safe:
...
SET rufvector.ef_search = 200;  -- Higher = better recall, slower
SET rufvector.probes = 10;      -- For IVFFlat: more probes = better recall
...
```
crates/rufvector-postgres/
├── Cargo.toml                    # Rust dependencies
├── rufvector.control              # Extension metadata
├── docs/
...
├── sql/
│   ├── rufvector--0.1.0.sql       # Extension SQL definitions
│   └── rufvector--0.0.0--0.1.0.sql # Migration script
├── src/
│   ├── lib.rs                    # Extension entry point
│   ├── types/
│   │   ├── mod.rs
│   │   ├── vector.rs             # rufvector type (zero-copy varlena)
...
MIT License - Same as rufvector-core
