...
-- Get node
SELECT rufvector_get_node('graph_name', 1);

-- Find by label
SELECT rufvector_find_nodes_by_label('graph_name', 'Person');
```
...
-- Get edge
SELECT rufvector_get_edge('graph_name', 1);

-- Get neighbors
SELECT rufvector_get_neighbors('graph_name', 1);
```
...
-- Create friendship
SELECT rufvector_add_edge('social', 1, 2, 'FRIENDS',
    '{"since": "2020-01-15"}'::jsonb);

-- Find path
SELECT rufvector_shortest_path('social', 1, 2, 10);
```
...
SELECT rufvector_cypher('knowledge',
...
DO $$
BEGIN
    IF 'my_graph' = ANY(rufvector_list_graphs()) THEN
        -- Perform operations
        RAISE NOTICE 'Graph exists';
    ELSE
        PERFORM rufvector_create_graph('my_graph');
    END IF;
END $$;
...
DECLARE
    result jsonb;
BEGIN
    result := rufvector_get_node('my_graph', 999);
