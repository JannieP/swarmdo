...
```sql
rufvector_attention_score(query, key, type) → float4
```
...
```sql
rufvector_softmax(scores) → float4[]
```
...
```sql
rufvector_multi_head_attention(query, keys, values, num_heads) → float4[]
```
...
```sql
rufvector_flash_attention(query, keys, values, block_size) → float4[]
```
...
```sql
rufvector_attention_scores(query, keys, type) → float4[]
```
...
```sql
rufvector_attention_types() → TABLE(name, complexity, best_for)
```
...
```rust
use rufvector_postgres::attention::ScaledDotAttention;
...
```rust
use rufvector_postgres::attention::MultiHeadAttention;
...
```rust
use rufvector_postgres::attention::FlashAttention;
...
SELECT
    id,
    rufvector_attention_score(query_vector, embedding, 'scaled_dot') AS score
FROM candidates
ORDER BY score DESC
LIMIT 10;
```
...
SELECT
    q.id AS query_id,
    d.id AS doc_id,
    rufvector_attention_score(q.embedding, d.embedding, 'scaled_dot') AS score
FROM queries q
...
WITH stage1 AS (
    SELECT id, embedding,
           rufvector_attention_score(query, embedding, 'scaled_dot') AS score
    FROM documents
    WHERE score > 0.5
    LIMIT 50
)
...
SELECT id,
       rufvector_multi_head_attention(
           query,
           ARRAY_AGG(embedding),
           ARRAY_AGG(embedding),
           8
...
SELECT
    query_id,
    doc_id,
    rufvector_attention_score(q_vec, d_vec, 'scaled_dot') AS score
FROM queries
