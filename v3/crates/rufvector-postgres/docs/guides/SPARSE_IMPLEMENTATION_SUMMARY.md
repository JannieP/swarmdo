...
Complete implementation of sparse vector support for rufvector-postgres PostgreSQL extension, providing efficient storage and operations for high-dimensional sparse embeddings.
...
**Distance Operations**:
- `rufvector_sparse_dot(a, b) -> f32`
- `rufvector_sparse_cosine(a, b) -> f32`
- `rufvector_sparse_euclidean(a, b) -> f32`
- `rufvector_sparse_manhattan(a, b) -> f32`

**Construction Functions**:
- `rufvector_to_sparse(indices, values, dim) -> sparsevec`
- `rufvector_dense_to_sparse(dense) -> sparsevec`
- `rufvector_sparse_to_dense(sparse) -> real[]`

**Utility Functions**:
- `rufvector_sparse_nnz(sparse) -> int` - Number of non-zeros
- `rufvector_sparse_dim(sparse) -> int` - Dimension
- `rufvector_sparse_norm(sparse) -> real` - L2 norm

**Sparsification Functions**:
- `rufvector_sparse_top_k(sparse, k) -> sparsevec`
- `rufvector_sparse_prune(sparse, threshold) -> sparsevec`

**BM25 Function**:
- `rufvector_sparse_bm25(query, doc, doc_len, avg_len, k1, b) -> real`
...
SELECT rufvector_sparse_euclidean(a, b);

-- Utility functions
SELECT rufvector_sparse_nnz(sparse_vec);
SELECT rufvector_sparse_dim(sparse_vec);
SELECT rufvector_sparse_norm(sparse_vec);

-- Sparsification
SELECT rufvector_sparse_top_k(sparse_vec, 100);
SELECT rufvector_sparse_prune(sparse_vec, 0.1);
```
...
SELECT id, content,
       rufvector_sparse_dot(sparse_embedding, '{1:0.5, 2:0.3}'::sparsevec) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
```
...
SELECT id, content,
       rufvector_sparse_dot(splade_embedding, query_splade) AS relevance
FROM documents
...
SELECT id, content,
       ...
       0.3 * rufvector_sparse_dot(sparse_embedding, query_sparse) AS hybrid_score
FROM documents
...
CREATE INDEX ON documents USING rufvector_sparse_ivf (
...
```bash
# Standard Rust tests
cargo test --package rufvector-postgres --lib sparse
...
# In PostgreSQL
CREATE EXTENSION rufvector_postgres;
...
✅ **Complete implementation** of sparse vectors for rufvector-postgres
...
```
/workspaces/rufvector/crates/rufvector-postgres/
├── src/
│   └── sparse/
│       ├── mod.rs           (30 lines)
│       ├── types.rs         (391 lines)
