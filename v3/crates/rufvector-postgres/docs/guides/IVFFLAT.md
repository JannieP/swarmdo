...
WITH (lists = 50);
SET rufvector.ivfflat_probes = 5;
```
...
|-----------------------------|---------|----------------------------------|
| `rufvector.ivfflat_probes`   | 1       | Number of lists to probe         |
...
# Rust tests
cargo test --package rufvector-postgres --lib index::ivfflat_am
```
...
FROM pg_opclass
WHERE opcname LIKE 'rufvector_ivfflat%';

-- Get statistics
SELECT * FROM rufvector_ivfflat_stats('your_index_name');
```
...
SET rufvector.ivfflat_probes = 1;
