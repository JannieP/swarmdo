...
-- Verify setting
SHOW rufvector.ivfflat_probes;

-- Reset to default
RESET rufvector.ivfflat_probes;
```
...
```sql
BEGIN;
SET LOCAL rufvector.ivfflat_probes = 15;
...
ORDER BY embedding <-> '[0.1, 0.2, ...]'::vector
LIMIT 10
SETTINGS (rufvector.ivfflat_probes = 10);
```
...
```sql
-- Get index metadata
SELECT * FROM rufvector_ivfflat_stats('products_embedding_idx');
...
```sql
-- Increase probes
SET rufvector.ivfflat_probes = 15;
