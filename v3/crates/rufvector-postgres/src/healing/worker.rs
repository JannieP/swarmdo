...
pub extern "C" fn healing_bgworker_main(_arg: pgrx::pg_sys::Datum) {
    pgrx::log!("RufVector healing background worker starting");
...
pub fn register_healing_worker() {
    pgrx::log!("Registering RufVector healing background worker");
...
#[pg_extern]
pub fn rufvector_healing_worker_start() -> bool {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn rufvector_healing_worker_stop() -> bool {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn rufvector_healing_worker_status() -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn rufvector_healing_worker_config(
...
#[pg_extern]
pub fn rufvector_healing_check_now() -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
/// Get recent health check results
#[pg_extern]
pub fn rufvector_healing_recent_checks(limit: default!(i32, 10)) -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
