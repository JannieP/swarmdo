...
use rufvector_solver::forward_push::ForwardPushSolver;
use rufvector_solver::traits::{SolverEngine, SublinearPageRank};
use rufvector_solver::types::{ComputeBudget, CsrMatrix};

use super::{edges_json_to_csr, matrix_json_to_csr};

/// Compute PageRank on an edge list using Forward Push.
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_pagerank(
    edges_json: JsonB,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_pagerank_personalized(
    edges_json: JsonB,
    source: i32,
    alpha: default!(f32, 0.85),
    epsilon: default!(f32, 1e-6),
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_pagerank_multi_seed(
    edges_json: JsonB,
    seeds_json: JsonB,
    alpha: default!(f32, 0.85),
    epsilon: default!(f32, 1e-6),
...
pub fn rufvector_solve_sparse(
    ...
    let result = match method.to_lowercase().as_str() {
        "cg" | "conjugate_gradient" => {
            let solver = rufvector_solver::cg::ConjugateGradientSolver::new(1e-6, 1000, true);
            solver.solve(&csr, &rhs_f64, &budget)
        }
        _ => {
            // Default to Neumann — use trait method explicitly for f64 interface
            let solver = rufvector_solver::neumann::NeumannSolver::new(1e-6, 1000);
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_solve_laplacian(laplacian_json: JsonB, rhs: Vec<f32>) -> JsonB {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_effective_resistance(laplacian_json: JsonB, source: i32, target: i32) -> f32 {
    let csr = match matrix_json_to_csr(&laplacian_json.0) {
        Ok(m) => m,
    ...
    let solver = rufvector_solver::cg::ConjugateGradientSolver::new(1e-8, 2000, true);
...
/// Run PageRank on an existing property graph stored via rufvector graph module.
#[cfg(feature = "graph")]
#[pg_extern]
pub fn rufvector_graph_pagerank(
    graph_name: &str,
    alpha: default!(f32, 0.85),
    epsilon: default!(f32, 1e-6),
...
#[pg_extern]
pub fn rufvector_solver_info() -> TableIterator<
    'static,
    (
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_matrix_analyze(matrix_json: JsonB) -> JsonB {
    let csr = match matrix_json_to_csr(&matrix_json.0) {
        Ok(m) => m,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_conjugate_gradient(
    matrix_json: JsonB,
    rhs: Vec<f32>,
    ...
    let solver =
        rufvector_solver::cg::ConjugateGradientSolver::new(tol as f64, max_iter as usize, true);
...
#[cfg(feature = "graph")]
#[pg_extern]
pub fn rufvector_graph_centrality(
    graph_name: &str,
