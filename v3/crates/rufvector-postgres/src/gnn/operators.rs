...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_gnn_aggregate(messages_json: JsonB, method: String) -> Vec<f32> {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_graphsage_forward(
    embeddings_json: JsonB,
    src: Vec<i32>,
    dst: Vec<i32>,
    out_dim: i32,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_gnn_batch_forward(
...
mod tests {
    ...
    #[pg_test]
    fn test_rufvector_gcn_forward() {
        let embeddings = to_json(vec![vec![1.0, 2.0], vec![3.0, 4.0], vec![5.0, 6.0]]);

        let src = vec![0, 1, 2];
        let dst = vec![1, 2, 0];

        let result = rufvector_gcn_forward(embeddings, src, dst, None, 2);
    ...
    #[pg_test]
    fn test_rufvector_gnn_aggregate_sum() {
        let messages = to_json(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);

        let result = rufvector_gnn_aggregate(messages, "sum".to_string());

        assert_eq!(result, vec![4.0, 6.0]);
    }

    #[pg_test]
    fn test_rufvector_gnn_aggregate_mean() {
        let messages = to_json(vec![vec![2.0, 4.0], vec![4.0, 6.0]]);

        let result = rufvector_gnn_aggregate(messages, "mean".to_string());

        assert_eq!(result, vec![3.0, 5.0]);
    }

    #[pg_test]
    fn test_rufvector_gnn_aggregate_max() {
        let messages = to_json(vec![vec![1.0, 6.0], vec![5.0, 2.0]]);

        let result = rufvector_gnn_aggregate(messages, "max".to_string());

        assert_eq!(result, vec![5.0, 6.0]);
    }

    #[pg_test]
    fn test_rufvector_graphsage_forward() {
        let embeddings = to_json(vec![vec![1.0, 2.0], vec![3.0, 4.0], vec![5.0, 6.0]]);

        let src = vec![0, 1, 2];
        let dst = vec![1, 2, 0];

        let result = rufvector_graphsage_forward(embeddings, src, dst, 2, 2);
        let parsed = parse_result(&result);
    ...
    #[pg_test]
    fn test_rufvector_message_pass() {
        let result = rufvector_message_pass(
            "nodes".to_string(),
            "edges".to_string(),
            "embedding".to_string(),
            3,
    ...
    fn test_empty_inputs() {
        ...
        let result = rufvector_gcn_forward(empty_embeddings, empty_src, empty_dst, None, 4);
        let parsed = parse_result(&result);
    ...
    fn test_weighted_gcn() {
        ...
        let result = rufvector_gcn_forward(embeddings, src, dst, weights, 2);
        let parsed = parse_result(&result);
