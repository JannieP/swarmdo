...
/// ```
#[pg_extern]
fn rufvector_register_agent_full(config: JsonB) -> Result<bool, String> {
    let registry = get_registry();
...
/// ```
#[pg_extern]
fn rufvector_update_agent_metrics(
    name: String,
    latency_ms: f32,
    success: bool,
    quality: Option<f32>,
...
/// # Example
/// ```sql
/// SELECT rufvector_remove_agent('gpt-4');
/// ```
#[pg_extern]
fn rufvector_remove_agent(name: String) -> Result<bool, String> {
    let registry = get_registry();
    registry
        .remove(&name)
        .ok_or_else(|| format!("Agent '{}' not found", name))?;
...
/// ```
#[pg_extern]
fn rufvector_set_agent_active(name: String, is_active: bool) -> Result<bool, String> {
    let registry = get_registry();
...
/// ```
#[pg_extern]
fn rufvector_list_agents() -> TableIterator<
    'static,
    (
        name!(name, String),
...
/// ```
#[pg_extern]
fn rufvector_get_agent(name: String) -> Result<JsonB, String> {
    let registry = get_registry();
...
/// # Example
/// ```sql
/// SELECT * FROM rufvector_find_agents_by_capability('code_generation', 5);
/// ```
#[pg_extern]
fn rufvector_find_agents_by_capability(
    capability: String,
...
/// ```
#[pg_extern]
fn rufvector_routing_stats() -> JsonB {
    let registry = get_registry();
...
#[pg_extern]
fn rufvector_clear_agents() -> bool {
    let registry = get_registry();
    registry.clear();
    true
}
...
mod tests {
    ...
    fn test_register_agent() {
        rufvector_clear_agents();

        let result = rufvector_register_agent(
        ...
        let agent = rufvector_get_agent("test-agent".to_string());
        assert!(agent.is_ok());
    }
    ...
    fn test_update_agent_metrics() {
        ...
        let result =
            rufvector_update_agent_metrics("test-agent".to_string(), 150.0, true, Some(0.9));
    ...
    fn test_remove_agent() {
        ...
        let result = rufvector_remove_agent("test-agent".to_string());
        assert!(result.is_ok());
    ...
    fn test_set_agent_active() {
        ...
        let result = rufvector_set_agent_active("test-agent".to_string(), false);
        assert!(result.is_ok());

        let agent_json = rufvector_get_agent("test-agent".to_string()).unwrap();
    ...
    fn test_list_agents() {
        ...
        let agents: Vec<_> = rufvector_list_agents().collect();
    ...
    fn test_find_agents_by_capability() {
        ...
        let coders: Vec<_> = rufvector_find_agents_by_capability("coding".to_string(), 10).collect();
    ...
    fn test_routing_stats() {
        ...
        let stats = rufvector_routing_stats();
    ...
    fn test_route_basic() {
        ...
        let result = rufvector_route(embedding.clone(), "cost".to_string(), None);
        assert!(result.is_ok());
