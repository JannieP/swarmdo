...
SELECT rufvector_register_agent('gpt-4', 'llm',
    ARRAY['coding', 'reasoning', 'math'], 0.03, 500.0, 0.95);
SELECT rufvector_register_agent('gpt-3.5-turbo', 'llm',
    ARRAY['general', 'fast'], 0.002, 200.0, 0.75);
SELECT rufvector_register_agent('claude-3-opus', 'llm',
    ARRAY['coding', 'writing', 'analysis'], 0.025, 400.0, 0.93);
SELECT rufvector_register_agent('llama-2-70b', 'llm',
...
BEGIN
    PERFORM rufvector_update_agent_metrics(
        agent_name,
        actual_latency_ms,
        success,
        quality_score
...
SELECT
    CASE
        ...
        ELSE (rufvector_route(embedding, 'balanced', NULL))::jsonb->>'agent_name'
...
4. **Update Regularly**: Use `rufvector_update_agent_metrics` after each request
