//! Tenant Isolation Enforcement for RufVector Multi-Tenancy
//!
...
DROP POLICY IF EXISTS rufvector_admin_bypass ON {table};

-- Create tenant isolation policy
CREATE POLICY rufvector_tenant_isolation ON {table}
    USING ({column} = current_setting('rufvector.tenant_id', true))
    WITH CHECK ({column} = current_setting('rufvector.tenant_id', true));

-- Create admin bypass policy (for rufvector_admin role)
CREATE POLICY rufvector_admin_bypass ON {table}
    FOR ALL
    TO rufvector_admin
    USING (true)
    WITH CHECK (true);
...
CREATE POLICY rufvector_admin_wildcard ON {table}
    FOR SELECT
...
GRANT USAGE ON SCHEMA {schema} TO rufvector_users;
GRANT ALL ON ALL TABLES IN SCHEMA {schema} TO rufvector_users;
GRANT ALL ON ALL SEQUENCES IN SCHEMA {schema} TO rufvector_users;
"#,
