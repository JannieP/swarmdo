//! Row-Level Security Integration for RufVector Multi-Tenancy
//!
...
impl Default for RlsPolicyConfig {
    fn default() -> Self {
        Self {
            ...
            policy_name: "rufvector_tenant_isolation".to_string(),
            admin_bypass: true,
    ...
    pub fn using_clause(&self, tenant_column: &str) -> String {
        match self {
            ...
            Self::ReadSharedWriteOwn => {
                format!(
                    "{} = current_setting('rufvector.tenant_id', true) OR is_public = true",
                    tenant_column
                )
            }
            Self::Hierarchical { path_column } => {
                format!(
                    "{} LIKE current_setting('rufvector.tenant_id', true) || '%'",
                    path_column
                )
            }
            Self::TimeBased {
                ...
                format!(
                    "{} = current_setting('rufvector.tenant_id', true) AND {} > NOW() - INTERVAL '{} days'",
...
-- Drop existing RufVector policies if any
...
CREATE POLICY {policy}_wildcard ON {table}
    FOR SELECT
    USING (current_setting('rufvector.tenant_id', true) = '*');
"#,
                table = config.table_name,
                policy = config.policy_name
            ));
...
-- Drop all RufVector policies
DROP POLICY IF EXISTS rufvector_tenant_isolation ON {table};
DROP POLICY IF EXISTS rufvector_tenant_isolation_admin ON {table};
DROP POLICY IF EXISTS rufvector_tenant_isolation_wildcard ON {table};
"#,
            table = table_name
        )
    }
    ...
    pub fn generate_add_tenant_column_sql(
        ...
        if default_current {
            sql.push_str(" DEFAULT current_setting('rufvector.tenant_id')");
        }
...
--     FOREIGN KEY ({}) REFERENCES rufvector.tenants(id) ON DELETE CASCADE;
...
"#,
    ...
    pub fn generate_roles_sql() -> String {
        r#"
-- Create RufVector roles for RLS
DO $$
BEGIN
    -- Admin role (bypasses RLS)
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'rufvector_admin') THEN
        CREATE ROLE rufvector_admin;
    END IF;

    -- Standard user role (subject to RLS)
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'rufvector_users') THEN
        CREATE ROLE rufvector_users;
    END IF;

    -- Read-only role
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'rufvector_readonly') THEN
        CREATE ROLE rufvector_readonly;
    END IF;
END
$$;

-- Grant basic permissions
GRANT USAGE ON SCHEMA public TO rufvector_users, rufvector_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO rufvector_readonly;
GRANT ALL ON ALL TABLES IN SCHEMA public TO rufvector_users;
"#
        .to_string()
    }
...
CREATE OR REPLACE FUNCTION rufvector_validate_tenant_context_{table_safe}()
RETURNS TRIGGER AS $$
DECLARE
    v_tenant_id TEXT;
BEGIN
    -- Get current tenant context
    v_tenant_id := current_setting('rufvector.tenant_id', true);

    -- Validate context is set
    IF v_tenant_id IS NULL OR v_tenant_id = '' THEN
        RAISE EXCEPTION 'No tenant context set. Use SET rufvector.tenant_id = ''your-tenant-id''';
    END IF;
...
-- Create trigger
DROP TRIGGER IF EXISTS trg_rufvector_validate_tenant_{table_safe} ON {table};
CREATE TRIGGER trg_rufvector_validate_tenant_{table_safe}
    BEFORE INSERT OR UPDATE ON {table}
    FOR EACH ROW
    EXECUTE FUNCTION rufvector_validate_tenant_context_{table_safe}();
"#,
            table = table_name,
...
CREATE OR REPLACE FUNCTION rufvector_check_tenant_exists_{table_safe}()
RETURNS TRIGGER AS $$
BEGIN
    -- Check tenant exists (skip for admin wildcard)
    IF NEW.{column} != '*' THEN
        IF NOT EXISTS (SELECT 1 FROM rufvector.tenants WHERE id = NEW.{column}) THEN
            RAISE EXCEPTION 'Tenant does not exist: %', NEW.{column};
        END IF;

        -- Check tenant is not suspended
        IF EXISTS (SELECT 1 FROM rufvector.tenants WHERE id = NEW.{column} AND suspended_at IS NOT NULL) THEN
...
DROP TRIGGER IF EXISTS trg_rufvector_check_tenant_{table_safe} ON {table};
CREATE TRIGGER trg_rufvector_check_tenant_{table_safe}
    BEFORE INSERT OR UPDATE ON {table}
    FOR EACH ROW
    EXECUTE FUNCTION rufvector_check_tenant_exists_{table_safe}();
"#,
            table = table_name,
            table_safe = table_name.replace('.', "_").replace('"', ""),
            column = tenant_column
...
mod tests {
    ...
    fn test_generate_enable_rls_sql() {
        ...
        assert!(sql.contains("rufvector.tenant_id"));
        assert!(sql.contains("rufvector_admin")); // Admin bypass
    ...
    fn test_set_tenant_sql() {
        ...
        assert!(sql.contains("SET rufvector.tenant_id"));
    ...
    fn test_roles_sql() {
        ...
        assert!(sql.contains("rufvector_readonly"));
    }
