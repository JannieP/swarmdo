//! Multi-Tenancy Module for RufVector PostgreSQL Extension
//!
...
//! ```sql
//! -- Create a tenant
//! SELECT rufvector_tenant_create('acme-corp', '{
...
//! SET rufvector.tenant_id = 'acme-corp';
//!
...
/// Note: rufvector.tenant_id is registered as a custom GUC that can be set
/// using SET rufvector.tenant_id = 'tenant-name' and read using
/// current_setting('rufvector.tenant_id', true)
pub fn init_tenant_gucs() {
    ...
    //   SET rufvector.tenant_id = 'my-tenant';
    //   SELECT current_setting('rufvector.tenant_id', true);
    //
    ...
    // current_setting('rufvector.tenant_id', true) directly.

    pgrx::log!("RufVector multi-tenancy initialized");
}
...
/// ```
#[pg_extern]
pub fn rufvector_tenant_create(
    tenant_id: &str,
    config: Option<JsonB>,
...
///
/// ```sql
/// SELECT rufvector_tenant_set('acme-corp');
/// -- All subsequent operations are now scoped to acme-corp
/// ```
#[pg_extern]
pub fn rufvector_tenant_set(
    tenant_id: &str,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_stats('acme-corp');
/// ```
#[pg_extern]
pub fn rufvector_tenant_stats(
    tenant_id: &str,
...
///
/// ```sql
/// SELECT rufvector_tenant_quota_check('acme-corp');
/// ```
#[pg_extern]
pub fn rufvector_tenant_quota_check(
    tenant_id: &str,
) -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_suspend('bad-actor');
/// ```
#[pg_extern]
pub fn rufvector_tenant_suspend(
    tenant_id: &str,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_resume('bad-actor');
/// ```
#[pg_extern]
pub fn rufvector_tenant_resume(
    tenant_id: &str,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
/// SELECT rufvector_tenant_delete('churned-customer');
///
/// -- Hard delete (immediate)
/// SELECT rufvector_tenant_delete('churned-customer', true);
/// ```
#[pg_extern]
pub fn rufvector_tenant_delete(
    tenant_id: &str,
...
///
/// ```sql
/// SELECT * FROM rufvector_tenants();
/// ```
#[pg_extern]
pub fn rufvector_tenants() -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_enable_tenant_rls('embeddings', 'tenant_id');
/// ```
#[pg_extern]
pub fn rufvector_enable_tenant_rls(
    table_name: &str,
...
///
/// ```sql
/// SELECT rufvector_tenant_migrate('enterprise-customer', 'dedicated');
/// ```
#[pg_extern]
pub fn rufvector_tenant_migrate(
    tenant_id: &str,
    target_level: &str,
) -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT * FROM rufvector_tenant_migration_status('enterprise-customer');
/// ```
#[pg_extern]
pub fn rufvector_tenant_migration_status(
    tenant_id: &str,
) -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_isolate('enterprise-customer');
/// ```
#[pg_extern]
pub fn rufvector_tenant_isolate(
    tenant_id: &str,
) -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_set_policy('{
...
/// ```
#[pg_extern]
pub fn rufvector_tenant_set_policy(
    policy_config: JsonB,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_tenant_update_quota('acme-corp', '{
///     "max_vectors": 10000000,
///     "max_qps": 500
/// }'::jsonb);
/// ```
#[pg_extern]
pub fn rufvector_tenant_update_quota(
    tenant_id: &str,
    quota_config: JsonB,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
///
/// ```sql
/// SELECT rufvector_generate_rls_sql('embeddings', 'tenant_id');
/// ```
#[pg_extern]
pub fn rufvector_generate_rls_sql(
    table_name: &str,
    tenant_column: default!(&str, "'tenant_id'"),
) -> String {
...
///
/// ```sql
/// SELECT rufvector_generate_tenant_column_sql('embeddings');
/// ```
#[pg_extern]
pub fn rufvector_generate_tenant_column_sql(
    table_name: &str,
...
///
/// ```sql
/// SELECT rufvector_generate_roles_sql();
/// ```
#[pg_extern]
pub fn rufvector_generate_roles_sql() -> String {
...
mod tests {
    ...
    fn test_tenant_create() {
        let result = rufvector_tenant_create("test-tenant", None);
        assert!(result.is_ok());
    ...
    fn test_tenant_create_with_config() {
        ...
        let result = rufvector_tenant_create("test-tenant-2", Some(config));
        assert!(result.is_ok());
    }
    ...
    fn test_tenant_list() {
        ...
        let _ = rufvector_tenant_create("list-test-tenant", None);

        let result = rufvector_tenants();
        assert!(result.is_ok());
    ...
    fn test_tenant_suspend_resume() {
        let _ = rufvector_tenant_create("suspend-test", None);

        // Suspend
        let result = rufvector_tenant_suspend("suspend-test");
        assert!(result.is_ok());

        // Resume
        let result = rufvector_tenant_resume("suspend-test");
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_rls_sql_generation() {
        let sql = rufvector_generate_rls_sql("embeddings", "tenant_id");
        assert!(sql.contains("ENABLE ROW LEVEL SECURITY"));
        assert!(sql.contains("rufvector_tenant_isolation"));
    }

    #[pg_test]
    fn test_tenant_column_sql_generation() {
        let sql = rufvector_generate_tenant_column_sql("embeddings", "tenant_id", true, true);
    ...
    fn test_roles_sql_generation() {
        let sql = rufvector_generate_roles_sql();
        assert!(sql.contains("rufvector_admin"));
        assert!(sql.contains("rufvector_users"));
    }
    ...
    fn test_policy_update() {
        ...
        let result = rufvector_tenant_set_policy(policy);
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_quota_check() {
        let _ = rufvector_tenant_create("quota-test", None);

        let result = rufvector_tenant_quota_check("quota-test");
        assert!(result.is_ok());
