// Graph operations module for rufvector-postgres
//
...
fn ensure_graph_tables() {
    use pgrx::prelude::*;
    Spi::run(
        "CREATE TABLE IF NOT EXISTS _rufvector_graphs (
            name TEXT PRIMARY KEY
        )",
    )
    .ok();
    Spi::run(
        "CREATE TABLE IF NOT EXISTS _rufvector_nodes (
            graph_name TEXT NOT NULL REFERENCES _rufvector_graphs(name) ON DELETE CASCADE,
            id BIGINT NOT NULL,
    ...
    .ok();
    Spi::run(
        "CREATE TABLE IF NOT EXISTS _rufvector_edges (
            graph_name TEXT NOT NULL REFERENCES _rufvector_graphs(name) ON DELETE CASCADE,
            id BIGINT NOT NULL,
...
fn load_graph_from_tables(name: &str) -> Option<Arc<GraphStore>> {
    ...
    let exists = Spi::get_one_with_args::<bool>(
        "SELECT EXISTS(SELECT 1 FROM _rufvector_graphs WHERE name = $1)",
    ...
    let _ = Spi::connect(|client| {
        let tup_table = client.select(
            "SELECT id, labels, properties FROM _rufvector_nodes WHERE graph_name = $1 ORDER BY id",
            None,
    ...
    let _ = Spi::connect(|client| {
        let tup_table = client.select(
            "SELECT id, source, target, edge_type, properties FROM _rufvector_edges WHERE graph_name = $1 ORDER BY id",
            None,
            Some(vec![(PgBuiltInOids::TEXTOID.oid(), name.into_datum())]),
        )?;
...
fn persist_graph_name(name: &str) {
    use pgrx::prelude::*;
    Spi::run_with_args(
        "INSERT INTO _rufvector_graphs (name) VALUES ($1) ON CONFLICT DO NOTHING",
        Some(vec![(PgBuiltInOids::TEXTOID.oid(), name.into_datum())]),
    )
    .ok();
}
...
pub fn persist_node(graph_name: &str, node: &Node) {
    ...
    Spi::run_with_args(
        "INSERT INTO _rufvector_nodes (graph_name, id, labels, properties)
         VALUES ($1, $2, $3, $4)
...
pub fn persist_edge(graph_name: &str, edge: &Edge) {
    ...
    Spi::run_with_args(
        "INSERT INTO _rufvector_edges (graph_name, id, source, target, edge_type, properties)
         VALUES ($1, $2, $3, $4, $5, $6)
...
pub fn delete_graph(name: &str) -> bool {
    ...
    Spi::run_with_args(
        "DELETE FROM _rufvector_graphs WHERE name = $1",
        Some(vec![(PgBuiltInOids::TEXTOID.oid(), name.into_datum())]),
    )
    .ok();
    true
...
pub fn list_graphs() -> Vec<String> {
    ...
    let _ = Spi::connect(|client| {
        let tup_table = client.select(
            "SELECT name FROM _rufvector_graphs ORDER BY name",
            None,
            None,
        )?;
        for row in tup_table {
