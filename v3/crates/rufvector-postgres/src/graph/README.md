...
This module provides graph database capabilities for the rufvector-postgres extension, including graph storage, traversal algorithms, Cypher query support, and W3C-standard SPARQL for RDF data.
...
SELECT rufvector_shortest_path(
    'my_graph',
    1,    -- start_id
    10,   -- end_id
    5     -- max_hops
...
SELECT rufvector_shortest_path_weighted(
    'my_graph',
    1,    -- start_id
    10,   -- end_id
...
SELECT rufvector_create_rdf_store('my_knowledge_base');
...
SELECT rufvector_load_ntriples('my_knowledge_base', '
...
SELECT rufvector_sparql('my_knowledge_base', '
...
SELECT rufvector_sparql_json('my_knowledge_base',
    'SELECT ?s ?p ?o WHERE { ?s ?p ?o } LIMIT 10'
);

-- Query triples by pattern (NULL = wildcard)
SELECT rufvector_query_triples('my_knowledge_base',
...
SELECT rufvector_rdf_stats('my_knowledge_base');
-- Returns: {"name": "...", "triple_count": 5, "subject_count": 2, ...}

-- SPARQL UPDATE
SELECT rufvector_sparql_update('my_knowledge_base', '
    INSERT DATA {
...
-- Clear store
SELECT rufvector_clear_rdf_store('my_knowledge_base');

-- Delete store
SELECT rufvector_delete_rdf_store('my_knowledge_base');
...
-- Add users
WITH users AS (
    SELECT rufvector_add_node('social_network', ARRAY['Person'],
...
-- Create friendships
SELECT rufvector_add_edge('social_network', 1, 2, 'FRIENDS',
    '{"since": "2020-01-15"}'::jsonb);
SELECT rufvector_add_edge('social_network', 2, 3, 'FRIENDS',
    '{"since": "2019-06-20"}'::jsonb);
SELECT rufvector_add_edge('social_network', 1, 4, 'FRIENDS',
...
SELECT rufvector_create_rdf_store('dbpedia_subset');

-- Load sample data
SELECT rufvector_load_ntriples('dbpedia_subset', '
...
SELECT rufvector_sparql('dbpedia_subset', '
...
SELECT rufvector_query_triples('dbpedia_subset',
...
-- Add concepts
SELECT rufvector_add_node('knowledge', ARRAY['Concept'],
...
-- Create relationships
SELECT rufvector_add_edge('knowledge', 1, 2, 'INCLUDES',
    '{"strength": 0.9}'::jsonb);
SELECT rufvector_add_edge('knowledge', 2, 3, 'SPECIALIZES_IN',
    '{"strength": 0.95}'::jsonb);

-- Find weighted path
SELECT rufvector_shortest_path_weighted('knowledge', 1, 3, 'strength');
```
...
-- Add users and items
SELECT rufvector_cypher('recommendations',
