// SPARQL (SPARQL Protocol and RDF Query Language) module for rufvector-postgres
//
...
fn ensure_rdf_tables() {
    use pgrx::prelude::*;
    Spi::run(
        "CREATE TABLE IF NOT EXISTS _rufvector_rdf_stores (
            name TEXT PRIMARY KEY
        )",
    )
    .ok();
    Spi::run(
        "CREATE TABLE IF NOT EXISTS _rufvector_triples (
            store_name TEXT NOT NULL REFERENCES _rufvector_rdf_stores(name) ON DELETE CASCADE,
            id BIGINT NOT NULL,
...
fn load_store_from_tables(name: &str) -> Option<Arc<TripleStore>> {
    ...
    let exists = Spi::get_one_with_args::<bool>(
        "SELECT EXISTS(SELECT 1 FROM _rufvector_rdf_stores WHERE name = $1)",
    ...
    let _ = pgrx::prelude::Spi::connect(|client| {
        let tup_table = client.select(
            "SELECT id, subject, predicate, object, graph_name FROM _rufvector_triples WHERE store_name = $1 ORDER BY id",
            None,
...
pub fn persist_triple(store_name: &str, id: u64, triple: &Triple, graph: Option<&str>) {
    ...
    Spi::run_with_args(
        "INSERT INTO _rufvector_triples (store_name, id, subject, predicate, object, graph_name)
         VALUES ($1, $2, $3, $4, $5, $6)
...
pub fn get_or_create_store(name: &str) -> Arc<TripleStore> {
    ...
    use pgrx::prelude::*;
    Spi::run_with_args(
        "INSERT INTO _rufvector_rdf_stores (name) VALUES ($1) ON CONFLICT DO NOTHING",
        Some(vec![(PgBuiltInOids::TEXTOID.oid(), name.into_datum())]),
    )
    .ok();
...
pub fn delete_store(name: &str) -> bool {
    ...
    Spi::run_with_args(
        "DELETE FROM _rufvector_rdf_stores WHERE name = $1",
        Some(vec![(PgBuiltInOids::TEXTOID.oid(), name.into_datum())]),
    )
    .ok();
    true
...
pub fn list_stores() -> Vec<String> {
    ...
    let _ = Spi::connect(|client| {
        let tup_table = client.select(
            "SELECT name FROM _rufvector_rdf_stores ORDER BY name",
            None,
            None,
        )?;
        for row in tup_table {
