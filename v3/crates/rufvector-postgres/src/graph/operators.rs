...
/// # Example
/// ```sql
/// SELECT rufvector_create_graph('my_graph');
/// ```
#[pg_extern]
fn rufvector_create_graph(name: &str) -> bool {
...
/// # Example
/// ```sql
/// SELECT rufvector_cypher('my_graph', 'CREATE (n:Person {name: ''Alice''}) RETURN n', NULL);
/// SELECT rufvector_cypher('my_graph', 'MATCH (n:Person) WHERE n.name = $name RETURN n', '{"name": "Alice"}');
/// ```
#[pg_extern]
fn rufvector_cypher(graph_name: &str, query: &str, params: Option<JsonB>) -> Result<JsonB, String> {
    let graph =
...
/// # Example
/// ```sql
/// SELECT rufvector_shortest_path('my_graph', 1, 10, 5);
/// ```
#[pg_extern]
fn rufvector_shortest_path(
    graph_name: &str,
    start_id: i64,
    end_id: i64,
    max_hops: i32,
...
/// # Example
/// ```sql
/// SELECT rufvector_shortest_path_weighted('my_graph', 1, 10, 'distance');
/// ```
#[pg_extern]
fn rufvector_shortest_path_weighted(
    graph_name: &str,
    start_id: i64,
    end_id: i64,
    weight_property: &str,
...
/// # Example
/// ```sql
/// SELECT rufvector_graph_stats('my_graph');
/// ```
#[pg_extern]
fn rufvector_graph_stats(graph_name: &str) -> Result<JsonB, String> {
    let graph =
        get_graph(graph_name).ok_or_else(|| format!("Graph '{}' does not exist", graph_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_add_node('my_graph', ARRAY['Person'], '{"name": "Alice", "age": 30}');
/// ```
#[pg_extern]
fn rufvector_add_node(
    graph_name: &str,
    labels: Vec<String>,
    properties: JsonB,
) -> Result<i64, String> {
...
/// # Example
/// ```sql
/// SELECT rufvector_add_edge('my_graph', 1, 2, 'KNOWS', '{"since": 2020}');
/// ```
#[pg_extern]
fn rufvector_add_edge(
    graph_name: &str,
    source_id: i64,
    target_id: i64,
    edge_type: &str,
...
/// # Example
/// ```sql
/// SELECT rufvector_get_node('my_graph', 1);
/// ```
#[pg_extern]
fn rufvector_get_node(graph_name: &str, node_id: i64) -> Result<Option<JsonB>, String> {
    let graph =
        get_graph(graph_name).ok_or_else(|| format!("Graph '{}' does not exist", graph_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_get_edge('my_graph', 1);
/// ```
#[pg_extern]
fn rufvector_get_edge(graph_name: &str, edge_id: i64) -> Result<Option<JsonB>, String> {
    let graph =
        get_graph(graph_name).ok_or_else(|| format!("Graph '{}' does not exist", graph_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_find_nodes_by_label('my_graph', 'Person');
/// ```
#[pg_extern]
fn rufvector_find_nodes_by_label(graph_name: &str, label: &str) -> Result<JsonB, String> {
    let graph =
        get_graph(graph_name).ok_or_else(|| format!("Graph '{}' does not exist", graph_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_get_neighbors('my_graph', 1);
/// ```
#[pg_extern]
fn rufvector_get_neighbors(graph_name: &str, node_id: i64) -> Result<Vec<i64>, String> {
    let graph =
        get_graph(graph_name).ok_or_else(|| format!("Graph '{}' does not exist", graph_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_delete_graph('my_graph');
/// ```
#[pg_extern]
fn rufvector_delete_graph(graph_name: &str) -> bool {
...
/// # Example
/// ```sql
/// SELECT rufvector_list_graphs();
/// ```
#[pg_extern]
fn rufvector_list_graphs() -> Vec<String> {
    super::list_graphs()
}
...
/// # Example
/// ```sql
/// SELECT rufvector_create_rdf_store('my_store');
/// ```
#[pg_extern]
fn rufvector_create_rdf_store(name: &str) -> bool {
...
/// ```sql
/// -- Simple SELECT query
/// SELECT rufvector_sparql('my_store', 'SELECT ?s ?p ?o WHERE { ?s ?p ?o } LIMIT 10', 'json');
///
/// -- Query with PREFIX
/// SELECT rufvector_sparql('my_store', '
///     PREFIX foaf: <http://xmlns.com/foaf/0.1/>
///     SELECT ?name WHERE { ?person foaf:name ?name }
/// ', 'json');
///
/// -- ASK query
/// SELECT rufvector_sparql('my_store', 'ASK { <http://example.org/s> ?p ?o }', 'json');
/// ```
#[pg_extern]
fn rufvector_sparql(store_name: &str, query: &str, format: &str) -> Result<String, String> {
...
/// # Example
/// ```sql
/// SELECT rufvector_sparql_json('my_store', 'SELECT ?s ?p ?o WHERE { ?s ?p ?o }');
/// ```
#[pg_extern]
fn rufvector_sparql_json(store_name: &str, query: &str) -> Result<JsonB, String> {
    // Validate input to prevent panics that would abort PostgreSQL
    if query.trim().is_empty() {
        return Err("SPARQL query cannot be empty".to_string());
    }

    let result = rufvector_sparql(store_name, query, "json")?;
...
/// # Example
/// ```sql
/// SELECT rufvector_insert_triple_graph(
///     'my_store',
///     '<http://example.org/person/1>',
...
/// ```
#[pg_extern]
fn rufvector_insert_triple_graph(
    store_name: &str,
    subject: &str,
    predicate: &str,
    object: &str,
...
/// # Example
/// ```sql
/// SELECT rufvector_load_ntriples('my_store', '
///     <http://example.org/s1> <http://example.org/p1> "value1" .
///     <http://example.org/s2> <http://example.org/p2> "value2" .
/// ');
/// ```
#[pg_extern]
fn rufvector_load_ntriples(store_name: &str, ntriples: &str) -> Result<i64, String> {
    let store = get_or_create_store(store_name);
...
/// # Example
/// ```sql
/// SELECT rufvector_rdf_stats('my_store');
/// ```
#[pg_extern]
fn rufvector_rdf_stats(store_name: &str) -> Result<JsonB, String> {
    let store = get_store(store_name)
        .ok_or_else(|| format!("Triple store '{}' does not exist", store_name))?;
...
/// SELECT rufvector_query_triples('my_store', '<http://example.org/person/1>', NULL, NULL);
///
/// -- Get all triples with a specific predicate
/// SELECT rufvector_query_triples('my_store', NULL, '<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>', NULL);
/// ```
#[pg_extern]
fn rufvector_query_triples(
    store_name: &str,
    subject: Option<&str>,
...
/// # Example
/// ```sql
/// SELECT rufvector_clear_rdf_store('my_store');
/// ```
#[pg_extern]
fn rufvector_clear_rdf_store(store_name: &str) -> Result<bool, String> {
    let store = get_store(store_name)
        .ok_or_else(|| format!("Triple store '{}' does not exist", store_name))?;
...
/// # Example
/// ```sql
/// SELECT rufvector_delete_rdf_store('my_store');
/// ```
#[pg_extern]
fn rufvector_delete_rdf_store(store_name: &str) -> bool {
...
/// # Example
/// ```sql
/// SELECT rufvector_list_rdf_stores();
/// ```
#[pg_extern]
fn rufvector_list_rdf_stores() -> Vec<String> {
    list_stores()
}
...
/// # Example
/// ```sql
/// SELECT rufvector_sparql_update('my_store', '
///     INSERT DATA {
...
/// ```
#[pg_extern]
fn rufvector_sparql_update(store_name: &str, query: &str) -> Result<bool, String> {
    let store = get_store(store_name)
        .ok_or_else(|| format!("Triple store '{}' does not exist", store_name))?;
...
mod tests {
    ...
    fn test_create_graph() {
        let result = rufvector_create_graph("test_graph");
        assert!(result);

        let graphs = rufvector_list_graphs();
        assert!(graphs.contains(&"test_graph".to_string()));

        rufvector_delete_graph("test_graph");
    }

    #[pg_test]
    fn test_add_node_and_edge() {
        rufvector_create_graph("test_graph");

        let node1 = rufvector_add_node(
            "test_graph",
        ...
        let stats = rufvector_graph_stats("test_graph").unwrap();
    ...
    fn test_cypher_create_and_match() {
        ...
        // Create a node
        let create_result = rufvector_cypher(
            "test_graph",
        ...
        // Match the node
        let match_result = rufvector_cypher(
            "test_graph",
    ...
    fn test_shortest_path() {
        ...
        let n1 = rufvector_add_node("test_graph", vec![], JsonB(json!({}))).unwrap();

        let n2 = rufvector_add_node("test_graph", vec![], JsonB(json!({}))).unwrap();

        let n3 = rufvector_add_node("test_graph", vec![], JsonB(json!({}))).unwrap();

        rufvector_add_edge("test_graph", n1, n2, "KNOWS", JsonB(json!({}))).unwrap();
        rufvector_add_edge("test_graph", n2, n3, "KNOWS", JsonB(json!({}))).unwrap();

        let path = rufvector_shortest_path("test_graph", n1, n3, 10).unwrap();
    ...
    fn test_find_nodes_by_label() {
        ...
        let nodes = rufvector_find_nodes_by_label("test_graph", "Person").unwrap();
    ...
    fn test_get_neighbors() {
        ...
        rufvector_add_edge("test_graph", n1, n2, "KNOWS", JsonB(json!({}))).unwrap();
        rufvector_add_edge("test_graph", n1, n3, "KNOWS", JsonB(json!({}))).unwrap();

        let neighbors = rufvector_get_neighbors("test_graph", n1).unwrap();
    ...
    fn test_create_rdf_store() {
        let result = rufvector_create_rdf_store("test_rdf_store");
        assert!(result);

        let stores = rufvector_list_rdf_stores();
        assert!(stores.contains(&"test_rdf_store".to_string()));

        rufvector_delete_rdf_store("test_rdf_store");
    }

    #[pg_test]
    fn test_insert_triple() {
        rufvector_create_rdf_store("test_rdf_store");

        let id = rufvector_insert_triple(
            "test_rdf_store",
            "<http://example.org/person/1>",
            "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>",
            "<http://example.org/Person>",
        ...
        let stats = rufvector_rdf_stats("test_rdf_store").unwrap();
        let stats_obj = stats.0.as_object().unwrap();
    ...
    fn test_query_triples_pattern() {
        ...
        // Query by predicate
        let result = rufvector_query_triples(
            "test_rdf_store",
            None,
    ...
    fn test_load_ntriples() {
        ...
        let count = rufvector_load_ntriples("test_rdf_store", ntriples).unwrap();
        assert_eq!(count, 3);
    ...
    fn test_sparql_json() {
        ...
        let result = rufvector_sparql_json(
            "test_rdf_store",
