...
/// Configuration for RufVector background worker
#[derive(Debug, Clone)]
...
#[pg_guard]
pub extern "C" fn rufvector_bgworker_main(_arg: pg_sys::Datum) {
    // Initialize worker
    pgrx::log!("RufVector background worker starting");
    ...
    worker_state.stop();
    pgrx::log!("RufVector background worker stopped");
}
...
fn perform_maintenance_cycle() -> Result<(), String> {
    ...
    // Find all RufVector indexes
    let indexes = find_rufvector_indexes(config.max_indexes_per_cycle)?;
...
/// Find all RufVector indexes in the database
fn find_rufvector_indexes(max_count: usize) -> Result<Vec<IndexInfo>, String> {
...
#[pg_extern]
pub fn rufvector_bgworker_start() -> bool {
    let worker_state = get_worker_state();
...
#[pg_extern]
pub fn rufvector_bgworker_stop() -> bool {
    let worker_state = get_worker_state();
    if !worker_state.is_running() {
...
#[pg_extern]
pub fn rufvector_bgworker_status() -> pgrx::JsonB {
    let worker_state = get_worker_state();
...
#[pg_extern]
pub fn rufvector_bgworker_config(
