...
//! SET rufvector.hnsw_ef_search = 100;
//! ```
...
fn get_ef_search_guc() -> usize {
    // In production, read from rufvector.hnsw_ef_search GUC
    ...
    if meta.entry_point == pg_sys::InvalidBlockNumber {
        pgrx::warning!(
            "HNSW search: entry_point is InvalidBlockNumber (node_count={}, dims={}). \
             Index may need REINDEX. Check: SELECT rufvector_hnsw_debug('index_name')",
            meta.node_count,
            meta.dimensions
        );
        return Vec::new();
...
unsafe extern "C" fn hnsw_build(
    ...
    // For rufvector(384), atttypmod == 384. Fixes #171 and #164.
    let dimensions = {
        ...
        if dims == 0 {
            pgrx::warning!(
                "HNSW: Could not determine vector dimensions from column type modifier, \
                 defaulting to 384. Ensure column is defined as rufvector(N)."
            );
            dims = 384;
        }
...
unsafe extern "C" fn hnsw_build_callback(
    ...
    let datum = *values;
    let vector = match RufVector::from_polymorphic_datum(datum, false, pg_sys::InvalidOid) {
...
unsafe extern "C" fn hnsw_insert(
    ...
    // Extract vector from datum using RufVector::from_polymorphic_datum
    let datum = *values;
    let vector = match RufVector::from_polymorphic_datum(datum, false, pg_sys::InvalidOid) {
        Some(v) => v.as_slice().to_vec(),
        None => {
...
unsafe extern "C" fn hnsw_rescan(
    ...
    if norderbys > 0 && !orderbys.is_null() {
        ...
        // Method 1: Try direct RufVector extraction (works for literals and some casts)
        if let Some(vector) = RufVector::from_polymorphic_datum(
        ...
        if !state.query_valid && !datum.is_null() {
            ...
            if is_text_type {
                // Try to convert text to rufvector using the input function
                if let Some(vec) = try_convert_text_to_rufvector(datum) {
    ...
    if !state.query_valid || state.query_vector.is_empty() {
        ...
        pgrx::error!(
            "HNSW: Could not extract query vector from parameter. \
             Ensure the query vector is properly cast to rufvector type, e.g.: \
             ORDER BY embedding <=> '[1,2,3]'::rufvector(dim)"
        );
    }
...
/// Try to convert a text datum to rufvector by calling the input function
unsafe fn try_convert_text_to_rufvector(datum: Datum) -> Option<Vec<f32>> {
    // Get the text value
...
#[pg_extern]
fn rufvector_hnsw_debug(index_name: &str) -> pgrx::JsonB {
    use pgrx::prelude::*;
    ...
    if !index_exists {
        return pgrx::JsonB(serde_json::json!({
            ...
            "hint": "Use: SELECT rufvector_hnsw_debug('idx_name') where idx_name is an HNSW index"
        }));
    }
