...
use rufvector_math::optimal_transport::GromovWasserstein;
use rufvector_math::optimal_transport::{OptimalTransport, SinkhornSolver, SlicedWasserstein};
use rufvector_math::product_manifold::ProductManifold;
use rufvector_math::spectral::{GraphFilter, ScaledLaplacian, SpectralClustering, SpectralFilter};
use rufvector_math::spherical::SphericalSpace;
...
pub fn rufvector_wasserstein_distance(a: Vec<f32>, b: Vec<f32>, p: default!(i32, 1)) -> f32 {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_sinkhorn_distance(
    cost_json: JsonB,
    w_a: Vec<f32>,
    w_b: Vec<f32>,
    reg: default!(f32, 0.1),
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_sliced_wasserstein(
    pts_a_json: JsonB,
    pts_b_json: JsonB,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_kl_divergence(p: Vec<f32>, q: Vec<f32>) -> f32 {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_jensen_shannon(p: Vec<f32>, q: Vec<f32>) -> f32 {
    if p.len() != q.len() || p.is_empty() {
        pgrx::error!("Distributions must have same non-zero length");
    }
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_fisher_information(dist: Vec<f32>, tangent: Vec<f32>) -> f32 {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_spectral_cluster(adj_json: JsonB, k: i32) -> Vec<i32> {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_chebyshev_filter(
    adj_json: JsonB,
    signal: Vec<f32>,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_graph_diffusion(
    adj_json: JsonB,
    signal: Vec<f32>,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_product_manifold_distance(
    a: Vec<f32>,
    b: Vec<f32>,
    e_dim: i32,
    h_dim: i32,
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_spherical_distance(a: Vec<f32>, b: Vec<f32>) -> f32 {
    if a.len() != b.len() || a.is_empty() {
...
#[pg_extern(immutable, parallel_safe)]
pub fn rufvector_gromov_wasserstein(pts_a_json: JsonB, pts_b_json: JsonB) -> JsonB {
    let pts_a = parse_points(&pts_a_json);
    let pts_b = parse_points(&pts_b_json);
