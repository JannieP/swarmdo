...
[![Crates.io](https://img.shields.io/crates/v/rufvector-postgres.svg)](https://crates.io/crates/rufvector-postgres)
...
[![Docker](https://img.shields.io/badge/Docker-available-blue.svg)](https://hub.docker.com/r/ruvnet/rufvector-postgres)
[![npm](https://img.shields.io/npm/v/@rufvector/core.svg)](https://www.npmjs.com/package/@rufvector/core)
...
```bash
# Start the container
docker run -d --name rufvector-pg \
...
# Or use the rufvector app user (created automatically)
PGPASSWORD=rufvector psql -h localhost -p 5432 -U rufvector -d postgres
```

The container initializes with:
- Extension `rufvector` pre-installed and tested
- User `rufvector` with password `rufvector` for application use
...
npm install @rufvector/core

# Or install the full rufvector package
npm install rufvector
```

```javascript
const { VectorDB, cosineDistance } = require('@rufvector/core');
...
rufvector-pg -c "postgresql://localhost:5432/mydb" install
```
...
SELECT content, embedding <-> '[0.15, 0.25, ...]'::rufvector AS distance
FROM documents
ORDER BY distance
LIMIT 10;
```
...
RufVector exposes all advanced AI capabilities as native PostgreSQL functions.

### Core Vector Operations

```sql
-- Distance metrics
SELECT rufvector_cosine_distance(a, b);
SELECT rufvector_l2_distance(a, b);
SELECT rufvector_inner_product(a, b);
SELECT rufvector_manhattan_distance(a, b);

-- Vector operations
SELECT rufvector_normalize(embedding);
SELECT rufvector_add(a, b);
SELECT rufvector_scalar_mul(embedding, 2.0);
```
...
```sql
-- Poincare ball model
SELECT rufvector_poincare_distance(a, b, -1.0);  -- curvature -1

-- Lorentz hyperboloid model
SELECT rufvector_lorentz_distance(a, b, -1.0);

-- Hyperbolic operations
SELECT rufvector_mobius_add(a, b, -1.0);       -- Hyperbolic translation
SELECT rufvector_exp_map(base, tangent, -1.0); -- Tangent to manifold
SELECT rufvector_log_map(base, target, -1.0);  -- Manifold to tangent

-- Model conversion
SELECT rufvector_poincare_to_lorentz(poincare_vec, -1.0);
SELECT rufvector_lorentz_to_poincare(lorentz_vec, -1.0);

-- Minkowski inner product
SELECT rufvector_minkowski_dot(a, b);
```
...
SELECT rufvector_sparse_create(ARRAY[0, 5, 10], ARRAY[0.5, 0.3, 0.2], 100);
SELECT rufvector_sparse_from_dense(dense_vector, 0.01);  -- threshold

-- Sparse operations
SELECT rufvector_sparse_dot(a, b);
SELECT rufvector_sparse_cosine(a, b);
SELECT rufvector_sparse_l2_distance(a, b);
SELECT rufvector_sparse_add(a, b);
SELECT rufvector_sparse_scale(vec, 2.0);
SELECT rufvector_sparse_normalize(vec);
SELECT rufvector_sparse_topk(vec, 10);  -- Top-k elements

-- Text scoring
SELECT rufvector_bm25_score(query_terms, doc_freqs, doc_len, avg_doc_len, total_docs);
SELECT rufvector_tf_idf(term_freq, doc_freq, total_docs);
```
...
SELECT rufvector_attention_multi_head(query, keys, values, num_heads);

-- Flash attention (memory efficient)
SELECT rufvector_attention_flash(query, keys, values, block_size);

-- Sparse attention patterns
SELECT rufvector_attention_sparse(query, keys, values, sparsity_pattern);

-- Linear attention (O(n) complexity)
SELECT rufvector_attention_linear(query, keys, values);

-- Causal/masked attention
SELECT rufvector_attention_causal(query, keys, values);

-- Cross attention
SELECT rufvector_attention_cross(query, context_keys, context_values);

-- Self attention
SELECT rufvector_attention_self(input, num_heads);
```

### Sublinear Solvers (11 functions)

Graph analytics powered by rufvector-solver's O(log n) to O(sqrt(n)) algorithms.

```sql
-- PageRank (Forward Push, O(1/epsilon))
SELECT rufvector_pagerank('{"edges":[[0,1],[1,2],[2,0]]}'::jsonb);

-- Personalized PageRank from a source node
SELECT rufvector_pagerank_personalized('{"edges":[[0,1],[1,2],[2,0]]}'::jsonb, 0);

-- Solve sparse linear system Ax=b (Neumann or CG)
SELECT rufvector_solve_sparse(matrix_json, ARRAY[1.0, 2.0]::real[], 'cg');

-- Conjugate Gradient for SPD systems
SELECT rufvector_conjugate_gradient(matrix_json, rhs);

-- Graph Laplacian solver
SELECT rufvector_solve_laplacian(laplacian_json, rhs);

-- Effective resistance between nodes
SELECT rufvector_effective_resistance(laplacian_json, 0, 1);

-- Matrix sparsity analysis
SELECT rufvector_matrix_analyze(matrix_json);

-- List available solver algorithms
SELECT * FROM rufvector_solver_info();
```
...
SELECT rufvector_wasserstein_distance(ARRAY[0.5,0.5]::real[], ARRAY[0.3,0.7]::real[]);

-- Sinkhorn optimal transport with regularization
SELECT rufvector_sinkhorn_distance(cost_json, weights_a, weights_b);

-- KL divergence and Jensen-Shannon divergence
SELECT rufvector_kl_divergence(ARRAY[0.5,0.5]::real[], ARRAY[0.3,0.7]::real[]);
SELECT rufvector_jensen_shannon(ARRAY[0.5,0.5]::real[], ARRAY[0.3,0.7]::real[]);

-- Spectral clustering
SELECT rufvector_spectral_cluster(adjacency_json, 3);  -- k=3 clusters

-- Chebyshev polynomial graph filter
SELECT rufvector_chebyshev_filter(adj_json, signal, 'low_pass', 10);

-- Heat kernel graph diffusion
SELECT rufvector_graph_diffusion(adj_json, signal);

-- Product manifold distance (Euclidean x Hyperbolic x Spherical)
SELECT rufvector_product_manifold_distance(a, b, 3, 2, 1);

-- Spherical (great-circle) distance
SELECT rufvector_spherical_distance(ARRAY[1,0,0]::real[], ARRAY[0,1,0]::real[]);
```
...
SELECT rufvector_persistent_homology('[[1,0],[0,1],[-1,0],[0,-1]]'::jsonb, 1, 3.0);

-- Betti numbers at a given radius
SELECT rufvector_betti_numbers('[[0,0],[1,0],[0,1]]'::jsonb, 1.5);

-- Bottleneck distance between persistence diagrams
SELECT rufvector_bottleneck_distance(diagram_a, diagram_b);

-- Wasserstein distance between persistence diagrams
SELECT rufvector_persistence_wasserstein(diagram_a, diagram_b, 2);

-- Topological summary (Betti + persistence statistics + entropy)
SELECT rufvector_topological_summary(points_json, 1);

-- Embedding drift detection via topology
SELECT rufvector_embedding_drift(old_embeddings, new_embeddings);

-- Build Vietoris-Rips simplicial complex
SELECT rufvector_vietoris_rips(points_json, 2.0, 2);
```
...
SELECT rufvector_sona_learn('my_table', trajectory_json);

-- Apply learned LoRA transform to an embedding
SELECT rufvector_sona_apply('my_table', embedding);

-- Check EWC++ forgetting metrics
SELECT rufvector_sona_ewc_status('my_table');

-- Get Sona engine statistics
SELECT rufvector_sona_stats('my_table');
```
...
SELECT rufvector_domain_transfer(embeddings_json, 'target_domain');
```
...
SELECT rufvector_gnn_graphsage_layer(features, neighbor_features, weights);

-- GAT (Graph Attention Network)
SELECT rufvector_gnn_gat_layer(features, adjacency, attention_weights);

-- Message passing
SELECT rufvector_gnn_message_pass(node_features, edge_index, edge_weights);

-- Aggregation
SELECT rufvector_gnn_aggregate(messages, aggregation_type);  -- mean, max, sum
```
...
SELECT rufvector_route_query(query_embedding, agent_registry);

-- Route with context
SELECT rufvector_route_with_context(query, context, agents);

-- Multi-agent routing
SELECT rufvector_multi_agent_route(query, agents, top_k);

-- Agent management
SELECT rufvector_register_agent(name, capabilities, embedding);
SELECT rufvector_update_agent_performance(agent_id, metrics);
SELECT rufvector_get_routing_stats();

-- Affinity calculation
SELECT rufvector_calculate_agent_affinity(query, agent);
SELECT rufvector_select_best_agent(query, agent_list);

-- Adaptive routing
SELECT rufvector_adaptive_route(query, context, learning_rate);

-- FastGRNN acceleration
SELECT rufvector_fastgrnn_forward(input, hidden, weights);
```
...
SELECT rufvector_embed('Hello, world!');

-- Use specific model
SELECT rufvector_embed('Hello, world!', 'bge-small-en-v1.5');

-- Batch embedding (efficient for multiple texts)
SELECT rufvector_embed_batch(ARRAY['First doc', 'Second doc', 'Third doc']);

-- List available models
SELECT rufvector_list_models();

-- Get model information (dimensions, description)
SELECT rufvector_model_info('all-MiniLM-L6-v2');

-- Preload model into cache for faster subsequent calls
SELECT rufvector_preload_model('bge-base-en-v1.5');
```
...
VALUES (
    ...
    rufvector_embed('Artificial intelligence is transforming...')
);

-- Semantic search
SELECT title, embedding <=> rufvector_embed('machine learning basics') AS distance
FROM articles
ORDER BY distance
LIMIT 5;
```
...
SELECT rufvector_get_verdict(trajectory_id);

-- Memory distillation
SELECT rufvector_distill_memory(trajectories, compression_ratio);

-- Adaptive search
SELECT rufvector_adaptive_search(query, context, ef_search);

-- Learning feedback
SELECT rufvector_learning_feedback(search_id, relevance_scores);

-- Get learned patterns
SELECT rufvector_get_learning_patterns(context);

-- Optimize search parameters
SELECT rufvector_optimize_search_params(query_type, historical_data);
```
...
SELECT rufvector_graph_create_node(labels, properties, embedding);
SELECT rufvector_graph_create_edge(from_node, to_node, edge_type, properties);

-- Graph queries
SELECT rufvector_graph_get_neighbors(node_id, edge_type, depth);
SELECT rufvector_graph_shortest_path(start_node, end_node);
SELECT rufvector_graph_pagerank(edge_table, damping, iterations);

-- Cypher queries
SELECT rufvector_cypher_query('MATCH (n:Person)-[:KNOWS]->(m) RETURN n, m');

-- Traversal
SELECT rufvector_graph_traverse(start_node, direction, max_depth);

-- Similarity search on graph
SELECT rufvector_graph_similarity_search(query_embedding, node_type, top_k);
```
...
SELECT rufvector_create_rdf_store('knowledge_graph');

-- Insert triples
SELECT rufvector_insert_triple(
    'knowledge_graph',
...
-- Bulk load N-Triples
SELECT rufvector_load_ntriples('knowledge_graph', '
...
-- SPARQL SELECT query
SELECT rufvector_sparql('knowledge_graph', '
...
SELECT rufvector_rdf_stats('knowledge_graph');

-- Query triples by pattern (NULL = wildcard)
SELECT rufvector_query_triples('knowledge_graph',
    NULL, -- any subject
...
SELECT rufvector_sparql_update('knowledge_graph', '
    INSERT DATA {
...
SELECT rufvector_hybrid_linear(
...
SELECT rufvector_hybrid_search(
...
SELECT rufvector_get_hybrid_alpha();  -- Returns current alpha
SELECT rufvector_set_hybrid_alpha(0.6);
SELECT rufvector_get_hybrid_rrf_k();
SELECT rufvector_set_hybrid_rrf_k(40);
```
...
SELECT rufvector_set_tenant('tenant_123');
SELECT rufvector_get_tenant();

-- Create tenant-isolated table
SELECT rufvector_create_tenant_table(
    'documents',
    'id SERIAL PRIMARY KEY, content TEXT, embedding rufvector(384)'
);

-- Automatic tenant filtering (via RLS policies)
INSERT INTO documents (content, embedding)
VALUES ('Hello', '[0.1, 0.2, ...]'::rufvector);
...
-- Tenant management
SELECT rufvector_list_tenants();
SELECT rufvector_tenant_stats('tenant_123');
SELECT rufvector_migrate_tenant('old_tenant', 'new_tenant');

-- Cross-tenant queries (admin only)
SELECT rufvector_admin_query_all_tenants('documents', 'SELECT count(*) FROM documents');
```
...
```sql
-- Check index health
SELECT rufvector_index_health('documents_embedding_idx');
-- Returns: {"status": "healthy", "fragmentation": 0.05, "orphaned_nodes": 0}

-- Automatic repair
SELECT rufvector_auto_repair('documents_embedding_idx');

-- Schedule maintenance
SELECT rufvector_schedule_maintenance(
...
SELECT rufvector_compact_index('documents_embedding_idx');
SELECT rufvector_rebalance_hnsw('documents_embedding_idx');
SELECT rufvector_rebuild_ivf_centroids('documents_embedding_idx');
SELECT rufvector_validate_graph_connectivity('documents_embedding_idx');

-- Monitor healing status
SELECT rufvector_healing_status();
SELECT rufvector_last_repair_log('documents_embedding_idx');

-- Integrity checks
SELECT rufvector_check_orphaned_vectors('documents');
SELECT rufvector_check_duplicate_vectors('documents', threshold := 0.001);
```
...
SELECT rufvector_integrity_validate(
    'production_sla',
    recall := 0.97,
    latency_ms := 45,
    mincut := 0.15
...
### `rufvector(n)` - Dense Vector

```sql
CREATE TABLE items (embedding rufvector(1536));
...
SET rufvector.ivfflat_probes = 10;  -- Tune search quality
```
...
```sql
SELECT rufvector_gnn_gcn_layer(
    node_features,
    adjacency_matrix,
    trained_weights
) AS updated_features
...
# Commands
rufvector-pg install                    # Install extension
rufvector-pg vector create table --dim 384 --index hnsw
rufvector-pg hyperbolic poincare-distance --a "[0.1,0.2]" --b "[0.3,0.4]"
rufvector-pg gnn gcn --features "[[...]]" --adj "[[...]]"
rufvector-pg graph query "MATCH (n) RETURN n"
rufvector-pg routing route --query "[...]" --agents agents.json
rufvector-pg learning adaptive-search --context "[...]"
rufvector-pg bench run --type all --size 10000
```

## Related Packages

- [`@rufvector/postgres-cli`](https://www.npmjs.com/package/@rufvector/postgres-cli) - CLI for RufVector PostgreSQL
- [`rufvector-core`](https://crates.io/crates/rufvector-core) - Core vector operations library
- [`rufvector-tiny-dancer`](https://crates.io/crates/rufvector-tiny-dancer) - Agent routing library
...
Part of [RufVector](https://github.com/ruvnet/rufvector) -- the self-learning vector database.
