#!/bin/bash
#
# RufVector PostgreSQL Extension Installer
...
RUFVECTOR_VERSION="0.1.0"
EXTENSION_NAME="rufvector"
...
build_from_source() {
    log_info "Building RufVector from source..."
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        cd "$PROJECT_ROOT/crates/rufvector-postgres"
        cargo pgrx package --pg-config "$PG_CONFIG"
    fi

    # Set build output path
    BUILD_OUTPUT="$PROJECT_ROOT/target/release/rufvector-pg${PG_VERSION}"

    log_success "Build completed"
}

install_extension() {
    log_info "Installing RufVector extension..."

    local SO_FILE="${BUILD_OUTPUT}/usr/lib/postgresql/${PG_VERSION}/lib/rufvector.so"
    local CONTROL_FILE="${BUILD_OUTPUT}/usr/share/postgresql/${PG_VERSION}/extension/rufvector.control"
    local SQL_FILE="${PROJECT_ROOT}/crates/rufvector-postgres/sql/rufvector--${RUFVECTOR_VERSION}.sql"
    ...
    run_cmd chmod 755 "$PG_LIBDIR/rufvector.so"
...
create_config() {
    ...
    local CONFIG_OUT="$CONFIG_DIR/rufvector.conf"
    ...
    cat > "$CONFIG_OUT" << EOF
# RufVector PostgreSQL Extension Configuration
...
#rufvector.simd_mode = 'auto'

# =============================================================================
# Memory Configuration
# =============================================================================
# Maximum memory for vector operations (in MB)
#rufvector.max_memory_mb = 1024

# Enable memory pooling for better performance
#rufvector.enable_memory_pool = on

# =============================================================================
# Index Configuration
# =============================================================================
# Default HNSW index parameters
#rufvector.hnsw_ef_construction = 64
#rufvector.hnsw_m = 16
#rufvector.hnsw_ef_search = 40

# Default IVF-Flat index parameters
#rufvector.ivfflat_lists = 100
#rufvector.ivfflat_probes = 10

# =============================================================================
# Distance Calculation
# =============================================================================
# Enable parallel distance computation for large batches
#rufvector.parallel_distance = on

# Minimum batch size for parallel processing
#rufvector.parallel_min_batch = 1000

# =============================================================================
# Quantization
# =============================================================================
# Enable product quantization for large datasets
#rufvector.enable_pq = off

# Product quantization parameters
#rufvector.pq_m = 8
#rufvector.pq_nbits = 8

# =============================================================================
# Logging
# =============================================================================
# Log level: debug, info, warning, error
#rufvector.log_level = 'info'

# Log SIMD operations (for debugging)
#rufvector.log_simd = off
EOF
...
-- RufVector Installation Test Suite

-- Test 1: Create extension
CREATE EXTENSION IF NOT EXISTS rufvector;
SELECT 'Test 1: Extension created' AS result;

-- Test 2: Create table with rufvector column
CREATE TABLE test_vectors (id serial PRIMARY KEY, embedding rufvector);
...
uninstall_extension() {
    log_info "Uninstalling RufVector extension..."

    # Remove files
    local files_to_remove=(
        "$PG_LIBDIR/rufvector.so"
        "$PG_SHAREDIR/extension/rufvector.control"
        "$PG_SHAREDIR/extension/rufvector--${RUFVECTOR_VERSION}.sql"
        "$PG_SHAREDIR/extension/rufvector.conf"
    )
    ...
    log_success "RufVector uninstalled"
    log_warning "Note: You may need to DROP EXTENSION rufvector in databases where it was created"
}
...
show_help() {
    cat << EOF
RufVector PostgreSQL Extension Installer v${RUFVECTOR_VERSION}
...
main() {
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║         RufVector PostgreSQL Extension Installer               ║"
    echo "║       High-Performance Vector Similarity Search               ║"
    echo "║                    Version ${RUFVECTOR_VERSION}                             ║"
    echo "╚═══════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo ""
    log_success "RufVector installation complete!"
    echo ""
    echo "Next steps:"
    echo "  1. Connect to your database: psql -d your_database"
    echo "  2. Create the extension: CREATE EXTENSION rufvector;"
    echo "  3. Create a table with vectors:"
    echo "     CREATE TABLE items (id serial, embedding rufvector);"
    ...
    echo ""
    echo "Documentation: https://github.com/ruvnet/rufvector"
    echo ""
}
