#!/bin/bash
#
# RufVector Installation Verification Script
...
test_extension_load() {
    ...
    run_test "Create extension" \
        "DROP EXTENSION IF EXISTS rufvector CASCADE; CREATE EXTENSION rufvector;" \
        ""
    ...
    run_test "Check version" \
        "SELECT extversion FROM pg_extension WHERE extname = 'rufvector';" \
        "0.1.0"
}
...
test_type_creation() {
    ...
    run_test "Create table with rufvector" \
        "DROP TABLE IF EXISTS test_vec; CREATE TABLE test_vec (id serial, v rufvector);" \
        ""

    run_test "Create table with dimension constraint" \
        "DROP TABLE IF EXISTS test_vec_dim; CREATE TABLE test_vec_dim (id serial, v rufvector(128));" \
        ""
}
...
test_distance_functions() {
    ...
    run_test_numeric "L2 distance operator" \
        "SELECT '[1,2,3]'::rufvector <-> '[4,5,6]'::rufvector;" \
        "5.196" \
        "0.01"

    # Cosine distance
    run_test_numeric "Cosine distance operator" \
        "SELECT '[1,0,0]'::rufvector <=> '[0,1,0]'::rufvector;" \
        "1.0" \
        "0.01"
    ...
    run_test "Distance from stored vectors" \
        "SELECT id FROM test_vec ORDER BY v <-> '[1,1,1]'::rufvector LIMIT 1;" \
        "1"
}
...
test_vector_functions() {
    ...
    run_test "Get dimensions" \
        "SELECT rufvector_dims('[1,2,3,4,5]'::rufvector);" \
        "5"

    run_test_numeric "Get norm" \
        "SELECT rufvector_norm('[3,4]'::rufvector);" \
        "5.0" \
        "0.001"

    run_test "Normalize vector" \
        "SELECT rufvector_dims(rufvector_normalize('[1,2,3]'::rufvector));" \
        "3"

    run_test_numeric "Normalized vector norm" \
        "SELECT rufvector_norm(rufvector_normalize('[3,4,0]'::rufvector));" \
        "1.0" \
        "0.001"
}
...
test_vector_arithmetic() {
    ...
    run_test "Vector addition" \
        "SELECT rufvector_add('[1,2,3]'::rufvector, '[4,5,6]'::rufvector);" \
        "[5,7,9]"

    run_test "Vector subtraction" \
        "SELECT rufvector_sub('[4,5,6]'::rufvector, '[1,2,3]'::rufvector);" \
        "[3,3,3]"

    run_test "Scalar multiplication" \
        "SELECT rufvector_mul_scalar('[1,2,3]'::rufvector, 2.0);" \
        "[2,4,6]"
}
...
test_aggregate_operations() {
    ...
    run_test "Count vectors" \
        "SELECT count(*) FROM test_vec WHERE v <-> '[0,0,0]'::rufvector < 100;" \
        ""

    run_test "Min distance" \
        "SELECT count(*) FROM (SELECT min(v <-> '[1,1,1]'::rufvector) FROM test_vec) t;" \
        "1"

    run_test "Nearest neighbor query" \
        "SELECT count(*) FROM (SELECT id FROM test_vec ORDER BY v <-> '[1,1,1]'::rufvector LIMIT 3) t;" \
        "3"
}
...
test_error_handling() {
    ...
    local result
    if result=$(psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$TEST_DB" \
                     -c "SELECT '[1,2,3]'::rufvector <-> '[1,2]'::rufvector;" 2>&1); then
    ...
    # Invalid format
    if result=$(psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$TEST_DB" \
                     -c "SELECT 'invalid'::rufvector;" 2>&1); then
...
run_benchmarks() {
    ...
    psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$TEST_DB" -c "
        ...
        CREATE TABLE bench_vec (id serial PRIMARY KEY, embedding rufvector);
    " >/dev/null 2>&1
    ...
    psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$TEST_DB" -c "
        ...
        BEGIN
            FOR i IN 1..10000 LOOP
                ...
                INSERT INTO bench_vec (embedding) VALUES (vec::rufvector);
            END LOOP;
        END \$\$;
    " >/dev/null 2>&1
...
main() {
    ...
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║        RufVector Installation Verification Suite               ║"
    echo "╚═══════════════════════════════════════════════════════════════╝"
    echo ""

    # Create temp database if needed
    if [ -z "$TEST_DB" ]; then
        TEST_DB="rufvector_verify_$$"
        TEMP_DB=true
