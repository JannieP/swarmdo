-- Realistic workload benchmark for rufvector vs pgvector
...
DROP TABLE IF EXISTS vectors_rufvector CASCADE;
DROP TABLE IF EXISTS vectors_pgvector CASCADE;
DROP TABLE IF EXISTS queries CASCADE;

-- Create tables
CREATE TABLE vectors_rufvector (
    id SERIAL PRIMARY KEY,
    embedding rufvector(:dims),
    metadata JSONB
);
...
-- Insert vectors (rufvector)
INSERT INTO vectors_rufvector (embedding, metadata)
SELECT
    array_to_rufvector(ARRAY(
        SELECT random()::real
...
-- RufVector scan
\echo 'RufVector sequential scan (p50, p99 latency):'
SELECT
...
FROM (
    ...
    FROM (
        SELECT
            ...
            (SELECT id FROM vectors_rufvector v ORDER BY v.embedding <-> (:test_query)::rufvector LIMIT :k)
        FROM queries
        LIMIT 100
    ) t
) times;
...
-- RufVector HNSW
\echo 'Building rufvector HNSW index...'
\timing on
CREATE INDEX vectors_rufvector_hnsw_idx ON vectors_rufvector
USING hnsw (embedding rufvector_l2_ops)
...
-- Warm up
SELECT COUNT(*) FROM vectors_rufvector v, queries q
WHERE v.embedding <-> q.query_vector < 1000 LIMIT 100;

-- RufVector HNSW search
\echo 'RufVector HNSW search (p50, p99 latency):'
SELECT
    percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS p50_ms,
    percentile_cont(0.99) WITHIN GROUP (ORDER BY duration) AS p99_ms,
    AVG(duration) AS avg_ms,
...
FROM (
    ...
    FROM (
        SELECT
            q.id,
            clock_timestamp() AS start_time,
            (SELECT id FROM vectors_rufvector v ORDER BY v.embedding <-> q.query_vector LIMIT :k)
        FROM queries q
        LIMIT 1000
    ) t
) times;
...
\timing on
SELECT SUM(rufvector_l2_distance(v1.embedding, v2.embedding))
FROM vectors_rufvector v1
CROSS JOIN vectors_rufvector v2
...
\timing on
SELECT SUM(rufvector_cosine_distance(v1.embedding, v2.embedding))
FROM vectors_rufvector v1
CROSS JOIN vectors_rufvector v2
WHERE v1.id <= 100 AND v2.id <= 1000;
...
\timing on
SELECT SUM(rufvector_inner_product(v1.embedding, v2.embedding))
FROM vectors_rufvector v1
CROSS JOIN vectors_rufvector v2
WHERE v1.id <= 100 AND v2.id <= 1000;
...
-- Compute recall for rufvector HNSW
...
SELECT
    'rufvector' AS type,
    pg_size_pretty(pg_total_relation_size('vectors_rufvector')) AS total_size,
    pg_size_pretty(pg_relation_size('vectors_rufvector')) AS table_size,
    pg_size_pretty(pg_indexes_size('vectors_rufvector')) AS index_size
UNION ALL
SELECT
    'pgvector' AS type,
...
FROM pg_indexes
WHERE tablename IN ('vectors_rufvector', 'vectors_pgvector')
