...
//! SET swarmvector.ivfflat_adaptive_probes = on;
//! ```
...
unsafe extern "C" fn ivfflat_amrescan(
    ...
    if norderbys > 0 && !orderbys.is_null() {
        ...
        // Method 1: Try direct SwarmVector extraction
        if let Some(vector) = SwarmVector::from_polymorphic_datum(datum, false, typoid) {
        ...
        if !(*state).query_valid && !datum.is_null() {
            ...
            if is_text_type {
                if let Some(vec) = ivfflat_try_convert_text_to_swarmvector(datum) {
                    (*state).query = vec;
                    (*state).query_valid = true;
                    pgrx::debug1!(
        ...
        if !(*state).query_valid || (*state).query.is_empty() {
            pgrx::error!(
                "IVFFlat: Could not extract query vector from parameter. \
                 Ensure the query vector is properly cast to swarmvector type."
            );
        }
...
/// Try to convert a text datum to swarmvector (for parameterized queries)
unsafe fn ivfflat_try_convert_text_to_swarmvector(datum: Datum) -> Option<Vec<f32>> {
