//! Tenant Isolation Enforcement for SwarmVector Multi-Tenancy
//!
...
DROP POLICY IF EXISTS swarmvector_admin_bypass ON {table};

-- Create tenant isolation policy
CREATE POLICY swarmvector_tenant_isolation ON {table}
    USING ({column} = current_setting('swarmvector.tenant_id', true))
    WITH CHECK ({column} = current_setting('swarmvector.tenant_id', true));

-- Create admin bypass policy (for swarmvector_admin role)
CREATE POLICY swarmvector_admin_bypass ON {table}
    FOR ALL
...
CREATE POLICY swarmvector_admin_wildcard ON {table}
    FOR SELECT
...
GRANT USAGE ON SCHEMA {schema} TO swarmvector_users;
GRANT ALL ON ALL TABLES IN SCHEMA {schema} TO swarmvector_users;
GRANT ALL ON ALL SEQUENCES IN SCHEMA {schema} TO swarmvector_users;
"#,
