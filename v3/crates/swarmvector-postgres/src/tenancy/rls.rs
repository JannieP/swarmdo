//! Row-Level Security Integration for SwarmVector Multi-Tenancy
//!
...
impl Default for RlsPolicyConfig {
    fn default() -> Self {
        Self {
            ...
            policy_name: "swarmvector_tenant_isolation".to_string(),
            admin_bypass: true,
    ...
    pub fn using_clause(&self, tenant_column: &str) -> String {
        match self {
            ...
            Self::ReadSharedWriteOwn => {
                format!(
                    "{} = current_setting('swarmvector.tenant_id', true) OR is_public = true",
                    tenant_column
                )
            }
            Self::Hierarchical { path_column } => {
                format!(
                    "{} LIKE current_setting('swarmvector.tenant_id', true) || '%'",
                    path_column
                )
            }
            Self::TimeBased {
                ...
                format!(
                    "{} = current_setting('swarmvector.tenant_id', true) AND {} > NOW() - INTERVAL '{} days'",
...
-- Drop existing SwarmVector policies if any
...
-- Requires role: swarmvector_admin
CREATE POLICY {policy}_admin ON {table}
    FOR ALL
    TO swarmvector_admin
    USING (true)
    WITH CHECK (true);
"#,
                table = config.table_name,
...
CREATE POLICY {policy}_wildcard ON {table}
    FOR SELECT
    USING (current_setting('swarmvector.tenant_id', true) = '*');
"#,
                table = config.table_name,
                policy = config.policy_name
            ));
...
-- Drop all SwarmVector policies
DROP POLICY IF EXISTS swarmvector_tenant_isolation ON {table};
DROP POLICY IF EXISTS swarmvector_tenant_isolation_admin ON {table};
DROP POLICY IF EXISTS swarmvector_tenant_isolation_wildcard ON {table};
"#,
            table = table_name
        )
    }
    ...
    pub fn generate_add_tenant_column_sql(
        ...
        if default_current {
            sql.push_str(" DEFAULT current_setting('swarmvector.tenant_id')");
        }
...
--     FOREIGN KEY ({}) REFERENCES swarmvector.tenants(id) ON DELETE CASCADE;
...
"#,
    ...
    pub fn generate_roles_sql() -> String {
        r#"
-- Create SwarmVector roles for RLS
DO $$
BEGIN
    -- Admin role (bypasses RLS)
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'swarmvector_admin') THEN
        CREATE ROLE swarmvector_admin;
    END IF;

    -- Standard user role (subject to RLS)
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'swarmvector_users') THEN
        CREATE ROLE swarmvector_users;
    END IF;

    -- Read-only role
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'swarmvector_readonly') THEN
        CREATE ROLE swarmvector_readonly;
    END IF;
END
$$;

-- Grant basic permissions
GRANT USAGE ON SCHEMA public TO swarmvector_users, swarmvector_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO swarmvector_readonly;
GRANT ALL ON ALL TABLES IN SCHEMA public TO swarmvector_users;
"#
        .to_string()
    }
...
CREATE OR REPLACE FUNCTION swarmvector_validate_tenant_context_{table_safe}()
RETURNS TRIGGER AS $$
DECLARE
    v_tenant_id TEXT;
BEGIN
    -- Get current tenant context
    v_tenant_id := current_setting('swarmvector.tenant_id', true);

    -- Validate context is set
    IF v_tenant_id IS NULL OR v_tenant_id = '' THEN
        RAISE EXCEPTION 'No tenant context set. Use SET swarmvector.tenant_id = ''your-tenant-id''';
    END IF;
...
-- Create trigger
DROP TRIGGER IF EXISTS trg_swarmvector_validate_tenant_{table_safe} ON {table};
CREATE TRIGGER trg_swarmvector_validate_tenant_{table_safe}
    BEFORE INSERT OR UPDATE ON {table}
    FOR EACH ROW
    EXECUTE FUNCTION swarmvector_validate_tenant_context_{table_safe}();
"#,
            table = table_name,
...
CREATE OR REPLACE FUNCTION swarmvector_check_tenant_exists_{table_safe}()
RETURNS TRIGGER AS $$
BEGIN
    -- Check tenant exists (skip for admin wildcard)
    IF NEW.{column} != '*' THEN
        IF NOT EXISTS (SELECT 1 FROM swarmvector.tenants WHERE id = NEW.{column}) THEN
            RAISE EXCEPTION 'Tenant does not exist: %', NEW.{column};
        END IF;

        -- Check tenant is not suspended
        IF EXISTS (SELECT 1 FROM swarmvector.tenants WHERE id = NEW.{column} AND suspended_at IS NOT NULL) THEN
...
DROP TRIGGER IF EXISTS trg_swarmvector_check_tenant_{table_safe} ON {table};
CREATE TRIGGER trg_swarmvector_check_tenant_{table_safe}
    BEFORE INSERT OR UPDATE ON {table}
    FOR EACH ROW
    EXECUTE FUNCTION swarmvector_check_tenant_exists_{table_safe}();
"#,
            table = table_name,
            table_safe = table_name.replace('.', "_").replace('"', ""),
            column = tenant_column
...
mod tests {
    ...
    fn test_generate_enable_rls_sql() {
        ...
        assert!(sql.contains("swarmvector.tenant_id"));
        assert!(sql.contains("swarmvector_admin")); // Admin bypass
    ...
    fn test_set_tenant_sql() {
        ...
        assert!(sql.contains("SET swarmvector.tenant_id"));
    ...
    fn test_roles_sql() {
        ...
        assert!(sql.contains("swarmvector_readonly"));
    }
