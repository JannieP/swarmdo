...
///
/// ```sql
/// SELECT swarmvector_enable_learning('my_table', '{"max_trajectories": 2000}'::jsonb);
/// ```
#[pg_extern]
fn swarmvector_enable_learning(
    table_name: &str,
    config: Option<JsonB>,
...
///
/// ```sql
/// SELECT swarmvector_record_feedback(
///     'my_table',
///     ARRAY[0.1, 0.2, 0.3],
///     ARRAY[1, 2, 3]::bigint[],
///     ARRAY[4, 5]::bigint[]
...
/// ```
#[pg_extern]
fn swarmvector_record_feedback(
    table_name: &str,
...
///
/// ```sql
/// SELECT swarmvector_learning_stats('my_table');
/// ```
#[pg_extern]
fn swarmvector_learning_stats(
    table_name: &str,
...
///
/// ```sql
/// SELECT swarmvector_consolidate_patterns('my_table', 0.95);
/// ```
#[pg_extern]
fn swarmvector_consolidate_patterns(
    table_name: &str,
...
///
/// ```sql
/// SELECT swarmvector_prune_patterns('my_table', 5, 0.5);
/// ```
#[pg_extern]
fn swarmvector_prune_patterns(
    table_name: &str,
...
///
/// ```sql
/// SELECT swarmvector_get_search_params('my_table', ARRAY[0.1, 0.2, 0.3]);
/// ```
#[pg_extern]
fn swarmvector_get_search_params(
    table_name: &str,
    query_vector: Vec<f32>,
) -> Result<JsonB, Box<dyn std::error::Error + Send + Sync>> {
    let optimizer = LEARNING_MANAGER
...
///
/// ```sql
/// SELECT swarmvector_extract_patterns('my_table', 10);
/// ```
#[pg_extern]
fn swarmvector_extract_patterns(
    table_name: &str,
...
///
/// ```sql
/// SELECT swarmvector_record_trajectory(
///     'my_table',
///     ARRAY[0.1, 0.2, 0.3],
///     ARRAY[1, 2, 3]::bigint[],
///     1500,
...
/// ```
#[pg_extern]
fn swarmvector_record_trajectory(
    table_name: &str,
    query_vector: Vec<f32>,
    result_ids: Vec<i64>,
    latency_us: i64,
...
///
/// ```sql
/// SELECT swarmvector_clear_learning('my_table');
/// ```
#[pg_extern]
fn swarmvector_clear_learning(
    table_name: &str,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let bank = LEARNING_MANAGER
        .get_reasoning_bank(table_name)
...
mod tests {
    ...
    fn test_enable_learning() {
        let result = swarmvector_enable_learning("test_table", None);
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_learning_stats_empty() {
        swarmvector_enable_learning("test_stats", None).unwrap();
        let stats = swarmvector_learning_stats("test_stats");
        assert!(stats.is_ok());
    }

    #[pg_test]
    fn test_record_trajectory() {
        swarmvector_enable_learning("test_trajectory", None).unwrap();

        let result = swarmvector_record_trajectory(
            "test_trajectory",
            vec![1.0, 2.0, 3.0],
            vec![1, 2, 3],
            1000,
    ...
    fn test_extract_patterns() {
        swarmvector_enable_learning("test_patterns", None).unwrap();

        // Record some trajectories
        for i in 0..20 {
            swarmvector_record_trajectory(
                "test_patterns",
        ...
        let result = swarmvector_extract_patterns("test_patterns", 5);
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_auto_tune() {
        swarmvector_enable_learning("test_autotune", None).unwrap();
        ...
        let result = swarmvector_auto_tune("test_autotune", "balanced", None);
    ...
    fn test_get_search_params() {
        swarmvector_enable_learning("test_search_params", None).unwrap();
        ...
        swarmvector_extract_patterns("test_search_params", 3).unwrap();

        let result = swarmvector_get_search_params("test_search_params", vec![5.0, 0.0]);
    ...
    fn test_consolidate_patterns() {
        swarmvector_enable_learning("test_consolidate", None).unwrap();
        ...
        swarmvector_extract_patterns("test_consolidate", 10).unwrap();

        let result = swarmvector_consolidate_patterns("test_consolidate", 0.95);
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_prune_patterns() {
        swarmvector_enable_learning("test_prune", None).unwrap();

        // Record trajectories and extract patterns
        for i in 0..20 {
            swarmvector_record_trajectory("test_prune", vec![i as f32, 0.0], vec![i], 1000, 50, 10)
                .unwrap();
        }

        swarmvector_extract_patterns("test_prune", 5).unwrap();

        let result = swarmvector_prune_patterns("test_prune", 100, 0.9);
        assert!(result.is_ok());
    }

    #[pg_test]
    fn test_clear_learning() {
        swarmvector_enable_learning("test_clear", None).unwrap();

        swarmvector_record_trajectory("test_clear", vec![1.0, 2.0], vec![1], 1000, 50, 10).unwrap();

        let result = swarmvector_clear_learning("test_clear");
        assert!(result.is_ok());

        let stats = swarmvector_learning_stats("test_clear").unwrap();
