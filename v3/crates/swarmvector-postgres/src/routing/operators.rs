...
/// ```
#[pg_extern]
fn swarmvector_register_agent_full(config: JsonB) -> Result<bool, String> {
    let registry = get_registry();
...
/// ```
#[pg_extern]
fn swarmvector_update_agent_metrics(
    name: String,
    latency_ms: f32,
    success: bool,
    quality: Option<f32>,
...
/// # Example
/// ```sql
/// SELECT swarmvector_remove_agent('gpt-4');
/// ```
#[pg_extern]
fn swarmvector_remove_agent(name: String) -> Result<bool, String> {
    let registry = get_registry();
    registry
        .remove(&name)
        .ok_or_else(|| format!("Agent '{}' not found", name))?;
...
/// ```
#[pg_extern]
fn swarmvector_set_agent_active(name: String, is_active: bool) -> Result<bool, String> {
    let registry = get_registry();
...
/// ```
#[pg_extern]
fn swarmvector_list_agents() -> TableIterator<
    'static,
    (
        name!(name, String),
...
/// ```
#[pg_extern]
fn swarmvector_get_agent(name: String) -> Result<JsonB, String> {
    let registry = get_registry();
...
/// # Example
/// ```sql
/// SELECT * FROM swarmvector_find_agents_by_capability('code_generation', 5);
/// ```
#[pg_extern]
fn swarmvector_find_agents_by_capability(
    capability: String,
...
/// ```
#[pg_extern]
fn swarmvector_routing_stats() -> JsonB {
    let registry = get_registry();
...
#[pg_extern]
fn swarmvector_clear_agents() -> bool {
    let registry = get_registry();
    registry.clear();
    true
}
...
mod tests {
    ...
    fn test_register_agent() {
        swarmvector_clear_agents();

        let result = swarmvector_register_agent(
        ...
        let agent = swarmvector_get_agent("test-agent".to_string());
        assert!(agent.is_ok());
    }
    ...
    fn test_update_agent_metrics() {
        ...
        let result =
            swarmvector_update_agent_metrics("test-agent".to_string(), 150.0, true, Some(0.9));
    ...
    fn test_remove_agent() {
        ...
        let result = swarmvector_remove_agent("test-agent".to_string());
        assert!(result.is_ok());
    ...
    fn test_set_agent_active() {
        ...
        let result = swarmvector_set_agent_active("test-agent".to_string(), false);
        assert!(result.is_ok());

        let agent_json = swarmvector_get_agent("test-agent".to_string()).unwrap();
    ...
    fn test_list_agents() {
        ...
        let agents: Vec<_> = swarmvector_list_agents().collect();
    ...
    fn test_find_agents_by_capability() {
        ...
        let coders: Vec<_> = swarmvector_find_agents_by_capability("coding".to_string(), 10).collect();
    ...
    fn test_routing_stats() {
        ...
        let stats = swarmvector_routing_stats();
    ...
    fn test_route_basic() {
        ...
        let result = swarmvector_route(embedding.clone(), "cost".to_string(), None);
        assert!(result.is_ok());
