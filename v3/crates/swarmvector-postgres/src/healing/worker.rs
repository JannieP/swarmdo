...
pub extern "C" fn healing_bgworker_main(_arg: pgrx::pg_sys::Datum) {
    pgrx::log!("SwarmVector healing background worker starting");
...
pub fn register_healing_worker() {
    pgrx::log!("Registering SwarmVector healing background worker");
...
#[pg_extern]
pub fn swarmvector_healing_worker_start() -> bool {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn swarmvector_healing_worker_stop() -> bool {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn swarmvector_healing_worker_status() -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
#[pg_extern]
pub fn swarmvector_healing_worker_config(
...
#[pg_extern]
pub fn swarmvector_healing_check_now() -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
...
/// Get recent health check results
#[pg_extern]
pub fn swarmvector_healing_recent_checks(limit: default!(i32, 10)) -> pgrx::JsonB {
    let engine = get_healing_engine();
    let engine_lock = engine.read();
