//! Hybrid Search (BM25 + Vector) for SwarmVector Postgres
//!
...
//! SELECT swarmvector_register_hybrid(
...
#[pg_extern]
fn swarmvector_register_hybrid(
    collection: &str,
    vector_column: &str,
    fts_column: &str,
    text_column: &str,
    ...
    // In production, this would query swarmvector.collections table
    ...
    match registry.register(config) {
        Ok(_) => pgrx::JsonB(serde_json::json!({
            ...
            "message": "Collection registered for hybrid search. Run swarmvector_hybrid_update_stats() to compute corpus statistics."
        })),
...
#[pg_extern]
fn swarmvector_hybrid_update_stats(collection: &str) -> pgrx::JsonB {
    let (schema, table) = parse_collection_name(collection);
...
/// ```
#[pg_extern]
fn swarmvector_hybrid_configure(collection: &str, config: pgrx::JsonB) -> pgrx::JsonB {
    let (schema, table) = parse_collection_name(collection);
    let qualified_name = format!("{}.{}", schema, table);
    ...
    let config = match registry.get_by_name(&qualified_name) {
        ...
        None => {
            ...
            return pgrx::JsonB(serde_json::json!({
                ...
                "message": format!("Collection '{}' is not registered for hybrid search. Run swarmvector_register_hybrid() first to enable results.", collection)
            }));
        }
    };
...
#[pg_extern]
fn swarmvector_hybrid_stats(collection: &str) -> pgrx::JsonB {
    let (schema, table) = parse_collection_name(collection);
    let qualified_name = format!("{}.{}", schema, table);
...
#[pg_extern]
fn swarmvector_hybrid_list() -> pgrx::JsonB {
    let registry = get_registry();
