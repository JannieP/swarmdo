...
/// SELECT swarmvector_embed('Hello world', 'bge-small');
/// ```
#[pg_extern(immutable, parallel_safe)]
pub fn swarmvector_embed(text: &str, model_name: default!(&str, "'all-MiniLM-L6-v2'")) -> Vec<f32> {
...
/// # Example
/// ```sql
/// SELECT swarmvector_embed_batch(ARRAY['Hello', 'World', 'Test']);
/// ```
#[pg_extern(immutable, parallel_safe)]
pub fn swarmvector_embed_batch(
    texts: Vec<String>,
...
/// ```
#[pg_extern]
pub fn swarmvector_embedding_models() -> TableIterator<
    'static,
    (
        name!(name, String),
        name!(dimensions, i32),
...
/// # Example
/// ```sql
/// SELECT swarmvector_load_model('bge-small');
/// ```
#[pg_extern]
pub fn swarmvector_load_model(model_name: &str) -> bool {
...
/// # Example
/// ```sql
/// SELECT swarmvector_unload_model('bge-small');
/// ```
#[pg_extern]
pub fn swarmvector_unload_model(model_name: &str) -> bool {
    let model = match EmbeddingModel::from_name(model_name) {
        Some(m) => m,
        None => {
            pgrx::warning!("Unknown model: {}", model_name);
...
/// ```
#[pg_extern]
pub fn swarmvector_model_info(model_name: &str) -> pgrx::JsonB {
    let model = match EmbeddingModel::from_name(model_name) {
        Some(m) => m,
        None => {
...
/// # Example
/// ```sql
/// SELECT swarmvector_set_default_model('bge-small');
/// ```
#[pg_extern]
pub fn swarmvector_set_default_model(model_name: &str) -> bool {
    let model = match EmbeddingModel::from_name(model_name) {
        Some(m) => m,
        None => {
            pgrx::warning!("Unknown model: {}", model_name);
...
/// # Example
/// ```sql
/// SELECT swarmvector_default_model();
/// ```
#[pg_extern]
pub fn swarmvector_default_model() -> String {
...
/// # Example
/// ```sql
/// SELECT swarmvector_embedding_stats();
/// ```
#[pg_extern]
pub fn swarmvector_embedding_stats() -> pgrx::JsonB {
    let cache = global_cache();
...
/// # Example
/// ```sql
/// SELECT swarmvector_embedding_dims('all-MiniLM-L6-v2');  -- Returns 384
/// ```
#[pg_extern(immutable, parallel_safe)]
pub fn swarmvector_embedding_dims(model_name: &str) -> i32 {
...
mod tests {
    ...
    fn test_embedding_models_list() {
        let models: Vec<_> = swarmvector_embedding_models().collect();
        assert!(!models.is_empty());
    }

    #[pg_test]
    fn test_model_info() {
        let info = swarmvector_model_info("all-MiniLM-L6-v2");
        let json = info.0;
    ...
    fn test_default_model() {
        let name = swarmvector_default_model();
        assert!(!name.is_empty());
    }

    #[pg_test]
    fn test_embedding_dims() {
        assert_eq!(swarmvector_embedding_dims("all-MiniLM-L6-v2"), 384);
        assert_eq!(swarmvector_embedding_dims("bge-base"), 768);
        assert_eq!(swarmvector_embedding_dims("unknown"), -1);
    }
}
