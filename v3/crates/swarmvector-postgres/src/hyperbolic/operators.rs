...
/// # Example
/// ```sql
/// SELECT swarmvector_poincare_distance(
///     ARRAY[0.3, 0.4]::real[],
///     ARRAY[0.1, 0.2]::real[],
///     -1.0
/// );
...
fn swarmvector_poincare_distance(
    a: Vec<f32>,
    b: Vec<f32>,
...
/// # Example
/// ```sql
/// SELECT swarmvector_lorentz_distance(
...
/// ```
#[pg_extern(immutable, parallel_safe)]
fn swarmvector_lorentz_distance(
    a: Vec<f32>,
    b: Vec<f32>,
    curvature: default!(f32, "DEFAULT_CURVATURE"),
) -> f32 {
...
/// # Example
/// ```sql
/// SELECT swarmvector_poincare_to_lorentz(
///     ARRAY[0.3, 0.4]::real[],
///     -1.0
/// );
/// ```
#[pg_extern(immutable, parallel_safe)]
fn swarmvector_poincare_to_lorentz(
    poincare: Vec<f32>,
    curvature: default!(f32, "DEFAULT_CURVATURE"),
) -> Vec<f32> {
...
/// # Example
/// ```sql
/// SELECT swarmvector_lorentz_to_poincare(
///     ARRAY[1.5, 1.0, 0.5]::real[],
///     -1.0
/// );
/// ```
#[pg_extern(immutable, parallel_safe)]
fn swarmvector_lorentz_to_poincare(
    lorentz: Vec<f32>,
    curvature: default!(f32, "DEFAULT_CURVATURE"),
) -> Vec<f32> {
    if lorentz.len() < 2 {
...
/// # Example
/// ```sql
/// SELECT swarmvector_minkowski_dot(
///     ARRAY[2.0, 1.0, 1.0]::real[],
///     ARRAY[3.0, 2.0, 1.0]::real[]
/// );
/// ```
#[pg_extern(immutable, parallel_safe)]
fn swarmvector_minkowski_dot(a: Vec<f32>, b: Vec<f32>) -> f32 {
    if a.len() < 2 || b.len() < 2 {
...
mod tests {
    ...
    fn test_poincare_distance_basic() {
        let a = vec![0.0, 0.0];
        let b = vec![0.5, 0.0];
        let dist = swarmvector_poincare_distance(a, b, DEFAULT_CURVATURE);
        assert!(dist > 0.0);
    ...
    fn test_poincare_distance_symmetric() {
        let a = vec![0.3, 0.4];
        let b = vec![0.1, 0.2];
        let d1 = swarmvector_poincare_distance(a.clone(), b.clone(), DEFAULT_CURVATURE);
        let d2 = swarmvector_poincare_distance(b, a, DEFAULT_CURVATURE);
    ...
    fn test_poincare_distance_same() {
        let a = vec![0.3, 0.4];
        let dist = swarmvector_poincare_distance(a.clone(), a, DEFAULT_CURVATURE);
        assert!(dist < TOL);
    }
    ...
    fn test_lorentz_distance_basic() {
        ...
        let dist = swarmvector_lorentz_distance(a, b, DEFAULT_CURVATURE);
        assert!(dist > 0.0);
        assert!(dist < f32::INFINITY);
    }
    ...
    fn test_mobius_add_identity() {
        ...
        let result = swarmvector_mobius_add(a.clone(), origin, DEFAULT_CURVATURE);
        for i in 0..a.len() {
    ...
    fn test_exp_log_inverse() {
        ...
        let point = swarmvector_exp_map(base.clone(), tangent.clone(), DEFAULT_CURVATURE);
        let recovered = swarmvector_log_map(base, point, DEFAULT_CURVATURE);
    ...
    fn test_poincare_lorentz_conversion() {
        ...
        let lorentz = swarmvector_poincare_to_lorentz(poincare.clone(), DEFAULT_CURVATURE);
        let recovered = swarmvector_lorentz_to_poincare(lorentz, DEFAULT_CURVATURE);
    ...
    fn test_minkowski_dot_basic() {
        ...
        let result = swarmvector_minkowski_dot(a, b);
        // -2*3 + 1*2 + 1*1 = -3
    ...
    fn test_poincare_distance_empty() {
        let _ = swarmvector_poincare_distance(vec![], vec![0.1], DEFAULT_CURVATURE);
    }

    #[pg_test]
    #[should_panic(expected = "Vectors must have the same dimension")]
    fn test_poincare_distance_different_dims() {
        let _ = swarmvector_poincare_distance(vec![0.1], vec![0.1, 0.2], DEFAULT_CURVATURE);
    }

    #[pg_test]
    #[should_panic(expected = "Curvature must be negative")]
    fn test_poincare_distance_positive_curvature() {
        let _ = swarmvector_poincare_distance(vec![0.1], vec![0.2], 1.0);
    }
}
