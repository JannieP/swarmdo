...
pub extern "C" fn _PG_init() {
    ...
    // Log initialization
    pgrx::log!(
        "SwarmVector {} initialized with {} SIMD support",
        VERSION,
        distance::simd_info()
    );
}
...
#[pg_extern]
fn swarmvector_index_maintenance(index_name: &str) -> String {
...
mod tests {
    ...
    #[pg_test]
    fn test_version() {
        assert!(!swarmvector_version().is_empty());
    }

    #[pg_test]
    fn test_simd_info() {
        let info = swarmvector_simd_info();
        assert!(
