...
This module provides graph database capabilities for the swarmvector-postgres extension, including graph storage, traversal algorithms, Cypher query support, and W3C-standard SPARQL for RDF data.
...
SELECT swarmvector_shortest_path(
    'my_graph',
    1,    -- start_id
    10,   -- end_id
    5     -- max_hops
...
SELECT swarmvector_shortest_path_weighted(
    'my_graph',
    1,    -- start_id
    10,   -- end_id
...
SELECT swarmvector_create_rdf_store('my_knowledge_base');
...
SELECT swarmvector_load_ntriples('my_knowledge_base', '
...
SELECT swarmvector_sparql('my_knowledge_base', '
...
SELECT swarmvector_sparql_json('my_knowledge_base',
    'SELECT ?s ?p ?o WHERE { ?s ?p ?o } LIMIT 10'
);

-- Query triples by pattern (NULL = wildcard)
SELECT swarmvector_query_triples('my_knowledge_base',
...
SELECT swarmvector_rdf_stats('my_knowledge_base');
-- Returns: {"name": "...", "triple_count": 5, "subject_count": 2, ...}

-- SPARQL UPDATE
SELECT swarmvector_sparql_update('my_knowledge_base', '
    INSERT DATA {
...
-- Clear store
SELECT swarmvector_clear_rdf_store('my_knowledge_base');

-- Delete store
SELECT swarmvector_delete_rdf_store('my_knowledge_base');
...
-- Add users
WITH users AS (
    SELECT swarmvector_add_node('social_network', ARRAY['Person'],
...
-- Create friendships
SELECT swarmvector_add_edge('social_network', 1, 2, 'FRIENDS',
    '{"since": "2020-01-15"}'::jsonb);
SELECT swarmvector_add_edge('social_network', 2, 3, 'FRIENDS',
    '{"since": "2019-06-20"}'::jsonb);
SELECT swarmvector_add_edge('social_network', 1, 4, 'FRIENDS',
...
SELECT swarmvector_create_rdf_store('dbpedia_subset');

-- Load sample data
SELECT swarmvector_load_ntriples('dbpedia_subset', '
...
SELECT swarmvector_sparql('dbpedia_subset', '
...
SELECT swarmvector_query_triples('dbpedia_subset',
...
-- Add concepts
SELECT swarmvector_add_node('knowledge', ARRAY['Concept'],
...
-- Create relationships
SELECT swarmvector_add_edge('knowledge', 1, 2, 'INCLUDES',
    '{"strength": 0.9}'::jsonb);
SELECT swarmvector_add_edge('knowledge', 2, 3, 'SPECIALIZES_IN',
    '{"strength": 0.95}'::jsonb);

-- Find weighted path
SELECT swarmvector_shortest_path_weighted('knowledge', 1, 3, 'strength');
```
...
-- Add users and items
SELECT swarmvector_cypher('recommendations',
