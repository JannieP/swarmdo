...
#[pg_extern]
fn swarmvector_dag_set_enabled(enabled: bool) {
    if enabled {
        DAG_STATE.enable();
    } else {
        DAG_STATE.disable();
...
#[pg_extern]
fn swarmvector_dag_is_enabled() -> bool {
    DAG_STATE.is_enabled()
}

#[pg_extern]
fn swarmvector_dag_status() -> pgrx::JsonB {
...
#[pg_extern]
fn swarmvector_dag_set_learning_rate(rate: f64) {
...
#[pg_extern]
fn swarmvector_dag_set_attention(mechanism: &str) {
