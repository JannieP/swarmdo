#!/usr/bin/env bash
# SwarmVector-Postgres Docker Test Script
...
cmd_build() {
    ...
    log_info "Building swarmvector-postgres image..."
    DOCKER_BUILDKIT=1 docker build \
        ${build_args} \
        -f crates/swarmvector-postgres/docker/Dockerfile \
        -t "swarmvector-postgres:pg${PG_VERSION}" \
        --progress=plain \
        .
...
cmd_test() {
    ...
    while [ ${waited} -lt ${max_wait} ]; do
        if dc exec -T postgres pg_isready -U swarmvector -d swarmvector_test &>/dev/null; then
    ...
    # Collect results
    log_info "Test results available in: swarmvector-test-results volume"
...
cmd_benchmark() {
    ...
    log_info "Benchmark results available in: swarmvector-benchmark-results volume"
...
cmd_clean() {
    ...
    docker rmi swarmvector-postgres:pg14 2>/dev/null || true
    docker rmi swarmvector-postgres:pg15 2>/dev/null || true
    docker rmi swarmvector-postgres:pg16 2>/dev/null || true
    docker rmi swarmvector-postgres:pg17 2>/dev/null || true

    log_info "Removing volumes..."
    docker volume rm swarmvector-postgres-data 2>/dev/null || true
    docker volume rm swarmvector-cargo-cache 2>/dev/null || true
    docker volume rm swarmvector-cargo-git 2>/dev/null || true
    docker volume rm swarmvector-target-cache 2>/dev/null || true
    docker volume rm swarmvector-test-results 2>/dev/null || true
    docker volume rm swarmvector-benchmark-results 2>/dev/null || true
