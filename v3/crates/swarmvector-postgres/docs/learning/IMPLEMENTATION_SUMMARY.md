...
The Self-Learning/ReasoningBank module has been successfully implemented for the swarmvector-postgres PostgreSQL extension.
...
6. **`src/learning/operators.rs`** (457 lines)
   ...
   - `swarmvector_enable_learning` - Setup
   - `swarmvector_record_trajectory` - Manual recording
   - `swarmvector_record_feedback` - Relevance feedback
   - `swarmvector_learning_stats` - Statistics
   - `swarmvector_auto_tune` - Auto-optimization
   - `swarmvector_get_search_params` - Parameter lookup
   - `swarmvector_extract_patterns` - Pattern extraction
   - `swarmvector_consolidate_patterns` - Memory optimization
   - `swarmvector_prune_patterns` - Quality management
   - `swarmvector_clear_learning` - Reset
...
| Function | Purpose |
|----------|---------|
| `swarmvector_enable_learning` | Enable learning for a table |
| `swarmvector_record_trajectory` | Manually record trajectory |
| `swarmvector_record_feedback` | Add relevance feedback |
| `swarmvector_learning_stats` | Get statistics (JsonB) |
| `swarmvector_auto_tune` | Auto-optimize parameters |
| `swarmvector_get_search_params` | Get optimized params for query |
| `swarmvector_extract_patterns` | Extract patterns via k-means |
| `swarmvector_consolidate_patterns` | Merge similar patterns |
| `swarmvector_prune_patterns` | Remove low-quality patterns |
| `swarmvector_clear_learning` | Reset all learning data |

## 🚀 Usage Workflow

```sql
-- 1. Enable
SELECT swarmvector_enable_learning('my_table');

-- 2. Use (trajectories recorded automatically)
SELECT * FROM my_table ORDER BY vec <=> '[0.1,0.2,0.3]' LIMIT 10;

-- 3. Optional: Add feedback
SELECT swarmvector_record_feedback('my_table', ...);

-- 4. Extract patterns
SELECT swarmvector_extract_patterns('my_table', 10);

-- 5. Auto-tune
SELECT swarmvector_auto_tune('my_table', 'balanced');
...
- Works with existing `types::SwarmVector`
...
The implementation follows the plan in `docs/integration-plans/01-self-learning.md` and provides a solid foundation for adaptive query optimization in the swarmvector-postgres extension.
