# SwarmVector-Postgres Architecture

## Overview

SwarmVector-Postgres is a high-performance, drop-in replacement for the pgvector extension, built in Rust using the pgrx framework. It provides SIMD-optimized vector similarity search with advanced indexing algorithms, quantization support, and hybrid search capabilities.
...
5. **Production Ready**: Battle-tested algorithms from swarmvector-core
...
│                                                                               │
│  ┌─────────────────────────────────────────────────────────────────────────┐ │
│  │                      SwarmVector-Postgres Extension                         │ │
│  ├─────────────────────────────────────────────────────────────────────────┤ │
│  │                                                                           │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐  │ │
...
#### `swarmvector` - Primary Vector Type

**Varlena Memory Layout (Zero-Copy Design)**

```
┌─────────────────────────────────────────────────────────────────┐
│                    SwarmVector Varlena Layout                       │
├─────────────────────────────────────────────────────────────────┤
...
pub unsafe fn as_ptr(&self) -> *const f32 {
    // Skip varlena header (4 bytes) + SwarmVectorHeader (4 bytes)
    let base = self as *const _ as *const u8;
    base.add(VARHDRSZ + SwarmVectorHeader::SIZE) as *const f32
}
...
CREATE TABLE items (
    ...
    embedding swarmvector(1536)  -- OpenAI embedding dimensions
);
...
- 50% memory savings vs swarmvector
...
PostgreSQL automatically TOASTs values > ~2KB. SwarmVector handles this transparently:
...
- SwarmVector: ~512+ dimensions (2048+ bytes)
...
| Feature | pgvector 0.8.0 | SwarmVector-Postgres |
|---------|---------------|-------------------|
...
SwarmVector-Postgres is fully thread-safe:
...
SET swarmvector.ef_search = 200;  -- Higher = better recall, slower
SET swarmvector.probes = 10;      -- For IVFFlat: more probes = better recall
...
```
crates/swarmvector-postgres/
├── Cargo.toml                    # Rust dependencies
├── swarmvector.control              # Extension metadata
├── docs/
...
├── sql/
│   ├── swarmvector--0.1.0.sql       # Extension SQL definitions
│   └── swarmvector--0.0.0--0.1.0.sql # Migration script
├── src/
│   ├── lib.rs                    # Extension entry point
│   ├── types/
│   │   ├── mod.rs
│   │   ├── vector.rs             # swarmvector type (zero-copy varlena)
...
MIT License - Same as swarmvector-core
