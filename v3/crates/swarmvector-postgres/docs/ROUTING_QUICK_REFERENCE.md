...
SELECT swarmvector_register_agent(
    'gpt-4',                    -- name
    'llm',                      -- type
...
```sql
-- Simple registration
SELECT swarmvector_register_agent(name, type, capabilities, cost, latency, quality);

-- Full configuration
SELECT swarmvector_register_agent_full('{
    "name": "claude-3",
    "agent_type": "llm",
...
```sql
-- Cost-optimized
SELECT swarmvector_route(emb, 'cost', NULL);

-- Quality-optimized
SELECT swarmvector_route(emb, 'quality', NULL);

-- Latency-optimized
SELECT swarmvector_route(emb, 'latency', NULL);

-- Balanced (default)
SELECT swarmvector_route(emb, 'balanced', NULL);
```
...
```sql
-- Max cost
SELECT swarmvector_route(emb, 'quality', '{"max_cost": 0.01}'::jsonb);

-- Max latency
SELECT swarmvector_route(emb, 'balanced', '{"max_latency_ms": 500}'::jsonb);

-- Min quality
SELECT swarmvector_route(emb, 'cost', '{"min_quality": 0.8}'::jsonb);

-- Required capability
SELECT swarmvector_route(emb, 'balanced',
...
```sql
-- List all
SELECT * FROM swarmvector_list_agents();

-- Get specific agent
SELECT swarmvector_get_agent('gpt-4');

-- Find by capability
SELECT * FROM swarmvector_find_agents_by_capability('coding', 5);

-- Update metrics
SELECT swarmvector_update_agent_metrics('gpt-4', 450.0, true, 0.92);

-- Deactivate
SELECT swarmvector_set_agent_active('gpt-4', false);

-- Remove
SELECT swarmvector_remove_agent('old-agent');

-- Statistics
SELECT swarmvector_routing_stats();
```
...
```sql
-- Get agent name
SELECT (swarmvector_route(emb, 'balanced', NULL))::jsonb->>'agent_name';

-- Get cost
SELECT (swarmvector_route(emb, 'cost', NULL))::jsonb->>'estimated_cost';
...
FROM (
    SELECT swarmvector_route(emb, 'balanced', NULL) AS route
...
SELECT
    id,
    (swarmvector_route(embedding, 'cost', '{"max_cost": 0.01}'::jsonb))::jsonb->>'agent_name' AS agent
FROM requests
...
SELECT * FROM swarmvector_list_agents() WHERE is_active = true;

-- Relax constraints
SELECT swarmvector_route(emb, 'balanced', '{"max_cost": 1.0}'::jsonb);
```
...
```sql
-- Check reasoning
SELECT (swarmvector_route(emb, 'balanced', NULL))::jsonb->>'reasoning';

-- View alternatives
SELECT (swarmvector_route(emb, 'balanced', NULL))::jsonb->'alternatives';
```
...
```sql
-- Verify registration
SELECT swarmvector_get_agent('agent-name');

-- Check active status
SELECT is_active FROM swarmvector_list_agents() WHERE name = 'agent-name';

-- Reactivate
SELECT swarmvector_set_agent_active('agent-name', true);
```
...
1. **Always set constraints in production**
   ```sql
   SELECT swarmvector_route(emb, 'balanced', '{"max_cost": 0.1}'::jsonb);
   ```

2. **Update metrics after each request**
   ```sql
   SELECT swarmvector_update_agent_metrics(agent, latency, success, quality);
   ```
...
4. **Use capability filters**
   ```sql
   SELECT swarmvector_route(emb, 'quality',
       '{"required_capabilities": ["coding"]}'::jsonb);
   ```
...
```sql
SELECT swarmvector_route(emb, 'cost', '{"max_cost": 0.005}'::jsonb);
```

### Real-Time Chat (Latency-Optimized)
```sql
SELECT swarmvector_route(emb, 'latency', '{"max_latency_ms": 200}'::jsonb);
```

### Critical Analysis (Quality-Optimized)
```sql
SELECT swarmvector_route(emb, 'quality', '{"min_quality": 0.95}'::jsonb);
```
...
-- List active agents
SELECT name, capabilities FROM swarmvector_list_agents() WHERE is_active;

-- Test simple route
SELECT swarmvector_route(ARRAY[0.1]::float4[] || ARRAY(SELECT 0::float4 FROM generate_series(1,383)), 'balanced', NULL);

-- View agent details
SELECT jsonb_pretty(swarmvector_get_agent('gpt-4'));

-- Clear and restart (testing only)
-- SELECT swarmvector_clear_agents();
```
