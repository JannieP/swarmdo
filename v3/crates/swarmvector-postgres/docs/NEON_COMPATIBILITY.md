...
SwarmVector-Postgres is designed with first-class support for Neon's serverless PostgreSQL platform. This guide covers deployment, configuration, and optimization for Neon environments.
...
| Neon Feature | SwarmVector Support | Notes |
|--------------|------------------|-------|
...
Neon compute nodes are ephemeral and may be replaced at any time. SwarmVector-Postgres handles this by:
...
Critical for scale-to-zero. SwarmVector-Postgres achieves sub-100ms initialization:
...
Neon compute instances have memory limits based on compute units (CU). SwarmVector-Postgres is memory-conscious:
...
SET swarmvector.max_index_memory = '256MB';
...
Neon restricts background workers for resource management. SwarmVector-Postgres is designed without them:

```rust
// ❌ NOT USED: Background workers
// BackgroundWorker::register("swarmvector_maintenance", ...);
...
Neon uses PgBouncer in **transaction mode** for connection pooling. SwarmVector-Postgres is fully compatible:
...
BEGIN;
SET LOCAL swarmvector.ef_search = 100;  -- Transaction-local setting
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;

-- Next transaction (potentially different connection)
BEGIN;
SET LOCAL swarmvector.ef_search = 200;  -- Different setting
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;
```
...
- SwarmVector requires custom extension approval
...
## Requesting SwarmVector on Neon
...
Body:
I would like to install the SwarmVector-Postgres extension for vector similarity search.

Details:
- Extension: swarmvector-postgres
- Version: 0.1.19
...
1. **Shared Library** (`.so` file):
   ...
   # Artifact: target/release/swarmvector-pg16/usr/lib/postgresql/16/lib/swarmvector.so
   ```

2. **Control File** (`swarmvector.control`):
   ```
   comment = 'High-performance vector similarity search'
   default_version = '0.1.19'
   module_pathname = '$libdir/swarmvector'
   relocatable = true
   ```

3. **SQL Scripts**:
   - `swarmvector--0.1.0.sql` (initial schema)
   - `swarmvector--0.1.0--0.1.19.sql` (migration script)
...
-- SwarmVector provides migration path
-- (See MIGRATION.md)
```

## Migration from pgvector

SwarmVector-Postgres is API-compatible with pgvector. Migration is seamless:
...
-- Create new table with swarmvector
...
INSERT INTO items_swarmvector (id, content, embedding)
SELECT id, content, embedding::swarmvector FROM items;
```
...
CREATE INDEX items_embedding_ruhnsw_idx ON items_swarmvector
USING ruhnsw (embedding swarmvector_l2_ops)
WITH (m = 32, ef_construction = 200);

-- Analyze for query planner
ANALYZE items_swarmvector;
```
...
swarmvector_results AS (
    SELECT id, embedding <-> '[...]'::swarmvector AS dist
    FROM items_swarmvector ORDER BY dist LIMIT 10
)
SELECT
    p.id AS pg_id,
    r.id AS ru_id,
...
SET swarmvector.ef_search = 200;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;

-- Low latency (use for real-time queries)
SET swarmvector.ef_search = 40;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;

-- Per-query tuning
SET LOCAL swarmvector.ef_search = 100;
```
...
## Neon Branching with SwarmVector
...
ALTER DATABASE dev_branch SET swarmvector.ef_search = 20;

-- Staging branch: Balanced
ALTER DATABASE staging SET swarmvector.ef_search = 100;

-- Production branch: High recall
ALTER DATABASE prod SET swarmvector.ef_search = 200;
```
...
```sql
-- Index statistics
SELECT * FROM swarmvector_index_stats();
...
SELECT extname, extversion FROM pg_extension WHERE extname = 'swarmvector';
...
```sql
-- Reduce index memory
SET swarmvector.max_index_memory = '128MB';
...
ALTER DATABASE mydb SET swarmvector.ef_search = 100;
```
