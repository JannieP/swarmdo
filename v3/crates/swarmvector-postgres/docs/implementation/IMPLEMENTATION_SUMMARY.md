...
Complete implementation of IVFFlat (Inverted File with Flat quantization) as a PostgreSQL index access method for the swarmvector extension. This provides native, high-performance approximate nearest neighbor (ANN) search directly integrated into PostgreSQL.
...
- **GUC Variables**: Configurable via `swarmvector.ivfflat_probes`
...
|----------|---------|-------------|
| swarmvector.ivfflat_probes | 1 | Number of lists to probe during search |
...
SET swarmvector.ivfflat_probes = 20;
...
2. **Types Module**: Compatible with `SwarmVector` type
...
| Feature | swarmvector IVFFlat | pgvector IVFFlat |
|---------|------------------|------------------|
