...
```bash
# Build Docker image
docker build -t swarmvector-postgres:latest \
  -f crates/swarmvector-postgres/Dockerfile .
...
# Test extension
docker exec -it <container> psql -U postgres -c "CREATE EXTENSION swarmvector;"
```
...
```
target/release/swarmvector-postgres-pg16/
├── usr/
