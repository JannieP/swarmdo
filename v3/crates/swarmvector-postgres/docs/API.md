# SwarmVector-Postgres API Reference

## Overview

Complete API reference for SwarmVector-Postgres extension, including SQL functions, operators, types, and GUC variables.
...
CREATE TABLE items (
    ...
    embedding swarmvector(1536)  -- OpenAI ada-002 dimensions
);

INSERT INTO items (embedding) VALUES ('[1.0, 2.0, 3.0]');
INSERT INTO items (embedding) VALUES (ARRAY[1.0, 2.0, 3.0]::swarmvector);
```
...
- 50% memory reduction vs `swarmvector`
...
CREATE TABLE items (
    id SERIAL PRIMARY KEY,
    embedding halfvec(1536)  -- 3,080 bytes vs 6,152 for swarmvector
);

-- Automatic conversion from swarmvector
INSERT INTO items (embedding)
SELECT embedding::halfvec FROM swarmvector_table;
```
...
```sql
swarmvector_l2_distance(a swarmvector, b swarmvector) → float4
```
...
```sql
SELECT swarmvector_l2_distance(
    '[1.0, 2.0, 3.0]'::swarmvector,
    '[4.0, 5.0, 6.0]'::swarmvector
);
-- Output: 5.196...
```

#### `swarmvector_cosine_distance(a, b)`
...
```sql
swarmvector_cosine_distance(a swarmvector, b swarmvector) → float4
```
...
```sql
SELECT swarmvector_cosine_distance(
    '[1.0, 0.0]'::swarmvector,
    '[0.0, 1.0]'::swarmvector
);
...
```sql
swarmvector_ip_distance(a swarmvector, b swarmvector) → float4
```
...
```sql
SELECT swarmvector_ip_distance(
    '[1.0, 2.0, 3.0]'::swarmvector,
    '[4.0, 5.0, 6.0]'::swarmvector
);
...
```sql
swarmvector_l1_distance(a swarmvector, b swarmvector) → float4
```
...
```sql
SELECT swarmvector_l1_distance(
    '[1.0, 2.0, 3.0]'::swarmvector,
    '[4.0, 5.0, 6.0]'::swarmvector
);
-- Output: 9.0
...
```sql
swarmvector_norm(v swarmvector) → float4
```
...
```sql
SELECT swarmvector_norm('[3.0, 4.0]'::swarmvector);
-- Output: 5.0
```

#### `swarmvector_normalize(v)`
...
```sql
swarmvector_normalize(v swarmvector) → swarmvector
```
...
```sql
SELECT swarmvector_normalize('[3.0, 4.0]'::swarmvector);
-- Output: [0.6, 0.8]
```

### Index Maintenance Functions

#### `swarmvector_index_stats(index_name)`
...
```sql
swarmvector_index_stats(index_name text) → TABLE(
    index_name text,
...
#### `swarmvector_index_maintenance(index_name)`
...
```sql
swarmvector_index_maintenance(index_name text) → void
```
...
```sql
SELECT * FROM items
ORDER BY embedding <-> '[1.0, 2.0, 3.0]'::swarmvector
LIMIT 10;
```
...
- `swarmvector_l2_ops`: For `<->` operator
- `swarmvector_ip_ops`: For `<#>` operator
- `swarmvector_cosine_ops`: For `<=>` operator
...
```sql
SET swarmvector.ef_search = value;
```
...
```sql
CREATE INDEX ... USING ruhnsw (embedding swarmvector_l2_ops);
SELECT ... ORDER BY embedding <-> query;
```
...
```sql
CREATE INDEX ... USING ruhnsw (embedding swarmvector_ip_ops);
SELECT ... ORDER BY embedding <#> query;
```

### `swarmvector_cosine_ops`
...
```sql
CREATE INDEX ... USING ruhnsw (embedding swarmvector_cosine_ops);
SELECT ... ORDER BY embedding <=> query;
```
...
INSERT INTO documents (content, embedding) VALUES
    ('Document 1', '[0.1, 0.2, ...]'::swarmvector),
    ('Document 2', '[0.3, 0.4, ...]'::swarmvector);
...
-- Search
SELECT content, embedding <-> '[0.5, 0.6, ...]'::swarmvector AS distance
FROM documents
ORDER BY distance
LIMIT 10;
```
...
BEGIN
    FOR ef_val IN 10, 20, 40, 80, 160 LOOP
        EXECUTE format('SET LOCAL swarmvector.ef_search = %s', ef_val);
