...
The Tiny Dancer Routing module is a neural-powered dynamic agent routing system for the swarmvector-postgres PostgreSQL extension. It intelligently routes AI requests to the best available agent based on cost, latency, quality, and capability requirements.
...
```sql
-- Register agent
swarmvector_register_agent(name, type, capabilities, cost, latency, quality)

-- Register with full config
swarmvector_register_agent_full(config_jsonb)

-- Update metrics
swarmvector_update_agent_metrics(name, latency_ms, success, quality)

-- Remove agent
swarmvector_remove_agent(name)

-- Set active status
swarmvector_set_agent_active(name, is_active)

-- Get agent details
swarmvector_get_agent(name) -> jsonb

-- List all agents
swarmvector_list_agents() -> table

-- Find by capability
swarmvector_find_agents_by_capability(capability, limit) -> table
```
...
swarmvector_routing_stats() -> jsonb

-- Clear all agents (testing only)
swarmvector_clear_agents() -> boolean
```
...
SELECT
    r.id,
    (swarmvector_route(r.embedding, 'balanced', NULL))::jsonb->>'agent_name' AS agent
FROM requests r
...
cd /workspaces/swarmvector/crates/swarmvector-postgres
cargo test routing::
```
...
```sql
-- 1. Register agents
SELECT swarmvector_register_agent('gpt-4', 'llm', ARRAY['coding'], 0.03, 500.0, 0.95);
SELECT swarmvector_register_agent('gpt-3.5', 'llm', ARRAY['general'], 0.002, 150.0, 0.75);
...
For issues, questions, or contributions, see the main swarmvector-postgres repository.

## License

Same as swarmvector-postgres (MIT/Apache-2.0 dual license)
