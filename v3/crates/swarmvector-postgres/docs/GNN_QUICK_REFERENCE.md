...
**Example**:
```sql
SELECT swarmvector_message_pass(
    'nodes',
    'edges',
    'embedding',
    3,
...
graph_embeddings AS (
    SELECT
        graph_id,
        swarmvector_gnn_aggregate(embeddings, 'mean') as graph_embedding
    FROM node_embeddings
)
...
1. **Use Sampling for Large Graphs**:
   ...
   SELECT swarmvector_graphsage_forward(..., 10);  -- Sample 10 neighbors
   ```

2. **Batch Processing**:
   ```sql
   -- Process multiple graphs at once
   SELECT swarmvector_gnn_batch_forward(...);
   ```
...
SELECT CASE
    ...
    THEN NULL
    ELSE swarmvector_gcn_forward(embeddings, src, dst, NULL, 64)
END;
...
SELECT swarmvector_graphsage_forward(..., 10);  -- Limit neighbors
```
