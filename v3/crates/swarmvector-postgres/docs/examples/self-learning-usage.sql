-- =============================================================================
-- SwarmVector Self-Learning Module Usage Examples
-- =============================================================================
...
SELECT swarmvector_enable_learning('my_vectors');

-- Enable with custom configuration
SELECT swarmvector_enable_learning(
    'my_vectors',
...
SELECT swarmvector_extract_patterns(
    'my_vectors',  -- table name
    10             -- number of clusters
);

-- Get current learning statistics
SELECT swarmvector_learning_stats('my_vectors');
...
SELECT swarmvector_auto_tune('my_vectors');

-- Auto-tune optimizing for speed
SELECT swarmvector_auto_tune('my_vectors', 'speed');

-- Auto-tune optimizing for accuracy
SELECT swarmvector_auto_tune('my_vectors', 'accuracy');
...
SELECT swarmvector_get_search_params(
    'my_vectors',
...
-- SET swarmvector.ef_search = 52;
-- SET swarmvector.probes = 12;
...
SELECT swarmvector_consolidate_patterns('my_vectors', 0.95);

-- Prune low-quality patterns
-- Remove patterns with usage < 5 or confidence < 0.5
SELECT swarmvector_prune_patterns(
    'my_vectors',
    5,    -- min_usage
    0.5   -- min_confidence
);
...
SELECT swarmvector_enable_learning('documents');
...
BEGIN
    FOR i IN 1..50 LOOP
        ...
        query_vec := swarmvector_random(384);
        ...
        -- Record trajectory
        PERFORM swarmvector_record_trajectory(
            'documents',
            query_vec::float4[],
            results,
        ...
        IF i % 5 = 0 THEN
            PERFORM swarmvector_record_feedback(
                'documents',
                query_vec::float4[],
...
SELECT swarmvector_extract_patterns('documents', 10);

-- View learning statistics
SELECT swarmvector_learning_stats('documents');

-- Auto-tune for optimal performance
SELECT swarmvector_auto_tune('documents', 'balanced');

-- Get optimized parameters for a new query
WITH query AS (
    SELECT swarmvector_random(384) AS vec
),
params AS (
    SELECT swarmvector_get_search_params('documents', (SELECT vec::float4[] FROM query)) AS p
)
SELECT
...
SELECT swarmvector_consolidate_patterns('documents', 0.92);

-- Prune low-quality patterns monthly
SELECT swarmvector_prune_patterns('documents', 10, 0.6);

-- Clear all learning data if needed
SELECT swarmvector_clear_learning('documents');
...
def search_with_learning(conn, table, query_vector, limit=10):
    ...
    with conn.cursor() as cur:
        cur.execute("""
            SELECT swarmvector_get_search_params(%s, %s::float4[])
        """, (table, query_vector))
    ...
    with conn.cursor() as cur:
        cur.execute(f"""
            SET swarmvector.ef_search = {params['ef_search']};
            SET swarmvector.probes = {params['probes']};
...
SELECT
    jsonb_pretty(swarmvector_learning_stats('documents')) AS stats,
    CASE
...
FROM (
    SELECT swarmvector_learning_stats('documents') AS stats
) t;
