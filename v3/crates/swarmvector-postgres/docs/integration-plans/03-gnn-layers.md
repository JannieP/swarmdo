...
Integrate Graph Neural Network layers from `swarmvector-gnn` into PostgreSQL, enabling graph-aware vector search, message passing, and neural graph queries directly in SQL.
...
│  │                    GNN Layer Registry                    │    │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────────┐  │    │
│  │  │  GCN  │ │GraphSAGE│ │  GAT  │ │  GIN  │ │ SwarmVector  │  │    │
│  │  └───┬───┘ └───┬───┘ └───┬───┘ └───┬───┘ └─────┬─────┘  │    │
│  └──────┼─────────┼─────────┼─────────┼───────────┼────────┘    │
│         └─────────┴─────────┴─────────┴───────────┘             │
│                              ▼                                   │
...
│   │   └── swarmvector.rs     # Custom SwarmVector layer
...
CREATE INDEX ON nodes USING swarmvector_gnn (
    embedding vector(256)
) WITH (
    edge_table = 'edges',
...
SELECT n.id, n.embedding,
       swarmvector_gnn_score(n.embedding, query_vec, 'edges', 2) AS score
FROM nodes n
ORDER BY score DESC
LIMIT 10;
...
SELECT * FROM swarmvector_subgraph_search(
    center_node := 42,
...
SELECT node_id,
       swarmvector_gnn_classify(embedding, 'edges', model_name := 'node_classifier') AS class
FROM nodes;
```
...
SELECT swarmvector_build_knn_graph(
    node_table := 'nodes',
    embedding_column := 'embedding',
...
SELECT swarmvector_build_eps_graph(
    node_table := 'nodes',
    embedding_column := 'embedding',
...
#[pg_extern]
fn swarmvector_graphsage_search(
    node_table: &str,
    edge_table: &str,
    query: Vec<f32>,
...
### Phase 5: Custom SwarmVector Layer (Week 9-10)

```rust
// src/gnn/layers/swarmvector.rs

/// SwarmVector's custom differentiable search layer
/// Combines HNSW navigation with learned message passing
pub struct SwarmVectorLayer {
    in_features: usize,
    out_features: usize,
    num_hops: usize,
...
#[pg_extern]
fn swarmvector_differentiable_search(
    query: Vec<f32>,
    index_name: &str,
...
| SwarmVector | 10K | 100K | 256 | 25 | 55MB |

## Dependencies

```toml
[dependencies]
# Link to swarmvector-gnn
swarmvector-gnn = { path = "../swarmvector-gnn", optional = true }
