...
Integrate adaptive learning capabilities into swarmvector-postgres, enabling the database to learn from query patterns, optimize search strategies, and improve recall/precision over time.
...
SELECT swarmvector_enable_learning('embeddings',
...
SELECT * FROM swarmvector_learning_stats('embeddings');

-- Export learned patterns
SELECT swarmvector_export_patterns('embeddings') AS patterns_json;

-- Import patterns from another instance
SELECT swarmvector_import_patterns('embeddings', patterns_json);
```
...
SELECT swarmvector_auto_tune('embeddings_idx',
    optimize_for := 'recall',  -- or 'latency', 'balanced'
    sample_queries := 1000
);

-- Get recommended index parameters
SELECT * FROM swarmvector_recommend_params('embeddings');
```
...
SELECT swarmvector_benchmark_recall(
...
SELECT swarmvector_benchmark_latency(
    table_name := 'embeddings',
    num_queries := 10000,
    k := 10,
...
```toml
[dependencies]
# Existing swarmvector crates (optional integration)
# swarmvector-core = { path = "../swarmvector-core", optional = true }
...
learning-distributed = ["learning", "swarmvector-replication"]
```
