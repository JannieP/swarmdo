...
SELECT id, content,
       swarmvector_sparse_dot(sparse_embedding, query_sparse) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
...
SELECT id,
       swarmvector_sparse_cosine(sparse_embedding, query_sparse) AS similarity
FROM documents
WHERE swarmvector_sparse_cosine(sparse_embedding, query_sparse) > 0.5;
...
CREATE INDEX ON documents USING swarmvector_sparse (
...
SELECT * FROM swarmvector_hybrid_search(
...
fn swarmvector_sparse_dot(a: PgSparseVec, b: PgSparseVec) -> f32 {
    sparse_dot(&a.0, &b.0)
}

#[pg_extern(immutable, parallel_safe)]
fn swarmvector_sparse_cosine(a: PgSparseVec, b: PgSparseVec) -> f32 {
    sparse_cosine(&a.0, &b.0)
}

#[pg_extern(immutable, parallel_safe)]
fn swarmvector_sparse_euclidean(a: PgSparseVec, b: PgSparseVec) -> f32 {
...
#[pg_extern]
fn swarmvector_sparse_handler(internal: Internal) -> Internal {
...
#[pg_extern]
fn swarmvector_sparse_top_k(sparse: PgSparseVec, k: i32) -> PgSparseVec {
    PgSparseVec(sparse.0.top_k(k as usize))
}

#[pg_extern]
fn swarmvector_sparse_prune(sparse: PgSparseVec, threshold: f32) -> PgSparseVec {
