...
Integrate graph database capabilities from `swarmvector-graph` into PostgreSQL, enabling Cypher query language support, property graph operations, and vector-enhanced graph traversals directly in SQL.
...
-- Define node labels
SELECT swarmvector_create_node_label('social_network', 'Person',
    properties := '{
        "name": "text",
        "age": "integer",
...
SELECT swarmvector_create_node_label('social_network', 'Company',
    properties := '{
        "name": "text",
        "industry": "text",
        "embedding": "vector(768)"
...
-- Define edge types
SELECT swarmvector_create_edge_type('social_network', 'KNOWS',
    properties := '{"since": "date", "strength": "float"}'
);

SELECT swarmvector_create_edge_type('social_network', 'WORKS_AT',
...
SELECT * FROM swarmvector_cypher('social_network', $$
...
-- Create nodes
SELECT swarmvector_cypher('social_network', $$
...
SELECT * FROM swarmvector_cypher('social_network', $$
    MATCH (p:Person)
    WHERE swarmvector.similarity(p.embedding, $query) > 0.8
    RETURN p.name, p.age, swarmvector.similarity(p.embedding, $query) AS similarity
...
SELECT * FROM swarmvector_cypher('social_network', $$
    ...
    WHERE p.name = 'Alice'
    WITH friend, swarmvector.similarity(friend.embedding, $query) AS sim
    WHERE sim > 0.7
...
SELECT * FROM swarmvector_cypher('social_network', $$
    CALL swarmvector.pagerank('Person', 'KNOWS', {
        dampingFactor: 0.85,
        iterations: 20,
...
SELECT * FROM swarmvector_cypher('social_network', $$
    MATCH p = swarmvector.vectorPath(
        (a:Person {name: 'Alice'}),
        (b:Person {name: 'Bob'}),
        'KNOWS',
        {
    ...
    RETURN p, swarmvector.pathEmbeddingDistance(p) AS distance
$$);
```
...
SELECT * FROM swarmvector_cypher('social_network', $$
    CALL swarmvector.louvain('Person', 'KNOWS', {resolution: 1.0})
...
SELECT * FROM swarmvector_cypher('social_network', $$
    CALL swarmvector.nodeSimilarity('Person', 'KNOWS', {
...
-- Centrality measures
SELECT * FROM swarmvector_cypher('social_network', $$
    CALL swarmvector.betweenness('Person', 'KNOWS')
    YIELD node, score
    RETURN node.name, score
    ORDER BY score DESC
    LIMIT 10
...
// Node label creation
#[pg_extern]
fn swarmvector_create_node_label(
    graph_name: &str,
    label: &str,
...
// Edge type creation
#[pg_extern]
fn swarmvector_create_edge_type(
    graph_name: &str,
    edge_type: &str,
    properties: pgrx::JsonB,
) -> bool {
...
#[pg_extern]
fn swarmvector_graph_stats(graph_name: &str) -> pgrx::JsonB {
    let graph = get_graph(graph_name).expect("Graph not found");
...
- **SwarmVector-specific**:
  - `swarmvector.similarity(embedding1, embedding2)`
  - `swarmvector.distance(embedding1, embedding2, metric)`
  - `swarmvector.knn(embedding, k)`
...
```toml
[dependencies]
# Link to swarmvector-graph
swarmvector-graph = { path = "../swarmvector-graph", optional = true }
