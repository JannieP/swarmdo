...
Integrate AI agent routing capabilities from `swarmvector-tiny-dancer` into PostgreSQL, enabling intelligent request routing, model selection, and cost optimization directly in SQL.
...
SELECT * FROM swarmvector_route_multi(
...
SELECT swarmvector_create_route(
    name := 'customer_support',
    description := 'Handle customer support inquiries, complaints, and feedback',
    embedding := swarmvector_embed('Customer support and help requests'),
...
SELECT swarmvector_create_route(
    ...
    embedding := swarmvector_embed('Technical documentation and API reference'),
    target_agent := 'docs-agent',
    priority := 2
);

-- Semantic route matching
SELECT * FROM swarmvector_semantic_route(
...
SELECT * FROM swarmvector_routing_analytics(
    time_range := '7 days',
    group_by := 'agent'
);
...
SELECT * FROM swarmvector_optimize_budget(
...
SELECT * FROM swarmvector_batch_route(
    requests := ARRAY[
        'Simple question 1',
...
SELECT request_id, swarmvector_classify_request(content) AS classification
...
#[pg_extern]
fn swarmvector_classify_request(request: &str) -> pgrx::JsonB {
...
#[pg_extern]
fn swarmvector_optimize_budget(
    monthly_budget: f64,
...
#[pg_extern]
fn swarmvector_routing_analytics(
...
```toml
[dependencies]
# Link to swarmvector-tiny-dancer
swarmvector-tiny-dancer-core = { path = "../swarmvector-tiny-dancer-core", optional = true }
