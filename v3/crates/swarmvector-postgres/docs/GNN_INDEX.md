...
Complete Graph Neural Network (GNN) implementation for swarmvector-postgres PostgreSQL extension.
...
- swarmvector_gcn_forward()
- swarmvector_gnn_aggregate()
- swarmvector_message_pass()
- swarmvector_graphsage_forward()
- swarmvector_gnn_batch_forward()
...
### 1. GCN Forward Pass
```sql
swarmvector_gcn_forward(embeddings, src, dst, weights, out_dim) -> FLOAT[][]
```

### 2. GNN Aggregation
```sql
swarmvector_gnn_aggregate(messages, method) -> FLOAT[]
```

### 3. GraphSAGE Forward Pass
```sql
swarmvector_graphsage_forward(embeddings, src, dst, out_dim, num_samples) -> FLOAT[][]
```

### 4. Multi-Hop Message Passing
```sql
swarmvector_message_pass(node_table, edge_table, embedding_col, hops, layer_type) -> TEXT
```

### 5. Batch Processing
```sql
swarmvector_gnn_batch_forward(embeddings_batch, edge_indices, graph_sizes, layer_type, out_dim) -> FLOAT[][]
```
...
### Basic GCN
```sql
SELECT swarmvector_gcn_forward(
...
### Aggregation
```sql
SELECT swarmvector_gnn_aggregate(
    ARRAY[ARRAY[1.0, 2.0], ARRAY[3.0, 4.0]],
    'mean'
);
```
...
```sql
SELECT swarmvector_graphsage_forward(
