# Migration Guide from pgvector to SwarmVector-Postgres

## Overview

This guide provides step-by-step instructions for migrating from pgvector to SwarmVector-Postgres. SwarmVector-Postgres is designed as a **drop-in replacement** for pgvector with 100% SQL API compatibility and significant performance improvements.

## Key Benefits of Migration

| Feature | pgvector 0.8.0 | SwarmVector-Postgres | Improvement |
|---------|---------------|-------------------|-------------|
...
1. Install SwarmVector-Postgres alongside pgvector
2. Create parallel tables with SwarmVector types
3. Dual-write to both tables during transition
4. Validate SwarmVector results match pgvector
5. Switch reads to SwarmVector tables
...
1. Create complete SwarmVector environment (green)
2. Replicate data from pgvector (blue) to SwarmVector
...
1. Backup database
2. Install SwarmVector-Postgres
...
### Step 1: Install SwarmVector-Postgres

See [INSTALLATION.md](./INSTALLATION.md) for detailed instructions.

```bash
# Install SwarmVector-Postgres extension
cd swarmvector/crates/swarmvector-postgres
cargo pgrx package --pg-config $(which pg_config)
sudo cp target/release/swarmvector-pg16/usr/lib/postgresql/16/lib/* /usr/lib/postgresql/16/lib/
sudo cp target/release/swarmvector-pg16/usr/share/postgresql/16/extension/* /usr/share/postgresql/16/extension/
...
SELECT swarmvector_version();
-- Expected: 0.1.19

-- pgvector can coexist (for parallel deployment)
SELECT extname, extversion FROM pg_extension WHERE extname IN ('vector', 'swarmvector');
```
...
| pgvector Type | SwarmVector Type | Notes |
|---------------|---------------|-------|
| `vector(n)` | `swarmvector(n)` | Direct replacement |
...
-- Create SwarmVector table
CREATE TABLE items_swarmvector (
    id INT PRIMARY KEY,
    content TEXT,
    metadata JSONB,
...
INSERT INTO items_swarmvector (id, content, metadata, embedding, created_at)
SELECT id, content, metadata, embedding::swarmvector, created_at
FROM items;

-- Verify row counts match
SELECT
    (SELECT COUNT(*) FROM items) AS pgvector_count,
    (SELECT COUNT(*) FROM items_swarmvector) AS swarmvector_count;
```
...
-- Create new table with swarmvector type
CREATE TABLE items (
    id INT PRIMARY KEY,
    content TEXT,
    metadata JSONB,
...
| pgvector Index | SwarmVector Index | Notes |
|----------------|----------------|-------|
...
-- SwarmVector HNSW index (compatible parameters)
CREATE INDEX items_embedding_idx ON items_swarmvector
USING ruhnsw (embedding swarmvector_l2_ops)
WITH (m = 16, ef_construction = 64);
...
-- SwarmVector IVFFlat index
CREATE INDEX items_embedding_idx ON items_swarmvector
USING ruivfflat (embedding swarmvector_l2_ops)
WITH (lists = 100);
...
| pgvector | SwarmVector | Description |
|----------|----------|-------------|
...
-- SwarmVector query (identical syntax)
SELECT * FROM items_swarmvector
ORDER BY embedding <-> '[0.1, 0.2, ...]'::swarmvector
LIMIT 10;
```
...
-- SwarmVector query (identical)
SELECT * FROM items_swarmvector
WHERE embedding <-> '[...]'::swarmvector < 0.5;
```
...
```sql
-- Compare results between pgvector and SwarmVector
...
swarmvector_results AS (
    SELECT id, embedding <-> '[...]'::swarmvector AS distance
    FROM items_swarmvector
    ORDER BY distance
    LIMIT 100
)
SELECT
...
FULL OUTER JOIN swarmvector_results r ON p.id = r.id
...
```sql
-- Benchmark SwarmVector
\timing on
SELECT COUNT(*) FROM items_swarmvector WHERE embedding <-> '[...]'::swarmvector < 0.5;
-- Compare with pgvector baseline

EXPLAIN ANALYZE
SELECT * FROM items_swarmvector
ORDER BY embedding <-> '[...]'::swarmvector
LIMIT 10;
...
SELECT
    (SELECT COUNT(*) FROM items) AS pgvector_count,
    (SELECT COUNT(*) FROM items_swarmvector) AS swarmvector_count,
    (SELECT COUNT(*) FROM items) = (SELECT COUNT(*) FROM items_swarmvector) AS counts_match;

-- Check for NULL vectors
SELECT COUNT(*) FROM items_swarmvector WHERE embedding IS NULL;
...
// SwarmVector code (identical)
const result = await client.query(
    'SELECT * FROM items_swarmvector ORDER BY embedding <-> $1 LIMIT 10',
    [queryVector]
);
```
...
-- (Update application to write only to items_swarmvector)

-- Step 2: Sync any final changes (if dual-writing was used)
INSERT INTO items_swarmvector (id, content, metadata, embedding, created_at)
SELECT id, content, metadata, embedding::swarmvector, created_at
FROM items
WHERE id NOT IN (SELECT id FROM items_swarmvector)
ON CONFLICT (id) DO NOTHING;

-- Step 3: Switch reads to SwarmVector table
-- (Update application queries from 'items' to 'items_swarmvector')

-- Step 4: Rename tables for seamless transition
BEGIN;
ALTER TABLE items RENAME TO items_pgvector_old;
ALTER TABLE items_swarmvector RENAME TO items;
COMMIT;
...
ALTER SYSTEM SET swarmvector.ef_search = 100;  -- Higher = better recall
ALTER SYSTEM SET swarmvector.probes = 10;      -- For IVFFlat indexes
SELECT pg_reload_conf();

-- Or set per-session
SET swarmvector.ef_search = 200;  -- For high-recall queries
SET swarmvector.ef_search = 40;   -- For low-latency queries
```
...
SELECT * FROM swarmvector_index_stats('items_embedding_idx');
...
```
ERROR: cannot cast type vector to swarmvector
```
...
```sql
-- Explicit conversion
INSERT INTO items_swarmvector (embedding)
SELECT embedding::text::swarmvector FROM items;
...
SHOW swarmvector.ef_search;
-- Try increasing: SET swarmvector.ef_search = 100;
```
...
WITH comparison AS (
    ...
    FROM pgvector_results p
    JOIN swarmvector_results r ON p.id = r.id
)
SELECT
...
BEGIN;
ALTER TABLE items RENAME TO items_swarmvector;
ALTER TABLE items_pgvector_old RENAME TO items;
COMMIT;

-- Drop SwarmVector extension (optional)
DROP EXTENSION swarmvector CASCADE;
```
...
- [ ] Team trained on SwarmVector-specific features

## Schema Compatibility Notes

### Compatible SQL Functions

| pgvector | SwarmVector | Compatible |
|----------|----------|------------|
| `vector_dims(v)` | `swarmvector_dims(v)` | ✓ |
| `vector_norm(v)` | `swarmvector_norm(v)` | ✓ |
| `l2_distance(a, b)` | `swarmvector_l2_distance(a, b)` | ✓ |
| `cosine_distance(a, b)` | `swarmvector_cosine_distance(a, b)` | ✓ |
| `inner_product(a, b)` | `swarmvector_ip_distance(a, b)` | ✓ |

### New Features in SwarmVector
...
SELECT swarmvector_index_maintenance('items_embedding_idx');

-- Memory statistics
SELECT * FROM swarmvector_memory_stats();
```
...
- [ ] Install SwarmVector extension

## Migration
- [ ] Create SwarmVector tables
