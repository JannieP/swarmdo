...
This document describes the build system for the swarmvector-postgres extension.
...
```bash
# Build image
docker build -t swarmvector-postgres:latest -f crates/swarmvector-postgres/Dockerfile .

# Run container
docker run -d \
  -e POSTGRES_PASSWORD=postgres \
  -p 5432:5432 \
  swarmvector-postgres:latest
```
...
# Package location
ls target/release/swarmvector-postgres-pg16/
```
...
```bash
# Copy files
sudo cp target/release/swarmvector-postgres-pg16/usr/lib/postgresql/16/lib/*.so \
  /usr/lib/postgresql/16/lib/
sudo cp target/release/swarmvector-postgres-pg16/usr/share/postgresql/16/extension/* \
  /usr/share/postgresql/16/extension/

# Verify installation
psql -c "CREATE EXTENSION swarmvector;"
```
