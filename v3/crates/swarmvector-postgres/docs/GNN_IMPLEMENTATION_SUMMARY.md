...
Complete implementation of Graph Neural Network (GNN) layers for the swarmvector-postgres PostgreSQL extension. This module enables efficient graph learning directly on relational data.
...
1. **`swarmvector_gcn_forward(embeddings, src, dst, weights, out_dim)`**
   - Apply GCN layer to node embeddings
   - Returns: Updated embeddings after GCN

2. **`swarmvector_gnn_aggregate(messages, method)`**
   - Aggregate neighbor messages
   - Methods: 'sum', 'mean', 'max'
   - Returns: Aggregated message vector

3. **`swarmvector_message_pass(node_table, edge_table, embedding_col, hops, layer_type)`**
   - Multi-hop message passing
   - Layer types: 'gcn', 'sage'
   - Returns: Query description

4. **`swarmvector_graphsage_forward(embeddings, src, dst, out_dim, num_samples)`**
   - Apply GraphSAGE with neighbor sampling
   - Returns: Updated embeddings after GraphSAGE

5. **`swarmvector_gnn_batch_forward(embeddings_batch, edge_indices, graph_sizes, layer_type, out_dim)`**
...
All components implemented, tested, and integrated into swarmvector-postgres extension.
