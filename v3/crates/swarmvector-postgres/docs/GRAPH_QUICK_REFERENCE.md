...
-- Get node
SELECT swarmvector_get_node('graph_name', 1);

-- Find by label
SELECT swarmvector_find_nodes_by_label('graph_name', 'Person');
```
...
-- Get edge
SELECT swarmvector_get_edge('graph_name', 1);

-- Get neighbors
SELECT swarmvector_get_neighbors('graph_name', 1);
```
...
-- Create friendship
SELECT swarmvector_add_edge('social', 1, 2, 'FRIENDS',
    '{"since": "2020-01-15"}'::jsonb);

-- Find path
SELECT swarmvector_shortest_path('social', 1, 2, 10);
```
...
SELECT swarmvector_cypher('knowledge',
...
DO $$
BEGIN
    IF 'my_graph' = ANY(swarmvector_list_graphs()) THEN
        -- Perform operations
        RAISE NOTICE 'Graph exists';
    ELSE
        PERFORM swarmvector_create_graph('my_graph');
    END IF;
END $$;
...
DECLARE
    result jsonb;
BEGIN
    result := swarmvector_get_node('my_graph', 999);
