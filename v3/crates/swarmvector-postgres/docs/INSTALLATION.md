# SwarmVector-Postgres Installation Guide

## Overview

This guide covers installation of SwarmVector-Postgres on various platforms including standard PostgreSQL, Neon, Supabase, and containerized environments.
...
SwarmVector-Postgres supports PostgreSQL 14-18:
...
**Critical:** SwarmVector-Postgres requires pgrx **0.12.9 or higher**.
...
# target/release/swarmvector-pg16/usr/share/postgresql/16/extension/
# target/release/swarmvector-pg16/usr/lib/postgresql/16/lib/
```
...
sudo cp target/release/swarmvector-pg16/usr/share/postgresql/16/extension/* \
    /usr/share/postgresql/16/extension/

sudo cp target/release/swarmvector-pg16/usr/lib/postgresql/16/lib/* \
    /usr/lib/postgresql/16/lib/

# Set proper permissions
sudo chmod 644 /usr/share/postgresql/16/extension/swarmvector*
sudo chmod 755 /usr/lib/postgresql/16/lib/swarmvector.so
...
docker pull swarmvector/postgres:16
...
docker run -d \
    ...
    -p 5432:5432 \
    -v swarmvector-data:/var/lib/postgresql/data \
    swarmvector/postgres:16

# Connect and enable extension
docker exec -it swarmvector-postgres psql -U postgres -d vectordb
```
...
```dockerfile
# Dockerfile for SwarmVector-Postgres
FROM postgres:16
...
COPY . /app/swarmvector
WORKDIR /app/swarmvector/crates/swarmvector-postgres
...
RUN apt-get remove -y build-essential git curl && \
    ...
    rm -rf /usr/local/cargo/registry /app/swarmvector

# Auto-enable extension on database creation
RUN echo "CREATE EXTENSION IF NOT EXISTS swarmvector;" > /docker-entrypoint-initdb.d/init-swarmvector.sql
...
```bash
# Build image
docker build -t swarmvector-postgres:custom .
...
docker run -d \
    ...
    swarmvector-postgres:custom

# Verify installation
docker exec -it swarmvector-db psql -U postgres -d vectordb -c "SELECT swarmvector_version();"
```
...
1. **Request Installation** (Scale Plan customers):
   ...
   Subject: Custom Extension Request - SwarmVector-Postgres
   Details:
   - PostgreSQL version: 16 (or your version)
   - Extension: swarmvector-postgres v0.1.19
   - Use case: Vector similarity search
   ```

2. **Provide Artifacts**:
   - Pre-built `.so` files
   - Control file (`swarmvector.control`)
   - SQL scripts (`swarmvector--0.1.0.sql`)
...
-- Migrate to EC2 with SwarmVector when needed
...
```ini
# SwarmVector settings
shared_preload_libraries = 'swarmvector'  # Optional, for background workers
...
SET swarmvector.ef_search = 100;          -- Default: 40, Range: 1-1000

-- IVFFlat probes (higher = better recall, slower)
SET swarmvector.probes = 10;              -- Default: 1, Range: 1-10000

-- Set globally in postgresql.conf:
ALTER SYSTEM SET swarmvector.ef_search = 100;
ALTER SYSTEM SET swarmvector.probes = 10;
...
BEGIN;
SET LOCAL swarmvector.ef_search = 200;
SET LOCAL swarmvector.probes = 20;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;

-- For low-latency queries
BEGIN;
SET LOCAL swarmvector.ef_search = 20;
SET LOCAL swarmvector.probes = 1;
SELECT * FROM items ORDER BY embedding <-> query LIMIT 10;
COMMIT;
```
...
SELECT * FROM pg_extension WHERE extname = 'swarmvector';
-- Expected: extname=swarmvector, extversion=0.1.19
...
SELECT id, embedding <-> '[1, 1, 1]'::swarmvector AS distance
FROM test_vectors
ORDER BY distance
LIMIT 3;
...
CREATE TABLE items (
    id SERIAL PRIMARY KEY,
    embedding swarmvector(128)
);

-- Insert sample data (10,000 vectors)
INSERT INTO items (embedding)
SELECT ('[' || array_to_string(array_agg(random()), ',') || ']')::swarmvector
...
# Check library path
pg_config --pkglibdir
ls -la $(pg_config --pkglibdir)/swarmvector*

# Expected output:
# -rwxr-xr-x ... swarmvector.so

# Check extension path
pg_config --sharedir
ls -la $(pg_config --sharedir)/extension/swarmvector*

# Expected output:
# -rw-r--r-- ... swarmvector.control
# -rw-r--r-- ... swarmvector--0.1.0.sql
...
```bash
sudo chmod 755 $(pg_config --pkglibdir)/swarmvector.so
sudo chmod 644 $(pg_config --sharedir)/extension/swarmvector*
sudo systemctl restart postgresql
```
...
-- Upgrade extension
ALTER EXTENSION swarmvector UPDATE TO '0.1.20';
...
# Upgrade in database
psql -U postgres -d your_database -c "ALTER EXTENSION swarmvector UPDATE;"
```
...
sudo rm $(pg_config --pkglibdir)/swarmvector.so
sudo rm $(pg_config --sharedir)/extension/swarmvector*
