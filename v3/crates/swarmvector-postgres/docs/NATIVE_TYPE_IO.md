# Native PostgreSQL Type I/O Functions for SwarmVector

## Overview

This document describes the native PostgreSQL type I/O functions implementation for the `SwarmVector` type, providing zero-copy access like pgvector.

## Implementation Summary

### Memory Layout

The `SwarmVector` type uses a pgvector-compatible varlena layout:
...
**Example:**
```sql
SELECT '[1.0, 2.0, 3.0]'::swarmvector;
```

#### 2. `swarmvector_out` - Text Output
...
#### 3. `swarmvector_recv` - Binary Input
...
#### 4. `swarmvector_send` - Binary Output
...
unsafe fn from_varlena(varlena_ptr: *const pgrx::pg_sys::varlena) -> Self {
    ...
    SwarmVector { dimensions, data: data.to_vec() }
}
```
...
CREATE FUNCTION swarmvector_in(cstring)
RETURNS swarmvector
AS 'MODULE_PATHNAME', 'swarmvector_in'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION swarmvector_out(swarmvector)
RETURNS cstring
AS 'MODULE_PATHNAME', 'swarmvector_out'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION swarmvector_recv(internal)
RETURNS swarmvector
AS 'MODULE_PATHNAME', 'swarmvector_recv'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION swarmvector_send(swarmvector)
RETURNS bytea
AS 'MODULE_PATHNAME', 'swarmvector_send'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
- **Main Implementation**: `/home/user/swarmvector/crates/swarmvector-postgres/src/types/vector.rs`
