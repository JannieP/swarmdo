...
```sql
swarmvector_attention_score(query, key, type) → float4
```
...
```sql
swarmvector_softmax(scores) → float4[]
```
...
```sql
swarmvector_multi_head_attention(query, keys, values, num_heads) → float4[]
```
...
```sql
swarmvector_flash_attention(query, keys, values, block_size) → float4[]
```
...
```sql
swarmvector_attention_scores(query, keys, type) → float4[]
```
...
```sql
swarmvector_attention_types() → TABLE(name, complexity, best_for)
```
...
```rust
use swarmvector_postgres::attention::ScaledDotAttention;
...
```rust
use swarmvector_postgres::attention::MultiHeadAttention;
...
```rust
use swarmvector_postgres::attention::FlashAttention;
...
SELECT
    id,
    swarmvector_attention_score(query_vector, embedding, 'scaled_dot') AS score
FROM candidates
ORDER BY score DESC
LIMIT 10;
```
...
SELECT
    q.id AS query_id,
    d.id AS doc_id,
    swarmvector_attention_score(q.embedding, d.embedding, 'scaled_dot') AS score
FROM queries q
...
WITH stage1 AS (
    SELECT id, embedding,
           swarmvector_attention_score(query, embedding, 'scaled_dot') AS score
    FROM documents
    WHERE score > 0.5
    LIMIT 50
)
...
SELECT id,
       swarmvector_multi_head_attention(
           query,
           ARRAY_AGG(embedding),
           ARRAY_AGG(embedding),
           8
...
SELECT
    query_id,
    doc_id,
    swarmvector_attention_score(q_vec, d_vec, 'scaled_dot') AS score
FROM queries
