...
```sql
CREATE EXTENSION IF NOT EXISTS swarmvector_postgres;
```
...
SELECT id, content,
       swarmvector_sparse_cosine(
           sparse_embedding,
           '{1:0.5, 2:0.3}'::sparsevec
       ) AS similarity
FROM documents
WHERE swarmvector_sparse_cosine(sparse_embedding, '{1:0.5, 2:0.3}'::sparsevec) > 0.5;
```
...
SELECT id, text,
       swarmvector_sparse_dot(splade_embedding, query_embedding) AS relevance
FROM ml_documents
...
UPDATE vectors
SET sparse_vec = swarmvector_dense_to_sparse(dense_vec);

-- Prune small values
UPDATE vectors
SET sparse_vec = swarmvector_sparse_prune(sparse_vec, 0.1);

-- Keep only top 100 elements
UPDATE vectors
SET sparse_vec = swarmvector_sparse_top_k(sparse_vec, 100);
```
...
-- Get properties
SELECT
    swarmvector_sparse_nnz(sparse_embedding) AS num_nonzero,
    swarmvector_sparse_dim(sparse_embedding) AS dimension,
    swarmvector_sparse_norm(sparse_embedding) AS l2_norm
FROM documents;

-- Sparsify
SELECT swarmvector_sparse_top_k(sparse_embedding, 50) FROM documents;
SELECT swarmvector_sparse_prune(sparse_embedding, 0.2) FROM documents;

-- Convert formats
SELECT swarmvector_sparse_to_dense(sparse_embedding) FROM documents;
SELECT swarmvector_dense_to_sparse(ARRAY[0, 0.5, 0, 0.3]::real[]);
```
...
SELECT d.id, d.content,
       swarmvector_sparse_cosine(d.sparse_embedding, q.query_vec) AS similarity
...
SELECT id, content,
       ...
       0.3 * swarmvector_sparse_dot(sparse_embedding, query_sparse) AS combined_score
FROM hybrid_docs
...
SELECT q.query_id, d.id, d.content,
       swarmvector_sparse_dot(d.sparse_embedding, q.query_vec) AS score
FROM documents d
...
2. **Prune small values**: Remove noise with `swarmvector_sparse_prune(vec, 0.1)`
3. **Top-k sparsification**: Keep most important features with `swarmvector_sparse_top_k(vec, 100)`
...
SELECT swarmvector_to_sparse(ARRAY[1,2]::int[], ARRAY[0.5]::real[], 10);

-- ✅ Correct: same lengths
SELECT swarmvector_to_sparse(ARRAY[1,2]::int[], ARRAY[0.5,0.3]::real[], 10);
```
...
SELECT swarmvector_to_sparse(ARRAY[100]::int[], ARRAY[0.5]::real[], 10);

-- ✅ Correct: all indices < dimension
SELECT swarmvector_to_sparse(ARRAY[5]::int[], ARRAY[0.5]::real[], 10);
```
