...
Complete implementation of sparse vector support for swarmvector-postgres PostgreSQL extension, providing efficient storage and operations for high-dimensional sparse embeddings.
...
**Distance Operations**:
- `swarmvector_sparse_dot(a, b) -> f32`
- `swarmvector_sparse_cosine(a, b) -> f32`
- `swarmvector_sparse_euclidean(a, b) -> f32`
- `swarmvector_sparse_manhattan(a, b) -> f32`

**Construction Functions**:
- `swarmvector_to_sparse(indices, values, dim) -> sparsevec`
- `swarmvector_dense_to_sparse(dense) -> sparsevec`
- `swarmvector_sparse_to_dense(sparse) -> real[]`

**Utility Functions**:
- `swarmvector_sparse_nnz(sparse) -> int` - Number of non-zeros
- `swarmvector_sparse_dim(sparse) -> int` - Dimension
- `swarmvector_sparse_norm(sparse) -> real` - L2 norm

**Sparsification Functions**:
- `swarmvector_sparse_top_k(sparse, k) -> sparsevec`
- `swarmvector_sparse_prune(sparse, threshold) -> sparsevec`

**BM25 Function**:
- `swarmvector_sparse_bm25(query, doc, doc_len, avg_len, k1, b) -> real`
...
SELECT swarmvector_sparse_euclidean(a, b);

-- Utility functions
SELECT swarmvector_sparse_nnz(sparse_vec);
SELECT swarmvector_sparse_dim(sparse_vec);
SELECT swarmvector_sparse_norm(sparse_vec);

-- Sparsification
SELECT swarmvector_sparse_top_k(sparse_vec, 100);
SELECT swarmvector_sparse_prune(sparse_vec, 0.1);
```
...
SELECT id, content,
       swarmvector_sparse_dot(sparse_embedding, '{1:0.5, 2:0.3}'::sparsevec) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
```
...
SELECT id, content,
       swarmvector_sparse_dot(splade_embedding, query_splade) AS relevance
FROM documents
...
SELECT id, content,
       ...
       0.3 * swarmvector_sparse_dot(sparse_embedding, query_sparse) AS hybrid_score
FROM documents
...
CREATE INDEX ON documents USING swarmvector_sparse_ivf (
...
```bash
# Standard Rust tests
cargo test --package swarmvector-postgres --lib sparse
...
# In PostgreSQL
CREATE EXTENSION swarmvector_postgres;
...
✅ **Complete implementation** of sparse vectors for swarmvector-postgres
...
```
/workspaces/swarmvector/crates/swarmvector-postgres/
├── src/
│   └── sparse/
│       ├── mod.rs           (30 lines)
│       ├── types.rs         (391 lines)
