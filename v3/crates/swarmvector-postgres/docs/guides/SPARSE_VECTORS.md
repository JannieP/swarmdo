...
VALUES (
    swarmvector_dense_to_sparse(ARRAY[0, 0.5, 0, 0.3, 0]::real[])
);
```
...
SELECT id, content,
       swarmvector_sparse_dot(sparse_embedding, query_vec) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
...
FROM documents
WHERE swarmvector_sparse_cosine(sparse_embedding, query_vec) > 0.5;

-- Euclidean distance
SELECT id,
       swarmvector_sparse_euclidean(sparse_embedding, query_vec) AS distance
FROM documents
...
-- Manhattan distance
SELECT id,
       swarmvector_sparse_manhattan(sparse_embedding, query_vec) AS distance
FROM documents
ORDER BY distance ASC
LIMIT 10;
```
...
SELECT swarmvector_sparse_nnz(sparse_embedding) FROM documents;

-- Get dimension
SELECT swarmvector_sparse_dim(sparse_embedding) FROM documents;

-- Get L2 norm
SELECT swarmvector_sparse_norm(sparse_embedding) FROM documents;

-- Keep top-k elements by magnitude
SELECT swarmvector_sparse_top_k(sparse_embedding, 100) FROM documents;

-- Prune elements below threshold
SELECT swarmvector_sparse_prune(sparse_embedding, 0.1) FROM documents;
...
```rust
use swarmvector_postgres::sparse::SparseVec;
...
```rust
use swarmvector_postgres::sparse::distance::*;
...
-- Efficient search
SELECT id, content,
       swarmvector_sparse_dot(splade_vec, query_splade) AS score
FROM documents
ORDER BY score DESC
LIMIT 10;
```
...
```rust
use swarmvector_postgres::sparse::types::SparseError;
...
UPDATE documents
SET sparse_embedding = swarmvector_dense_to_sparse(dense_embedding);

-- Only keep significant elements
UPDATE documents
SET sparse_embedding = swarmvector_sparse_prune(sparse_embedding, 0.1);

-- Further compress with top-k
UPDATE documents
SET sparse_embedding = swarmvector_sparse_top_k(sparse_embedding, 100);
```
