...
|-----------------------------|---------|----------------------------------|
| `swarmvector.ivfflat_probes`   | 1       | Number of lists to probe         |
...
# Rust tests
cargo test --package swarmvector-postgres --lib index::ivfflat_am
```
...
FROM pg_opclass
WHERE opcname LIKE 'swarmvector_ivfflat%';

-- Get statistics
SELECT * FROM swarmvector_ivfflat_stats('your_index_name');
```
