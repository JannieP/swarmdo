...
## Why SwarmVector vs pgvector?

| Feature | pgvector | SwarmVector-Postgres |
|---------|----------|-------------------|
...
```sql
SELECT swarmvector_poincare_distance(a, b, -1.0);
SELECT swarmvector_mobius_add(a, b, -1.0);
```
...
SELECT swarmvector_bm25_score(query, doc_freqs, doc_len, avg_len, total);
```
...
SELECT swarmvector_attention_multi_head(query, keys, values, 8);
```
...
SELECT swarmvector_gnn_graphsage_layer(features, neighbors, weights);
```
...
CREATE TABLE taxonomy_nodes (
    ...
    name TEXT,
    parent_id INTEGER,
    embedding swarmvector(128)  -- Poincare embeddings
);
...
SELECT swarmvector_register_agent('code_expert', ARRAY['coding', 'debugging'], $embedding);
SELECT swarmvector_register_agent('math_expert', ARRAY['math', 'statistics'], $embedding);
...
SET swarmvector.ivfflat_probes = 10;
```
...
|----------|---------|-------------|
| `POSTGRES_USER` | swarmvector | Database user |
| `POSTGRES_PASSWORD` | swarmvector | Database password |
| `POSTGRES_DB` | swarmvector_test | Default database |
...
swarmvector-pg install --method docker
