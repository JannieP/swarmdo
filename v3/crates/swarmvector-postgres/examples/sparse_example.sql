...
SELECT d.id, d.title,
       swarmvector_sparse_dot(d.sparse_embedding, q.query_vec) AS dot_product,
       swarmvector_sparse_cosine(d.sparse_embedding, q.query_vec) AS cosine_sim,
       swarmvector_sparse_euclidean(d.sparse_embedding, q.query_vec) AS euclidean_dist
...
SELECT id, title,
       swarmvector_sparse_cosine(sparse_embedding, query_vec) AS similarity
FROM sparse_documents, query
WHERE swarmvector_sparse_cosine(sparse_embedding, query_vec) > 0.3
...
SELECT id, title,
       ...
       swarmvector_sparse_top_k(sparse_embedding, 2) AS top_2_elements
FROM sparse_documents
LIMIT 3;
...
SELECT id, title,
       sparse_embedding AS original,
       swarmvector_sparse_prune(sparse_embedding, 0.4) AS pruned
FROM sparse_documents
LIMIT 3;
...
SELECT id, title,
       swarmvector_sparse_to_dense(sparse_embedding) AS dense_array
FROM sparse_documents
LIMIT 3;
...
SELECT 
    ...
    AVG(swarmvector_sparse_nnz(sparse_embedding)) AS avg_nonzero,
    MIN(swarmvector_sparse_nnz(sparse_embedding)) AS min_nonzero,
    MAX(swarmvector_sparse_nnz(sparse_embedding)) AS max_nonzero,
    AVG(swarmvector_sparse_norm(sparse_embedding)) AS avg_norm
FROM sparse_documents;

-- Find documents with similar sparsity
WITH target AS (
    SELECT sparse_embedding, swarmvector_sparse_nnz(sparse_embedding) AS target_nnz
    FROM sparse_documents
    WHERE id = 1
)
SELECT d.id, d.title,
       swarmvector_sparse_nnz(d.sparse_embedding) AS doc_nnz,
       ABS(swarmvector_sparse_nnz(d.sparse_embedding) - t.target_nnz) AS nnz_diff
...
SELECT id, title,
       ...
       pg_column_size(sparse_embedding)::float / 
           GREATEST(swarmvector_sparse_nnz(sparse_embedding), 1) AS bytes_per_element
FROM sparse_documents
