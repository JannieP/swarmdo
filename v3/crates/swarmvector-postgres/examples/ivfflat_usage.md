...
SET swarmvector.ivfflat_probes = 1;

-- Balanced search (medium recall ~85%)
SET swarmvector.ivfflat_probes = 5;
...
-- Verify setting
SHOW swarmvector.ivfflat_probes;

-- Reset to default
RESET swarmvector.ivfflat_probes;
```
...
```sql
BEGIN;
SET LOCAL swarmvector.ivfflat_probes = 15;
...
ORDER BY embedding <-> '[0.1, 0.2, ...]'::vector
LIMIT 10
SETTINGS (swarmvector.ivfflat_probes = 10);
```
...
```sql
-- Get index metadata
SELECT * FROM swarmvector_ivfflat_stats('products_embedding_idx');
...
```sql
-- Increase probes
SET swarmvector.ivfflat_probes = 15;
