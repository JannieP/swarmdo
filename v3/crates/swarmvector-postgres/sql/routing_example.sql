...
SELECT swarmvector_update_agent_metrics(
    'gpt-4',
    450.0,
    true,
    0.92
...
SELECT * FROM swarmvector_find_agents_by_capability('writing', 5);
SELECT * FROM swarmvector_find_agents_by_capability('fast', 5);

-- Example 15: Get detailed agent information
SELECT swarmvector_get_agent('gpt-4') AS agent_details;
SELECT swarmvector_get_agent('claude-3-opus') AS agent_details;

-- Example 16: View routing statistics
SELECT swarmvector_routing_stats() AS stats;
...
SELECT
    ...
    COALESCE(
        (SELECT swarmvector_route(r.embedding, 'cost', '{"max_cost": 0.01, "min_quality": 0.8}'::jsonb)),
        -- Fallback to balanced if no cheap option
        swarmvector_route(r.embedding, 'balanced', '{"max_cost": 0.05}'::jsonb)
    ) AS routing_decision
FROM ai_requests r;
...
SELECT
    ...
    (swarmvector_route(r.embedding, 'quality', '{"excluded_agents": ["claude-3-opus"]}'::jsonb))::jsonb->>'expected_quality' AS a_score,
    (swarmvector_route(r.embedding, 'quality', '{"excluded_agents": ["gpt-4"]}'::jsonb))::jsonb->>'expected_quality' AS b_score
FROM ai_requests r
WHERE created_at > NOW() - INTERVAL '1 hour'
LIMIT 100;
...
UPDATE swarmvector_list_agents()
SET is_active = false
WHERE success_rate < 0.80;

-- Example 23: Remove inactive agents
SELECT swarmvector_remove_agent(name)
FROM swarmvector_list_agents()
WHERE NOT is_active
