...
CREATE OPERATOR CLASS swarmvector_ivfflat_l2_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <-> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 swarmvector_l2_distance(vector, vector);

-- Inner product distance operator class
CREATE OPERATOR CLASS swarmvector_ivfflat_ip_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <#> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 swarmvector_ip_distance(vector, vector);

-- Cosine distance operator class
CREATE OPERATOR CLASS swarmvector_ivfflat_cosine_ops
    FOR TYPE vector USING ruivfflat AS
    OPERATOR 1 <=> (vector, vector) FOR ORDER BY float_ops,
    FUNCTION 1 swarmvector_cosine_distance(vector, vector);

-- Helper function to get IVFFlat index statistics
CREATE OR REPLACE FUNCTION swarmvector_ivfflat_stats(index_name text)
RETURNS TABLE(
    lists integer,
    probes integer,
    dimensions integer,
...
--
-- SELECT * FROM swarmvector_ivfflat_stats('items_embedding_idx');
