...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}  SwarmVector Comprehensive Benchmark Suite${NC}"
echo -e "${BLUE}==================================================${NC}"
echo ""
...
if command -v psql &> /dev/null; then
    ...
    if psql -lqt | cut -d \| -f 1 | grep -qw swarmvector_bench; then
        echo -e "${YELLOW}5. Quick SQL benchmark${NC}"
        psql -d swarmvector_bench -f "${BENCHMARK_DIR}/sql/quick_benchmark.sql" | tee "${RESULTS_DIR}/sql_quick_${TIMESTAMP}.txt"

        echo -e "${YELLOW}6. Full workload benchmark${NC}"
        echo -e "${RED}Warning: This may take several minutes...${NC}"
        psql -d swarmvector_bench -f "${BENCHMARK_DIR}/sql/benchmark_workload.sql" | tee "${RESULTS_DIR}/sql_workload_${TIMESTAMP}.txt"
    else
        echo -e "${YELLOW}Skipping SQL benchmarks (database 'swarmvector_bench' not found)${NC}"
        echo -e "${YELLOW}To run SQL benchmarks:${NC}"
        echo -e "  createdb swarmvector_bench"
        echo -e "  psql -d swarmvector_bench -c 'CREATE EXTENSION swarmvector;'"
        echo -e "  psql -d swarmvector_bench -c 'CREATE EXTENSION pgvector;'"
    fi
else
