...
use swarmvector_postgres::distance::DistanceMetric;
use swarmvector_postgres::types::{BinaryVec, ProductVec, SwarmVector, ScalarVec};
...
fn bench_sq8_distance(c: &mut Criterion) {
    ...
    for dims in [128, 384, 768, 1536, 3072].iter() {
        ...
        let a_exact = SwarmVector::from_slice(&a_data);
        let b_exact = SwarmVector::from_slice(&b_data);
...
fn bench_sq8_search(c: &mut Criterion) {
    ...
    for dims in [128, 768, 1536].iter() {
        ...
        // Exact search
        let exact_vecs: Vec<SwarmVector> = vectors.iter().map(|v| SwarmVector::from_slice(v)).collect();

        let exact_query = SwarmVector::from_slice(&query);
...
fn bench_quantization_tradeoff(c: &mut Criterion) {
    ...
    let ground_truth: Vec<Vec<usize>> = queries
        .iter()
        .map(|query| {
            let query_vec = SwarmVector::from_slice(query);
            let mut distances: Vec<(usize, f32)> = exact_vecs
                .iter()
                .enumerate()
                .map(|(id, vec)| {
