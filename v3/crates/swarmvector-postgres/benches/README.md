# SwarmVector Benchmark Suite

Comprehensive benchmarks comparing swarmvector vs pgvector across multiple dimensions.
...
```bash
# Setup database
createdb swarmvector_bench
psql -d swarmvector_bench -c 'CREATE EXTENSION swarmvector;'
psql -d swarmvector_bench -c 'CREATE EXTENSION pgvector;'

# Quick benchmark (10k vectors)
psql -d swarmvector_bench -f benches/sql/quick_benchmark.sql

# Full workload (1M vectors)
psql -d swarmvector_bench -f benches/sql/benchmark_workload.sql
```
...
- swarmvector vs pgvector comparison
...
Same as swarmvector project - MIT
