#!/usr/bin/env bash
# SwarmVector-Postgres Test Runner Script
...
cd /app/crates/swarmvector-postgres 2>/dev/null || cd /app
...
<testsuites name="swarmvector-postgres" tests="${TESTS_RUN}" failures="${TESTS_FAILED}" time="${DURATION}">
  <testsuite name="cargo-test" tests="${TESTS_RUN}" failures="${TESTS_FAILED}" time="${DURATION}">
    <testcase name="pgrx-tests" classname="swarmvector_postgres" time="${DURATION}">
