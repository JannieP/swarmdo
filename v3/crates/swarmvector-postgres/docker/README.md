# SwarmVector-Postgres v2 Docker
...
docker pull swarmvector/swarmvector-postgres:latest

# Or specific PostgreSQL version
docker pull swarmvector/swarmvector-postgres:2.0.0-pg17
docker pull swarmvector/swarmvector-postgres:2.0.0-pg16
docker pull swarmvector/swarmvector-postgres:2.0.0-pg15
docker pull swarmvector/swarmvector-postgres:2.0.0-pg14
...
docker run -d \
  ...
  swarmvector/swarmvector-postgres:latest

# Connect and test
psql -h localhost -U swarmvector -d swarmvector_test -c "SELECT swarmvector_version();"
```
...
| **SIMD** | AVX2/AVX512/NEON acceleration | `swarmvector_simd_info()` |
...
| **Flash Attention** | Memory-efficient attention | `swarmvector_flash_attention()` |
| **Multi-Head Attention** | Transformer attention | `swarmvector_multi_head_attention()` |
...
- `-d, --database DB` - PostgreSQL database (default: swarmvector_dev)
...
**Services:**
- `postgres` - PostgreSQL 16 with swarmvector extension
...
POSTGRES_DB=swarmvector_dev    # Database name
```
...
POSTGRES_USER=swarmvector      # PostgreSQL user
POSTGRES_PASSWORD=swarmvector  # PostgreSQL password
POSTGRES_DB=swarmvector_test   # Test database name
...
./dev.sh psql
# DROP EXTENSION swarmvector_postgres CASCADE;
# CREATE EXTENSION swarmvector_postgres;
```
...
# View logs
docker logs swarmvector-postgres-test
```
...
```yaml
name: Test SwarmVector-Postgres
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: |
          cd crates/swarmvector-postgres/docker
          ./run-tests.sh
```
