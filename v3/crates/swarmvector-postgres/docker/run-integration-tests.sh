#!/usr/bin/env bash
# SwarmVector-Postgres Integration Test Runner
...
POSTGRES_CONTAINER="swarmvector-postgres-integration"
TEST_RUNNER_CONTAINER="swarmvector-integration-runner"
...
wait_for_postgres() {
    ...
    while [ ${attempt} -le ${max_attempts} ]; do
        if docker exec "${POSTGRES_CONTAINER}" pg_isready -U swarmvector -d swarmvector_test &>/dev/null; then
...
verify_extension() {
    log_info "Verifying SwarmVector extension..."

    docker exec "${POSTGRES_CONTAINER}" psql -U swarmvector -d swarmvector_test -c "
        SELECT swarmvector_version();
        SELECT swarmvector_simd_info();
    " || {
        log_error "Failed to verify SwarmVector extension"
        return 1
    }

    log_success "SwarmVector extension verified"
}

build_extension() {
    log_section "Building SwarmVector Extension"

    cd "${PROJECT_ROOT}"

    DOCKER_BUILDKIT=1 docker build \
        -f crates/swarmvector-postgres/docker/Dockerfile \
        -t "swarmvector-postgres:pg${PG_VERSION}-test" \
...
start_postgres() {
    ...
    docker run -d \
        ...
        -e POSTGRES_USER=swarmvector \
        -e POSTGRES_PASSWORD=swarmvector \
        -e POSTGRES_DB=swarmvector_test \
        -p 5433:5432 \
        --health-cmd="pg_isready -U swarmvector -d swarmvector_test" \
        --health-interval=5s \
        --health-timeout=5s \
        --health-retries=10 \
        "swarmvector-postgres:pg${PG_VERSION}-test"
...
setup_test_schema() {
    ...
    docker exec "${POSTGRES_CONTAINER}" psql -U swarmvector -d swarmvector_test << 'EOF'
...
-- Grant permissions
GRANT ALL ON ALL SCHEMAS IN DATABASE swarmvector_test TO swarmvector;
...
run_integration_tests() {
    ...
    export DATABASE_URL="postgresql://swarmvector:swarmvector@localhost:5433/swarmvector_test"
...
EOF
    ...
    docker exec "${POSTGRES_CONTAINER}" psql -U swarmvector -d swarmvector_test \
...
generate_report() {
    ...
    cat > "${report_file}" << EOF
# SwarmVector Postgres Integration Test Report
...
main() {
    ...
    log_section "SwarmVector Integration Test Suite"
