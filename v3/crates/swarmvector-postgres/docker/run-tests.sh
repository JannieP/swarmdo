#!/usr/bin/env bash
# SwarmVector-Postgres Test Runner
...
CONTAINER_NAME="swarmvector-postgres-test"
IMAGE_NAME="swarmvector-postgres:test"
POSTGRES_PORT="${POSTGRES_PORT:-5433}"
POSTGRES_USER="${POSTGRES_USER:-swarmvector}"
POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-swarmvector}"
POSTGRES_DB="${POSTGRES_DB:-swarmvector_test}"
...
run_integration_tests() {
    ...
    docker exec -it "${CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -c "CREATE EXTENSION IF NOT EXISTS swarmvector_postgres;" || {
...
Environment Variables:
    ...
    POSTGRES_USER          PostgreSQL user (default: swarmvector)
    POSTGRES_PASSWORD      PostgreSQL password (default: swarmvector)
    POSTGRES_DB            PostgreSQL database (default: swarmvector_test)
...
main() {
    ...
    log_info "SwarmVector-Postgres Test Runner"
    log_info "Platform: ${PLATFORM}"
