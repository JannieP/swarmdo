#!/bin/bash
#
# SwarmVector PostgreSQL Extension Installer
...
#   --uninstall             Uninstall SwarmVector
...
SWARMVECTOR_VERSION="0.1.0"
EXTENSION_NAME="swarmvector"
...
build_from_source() {
    log_info "Building SwarmVector from source..."
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        cd "$PROJECT_ROOT/crates/swarmvector-postgres"
        cargo pgrx package --pg-config "$PG_CONFIG"
    fi

    # Set build output path
    BUILD_OUTPUT="$PROJECT_ROOT/target/release/swarmvector-pg${PG_VERSION}"

    log_success "Build completed"
}

install_extension() {
    log_info "Installing SwarmVector extension..."

    local SO_FILE="${BUILD_OUTPUT}/usr/lib/postgresql/${PG_VERSION}/lib/swarmvector.so"
    local CONTROL_FILE="${BUILD_OUTPUT}/usr/share/postgresql/${PG_VERSION}/extension/swarmvector.control"
    local SQL_FILE="${PROJECT_ROOT}/crates/swarmvector-postgres/sql/swarmvector--${SWARMVECTOR_VERSION}.sql"
    ...
    run_cmd chmod 755 "$PG_LIBDIR/swarmvector.so"
...
create_config() {
    ...
    local CONFIG_OUT="$CONFIG_DIR/swarmvector.conf"
    ...
    cat > "$CONFIG_OUT" << EOF
# SwarmVector PostgreSQL Extension Configuration
...
#swarmvector.simd_mode = 'auto'

# =============================================================================
# Memory Configuration
# =============================================================================
# Maximum memory for vector operations (in MB)
#swarmvector.max_memory_mb = 1024

# Enable memory pooling for better performance
#swarmvector.enable_memory_pool = on

# =============================================================================
# Index Configuration
# =============================================================================
# Default HNSW index parameters
#swarmvector.hnsw_ef_construction = 64
#swarmvector.hnsw_m = 16
#swarmvector.hnsw_ef_search = 40

# Default IVF-Flat index parameters
#swarmvector.ivfflat_lists = 100
#swarmvector.ivfflat_probes = 10

# =============================================================================
# Distance Calculation
# =============================================================================
# Enable parallel distance computation for large batches
#swarmvector.parallel_distance = on

# Minimum batch size for parallel processing
#swarmvector.parallel_min_batch = 1000

# =============================================================================
# Quantization
# =============================================================================
# Enable product quantization for large datasets
#swarmvector.enable_pq = off

# Product quantization parameters
#swarmvector.pq_m = 8
#swarmvector.pq_nbits = 8

# =============================================================================
# Logging
# =============================================================================
# Log level: debug, info, warning, error
#swarmvector.log_level = 'info'

# Log SIMD operations (for debugging)
#swarmvector.log_simd = off
EOF
...
run_tests() {
    ...
    local TEST_DB="swarmvector_test_$$"
...
-- SwarmVector Installation Test Suite

-- Test 1: Create extension
CREATE EXTENSION IF NOT EXISTS swarmvector;
SELECT 'Test 1: Extension created' AS result;

-- Test 2: Create table with swarmvector column
CREATE TABLE test_vectors (id serial PRIMARY KEY, embedding swarmvector);
...
uninstall_extension() {
    log_info "Uninstalling SwarmVector extension..."

    # Remove files
    local files_to_remove=(
        "$PG_LIBDIR/swarmvector.so"
        "$PG_SHAREDIR/extension/swarmvector.control"
        "$PG_SHAREDIR/extension/swarmvector--${SWARMVECTOR_VERSION}.sql"
        "$PG_SHAREDIR/extension/swarmvector.conf"
    )
    ...
    log_success "SwarmVector uninstalled"
    log_warning "Note: You may need to DROP EXTENSION swarmvector in databases where it was created"
}
...
show_help() {
    cat << EOF
SwarmVector PostgreSQL Extension Installer v${SWARMVECTOR_VERSION}
...
main() {
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║         SwarmVector PostgreSQL Extension Installer               ║"
    echo "║       High-Performance Vector Similarity Search               ║"
    echo "║                    Version ${SWARMVECTOR_VERSION}                             ║"
    echo "╚═══════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo ""
    log_success "SwarmVector installation complete!"
    echo ""
    echo "Next steps:"
    echo "  1. Connect to your database: psql -d your_database"
    echo "  2. Create the extension: CREATE EXTENSION swarmvector;"
    echo "  3. Create a table with vectors:"
    echo "     CREATE TABLE items (id serial, embedding swarmvector);"
