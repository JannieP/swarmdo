...
function loadEnvConfig(): Partial<SystemConfig> {
  ...
  if (process.env.SWARMDO_MAX_AGENTS) {
    config.orchestrator = {
      ...defaultSystemConfig.orchestrator,
      lifecycle: {
        ...defaultSystemConfig.orchestrator.lifecycle,
        maxConcurrentAgents: parseInt(process.env.SWARMDO_MAX_AGENTS, 10),
      },
    };
  }

  // Data directory
  if (process.env.SWARMDO_DATA_DIR) {
    config.orchestrator = {
      ...config.orchestrator,
      ...defaultSystemConfig.orchestrator,
      session: {
        ...defaultSystemConfig.orchestrator.session,
        dataDir: process.env.SWARMDO_DATA_DIR,
      },
    };
  }

  // Memory type
  if (process.env.SWARMDO_MEMORY_TYPE) {
    const memoryType = process.env.SWARMDO_MEMORY_TYPE as NonNullable<SystemConfig['memory']>['type'];
  ...
  // MCP transport
  const defaultMcp = defaultSystemConfig.mcp ?? { name: 'swarmdo', version: '3.0.0', transport: { type: 'stdio' as const } };
  if (process.env.SWARMDO_MCP_TRANSPORT) {
    const transport = process.env.SWARMDO_MCP_TRANSPORT as 'stdio' | 'http' | 'websocket';
  ...
  if (process.env.SWARMDO_MCP_PORT) {
    config.mcp = {
      ...config.mcp,
      ...defaultMcp,
      transport: {
        ...
        ...defaultMcp.transport,
        port: parseInt(process.env.SWARMDO_MCP_PORT, 10),
      },
    };
  }
  ...
  if (process.env.SWARMDO_SWARM_TOPOLOGY) {
    const topology = process.env.SWARMDO_SWARM_TOPOLOGY as NonNullable<SystemConfig['swarm']>['topology'];
...
export class ConfigLoader {
  ...
  constructor(additionalPaths?: string[]) {
    ...
    this.searchPaths = [
      process.cwd(),
      resolve(process.cwd(), '..'),
      resolve(process.env.HOME ?? '', '.swarmdo'),
    ];
