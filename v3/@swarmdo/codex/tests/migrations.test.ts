...
const SAMPLE_CLAUDE_MD = `# Swarmdo V3 Project
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('args = ["-y", "@swarmdo/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default swarmdo server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default swarmdo server when none specified
    expect(result).toContain('[mcp_servers.swarmdo]');
    expect(result).toContain('command = "npx"');
  });
