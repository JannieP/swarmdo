/**
 * @swarmdo/codex - CodexInitializer
 *
...
export class CodexInitializer {
  ...
  async initialize(options: CodexInitOptions): Promise<CodexInitResult> {
    ...
    try {
      ...
      if (mcpResult.registered) {
        filesCreated.push('MCP server (swarmdo) registered');
      }
  ...
  /**
   * Register swarmdo as MCP server with Codex
   */
  private async registerMCPServer(): Promise<{ registered: boolean; warning?: string }> {
    try {
      ...
      try {
        ...
        return {
          registered: false,
          warning: 'Codex CLI not found. Run: codex mcp add swarmdo -- npx swarmdo mcp start',
        };
      }
      ...
      try {
        execSync('codex mcp add swarmdo -- npx swarmdo mcp start', {
          stdio: 'pipe',
          timeout: 10000,
        });
        ...
        return {
          registered: false,
          warning: `Failed to register MCP server: ${errorMessage}. Run manually: codex mcp add swarmdo -- npx swarmdo mcp start`,
        };
      }
    } catch {
      return {
        registered: false,
        warning: 'Could not register MCP server. Run manually: codex mcp add swarmdo -- npx swarmdo mcp start',
      };
    }
  }
...
*Generated by @swarmdo/codex - Dual platform mode*
`;
