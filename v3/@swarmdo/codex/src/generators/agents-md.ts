/**
 * @swarmdo/codex - AGENTS.md Generator
 *
...
function generateMinimal(options: AgentsMdOptions): string {
  const {
    projectName,
    description = 'A Swarmdo powered project',
...
- **swarmdo** = LEDGER (coordinates: memory, routing, swarm state)
- **Codex** = EXECUTOR (writes code, runs tests, creates files)

**Critical rule:** DON'T STOP after calling swarmdo commands. Coordination commands return instantly — continue immediately with the next implementation step.
...
Co-Authored-By: swarmdo-bot <swarmdo-bot@users.noreply.github.com>
\`\`\`
...
\`\`\`bash
npx @swarmdo/cli memory search --query "relevant patterns"
npx @swarmdo/cli hooks route --task "current task description"
npx @swarmdo/cli swarm init --topology hierarchical
npx @swarmdo/cli hooks pre-task --description "task summary"
\`\`\`
...
Swarmdo exposes tools via Model Context Protocol:
...
Swarmdo uses hooks for lifecycle automation:
...
npx @swarmdo/cli hooks pre-task \\
  --description "implementing authentication"

# After completing a task
npx @swarmdo/cli hooks post-task \\
  --task-id "task-123" \\
  --success true

# Route a task to agents
npx @swarmdo/cli hooks route \\
...
\`\`\`bash
# List workers
npx @swarmdo/cli hooks worker list

# Trigger specific worker
npx @swarmdo/cli hooks worker dispatch --trigger audit

# Check worker status
npx @swarmdo/cli hooks worker status
\`\`\`

## Intelligence System

The SwarmVector Intelligence System provides neural learning:
...
\`\`\`bash
# Set log level
export SWARMDO_LOG_LEVEL=debug

# Enable verbose mode
npx @swarmdo/cli --verbose <command>
\`\`\`

### Health Checks
\`\`\`bash
# Run diagnostics
npx @swarmdo/cli doctor --fix

# Check system status
npx @swarmdo/cli status
\`\`\`
`;
...
npx @swarmdo/cli logs --type agent-actions --last 1h

# Export audit log
npx @swarmdo/cli logs export --format json --output audit.json
\`\`\`
...
\`\`\`bash
# Check current role
npx @swarmdo/cli claims list

# Request elevated permissions
npx @swarmdo/cli claims request --permission deploy:production
\`\`\`
...
1. **Contain** - Immediately stop affected agents
   \`\`\`bash
   npx @swarmdo/cli agent stop --all --force
   \`\`\`
...
2. **Decide** - Roll back if safe, or forward-fix
   \`\`\`bash
   # Rollback
   npx @swarmdo/cli deployment rollback --env production

   # Or forward-fix
   npx @swarmdo/cli workflow run hotfix
   \`\`\`
...
\`\`\`bash
# Restore from backup
npx @swarmdo/cli memory restore --snapshot latest

# Restore specific checkpoint
npx @swarmdo/cli session restore --checkpoint <id>
\`\`\`
...
- [ ] Run \`npx @swarmdo/cli doctor\` to verify setup
