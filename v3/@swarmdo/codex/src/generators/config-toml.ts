/**
 * @swarmdo/codex - config.toml Generator
 *
...
export async function generateConfigToml(options: ExtendedConfigTomlOptions = {}): Promise<string> {
  ...
  // Header
  lines.push('# =============================================================================');
  lines.push('# Swarmdo V3 - Codex Configuration');
  lines.push('# =============================================================================');
  lines.push('# Generated by: @swarmdo/codex');
  lines.push('# Documentation: the upstream project (see NOTICE)');
  lines.push('# ');
  lines.push('# This file configures the Codex CLI for Swarmdo integration.');
  ...
  // MCP Servers
  if (mcpServers.length > 0 || true) { // Always include swarmdo by default
    lines.push('# =============================================================================');
    ...
    const hasSwarmdo = mcpServers.some(s => s.name === 'swarmdo' || s.name === 'swarmdo');
    if (!hasSwarmdo) {
      lines.push(...generateMcpServer({
        name: 'swarmdo',
        command: 'npx',
        args: ['-y', 'swarmdo@latest', 'mcp', 'start'],
        enabled: true,
        toolTimeout: 120,
      }));
      lines.push('');
  ...
  if (logging.filePath || logging.destination === 'file' || logging.destination === 'both') {
    ...
    lines.push(`file_path = "${logging.filePath ?? './logs/swarmdo.log'}"`);
    lines.push('');
...
export async function generateMinimalConfigToml(options: ConfigTomlOptions = {}): Promise<string> {
  ...
  return `# Swarmdo V3 - Minimal Codex Configuration
...
export async function generateEnterpriseConfigToml(): Promise<string> {
  return `# =============================================================================
# Swarmdo V3 - Enterprise Configuration
# =============================================================================
...
export async function generateDevConfigToml(): Promise<string> {
  return `# =============================================================================
# Swarmdo V3 - Development Configuration
# =============================================================================
...
export async function generateSecureConfigToml(): Promise<string> {
  return `# =============================================================================
# Swarmdo V3 - Security-Focused Configuration
# =============================================================================
