...
function printBanner(): void {
  ...
  console.log(chalk.gray('  OpenAI Codex integration for Swarmdo'));
  console.log(chalk.gray('  ----------------------------------------\n'));
}

program
  .name('swarmdo-codex')
  .description('OpenAI Codex integration for Swarmdo - Part of the coflow ecosystem')
  ...
  .action(async (options) => {
    try {
      ...
      if (filesToValidate.length === 0) {
        ...
        console.log(chalk.gray('Run `swarmdo-codex init` to create a project'));
        return;
      }
    ...
    try {
      ...
      if (options.analyzeOnly) {
        ...
        if (result.success) {
          ...
          console.log(chalk.gray('    3. Run `swarmdo-codex validate` to verify'));
          console.log();
        } else {
          console.log(chalk.red.bold('\n  Migration failed'));
          process.exit(1);
...
program
  ...
  .action((options) => {
    try {
      ...
      console.log(chalk.gray('Use: swarmdo-codex init --template <name>'));
      console.log();
    } catch (error) {
...
program
  ...
  .action((options) => {
    try {
      ...
      console.log(chalk.gray('Use: swarmdo-codex generate-skill -n <name> to create a custom skill'));
      console.log();
    } catch (error) {
...
program.on('command:*', () => {
  ...
  console.log(chalk.gray(`Run ${chalk.white('swarmdo-codex --help')} for available commands.`));
  process.exit(1);
});
