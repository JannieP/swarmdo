...
const WARNING_PATTERNS: Array<{ pattern: RegExp | string; message: string }> = [
  ...
  { pattern: /mcp__swarmdo__/g, message: 'MCP tool calls need migration to Codex MCP configuration' },
  { pattern: /mcp__swarmdo-swarm__/g, message: 'Swarm MCP calls - ensure swarmdo-swarm MCP server is configured in config.toml' },
];
...
export function generateAgentsMdFromParsed(parsed: ParsedClaudeMd): string {
  ...
  lines.push('');
  lines.push('> Migrated from CLAUDE.md by @swarmdo/codex');
  lines.push('');
...
export function convertSettingsToToml(settings: Record<string, unknown>): string {
  ...
  lines.push('# Generated by @swarmdo/codex');
  lines.push('');
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    ...
    // Add default swarmdo server
    lines.push('[mcp_servers.swarmdo]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "swarmdo@latest", "mcp", "start"]');
    lines.push('enabled = true');
    lines.push('');
  }
