...
<p align="center">
  <strong>OpenAI Codex CLI Adapter for Swarmdo V3</strong><br/>
...
Transform OpenAI Codex CLI into a **self-improving AI development system**. While Codex executes code, swarmdo orchestrates, coordinates, and **learns from every interaction**.

| Traditional Codex | With Swarmdo |
|-------------------|------------------|
...
npx swarmdo@alpha init --codex

# Full setup with all 137+ skills
npx swarmdo@alpha init --codex --full

# Dual mode (both Claude Code and Codex)
npx swarmdo@alpha init --dual
```
...
| `.swarmdo/data/memory.db` | Vector memory database |
...
npx swarmdo@alpha init --codex --minimal
...
Both platforms share the same `.swarmdo/` runtime:
...
The `@swarmdo/codex` package now includes built-in dual-mode orchestration:
...
npx swarmdo-codex dual run --template feature --task "Add user authentication"
...
Codex does not expose Claude Code's `ScheduleWakeup`, so `@swarmdo/codex` provides a process-based equivalent:

```bash
# Run Codex repeatedly until it creates .codex/loop/default.complete or reaches 10 iterations
npx swarmdo-codex loop run "Fix failing tests and create the completion marker when done"

# Use command mode for recurring Swarmdo workers or custom scripts
npx swarmdo-codex loop run --name testgaps --interval 270 --max-iterations 0 \
  --command "npx swarmdo hooks worker dispatch --trigger testgaps"

# Inspect or stop a loop from another terminal
npx swarmdo-codex loop status --name testgaps
npx swarmdo-codex loop stop --name testgaps
```
...
SWARMDO_MEMORY_PATH=./.swarmdo/data
...
```typescript
import { initializeCodexProject } from '@swarmdo/codex';
...
```typescript
import { CodexInitializer, initializeCodexProject } from '@swarmdo/codex';
...
```typescript
import { migrate } from '@swarmdo/codex';
...
5. **Register MCP**: `codex mcp add swarmdo -- npx swarmdo mcp start`
...
# Re-register
codex mcp remove swarmdo
codex mcp add swarmdo -- npx swarmdo mcp start

# Test connection
npx swarmdo mcp test
```
...
npx swarmdo memory list

# Manually add a test pattern
npx swarmdo memory store --key "test" --value "test pattern" --namespace patterns
```
...
# Rebuild skills
npx swarmdo@alpha init --codex --force
```
...
# Rebuild index
npx swarmdo memory optimize --rebuild-index
```
...
| Package | Description |
|---------|-------------|
| [@swarmdo/cli](https://www.npmjs.com/package/@swarmdo/cli) | Main CLI (26 commands, 140+ subcommands) |
| [swarmdo](https://www.npmjs.com/package/swarmdo) | Umbrella package |
| [@swarmdo/memory](https://www.npmjs.com/package/@swarmdo/memory) | AgentDB with HNSW vector search |
| [@swarmdo/security](https://www.npmjs.com/package/@swarmdo/security) | Security module |
