...
const TEST_PROMPT = 'Say "Hello from Swarmdo V3!" Be brief.';
...
async function testOpenRouter() {
  ...
  const provider = new OpenAIProvider({
    config: {
      ...
      providerOptions: {
        headers: {
          'HTTP-Referer': 'https://swarmdo.dev',
          'X-Title': 'Swarmdo V3 Test',
        },
      },
    },
    logger: consoleLogger,
...
async function testSwarmVector() {
  console.log('\n🔷 Testing SwarmVector (SONA + Local Qwen)...');

  const provider = new SwarmVectorProvider({
    config: {
  ...
  try {
    ...
    console.log('✅ SwarmVector Response:', response.content);
    console.log('   Tokens:', response.usage);
    ...
    if (error.message?.includes('ECONNREFUSED') || error.message?.includes('fetch failed')) {
      console.log('⏭️  Skipping SwarmVector - Ollama not running locally');
      console.log('   To test: ollama pull qwen2.5:0.5b && ollama serve');
    } else {
      console.error('❌ SwarmVector Error:', error.message);
    }
    provider.destroy();
    return null;
  }
...
async function main() {
  console.log('╔════════════════════════════════════════════════╗');
  console.log('║     Swarmdo V3 - Provider Test Suite       ║');
  console.log('╚════════════════════════════════════════════════╝');
  ...
  const results = {
    ...
    swarmvector: await testSwarmVector(),
  ...
  if (results.ollama === null && results.swarmvector === null) {
