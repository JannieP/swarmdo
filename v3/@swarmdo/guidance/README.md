...
`@swarmdo/guidance` fixes this. It takes the memory files Claude Code already uses — `CLAUDE.md` and `CLAUDE.local.md` — and turns them into a structured control plane that compiles rules, enforces them through gates the agent cannot bypass, proves every decision cryptographically, and evolves the rule set over time based on what actually works.
...
| Capability | Plain CLAUDE.md | Prompt libraries | @swarmdo/guidance |
|-----------|:-:|:-:|:-:|
...
The most important gain: **Swarmdo can now say "no" to itself and survive.** Self-limiting behavior, self-correction, and self-preservation compound over time.
...
| **Bridge** | `SwarmBotGuidanceBridge` | Wires swarmbot events to guidance hooks, AIDefence gate, memory adapter |
...
```typescript
import { getKernel } from '@swarmdo/guidance/wasm-kernel';
...
import { createCompiler } from '@swarmdo/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(claudeMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@swarmdo/guidance/retriever';
...
import { createGates } from '@swarmdo/guidance/gates';
...
```typescript
import { createContinueGate } from '@swarmdo/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@swarmdo/guidance/proof';
...
import { createToolGateway } from '@swarmdo/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@swarmdo/guidance/memory-gate';
...
import { TrustSystem } from '@swarmdo/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@swarmdo/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@swarmdo/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@swarmdo/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@swarmdo/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@swarmdo/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@swarmdo/guidance/meta-governance';
...
```typescript
import { createGuidanceHooks } from '@swarmdo/guidance';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@swarmdo/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@swarmdo/guidance/evolution';
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@swarmdo/guidance/generators';
...
} from '@swarmdo/guidance/analyzer';
...
```typescript
import type { IContentAwareExecutor } from '@swarmdo/guidance/analyzer';
...
The final proof: does the control plane actually help? The `abBenchmark()` function implements the Measurement Plan: run 20 real tasks drawn from Swarmdo repo history under two configs — **A** (no control plane) vs **B** (with Phase 1 guidance) — and compute KPIs, composite scores, and category shift detection.

```typescript
import { abBenchmark, getDefaultABTasks } from '@swarmdo/guidance/analyzer';
...
- **A**: Current Swarmdo without the wired control plane
...
Take 20 real Swarmdo tasks from repo history. Run A without control plane, run B with Phase 1 only. Success is B improves success rate and reduces tool calls per successful task, while producing replayable ledgers for every failure.
