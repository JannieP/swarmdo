 ...
 *
 * @module @swarmdo/guidance/analyzer
 */
...
/** A single benchmark task representing a real Swarmdo scenario */
...
// ── 20 Representative Tasks from Swarmdo History ───────────────────────
...
function getABTasks(): ABTask[] {
  ...
  return [
    ...
    // ── Deployment (2 tasks) ──────────────────────────────────────────
    {
      ...
      prompt: 'Create a multi-stage Dockerfile for the Swarmdo CLI. Include a build stage and a minimal runtime stage. Never include dev dependencies in production.',
      assertions: [
 ...
 *
 * The 20 tasks span 7 task classes drawn from real Swarmdo repo history:
...
export async function abBenchmark(
  ...
  if (!contentAware) {
    throw new Error(
      'abBenchmark requires a content-aware executor. The provided IHeadlessExecutor lacks `setContext()`, so Config A and Config B will both read the same on-disk CLAUDE.md and the delta is guaranteed to be zero. Either use the DefaultHeadlessExecutor (content-aware as of @swarmdo/guidance@3.0.0-alpha.2) or implement IContentAwareExecutor on your custom executor.',
    );
  }
