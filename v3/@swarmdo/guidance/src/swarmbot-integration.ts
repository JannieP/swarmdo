/**
 * SwarmBot Integration Bridge
 *
 * Bridges swarmbot (npm: swarmbot@0.1.8) with the @swarmdo/guidance control
 * plane. Wires swarmbot events to guidance hooks, wraps AIDefence as an
 * enforcement gate, governs memory operations, and feeds trust accumulation.
 *
 * swarmbot is an optional peer dependency. All types and classes are exported
 * regardless of whether swarmbot is installed. Runtime calls that require the
 * swarmbot package will throw a clear error if the package is missing.
 *
 * Components:
 * 1. SwarmBotGuidanceBridge  - Event wiring, gate delegation, trust tracking
 * 2. AIDefenceGate         - Prompt injection, jailbreak, PII detection gate
 * 3. SwarmBotMemoryAdapter   - Governed memory read/write with proof logging
 *
 * @module @swarmdo/guidance/swarmbot-integration
 */
...
// ============================================================================
// SwarmBot Ambient Types (optional peer dependency)
// ============================================================================

/**
 * Minimal interface for a swarmbot instance. Mirrors the event-emitter surface
 * exposed by `createSwarmBot()` without importing the package at compile time.
 */
export interface SwarmBotInstance {
...
/**
 * Minimal interface for swarmbot's AIDefence guard returned by
 * `createAIDefenceGuard()`.
 */
export interface SwarmBotAIDefenceGuard {
...
/**
 * Minimal interface for swarmbot's memory subsystem.
 */
export interface SwarmBotMemory {
...
/**
 * Configuration for the SwarmBotGuidanceBridge.
 */
export interface SwarmBotBridgeConfig {
...
/**
 * A normalized swarmbot event for internal processing.
 */
export interface SwarmBotEvent {
  type: string;
  timestamp: number;
  sessionId?: string;
  agentId?: string;
...
/** Maps swarmbot threat type strings to our typed threat type. */
...
/** Maps swarmbot severity strings to our typed severity. */
...
/** Resolved swarmbot module cache (null = not attempted, undefined = failed). */
let swarmbotModuleCache: Record<string, unknown> | null = null;
 ...
 */
const RUFBOT_MODULE = 'swarmbot';
const RUFBOT_CORE_MODULE = 'swarmbot/core';

/**
 * Attempt to dynamically import the swarmbot package.
 * Throws a descriptive error if the package is not installed.
 */
async function requireSwarmBot(): Promise<Record<string, unknown>> {
  if (swarmbotModuleCache) return swarmbotModuleCache;

  try {
    const mod = await import(RUFBOT_MODULE) as Record<string, unknown>;
    swarmbotModuleCache = mod;
    return mod;
  } catch {
    throw new Error(
      'swarmbot is not installed. Install it with: npm install swarmbot@0.1.8\n' +
      'swarmbot is an optional peer dependency of @swarmdo/guidance.',
    );
  }
}

/**
 * Attempt to dynamically import swarmbot/core sub-export.
 */
async function requireSwarmBotCore(): Promise<Record<string, unknown>> {
  try {
    return await import(RUFBOT_CORE_MODULE) as Record<string, unknown>;
  } catch {
    // Fall back to the main export
    return requireSwarmBot();
  }
}
...
/**
 * Wraps swarmbot's 6-layer AIDefence as an enforcement gate compatible with the
 ...
 * - Jailbreak detection
 * - PII detection
 * - Control character and homoglyph detection (via swarmbot internals)
...
export class AIDefenceGate {
  ...
  private guard: SwarmBotAIDefenceGuard | null = null;
  ...
  /**
   * Lazily initialize the underlying swarmbot AIDefence guard.
   * Safe to call multiple times; only the first call creates the guard.
   */
  private async ensureGuard(): Promise<SwarmBotAIDefenceGuard> {
    if (this.guard) return this.guard;

    if (!this.guardInitPromise) {
      this.guardInitPromise = (async () => {
        const mod = await requireSwarmBot();
        const createGuard = mod['createAIDefenceGuard'] as
          | ((config?: Record<string, unknown>) => SwarmBotAIDefenceGuard)
          | undefined;

        if (typeof createGuard !== 'function') {
          throw new Error(
            'swarmbot does not export createAIDefenceGuard. ' +
            'Ensure swarmbot@0.1.8 or later is installed.',
          );
        }
...
// ============================================================================
// SwarmBotMemoryAdapter
// ============================================================================

/**
 * Wraps swarmbot's memory read/write operations with guidance control plane
 ...
 */
export class SwarmBotMemoryAdapter {
  ...
  private swarmbotMemory: SwarmBotMemory | null = null;
  ...
  /**
   * Attach a swarmbot memory instance for proxied operations.
   */
  attachMemory(memory: SwarmBotMemory): void {
    this.swarmbotMemory = memory;
  }
  ...
  /**
   * Governed read: reads through swarmbot memory, logs to proof chain.
   */
  async read(
    key: string,
    ...
    const value = await this.swarmbotMemory!.read(key, namespace);
   ...
   * logs to proof chain, then delegates to swarmbot memory.
   *
  ...
  async write(
    ...
    // Step 4: Perform the write through swarmbot
    await this.swarmbotMemory!.write(key, value, namespace);
  ...
  async delete(
    ...
    if (typeof this.swarmbotMemory!.delete === 'function') {
      await this.swarmbotMemory!.delete(key, namespace);
    }
  ...
  private ensureMemoryAttached(): void {
    if (!this.swarmbotMemory) {
      throw new Error(
        'No swarmbot memory instance attached. Call attachMemory() before ' +
...
// ============================================================================
// SwarmBotGuidanceBridge
// ============================================================================

/**
 * Bridges a swarmbot instance with the @swarmdo/guidance control plane.
 *
 * Wires swarmbot event hooks to guidance enforcement and trust systems:
 *
 ...
 *
 * All gate outcomes are fed into the TrustAccumulator so that swarmbot agents
 * build (or lose) trust over time.
 */
export class SwarmBotGuidanceBridge {
  private readonly config: SwarmBotBridgeConfig;
  private swarmbot: SwarmBotInstance | null = null;
  ...
  private memoryAdapter: SwarmBotMemoryAdapter | null = null;
  ...
  private eventLog: SwarmBotEvent[] = [];
  private static readonly MAX_EVENT_LOG = 1000;

  constructor(config: Partial<SwarmBotBridgeConfig> = {}) {
    this.config = {
  ...
  attachGuidance(components: {
    ...
    memoryAdapter?: SwarmBotMemoryAdapter;
  }): void {
  ...
  /**
   * Connect to a swarmbot instance and wire all event handlers.
   *
   * This is the primary entry point. Once called, the bridge will
   * intercept swarmbot events and route them through guidance gates.
   */
  connect(swarmbot: SwarmBotInstance): void {
    if (this.swarmbot) {
      this.disconnect();
    }
  ...
  /**
   * Disconnect from the swarmbot instance, removing all event handlers.
   */
  disconnect(): void {
    if (!this.swarmbot) return;

    for (const [event, handler] of this.boundHandlers) {
      this.swarmbot.off(event, handler);
    }
  ...
  /**
   * Evaluate a swarmbot AIDefence result and return a GateResult-compatible
   ...
   */
  getEventLog(): ReadonlyArray<SwarmBotEvent> {
   ...
   */
  getConfig(): SwarmBotBridgeConfig {
    return { ...this.config };
  }

  /**
   * Whether the bridge is currently connected to a swarmbot instance.
   */
  get connected(): boolean {
    return this.swarmbot !== null;
  }
  ...
  private async handleSessionCreate(...args: unknown[]): Promise<void> {
    ...
    if (this.config.enableProofChain) {
      if (!this.config.proofSigningKey) {
        throw new Error(
          'SwarmBotBridgeConfig.proofSigningKey is required when enableProofChain is true',
        );
      }
  ...
  private logEvent(type: string, data: Record<string, unknown>): void {
    ...
    if (this.eventLog.length > SwarmBotGuidanceBridge.MAX_EVENT_LOG) {
      this.eventLog = this.eventLog.slice(-SwarmBotGuidanceBridge.MAX_EVENT_LOG);
    }
  }
}
...
/**
 * Create a fully wired SwarmBotGuidanceBridge.
 *
 * Connects the bridge to a swarmbot instance and attaches the guidance
 * control plane components. The bridge immediately begins intercepting
 * swarmbot events.
 *
 * @param swarmbotInstance - A swarmbot instance (from createSwarmBot())
 * @param guidancePlane - A GuidanceControlPlane or individual components
 * @param config - Optional bridge configuration
 * @returns The connected SwarmBotGuidanceBridge
 */
export function createSwarmBotBridge(
  swarmbotInstance: SwarmBotInstance,
  guidancePlane: {
    gates?: import('./gates.js').EnforcementGates;
    manifestValidator?: import('./manifest-validator.js').ManifestValidator;
    trustSystem?: import('./trust.js').TrustSystem;
  ...
  config?: Partial<SwarmBotBridgeConfig>,
): SwarmBotGuidanceBridge {
  const bridge = new SwarmBotGuidanceBridge(config);
  bridge.attachGuidance(guidancePlane);
  bridge.connect(swarmbotInstance);
  return bridge;
}
 ...
 *
 * The gate lazily initializes the underlying swarmbot AIDefence guard
...
/**
 * Create a SwarmBotMemoryAdapter with governance components.
 *
 * The adapter wraps swarmbot memory operations with MemoryWriteGate authority
 ...
 * @returns A new SwarmBotMemoryAdapter instance
 */
export function createSwarmBotMemoryAdapter(
  memoryGate: import('./memory-gate.js').MemoryWriteGate,
  coherenceScheduler: import('./coherence.js').CoherenceScheduler,
): SwarmBotMemoryAdapter {
  return new SwarmBotMemoryAdapter(memoryGate, coherenceScheduler);
}
