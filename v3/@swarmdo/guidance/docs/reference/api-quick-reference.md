...
All exports from `@swarmdo/guidance`. Each module is also available as a standalone import.

## Import Map

| Import Path | Key Exports |
|-------------|-------------|
| `@swarmdo/guidance` | `GuidanceControlPlane`, `createGuidanceControlPlane` |
| `@swarmdo/guidance/compiler` | `GuidanceCompiler`, `createCompiler` |
| `@swarmdo/guidance/retriever` | `ShardRetriever`, `createRetriever`, `HashEmbeddingProvider` |
| `@swarmdo/guidance/gates` | `EnforcementGates`, `createGates` |
| `@swarmdo/guidance/hooks` | `GuidanceHookProvider`, `createGuidanceHooks` |
| `@swarmdo/guidance/ledger` | `RunLedger`, `createLedger`, `TestsPassEvaluator`, `ForbiddenCommandEvaluator`, `ForbiddenDependencyEvaluator`, `ViolationRateEvaluator`, `DiffQualityEvaluator` |
| `@swarmdo/guidance/optimizer` | `OptimizerLoop`, `createOptimizer` |
| `@swarmdo/guidance/persistence` | `PersistentLedger`, `EventStore`, `createPersistentLedger`, `createEventStore` |
| `@swarmdo/guidance/headless` | `HeadlessRunner`, `createHeadlessRunner`, `createComplianceSuite` |
| `@swarmdo/guidance/gateway` | `DeterministicToolGateway`, `createToolGateway` |
| `@swarmdo/guidance/artifacts` | `ArtifactLedger`, `createArtifactLedger` |
| `@swarmdo/guidance/evolution` | `EvolutionPipeline`, `createEvolutionPipeline` |
| `@swarmdo/guidance/manifest-validator` | `ManifestValidator`, `ConformanceSuite`, `createManifestValidator`, `createConformanceSuite` |
| `@swarmdo/guidance/proof` | `ProofChain`, `createProofChain` |
| `@swarmdo/guidance/memory-gate` | `MemoryWriteGate`, `createMemoryWriteGate`, `createMemoryEntry` |
| `@swarmdo/guidance/coherence` | `CoherenceScheduler`, `EconomicGovernor`, `createCoherenceScheduler`, `createEconomicGovernor` |
| `@swarmdo/guidance/capabilities` | `CapabilityAlgebra`, `createCapabilityAlgebra` |
| `@swarmdo/guidance/conformance-kit` | `SimulatedRuntime`, `MemoryClerkCell`, `ConformanceRunner`, `createMemoryClerkCell`, `createConformanceRunner` |
| `@swarmdo/guidance/swarmbot-integration` | `SwarmBotGuidanceBridge`, `AIDefenceGate`, `SwarmBotMemoryAdapter`, `createSwarmBotBridge`, `createAIDefenceGate`, `createSwarmBotMemoryAdapter` |
| `@swarmdo/guidance/meta-governance` | `MetaGovernor`, `createMetaGovernor` |
| `@swarmdo/guidance/adversarial` | `ThreatDetector`, `CollusionDetector`, `MemoryQuorum`, `createThreatDetector`, `createCollusionDetector`, `createMemoryQuorum` |
| `@swarmdo/guidance/trust` | `TrustAccumulator`, `TrustScoreLedger`, `TrustSystem`, `getTrustBasedRateLimit`, `createTrustAccumulator`, `createTrustSystem` |
| `@swarmdo/guidance/truth-anchors` | `TruthAnchorStore`, `TruthResolver`, `createTruthAnchorStore`, `createTruthResolver` |
| `@swarmdo/guidance/uncertainty` | `UncertaintyLedger`, `UncertaintyAggregator`, `createUncertaintyLedger`, `createUncertaintyAggregator` |
| `@swarmdo/guidance/temporal` | `TemporalStore`, `TemporalReasoner`, `createTemporalStore`, `createTemporalReasoner` |
| `@swarmdo/guidance/authority` | `AuthorityGate`, `IrreversibilityClassifier`, `createAuthorityGate`, `createIrreversibilityClassifier`, `isHigherAuthority`, `getAuthorityHierarchy` |
| `@swarmdo/guidance/continue-gate` | `ContinueGate`, `createContinueGate` |
| `@swarmdo/guidance/wasm-kernel` | `getKernel`, `isWasmAvailable`, `resetKernel` |
| `@swarmdo/guidance/generators` | `generateClaudeMd`, `generateClaudeLocalMd`, `generateSkillMd`, `generateAgentMd`, `generateAgentIndex`, `scaffold` |
| `@swarmdo/guidance/analyzer` | `analyze`, `benchmark`, `autoOptimize`, `optimizeForSize`, `headlessBenchmark`, `validateEffect`, `abBenchmark`, `getDefaultABTasks`, `formatReport`, `formatBenchmark` |
...
```ts
import { generateClaudeMd, scaffold } from '@swarmdo/guidance/generators';
```
...
```ts
import { analyze, validateEffect } from '@swarmdo/guidance/analyzer';
```
