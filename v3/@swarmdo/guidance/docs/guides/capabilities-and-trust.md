...
```ts
import { createCapabilityAlgebra } from '@swarmdo/guidance/capabilities';
...
```ts
import { getTrustBasedRateLimit } from '@swarmdo/guidance/trust';
...
```ts
import { createAuthorityGate, createIrreversibilityClassifier } from '@swarmdo/guidance/authority';
