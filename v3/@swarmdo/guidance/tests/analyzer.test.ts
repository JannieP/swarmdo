...
npx swarmdo agent spawn -t coder
npx swarmdo swarm init --v3-mode
npx swarmdo memory search -q "auth patterns"
npx swarmdo doctor --fix
npx swarmdo security scan --depth full
\`\`\`
...
class ABDifferentialExecutor implements IContentAwareExecutor {
  ...
  async execute(prompt: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    if (lower.includes('publish') && lower.includes('npm')) {
      if (this.hasDeployInfo) {
        return { stdout: 'Set up npm publish workflow with dist-tag management. Publishes to alpha and latest tags. Both @swarmdo/cli and swarmdo umbrella package are updated. Tags verified after publish.', stderr: '', exitCode: 0 };
      }
