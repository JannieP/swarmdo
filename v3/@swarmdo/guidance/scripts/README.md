...
Phase 1 benchmarks for the `@swarmdo/guidance` SOTA optimization
...
```bash
cd v3/@swarmdo/guidance && npm run build
node v3/@swarmdo/guidance/scripts/bench-phase-1.mjs --tag=baseline
node v3/@swarmdo/guidance/scripts/bench-retriever-scale.mjs --tag=baseline
```
