...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# Swarmdo V3 - Generated Credentials
# Generated: ${credentials.generatedAt.toISOString()}
# IMPORTANT: Store these securely and delete this file after use

export SWARMDO_ADMIN_PASSWORD="${credentials.adminPassword}"
export SWARMDO_SERVICE_PASSWORD="${credentials.servicePassword}"
export SWARMDO_JWT_SECRET="${credentials.jwtSecret}"
export SWARMDO_SESSION_SECRET="${credentials.sessionSecret}"
export SWARMDO_ENCRYPTION_KEY="${credentials.encryptionKey}"
`;
  }
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'swarmdo/admin-password': credentials.adminPassword,
      'swarmdo/service-password': credentials.servicePassword,
      'swarmdo/jwt-secret': credentials.jwtSecret,
      'swarmdo/session-secret': credentials.sessionSecret,
      'swarmdo/encryption-key': credentials.encryptionKey,
      'swarmdo/generated-at': credentials.generatedAt.toISOString(),
      'swarmdo/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}
