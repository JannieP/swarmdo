...
describe('CredentialGenerator', () => {
  ...
  describe('Environment Script Generation', () => {
    it('should generate valid shell script', () => {
      ...
      expect(script).toContain('export SWARMDO_ADMIN_PASSWORD=');
      expect(script).toContain('export SWARMDO_SERVICE_PASSWORD=');
      expect(script).toContain('export SWARMDO_JWT_SECRET=');
      expect(script).toContain('export SWARMDO_SESSION_SECRET=');
      expect(script).toContain('export SWARMDO_ENCRYPTION_KEY=');
    });
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['swarmdo/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['swarmdo/service-password']).toBe(credentials.servicePassword);
      expect(parsed['swarmdo/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['swarmdo/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['swarmdo/encryption-key']).toBe(credentials.encryptionKey);
    });
  });
