/**
 * Tests for PluginIntegrityVerifier (ADR-145 P1, upstream/swarmdo#2254).
 *
 * Covers:
...
describe('findAnchor — trust resolution', () => {
  ...
  it('matches by exact key + matching exact scope', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@swarmdo/memory' },
    ];
    expect(findAnchor(anchors, '@swarmdo/memory', 'PK', NOW)).not.toBeNull();
  });

  it('matches by exact key + wildcard scope', () => {
    const anchors: TrustAnchor[] = [{ publicKey: 'PK', owner: 'o', scope: '@swarmdo/*' }];
    expect(findAnchor(anchors, '@swarmdo/anything', 'PK', NOW)).not.toBeNull();
  });

  it('rejects exact-key with out-of-scope plugin id', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@swarmdo/*' },
    ];
