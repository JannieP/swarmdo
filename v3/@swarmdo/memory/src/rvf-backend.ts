...
export class RvfBackend implements IMemoryBackend {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.config.verbose) {
      const mode = this.nativeDb ? 'native @swarmvector/rvf' : 'pure-TS fallback';
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    ...
    if (!this.hnswIndex && !this.nativeDb) {
      ...
      recommendations.push('Install @swarmvector/rvf for native HNSW performance');
    }
  ...
  private async tryNativeInit(): Promise<boolean> {
    try {
      const rvf = await import('@swarmvector/rvf' as string);
      ...
      if (this.config.verbose) {
        console.log('[RvfBackend] Native @swarmvector/rvf loaded successfully');
      }
      return true;
    } catch {
      if (this.config.verbose) {
        console.log('[RvfBackend] @swarmvector/rvf not available, using pure-TS fallback');
      }
      return false;
    }
  }
