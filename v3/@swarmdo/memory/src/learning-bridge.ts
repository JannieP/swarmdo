 ...
 *
 * @module @swarmdo/memory/learning-bridge
 */
...
export interface LearningBridgeConfig {
   ...
   * When provided, this replaces the default dynamic import of @swarmdo/neural.
...
export class LearningBridge extends EventEmitter {
  ...
  private async loadNeural(): Promise<void> {
    try {
      ...
      const mod = await import('@swarmdo/neural' as string);
      ...
      // @swarmdo/neural not installed or failed to initialize.
