...
[![npm version](https://img.shields.io/npm/v/@swarmdo/memory.svg)](https://www.npmjs.com/package/@swarmdo/memory)
[![Ecosystem downloads](https://img.shields.io/badge/ecosystem%20downloads-22.2M%2B-blue.svg)](https://github.com/ruvnet/swarmdo/blob/main/data/clone-data.proof.json)
[![Git clones (14d)](https://img.shields.io/badge/git%20clones%2014d-115k-blueviolet.svg)](https://github.com/ruvnet/swarmdo/blob/main/data/clone-data.ledger.json)
...
- **Graceful retrieval.** `service.search('query')` no longer throws when `@swarmdo/embeddings` is unavailable — it degrades to FTS5 keyword search and emits `health.embedder = 'degraded'`. The full hybrid path also adds real Reciprocal Rank Fusion + MMR diversity rerank.
...
- **Node 26 install support** ([#1867](https://github.com/ruvnet/swarmdo/issues/1867)) — `better-sqlite3` is fully optional; installs succeed on Node 24/26 and the package falls back to sql.js when the native build isn't available.
...
```bash
npm install @swarmdo/memory
```

## Standalone use (without the Swarmdo CLI)

This package works on its own — you don't need `@swarmdo/cli` or the
full Swarmdo install. Use it any time you want HNSW vector search, an
...
```typescript
import { ControllerRegistry } from '@swarmdo/memory';
...
```typescript
import { createHybridService } from '@swarmdo/memory';

async function embedder(text: string): Promise<Float32Array> {
  // Use any embedding provider — OpenAI, @swarmdo/embeddings, your own model.
...
import { MemoryService } from '@swarmdo/memory';
...
```typescript
import { MemoryService, HNSWIndex } from '@swarmdo/memory';
...
```typescript
import { MemoryConsolidator } from '@swarmdo/memory';
...
`MemoryService.search()` and `semanticSearch()` no longer hard-fail when `@swarmdo/embeddings` (or whatever embedder you injected) is unavailable. They:
...
```typescript
import { AgentDBAdapter } from '@swarmdo/memory';
...
```typescript
import { CacheManager } from '@swarmdo/memory';
...
```typescript
import { MemoryMigrator, createMigrator } from '@swarmdo/memory';
...
```typescript
import { AutoMemoryBridge } from '@swarmdo/memory';
...
Connects insights to the `@swarmdo/neural` learning pipeline. When neural is unavailable, all operations degrade to no-ops.

### Quick Start

```typescript
import { AutoMemoryBridge, LearningBridge } from '@swarmdo/memory';
...
```typescript
import { LearningBridge } from '@swarmdo/memory';

const lb = new LearningBridge(backend, {
  // Optional: inject neural loader for custom setups
  neuralLoader: async () => {
    const { NeuralLearningSystem } = await import('@swarmdo/neural');
...
```typescript
import { AutoMemoryBridge, MemoryGraph } from '@swarmdo/memory';
...
```typescript
import { MemoryGraph } from '@swarmdo/memory';
...
```typescript
import { createAgentBridge, transferKnowledge } from '@swarmdo/memory';
...
- `better-sqlite3` — **Optional** native SQLite driver for higher throughput. The package works without it (sql.js fallback), so installs succeed on Node 24/26 even when the native build can't compile ([#1867](https://github.com/ruvnet/swarmdo/issues/1867))
- `@swarmdo/embeddings` — **Optional** for semantic search. When absent or failing, `search()` automatically falls back to FTS5 keyword (see [Graceful retrieval](#graceful-retrieval-fts5-fallback))
- `@swarmdo/neural` — **Optional** peer dependency for self-learning (graceful fallback when unavailable)

## Related Packages

- [@swarmdo/neural](../neural) - Neural learning integration (SONA, ReasoningBank, EWC++)
- [@swarmdo/shared](../shared) - Shared types and utilities
- [@swarmdo/hooks](../hooks) - Session lifecycle hooks for auto memory sync
