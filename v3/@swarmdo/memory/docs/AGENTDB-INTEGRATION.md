...
The V3 memory module now integrates with **agentdb@2.0.0-alpha.3.4** to provide high-performance vector search capabilities with HNSW indexing (~1.9x-4.7x measured faster than brute-force approaches).
...
- **Automatic backend selection**: Native hnswlib → swarmvector → WASM fallback
...
```typescript
import { AgentDBBackend } from '@swarmdo/memory';
...
```typescript
import { HybridBackend } from '@swarmdo/memory';
...
// Semantic queries go to AgentDB (~4.7x faster)
...
interface AgentDBBackendConfig {
  ...
  /** Vector backend: 'auto', 'swarmvector', 'hnswlib' */
...
// 2. Fallback to swarmvector (fast, pure JS)
...
|-----------|-------------|----------------|---------|
| 10k vectors, k=10 | 150ms | 1ms | ~4.7x |
...
```typescript
import { MemoryMigrator } from '@swarmdo/memory';
