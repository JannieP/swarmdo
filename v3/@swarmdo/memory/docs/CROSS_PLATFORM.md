...
The `@swarmdo/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@swarmdo/memory';
...
```typescript
import { createDatabase } from '@swarmdo/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@swarmdo/memory';
...
npm install @swarmdo/memory --no-optional
...
SWARMDO_DB_PROVIDER=sql.js

# Enable verbose logging
SWARMDO_DB_VERBOSE=true

# Set auto-persist interval (ms)
SWARMDO_DB_PERSIST_INTERVAL=10000
```
