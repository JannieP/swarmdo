# Migration Guide — `@swarmdo/memory@3.0.0-alpha.18`
...
```diff
- import { UnifiedMemoryService, createHybridService, HnswLite, RvfBackend } from '@swarmdo/memory';
+ import { MemoryService, createHybridService } from '@swarmdo/memory';
```
...
```ts
import { MemoryService, UnifiedMemoryService } from '@swarmdo/memory';
...
import { HnswLite, RvfBackend } from '@swarmdo/memory';

// After — if you actually need them, import from the explicit submodule
import { HnswLite } from '@swarmdo/memory/hnsw-lite';
import { RvfBackend } from '@swarmdo/memory/rvf-backend';
```
...
### 6. SwarmVector boundary (Phase 7)

`v3/@swarmdo/memory/swarmvector.db` is test pollution, not a runtime artifact. It is now wiped before and after tests by `vitest.setup.ts`, and a `scripts/smoke-memory-no-stray-db.mjs` CI guard fails the build if any `*.db` / `*.redb` / `*.rvf` file appears in `git status` under the package after `npm test` runs.
...
```diff
- "@swarmdo/memory": "^3.0.0-alpha.17"
+ "@swarmdo/memory": "^3.0.0-alpha.18"
```
