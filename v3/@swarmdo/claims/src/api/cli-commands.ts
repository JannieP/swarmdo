// @ts-nocheck - CLI integration requires the full @swarmdo/cli package
/**
...
const claimCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      ...
      output.printInfo('Usage: swarmdo issues claim <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const releaseCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues release <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const handoffCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues handoff <issueId> --to <target>');
      return { success: false, exitCode: 1 };
    }
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues status <issueId> [options]');
      return { success: false, exitCode: 1 };
    }
...
const stealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln();
      output.printInfo('Use "swarmdo issues steal <issueId>" to take over an issue');
...
const stealCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues steal <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const markStealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues mark-stealable <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const contestCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: swarmdo issues contest <issueId> --reason "..."');
      return { success: false, exitCode: 1 };
    }
...
export const issuesCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo issues list --available', description: 'List unclaimed issues' },
    { command: 'swarmdo issues list --mine', description: 'List my claims' },
    { command: 'swarmdo issues claim GH-123', description: 'Claim an issue' },
    { command: 'swarmdo issues release GH-123', description: 'Release a claim' },
    { command: 'swarmdo issues handoff GH-123 --to agent:coder-1', description: 'Request handoff to agent' },
    { command: 'swarmdo issues handoff GH-123 --to human:alice', description: 'Request handoff to human' },
    { command: 'swarmdo issues status GH-123 --blocked "Waiting for API"', description: 'Mark as blocked' },
    { command: 'swarmdo issues status GH-123 --review-requested', description: 'Request review' },
    { command: 'swarmdo issues board', description: 'View who is working on what' },
    { command: 'swarmdo issues stealable', description: 'List stealable issues' },
    { command: 'swarmdo issues steal GH-123', description: 'Steal an issue' },
    { command: 'swarmdo issues mark-stealable GH-123', description: 'Mark my claim as stealable' },
    { command: 'swarmdo issues contest GH-123 -r "I was actively working on it"', description: 'Contest a steal' },
    { command: 'swarmdo issues load', description: 'View agent load distribution' },
    { command: 'swarmdo issues load --agent coder-1', description: 'View specific agent load' },
    { command: 'swarmdo issues rebalance --dry-run', description: 'Preview rebalancing' },
    { command: 'swarmdo issues rebalance', description: 'Trigger swarm rebalancing' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: swarmdo issues <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "swarmdo issues <subcommand> --help" for subcommand help');
