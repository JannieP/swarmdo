#!/usr/bin/env node
/**
 * Standalone CLI shim for @swarmdo/plugin-agent-federation.
 * Maps `swarmdo-federation <subcommand> [args]` -> CLICommandDefinition handlers.
 *
...
function printHelp(commandNames: string[]): void {
  console.log('swarmdo-federation — Cross-installation agent federation CLI');
  console.log('');
  console.log('Usage: swarmdo-federation <subcommand> [options]');
  console.log('');
  ...
  console.log('');
  console.log('Run "swarmdo-federation <subcommand> --help" for command-specific options.');
}
...
async function main(): Promise<void> {
  ...
  if (!resolved) {
    ...
    console.error('Run "swarmdo-federation --help" to list available subcommands.');
    process.exitCode = 1;
    return;
  }
