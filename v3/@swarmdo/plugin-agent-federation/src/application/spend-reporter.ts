 ...
 * plugins/swarmdo-cost-tracker/scripts/federation.mjs:
 *
 ...
 * persists to swarmdo memory / Redis / Datadog / their accounting system.
 */
 ...
 * swarmdo memory, Redis, DynamoDB, file-backed JSON, etc. The federation
...
/** Default namespace per cost-tracker's `plugins/swarmdo-cost-tracker/scripts/federation.mjs`. */
 ...
 * (`plugins/swarmdo-cost-tracker/scripts/federation.mjs`) `memory list`s
 ...
 *   });
 *
 *   // swarmdo memory CLI shell-out
 *   new MemorySpendReporter({
 *     memoryStore: {
 *       store: async ({namespace, key, value, ttl}) => {
 *         await execFile('npx', ['swarmdo', 'memory', 'store',
