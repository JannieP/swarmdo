 ...
 *   - `pf` for macOS — `pfctl -a swarmdo-fed -f <file>`, anchor-scoped so
 *     swarmdo's rules don't collide with the operator's main pf ruleset.
 *
...
export interface WgFirewallServiceConfig {
  ...
  /** WG interface name to scope rules to. Defaults to `swarmdo-fed`. */
  readonly interfaceName?: string;
  /** Where the operator should write the rule file. Used for the load-command string. */
  readonly rulePath?: string;
  /** pf anchor (macOS only). Defaults to `swarmdo-fed`. */
...
const DEFAULT_INTERFACE = 'swarmdo-fed';
const DEFAULT_PF_ANCHOR = 'swarmdo-fed';
...
export class WgFirewallService {
  ...
  private renderNftables(projections: ReadonlyArray<{
    ...
    lines.push('# Generated by swarmdo federation plugin — ADR-111 Phase 4.');
    ...
    lines.push('');
    lines.push('table inet swarmdo_fed {');
  ...
  private defaultRulePath(): string {
    return this.platform === 'linux-nftables'
      ? '/etc/nftables.d/swarmdo-fed.nft'
