import type {
  ...
  AgentTypeDefinition,
} from '@swarmdo/shared/src/plugin-interface.js';
...
export class AgentFederationPlugin implements ClaudeFlowPlugin {
  readonly name = '@swarmdo/plugin-agent-federation';
  ...
  readonly dependencies = ['@swarmdo/security', '@swarmdo/aidefence'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    // signatures. Persist to .swarmdo/federation/key-<nodeId>.json so
    ...
    const keyDir = join(process.cwd(), '.swarmdo', 'federation');
    ...
    return [
      {
        ...
        defaultConfig: {
          ...
          timeout: 300_000,
          metadata: {
            pluginSource: '@swarmdo/plugin-agent-federation',
          },
        },
