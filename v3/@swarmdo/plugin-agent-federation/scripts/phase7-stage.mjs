...
// Persist local private key (mode 0600) for operator to move to .swarmdo/federation/
writeFileSync(
...
writeFileSync(join(stageDir, 'swarmdo-fed.conf'), interfaceConfig);

// Firewall projection per OS
const fw = new wgFirewall.WgFirewallService();
const fwFile = process.platform === 'linux' ? 'swarmdo-fed.nft' : 'swarmdo-fed.pf';
...
console.log(`  ${stageDir}/swarmdo-fed.conf               (wg-quick config)`);
...
console.log('  [ ] Review swarmdo-fed.conf for unexpected [Peer] blocks (defense vs compromised manifest)');
...
console.log(`  sudo install -m 0600 ${stageDir}/swarmdo-fed.conf /etc/wireguard/swarmdo-fed.conf`);
if (process.platform === 'linux') {
  console.log(`  sudo nft -f ${stageDir}/${fwFile}`);
} else {
  console.log(`  sudo pfctl -a swarmdo-fed -f ${stageDir}/${fwFile}`);
}
console.log('  sudo wg-quick up swarmdo-fed');
console.log();
console.log('=== Verification ===');
console.log('  sudo wg show swarmdo-fed');
console.log(`  ping ${peerMeshIP.replace('/32', '')}    # mesh reachability`);
console.log();
console.log('=== Rollback ===');
console.log('  sudo wg-quick down swarmdo-fed');
if (process.platform === 'linux') {
  console.log('  sudo nft delete table inet swarmdo_fed');
} else {
  console.log('  sudo pfctl -a swarmdo-fed -F all');
}
