...
describe('ADR-111 Phase 3 — coordinator wg integration', () => {
  ...
  it('evictPeer emits remove-peer WG command for a peer with wg metadata', async () => {
    ...
    await discovery.addStaticPeer('ws://swarmultra:9100', {
      nodeId: 'swarmultra',
      publicKey: 'pk',
    ...
    attachWgMetadata(discovery.getPeer('swarmultra'), 'swarmultra');

    const ok = await coordinator.evictPeer('swarmultra', 'MANUAL_EVICT');
    expect(ok).toBe(true);
