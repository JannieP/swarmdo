...
Integration of `@swarmvector/attention` Flash Attention capabilities into the V3 performance module.

## Overview

This module provides high-performance attention mechanisms optimized for V3's unverified (no benchmark) speedup targets. Flash Attention reduces memory usage by ~50% while achieving significant performance improvements through block-wise computation.
...
The `@swarmvector/attention` package is already installed as a dependency:

```bash
npm install @swarmvector/attention@latest
```
...
```typescript
import { createFlashAttentionOptimizer } from '@swarmdo/performance';
...
```typescript
import { quickBenchmark } from '@swarmdo/performance';
...
```typescript
import { quickValidation } from '@swarmdo/performance';

// Validate V3 performance targets (unverified (no benchmark))
...
```typescript
import { runAndDisplaySuite } from '@swarmdo/performance';
...
Validate against V3 performance targets (unverified (no benchmark)).
...
```bash
# Run all examples
npx tsx v3/@swarmdo/performance/src/examples/flash-attention-demo.ts
```
...
```typescript
import { FlashAttentionOptimizer } from '@swarmdo/performance';
...
MIT OR Apache-2.0 (follows @swarmvector/attention license)
