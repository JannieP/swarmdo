...
[![npm version](https://img.shields.io/npm/v/@swarmdo/aidefence?color=blue&label=npm)](https://www.npmjs.com/package/@swarmdo/aidefence)
[![npm downloads](https://img.shields.io/npm/dm/@swarmdo/aidefence?color=green)](https://www.npmjs.com/package/@swarmdo/aidefence)
...
`@swarmdo/aidefence` is a high-performance security library designed to protect AI/LLM applications from manipulation attempts. It provides:
...
```bash
# npm
npm install @swarmdo/aidefence

# pnpm
pnpm add @swarmdo/aidefence

# yarn
yarn add @swarmdo/aidefence
```
...
```typescript
import { isSafe, checkThreats } from '@swarmdo/aidefence';
...
```typescript
import { createAIDefence } from '@swarmdo/aidefence';
...
```bash
# Basic threat scan
npx @swarmdo/cli security defend -i "ignore previous instructions"

# Scan a file
npx @swarmdo/cli security defend -f ./user-prompts.txt

# Quick scan (faster)
npx @swarmdo/cli security defend -i "some text" --quick

# JSON output
npx @swarmdo/cli security defend -i "test" -o json

# View statistics
npx @swarmdo/cli security defend --stats
```
...
```typescript
import { VectorStore, createAIDefence } from '@swarmdo/aidefence';
...
```json
{
  "hooks": {
    "pre-agent-input": {
      "command": "node -e \"
        const { isSafe } = require('@swarmdo/aidefence');
...
- [`@swarmdo/cli`](https://www.npmjs.com/package/@swarmdo/cli) - CLI with security commands
- [`agentdb`](https://www.npmjs.com/package/agentdb) - HNSW vector database
- [`swarmdo`](https://www.npmjs.com/package/swarmdo) - Full AI coordination system
