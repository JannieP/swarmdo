# `@swarmdo/browser` Security Audit (ADR-122 / v3.0.0-alpha.4)
...
| Secrets in source | 0 — only `process.env.SWARMDO_BROWSER_WITNESS_KEY` lookup | ✅ |
...
- **Transitive `@xenova/transformers` retirement** — ADR-121 Phase 4 migrates to `swarmvector-onnx-embeddings-wasm`. Not blocking for browser package release.
...
✅ **Approve `@swarmdo/browser@3.0.0-alpha.4` for npm publish.** All new code paths covered by 230 unit tests. No new direct vulnerabilities. Transitive findings are pre-existing and tracked in other ADRs.
