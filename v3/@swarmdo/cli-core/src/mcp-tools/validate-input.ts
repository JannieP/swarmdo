 ...
 *
 * Loads @swarmdo/security validators when available
...
// Try to load the full @swarmdo/security module for enhanced validation
...
async function getSecurityModule(): Promise<Record<string, any> | null> {
  ...
  try {
    // Dynamic import — @swarmdo/security is an optional dependency
    _securityModule = await (Function('return import("@swarmdo/security")')() as Promise<Record<string, any>>);
  } catch {
    // @swarmdo/security is optional — fallback to inline validation above
  }
  return _securityModule;
}

/**
 * Enhanced validation using @swarmdo/security Zod schemas when available.
...
export async function validateAgentSpawn(input: Record<string, unknown>): Promise<{ valid: boolean; errors: string[] }> {
  ...
  // Fix for #1567: @swarmdo/security's SpawnAgentSchema expects `type` and
