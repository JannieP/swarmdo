#!/usr/bin/env node
/**
 * @swarmdo/cli-core entry point — alpha.1 surface.
 *
 ...
 *                       stay in @swarmdo/cli, dynamic-imported)
...
// Bin entry — runs when invoked as `swarmdo-core <command>`.
...
const isMain = (() => {
  ...
  try {
    ...
    // npm bin-shim case: argv[1] is the .bin/swarmdo-core symlink.
...
if (isMain) {
  ...
  if (args[0] === '--help' || args[0] === '-h' || args.length === 0) {
    console.log(`@swarmdo/cli-core — alpha.1 (ADR-100)
...
Storage:
  ...
  SWARMDO_MEMORY_PATH or --path. Search is substring-only — for semantic
  vector search, install the heavy @swarmdo/cli@alpha.
...
For now, use the full CLI:  npx @swarmdo/cli@alpha ${args.join(' ')}`);
  process.exit(1);
}
