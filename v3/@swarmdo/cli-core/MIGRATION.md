# Migration Guide — switching plugin scripts to `@swarmdo/cli-core`
...
> **alpha.5 adds two subpath exports** (`./mcp-tools/types`, `./mcp-tools/validate-input`) so `@swarmdo/cli` can re-export those foundation modules from cli-core in a follow-up PR (eliminating ~1,229 LOC of byte-identical duplication). No behavior change for existing plugin scripts; this is groundwork for the cli-side metapackage refactor.

## TL;DR

Each `npx @swarmdo/cli@latest memory <subcommand> ...` in a plugin script can be swapped for `npx @swarmdo/cli-core@alpha memory <subcommand> ...`. Cold-cache wall-time drops from ~25s to ~2s.
...
const r = spawnSync('npx', [
  '@swarmdo/cli@latest', 'memory', 'store',
...
const cliPkg = process.env.CLI_CORE === '1'
  ? '@swarmdo/cli-core@alpha'  // lite path — JSON backend, fast cold cache
  : '@swarmdo/cli@latest';     // heavy path — SQLite/HNSW, slow cold cache
const r = spawnSync('npx', [
  cliPkg, 'memory', 'store',
  '--namespace', 'cost-tracking',
  '--key', `session-${id}`,
...
3. **If yes**, fall back to `cli@latest` for the affected call sites and document the gap; ping us on issue [#1760](https://github.com/ruvnet/swarmdo/issues/1760) so we can prioritize.
...
Or replace the conditional with the bare `'@swarmdo/cli@latest'` and re-run. cli-core left no permanent footprint other than the JSON file (which you can delete: `rm .swarm/memory.json`).

## Reporting issues

cli-core is alpha — please file feedback at https://github.com/ruvnet/swarmdo/issues with the label `cli-core-alpha`. Specifically helpful:

- Cold-cache wall-time on your network (run `rm -rf ~/.npm/_npx; time npx -y @swarmdo/cli-core@alpha --version`)
...
## Planned: `@swarmdo/cli` foundation re-export (post-alpha.5)

The following 4 foundation modules are byte-identical between `@swarmdo/cli` and `@swarmdo/cli-core` (verified via `diff -q`):

| File | Lines | cli-core export |
|---|---:|---|
| `cli/src/types.ts` | 287 | `@swarmdo/cli-core/types` |
| `cli/src/output.ts` | 640 | `@swarmdo/cli-core/output` |
| `cli/src/mcp-tools/types.ts` | 46 | `@swarmdo/cli-core/mcp-tools/types` |
| `cli/src/mcp-tools/validate-input.ts` | 256 | `@swarmdo/cli-core/mcp-tools/validate-input` |
| **Total** | **1,229** | |

The follow-up PR will:

1. Add `@swarmdo/cli-core: ^3.7.0-alpha.5` as a runtime dep in `cli/package.json`.
...
- [Issue #1748 #3](https://github.com/ruvnet/swarmdo/issues/1748) — the cold-cache problem this addresses
- [Issue #1760](https://github.com/ruvnet/swarmdo/issues/1760) — alpha tracking
- Branch [`feat/cli-core-split`](https://github.com/ruvnet/swarmdo/tree/feat/cli-core-split) — work in progress
