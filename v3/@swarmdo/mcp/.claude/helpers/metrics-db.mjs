#!/usr/bin/env node
/**
 * Swarmdo V3 - Metrics Database Manager
...
const DB_PATH = join(PROJECT_ROOT, '.swarmdo', 'metrics.db');
...
function checkSecurityFile(filename, minLines = 100) {
  const filePath = join(V3_DIR, '@swarmdo/security/src', filename);
...
async function syncMetrics() {
  ...
  if (existsSync(modulesDir)) {
    ...
    for (const entry of entries) {
      // Skip hidden directories (like .agentic-flow, .swarmdo)
...
function exportToJSON() {
  ...
  const securityDir = join(PROJECT_ROOT, '.swarmdo/security');
