...
```bash
npx swarmdo auto agent [options]
```
...
```bash
npx swarmdo auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx swarmdo auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx swarmdo auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx swarmdo auto agent -t "Fix bug in login" -s minimal
```
