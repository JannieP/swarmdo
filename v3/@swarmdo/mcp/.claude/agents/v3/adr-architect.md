...
hooks:
  pre: |
    ...
    mcp__swarmdo__memory_search --pattern="adr:*" --namespace="decisions" --limit=10
...
The following ADRs define the Swarmdo V3 architecture:
...
```bash
# Create new ADR
npx swarmdo@v3alpha adr create "Decision Title"

# List all ADRs
npx swarmdo@v3alpha adr list

# Search ADRs
npx swarmdo@v3alpha adr search "memory backend"

# Check ADR status
npx swarmdo@v3alpha adr status ADR-006

# Supersede an ADR
npx swarmdo@v3alpha adr supersede ADR-005 ADR-012
```
