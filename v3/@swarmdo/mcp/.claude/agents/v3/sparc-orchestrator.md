...
hooks:
  pre: |
    ...
    mcp__swarmdo__memory_usage --action="store" --namespace="sparc" --key="session:$SESSION_ID" --value="$(date -Iseconds): SPARC workflow initiated for: $TASK"
  ...
  post: |
    ...
    # Store completion
    mcp__swarmdo__memory_usage --action="store" --namespace="sparc" --key="complete:$SESSION_ID" --value="$(date -Iseconds): SPARC workflow completed"
...
mcp__swarmdo__memory_usage --action="store" --namespace="patterns" \
