...
hooks:
  pre: |
    ...
    mcp__swarmdo__memory_search --pattern="ddd:*" --namespace="architecture" --limit=10
    # Load domain context
    mcp__swarmdo__memory_store --action="retrieve" --namespace="architecture" --key="domain:model"
  post: |
    echo "✅ Domain model analysis complete"
    # Store domain patterns
    mcp__swarmdo__memory_store --action="store" --namespace="architecture" --key="ddd:analysis:$(date +%s)" --value="$DOMAIN_SUMMARY"
---
...
npx swarmdo@v3alpha ddd analyze --path ./src

# Generate bounded context map
npx swarmdo@v3alpha ddd context-map

# Validate aggregate design
npx swarmdo@v3alpha ddd validate-aggregates

# Check ubiquitous language consistency
npx swarmdo@v3alpha ddd language-check
```
...
mcp__swarmdo__memory_search --pattern="ddd:aggregate:*" --namespace="architecture"
```
