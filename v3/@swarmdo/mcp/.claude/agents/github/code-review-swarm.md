...
npx swarmdo@v3alpha github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
# Run security review
SECURITY_RESULTS=$(npx swarmdo@v3alpha github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
| **Review Speed** | unverified (no benchmark) faster | Flash Attention |
