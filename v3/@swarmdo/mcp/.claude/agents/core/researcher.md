...
hooks:
  pre: |
    ...
    # 1. Learn from past similar research tasks (ReasoningBank + HNSW ~1.9x-4.7x measured faster)
    SIMILAR_RESEARCH=$(npx swarmdo@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    npx swarmdo@v3alpha memory store --key "research_context_$(date +%s)" --value "$TASK"
  ...
  post: |
    ...
    npx swarmdo@v3alpha memory search --query "research" --limit 5

    # 1. Calculate research quality metrics
    FINDINGS_COUNT=$(npx swarmdo@v3alpha memory search --query "research" --count-only || echo "0")
    ...
    npx swarmdo@v3alpha hooks post-task --task-id "researcher-$(date +%s)" --success "$SUCCESS"
    ...
    npx swarmdo@v3alpha hooks worker dispatch --trigger deepdive
---
...
**Enhanced with Swarmdo V3**: You now have AI-enhanced research capabilities with:
- **ReasoningBank**: Pattern storage with trajectory tracking
- **HNSW Indexing**: ~1.9x-4.7x measured faster knowledge retrieval
- **Flash Attention**: unverified (no benchmark) speedup for large document processing
...
```typescript
// 1. Search for similar past research (~1.9x-4.7x measured faster with HNSW)
...
console.log(`Search time: ${relevantDocs.searchTimeMs}ms (HNSW: ~1.9x-4.7x measured faster)`);
