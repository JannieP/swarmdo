...
hooks:
  pre: |
    ...
    # 1. Learn from past review patterns (ReasoningBank + HNSW ~1.9x-4.7x measured faster)
    SIMILAR_REVIEWS=$(npx swarmdo@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    MISSED_ISSUES=$(npx swarmdo@v3alpha memory search --query "$TASK missed issues" --limit 3 --failures-only --use-hnsw)
    if [ -n "$MISSED_ISSUES" ]; then
      echo "⚠️  Learning from previously missed issues"
    fi

    # Create review checklist via memory
    npx swarmdo@v3alpha memory store --key "review_checklist_$(date +%s)" --value "functionality,security,performance,maintainability,documentation"
  ...
  post: |
    ...
    ISSUES_FOUND=$(npx swarmdo@v3alpha memory search --query "review_issues" --count-only || echo "0")
    CRITICAL_ISSUES=$(npx swarmdo@v3alpha memory search --query "review_critical" --count-only || echo "0")
    ...
    npx swarmdo@v3alpha hooks post-task --task-id "reviewer-$(date +%s)" --success "$SUCCESS"
...
**Enhanced with Swarmdo V3**: You now have AI-powered code review with:
- **ReasoningBank**: Learn from review patterns with trajectory tracking
- **HNSW Indexing**: ~1.9x-4.7x measured faster issue pattern search
- **Flash Attention**: unverified (no benchmark) speedup for large code reviews
...
```typescript
// 1. Learn from past reviews of similar code (~1.9x-4.7x measured faster with HNSW)
...
console.log(`Search time: ${relatedCode.searchTimeMs}ms (HNSW: ~1.9x-4.7x measured faster)`);
