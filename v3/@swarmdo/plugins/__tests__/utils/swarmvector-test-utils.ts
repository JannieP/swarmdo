/**
 * SwarmVector Test Utilities
 *
 * Shared utilities for SwarmVector integration tests including:
 * - Random vector generation
 * - Mock data factories
 * - Test database helpers
 * - Performance measurement utilities
 *
 * @module @swarmdo/plugins/__tests__/utils/swarmvector-test-utils
 */
...
export function useRealDatabase(): boolean {
  return process.env.SWARMVECTOR_TEST_DB === 'true';
}

/**
 * Get test database configuration from environment
 */
export function getTestDatabaseConfig(): SwarmVectorConfig {
  return {
    host: process.env.SWARMVECTOR_TEST_HOST ?? 'localhost',
    port: parseInt(process.env.SWARMVECTOR_TEST_PORT ?? '5432', 10),
    database: process.env.SWARMVECTOR_TEST_DATABASE ?? 'swarmvector_test',
    user: process.env.SWARMVECTOR_TEST_USER ?? 'postgres',
    password: process.env.SWARMVECTOR_TEST_PASSWORD ?? 'postgres',
    poolSize: 5,
 ...
 */
export function createTestConfig(overrides: Partial<SwarmVectorConfig> = {}): SwarmVectorConfig {
  return {
    host: 'localhost',
    port: 5432,
    database: 'test_db',
...
export function createTestClientOptions(
  overrides: Partial<SwarmVectorClientOptions> = {}
): SwarmVectorClientOptions {
  return {
    ...createTestConfig(),
    autoReconnect: true,
 ...
 * Create mock stats
 */
export function createMockStats(): SwarmVectorStats {
  return {
    version: '1.0.0',
    totalVectors: 100000,
