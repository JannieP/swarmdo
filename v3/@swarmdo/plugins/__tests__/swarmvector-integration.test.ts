/**
 * SwarmVector PostgreSQL Integration Tests
 *
 * Comprehensive integration tests for the SwarmVector PostgreSQL Bridge plugin.
 ...
 * - SWARMVECTOR_TEST_DB=true - Enable real database tests
 * - SWARMVECTOR_TEST_HOST - PostgreSQL host (default: localhost)
 * - SWARMVECTOR_TEST_PORT - PostgreSQL port (default: 5432)
 * - SWARMVECTOR_TEST_DATABASE - Database name (default: swarmvector_test)
 * - SWARMVECTOR_TEST_USER - Database user (default: postgres)
 * - SWARMVECTOR_TEST_PASSWORD - Database password (default: postgres)
 *
 * @module @swarmdo/plugins/__tests__/swarmvector-integration
 */
...
import {
  ...
  type MockPgPool,
  type MockPgClient,
} from './utils/swarmvector-test-utils.js';

import type {
  SwarmVectorConfig,
  SwarmVectorClientOptions,
  VectorSearchOptions,
  VectorSearchResult,
  VectorInsertOptions,
  VectorIndexOptions,
  ...
  IndexStats,
  ISwarmVectorClient,
  ISwarmVectorTransaction,
  QueryResult,
  BatchResult,
  SwarmVectorEventType,
  SwarmVectorEvent,
  PoolConfig,
  SSLConfig,
  RetryConfig,
} from '../src/integrations/swarmvector/types.js';
...
// ============================================================================
// Mock SwarmVector Client Implementation
// ============================================================================

/**
 * Mock SwarmVector client for unit testing without real database
 */
class MockSwarmVectorClient implements ISwarmVectorClient {
  private pool: MockPgPool;
  private config: SwarmVectorClientOptions;
  private connected: boolean = false;
  private eventHandlers: Map<SwarmVectorEventType, Set<(event: SwarmVectorEvent) => void>> = new Map();
  private connectionInfo: ConnectionResult | null = null;
  private queryCount: number = 0;

  constructor(config: SwarmVectorClientOptions) {
    this.config = config;
    this.pool = createMockPgPool();
  }

  // Event Emitter Implementation
  on<T extends SwarmVectorEventType>(event: T, handler: (e: SwarmVectorEvent<T>) => void): () => void {
    if (!this.eventHandlers.has(event)) {
      this.eventHandlers.set(event, new Set());
    }
    this.eventHandlers.get(event)!.add(handler as (e: SwarmVectorEvent) => void);
    return () => this.off(event, handler);
  }

  off<T extends SwarmVectorEventType>(event: T, handler: (e: SwarmVectorEvent<T>) => void): void {
    this.eventHandlers.get(event)?.delete(handler as (e: SwarmVectorEvent) => void);
  }

  once<T extends SwarmVectorEventType>(event: T, handler: (e: SwarmVectorEvent<T>) => void): () => void {
    const wrappedHandler = (e: SwarmVectorEvent<T>) => {
  ...
  emit<T extends SwarmVectorEventType>(event: T, data: SwarmVectorEvent<T>['data']): void {
    const handlers = this.eventHandlers.get(event);
    if (handlers) {
      handlers.forEach((handler) =>
        handler({ type: event, timestamp: new Date(), data } as SwarmVectorEvent)
      );
    }
  }
  ...
  // Transaction Support
  async transaction<T>(fn: (tx: ISwarmVectorTransaction) => Promise<T>): Promise<{ transactionId: string; committed: boolean; data?: T; durationMs: number; queryCount: number }> {
    this.ensureConnected();

    const startTime = Date.now();
    let txQueryCount = 0;

    const tx: ISwarmVectorTransaction = {
  ...
  getConfig(): SwarmVectorClientOptions {
    return this.config;
  }
}
...
describe('SwarmVector PostgreSQL Integration', () => {
  let client: MockSwarmVectorClient;
  let config: SwarmVectorClientOptions;

  beforeEach(() => {
    config = createTestClientOptions();
    client = new MockSwarmVectorClient(config);
  });
  ...
  describe('Connection Management', () => {
    ...
    it('should respect pool limits', async () => {
      ...
      const customClient = new MockSwarmVectorClient(customConfig);
    ...
    it('should support SSL configuration', async () => {
      ...
      const sslClient = new MockSwarmVectorClient({
        ...config,
        ssl: sslConfig,
      });
    ...
    it('should support retry configuration', () => {
      ...
      const retryClient = new MockSwarmVectorClient({
        ...config,
        retry: retryConfig,
      });
...
describe('Edge Cases and Error Handling', () => {
  ...
  beforeEach(async () => {
    client = new MockSwarmVectorClient(createTestClientOptions());
    await client.connect();
  });
