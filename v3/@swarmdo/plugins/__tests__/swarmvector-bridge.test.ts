/**
 * SwarmVector PostgreSQL Bridge Tests
 *
 * Comprehensive test suite for the SwarmVector PostgreSQL vector database
 ...
 *
 * @module @swarmdo/plugins/__tests__/swarmvector-bridge
 */
 ...
 */
function createTestConfig(overrides: Partial<SwarmVectorConfig> = {}): SwarmVectorConfig {
  return {
    host: 'localhost',
    port: 5432,
    database: 'test_db',
...
// ============================================================================
// Mock SwarmVector Client Implementation for Testing
// ============================================================================

/**
 * Mock SwarmVector client for unit testing
 */
class MockSwarmVectorClient implements ISwarmVectorClient {
  ...
  async getStats(): Promise<SwarmVectorStats> {
    if (!this.connected) throw new Error('Not connected');
  ...
  describe('Connection Management', () => {
    describe('Pool Creation and Configuration', () => {
      ...
      it('should apply custom pool configuration', async () => {
        const customConfig: SwarmVectorClientOptions = {
          ...config,
          pool: {
            min: 5,
            max: 20,
      ...
      it('should respect pool size limits', async () => {
        ...
        const limitedClient = new MockSwarmVectorClient({
          ...config,
          pool: poolConfig,
        });
    ...
    describe('Connection Health Checks', () => {
      it('should report healthy status when connected', async () => {
        ...
        expect(health.components.swarmvector.healthy).toBe(true);
...
describe('Edge Cases', () => {
  ...
  beforeEach(async () => {
    client = new MockSwarmVectorClient(createTestConfig());
    await client.connect();
  });
