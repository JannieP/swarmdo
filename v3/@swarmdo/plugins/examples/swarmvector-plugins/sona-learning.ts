 ...
 *
 * Self-Optimizing Neural Adaptation using @swarmvector/learning-wasm.
 ...
 * @example
 * ```typescript
 * import { sonaLearningPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export class SONALearning {
  ...
  /**
   * Learn a new pattern (<100μs with @swarmvector/learning-wasm).
   */
  async learn(
    category: string,
    trigger: string,
...
export const sonaLearningPlugin = new PluginBuilder('sona-learning', '1.0.0')
  .withDescription('Self-Optimizing Neural Adaptation with @swarmvector/learning-wasm (<100μs LoRA)')
  .withAuthor('Swarmdo Team')
  .withTags(['learning', 'neural', 'adaptation', 'lora', 'swarmvector', 'sona', 'ewc'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('SONA Learning initializing with @swarmvector/learning-wasm...');
