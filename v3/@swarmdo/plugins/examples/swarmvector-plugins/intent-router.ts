 ...
 *
 * Smart query → agent/tool mapping using @swarmvector/wasm semantic matching.
 ...
 * @example
 * ```typescript
 * import { intentRouterPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export const intentRouterPlugin = new PluginBuilder('intent-router', '1.0.0')
  .withDescription('Smart intent → handler routing using @swarmvector/wasm HNSW (<1ms)')
  .withAuthor('Swarmdo Team')
  .withTags(['routing', 'intent', 'semantic', 'swarmvector', 'hnsw'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('Intent Router initializing with @swarmvector/wasm...');
    const router = await getRouter();
