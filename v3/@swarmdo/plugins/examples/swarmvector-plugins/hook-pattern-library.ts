 ...
 * @example
 * ```typescript
 * import { hookPatternLibraryPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export const hookPatternLibraryPlugin = new PluginBuilder('hook-pattern-library', '1.0.0')
  .withDescription('Learn optimal hook patterns for file types using @swarmvector/wasm')
  .withAuthor('Swarmdo Team')
  .withTags(['hooks', 'patterns', 'learning', 'swarmvector', 'optimization'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('Hook Pattern Library initializing with @swarmvector/wasm...');
    const library = await getLibrary();
    const stats = library.getStats();
