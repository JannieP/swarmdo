 ...
 * Uses @swarmvector/wasm for pattern storage and @swarmvector/learning-wasm for optimization.
 *
 * Features:
 ...
 * @example
 * ```typescript
 * import { mcpToolOptimizerPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export const mcpToolOptimizerPlugin = new PluginBuilder('mcp-tool-optimizer', '1.0.0')
  .withDescription('Learn tool patterns and suggest optimal sequences using @swarmvector/wasm + @swarmvector/learning-wasm')
  .withAuthor('Swarmdo Team')
  .withTags(['optimization', 'tools', 'patterns', 'swarmvector', 'learning', 'hnsw'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('MCP Tool Optimizer initializing with @swarmvector/wasm + @swarmvector/learning-wasm...');
    await getOptimizer();
