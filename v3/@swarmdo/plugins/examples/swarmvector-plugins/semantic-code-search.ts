 ...
 *
 * Index and search code semantically using @swarmvector/wasm vector database.
 ...
 * @example
 * ```typescript
 * import { semanticCodeSearchPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export const semanticCodeSearchPlugin = new PluginBuilder('semantic-code-search', '1.0.0')
  .withDescription('Semantic code search using @swarmvector/wasm HNSW indexing (<1ms search)')
  .withAuthor('Swarmdo Team')
  .withTags(['search', 'code', 'semantic', 'swarmvector', 'hnsw'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('Semantic Code Search initializing with @swarmvector/wasm...');
    await getCodeSearch();
