 ...
 *
 * Consolidated implementations for all SwarmVector plugins.
 */
...
/**
 * Fallback vector database when @swarmvector/wasm is not available.
...
/**
 * Fallback LoRA engine when @swarmvector/learning-wasm is not available.
...
/**
 * Create a vector database - uses @swarmvector/wasm in production, fallback otherwise.
 */
export async function createVectorDB(dimensions: number): Promise<IVectorDB> {
  try {
    // @ts-expect-error - @swarmvector/wasm types may not be available
    const { VectorDB: SwarmVectorDB } = await import('@swarmvector/wasm');
    ...
    console.warn('[@swarmdo/plugins] @swarmvector/wasm not available, using fallback');
    return new FallbackVectorDB(dimensions);
  }
}

/**
 * Create a LoRA engine - uses @swarmvector/learning-wasm in production, fallback otherwise.
 */
export async function createLoRAEngine(): Promise<ILoRAEngine> {
  try {
    // @ts-expect-error - @swarmvector/learning-wasm types may not be available
    const { LoRAEngine } = await import('@swarmvector/learning-wasm');
    const engine = new LoRAEngine({ defaultRank: 8, defaultAlpha: 16 });
    await engine.initialize?.();
    return engine as ILoRAEngine;
  } catch {
    console.warn('[@swarmdo/plugins] @swarmvector/learning-wasm not available, using fallback');
