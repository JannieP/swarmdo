 ...
 * Uses @swarmvector/wasm for vector storage with HNSW indexing (<1ms search).
 *
 * Features:
 ...
 * @example
 * ```typescript
 * import { reasoningBankPlugin } from '@swarmdo/plugins/examples/swarmvector-plugins';
...
export class ReasoningBank {
   ...
   * When set, uses @swarmdo/embeddings for high-quality embeddings
   */
  private embeddingProvider: ((text: string) => Promise<Float32Array>) | null = null;

  /**
   * Set external embedding provider from @swarmdo/embeddings
   *
   * @example
   * ```typescript
   ...
   * Used when @swarmdo/embeddings is not configured
   */
...
export const reasoningBankPlugin = new PluginBuilder('reasoning-bank', '1.0.0')
  .withDescription('Store and retrieve reasoning trajectories using @swarmvector/wasm HNSW indexing')
  .withAuthor('Swarmdo Team')
  .withTags(['reasoning', 'memory', 'learning', 'swarmvector', 'hnsw'])
  .withMCPTools([
  ...
  .onInitialize(async (ctx) => {
    ctx.logger.info('ReasoningBank plugin initializing with @swarmvector/wasm...');
