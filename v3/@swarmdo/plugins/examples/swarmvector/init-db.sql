-- SwarmVector PostgreSQL Bridge - Database Initialization
--
...
-- Create a schema for SwarmVector-specific objects
CREATE SCHEMA IF NOT EXISTS swarmvector;

-- Set search path to include swarmvector schema
ALTER DATABASE vectors SET search_path TO public, swarmvector;

-- Create a function to normalize vectors
CREATE OR REPLACE FUNCTION swarmvector.normalize_vector(v vector)
RETURNS vector AS $$
DECLARE
    magnitude float;
BEGIN
...
CREATE OR REPLACE FUNCTION swarmvector.cosine_similarity(a vector, b vector)
RETURNS float AS $$
BEGIN
    RETURN 1 - (a <=> b);
END;
...
CREATE OR REPLACE FUNCTION swarmvector.insert_normalized(
    table_name text,
    id_col text,
    vec_col text,
    ids text[],
...
BEGIN
    FOR i IN 1..array_length(ids, 1) LOOP
        EXECUTE format(
            'INSERT INTO %I (%I, %I) VALUES ($1, swarmvector.normalize_vector($2))',
...
CREATE OR REPLACE FUNCTION swarmvector.analyze_vectors(
    table_name text,
    vec_col text
) RETURNS TABLE (
    total_count bigint,
...
CREATE TABLE IF NOT EXISTS swarmvector.sample_vectors (
    id text PRIMARY KEY,
...
-- Grant permissions
GRANT ALL ON SCHEMA swarmvector TO postgres;
GRANT ALL ON ALL TABLES IN SCHEMA swarmvector TO postgres;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA swarmvector TO postgres;

-- Log completion
DO $$
BEGIN
    RAISE NOTICE 'SwarmVector database initialization complete';
