/**
 * SwarmVector PostgreSQL Bridge - Hyperbolic Embeddings Module
 *
 ...
 *
 * @module @swarmdo/plugins/integrations/swarmvector/hyperbolic
 * @version 1.0.0
 */
...
// ============================================================================
// SQL Generation for SwarmVector PostgreSQL Functions
// ============================================================================

/**
 * SQL function call builder for SwarmVector hyperbolic operations.
 */
export class HyperbolicSQL {
  ...
  static poincareDistance(column: string, query: number[], curvature: number): string {
    ...
    return `swarmvector_poincare_distance(${column}, ${vectorStr}, ${curvature})`;
  }
  ...
  static lorentzDistance(column: string, query: number[], curvature: number): string {
    const vectorStr = `'[${query.join(',')}]'::vector`;
    return `swarmvector_lorentz_distance(${column}, ${vectorStr}, ${curvature})`;
  }
  ...
  static expMap(
    ...
    const funcName = model === 'lorentz' ? 'swarmvector_lorentz_exp_map' : 'swarmvector_poincare_exp_map';
  ...
  static logMap(
    ...
    const funcName = model === 'lorentz' ? 'swarmvector_lorentz_log_map' : 'swarmvector_poincare_log_map';
  ...
  static mobiusAdd(aColumn: string, bColumn: string, curvature: number): string {
    return `swarmvector_poincare_mobius_add(${aColumn}, ${bColumn}, ${curvature})`;
  }
  ...
  static mobiusMatVec(matrixColumn: string, vectorColumn: string, curvature: number): string {
    return `swarmvector_poincare_mobius_matvec(${matrixColumn}, ${vectorColumn}, ${curvature})`;
  }
  ...
  static parallelTransport(
    ...
    const funcName = model === 'lorentz'
      ? 'swarmvector_lorentz_parallel_transport'
      : 'swarmvector_poincare_parallel_transport';
  ...
  static centroid(column: string, model: HyperbolicModel, curvature: number): string {
    const funcName = model === 'lorentz'
      ? 'swarmvector_lorentz_centroid'
      : 'swarmvector_poincare_centroid';
  ...
  static poincareToLorentz(column: string, curvature: number): string {
    return `swarmvector_poincare_to_lorentz(${column}, ${curvature})`;
  }
  ...
  static lorentzToPoincare(column: string, curvature: number): string {
    return `swarmvector_lorentz_to_poincare(${column}, ${curvature})`;
  }
  ...
  static batchNearestNeighbors(
    ...
    const distFunc = model === 'lorentz'
      ? `swarmvector_lorentz_distance(t.${vectorColumn}, q.query_vec, ${curvature})`
      : `swarmvector_poincare_distance(t.${vectorColumn}, q.query_vec, ${curvature})`;
