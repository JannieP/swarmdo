...
const PLUGIN_NAME = 'swarmvector-postgres';
...
class ConnectionManager extends EventEmitter {
  ...
  private readonly config: SwarmVectorConfig;
  ...
  async initialize(): Promise<ConnectionResult> {
    ...
    try {
      ...
      const result = await client.query<{ version: string; swarmvector_version?: string }>(
        "SELECT version() as version, COALESCE(swarmvector.version(), 'N/A') as swarmvector_version"
      );
      client.release();
      ...
      const connectionResult: ConnectionResult = {
        ...
        swarmVectorVersion: result.rows[0]?.swarmvector_version ?? 'N/A',
        parameters: {
          host: this.config.host,
  ...
  /**
   * Build pool configuration from SwarmVector config.
   */
  private buildPoolConfig(): PgPoolConfig {
    ...
    return {
      ...
      application_name: this.config.applicationName ?? 'swarmdo-swarmvector',
    };
  }
...
class VectorOps {
  ...
  constructor(connectionManager: ConnectionManager, config: SwarmVectorConfig) {
 ...
 * ```
 */
export class SwarmVectorBridge extends BasePlugin {
  private readonly swarmVectorConfig: SwarmVectorConfig;
  private connectionManager: ConnectionManager | null = null;
  private vectorOps: VectorOps | null = null;
  private metrics: SwarmVectorMetrics;
  ...
  constructor(config: SwarmVectorConfig) {
    super({
      ...
      description: 'SwarmVector PostgreSQL Bridge for Swarmdo v3 - Advanced vector database integration',
      tags: ['database', 'vector', 'postgresql', 'search', 'embeddings'],
    });

    this.swarmVectorConfig = config;
  ...
  protected async onInitialize(): Promise<void> {
    this.logger.info('Initializing SwarmVector PostgreSQL Bridge...');
    this.initTime = new Date();

    // Create connection manager
    this.connectionManager = new ConnectionManager(this.swarmVectorConfig);
    ...
    try {
      ...
      this.logger.info(`SwarmVector extension version: ${connectionResult.swarmVectorVersion}`);

      // Initialize vector operations
      this.vectorOps = new VectorOps(this.connectionManager, this.swarmVectorConfig);

      // Ensure pgvector extension is available
      await this.ensureExtension();

      this.eventBus.emit('swarmvector:initialized', {
        connectionResult,
        timestamp: new Date(),
      });
    } catch (error) {
      this.logger.error('Failed to initialize SwarmVector Bridge', error);
      throw error;
    }
  }
  ...
  protected async onShutdown(): Promise<void> {
    this.logger.info('Shutting down SwarmVector PostgreSQL Bridge...');
    ...
    this.eventBus.emit('swarmvector:shutdown', {
  ...
  override registerMCPTools(): MCPToolDefinition[] {
    return [
      ...
      // Health Check Tool
      {
        name: 'swarmvector_health',
        description: 'Check the health status of the SwarmVector connection and database.',
        inputSchema: {
          type: 'object',
          properties: {},
        },
  ...
  private ensureInitialized(): void {
    if (!this.vectorOps || !this.connectionManager) {
      throw new Error('SwarmVector Bridge not initialized. Call initialize() first.');
    }
  }
  ...
  private forwardConnectionEvents(): void {
    const events: SwarmVectorEventType[] = [
      'connection:open',
      'connection:close',
      'connection:error',
    ...
    for (const event of events) {
      this.connectionManager!.on(event, (data) => {
        this.eventBus.emit(`swarmvector:${event}`, data);
   ...
   */
  private createInitialMetrics(): SwarmVectorMetrics {
    return {
      queriesTotal: 0,
      queriesSucceeded: 0,
      queriesFailed: 0,
 ...
 * ```
 */
export function createSwarmVectorBridge(config: SwarmVectorConfig): SwarmVectorBridge {
  return new SwarmVectorBridge(config);
}
