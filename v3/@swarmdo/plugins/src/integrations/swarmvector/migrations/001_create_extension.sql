...
-- SwarmVector PostgreSQL Bridge - Claude Flow V3
--
...
-- ----------------------------------------------------------------------------
-- SwarmVector Extension (if available)
-- Provides advanced neural search capabilities
-- ----------------------------------------------------------------------------
DO $$
BEGIN
    -- Try to create SwarmVector extension if available
    BEGIN
        CREATE EXTENSION IF NOT EXISTS swarmvector;
        RAISE NOTICE 'SwarmVector extension enabled successfully';
    EXCEPTION
        WHEN undefined_file THEN
            RAISE NOTICE 'SwarmVector extension not available - using pgvector only';
        WHEN OTHERS THEN
            RAISE NOTICE 'SwarmVector extension not available: %', SQLERRM;
    END;
END $$;
...
COMMENT ON SCHEMA claude_flow IS 'Claude Flow V3 - SwarmVector PostgreSQL Bridge schema';
...
-- ----------------------------------------------------------------------------
-- Configuration table for SwarmVector settings
-- ----------------------------------------------------------------------------
...
-- DROP EXTENSION IF EXISTS swarmvector;
