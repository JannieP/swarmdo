/**
 * SwarmVector PostgreSQL Bridge - Vector Quantization Module
 *
 ...
 *
 * @module @swarmdo/plugins/integrations/swarmvector/quantization
 * @version 1.0.0
 */
...
WITH candidates AS (
  SELECT id, original_vector, metadata,
         swarmvector_scalar_distance(quantized_vector, $1::bytea) AS approx_dist
  FROM ${tableName}
...
LIMIT ${k};
    ...
    return `
SELECT id, metadata,
       swarmvector_scalar_distance(quantized_vector, $1::bytea) AS distance
FROM ${tableName}
...
LIMIT ${k};
  ...
  static pqSearchSQL(
    ...
    const distanceTerms = Array.from(
      ...
      (_, m) => `swarmvector_pq_subvector_dist($1, ${m}, get_byte(pq_codes, ${m}))`
    ).join(' + ');
