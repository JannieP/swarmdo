/**
 * SwarmVector PostgreSQL Bridge - Graph Neural Network (GNN) Layers Module
 *
 * Comprehensive GNN support for SwarmVector PostgreSQL vector database integration.
 * Implements GCN, GAT, GraphSAGE, GIN, MPNN, EdgeConv, and more.
 *
 * @module @swarmdo/plugins/integrations/swarmvector/gnn
 * @version 1.0.0
 */
...
export const GNN_SQL_FUNCTIONS = {
  gcn: 'swarmvector.gcn_layer',
  gat: 'swarmvector.gat_layer',
  gat_v2: 'swarmvector.gat_v2_layer',
  sage: 'swarmvector.sage_layer',
  gin: 'swarmvector.gin_layer',
  mpnn: 'swarmvector.mpnn_layer',
  edge_conv: 'swarmvector.edge_conv_layer',
  point_conv: 'swarmvector.point_conv_layer',
  transformer: 'swarmvector.graph_transformer_layer',
  pna: 'swarmvector.pna_layer',
  film: 'swarmvector.film_layer',
  rgcn: 'swarmvector.rgcn_layer',
  hgt: 'swarmvector.hgt_layer',
  han: 'swarmvector.han_layer',
  metapath: 'swarmvector.metapath_layer',
} as const;
...
export abstract class BaseGNNLayer implements IGNNLayer {
  ...
  toSQL(tableName: string, options: SQLGenerationOptions = {}): string {
    ...
    const sqlFunction = GNN_SQL_FUNCTIONS[this.type] ?? 'swarmvector.gnn_layer';
...
SELECT swarmvector.page_rank(
  ...
  communityDetectionSQL(tableName: string, options: CommunityOptions & SQLGenerationOptions): string {
    ...
    return `
SELECT swarmvector.community_detection(
  (SELECT array_agg(ARRAY[source_id::text, target_id::text]) FROM "${schema}"."${edgeTable}"),
  '${algorithm}',
  ${resolution}
);`.trim();
...
/**
 * SQL generator for GNN operations in PostgreSQL with SwarmVector.
 */
export class GNNSQLGenerator {
  ...
  static batchGNNSQL(
    ...
    return `
SELECT swarmvector.batch_gnn_forward(
  (SELECT array_agg(${nodeColumn}) FROM "${schema}"."${tableName}"),
  (SELECT array_agg(ARRAY[source_id, target_id]) FROM "${schema}"."${edgeTable}"),
...
SELECT
  n.id,
  swarmvector.vector_${aggFunction}(array_agg(neighbor.${nodeColumn})) AS aggregated_embedding
...
GROUP BY n.id;`.trim();
  ...
  static graphPoolingSQL(
    ...
    return `
SELECT swarmvector.${poolFunction}(
  (SELECT array_agg(${nodeColumn}) FROM "${schema}"."${tableName}")
