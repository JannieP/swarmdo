# SwarmVector PostgreSQL Bridge Plugin Architecture
...
| Version | 1.0.0 |
| Status | Draft |
| Author | Swarmdo Architecture Team |
...
The SwarmVector PostgreSQL Bridge plugin provides a high-performance integration layer between Swarmdo v3 and PostgreSQL databases enhanced with SwarmVector extensions. This plugin exposes 53+ SQL functions for vector operations, 39 attention mechanisms, Graph Neural Network (GNN) layers, hyperbolic embeddings, and self-learning capabilities through MCP tools.

### Key Capabilities

- **Vector Operations**: Similarity search, clustering, quantization (~1.9x-4.7x measured faster)
...
+------------------------------------------------------------------+
                                ...
                                v
+------------------------------------------------------------------+
|                    PostgreSQL + SwarmVector Extensions               |
|  +------------------+  +------------------+  +----------------+   |
|  | pg_swarmvector      |  | pg_swarmvector_gnn  |  | pg_swarmvector    |   |
...
```
@swarmdo/plugins/src/integrations/swarmvector/
|
 ...
 * for PostgreSQL instances with SwarmVector extensions.
 */
...
interface ConnectionConfig {
  ...
  extensions: {
    swarmvector: boolean;
    swarmvector_gnn?: boolean;
    swarmvector_attention?: boolean;
  };
}
```
...
| `mha_flash` | Flash Attention v2 | Long sequences (unverified (no benchmark) speedup) |
...
```
+-------------+     +-----------------+     +------------------+
|   Client    |     |  SwarmVector       |     |   PostgreSQL     |
|   Request   | --> |  Bridge Plugin  | --> |   + SwarmVector     |
+-------------+     +-----------------+     +------------------+
      |                    |                        |
      | 1. MCP Tool Call   |                        |
      | (vector_search)    |                        |
...
```typescript
import { BasePlugin } from '@swarmdo/plugins';

export class SwarmVectorPostgreSQLBridgePlugin extends BasePlugin {
  // Metadata
  static readonly PLUGIN_NAME = 'swarmvector-postgresql-bridge';
...
```typescript
interface SwarmVectorBridgeConfig {
  // Connection settings
  connection: {
    host: string;
    port: number;
...
|--------|------|-------------|
| `swarmvector_query_duration_seconds` | Histogram | Query execution time |
| `swarmvector_pool_connections_active` | Gauge | Active connections |
| `swarmvector_pool_connections_idle` | Gauge | Idle connections |
| `swarmvector_index_size_bytes` | Gauge | Index storage size |
| `swarmvector_attention_tokens_processed` | Counter | Tokens processed |
| `swarmvector_learning_patterns_detected` | Counter | Patterns detected |
...
```sql
-- Convert pgvector to swarmvector
ALTER TABLE items
  ALTER COLUMN embedding TYPE swarmvector
  USING embedding::swarmvector;

-- Rebuild indexes
DROP INDEX IF EXISTS items_embedding_idx;
CREATE INDEX items_embedding_idx ON items
  USING swarmvector_hnsw (embedding swarmvector_cosine_ops)
...
async function migrate(agentDB: AgentDBBridge, pgBridge: SwarmVectorBridge) {
