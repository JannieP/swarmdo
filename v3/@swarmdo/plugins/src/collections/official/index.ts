...
/**
 * SwarmVector PostgreSQL Bridge plugin - advanced vector database with AI capabilities.
 *
 * Provides integration with @swarmvector/postgres-cli including:
 ...
 */
export const swarmvectorPostgresPlugin = new PluginBuilder('swarmvector-postgres', '3.0.0')
  .withDescription('SwarmVector PostgreSQL Bridge - Advanced vector search with attention, GNN, and hyperbolic embeddings')
  .withAuthor('Swarmdo')
  .withTags(['database', 'vector', 'postgresql', 'attention', 'gnn', 'hyperbolic', 'intelligence'])
  .withDependencies(['memory-coordinator'])
  .withMCPTools([
    {
      name: 'swarmvector-search',
  ...
  .withHooks([
    {
      event: HookEvent.PostMemoryStore,
      priority: HookPriority.Normal,
      name: 'swarmvector-learn-pattern',
      async: true,
      handler: async (ctx) => {
    ...
    },
    {
      event: HookEvent.PostToolUse,
      priority: HookPriority.Low,
      name: 'swarmvector-collect-stats',
      async: true,
      handler: async (ctx) => {
...
/**
 * Core collection - essential plugins for Swarmdo operation.
 */
export const coreCollection: PluginCollection = {
  id: 'swarmdo-core',
  name: 'Swarmdo Core Plugins',
  version: '3.0.0',
  description: 'Essential plugins for Swarmdo operation',
  author: 'Swarmdo',
  license: 'MIT',
...
export const intelligenceCollection: PluginCollection = {
  ...
  plugins: [
    ...
    {
      plugin: swarmvectorPostgresPlugin,
      defaultEnabled: false,
      category: 'database',
      tags: ['intelligence', 'vector', 'postgresql', 'attention', 'gnn'],
      requiredCapabilities: ['memory', 'database'],
      description: 'SwarmVector PostgreSQL Bridge - Advanced vector search with 39 attention mechanisms, GNN layers, and hyperbolic embeddings',
    },
  ],
};
  ...
  plugins: [
    {
      ...
      description: 'SwarmVector PostgreSQL - 52K+ inserts/sec, sub-ms queries, 39 attention mechanisms, GNN, hyperbolic embeddings',
    },
  ],
};
