...
[![npm version](https://img.shields.io/npm/v/@swarmdo/neural.svg)](https://www.npmjs.com/package/@swarmdo/neural)
[![npm downloads](https://img.shields.io/npm/dm/@swarmdo/neural.svg)](https://www.npmjs.com/package/@swarmdo/neural)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0+-blue.svg)](https://www.typescriptlang.org/)

> Self-Optimizing Neural Architecture (SONA) for Swarmdo V3 — adaptive learning, trajectory tracking, pattern reuse, and 7 RL algorithms in a single package.

## What this is

A self-contained learning module that records agent execution trajectories, distills them into reusable patterns, retrieves matches for new tasks, and adapts via SONA + LoRA + EWC++. Designed to be the substrate that the Swarmdo CLI's intelligence layer composes onto — the package owns the algorithms, the CLI owns the orchestration.

## Install

```bash
npm install @swarmdo/neural
```

> **Note (2026-05-16):** `@swarmdo/neural@3.0.0-alpha.9+` pins
> `@swarmvector/sona` to the exact known-good `0.1.5` because
> `@swarmvector/sona@0.1.6` shipped as an empty publish (README +
> `package.json` only — no `index.js`, no native bins). Prior alpha.8
> used `"latest"` and broke on every fresh install. The pin will
> stay until `@swarmvector/sona@0.1.7+` ships with content.

## Standalone use (without the Swarmdo CLI)

```typescript
// route a task across 8 specialized experts (MoE) — no other deps
import { getMoERouter } from '@swarmdo/neural';
...
```typescript
import { createNeuralLearningSystem } from '@swarmdo/neural';
...
```typescript
import { createSONAManager, type Trajectory } from '@swarmdo/neural';
...
import { createAlgorithm, getDefaultConfig } from '@swarmdo/neural';
...
If you're building a Swarmdo plugin that wants neural learning, depend on `@swarmdo/neural` directly rather than reaching into cli internals.

## Dependencies

- [`@swarmdo/memory`](../memory) — vector memory for patterns
- `@swarmvector/sona` — SONA learning engine

## Related packages

- [`@swarmdo/memory`](../memory) — memory backend
- [`@swarmdo/cli`](../cli) — primary consumer + MCP tool surface
- [`@swarmdo/cli-core`](../cli-core) — lite path (no neural; for plugin scripts)
