/**
 * Swarmdo Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@swarmdo.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'swarmdo';
const BUCKET_NAME = 'swarmdo-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `swarmdo-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'swarmdo-plugin-registry', pinataJwt);
