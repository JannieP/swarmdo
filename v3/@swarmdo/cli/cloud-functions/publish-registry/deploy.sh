#!/bin/bash
# Deploy Swarmdo Registry Cloud Function

set -e

PROJECT_ID="${GCP_PROJECT:-swarmdo}"
...
gsutil mb -p $PROJECT_ID gs://swarmdo-plugin-registry 2>/dev/null || true
...
echo ""
echo "Rate a plugin:"
echo "  curl -X POST '$FUNCTION_URL?action=rate' -H 'Content-Type: application/json' -d '{\"itemId\":\"@swarmdo/embeddings\",\"rating\":5}'"
