...
| `usage` | 10 | Claude Code token/cost analytics from local transcripts (daily, monthly, models, projects, sessions, 5h `blocks` w/ live burn, `errors` tool-failure analytics, `cache` prompt-cache efficiency + $ saved, `guard` budget policy w/ `--strict` exit, `reflect` wrapped-style retrospective `[--period 1m\|3m\|6m\|12m]`; alias: `cost`) |
