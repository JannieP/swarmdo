# Claude Code Configuration - Swarmdo V3
...
```bash
npx @swarmdo/cli@latest hooks pre-task --description "[task description]"
```
...
npx @swarmdo/cli@latest swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
Bash("npx @swarmdo/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("npx @swarmdo/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @swarmdo/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @swarmdo/cli@latest hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("npx @swarmdo/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @swarmdo/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @swarmdo/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @swarmdo/cli@latest hooks worker dispatch --trigger optimize")
```
...
| `usage` | 10 | Claude Code token/cost analytics from local transcripts (daily, weekly, monthly, models, projects, sessions, 5h `blocks` w/ live burn, `errors` tool-failure analytics, `cache` prompt-cache efficiency + $ saved, `guard` budget policy w/ `--strict` exit, `reflect` wrapped-style retrospective `[--period 1m\|3m\|6m\|12m]`, `limits` quota-cap forecaster (reads the statusline `rate_limits` payload from stdin/`--payload`); alias: `cost`) |
...
npx @swarmdo/cli@latest daemon start

# Spawn an agent
npx @swarmdo/cli@latest agent spawn -t coder --name my-coder

# Initialize swarm
npx @swarmdo/cli@latest swarm init --v3-mode

# Search memory (HNSW-indexed)
npx @swarmdo/cli@latest memory search --query "authentication patterns"

# System diagnostics
npx @swarmdo/cli@latest doctor --fix

# Security scan
npx @swarmdo/cli@latest security scan --depth full

# Performance benchmark
npx @swarmdo/cli@latest performance benchmark --suite all
```
...
| `intelligence` | SwarmVector intelligence system | `trajectory-*`, `pattern-*`, `stats` |
...
```bash
# Core hooks
npx @swarmdo/cli@latest hooks pre-task --description "[task]"
npx @swarmdo/cli@latest hooks post-task --task-id "[id]" --success true
npx @swarmdo/cli@latest hooks post-edit --file "[file]" --train-neural true

# Session management
npx @swarmdo/cli@latest hooks session-start --session-id "[id]"
npx @swarmdo/cli@latest hooks session-end --export-metrics true
npx @swarmdo/cli@latest hooks session-restore --session-id "[id]"

# Intelligence routing
npx @swarmdo/cli@latest hooks route --task "[task]"
npx @swarmdo/cli@latest hooks explain --topic "[topic]"
...
# Background workers
npx @swarmdo/cli@latest hooks worker list
npx @swarmdo/cli@latest hooks worker dispatch --trigger audit
npx @swarmdo/cli@latest hooks worker status

# Coverage-aware routing
npx @swarmdo/cli@latest hooks coverage-gaps --format table
npx @swarmdo/cli@latest hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
npx @swarmdo/cli@latest hooks statusline
npx @swarmdo/cli@latest hooks statusline --json
```
...
npx @swarmdo/cli@latest migrate status

# Run migration with backup
npx @swarmdo/cli@latest migrate run --backup

# Rollback if needed
npx @swarmdo/cli@latest migrate rollback

# Validate migration
npx @swarmdo/cli@latest migrate validate
```
...
Bash("npx @swarmdo/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @swarmdo/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @swarmdo/cli@latest performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("npx @swarmdo/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @swarmdo/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("npx @swarmdo/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @swarmdo/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @swarmdo/cli@latest neural patterns --list")
```
...
Run `npx @swarmdo/cli@latest doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
claude mcp add swarmdo-swarm -- npx -y swarmdo-swarm mcp start  # Optional
...
- **Swarm init**: `npx @swarmdo/cli@latest swarm init --topology <type>`
- **Swarm status**: `npx @swarmdo/cli@latest swarm status`
- **Agent spawn**: `npx @swarmdo/cli@latest agent spawn -t <type> --name <name>`
- **Memory store**: `npx @swarmdo/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `npx @swarmdo/cli@latest memory search --query "search terms"`
- **Memory list**: `npx @swarmdo/cli@latest memory list --namespace patterns`
- **Memory retrieve**: `npx @swarmdo/cli@latest memory retrieve --key "mykey" --namespace patterns`
- **Hooks**: `npx @swarmdo/cli@latest hooks <hook-name> [options]`
...
npx @swarmdo/cli@latest memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx @swarmdo/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
npx @swarmdo/cli@latest memory search --query "authentication patterns"
npx @swarmdo/cli@latest memory search --query "error handling" --namespace patterns --limit 5
```

### List Entries
```bash
# OPTIONAL: --namespace, --limit
npx @swarmdo/cli@latest memory list
npx @swarmdo/cli@latest memory list --namespace patterns --limit 10
```
...
npx @swarmdo/cli@latest memory retrieve --key "pattern-auth"
npx @swarmdo/cli@latest memory retrieve --key "pattern-auth" --namespace patterns
```

### Initialize Memory Database
```bash
npx @swarmdo/cli@latest memory init --force --verbose
```
...
For a comprehensive overview of all Swarmdo V3 features, agents, commands, and integrations, see:

**`.swarmdo/CAPABILITIES.md`** - Complete reference generated during init
...
Remember: **Swarmdo CLI coordinates, Claude Code Task tool creates!**
