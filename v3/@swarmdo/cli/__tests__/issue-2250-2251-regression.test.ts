...
import { ModelRouter } from '../src/swarmvector/model-router.js';
...
describe('#2250 — escalation does not override learned opus suppression', () => {
  ...
  it('respects SWARMDO_MAX_UNCERTAINTY env override', async () => {
    ...
    const old = process.env.SWARMDO_MAX_UNCERTAINTY;
    process.env.SWARMDO_MAX_UNCERTAINTY = '0.99';
    try {
      ...
      if (old === undefined) delete process.env.SWARMDO_MAX_UNCERTAINTY;
      else process.env.SWARMDO_MAX_UNCERTAINTY = old;
    }
  });
});
