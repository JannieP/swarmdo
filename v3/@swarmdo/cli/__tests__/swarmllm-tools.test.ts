/**
 * Tests for swarmllm-wasm MCP tools.
...
vi.mock('../src/swarmvector/swarmllm-wasm.js', () => ({
  isSwarmllmWasmAvailable: vi.fn().mockResolvedValue(true),
  initSwarmllmWasm: vi.fn().mockResolvedValue(undefined),
  getSwarmllmStatus: vi.fn().mockResolvedValue({ available: true, initialized: true, version: '2.0.1' }),
...
import { swarmllmWasmTools } from '../src/mcp-tools/swarmllm-tools.js';

function findTool(name: string) {
  const tool = swarmllmWasmTools.find(t => t.name === name);
  if (!tool) throw new Error(`Tool not found: ${name}`);
  return tool;
}

// Same WASM-init issue as swarmllm-wasm.test.ts — mocks intercept the
// integration layer but the real @swarmvector/swarmllm-wasm package still
// loads transitively and crashes in CI without prebuilds.
const __SKIP_WASM_TESTS = process.env.CI === 'true';

describe.skipIf(__SKIP_WASM_TESTS)('swarmllm-wasm MCP tools', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should export 10 tools', () => {
    expect(swarmllmWasmTools).toHaveLength(10);
  });

  describe('swarmllm_status', () => {
    it('should return status', async () => {
      const tool = findTool('swarmllm_status');
  ...
  describe('swarmllm_hnsw_create', () => {
    it('should create router and return ID', async () => {
      const tool = findTool('swarmllm_hnsw_create');
  ...
  describe('swarmllm_hnsw_add', () => {
    it('should add pattern to router', async () => {
      // First create
      const createTool = findTool('swarmllm_hnsw_create');
      ...
      const addTool = findTool('swarmllm_hnsw_add');
  ...
  describe('swarmllm_sona_create', () => {
    it('should create SONA instance', async () => {
      const tool = findTool('swarmllm_sona_create');
  ...
  describe('swarmllm_microlora_create', () => {
    it('should create MicroLoRA', async () => {
      const tool = findTool('swarmllm_microlora_create');
  ...
  describe('swarmllm_microlora_adapt', () => {
    it('should adapt with feedback', async () => {
      const createTool = findTool('swarmllm_microlora_create');
      ...
      const adaptTool = findTool('swarmllm_microlora_adapt');
  ...
  describe('swarmllm_chat_format', () => {
    it('should format with preset', async () => {
      const tool = findTool('swarmllm_chat_format');
  ...
  describe('swarmllm_generate_config', () => {
    it('should create config', async () => {
      const tool = findTool('swarmllm_generate_config');
