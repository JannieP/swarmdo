/**
 * ADR-087: @swarmvector/graph-node native graph database backend tests
 *
...
describe('ADR-087: graph-node backend', () => {
  describe('module exports', () => {
    it('exports all expected functions', async () => {
      const mod = await import('../src/swarmvector/graph-backend.js');
  ...
  describe('graceful degradation', () => {
    it('getGraphStats returns unavailable when graph-node not loaded', async () => {
      ...
      const { getGraphStats } = await import('../src/swarmvector/graph-backend.js');
    ...
    it('addNode returns string or null', async () => {
      const { addNode } = await import('../src/swarmvector/graph-backend.js');
    ...
    it('getNeighbors returns array', async () => {
      const { getNeighbors } = await import('../src/swarmvector/graph-backend.js');
      const result = await getNeighbors('nonexistent', 2);
      expect(Array.isArray(result)).toBe(true);
    });

    it('recordCausalEdge returns result with backend', async () => {
      const { recordCausalEdge } = await import('../src/swarmvector/graph-backend.js');
    ...
    it('recordSwarmTeam returns result', async () => {
      const { recordSwarmTeam } = await import('../src/swarmvector/graph-backend.js');
  ...
  describe('CJS import pattern', () => {
    it('source uses createRequire for @swarmvector/graph-node', async () => {
      const { readFileSync } = await import('fs');
      const { join } = await import('path');
      const content = readFileSync(join(import.meta.dirname, '..', 'src', 'swarmvector', 'graph-backend.ts'), 'utf8');
      expect(content).toContain('createRequire');
      expect(content).toContain("'@swarmvector/graph-node'");
      // Should NOT use ESM dynamic import for graph-node
      expect(content).not.toMatch(/await import\(['"]@swarmvector\/graph-node['"]\)/);
    });
  });
  ...
  describe('type exports', () => {
    ...
    it('exports GraphEdgeData interface shape', async () => {
      const { addEdge } = await import('../src/swarmvector/graph-backend.js');
