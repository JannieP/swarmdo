 ...
 * error — `swarmdo memory list` / `memory search` on first run should succeed
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'swarmdo-empty-'));
  prev = process.env.SWARMDO_MEMORY_PATH;
  process.env.SWARMDO_MEMORY_PATH = dir; // fresh, no memory.db
});
afterEach(() => {
  if (prev === undefined) delete process.env.SWARMDO_MEMORY_PATH; else process.env.SWARMDO_MEMORY_PATH = prev;
