...
import { createQLearningRouter } from '../src/swarmvector/q-learning-router.js';
...
describe('#2221 — statusline version probe covers global npm installs', () => {
  ...
  it('still keeps the project-local and plugin-marketplace probes', () => {
    expect(SCRIPT).toContain("'marketplaces', 'swarmdo', 'package.json'");
    expect(SCRIPT).toContain("'node_modules', 'swarmdo', 'package.json'");
  });
});
...
describe('#2222 — route feedback persists the Q-table to disk', () => {
  it('writes the model after a single update + explicit save', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'swarmdo-qlearn-'));
...
describe('#2226 — pattern store and search share a backend', () => {
  ...
  // (getMemoryRoot honors SWARMDO_MEMORY_PATH dynamically).
  let memDir: string;
  let prevMemPath: string | undefined;
  beforeEach(() => {
    memDir = mkdtempSync(path.join(tmpdir(), 'swarmdo-2226-'));
  ...
  it('stores a pattern and finds it via search', async () => {
    ...
    // Semantic retrieval needs a real vector index (HNSW via @swarmvector/core).
    ...
    // wherever the index is available. Mirrors the store-unavailable guard above.
...
describe('3.10.8 #bugB — Q-router reflects a learned update immediately (no 50-update cache lag)', () => {
  it('changes the exploited route within a handful of updates', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'swarmdo-qcache-'));
    try {
