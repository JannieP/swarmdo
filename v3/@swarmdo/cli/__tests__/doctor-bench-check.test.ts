 ...
 *
 * Verifies the health-check honestly reflects `.swarmdo/bench-results.json`:
...
let dir: string;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'swarmdo-doc-')); });
afterEach(() => { try { rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ } });

function writeBench(payload: unknown): void {
  const d = join(dir, '.swarmdo');
