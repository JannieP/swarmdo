/**
 * storage-migration tests — clean-break rename (.claude-flow -> .swarmdo).
 *
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'swarmdo-mig-'));
});
afterEach(() => {
...
describe('migrateStorageDir', () => {
  it('returns noop when neither directory exists', () => {
    ...
    expect(existsSync(join(dir, '.swarmdo'))).toBe(false);
  });

  it('renames .claude-flow -> .swarmdo and preserves contents', () => {
  ...
  it('is a noop when .swarmdo already exists (does not clobber)', () => {
    ...
    expect(readFileSync(join(dir, '.swarmdo', 'keep.txt'), 'utf8')).toBe('new-data');
