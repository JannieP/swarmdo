...
// Mock swarmvector modules
vi.mock('../src/swarmvector/model-router.js', () => ({
  getModelRouter: vi.fn(() => ({ route: async () => ({ model: 'sonnet', routedBy: 'router' }) })),
}));

vi.mock('../src/swarmvector/enhanced-model-router.js', () => ({
  getEnhancedModelRouter: vi.fn(() => ({
    route: async () => ({ tier: 2, model: 'sonnet', canSkipLLM: false }),
  })),
}));

vi.mock('../src/swarmvector/diff-classifier.js', () => ({
...
vi.mock('../src/swarmvector/moe-router.js', () => ({
...
vi.mock('@swarmdo/aidefence', () => {
  throw new Error('Cannot find package');
});

// Mock embeddings package
vi.mock('@swarmdo/embeddings', () => {
  throw new Error('Cannot find package');
});
