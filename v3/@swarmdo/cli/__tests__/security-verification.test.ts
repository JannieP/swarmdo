...
describe('Scenario 1: SQL injection in embeddings search', () => {
  ...
  it('import.ts formatEmbedding must validate array elements', async () => {
    const { readFileSync } = await import('fs');
    const source = readFileSync(
      new URL('../src/commands/swarmvector/import.ts', import.meta.url),
      'utf-8'
    );
...
describe('Scenario 3: Container name injection prevention', () => {
  ...
  it('should validate container names with strict regex', () => {
    ...
    // Valid names
    expect(validPattern.test('swarmvector-postgres')).toBe(true);
...
describe('Scenario 6: Browser eval length limit and pattern blocking (defense-in-depth)', () => {
  it('should enforce configurable max script length (default 20KB)', async () => {
    ...
    expect(source).toContain('SWARMDO_MAX_EVAL_SCRIPT_LENGTH');
...
// ============================================================================
// Scenario 9: SwarmVector Schema Name & Timestamp Injection (DA-CRIT-2, DA-HIGH-3)
// ============================================================================
describe('Scenario 9: Schema name and timestamp validation', () => {
  it('validateSchemaName should reject SQL injection payloads', async () => {
    const { validateSchemaName } = await import(
      '../src/commands/swarmvector/pg-utils.js'
    );
  ...
  it('all swarmvector commands must validate schema names', async () => {
    const { readFileSync } = await import('fs');
    const commands = [
      '../src/commands/swarmvector/init.ts',
      '../src/commands/swarmvector/backup.ts',
      '../src/commands/swarmvector/status.ts',
      '../src/commands/swarmvector/optimize.ts',
      '../src/commands/swarmvector/benchmark.ts',
      '../src/commands/swarmvector/migrate.ts',
    ];
