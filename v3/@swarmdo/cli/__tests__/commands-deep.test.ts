...
import { swarmvectorCommand } from '../src/commands/swarmvector/index.js';
...
describe('Command Definitions', () => {
  ...
  describe('swarmvector command', () => {
    it('should have correct name', () => {
      expectValidCommand(swarmvectorCommand, 'swarmvector');
    });
  });
});
...
const allCommands: { cmd: Command; name: string }[] = [
  ...
  { cmd: swarmvectorCommand, name: 'swarmvector' },
];
...
describe('Init System', () => {
  ...
  describe('FULL_INIT_OPTIONS', () => {
    ...
    it('should have all MCP servers enabled', () => {
      ...
      expect(FULL_INIT_OPTIONS.mcp.swarmdoSwarm).toBe(true);
    });
  });
  ...
  describe('generateClaudeMd', () => {
    ...
    // imperative schema (header "# Swarmdo — Claude Code Configuration", section
    ...
    it('should contain the Swarmdo header', () => {
      const md = generateClaudeMd(DEFAULT_INIT_OPTIONS);
      expect(md).toContain('# Swarmdo');
    });
    ...
    it('full template should include intelligence/SONA reference', () => {
      const md = generateClaudeMd(DEFAULT_INIT_OPTIONS, 'full');
      // intelligenceSystem() mentions SONA / SwarmVector / HNSW
      expect(md.toLowerCase()).toMatch(/sona|swarmvector|hnsw|intelligence/);
    });
