/**
 * Integration: `swarmdo demo` → `.swarmdo/bench-results.json` → `swarmdo doctor`.
 *
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'swarmdo-demo-doc-'));
  prev = process.env.SWARMDO_CWD;
  process.env.SWARMDO_CWD = dir; // getProjectCwd() honors this for persistence
});
afterEach(() => {
...
describe('demo → doctor bench-results contract', () => {
  it('demo persists to .swarmdo/bench-results.json and doctor reads it (not "absent")', async () => {
    ...
    expect(persisted.source).toBe('swarmdo demo');
