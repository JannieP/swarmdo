/**
 * Regression: the memory embedding loader must use the WORKING swarmvector.embed()
 ...
 * fails in environments where swarmvector.embed() itself works fine — so memory
 * embeddings silently degraded to semantically-useless `mock` hash vectors,
 * breaking semantic memory/pattern search.
 *
 * Skips honestly when swarmvector ONNX genuinely isn't available (edge/CI without
 * the native build) — there, hash IS the correct backend.
 */
import { describe, it, expect } from 'vitest';

const swarmvectorOnnx = await import('swarmvector')
  .then((rv: any) => typeof rv.isOnnxAvailable === 'function' && rv.isOnnxAvailable())
  .catch(() => false);

describe.skipIf(!swarmvectorOnnx)('memory embeddings use real swarmvector ONNX (not hash) when available', () => {
