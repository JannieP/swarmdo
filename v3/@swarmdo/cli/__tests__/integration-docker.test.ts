 ...
 *
 * Validates the Swarmdo Docker-based deployment stack without running Docker.
...
const ROOT = resolve(__dirname, '..', '..', '..', '..');            // /workspaces/swarmdo
const CLI_DIR = resolve(__dirname, '..');                             // v3/@swarmdo/cli
const SWARMDO_DIR = join(ROOT, 'swarmdo');
const COMPOSE_PATH = join(SWARMDO_DIR, 'docker-compose.yml');
const NGINX_CONF_PATH = join(SWARMDO_DIR, 'src', 'nginx', 'nginx.conf');
const NGINX_DOCKERFILE = join(SWARMDO_DIR, 'src', 'nginx', 'Dockerfile');
const MCP_BRIDGE_INDEX = join(SWARMDO_DIR, 'src', 'mcp-bridge', 'index.js');
const MCP_BRIDGE_DOCKERFILE = join(SWARMDO_DIR, 'src', 'mcp-bridge', 'Dockerfile');
const CLI_DOCKERFILE = join(CLI_DIR, 'docker', 'Dockerfile');
const ENV_EXAMPLE = join(SWARMDO_DIR, '.env.example');
...
describe('Docker Compose Configuration', () => {
  ...
  it('mcp-bridge builds from correct context', () => {
    expect(composeContent).toMatch(/context:\s*\.\/src\/swarmvocal\/mcp-bridge/);
  });
...
describe('Nginx Configuration', () => {
  ...
  it('injects Swarmdo welcome.js script via sub_filter', () => {
    expect(nginxContent).toMatch(/sub_filter\s+'<\/head>'\s+'<script src="\/swarmdo\/welcome\.js"/);
  });
  ...
  it('serves /swarmdo/ static assets from /etc/nginx/static/', () => {
    expect(nginxContent).toMatch(/location\s+\/swarmdo\//);
  });
...
describe('MCP Bridge Dockerfile', () => {
  ...
  it('creates writable .swarmdo directories', () => {
    expect(dockerContent).toContain('/app/.swarmdo/tasks');
    expect(dockerContent).toContain('/app/.swarmdo/memory');
    expect(dockerContent).toContain('/app/.swarmdo/sessions');
  });
...
describe('CLI Dockerfile (swarmdo:lite)', () => {
  let dockerContent: string;
  ...
  it('installs swarmdo globally in the build stage', () => {
    expect(dockerContent).toContain('npm install -g swarmdo@latest');
  });
  ...
  it('creates a non-root user (swarmdo)', () => {
    expect(dockerContent).toContain('adduser');
    expect(dockerContent).toContain('swarmdo');
    expect(dockerContent).toContain('USER swarmdo');
  });
  ...
  it('has a HEALTHCHECK using swarmdo doctor', () => {
    expect(dockerContent).toContain('HEALTHCHECK');
    expect(dockerContent).toContain('swarmdo doctor');
  });

  it('default CMD starts MCP server', () => {
    expect(dockerContent).toContain('CMD ["swarmdo", "mcp", "start"]');
  });
});
...
describe('CLI Build', () => {
  it('package.json exists and has correct name', () => {
    ...
    expect(pkg.name).toBe('@swarmdo/cli');
  });
...
describe('Security Checks', () => {
  ...
  it('CLI Dockerfile runs as non-root', () => {
    ...
    expect(dockerfile).toContain('USER swarmdo');
  });
