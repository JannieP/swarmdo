...
describe('WorkerDaemon resource thresholds', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(tempDir, '.swarmdo', 'logs'), { recursive: true });
  });
  ...
  describe('config.json reading', () => {
    it('should read daemon settings from flat dot-notation keys', () => {
      const configFile = join(tempDir, '.swarmdo', 'config.json');
  ...
  describe('state persistence', () => {
    it('should restore resourceThresholds from daemon-state.json', () => {
      const stateFile = join(tempDir, '.swarmdo', 'daemon-state.json');
  ...
  describe('mutation killing', () => {
    ...
    it('should independently check CPU and memory (not conflate them)', async () => {
      ...
      mkdirSync(join(tempDir2, '.swarmdo', 'logs'), { recursive: true });
  ...
  // (SWARMDO_DAEMON_TTL_SECS / SWARMDO_DAEMON_IDLE_SECS) > built-in default.
  // =========================================================================
  describe('self-terminating lifecycle (ttl/idle)', () => {
    const TTL_ENV = 'SWARMDO_DAEMON_TTL_SECS';
    const IDLE_ENV = 'SWARMDO_DAEMON_IDLE_SECS';
    ...
    it('honors SWARMDO_DAEMON_TTL_SECS env override (seconds → ms)', () => {
      process.env[TTL_ENV] = '3600';
      const config = new WorkerDaemon(tempDir).getStatus().config;
      expect(config.ttlMs).toBe(3600 * 1000);
    });

    it('honors an explicit SWARMDO_DAEMON_TTL_SECS=0 as "disabled" (not falling back to default)', () => {
    ...
    it('honors SWARMDO_DAEMON_IDLE_SECS env override', () => {
