 ...
 *      `.swarmdo/agents.json`, a *different* file from the canonical
 *      `.swarmdo/agents/store.json` the agent tools read, so
...
describe('#1916 — agent_logs registered + hive-worker resolution', () => {
  ...
  afterEach(() => {
    if (prevCwd === undefined) delete process.env.SWARMDO_CWD;
    else process.env.SWARMDO_CWD = prevCwd;
  ...
  it('canonical store wins on id collision', async () => {
    ...
    writeFileSync(
      join(dir, '.swarmdo', 'agents', 'store.json'),
      JSON.stringify({
        version: '3.0.0',
        agents: {
          'hive-worker-1716-abcd': {
