 ...
 * `{ agentType, name }` to `@swarmdo/security`'s SpawnAgentSchema, which
...
describe('validateAgentSpawn (#1567)', () => {
  ...
  it('accepts a custom agent type not in the SpawnAgentSchema enum', async () => {
    ...
    // @swarmdo/security's AgentTypeSchema enumerates, but it's a valid
    // Swarmdo agent type and should NOT be rejected.
