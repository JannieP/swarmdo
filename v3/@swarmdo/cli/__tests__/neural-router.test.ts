 ...
 * optional `@swarmvector/tiny-dancer` acceleration) into the model-routing
...
import { __resetNeuralRouterForTests, tryCostOptimalRoute, neuralRouterStatus } from '../src/swarmvector/neural-router.js';
import { __resetTrajectoryRecorderForTests, recordDecision, taskHash, trajectoryRecorderStatus } from '../src/swarmvector/router-trajectory.js';
...
const ENV_KEYS = [
  ...
  'SWARMDO_ROUTER_CALIBRATE',           // iter 24 — default-on; tests should not leak overrides
  'SWARMDO_ROUTER_CALIBRATOR_PATH',
  'SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK',  // iter 29
  'SWARMDO_ROUTER_AB',                          // iter 37
  'SWARMDO_ROUTER_AB_SAMPLE_RATE',
  'SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD',  // iter 44
  'SWARMDO_ROUTER_BANDIT_WARMUP_RANGE',           // iter 52
  'SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE',         // iter 53
  'SWARMDO_ROUTER_BANDIT_SHRINKAGE_LAMBDA',       // iter 57
  'OPENROUTER_API_KEY',
  'ANTHROPIC_API_KEY',
];
...
describe('neural-router (ADR-148)', () => {
  ...
  it('returns null when SWARMDO_ROUTER_NEURAL is not set (gate closed)', async () => {
  ...
  it('calibration is default-ON; SWARMDO_ROUTER_CALIBRATE=0 opts out (ADR-149 iter 24)', async () => {
    ...
    process.env.SWARMDO_ROUTER_CALIBRATE = '1';
    __resetNeuralRouterForTests();
    const sOn = await neuralRouterStatus();
    expect(sOn.reason).toContain('calibrated');

    // Opt-out: '0' bypasses.
    process.env.SWARMDO_ROUTER_CALIBRATE = '0';
    __resetNeuralRouterForTests();
  ...
  it('ensemble-uncertainty threshold triggers null/fallback when unified and specialist disagree (ADR-149 iter 44)', async () => {
    ...
    process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD = '0.0001';
    __resetNeuralRouterForTests();
    ...
    // matches baseline.
    process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD = '0.99';
    __resetNeuralRouterForTests();
  ...
  it('cost-ceiling mode picks highest-quality candidate under budget (ADR-149 iter 29)', async () => {
    // When SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK is set, the selector
    ...
    process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK = '30';
    __resetNeuralRouterForTests();
...
describe('router-trajectory (ADR-148)', () => {
  ...
  it('writes nothing when gate is closed', () => {
    ...
    process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
    __resetTrajectoryRecorderForTests();
    recordDecision({
  ...
  it('writes one JSONL row per call when gate is open', () => {
    const path = join(tmpDir, 'trajectories.jsonl');
    process.env.SWARMDO_ROUTER_TRAJECTORY = '1';
    process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
    __resetTrajectoryRecorderForTests();
    recordDecision({
      task: 'add console.log to cache',
  ...
  it('truncates task text to the configured limit', () => {
    ...
    process.env.SWARMDO_ROUTER_TRAJECTORY = '1';
    process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
    process.env.SWARMDO_ROUTER_TRAJECTORY_TASKLEN = '10';
    __resetTrajectoryRecorderForTests();
    recordDecision({
      task: 'a'.repeat(500),
  ...
  it('exposes accurate status via trajectoryRecorderStatus()', () => {
    process.env.SWARMDO_ROUTER_TRAJECTORY = '1';
    process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = '/tmp/x.jsonl';
    __resetTrajectoryRecorderForTests();
  ...
  it('IsotonicCalibrator: fit + transform corrects monotone bias (ADR-149 iter 22)', async () => {
    const { IsotonicCalibrator } = await import('../src/swarmvector/router-calibrator.js');
  ...
  it('decision rows carry ensemble_disagreement when provided (ADR-149 iter 46)', async () => {
    ...
    try {
      ...
      process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
      __resetTrajectoryRecorderForTests();
      const { recordDecision } = await import('../src/swarmvector/router-trajectory.js');
      ...
      delete process.env.SWARMDO_ROUTER_TRAJECTORY;
      delete process.env.SWARMDO_ROUTER_TRAJECTORY_PATH;
  ...
  it('outcome rows carry tokens/cost_usd/model_id when provided (ADR-149 iter 31)', async () => {
    ...
    try {
      ...
      process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
      __resetTrajectoryRecorderForTests();
      const { recordTrajectoryOutcome } = await import('../src/swarmvector/router-trajectory.js');
      const { MODEL_PRICES } = await import('../src/swarmvector/model-prices.js');
      ...
      process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path2;
      __resetTrajectoryRecorderForTests();
      ...
      process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path3;
      __resetTrajectoryRecorderForTests();
      recordTrajectoryOutcome({
  ...
  it('pairTrajectoryRows reconstructs training rows from decision+outcome (ADR-149 iter 18)', async () => {
    const { pairTrajectoryRows, tierFromComplexity } = await import('../src/swarmvector/router-trajectory.js');
...
describe('ModelRouter integration (ADR-148)', () => {
  ...
  it('result carries routedBy="heuristic" when neural gate is closed (default)', async () => {
    const { resetModelRouter, routeToModelFull } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
  ...
  it('recordModelOutcome updates the bandit prior for the target tier (ADR-149 iter 2)', async () => {
    ...
    const { resetModelRouter, recordModelOutcome, getModelRouterStats } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
  ...
  it('nextCostOptimalAlternative returns a different model when the picked one is excluded (ADR-149 iter 7)', async () => {
    process.env.SWARMDO_ROUTER_NEURAL = '1';
    __resetNeuralRouterForTests();
    const { nextCostOptimalAlternative, tryCostOptimalRoute } = await import('../src/swarmvector/neural-router.js');
    const e = new Array(384).fill(0);
  ...
  it('trajectory recorder pairs decision+outcome by task_hash (ADR-149 iter 17)', async () => {
    ...
    try {
      ...
      process.env.SWARMDO_ROUTER_TRAJECTORY_PATH = path;
      __resetTrajectoryRecorderForTests();
      const { recordDecision, recordTrajectoryOutcome, taskHash } = await import('../src/swarmvector/router-trajectory.js');
  ...
  it('per-modelId Thompson is hooked when gated on (ADR-149 iter 14)', async () => {
    ...
    const { resetModelRouter, recordModelOutcomeByModelId, getModelRouterPriorsById } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
    ...
    process.env.SWARMDO_ROUTER_NEURAL = '1';
    process.env.SWARMDO_ROUTER_BANDIT_PER_MODEL = '1';
    __resetNeuralRouterForTests();
    const { tryCostOptimalRoute } = await import('../src/swarmvector/neural-router.js');
  ...
  it('latency budget filters slow candidates from the pick (ADR-149 iter 12)', async () => {
    ...
    process.env.SWARMDO_ROUTER_LATENCY_BUDGET_MS = '300';
    __resetNeuralRouterForTests();
  ...
  it('embedTaskWithCacheBatch matches single-call results + amortizes setup (ADR-149 iter 11)', async () => {
    const { embedTaskWithCache, embedTaskWithCacheBatch, __resetTaskEmbedderForTests, embedderStats } = await import('../src/swarmvector/task-embedder.js');
  ...
  it('tryCostOptimalRouteBatch matches single-call shape (ADR-149 iter 11)', async () => {
    process.env.SWARMDO_ROUTER_NEURAL = '1';
    __resetNeuralRouterForTests();
    const { tryCostOptimalRoute, tryCostOptimalRouteBatch } = await import('../src/swarmvector/neural-router.js');
  ...
  it('tryCostOptimalRouteBatch returns null entries for invalid embeddings (ADR-149 iter 11)', async () => {
    process.env.SWARMDO_ROUTER_NEURAL = '1';
    __resetNeuralRouterForTests();
    const { tryCostOptimalRouteBatch } = await import('../src/swarmvector/neural-router.js');
  ...
  it('embedTaskWithCache caches by task hash (ADR-149 iter 9)', async () => {
    const { embedTaskWithCache, embedderStats, __resetTaskEmbedderForTests } = await import('../src/swarmvector/task-embedder.js');
    __resetTaskEmbedderForTests();
  ...
  it('recordModelOutcomeByModelId writes shadow per-modelId state (ADR-149 iter 6)', async () => {
    const { resetModelRouter, recordModelOutcomeByModelId, getModelRouterStats } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
...
describe('OpenRouter alternates (ADR-148 phase 2)', () => {
  ...
  it('switches to "openrouter" when SWARMDO_ROUTER_PROVIDER=openrouter', async () => {
    process.env.SWARMDO_ROUTER_PROVIDER = 'openrouter';
  ...
  it('respects explicit ANTHROPIC_API_KEY presence even when OpenRouter key is also set', async () => {
    process.env.ANTHROPIC_API_KEY = 'sk-ant-test';
    process.env.OPENROUTER_API_KEY = 'sk-or-test';
    // No explicit SWARMDO_ROUTER_PROVIDER — defaults to anthropic
    const { resetModelRouter, routeToModelFull } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
    const r = await routeToModelFull('add console.log to cache');
    expect(r.provider).toBe('anthropic');
  ...
  it('explicit SWARMDO_ROUTER_PROVIDER=anthropic overrides both keys', async () => {
    process.env.SWARMDO_ROUTER_PROVIDER = 'anthropic';
    process.env.OPENROUTER_API_KEY = 'sk-or-test';
    const { resetModelRouter, routeToModelFull } = await import('../src/swarmvector/model-router.js');
    resetModelRouter();
