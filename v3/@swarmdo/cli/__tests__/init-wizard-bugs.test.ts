 ...
 *
 * #2206 — mcp-generator must register the server under the 'swarmdo' key
 *          (not 'swarmdo') so all plugins resolve as mcp__swarmdo__*.
 * #2207 — detectExistingSwarmdoMCP must accept both 'swarmdo' and 'swarmdo' keys;
 *          a bare .claude/settings.json must NOT be a false positive.
 * #2208 — writeClaudeMd (--force) must back up an existing CLAUDE.md to
 *          CLAUDE.md.pre-swarmdo before overwriting it.
 */
...
describe('#2206 — mcp-generator server key', () => {
  it('registers the swarmdo MCP server under the swarmdo key', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    expect(config.mcpServers).toHaveProperty('swarmdo');
  });

  // (Post-rename: the historical #2206 distinction between the umbrella key and
  // the bare wrapper key collapsed — both are now the single canonical `swarmdo`.
  // The "registers under the swarmdo key" test above is the live invariant.)

  it('still invokes swarmdo@latest mcp start as the command args', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    const entry = config.mcpServers['swarmdo'] as { command: string; args: string[] };
    expect(entry.args).toContain('swarmdo@latest');
    expect(entry.args).toContain('mcp');
    expect(entry.args).toContain('start');
  });

  it('generateMCPCommands uses swarmdo as the registration name', () => {
    const cmds = generateMCPCommands(makeMCPOptions());
    expect(cmds.length).toBeGreaterThan(0);
    // The main server registers under the canonical `swarmdo` name.
    const mainCmd = cmds.find(c => c.includes('swarmdo@latest'));
    expect(mainCmd).toBeDefined();
    expect(mainCmd).toMatch(/claude mcp add swarmdo\b/);
  });
});

// ─── #2207: detectExistingSwarmdoMCP — both keys + no false positive ───────────
...
// calls writeMCPConfig → detectExistingSwarmdoMCP.
...
describe('#2207 — init detector accepts both server keys', () => {
  ...
  it('detects a prior install registered under the new swarmdo key', async () => {
    // Write a parent .mcp.json with the 'swarmdo' key (post-#2206 install)
    const parentDir = mkdtempSync(path.join(tmpdir(), 'wizard-2207-parent-'));
    try {
      writeFileSync(
        path.join(parentDir, '.mcp.json'),
        JSON.stringify({ mcpServers: { 'swarmdo': { command: 'npx', args: ['-y', 'swarmdo@latest', 'mcp', 'start'] } } }),
      );
      ...
      // Because a 'swarmdo' key was found in the parent .mcp.json, init should
  ...
  it('detects a prior install registered under the legacy swarmdo key', async () => {
  ...
  it('does NOT false-positive on a bare .claude/settings.json with no MCP registration', async () => {
    ...
    try {
      ...
      try {
        const result = await executeInit(opts);
        // No pre-existing swarmdo/swarmdo key → init must NOT skip .mcp.json
...
describe('#2208 — CLAUDE.md backup before overwrite', () => {
  ...
  it('backs up existing CLAUDE.md to CLAUDE.md.pre-swarmdo when --force is used', async () => {
    ...
    const backupPath = path.join(tmp, 'CLAUDE.md.pre-swarmdo');
    expect(existsSync(backupPath)).toBe(true);
    expect(readFileSync(backupPath, 'utf-8')).toBe(sentinel);
  });

  it('uses a timestamped backup name when CLAUDE.md.pre-swarmdo already exists', async () => {
    writeFileSync(path.join(tmp, 'CLAUDE.md'), '# First content\n', 'utf-8');
    writeFileSync(path.join(tmp, 'CLAUDE.md.pre-swarmdo'), '# Previous backup\n', 'utf-8');
    ...
    // The original .pre-swarmdo must still have its old content
    expect(readFileSync(path.join(tmp, 'CLAUDE.md.pre-swarmdo'), 'utf-8')).toBe('# Previous backup\n');
    // A timestamped backup must have been created
    const entries = readdirSync(tmp);
    const timestamped = entries.find(e => e.startsWith('CLAUDE.md.pre-swarmdo.') && e !== 'CLAUDE.md.pre-swarmdo');
  ...
  it('does NOT create a backup when no pre-existing CLAUDE.md exists', async () => {
    ...
    expect(existsSync(path.join(tmp, 'CLAUDE.md.pre-swarmdo'))).toBe(false);
