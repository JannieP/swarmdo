/**
 * SwarmVector Integration Tests
 *
 * Tests for the main swarmvector module exports and integration.
 */

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import {
  isSwarmvectorAvailable,
  getSwarmvectorVersion,
  ...
  fallbackMinCut,
  fallbackLouvain,
} from '../../src/swarmvector/index.js';

// Mock all swarmvector modules
vi.mock('@swarmvector/core', () => ({
  createQLearning: vi.fn(() => null),
  version: '1.0.0',
}));

vi.mock('@swarmvector/ast', () => ({
  createASTAnalyzer: vi.fn(() => null),
}));

vi.mock('@swarmvector/diff', () => ({
  createDiffClassifier: vi.fn(() => null),
}));

vi.mock('@swarmvector/coverage', () => ({
  createCoverageRouter: vi.fn(() => null),
}));

vi.mock('@swarmvector/wasm', () => ({
  minCut: vi.fn(() => null),
  louvain: vi.fn(() => null),
}));

describe('SwarmVector Module Exports', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  describe('isSwarmvectorAvailable', () => {
    it('should return boolean', async () => {
      const result = await isSwarmvectorAvailable();
      expect(typeof result).toBe('boolean');
    });

    it('returns true when swarmvector resolves (mocked at top of file)', async () => {
      // The top-level vi.mock('@swarmvector/core', ...) makes the dynamic
      // import inside isSwarmvectorAvailable resolve, so the value must be
  ...
  describe('getSwarmvectorVersion', () => {
    it('should return version string or null', async () => {
      const version = await getSwarmvectorVersion();
  ...
  describe('Graph Analyzer Functions', () => {
    ...
    it('should export loadSwarmVector', () => {
      expect(typeof loadSwarmVector).toBe('function');
    });
...
describe('SwarmVector Integration Scenarios', () => {
