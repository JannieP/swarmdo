...
import { QLearningRouter, createQLearningRouter, type RouteDecision } from '../../src/swarmvector/q-learning-router';

// Mock the @swarmvector/core module
...
describe('QLearningRouter', () => {
  ...
  describe('initialize', () => {
    it('should initialize without swarmvector (fallback mode)', async () => {
      await router.initialize();
      const stats = router.getStats();
      expect(stats.useNative).toBe(0);
    });

    it('should handle swarmvector import failure gracefully', async () => {
      vi.doMock('@swarmvector/core', () => {
