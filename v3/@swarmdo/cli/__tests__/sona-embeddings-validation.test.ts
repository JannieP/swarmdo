...
import { generateEmbedding } from '../src/swarmvector/vector-db.js';
...
describe('SONA Optimizer', () => {
  ...
  it('4: loadSonaEngine() gracefully handles missing @swarmvector/sona', async () => {
    // With no @swarmvector/sona installed, should still return a result via keyword/default
...
describe('Neural Tools (neural-tools)', () => {
  ...
  it('10: embeddingServiceName reflects active provider (real or hash-fallback)', async () => {
    ...
    // Must match one of the known provider tiers or fallback. `swarmvector` /
    // `@swarmdo/embeddings` is the Tier-0 bundled-ONNX backend (neural-tools.ts
    // line ~49) — it was missing from this allowlist, so the test failed
    // wherever swarmvector's native ONNX IS available and becomes the active tier.
    const knownProviders = /agentic-flow|onnx|mock|hash|fallback|reasoningbank|none|swarmvector|@swarmdo\/embeddings/i;
