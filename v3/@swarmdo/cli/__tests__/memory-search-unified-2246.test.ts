...
//   3. env SWARMDO_MEMORY_SEARCH_NAMESPACES
...
function snapshotEnv() {
  return process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES;
}

function restoreEnv(v: string | undefined) {
  if (v === undefined) delete process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES;
  else process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES = v;
}
...
describe('memory_search_unified namespace fan-out (#2246)', () => {
  ...
  //
  // SWARMDO_MEMORY_PATH alone is not enough: the AgentDB bridge is
  // module-cached on first import and won't re-bind to the new path. Setting
  // SWARMDO_DISABLE_BRIDGE=1 forces the per-call sql.js fallback path,
  ...
  it('with no params, reports `namespaceSource` (not the legacy hardcode silently)', async () => {
    delete process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES;
  ...
  it('with env SWARMDO_MEMORY_SEARCH_NAMESPACES, reports namespaceSource=env', async () => {
    process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES = 'foo,bar,baz';
  ...
  it('explicit `namespaces` array beats env (priority 2 > priority 3)', async () => {
    process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES = 'env-a,env-b';
