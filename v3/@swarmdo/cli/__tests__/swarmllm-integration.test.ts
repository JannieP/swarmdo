/**
 * ADR-086: @swarmvector/swarmllm native intelligence backend integration tests
 *
...
// Mock createRequire to return a swarmllm-like module
...
vi.mock('module', () => ({
  createRequire: vi.fn().mockReturnValue(
    vi.fn().mockImplementation((specifier: string) => {
      if (specifier === '@swarmvector/swarmllm') return mockSwarmllm;
      throw new Error(`Cannot find module '${specifier}'`);
    })
  ),
}));

// Skip in CI — even though the @swarmvector/swarmllm package resolves, the
...
describe.skipIf(__SKIP_WASM_TESTS)('ADR-086: swarmllm native intelligence backend', () => {
  describe('SonaCoordinator via intelligence.ts', () => {
    ...
    it('exposes _swarmllmBackend in stats', async () => {
      const intel = await import('../src/memory/intelligence.js');
      const stats = intel.getIntelligenceStats();
      // Stats should include swarmllm fields
      expect(stats).toHaveProperty('_swarmllmBackend');
      expect(stats).toHaveProperty('_swarmllmTrajectories');
    });
  });
  ...
  describe('TrainingPipeline via lora-adapter.ts', () => {
    it('exposes initBackend method', async () => {
      const lora = await import('../src/swarmvector/lora-adapter.js');
  ...
  describe('CJS import pattern (createRequire)', () => {
    it('uses createRequire instead of ESM dynamic import', async () => {
      // The source files should NOT contain `await import('@swarmvector/swarmllm')`
      // They should use createRequire(import.meta.url)('@swarmvector/swarmllm')
      ...
      const files = [
        ...
        'swarmvector/lora-adapter.ts',
      ];

      for (const file of files) {
        const content = readFileSync(join(srcDir, file), 'utf8');
        // Should NOT have bare ESM import of swarmllm
        expect(content).not.toMatch(/await import\(['"]@swarmvector\/swarmllm['"]\)/);
        // Should have createRequire pattern
        expect(content).toContain('createRequire');
        expect(content).toContain("requireCjs('@swarmvector/swarmllm')");
      }
    });
  });

  describe('graceful degradation', () => {
    it('intelligence falls back to null coordinator when swarmllm unavailable', async () => {
      // The loadSwarmllmCoordinator function catches errors and returns null
      // Stats show 'unavailable' in that case
      const intel = await import('../src/memory/intelligence.js');
      const stats = intel.getIntelligenceStats();
      // Just verify the field exists — actual value depends on module loading order
      expect(['active', 'unavailable']).toContain(stats._swarmllmBackend);
    });
    ...
    it('lora-adapter falls back to js-fallback when pipeline unavailable', async () => {
      ...
      const adapter = new lora.LoRAAdapter({ inputDim: 64, outputDim: 64, rank: 4 });
      const stats = adapter.getStats();
      expect(['swarmllm', 'js-fallback']).toContain(stats._trainingBackend);
    });
  });
});
