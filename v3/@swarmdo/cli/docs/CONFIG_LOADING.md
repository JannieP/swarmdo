...
1. **`src/config-adapter.ts`** (NEW)
   - Converts between `SystemConfig` (from `@swarmdo/shared`) and `V3Config` (CLI-specific format)
...
- `SWARMDO_MAX_AGENTS` - Maximum concurrent agents
- `SWARMDO_DATA_DIR` - Data directory path
- `SWARMDO_MEMORY_TYPE` - Memory backend type
- `SWARMDO_MCP_TRANSPORT` - MCP transport type
- `SWARMDO_MCP_PORT` - MCP server port
- `SWARMDO_SWARM_TOPOLOGY` - Swarm topology type
...
swarmdo agent spawn -t coder

# Use specific config file
swarmdo agent spawn -t coder --config ./custom-config.json

# Override with environment variables
SWARMDO_MAX_AGENTS=20 swarmdo swarm init
```
...
3. **Validation** - Uses existing Zod schemas from `@swarmdo/shared`
...
- [ ] Config validation command (`swarmdo config validate`)
