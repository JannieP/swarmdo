#!/usr/bin/env node
/**
 * Swarmdo Statusline Generator
 * Displays real-time V3 implementation progress and system status.
 * Version is read from the installed @swarmdo/cli package.json at
...
// #1892 — derive Swarmdo banner version from the installed cli package.json
// so the statusline never drifts from `swarmdo doctor`. Falls back to a
// generic "Swarmdo" label only if every resolution path fails.
function resolveBannerVersion() {
  const candidates = [
    // Local-checkout / monorepo case
    path.join(__dirname, '..', '..', 'package.json'),
    // npm-installed-as-dep case
    path.join(__dirname, '..', '..', '..', '@swarmdo', 'cli', 'package.json'),
  ...
  for (const p of candidates) {
    try {
      ...
      if (pkg.name && pkg.name.includes('swarmdo') && typeof pkg.version === 'string') {
  ...
  return ''; // empty → header just says "Swarmdo"
}
...
function generateStatusline() {
  ...
  let header = `${c.bold}${c.brightPurple}▊ Swarmdo${BANNER_VERSION ? ' ' + BANNER_VERSION : ''} ${c.reset}`;
