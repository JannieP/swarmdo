...
swarmdo truth --period 7d

# Export to HTML report
swarmdo truth --export report.html --format html
```
...
swarmdo truth --threshold 0.8

# Agent-specific metrics
swarmdo truth --agent coder --period 24h

# JSON for processing
swarmdo truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    swarmdo truth --format json > truth.json
...
Set truth display preferences in `.swarmdo/config.json`:
