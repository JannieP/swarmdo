...
npx swarmdo agent pool

# Set pool size
npx swarmdo agent pool --size 5

# Configure auto-scaling
npx swarmdo agent pool --min 2 --max 15 --auto-scale

# Pre-warm agents for fast response
npx swarmdo agent pool --warmup

# Disable auto-scaling
npx swarmdo agent pool --auto-scale false
```
...
Configure in `.swarmdo/config.yaml`:
...
- `npx swarmdo agent spawn` - Manual agent spawning
- `npx swarmdo agent list` - View active agents
- `npx swarmdo swarm scale` - Swarm-level scaling
