...
```bash
npx swarmdo agent metrics [agent-id] [options]
```
...
npx swarmdo agent metrics coder-lx7m9k2

# Last hour
npx swarmdo agent metrics -p 1h

# Last 7 days
npx swarmdo agent metrics --period 7d

# JSON output
npx swarmdo agent metrics --format json
```
...
|--------|--------|-------------|
| Flash Attention | unverified (no benchmark) | Neural attention speedup |
| Memory Reduction | 50-75% | Quantization savings |
| HNSW Search | ~1.9x-4.7x measured | Vector search improvement |
...
- `npx swarmdo agent status` - Individual agent metrics
- `npx swarmdo performance benchmark` - Full performance suite
- `npx swarmdo status` - System-wide status
