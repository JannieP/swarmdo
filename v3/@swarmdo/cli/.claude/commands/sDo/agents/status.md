...
```bash
npx swarmdo agent status <agent-id>
npx swarmdo agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx swarmdo agent status coder-lx7m9k2

# Using --id flag
npx swarmdo agent status --id researcher-abc123

# JSON output
npx swarmdo agent status coder-lx7m9k2 --format json
```
...
- `npx swarmdo agent metrics` - Aggregate metrics
- `npx swarmdo agent health` - Health check
- `npx swarmdo agent logs <id>` - Activity logs
