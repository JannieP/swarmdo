...
```bash
npx swarmdo agent stop <agent-id> [options]
npx swarmdo agent kill <agent-id> [options]  # Alias
```
...
npx swarmdo agent stop coder-lx7m9k2

# Force stop (immediate termination)
npx swarmdo agent stop coder-lx7m9k2 --force

# Custom shutdown timeout
npx swarmdo agent stop coder-lx7m9k2 --timeout 60

# Using kill alias
npx swarmdo agent kill researcher-abc123 -f
```
...
npx swarmdo agent list -t coder --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} npx swarmdo agent stop {} -f

# Stop all idle agents
npx swarmdo agent list -s idle --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} npx swarmdo agent stop {}
```

## Related Commands

- `npx swarmdo agent list` - Find agent IDs
- `npx swarmdo agent status` - Check status before stopping
- `npx swarmdo swarm destroy` - Stop all agents in swarm
