---
name: sparc
description: Execute SPARC methodology workflows with Swarmdo
---
...
```bash
# Run SPARC orchestrator (default)
npx swarmdo sparc "build complete authentication system"

# Run a specific mode
npx swarmdo sparc run architect "design API structure"
npx swarmdo sparc run tdd "implement user service"

# Execute full TDD workflow
npx swarmdo sparc tdd "implement user authentication"

# List all modes with details
npx swarmdo sparc modes --verbose

# For alpha features
npx swarmdo@alpha sparc run <mode> "your task"
```

### Option 3: Local Installation
```bash
# If swarmdo is installed locally
./swarmdo sparc "build complete authentication system"
./swarmdo sparc run architect "design API structure"
```
...
```bash
# Store specifications
npx swarmdo memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./swarmdo memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./swarmdo memory query "authentication" --limit 10

# Export project memory
./swarmdo memory export sparc-project-backup.json
```
...
./swarmdo swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./swarmdo swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./swarmdo swarm "Analyze market trends" --strategy research --distributed --ui
```
...
```bash
./swarmdo sparc run code "implement API" --non-interactive
./swarmdo sparc tdd "user tests" --non-interactive --enable-permissions
```
...
See `/sDo:swarmdo-help` for all available commands.
