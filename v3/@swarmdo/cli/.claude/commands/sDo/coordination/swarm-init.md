...
Initialize a Swarmdo swarm with specified topology and configuration.
...
```bash
npx swarmdo swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development

```bash
npx swarmdo swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm

```bash
npx swarmdo swarm init --topology star --github --memory
```
