...
npx swarmdo sparc run mcp "integrate with external API"

# For alpha features
npx swarmdo@alpha sparc run mcp "integrate with external API"

# With namespace
npx swarmdo sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx swarmdo sparc run mcp "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If swarmdo is installed locally
./swarmdo sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx swarmdo memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx swarmdo memory query "mcp" --limit 5
```
