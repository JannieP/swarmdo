...
List all active agents in the Swarmdo system with filtering options.

## Usage

```bash
npx swarmdo agent list [options]
npx swarmdo agent ls [options]  # Alias
```
...
npx swarmdo agent list

# List all agents including inactive
npx swarmdo agent list --all

# Filter by type
npx swarmdo agent list -t coder

# Filter by status
npx swarmdo agent list -s idle

# JSON output for scripting
npx swarmdo agent list --format json

# Combined filters
npx swarmdo agent list -t researcher -s active
```
