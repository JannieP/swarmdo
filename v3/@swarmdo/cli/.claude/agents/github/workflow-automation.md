...
console.log(`Job prioritization completed in ${processingTime}ms (unverified (no benchmark) faster)`);
```
  ...
  swarm-analysis:
    ...
    steps:
      ...
      - name: Analyze Changes
        run: |
          npx swarmdo@v3alpha actions analyze \
            --commit ${{ github.sha }} \
            --suggest-tests \
            --optimize-pipeline
```
...
npx swarmdo@v3alpha actions generate-workflow \
  --analyze-codebase \
  --detect-languages \
...
- name: Swarm Test Selection
  run: |
    npx swarmdo@v3alpha actions smart-test \
  ...
  detect-and-build:
    ...
    steps:
      ...
      - name: Detect Languages
        id: detect
        run: |
          npx swarmdo@v3alpha actions detect-stack \
            --output json > stack.json
            
      - name: Dynamic Build Matrix
        run: |
          npx swarmdo@v3alpha actions create-matrix \
            --from stack.json \
            --parallel-builds
```
    ...
    steps:
      - name: Security Analysis Swarm
        run: |
          ...
          SECURITY_ISSUES=$(npx swarmdo@v3alpha actions security \
            --deep-scan \
            --format json)
...
npx swarmdo@v3alpha actions optimize \
...
gh run view ${{ github.run_id }} --json jobs,conclusion | \
  npx swarmdo@v3alpha actions analyze-failure \
    --suggest-fixes \
    --auto-retry-flaky
...
npx swarmdo@v3alpha actions resources \
  --analyze-usage \
  --suggest-runners \
  --cost-optimize
```
  ...
  heal-pipeline:
    ...
    steps:
      - name: Diagnose and Fix
        run: |
          npx swarmdo@v3alpha actions self-heal \
  ...
  progressive-deploy:
    ...
    steps:
      - name: Analyze Risk
        id: risk
        run: |
          npx swarmdo@v3alpha actions deploy-risk \
            --changes ${{ github.sha }} \
            --history 30d
            
      - name: Choose Strategy
        run: |
          npx swarmdo@v3alpha actions deploy-strategy \
  ...
  perf-swarm:
    ...
    steps:
      - name: Performance Analysis
        run: |
          npx swarmdo@v3alpha actions perf-test \
            --baseline main \
            --threshold 10% \
...
jobs:
  generate-matrix:
    ...
    steps:
      - id: set-matrix
        run: |
          MATRIX=$(npx swarmdo@v3alpha actions test-matrix \
            --detect-frameworks \
            --optimize-coverage)
...
npx swarmdo@v3alpha actions parallel-strategy \
...
npx swarmdo@v3alpha actions analytics \
  --workflow "ci.yml" \
  --period 30d \
...
npx swarmdo@v3alpha actions cost-optimize \
  --analyze-usage \
  --suggest-caching \
...
npx swarmdo@v3alpha actions failure-patterns \
  --period 90d \
  --classify-failures \
  --suggest-preventions
```
    ...
    steps:
      - name: Multi-Agent Validation
        run: |
          ...
          RESULTS=$(npx swarmdo@v3alpha actions pr-validate \
...
jobs:
  release:
    ...
    steps:
      - name: Release Swarm
        run: |
          npx swarmdo@v3alpha actions release \
            --analyze-changes \
            --generate-notes \
            --create-artifacts \
            --publish-smart
  ...
  docs:
    ...
    steps:
      - name: Documentation Swarm
        run: |
          npx swarmdo@v3alpha actions update-docs \
            --analyze-changes \
            --update-api-docs \
            --check-examples
```
...
npx swarmdo@v3alpha actions predict \
  --analyze-history \
  --identify-risks \
  --suggest-preventive
```
...
npx swarmdo@v3alpha actions recommend \
  --analyze-repo \
  --suggest-workflows \
...
npx swarmdo@v3alpha actions auto-optimize \
  --monitor-performance \
  --apply-improvements \
  --track-savings
```
...
- name: Debug Swarm
  run: |
    npx swarmdo@v3alpha actions debug \
      --verbose \
      --trace-agents \
      --export-logs
```
...
npx swarmdo@v3alpha actions profile \
  --workflow "ci.yml" \
  --identify-slow-steps \
