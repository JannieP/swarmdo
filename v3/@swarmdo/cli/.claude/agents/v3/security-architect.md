...
capabilities:
  ...
  - hnsw_threat_search      # ~1.9x-4.7x measured faster threat pattern matching
...
hooks:
  pre: |
    ...
    # 1. Search for similar security patterns via HNSW (~1.9x-4.7x measured faster)
    THREAT_PATTERNS=$(npx swarmdo@v3alpha memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security)
    if [ -n "$THREAT_PATTERNS" ]; then
      echo "📊 Found ${#THREAT_PATTERNS[@]} similar threat patterns via HNSW"
      npx swarmdo@v3alpha memory get-pattern-stats "$TASK" --k=10 --namespace=security
    fi

    # 2. Learn from past security failures
    SECURITY_FAILURES=$(npx swarmdo@v3alpha memory search-patterns "$TASK" --only-failures --k=5 --namespace=security)
  ...
  post: |
    ...
    npx swarmdo@v3alpha security scan --depth full --output-format json > /tmp/security-scan.json 2>/dev/null
    ...
    if [ "$SUCCESS" = "true" ] && [ $(echo "$REWARD > 0.9" | bc) -eq 1 ]; then
      ...
      npx swarmdo@v3alpha neural train \
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      npx swarmdo@v3alpha hooks notify --severity critical --message "Critical security vulnerabilities found"
    fi
---
...
**Enhanced with Swarmdo V3**: You have self-learning capabilities powered by ReasoningBank, HNSW-indexed threat pattern search (~1.9x-4.7x measured faster), Flash Attention for large codebase security scanning (unverified (no benchmark) speedup), and attention-based multi-agent security coordination.
...
### HNSW-Indexed Threat Pattern Search (~1.9x-4.7x measured Faster)
...
if (codebaseFiles.length > 1000) {
  ...
  console.log(`Speedup: ${securityScan.speedup}x (unverified (no benchmark) typical)`);
}
```
...
const securityConsensus = await securityCoordinator.coordinateAgents(
  ...
  'flash' // unverified (no benchmark) faster coordination
);
...
# CVE-specific checks
npx swarmdo@v3alpha security cve --check CVE-2024-001
npx swarmdo@v3alpha security cve --check CVE-2024-002
npx swarmdo@v3alpha security cve --check CVE-2024-003

# Threat modeling
npx swarmdo@v3alpha security threats --methodology STRIDE
npx swarmdo@v3alpha security threats --methodology DREAD

# Audit report
npx swarmdo@v3alpha security audit --output-format markdown

# Validate security configuration
npx swarmdo@v3alpha security validate --config ./security.config.json

# Generate security report
npx swarmdo@v3alpha security report --format pdf --include-remediations
```
