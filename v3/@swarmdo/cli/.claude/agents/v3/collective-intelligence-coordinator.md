...
hooks:
  pre: |
    ...
    mcp__swarmdo__memory_usage store "collective:crdt:${TASK_ID}" "$(date): CRDT sync initialized" --namespace=collective
    # Initialize Byzantine consensus protocol
    mcp__swarmdo__daa_consensus --agents="all" --proposal="{\"protocol\":\"byzantine\",\"threshold\":0.67,\"fault_tolerance\":0.33}"
    ...
    mcp__swarmdo__swarm_monitor --interval=3000 --swarmId="${SWARM_ID}"
  post: |
    echo "✨ Collective intelligence coordination complete - consensus achieved"
    # Store collective decision metrics
    mcp__swarmdo__memory_usage store "collective:decision:${TASK_ID}" "$(date): Consensus decision: $(mcp__swarmdo__swarm_status | jq -r '.consensus')" --namespace=collective
...
mcp__swarmdo__daa_consensus --agents="all" --proposal="{\"task\":\"auth_design\",\"type\":\"collective_vote\"}"

# CRDT synchronization
mcp__swarmdo__memory_sync --target="all_agents" --crdt_type="OR_SET"

# Attention-based coordination
mcp__swarmdo__neural_patterns analyze --operation="collective_attention" --metadata="{\"mechanism\":\"multi-head\",\"heads\":8}"

# Knowledge aggregation
mcp__swarmdo__memory_usage store "collective:knowledge:${TASK_ID}" "$(date): Knowledge synthesis complete" --namespace=collective

# Monitor collective health
mcp__swarmdo__swarm_monitor --interval=3000 --metrics="consensus,byzantine,attention"
```
...
mcp__swarmdo__memory_usage store "crdt:state:init" "{\"type\":\"OR_SET\",\"nodes\":[]}" --namespace=crdt

# Propagate deltas
mcp__swarmdo__coordination_sync --swarmId="${SWARM_ID}"

# Verify convergence
mcp__swarmdo__health_check --components="crdt,consensus,memory"
...
mcp__swarmdo__health_check --components="collective,consensus,crdt,attention"

# Performance report
mcp__swarmdo__performance_report --format=detailed --timeframe=24h

# Bottleneck analysis
mcp__swarmdo__bottleneck_analyze --component="collective" --metrics="latency,throughput,accuracy"
```
