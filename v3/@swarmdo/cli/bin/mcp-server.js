#!/usr/bin/env node
/**
 * @swarmdo/cli - MCP Server Entry Point
 *
  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] ERROR [swarmdo-mcp] Failed to parse:`,
...
process.on('SIGINT', () => {
  console.error(`[${new Date().toISOString()}] INFO [swarmdo-mcp] Received SIGINT`);
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.error(`[${new Date().toISOString()}] INFO [swarmdo-mcp] Received SIGTERM`);
  process.exit(0);
});
  ...
  try {
    switch (message.method) {
      ...
      case 'notifications/initialized':
        console.error(`[${new Date().toISOString()}] INFO [swarmdo-mcp] Client initialized`);
    ...
    console.error(`[${new Date().toISOString()}] ERROR [swarmdo-mcp] ${message.method}:`, error);
    return {
      jsonrpc: '2.0',
      id: message.id,
      error: {
