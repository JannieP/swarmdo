#!/usr/bin/env node
/**
 * @swarmdo/cli - CLI Entry Point
 *
 * Swarmdo V3 Command Line Interface
 *
 * Auto-detects MCP mode when stdin is piped and no args provided.
 * This allows: echo '{"jsonrpc":"2.0",...}' | npx @swarmdo/cli
 */
...
//    (swarmvector ONNX loader, swarmvector-onnx-embeddings-wasm parallel
...
// patch is in place before agentic-flow / swarmvector load transitively.
...
// rescueAgentdbEmbedder swaps the embedder over to swarmvector ONNX in
...
// version directly from package.json and exit before any heavy import.
{
  ...
  if (_argv.length === 1 && (_argv[0] === '--version' || _argv[0] === '-V')) {
    try {
      ...
      process.stdout.write(`swarmdo v${pkg.version || '0.0.0'}\n`);
    } catch {
      process.stdout.write('swarmdo v0.0.0\n');
    }
    process.exit(0);
  }
...
//   2. stdin is being piped AND args are "mcp start" (explicit, e.g. npx swarmdo@alpha mcp start)
      ...
      case 'initialize':
        return {
          ...
          result: {
            ...
            serverInfo: { name: 'swarmdo', version: VERSION },
            capabilities: {
