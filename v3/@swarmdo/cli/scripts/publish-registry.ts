...
async function generateRegistry(): Promise<PluginRegistry> {
  ...
  for (const pkg of officialPackages) {
    ...
    plugins.push({
      id: pkg,
      name: pkg,
      displayName: pkg.replace('@swarmdo/plugin-', '').replace('@swarmdo/', ''),
      description: `Official Swarmdo plugin: ${pkg}`,
  ...
  return {
    ...
    plugins,
    categories: [
      { id: 'official', name: 'Official', description: 'Official Swarmdo plugins', pluginCount: plugins.length },
    ],
...
async function main() {
  ...
  try {
    const result = await pinToIPFS(registry, 'swarmdo-plugin-registry', jwt);
    ...
    console.log('   3. Test with: npx swarmdo@latest plugins list');
  } catch (error) {
