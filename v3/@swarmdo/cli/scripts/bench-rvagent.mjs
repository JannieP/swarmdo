 ...
 *
 * Usage:
 *   node v3/@swarmdo/cli/scripts/bench-rvagent.mjs [--tag=baseline] [--trials=5]
 *
 * Pattern: mirrors v3/@swarmdo/guidance/scripts/bench-phase-1.mjs.
 * Standalone — does NOT require the test runner, only Node ≥20 + the WASM
 * package (@swarmvector/rvagent-wasm must be installed for realistic numbers;
...
let wasmMod;
try {
  wasmMod = await import(resolve(DIST_SRC, 'swarmvector/agent-wasm.js'));
} catch {
...
console.log('When @swarmvector/rvagent-wasm is not installed, timings reflect the import error cost only.');
