...
```bash
# Create new project
gcloud projects create swarmdo-registry --name="Swarmdo Registry"

# Set as active project
gcloud config set project swarmdo-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link swarmdo-registry \
...
gcloud storage buckets create gs://swarmdo-plugin-registry \
  --location=US \
  --uniform-bucket-level-access

# Enable versioning for rollback
gsutil versioning set on gs://swarmdo-plugin-registry

# Make registry.json publicly readable (optional)
gsutil iam ch allUsers:objectViewer gs://swarmdo-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding swarmdo-registry \
  --member="serviceAccount:plugin-registry-publisher@swarmdo-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/swarmdo-registry/secrets/${name}/versions/latest`,
  });
...
export async function publishRegistry(req, res) {
  try {
    ...
    const bucket = storage.bucket('swarmdo-plugin-registry');
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@swarmdo-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=swarmdo-registry
```
...
# Upload to GCS
gsutil cp registry.json gs://swarmdo-plugin-registry/registry.json
```
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://swarmdo-plugin-registry/registry.json']
...
substitutions:
  _FUNCTION_URL: 'https://us-central1-swarmdo-registry.cloudfunctions.net/publish-registry'
...
gcloud scheduler jobs create http publish-registry-daily \
  ...
  --schedule="0 2 * * *" \
  --uri="https://us-central1-swarmdo-registry.cloudfunctions.net/publish-registry" \
  --http-method=POST
```

## Step 6: Update Swarmdo CLI
