 ...
 *
 * Creates a single binary containing kernel, runtime, Swarmdo CLI, models/keys,
...
const SWARMDO_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'swarmdo' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      swarmdoVersion: options.swarmdoVersion || detectSwarmdoVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'swarmdo',   raw: this.buildSwarmdoSection(),   label: 'Swarmdo CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> swarmdo-init (PID 1)',
  ...
  private buildSwarmdoSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack swarmdo@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'swarmdo', version: this.opts.swarmdoVersion,
      package: packageMeta ?? { name: 'swarmdo', version: this.opts.swarmdoVersion },
      commands: SWARMDO_COMMANDS, commandCount: SWARMDO_COMMANDS.length,
  ...
  private buildModelsSection(): Buffer {
    ...
    if (p === 'hybrid') {
      const content: Record<string, unknown> = {
        type: 'models', profile: 'hybrid', provider: 'hybrid', engine: 'swarmllm+api-vault',
    ...
    return jsonBuf({
      type: 'models', profile: 'offline', provider: 'swarmllm', engine: 'swarmllm',
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'SWARMDO_CMD="${SWARMDO_CMD:-swarmdo}"',
        'echo "Running basic verification..."',
        '$SWARMDO_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$SWARMDO_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    const providerMap: Record<string, RvfaModelConfig['provider']> = { cloud: 'api-vault', hybrid: 'hybrid', offline: 'swarmllm' };
    ...
    if (this.opts.profile !== 'cloud') caps.push('local-inference-swarmllm');
    if (this.opts.profile !== 'offline') caps.push('cloud-api-vault');

    const boot: RvfaBootConfig = {
      entrypoint: '/opt/swarmdo/bin/cli.js',
      args: ['--profile', this.opts.profile],
      env: { NODE_ENV: 'production', SWARMDO_MEMORY_BACKEND: 'hybrid', SWARMDO_LOG_LEVEL: 'info' },
      isolation: this.opts.profile === 'cloud' ? 'container' : 'native',
    };

    const models: RvfaModelConfig = {
      provider: providerMap[this.opts.profile],
      engine: this.opts.profile === 'cloud' ? undefined : 'swarmllm-0.1.0',
    ...
    return {
      magic: 'RVFA', version: 1, name: 'swarmdo-appliance', appVersion: this.opts.swarmdoVersion,
...
function detectSwarmdoVersion(): string {
  try {
