...
export class RvfaRunner {
  ...
  /**
   * Run natively via Node.js: extract SWARMDO section to a temp dir,
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const swarmdo = tryExtract(this.reader, 'swarmdo');
      if (!swarmdo) return fail('RVFA appliance does not contain a "swarmdo" section');

      const entryFile = join(workDir, 'swarmdo-bundle.js');
      await writeFile(entryFile, swarmdo);
      ...
      if (options.passphrase && this.header.models.provider !== 'swarmllm') {
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const swarmdo = tryExtract(this.reader, 'swarmdo');
      if (!swarmdo) return fail('RVFA appliance does not contain a "swarmdo" section');
      await writeFile(join(workDir, 'swarmdo-bundle.js'), swarmdo);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY swarmdo-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "swarmdo-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
