/**
 * RVFA (SwarmVector Format Appliance) — Binary format reader/writer
 * for self-contained Swarmdo appliances.
 *
 * Binary layout:
...
export interface RvfaModelConfig {
  provider: 'swarmllm' | 'api-vault' | 'hybrid';
  engine?: string;
  models?: string[];
...
export function createDefaultHeader(
  ...
  const capDefaults: Record<string, string[]> = {
    ...
    hybrid: ['mcp', 'swarm', 'memory', 'hooks', 'neural', 'swarmllm', 'api-vault'],
    offline: ['mcp', 'swarm', 'memory', 'hooks', 'neural', 'swarmllm'],
  };
  ...
  return {
    ...
    sections: [],
    boot: {
      entrypoint: '/opt/swarmdo/bin/swarmdo',
      args: ['--appliance'],
      env: {},
...
export function validateHeader(header: unknown): header is RvfaHeader {
  ...
  if (!oneOf((h.models as Record<string, unknown>).provider, ['swarmllm', 'api-vault', 'hybrid'])) return false;
...
export class RvfaWriter {
   ...
   *
   * @param id      Section identifier (e.g. 'kernel', 'runtime', 'swarmdo').
