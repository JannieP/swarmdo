...
async function main() {
  ...
  try {
    const similar = findSimilarPlugins(registry, '@swarmdo/neural', 3);
    console.log(`  ✅ Found ${similar.length} similar plugins to @swarmdo/neural`);
    passed++;
  } catch (e) {
    console.log(`  ❌ Failed: ${(e as Error).message}`);
    failed++;
  ...
  if (failed === 0) {
    ...
    console.log('    npx @swarmdo/cli plugins list');
    console.log('    npx @swarmdo/cli plugins list --official');
    console.log('    npx @swarmdo/cli plugins search -q "plugin creator"');
    console.log('    npx @swarmdo/cli plugins info -n plugin-creator');
    console.log('');
  }
