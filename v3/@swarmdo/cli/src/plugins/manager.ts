...
export class PluginManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    const pluginsDir = path.join(baseDir, '.swarmdo', 'plugins');
    this.config = {
      pluginsDir,
  ...
  async installFromNpm(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Check for swarmdo plugin metadata
        if (pkg['swarmdo']) {
          commands = pkg['swarmdo'].commands || [];
          hooks = pkg['swarmdo'].hooks || [];
        }
      }
  ...
  async upgrade(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        existing.commands = pkg['swarmdo']?.commands || existing.commands;
        existing.hooks = pkg['swarmdo']?.hooks || existing.hooks;
      }
...
export function getPluginManager(baseDir?: string): PluginManager {
  ...
  } else if (baseDir && defaultManager.getPluginsDir() !== path.join(baseDir, '.swarmdo', 'plugins')) {
