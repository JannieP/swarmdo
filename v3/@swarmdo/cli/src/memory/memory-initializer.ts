 ...
 * alone produced a false "Not loaded — @swarmvector/core not available" even when
 ...
 */
let _swarmvectorCoreResolvable: boolean | undefined;
function isSwarmvectorCoreResolvable(): boolean {
  if (_swarmvectorCoreResolvable !== undefined) return _swarmvectorCoreResolvable;
  try {
    const req = createRequire(import.meta.url);
    req.resolve('@swarmvector/core');
    _swarmvectorCoreResolvable = true;
  } catch {
    _swarmvectorCoreResolvable = false;
  }
  return _swarmvectorCoreResolvable;
}
 ...
 * `SWARMDO_MEMORY_PATH` env var) all silently no-op'd. This helper
 * is the single source of truth — every `.swarm/memory.db` resolution in
 * this file flows through it.
 *
 * Precedence (highest → lowest):
 *   1. SWARMDO_MEMORY_PATH env var
 *   2. memory.persistPath / memory.path in swarmdo.config.json (cwd or
...
export function getMemoryRoot(): string {
  ...
  //    explicit SWARMDO_MEMORY_PATH should take effect immediately whenever
  ...
  const envPath = process.env.SWARMDO_MEMORY_PATH;
  ...
  // 2. Config file (swarmdo.config.json)
  const configCandidates = [
    path.resolve(process.cwd(), 'swarmdo.config.json'),
    path.resolve(process.cwd(), '.swarmdo', 'config.json'),
  ];
 ...
 *   2. SWARMDO_DB_PATH - full file-path override (new in #2105)
 *   3. getMemoryRoot()/memory.db - directory from SWARMDO_MEMORY_PATH /
...
export function resolveDbPath(cliFlag?: string): string {
  ...
  const envDb = process.env.SWARMDO_DB_PATH;
...
async function getBridge(): Promise<typeof import('./memory-bridge.js') | null> {
  ...
  if (process.env.SWARMDO_DISABLE_BRIDGE === '1') return null;
...
// Uses @swarmvector/core from agentic-flow for WASM-accelerated HNSW
// ============================================================================
...
export async function getHNSWIndex(options?: {
  ...
  try {
    // Import @swarmvector/core dynamically
    // Handle both ESM (default export) and CJS patterns
    const swarmvectorModule = await import('@swarmvector/core').catch(() => null);
    ...
    const swarmvectorCore = (swarmvectorModule as any).default || swarmvectorModule;
    if (!swarmvectorCore?.VectorDb) {
      hnswInitializing = false;
      return null; // VectorDb not found
    }

    const { VectorDb } = swarmvectorCore;
    ...
    // @swarmvector/core uses string enum for distanceMetric: 'Cosine', 'Euclidean', 'DotProduct', 'Manhattan'
...
export async function searchHNSWIndex(
  ...
  try {
    ...
    for (const result of results) {
      ...
      // Cosine distance from @swarmvector/core: 0 = identical, 2 = opposite
...
export function getHNSWStatus(): {
  ...
  // @swarmvector/core installed and resolvable), not merely whether the lazy
  ...
  return {
    available: hnswIndex !== null || isSwarmvectorCoreResolvable(),
 ...
 *
 * Failures are isolated: if @swarmdo/memory or agentdb is not installed,
...
export async function loadEmbeddingModel(options?: {
  ...
  try {
    ...
    // Inlined here rather than depending on @swarmdo/embeddings to
    // avoid a circular optional-dep at install time; the logic mirrors
    // @swarmdo/embeddings/src/transformers-loader.ts.
    ...
    // Fallback: Check for swarmvector ONNX embedder (bundled MiniLM-L6-v2 since v0.2.15)
    ...
    const swarmvector = await import('swarmvector').catch(() => null);

    if (swarmvector?.initOnnxEmbedder) {
      try {
        await swarmvector.initOnnxEmbedder();

        // Fallback: OptimizedOnnxEmbedder (raw ONNX, lazy-inits on first embed)
        const onnxEmb = swarmvector.getOptimizedOnnxEmbedder?.();
        if (onnxEmb?.embed) {
          ...
          if (probe && probe.length > 0 && (Array.isArray(probe) ? probe.some((v: number) => v !== 0) : true)) {
            if (verbose) {
              console.log(`Loading swarmvector ONNX embedder (all-MiniLM-L6-v2, ${probe.length}d)...`);
            }
            embeddingModelState = {
              loaded: true,
            ...
            return {
              success: true,
              dimensions: probe.length || 384,
              modelName: 'swarmvector/onnx',
              loadTime: Date.now() - startTime
            };
          }
        }
      } catch {
        // swarmvector ONNX init failed, continue to next fallback
      }
    }

    // Direct `swarmvector.embed()` tier — the SAME API neural-tools + demo use and
    ...
    if ((swarmvector as any)?.embed && (swarmvector as any).isOnnxAvailable?.()) {
      try {
        ...
        const probe = unwrap(await (swarmvector as any).embed('test'));
        if (probe && probe.length > 0 && probe.some((x) => x !== 0)) {
          if (verbose) {
            console.log(`Loading swarmvector embedder via embed() (all-MiniLM-L6-v2, ${probe.length}d)...`);
          }
          embeddingModelState = {
            loaded: true,
            model: async (text: string) => unwrap(await (swarmvector as any).embed(text)) ?? [],
            tokenizer: null,
          ...
          return {
            success: true,
            dimensions: probe.length,
            modelName: 'swarmvector (all-MiniLM-L6-v2)',
            loadTime: Date.now() - startTime,
          };
        }
      } catch {
        // swarmvector.embed() probe failed — continue to next fallback.
      }
    }
 ...
 * swarmvector ONNX / hash fallback) — never the AgentDB bridge.
 *
...
export async function generateLocalEmbedding(text: string): Promise<{
  ...
  if (state.model && typeof (state.model as any) === 'function') {
    try {
      ...
      // Handle both @xenova/transformers (output.data) and swarmvector (plain array) formats
...
export async function storeEntry(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      return { success: false, id: '', error: 'Database not initialized. Run: swarmdo memory init' };
    }
...
export async function searchEntries(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      // A missing store is an EMPTY store, not an error — `swarmdo memory
...
export async function listEntries(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      // A missing store is an EMPTY store, not an error — `swarmdo memory list`
