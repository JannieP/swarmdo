 ...
 *
 * Wraps @swarmvector/rabitq-wasm to provide Hamming-scan pre-filtering
...
async function loadRabitqModule(): Promise<{
  ...
  try {
    const mod = await import('@swarmvector/rabitq-wasm');

    // Node.js: use initSync with the WASM bytes
    const { createRequire } = await import('module');
    const require = createRequire(import.meta.url);
    const wasmPath = require.resolve('@swarmvector/rabitq-wasm/swarmvector_rabitq_wasm_bg.wasm');
...
export async function buildRabitqIndex(options?: {
  ...
  try {
    ...
    if (!mod) {
      ...
      return { success: false, vectorCount: 0, dimensions: 0, compressionRatio: 0, buildTimeMs: 0, error: '@swarmvector/rabitq-wasm not available' };
    }
