 ...
 * `SWARMDO_MEMORY_PATH` / `swarmdo.config.json#memory.persistPath`
...
function getDbPath(customPath?: string): string {
  ...
  try {
    // `getMemoryRoot()` honors $SWARMDO_MEMORY_PATH, then the
    // swarmdo.config.json `memory.persistPath`, then defaults to `.swarm`.
 ...
 * Returns null if @swarmdo/memory is not available.
 */
async function getRegistry(dbPath?: string): Promise<any | null> {
  ...
  if (!registryPromise) {
    registryPromise = (async () => {
      try {
        const { ControllerRegistry } = await import('@swarmdo/memory');
        ...
        console.log = (...args: unknown[]) => {
          ...
          if (msg.includes('Transformers.js') ||
              ...
              msg.includes('SwarmVector graph')) return;
        ...
        try {
          ...
          const agentdbPromise = (async () => {
            ...
            // it in favor of @swarmvector/router; future versions may
            ...
            try {
              ...
              if (adbPkgJsonPath) {
                ...
                if (!reg.get('guardedVectorBackend')) {
                  try {
                    const gvbFile = path.join(adbDir, 'dist/src/backends/swarmvector/GuardedVectorBackend.js');
...
function getDb(registry: any): any | null {
  ...
  // tried this same chain and ALSO has a working swarmvector-ONNX fallback
  // that the user typically reaches before the bridge initialises. The
  // swarmvector ONNX model has been loaded; the agentdb instance just doesn't
  ...
  // Patch is idempotent (`__swarmvectorRescued` sentinel) and a no-op if
  // swarmvector is also unavailable.
...
async function rescueAgentdbEmbedder(agentdb: { embedder?: { pipeline?: unknown; embed?: (t: string) => Promise<Float32Array>; embedBatch?: (ts: string[]) => Promise<Float32Array[]>; __swarmvectorRescued?: boolean } }): Promise<void> {
  if (_embedderPatched) return;
  const emb = agentdb?.embedder;
  if (!emb || emb.__swarmvectorRescued) return;
...
export async function bridgeGenerateEmbedding(
  ...
  try {
    ...
    // `enabled: false` (e.g., swarmvector isn't wired into its neural substrate
    ...
    // transformers.js / swarmvector ONNX directly.
    if (emb.length !== 384) {
      return null;
    }
...
export async function bridgeDeleteHierarchical(options: {
  ...
  try {
    ...
    //    See agentic-flow#150/#151 (closes upstream/SwarmVector#427 the cli-visible way).
 ...
 * Returns null to fall back to local swarmvector router.
 */
 ...
 * Available since agentdb 3.0.0-alpha.10 — uses @swarmvector/router for
