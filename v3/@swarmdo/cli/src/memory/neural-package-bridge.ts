/**
 * Bridge to @swarmdo/neural — Phase 1 convergence (#1773).
 *
 * The cli has historically reimplemented SONA / ReasoningBank / PatternLearner
 * locally in `intelligence.ts`. The dedicated `@swarmdo/neural` package
 ...
 *
 * Why lazy: instantiating NeuralLearningSystem pulls in @swarmvector/sona and
 * a few transitive WASM modules — not free at process startup. The bridge
 * defers until something actually asks for it.
 */

import type { NeuralLearningSystem, SONAMode } from '@swarmdo/neural';
...
/**
 * Lazy-load + initialize the @swarmdo/neural NeuralLearningSystem. Returns
...
export async function getNeuralPackage(mode: SONAMode = 'balanced'): Promise<NeuralLearningSystem | null> {
  ...
  pkgInitPromise = (async () => {
    try {
      const m = await import('@swarmdo/neural');
      const sys = m.createNeuralLearningSystem(mode);
      await sys.initialize();
      pkgInstance = sys;
      return sys;
    } catch (err) {
      // SWARMDO_DEBUG-gated log so future regressions of this shape
      // don't disappear silently — same convention as the swarmllm coordinator
      // bridge (#1770).
      if (process.env.SWARMDO_DEBUG) {
        // eslint-disable-next-line no-console
        console.error('[neural-package] @swarmdo/neural load failed:', (err as Error).message);
      }
      pkgInitFailed = true;
      return null;
    }
