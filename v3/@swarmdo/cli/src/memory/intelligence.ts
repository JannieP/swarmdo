 ...
 * Uses .swarmdo/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.swarmdo', 'neural');
  const homeDir = join(homedir(), '.swarmdo', 'neural');

  // Prefer local directory if .swarmdo exists
  if (existsSync(join(cwd, '.swarmdo'))) {
    return localDir;
  }
...
// ============================================================================
// @swarmvector/swarmllm SonaCoordinator Integration
// ============================================================================

let swarmllmCoordinator: any = null;
let swarmllmLoaded = false;

/**
 * Synchronously load the @swarmvector/swarmllm SonaCoordinator. Used both by the
 ...
 */
function loadSwarmllmCoordinatorSync(): any {
  if (swarmllmLoaded) return swarmllmCoordinator;
  swarmllmLoaded = true;
  try {
    const requireCjs = createRequire(import.meta.url);
    const swarmllm = requireCjs('@swarmvector/swarmllm');
    swarmllmCoordinator = new swarmllm.SonaCoordinator(swarmllm.DEFAULT_SONA_CONFIG);
    return swarmllmCoordinator;
  } catch (err) {
    ...
    if (process.env.SWARMDO_DEBUG) {
      // eslint-disable-next-line no-console
      console.error('[swarmllm] SonaCoordinator load failed, falling back to JS:', (err as Error).message);
    }
    swarmllmCoordinator = null;
    return null;
  }
}

async function loadSwarmllmCoordinator(): Promise<any> {
  return loadSwarmllmCoordinatorSync();
}
...
export async function getUnifiedLearningStats(): Promise<UnifiedLearningStats> {
  ...
  return {
    global: {
      ...
      source: '.swarmdo/neural/stats.json (globalStats)',
    },
    sona: {
      ...sonaStats,
...
export async function initializeIntelligence(config?: Partial<SonaConfig>): Promise<{
  ...
  try {
    ...
    // Eagerly load swarmllm coordinator so stats reflect backend status
    await loadSwarmllmCoordinator();
...
export async function recordTrajectory(
  ...
  try {
    ...
    // Forward trajectory to @swarmvector/swarmllm SonaCoordinator if available
    const swarmllmCoord = await loadSwarmllmCoordinator();
    if (swarmllmCoord) {
      try {
        const avgQuality = verdict === 'success' ? 1.0 : verdict === 'partial' ? 0.5 : 0.0;
        swarmllmCoord.recordTrajectory({
        ...
        // swarmllm recording failed silently
      }
    }
...
export function getIntelligenceStats(): IntelligenceStats & {
  _swarmllmBackend: string;
  _swarmllmTrajectories: number;
  ...
  // Lazy-init the swarmllm coordinator if it hasn't been loaded yet. The MCP
  ...
  // when @swarmvector/swarmllm is fully resolvable. Sync require — cheap, idempotent.
  if (!swarmllmLoaded) {
    loadSwarmllmCoordinatorSync();
  }
  const swarmllmStats = swarmllmCoordinator?.stats?.() || null;
  ...
  return {
    ...
    _swarmllmBackend: swarmllmStats ? 'active' : 'unavailable',
    _swarmllmTrajectories: swarmllmStats?.trajectoriesBuffered || 0,
...
/**
 * Trigger background learning on the @swarmvector/swarmllm SonaCoordinator.
 * No-op if swarmllm is not installed.
 */
export async function runBackgroundLearning(): Promise<void> {
  const coord = await loadSwarmllmCoordinator();
