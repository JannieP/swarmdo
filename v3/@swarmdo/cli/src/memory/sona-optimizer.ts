...
export interface SONAStats {
  ...
  /** Contrastive trainer status (from @swarmvector/swarmllm) */
...
// ============================================================================
// Contrastive Trainer (lazy-loaded from @swarmvector/swarmllm)
// ============================================================================
...
async function loadContrastiveTrainer(): Promise<any> {
  ...
  try {
    ...
    contrastiveTrainer = new swarmllm.ContrastiveTrainer({ batchSize: 32, margin: 0.5 });
...
export class SONAOptimizer {
  ...
  /** Real @swarmvector/sona engine — null if native not available, undefined if not yet tried */
  ...
  /**
   * Attempt to load the native @swarmvector/sona engine (once).
  ...
  async initialize(): Promise<{ success: boolean; patternsLoaded: number }> {
    ...
    try {
      const { QLearningRouter } = await import('../swarmvector/q-learning-router.js');
   ...
   *
   * Priority order:
   * 1. Real @swarmvector/sona native engine (if available and has matches)
  ...
  async getRoutingSuggestion(task: string): Promise<RoutingSuggestion> {
    // Priority 1: Try real @swarmvector/sona native engine
