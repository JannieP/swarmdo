...
### Before Any Task
\`\`\`bash
npx @swarmdo/cli@latest memory search --query "[task keywords]" --namespace patterns
npx @swarmdo/cli@latest hooks route --task "[task description]"
\`\`\`

### After Success
\`\`\`bash
npx @swarmdo/cli@latest memory store --namespace patterns --key "[name]" --value "[what worked]"
npx @swarmdo/cli@latest hooks post-task --task-id "[id]" --success true --store-results true
\`\`\`
...
\`\`\`bash
npx @swarmdo/cli@latest init --wizard           # Setup
npx @swarmdo/cli@latest swarm init --v3-mode     # Start swarm
npx @swarmdo/cli@latest memory search --query "" # Vector search
npx @swarmdo/cli@latest hooks route --task ""    # Route to agent
npx @swarmdo/cli@latest doctor --fix             # Diagnostics
npx @swarmdo/cli@latest security scan            # Security scan
npx @swarmdo/cli@latest performance benchmark    # Benchmarks
\`\`\`
...
> if you want those sweeps: \`npx swarmdo@latest daemon start\` (self-stops after 12h
...
npx @swarmdo/cli@latest security audit --report
\`\`\`
...
npx @swarmdo/cli@latest performance profile --target "[component]"
\`\`\`
...
\`\`\`bash
npx @swarmdo/cli@latest federation init
npx @swarmdo/cli@latest federation join wss://peer:8443
npx @swarmdo/cli@latest federation send --to peer --type task-request --message "..."
npx @swarmdo/cli@latest federation status
\`\`\`
...
export function generateClaudeMd(options: InitOptions, template?: ClaudeMdTemplate): string {
  ...
  const header = `# Swarmdo — Claude Code Configuration\n`;
