...
export function generateSettings(options: InitOptions): object {
  ...
  // #1670 — Swarmdo attribution (Co-Authored-By trailer + PR footer) is now
  ...
  // `swarmdo-bot@users.noreply.github.com` is GitHub's no-reply convention and
  ...
  settings.env = {
    ...
    // Swarmdo specific environment
    SWARMDO_V3_ENABLED: 'true',
 ...
 * install layout. `swarmdo init` can land helpers either project-locally
...
function hookCmd(script: string, subcommand: string): string {
  ...
  // The fallback handles `swarmdo init`'s global-install path where helpers
...
function generateStatusLineConfig(_options: InitOptions): object {
  ...
  //
  // swarmdo#1948 + #1973: the previous `sh -c 'D="${CLAUDE_PROJECT_DIR:-.}"; …'`
