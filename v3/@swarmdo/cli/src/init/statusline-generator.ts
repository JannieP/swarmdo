...
/**
 * Read the @swarmdo/cli version at generation time so the emitted statusline.cjs
 ...
 * `@swarmdo/cli`. Works in both the src tree (under tsx) and the compiled dist
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (fs.existsSync(candidate)) {
        try {
          ...
          if (pkg.name === '@swarmdo/cli' && typeof pkg.version === 'string' && pkg.version.length > 0) {
            return pkg.version;
          }
        } catch { /* keep walking */ }
      }
