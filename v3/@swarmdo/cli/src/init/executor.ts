...
function mergeSettingsForUpgrade(existing: Record<string, unknown>): Record<string, unknown> {
  ...
  merged.env = {
    ...
    SWARMDO_V3_ENABLED: existingEnv.SWARMDO_V3_ENABLED || 'true',
    SWARMDO_HOOKS_ENABLED: existingEnv.SWARMDO_HOOKS_ENABLED || 'true',
  };
  ...
  //
  // #2448 — Detect and REGENERATE the runaway `npx @swarmdo/cli@latest`
  ...
  // Matches any invocation of `swarmdo hooks statusline` — either via npx
  // (`npx [--prefer-offline] [@]swarmdo[/cli][@<tag>] hooks statusline …`)
  // or as a bare command (`swarmdo hooks statusline …`). Both forms cold-
  ...
  // the runaway `npx @swarmdo/cli@latest hooks <sub>` form. These fire
...
export async function executeUpgrade(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    const metricsDir = path.join(targetDir, '.swarmdo', 'metrics');
    const securityDir = path.join(targetDir, '.swarmdo', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use Swarmdo'
      };
      fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
      result.created.push('.swarmdo/metrics/v3-progress.json');
    } else {
      result.preserved.push('.swarmdo/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.swarmdo/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.swarmdo/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Swarmdo'
      };
      fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
      result.created.push('.swarmdo/metrics/learning.json');
    } else {
      result.preserved.push('.swarmdo/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      const audit = {
        ...
        totalCves: 3,
        lastScan: null,
        _note: 'Run: npx @swarmdo/cli@latest security scan'
      };
      fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
      result.created.push('.swarmdo/security/audit-status.json');
    } else {
      result.preserved.push('.swarmdo/security/audit-status.json');
    }
...
export async function executeUpgradeWithMissing(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    if (process.env.DEBUG || process.env.SWARMDO_DEBUG) {
    ...
    if (sourceSkillsDir) {
      ...
      const debugMode = process.env.DEBUG || process.env.SWARMDO_DEBUG;
      if (debugMode) {
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add swarmdo allow rules)
 ...
 * that already declares a `swarmdo`-keyed MCP server. We use this to skip
 * writing our own `swarmdo`-keyed entry when the user has already
 ...
 * issue describes.
 *
 * Returns the path of the file that already declares `swarmdo` (so we can
 * surface it in the skipped-message), or null if none found.
 */
function detectExistingSwarmdoMCP(targetDir: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // #2207: accept BOTH the old 'swarmdo' key AND the new 'swarmdo' key so that
      ...
      // 'swarmdo', a second `swarmdo init` must still recognise the existing install.
      if (parsed.mcpServers && typeof parsed.mcpServers === 'object') {
        const servers = parsed.mcpServers as Record<string, unknown>;
        // #2369: also recognise the legacy dist-tag keys generated by
        // pre-rename installs (swarmdo ≤ 2.x). Without these the
        // detection walks parent dirs, doesn't match, and writes a NEW
        // swarmdo-keyed config — both servers then run under
        // different prefixes producing duplicate-tool noise.
        if (
          'swarmdo' in servers ||
          'swarmdo' in servers ||
          'swarmdo@alpha' in servers ||
          'swarmdo@v3alpha' in servers
        ) return candidate;
      }
      ...
      //     a `claude mcp add swarmdo` (or legacy `swarmdo`) in this repo is
      ...
      if (parsed.projects && typeof parsed.projects === 'object') {
        for (const [projectKey, projectVal] of Object.entries(parsed.projects)) {
          ...
          if (
            !('swarmdo' in mcp) &&
            !('swarmdo' in mcp) &&
            !('swarmdo@alpha' in mcp) &&
            !('swarmdo@v3alpha' in mcp)
          ) continue;
...
async function writeMCPConfig(
  ...
  if (fs.existsSync(mcpPath) && !options.force) {
    ...
    try {
      ...
      if (servers && typeof servers === 'object') {
        const staleKeys = ['swarmdo@alpha', 'swarmdo@v3alpha'].filter(k => k in servers);
  ...
  // #1779 — Skip writing if the user already has a `swarmdo`-keyed MCP
  // server registered elsewhere (parent .mcp.json, ~/.claude.json, etc).
  // Writing our `swarmdo`-keyed entry on top of that produces the
  ...
  if (!options.force) {
    const existingSwarmdoPath = detectExistingSwarmdoMCP(targetDir);
    if (existingSwarmdoPath) {
      result.skipped.push(`.mcp.json (existing 'swarmdo' MCP registration found at ${existingSwarmdoPath} — would create duplicate; pass --force to write anyway)`);
      return;
    }
  }
...
function findSourceHelpersDir(sourceBaseDir?: string): string | null {
  ...
  try {
    ...
    const pkgJsonPath = esmRequire.resolve('@swarmdo/cli/package.json');
...
async function writeHelpers(
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    // #2132: Always generate swarmdo-hook.cjs regardless of source copy path.
    // The source helpers dir may not contain this file (it lives in
    // plugins/swarmdo-core/scripts/, not .claude/helpers/), but it must
    // always be present so Windows users can use the node-based shim.
    const swarmdoHookDest = path.join(helpersDir, 'swarmdo-hook.cjs');
    if (!fs.existsSync(swarmdoHookDest) || options.force) {
      fs.writeFileSync(swarmdoHookDest, generateSwarmdoHookCjs(), 'utf-8');
      result.created.files.push('.claude/helpers/swarmdo-hook.cjs');
    } else {
      result.skipped.push('.claude/helpers/swarmdo-hook.cjs');
    }
...
/**
 * Write runtime configuration (.swarmdo/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.swarmdo', 'config.yaml');

  if (fs.existsSync(configPath) && !options.force) {
    result.skipped.push('.swarmdo/config.yaml');
    return;
  }

  const config = `# Swarmdo V3 Runtime Configuration
...
`;
  ...
  result.created.files.push('.swarmdo/config.yaml');

  // Write .gitignore
  const gitignorePath = path.join(targetDir, '.swarmdo', '.gitignore');
  const gitignore = `# Swarmdo runtime files
data/
logs/
sessions/
neural/
...
`;
  ...
  if (!fs.existsSync(gitignorePath) || options.force) {
    ...
    result.created.files.push('.swarmdo/.gitignore');
  }
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.swarmdo', 'metrics');
  const learningDir = path.join(targetDir, '.swarmdo', 'learning');
  const securityDir = path.join(targetDir, '.swarmdo', 'security');
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Swarmdo. Run: npx swarmdo@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
    result.created.files.push('.swarmdo/metrics/v3-progress.json');
  }
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.swarmdo/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.swarmdo/metrics/learning.json');
  }
  ...
  if (!fs.existsSync(auditPath) || options.force) {
    ...
    fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
    result.created.files.push('.swarmdo/security/audit-status.json');
  }
}

/**
 * Write CAPABILITIES.md - comprehensive overview of all Swarmdo features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.swarmdo', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.swarmdo/CAPABILITIES.md');
    return;
  }
...
6. [Memory & Intelligence (SwarmVector)](#memory--intelligence)
...
Swarmdo V3 is a domain-driven design architecture for multi-agent AI coordination with:
...
\`\`\`bash
# Initialize swarm
npx @swarmdo/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized

# Check status
npx @swarmdo/cli@latest swarm status

# Monitor activity
npx @swarmdo/cli@latest swarm monitor
\`\`\`
...
\`\`\`bash
# Initialize
npx @swarmdo/cli@latest init --wizard

# Spawn agent
npx @swarmdo/cli@latest agent spawn -t coder --name my-coder

# Memory operations
npx @swarmdo/cli@latest memory store --key "pattern" --value "data" --namespace patterns
npx @swarmdo/cli@latest memory search --query "authentication"

# Diagnostics
npx @swarmdo/cli@latest doctor --fix
\`\`\`
...
\`\`\`bash
# Store pattern
npx @swarmdo/cli@latest memory store --key "name" --value "data" --namespace patterns

# Semantic search
npx @swarmdo/cli@latest memory search --query "authentication"

# List entries
npx @swarmdo/cli@latest memory list --namespace patterns

# Initialize database
npx @swarmdo/cli@latest memory init --force
\`\`\`
...
\`\`\`bash
# Initialize
npx @swarmdo/cli@latest hive-mind init --queen-type strategic

# Status
npx @swarmdo/cli@latest hive-mind status

# Spawn workers
npx @swarmdo/cli@latest hive-mind spawn --count 5 --type worker

# Consensus
npx @swarmdo/cli@latest hive-mind consensus --propose "task"
\`\`\`
...
| @swarmvector/attention | 0.1.3 | Flash attention |
| @swarmvector/sona | 0.1.5 | Neural learning |

### Optional Integrations
| Package | Command |
|---------|---------|
| swarmdo-swarm | \`npx swarmdo-swarm mcp start\` |
| agentic-jujutsu | \`npx agentic-jujutsu@latest\` |

### MCP Server Setup
\`\`\`bash
# Add Swarmdo MCP
claude mcp add swarmdo -- npx -y swarmdo@latest

# Optional servers
claude mcp add swarmdo-swarm -- npx -y swarmdo-swarm mcp start
\`\`\`
...
# Swarm
npx swarmdo@latest swarm init --topology hierarchical --max-agents 8
npx swarmdo@latest swarm status
...
# Memory
npx swarmdo@latest memory search --query "patterns"

# Hooks
npx swarmdo@latest hooks pre-task --description "task"
npx swarmdo@latest hooks worker dispatch --trigger optimize
\`\`\`
...
`;
  ...
  result.created.files.push('.swarmdo/CAPABILITIES.md');
}
...
async function writeClaudeMd(
  ...
  if (fs.existsSync(claudeMdPath) && !options.force) {
    ...
    if (fs.existsSync(claudeMdPath)) {
      const backupBase = `${claudeMdPath}.pre-swarmdo`;
      ...
      console.warn(`[swarmdo init] Existing CLAUDE.md backed up to ${path.basename(backupPath)} before overwrite`);
    }
  ...
  // Also write/append global ~/.claude/CLAUDE.md so swarmdo tools are used automatically (#1497).
  ...
  if (homeDir && !options.skipGlobalClaudeMd) {
    ...
    const swarmdoBlock = [
      '',
      '# Swarmdo Integration (auto-generated by swarmdo init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke swarmdo MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('Swarmdo Integration')) {
          fs.appendFileSync(globalClaudeMd, swarmdoBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended swarmdo block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, swarmdoBlock.trimStart(), 'utf-8');
...
function findSourceDir(type: 'skills' | 'commands' | 'agents', sourceBaseDir?: string): string | null {
  ...
  // __dirname is typically /path/to/node_modules/@swarmdo/cli/dist/src/init
