...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# Swarmdo Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running Swarmdo pre-commit checks..."
...
for FILE in $STAGED_FILES; do
  if [[ "$FILE" =~ \\.(ts|js|tsx|jsx)$ ]]; then
    ...
    npx @swarmdo/cli hooks pre-edit --file "$FILE" --validate-syntax 2>/dev/null || true
  fi
done
...
# Notify swarmdo of commit
npx swarmdo@latest hooks notify \\
...
export function generateHookHandler(): string {
  ...
  const lines = [
    ...
    '      try {',
    '        // #118: never fabricate a blanket success. Feed a verdict only on a',
    '        // real outcome signal in the payload; abstain otherwise (a constant',
    '        // true poisons the learning verdict).',
    '        var resp = (hookInput && (hookInput.tool_response || hookInput.toolResponse)) || null;',
    '        var v = null;',
    "        if (resp && typeof resp === 'object') {",
    "          if (resp.is_error === true || (typeof resp.error === 'string' && resp.error)) v = false;",
    '          else if (resp.is_error === false) v = true;',
    '        }',
    '        if (v !== null) intelligence.feedback(v);',
    '      } catch (e) { /* non-fatal */ }',
    '    }',
 ...
 * @swarmdo/memory is not installed. Gets overwritten when source copy succeeds.
 */
...
async function loadMemoryPackage() {
  ...
  // Strategy 2: ESM import (works when @swarmdo/memory is a direct dependency)
  try { return await import('@swarmdo/memory'); } catch { /* fall through */ }
...
export function generateWindowsDaemonManager(): string {
  return `# Swarmdo V3 Daemon Manager for Windows
...
$ClaudeFlowDir = Join-Path $PWD '.swarmdo'
...
function Start-SwarmMonitor {
    ...
    $process = Start-Process -FilePath 'node' -ArgumentList @(
        '-e',
        'setInterval(() => { require("fs").writeFileSync(".swarmdo/metrics/swarm-activity.json", JSON.stringify({swarm:{active:true,agent_count:0},timestamp:Date.now()})) }, 5000)'
...
function Show-Status {
    Write-Host ""
    Write-Host "Swarmdo V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM Swarmdo V3 - Windows Batch Wrapper
 ...
 *   - Both: .claude/helpers/swarmdo-hook.cjs available as a canonical cross-platform shim
 */
export function generateSwarmdoHookCjs(): string {
  return `#!/usr/bin/env node
/**
 * swarmdo-hook.cjs — cross-platform Node.js port of swarmdo-hook.sh (#2132)
 *
 * Deployed to .claude/helpers/ during swarmdo init. On Windows, the
 * generated .claude/settings.json hooks point here instead of the
 * plugin's bash-only swarmdo-hook.sh.
 *
...
function main() {
  ...
  if (commandExists('swarmdo')) { invokeHook('swarmdo', [], hookArgs, stdinData); done(); }
  if (commandExists('swarmdo')) { invokeHook('swarmdo', [], hookArgs, stdinData); done(); }
  invokeHook('npx', ['--prefer-offline', '--yes', 'swarmdo@latest'], hookArgs, stdinData);
  done();
}
