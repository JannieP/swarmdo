...
import { orchestrationTools } from './mcp-tools/orchestration-tools.js';
...
// SwarmVector WASM tools
import { swarmllmWasmTools } from './mcp-tools/swarmllm-tools.js';
...
// #1916: coverage-aware routing tools — defined in swarmvector/coverage-tools.ts
// but were never registered, so the `swarmdo hooks coverage-*` CLI subcommands
// failed with `Tool not found: hooks_coverage-route`.
import { coverageRouterTools } from './swarmvector/coverage-tools.js';
...
/**
 * Lifecycle MCP tools for swarmdo-browser session-as-skill architecture
 * (ADR-0001 swarmdo-browser §7). Always registered: their handlers shell out
 * to swarmvector + agent-browser + swarmdo memory and degrade gracefully
...
const TOOL_GROUPS: Record<string, () => MCPTool[]> = {
  ...
  orchestrate: () => orchestrationTools,
 ...
 * `'all'`. Also honored implicitly at module load via SWARMDO_TOOLS_PROFILE.
 */
...
// Initialize registry at module load. `mcp start` sets SWARMDO_TOOLS_PROFILE
// (and/or SWARMDO_TOOLS_GROUPS as an explicit csv) before the server's dynamic
...
(() => {
  const csv = process.env.SWARMDO_TOOLS_GROUPS;
  if (csv && csv.trim()) {
    applyToolGroups(csv.split(',').map(s => s.trim()).filter(Boolean));
    return;
  }
  applyToolProfile(process.env.SWARMDO_TOOLS_PROFILE);
})();
 ...
 * With `SWARMDO_STRICT_GUARDRAIL=true`, scans every string field of the
 ...
 * cost of `@swarmdo/security` does not hit every CLI invocation. Once
...
function applyContentBoundaryGuardrail(toolName: string, result: unknown): unknown {
  if (process.env.SWARMDO_STRICT_GUARDRAIL !== 'true') return result;
  ...
  if (_guardrailInstance === null) {
    try {
      ...
      const sec = require('@swarmdo/security') as {
