 ...
 * `@swarmvector/tiny-dancer`) into the model-routing path as a graceful, gated
 * addition. The shipped heuristic + Thompson bandit stays as the default;
 * this module only contributes a decision when:
 *
 *   1. `SWARMDO_ROUTER_NEURAL=1` is set
 *   2. Either a trained artifact path resolves (`SWARMDO_ROUTER_MODEL_PATH`)
...
export interface NeuralRouteResult {
   ...
   * `SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD` need to observe
...
interface NeuralRouterConfig {
   ...
   * `SWARMDO_ROUTER_CALIBRATE=0` to opt out and recover iter 0-21 raw
   * KRR behavior.
   */
...
function getConfig(): NeuralRouterConfig {
  ...
  // dist layout:  dist/src/swarmvector/neural-router.js → assets at dist/assets/...
  // src layout:   src/swarmvector/neural-router.ts     → assets at assets/...
  // We probe both candidate locations.
  let assetsDir: string;
  try {
    const here = dirname(fileURLToPath(import.meta.url));
    const candidates = [
      resolvePath(here, '..', '..', 'assets', 'model-router'),       // src/swarmvector → src/assets/...
      resolvePath(here, '..', '..', '..', 'assets', 'model-router'), // dist/src/swarmvector → dist/assets/...
      resolvePath(here, '..', '..', '..', '..', 'assets', 'model-router'), // safety net
    ];
    assetsDir = candidates.find(existsSync) ?? candidates[0];
  } catch {
    assetsDir = resolvePath(process.cwd(), 'v3', '@swarmdo', 'cli', 'assets', 'model-router');
  }

  _config = {
    enabled: process.env.SWARMDO_ROUTER_NEURAL === '1',
    modelPath: process.env.SWARMDO_ROUTER_MODEL_PATH || undefined,
    ...
    qualityBar: parseFloat(process.env.SWARMDO_ROUTER_QUALITY_BAR ?? '0.50') || 0.50,
    seedCorpusPath: process.env.SWARMDO_ROUTER_SEED_CORPUS
      ?? join(assetsDir, 'seed-rows.json'),
    k: parseInt(process.env.SWARMDO_ROUTER_KNN_K ?? '5', 10) || 5,
    latencyBudgetMs: Math.max(0, parseInt(process.env.SWARMDO_ROUTER_LATENCY_BUDGET_MS ?? '0', 10) || 0),
    // ADR-149 iter 24 — DEFAULT ON. Opt out with `=0`. Iter 23 OOS
    // validation: ECE 0.1604 → 0.0335 (-79%), well-calibrated.
    calibrateEnabled: process.env.SWARMDO_ROUTER_CALIBRATE !== '0',
    ...
    costCeilingPerMTok: Math.max(0, parseFloat(process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK ?? '0') || 0),
    // iter 44 — ensemble disagreement threshold; 0 disables.
    ensembleUncertaintyThreshold: Math.max(0, parseFloat(process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD ?? '0') || 0),
    calibratorPath: process.env.SWARMDO_ROUTER_CALIBRATOR_PATH
...
async function resolveBackend(cfg: NeuralRouterConfig): Promise<ResolvedBackend> {
  ...
  if (existsSync(cfg.bundledKrrPath)) {
    try {
      ...
      // `SWARMDO_ROUTER_CALIBRATE=0` to opt out of all calibration.
...
export async function tryCostOptimalRoute(
  ...
  try {
    ...
    // SWARMDO_ROUTER_BANDIT_PER_MODEL=1, perturb each candidate's
    ...
    let all = allRaw;
    if (process.env.SWARMDO_ROUTER_BANDIT_PER_MODEL === '1') {
      try {
        ...
        if (priorsById) {
          ...
          const shrinkageLambda = Math.max(0, parseFloat(process.env.SWARMDO_ROUTER_BANDIT_SHRINKAGE_LAMBDA ?? '4') || 0);
          all = allRaw.map(a => {
            ...
            //                       Gate: SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE=1.
            const warmupRange = Math.max(1, parseFloat(process.env.SWARMDO_ROUTER_BANDIT_WARMUP_RANGE ?? '8') || 8);
            const fullInfluence = process.env.SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE === '1';
    ...
    // ADR-149 iter 12 — latency-aware filtering. When SWARMDO_ROUTER_
 ...
 * Used by the bench and by `swarmdo neural router status` (future CLI).
 */
export async function neuralRouterStatus(): Promise<{ enabled: boolean; available: boolean; routedBy: NeuralRoutedBy | null; reason: string; config: NeuralRouterConfig }> {
  const cfg = getConfig();
  if (!cfg.enabled) return { enabled: false, available: false, routedBy: null, reason: 'SWARMDO_ROUTER_NEURAL!=1', config: cfg };
  const backend = await getBackend();
