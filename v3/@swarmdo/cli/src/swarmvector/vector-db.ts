 ...
 *
 * Provides optional swarmvector WASM-accelerated vector operations for:
...
export interface SwarmVectorModule {
...
// ============================================================================
// Fallback Implementation (when swarmvector not available)
// ============================================================================
...
/**
 * Generate a simple hash-based embedding (fallback when swarmvector not available)
...
function generateHashEmbedding(text: string, dimensions: number = 768): Float32Array {
  if (!hashEmbeddingWarned) {
    ...
    console.warn(
      ...
      'Install swarmvector or @swarmdo/embeddings for real ML embeddings.'
    );
  }
...
let swarmvectorModule: SwarmVectorModule | null = null;
...
/**
 * Attempt to load the swarmvector module
 * Returns true if successfully loaded, false otherwise
 */
export async function loadSwarmVector(): Promise<boolean> {
  if (loadAttempted) {
    return isAvailable;
  }
  ...
  try {
    ...
    // swarmvector exports VectorDB class, not createVectorDB function
    if (swarmvector && (typeof swarmvector.VectorDB === 'function' || typeof swarmvector.VectorDb === 'function')) {
      // Create adapter module that matches our expected interface
      const VectorDBClass = swarmvector.VectorDB || swarmvector.VectorDb;
      swarmvectorModule = {
        createVectorDB: async (dimensions: number): Promise<VectorDB> => {
          ...
          // These map onto swarmvector's DbOptions.hnsw { m, efConstruction }.
          //
          // storagePath: the native swarmvector DB takes an exclusive file lock on
          ...
          // Wrap swarmvector's VectorDB to match our interface
          return {
        ...
        generateEmbedding: (text: string, dimensions: number = 768): Float32Array => {
          // swarmvector may not have this - use fallback
        ...
        isWASMAccelerated: (): boolean => {
          return swarmvector.isWasm?.() ?? false;
        },
      };
      isAvailable = true;
      return true;
    ...
    // Silently fail - swarmvector is optional
  }
...
/**
 * Check if swarmvector is available
 */
export function isSwarmVectorAvailable(): boolean {
  return isAvailable;
}
...
export function isWASMAccelerated(): boolean {
  if (swarmvectorModule && typeof swarmvectorModule.isWASMAccelerated === 'function') {
    return swarmvectorModule.isWASMAccelerated();
  }
  return false;
}
 ...
 * Uses swarmvector HNSW if available, falls back to brute-force search
 */
export async function createVectorDB(dimensions: number = 768): Promise<VectorDB> {
  await loadSwarmVector();

  if (swarmvectorModule && typeof swarmvectorModule.createVectorDB === 'function') {
    try {
      return await swarmvectorModule.createVectorDB(dimensions);
    } catch (err) {
      ...
      console.warn(
        `[vector-db] swarmvector HNSW init failed, falling back to brute-force search: ${(err as Error)?.message ?? err}`
      );
    }
  }
 ...
 * Uses swarmvector if available, falls back to hash-based embedding
 *
...
export function generateEmbedding(text: string, dimensions: number = 768): Float32Array {
  if (swarmvectorModule && typeof swarmvectorModule.generateEmbedding === 'function') {
    try {
      return swarmvectorModule.generateEmbedding(text, dimensions);
    } catch {
...
export function computeSimilarity(a: Float32Array, b: Float32Array): number {
  if (swarmvectorModule && typeof swarmvectorModule.cosineSimilarity === 'function') {
    try {
...
/**
 * Get status information about the swarmvector module
 */
export function getStatus(): {
  available: boolean;
  wasmAccelerated: boolean;
  backend: 'swarmvector-stub-search-disabled' | 'swarmvector-native' | 'fallback';
} {
  if (!isAvailable) {
    return {
      available: false,
  ...
  // swarmvector's `isWasm()` does NOT mean "WASM-accelerated" — there is no WASM
  ...
  return {
    ...
    backend: stubActive ? 'swarmvector-stub-search-disabled' : 'swarmvector-native',
  };
}
