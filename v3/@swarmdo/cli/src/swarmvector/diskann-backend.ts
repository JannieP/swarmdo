 ...
 * Falls back gracefully to HNSW (@swarmvector/router VectorDb) or
 * pure-JS cosine similarity when DiskANN is unavailable.
 *
 * @module v3/cli/swarmvector/diskann-backend
 */
...
/**
 * Check if @swarmvector/diskann is available
 */
export async function isDiskAnnAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const mod = require2('@swarmvector/diskann');
...
export async function getDiskAnnIndex(config: DiskAnnConfig): Promise<{
  ...
  if (await isDiskAnnAvailable()) {
    try {
      const { createRequire } = await import('module');
      const require2 = createRequire(import.meta.url);
      const { DiskAnn } = require2('@swarmvector/diskann');
  ...
  // Try HNSW (@swarmvector/router VectorDb) as fallback
  try {
    const { createRequire } = await import('module');
    const require2 = createRequire(import.meta.url);
    const router = require2('@swarmvector/router');
