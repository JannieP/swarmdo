...
export class ASTAnalyzer {
  ...
  async initialize(): Promise<void> {
    try {
      // @swarmvector/ast is optional - gracefully fallback if not installed
      const swarmvector = await import('@swarmvector/ast' as string).catch(() => null);
      if (swarmvector) {
        this.swarmvectorEngine = (swarmvector as any).createASTAnalyzer?.(this.config);
