...
export class DiffClassifier {
  ...
  async initialize(): Promise<void> {
    try {
      // @swarmvector/diff is optional - gracefully fallback if not installed
      const swarmvector = await import('@swarmvector/diff' as string).catch(() => null);
      if (swarmvector) {
        this.swarmvectorEngine = (swarmvector as any).createDiffClassifier?.(this.config);
