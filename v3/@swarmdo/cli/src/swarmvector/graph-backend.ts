/**
 * @swarmvector/graph-node native graph database backend (ADR-087)
 *
...
/**
 * Load @swarmvector/graph-node via createRequire (CJS package)
 */
async function loadGraphNode(): Promise<any> {
  ...
  try {
    ...
    graphNodeModule = requireCjs('@swarmvector/graph-node');
...
async function getGraphDb(): Promise<any> {
  ...
  const dataDir = join(process.cwd(), '.swarmdo', 'graph');
  try {
