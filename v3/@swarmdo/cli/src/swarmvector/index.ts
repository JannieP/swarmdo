/**
 * SwarmVector Integration Module for Swarmdo CLI
 *
 * Provides integration with @swarmvector packages for:
 ...
 *
 * @module @swarmdo/cli/swarmvector
 */

export { QLearningRouter, createQLearningRouter, type QLearningRouterConfig, type RouteDecision } from './q-learning-router.js';
// #1773 item 4 — moe-router migrated to @swarmdo/neural. Direct
// consumers (hooks-tools.ts) import from '@swarmdo/neural' explicitly;
// re-exporting through this barrel would force vitest to resolve the
// neural pkg's transitive @swarmvector/sona dep eagerly. Keep imports direct.
...
// #1773 item 4 — flash-attention migrated to @swarmdo/neural. Direct
// consumers (hooks-tools.ts, neural-tools.ts) import from '@swarmdo/neural'
// explicitly; re-exporting through this barrel pulls the package's
// transitive @swarmvector/sona dep into vitest's eager resolution.
export {
  LoRAAdapter,
  getLoRAAdapter,
  resetLoRAAdapter,
...
// ── SwarmVector LLM WASM (inference utilities) ─────────────────
export {
  isSwarmllmWasmAvailable,
  initSwarmllmWasm,
  getSwarmllmStatus,
  createHnswRouter,
  createSonaInstant,
...
/**
 * Check if swarmvector packages are available
 */
export async function isSwarmvectorAvailable(): Promise<boolean> {
  try {
    await import('@swarmvector/core');
    return true;
  } catch {
    return false;
  }
...
/**
 * Check if @swarmvector/learning-wasm is available and loadable
 */
export async function isWasmBackendAvailable(): Promise<boolean> {
  try {
    const wasm = await import('@swarmvector/learning-wasm');
...
/**
 * Get swarmvector version if available
 */
export async function getSwarmvectorVersion(): Promise<string | null> {
  try {
    const swarmvector = await import('@swarmvector/core');
    return (swarmvector as any).version || '1.0.0';
  } catch {
    return null;
  }
}
