 ...
 * Integrates with hooks_coverage_route and hooks_coverage_suggest from swarmvector.
 */
 ...
 * Uses swarmvector's hooks_coverage_route when available.
 */
export const hooksCoverageRoute: MCPTool = {
  ...
  category: 'hooks',
  tags: ['coverage', 'routing', 'testing', 'swarmvector'],
  inputSchema: {
    type: 'object',
    properties: {
      task: {
      ...
      useSwarmvector: {
        type: 'boolean',
        description: 'Use swarmvector integration if available (default: true)',
      },
    },
    required: ['task'],
  },
  handler: async (params: Record<string, unknown>): Promise<CoverageRouteResult> => {
    ...
    const useSwarmvector = params.useSwarmvector as boolean | undefined;
 ...
 * Uses swarmvector's hooks_coverage_suggest when available.
 */
export const hooksCoverageSuggest: MCPTool = {
  ...
  category: 'hooks',
  tags: ['coverage', 'suggestions', 'testing', 'swarmvector'],
  inputSchema: {
    type: 'object',
    properties: {
      path: {
