 ...
 *
 * Provides code dependency graph analysis using swarmvector's graph algorithms:
 * - MinCut for code boundary detection (refactoring suggestions)
 * - Louvain for module/community detection
 * - Circular dependency detection
 * - DOT format export for visualization
 *
 * Falls back to built-in implementations when @swarmvector/wasm is not available.
 *
 * @module @swarmdo/cli/swarmvector/graph-analyzer
 */
...
// ============================================================================
// SwarmVector Integration (with graceful fallback)
// ============================================================================

/**
 * Interface for swarmvector graph operations
 */
interface ISwarmVectorGraph {
...
let swarmVectorGraph: ISwarmVectorGraph | null = null;
let swarmVectorLoadAttempted = false;

/**
 * Attempt to load swarmvector graph algorithms
 */
async function loadSwarmVector(): Promise<ISwarmVectorGraph | null> {
  if (swarmVectorLoadAttempted) return swarmVectorGraph;
  swarmVectorLoadAttempted = true;

  // Use dynamic module names to bypass TypeScript static analysis
  // These modules are optional and may not be installed
  const swarmvectorModule = 'swarmvector';
  const wasmModule = '@swarmvector/wasm';

  try {
    // Try to load swarmvector's graph module
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const swarmvector: any = await import(/* webpackIgnore: true */ swarmvectorModule).catch(() => null);

    if (swarmvector && typeof swarmvector.hooks_graph_mincut === 'function' && typeof swarmvector.hooks_graph_cluster === 'function') {
      swarmVectorGraph = {
        mincut: (nodes, edges) => swarmvector.hooks_graph_mincut(nodes, edges),
        louvain: (nodes, edges) => swarmvector.hooks_graph_cluster(nodes, edges),
      };
      return swarmVectorGraph;
    }
  } catch {
...
/**
 * Stoer-Wagner MinCut algorithm (fallback when swarmvector not available)
...
/**
 * Louvain community detection algorithm (fallback when swarmvector not available)
...
export async function analyzeMinCutBoundaries(
  ...
  // Try to use swarmvector, fallback to built-in
  const swarmVector = await loadSwarmVector();

  // Get initial partition
  let result: ReturnType<typeof fallbackMinCut>;
  if (swarmVector) {
    result = swarmVector.mincut(nodes, edges);
  } else {
  ...
  if (numPartitions > 2 && result.partition1.length > 2) {
    ...
    const subResult = swarmVector
      ? swarmVector.mincut(result.partition1, subEdges)
...
export async function analyzeModuleCommunities(graph: DependencyGraph): Promise<ModuleCommunity[]> {
  ...
  // Try to use swarmvector, fallback to built-in
  const swarmVector = await loadSwarmVector();
  const result = swarmVector ? swarmVector.louvain(nodes, edges) : fallbackLouvain(nodes, edges);
