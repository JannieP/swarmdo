# SwarmVector NPM Package API Documentation
...
SwarmVector is a high-performance vector database for Node.js with automatic native/WASM fallback. It provides self-learning intelligence for Claude Code with Q-learning optimization, vector memory, and automatic agent routing.
...
// CLI Usage
npx swarmvector hooks route "implement user login"
```
...
// CLI Usage
npx swarmvector hooks ast-analyze src/api/routes.ts --json
```
...
// CLI Usage
npx swarmvector hooks diff-analyze --json
npx swarmvector hooks diff-analyze abc123 --json
```
...
// CLI Usage
npx swarmvector hooks coverage-route src/services/auth.ts
```
...
npx swarmvector hooks graph-cluster src/**/*.ts --method spectral --clusters 5
```
...
```typescript
import { VectorDb } from 'swarmvector';
...
|----------|---------|-------------|
| `SWARMVECTOR_INTELLIGENCE_ENABLED` | `true` | Enable/disable intelligence |
...
# Verify setup
npx swarmvector hooks verify
npx swarmvector hooks doctor --fix

# Analysis
npx swarmvector hooks ast-analyze <file> --json
npx swarmvector hooks ast-complexity <files> --threshold 10
npx swarmvector hooks diff-analyze [commit] --json
npx swarmvector hooks diff-classify [commit]
npx swarmvector hooks coverage-route <file>
npx swarmvector hooks coverage-suggest <files>
npx swarmvector hooks graph-mincut <files>
npx swarmvector hooks graph-cluster <files> --method louvain

# Memory
npx swarmvector hooks remember "context" -t project
npx swarmvector hooks recall "query"
npx swarmvector hooks route "task description"

# Stats and export
npx swarmvector hooks stats
npx swarmvector hooks export -o backup.json
npx swarmvector hooks import backup.json --merge
```
...
- `@swarmvector/core`: ^0.1.25
- `@swarmvector/gnn`: ^0.1.22
- `@swarmvector/sona`: ^0.1.4
