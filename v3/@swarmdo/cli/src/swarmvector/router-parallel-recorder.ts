 ...
 * `plugins/swarmdo-metaharness/scripts/router-parallel-analyze.mjs` which
 ...
 * 2. OPTIONAL — every write goes through the `SWARMDO_ROUTER_PARALLEL_LOG=1`
 ...
 *     task?: <truncated to 500 chars when SWARMDO_ROUTER_PARALLEL_LOG_TASK=1>,
...
function getConfig(): RecorderConfig {
  ...
  _cfg = {
    enabled: process.env.SWARMDO_ROUTER_PARALLEL_LOG === '1',
    path: process.env.SWARMDO_ROUTER_PARALLEL_LOG_PATH
      ? resolvePath(process.env.SWARMDO_ROUTER_PARALLEL_LOG_PATH)
      : resolvePath('.swarm', 'router-parallel.jsonl'),
    includeTaskText: process.env.SWARMDO_ROUTER_PARALLEL_LOG_TASK === '1',
    taskCharLimit: parseInt(process.env.SWARMDO_ROUTER_PARALLEL_LOG_TASK_LIMIT || '500', 10),
    maxBytes: parseInt(process.env.SWARMDO_ROUTER_PARALLEL_LOG_MAX_BYTES || String(10 * 1024 * 1024), 10),
  };
  return _cfg;
}
 ...
 * JSONL row. No-op when SWARMDO_ROUTER_PARALLEL_LOG is unset (the
