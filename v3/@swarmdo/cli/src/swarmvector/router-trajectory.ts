 ...
 *
 * Gated behind `SWARMDO_ROUTER_TRAJECTORY=1`. Default: **off** — rows
...
export interface TrajectoryDecisionRow {
  ...
  /**
   * A/B mode (SWARMDO_ROUTER_AB=1) attaches both the bandit-only pick
...
function getConfig(): RecorderConfig {
  ...
  const enabled = process.env.SWARMDO_ROUTER_TRAJECTORY === '1';
  const swarmDir = process.env.SWARMDO_SWARM_DIR
    ?? resolvePath(process.cwd(), '.swarm');
  _cfg = {
    enabled,
    path: process.env.SWARMDO_ROUTER_TRAJECTORY_PATH
      ?? join(swarmDir, 'model-router-trajectories.jsonl'),
    taskCharLimit: parseInt(process.env.SWARMDO_ROUTER_TRAJECTORY_TASKLEN ?? '500', 10) || 500,
    maxSizeBytes: parseInt(process.env.SWARMDO_ROUTER_TRAJECTORY_MAXSIZE ?? `${10 * 1024 * 1024}`, 10) | 0,
    maxRotations: Math.max(0, parseInt(process.env.SWARMDO_ROUTER_TRAJECTORY_MAXROTATIONS ?? '3', 10) || 3),
  };
  return _cfg;
}
