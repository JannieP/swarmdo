/**
 * SwarmVector Agent WASM Integration
 *
 * Wraps @swarmvector/rvagent-wasm for sandboxed AI agent execution.
 ...
 *
 * @module @swarmdo/cli/swarmvector/agent-wasm
 */
...
/**
 * Check if @swarmvector/rvagent-wasm is installed and loadable.
 */
export async function isAgentWasmAvailable(): Promise<boolean> {
  try {
    const mod = await import('@swarmvector/rvagent-wasm');
...
export async function initAgentWasm(): Promise<void> {
  ...
  try {
    ...
    const wasmPath = require_.resolve('@swarmvector/rvagent-wasm/rvagent_wasm_bg.wasm');
    const wasmBytes = readFileSync(wasmPath);
    mod.initSync(wasmBytes);
    _wasmReady = true;
  } catch (err) {
    throw new Error(`Failed to initialize @swarmvector/rvagent-wasm: ${err}`);
  }
}
 ...
 * Ollama routing via SWARMDO_PROVIDER + key-presence precedence (#2042).
 *
...
async function attachJsModelProvider(agent: any, config: WasmAgentConfig): Promise<boolean> {
  ...
  const systemPrompt = config.instructions || 'You are a helpful coding assistant running in a Swarmdo WASM agent sandbox.';
...
export async function promptWasmAgent(agentId: string, input: string): Promise<string> {
  ...
  try {
    ...
    const systemPrompt = entry.info.config.instructions || 'You are a helpful coding assistant running in a Swarmdo WASM agent sandbox.';
 ...
 * composed agents can declare which of swarmdo's 314 MCP tools they need.
 */
