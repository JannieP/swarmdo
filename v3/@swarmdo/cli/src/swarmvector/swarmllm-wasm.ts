/**
 * SwarmVector LLM WASM Integration
 *
 * Wraps @swarmvector/swarmllm-wasm for browser-native LLM inference utilities.
 ...
 *
 * Published API (v2.0.2): SwarmLLMWasm, ChatTemplateWasm, ChatMessageWasm,
 ...
 *
 * @module @swarmdo/cli/swarmvector/swarmllm-wasm
 */
...
export interface SwarmllmStatus {
  available: boolean;
  initialized: boolean;
  version: string | null;
}
...
/**
 * Check if @swarmvector/swarmllm-wasm is installed and loadable.
 */
export async function isSwarmllmWasmAvailable(): Promise<boolean> {
  try {
    const mod = await import('@swarmvector/swarmllm-wasm');
    return typeof mod.SwarmLLMWasm === 'function';
  } catch {
    return false;
  }
}
 ...
 */
export async function initSwarmllmWasm(): Promise<void> {
  if (_wasmReady) return;
  try {
    const mod = await import('@swarmvector/swarmllm-wasm');
    const require_ = createRequire(import.meta.url);
    const wasmPath = require_.resolve('@swarmvector/swarmllm-wasm/swarmllm_wasm_bg.wasm');
    const wasmBytes = readFileSync(wasmPath);
    // MUST use object form — initSync(bytes) is deprecated
    mod.initSync({ module: wasmBytes });
    _wasmReady = true;
  } catch (err) {
    throw new Error(`Failed to initialize @swarmvector/swarmllm-wasm: ${err}`);
  }
}

/**
 * Get swarmllm-wasm status.
 */
export async function getSwarmllmStatus(): Promise<SwarmllmStatus> {
  ...
  try {
    const mod = await import('@swarmvector/swarmllm-wasm');
    // version is a standalone function, not on SwarmLLMWasm class
...
export async function createMicroLora(config: MicroLoraConfig): Promise<{
  ...
  return {
    ...
    adapt(quality: number, learningRate = 0.01, success = true): void {
      ...
      // flush, the shipped @swarmvector/swarmllm-wasm@2.0.2 MicroLoraWasm.apply()
