...
export class CoverageRouter {
  ...
  private swarmvectorEngine: unknown = null;
  ...
  async initialize(): Promise<void> {
    try {
      // @swarmvector/coverage is optional - gracefully fallback if not installed
      const swarmvector = await import('@swarmvector/coverage' as string).catch(() => null);
      if (swarmvector) {
        this.swarmvectorEngine = (swarmvector as any).createCoverageRouter?.(this.config);
        this.useNative = !!this.swarmvectorEngine;
      }
