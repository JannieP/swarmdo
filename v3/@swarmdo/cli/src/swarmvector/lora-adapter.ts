...
export interface LoRAStats {
  ...
  /** Training backend in use ('swarmllm' | 'js-fallback') */
...
// ============================================================================
// @swarmvector/swarmllm TrainingPipeline (lazy-loaded)
// ============================================================================

let swarmllmPipeline: any = null;
let pipelineLoaded = false;

async function loadTrainingPipeline(adapter: LoRAAdapter): Promise<any> {
  if (pipelineLoaded) return swarmllmPipeline;
  pipelineLoaded = true;
  try {
    ...
    // Create a swarmllm LoraAdapter that mirrors our JS adapter's config
    const nativeAdapter = new (swarmllm as any).LoraAdapter(config.inputDim, config.outputDim, config.rank);
    swarmllmPipeline = new (swarmllm as any).TrainingPipeline({
      adapter: nativeAdapter,
      learningRate: 0.001,
      batchSize: 8,
    });
    return swarmllmPipeline;
  } catch {
    return null;
  }
}
...
export class LoRAAdapter {
  ...
  /**
   * Eagerly load the swarmllm TrainingPipeline backend.
  ...
  getStats(): LoRAStats {
    ...
    return {
      ...
      _trainingBackend: pipelineLoaded
        ? (swarmllmPipeline ? 'swarmllm' : 'js-fallback')
        : 'js-fallback',
    };
  }
  ...
  /**
   * Save a training checkpoint via swarmllm TrainingPipeline.
   * Falls back to writing our own weight JSON if swarmllm is unavailable.
   */
  async saveCheckpoint(path: string): Promise<boolean> {
    ...
    if (pipeline) {
      try {
        ...
        // Verify swarmllm actually wrote the file (some versions are no-op)
  ...
  /**
   * Load a training checkpoint via swarmllm TrainingPipeline.
   * Falls back to reading our own weight JSON if swarmllm is unavailable.
   */
