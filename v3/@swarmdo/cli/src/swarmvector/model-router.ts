 ...
 * `@metaharness/router`, optionally accelerated by `@swarmvector/tiny-dancer`).
 * It is double-gated on `SWARMDO_ROUTER_NEURAL=1` + an embedding being
...
// pays the load cost when SWARMDO_ROUTER_PARALLEL_LOG is unset.
...
function loadOpenRouterAlts(): OpenRouterAlts | null {
  ...
  try {
    ...
    const explicit = process.env.SWARMDO_ROUTER_OPENROUTER_ALTS;
    ...
    candidates.push(join(process.cwd(), 'v3', '@swarmdo', 'cli', 'assets', 'model-router', 'openrouter-alts.json'));
...
function resolveExecutionProvider(model: ClaudeModel): { provider: 'anthropic' | 'openrouter'; openrouterModel?: string } {
  const explicit = process.env.SWARMDO_ROUTER_PROVIDER?.toLowerCase();
...
export interface ModelRoutingResult {
   ...
   * `SWARMDO_ROUTER_PROVIDER` / `OPENROUTER_API_KEY`.
   */
   ...
   * `SWARMDO_ROUTER_OPENROUTER_ALTS=<path>`). Downstream consumers can
...
function envMaxUncertainty(): number | undefined {
  const raw = process.env.SWARMDO_MAX_UNCERTAINTY;
...
export class ModelRouter {
   ...
   *
   * When `embedding` is supplied and `SWARMDO_ROUTER_NEURAL=1` is set,
  ...
  async route(task: string, embedding?: number[]): Promise<ModelRoutingResult> {
    ...
    if (embedding && embedding.length > 0 && process.env.SWARMDO_ROUTER_NEURAL === '1') {
      try {
        ...
        if (nr) {
          ...
          const weight = parseFloat(process.env.SWARMDO_ROUTER_NEURAL_WEIGHT ?? '5') || 5;
    ...
    //
    // iter 37 — TWO knobs:
    //   SWARMDO_ROUTER_AB=1                    → A/B on every call (orig)
    //   SWARMDO_ROUTER_AB_SAMPLE_RATE=<0..1>   → A/B on a sampled subset,
    ...
    const rateRaw = process.env.SWARMDO_ROUTER_AB_SAMPLE_RATE;
    const rate = rateRaw ? Math.max(0, Math.min(1, parseFloat(rateRaw) || 0)) : 0;
    const allOn = process.env.SWARMDO_ROUTER_AB === '1';
    let inSample = false;
    if (rate > 0) {
    ...
    if (process.env.SWARMDO_ROUTER_TRAJECTORY === '1') {
      try {
 ...
 * of its predicted-quality vector when SWARMDO_ROUTER_BANDIT_PER_MODEL=1.
