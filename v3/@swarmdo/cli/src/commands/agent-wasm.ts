 ...
 *
 * Exposes @swarmvector/rvagent-wasm operations via the `agent` CLI command.
 * Wraps functions from swarmvector/agent-wasm.ts for CLI usage.
 */
...
const WASM_NOT_AVAILABLE_MSG =
  '@swarmvector/rvagent-wasm is not installed.\n' +
  'Install it with: npm install @swarmvector/rvagent-wasm';
...
export const wasmStatusCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo agent wasm-status', description: 'Check WASM agent runtime status' },
  ],
...
export const wasmCreateCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent wasm-create', description: 'Create a default WASM agent' },
    { command: 'swarmdo agent wasm-create -t coder', description: 'Create from gallery template' },
    { command: 'swarmdo agent wasm-create -m "anthropic:claude-sonnet-4-6" -i "You are a security auditor"', description: 'Create with custom config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const wasm = await loadWasm();
...
export const wasmPromptCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent wasm-prompt -a wasm-agent-1-abc -i "Write a hello world"', description: 'Send prompt to WASM agent' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const wasmGalleryCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent wasm-gallery', description: 'List all gallery templates' },
    { command: 'swarmdo agent wasm-gallery -s coder', description: 'Search gallery templates' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const wasm = await loadWasm();
