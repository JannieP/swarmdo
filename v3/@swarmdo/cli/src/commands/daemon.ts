...
const startCommand: Command = {
  ...
  options: [
    ...
    // keep dispatching headless worker sweeps. Default 12h (or SWARMDO_DAEMON_TTL_SECS); 0 = run until stopped.
  ...
  examples: [
    { command: 'swarmdo daemon start', description: 'Start daemon in background (default)' },
    { command: 'swarmdo daemon start --foreground', description: 'Start in foreground (blocks terminal)' },
    { command: 'swarmdo daemon start -w map,audit,optimize', description: 'Start with specific workers' },
    { command: 'swarmdo daemon start --headless --sandbox strict', description: 'Start with headless workers in strict sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const isDaemonProcess = process.env.SWARMDO_DAEMON === '1';
    ...
    if (!isDaemonProcess) {
      const stateDir = join(resolve(projectRoot), '.swarmdo');
    ...
    try {
      const stateDir = join(projectRoot, '.swarmdo');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.swarmdo') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean, forwarded: ForwardedDaemonFlags = {}): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.swarmdo');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  const forkOpts: Record<string, unknown> = {
    ...
    // daemon writes its own logs via appendFileSync to .swarmdo/logs/.
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: swarmdo daemon stop`);
  }
...
const stopCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo daemon stop', description: 'Stop the daemon' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.swarmdo', 'daemon.pid');
...
async function killStaleDaemonsPosix(projectRoot: string, quiet: boolean): Promise<void> {
  try {
    ...
    for (const line of lines) {
      ...
      if (!line.includes('swarmdo') && !line.includes('@swarmdo/cli')) continue;
...
/**
 * #2356: enumerate every running swarmdo daemon across ALL workspaces. Reuses
...
async function renderAllDaemonsStatus(): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    output.printBox(
      'No swarmdo daemons are running in any workspace.',
      'Swarmdo Daemons (all workspaces)'
    );
  ...
  const rows = daemons.map(d => {
    ...
    if (d.workspace) {
      try {
        const statePath = join(d.workspace, '.swarmdo', 'daemon-state.json');
  ...
  if (staleCount > 0) {
    output.printWarning(
      `${staleCount} daemon(s) have outlived their TTL (or have run >12h). Stop one with: cd <workspace> && swarmdo daemon stop`
    );
  } else {
...
const statusCommand: Command = {
  ...
  options: [
    ...
    // leaked in another project is invisible. --all scans every running swarmdo
    // daemon across all workspaces (the global view that surfaces leaks).
    { name: 'all', short: 'a', type: 'boolean', description: 'List swarmdo daemons across ALL workspaces (global view — surfaces leaked daemons)' },
  ],
  examples: [
    { command: 'swarmdo daemon status', description: 'Show daemon status' },
    { command: 'swarmdo daemon status -v', description: 'Show detailed status' },
    { command: 'swarmdo daemon status --show-modes', description: 'Show worker execution modes' },
    { command: 'swarmdo daemon status --all', description: 'List daemons across all workspaces' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox(
        [
          ...
          '',
          'Run "swarmdo daemon start" to start the daemon',
        ].join('\n'),
        'Swarmdo Daemon'
      );
...
const triggerCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo daemon trigger -w map', description: 'Trigger the map worker' },
    { command: 'swarmdo daemon trigger -w audit', description: 'Trigger security audit' },
    { command: 'swarmdo daemon trigger -w audit --headless', description: 'Trigger audit in headless sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const enableCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo daemon enable -w predict', description: 'Enable predict worker' },
    { command: 'swarmdo daemon enable -w document --disable', description: 'Disable document worker' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerType = ctx.flags.worker as WorkerType;
...
const installSupervisorCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo daemon install-supervisor', description: 'Install + load (auto-restart enabled)' },
    { command: 'swarmdo daemon install-supervisor --no-load', description: 'Write unit file but do not enable yet' },
    { command: 'swarmdo daemon install-supervisor --dry-run', description: 'Preview the unit file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      ...
      const plistPath = join(plistDir, 'io.ruv.swarmdo.daemon.plist');
      const logDir = join(projectRoot, '.swarmdo', 'logs');
      const plist = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key><string>io.ruv.swarmdo.daemon</string>
    ...
    <dict>
        <key>SWARMDO_DAEMON</key><string>1</string>
    </dict>
</dict>
</plist>
`;
    ...
    const unitPath = join(unitDir, 'swarmdo-daemon.service');
    const unit = `[Unit]
Description=Swarmdo background worker daemon
After=default.target

[Service]
Type=simple
WorkingDirectory=${projectRoot}
Environment=SWARMDO_DAEMON=1
...
`;
    ...
    if (load) {
      try {
        ...
        execFileSync('systemctl', ['--user', 'enable', '--now', 'swarmdo-daemon.service'], { encoding: 'utf-8', timeout: 10000 });
        ...
        output.printInfo(`Run manually: systemctl --user daemon-reload && systemctl --user enable --now swarmdo-daemon.service`);
      }
    } else {
      output.printInfo(`Run when ready:  systemctl --user daemon-reload && systemctl --user enable --now swarmdo-daemon.service`);
    }
    return { success: true };
  },
};
...
const uninstallSupervisorCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      const plistPath = join(home, 'Library', 'LaunchAgents', 'io.ruv.swarmdo.daemon.plist');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('launchctl', ['unload', plistPath], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
    ...
    if (platform === 'linux') {
      const unitPath = join(home, '.config', 'systemd', 'user', 'swarmdo-daemon.service');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('systemctl', ['--user', 'disable', '--now', 'swarmdo-daemon.service'], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
...
export const daemonCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo daemon start', description: 'Start the daemon' },
    { command: 'swarmdo daemon start --headless', description: 'Start with headless workers (E2B sandbox)' },
    { command: 'swarmdo daemon status', description: 'Check daemon status' },
    { command: 'swarmdo daemon stop', description: 'Stop the daemon' },
    { command: 'swarmdo daemon trigger -w audit', description: 'Run security audit' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Daemon - Background Task Management'));
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "swarmdo daemon <subcommand> --help" for details');
