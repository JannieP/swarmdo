 ...
 *
 * Lifts the calling swarmdo/swarmdo project into a renamed standalone
 * harness using `metaharness --from-existing <dir>`. Attribution to
 * swarmdo is preserved via the upstream's `<!-- swarmdo-attribution-block -->`
 * convention.
 *
 *   npx swarmdo eject --name my-harness                  # dry-run plan
 *   npx swarmdo eject --name my-harness --confirm        # actually eject
 *   npx swarmdo eject --name my-harness --target /abs/out --confirm
 *
 ...
 *      is `/tmp/swarmdo-eject-<ts>-<name>/`. Writing to the calling repo
 *      is refused with exit 2 — protects the user from `cwd: $swarmdo`
 *      eject accidents.
...
export const ejectCommand: Command = {
  name: 'eject',
  description:
    'Lift the calling swarmdo project into a renamed standalone harness via metaharness --from-existing (ADR-150 Phase 2). Dry-run by default; --confirm required to write.',
  options: [
    {
      name: 'name',
    ...
    {
      name: 'target',
      description: 'Absolute output dir (default: /tmp/swarmdo-eject-<ts>-<name>/); refused if inside the calling repo',
      type: 'string' as const,
    },
    {
      name: 'confirm',
  ...
  examples: [
    { command: 'npx swarmdo eject --name my-harness', description: 'Dry-run; prints what would happen' },
    {
      command: 'npx swarmdo eject --name my-harness --confirm',
      description: 'Eject to /tmp/swarmdo-eject-<ts>-my-harness/',
    },
    {
      command: 'npx swarmdo eject --name my-harness --target /abs/out --confirm',
  ...
  async action(context: CommandContext): Promise<CommandResult> {
    ...
    if (!opts.name) {
      ...
      output.writeln('');
      output.writeln('Example: npx swarmdo eject --name my-harness');
    ...
    const target = opts.target
      ...
      : resolvePath(tmpdir(), `swarmdo-eject-${Date.now()}-${opts.name}`);
    ...
    if (!opts.confirm) {
      if (opts.format === 'json') {
        ...
        output.writeln(output.bold('# swarmdo eject (dry-run)'));
        output.writeln('');
    ...
    // Actually run.
    output.writeln(output.bold('# swarmdo eject — running'));
    output.writeln('');
    ...
    if (r.degraded) {
      ...
      output.writeln(output.dim('(ADR-150 graceful degradation: swarmdo runs without it; install with `npm i -D metaharness`.)'));
