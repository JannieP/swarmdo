...
async function getEmbeddings() {
  try {
    return await import('@swarmdo/embeddings');
  } catch {
    return null;
  }
}
...
const generateCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings generate -t "Hello world"', description: 'Generate embedding' },
    { command: 'swarmdo embeddings generate -t "Test" -o json', description: 'Output as JSON' },
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings search -q "error handling"', description: 'Search for similar' },
    { command: 'swarmdo embeddings search -q "test" -l 5', description: 'Limit results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!fs.existsSync(fullDbPath)) {
        ...
        output.printInfo('Run: swarmdo memory init');
        return { success: false, exitCode: 1 };
      }
      ...
      if (topResults.length === 0) {
        ...
        output.printInfo(`Try: swarmdo memory store -k "key" --value "your data"`);
...
const compareCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings compare --text1 "Hello" --text2 "Hi there"', description: 'Compare texts' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const collectionsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings collections', description: 'List collections' },
    { command: 'swarmdo embeddings collections -a stats', description: 'Show detailed stats' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (collections.length === 0) {
        ...
        output.writeln(output.highlight('  swarmdo memory store -k "key" --value "data"'));
        return { success: true, data: [] };
      }
...
const indexCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings index', description: 'Show index status' },
    { command: 'swarmdo embeddings index -a build', description: 'Build index from all namespaces' },
    { command: 'swarmdo embeddings index -a rebuild -c project', description: 'Rebuild (label as `project`)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // singleton stays null and produces a misleading "@swarmvector/core not
      // available" warning even when the package is present (#1698).
      await getHNSWIndex().catch(() => null);

      // Probe whether @swarmvector/core is loadable so we can distinguish
      // "package missing" from "package present but index empty".
      const swarmvectorAvailable = await import('@swarmvector/core').then(() => true).catch(() => false);
      ...
      if (action === 'status') {
        ...
        output.printTable({
          ...
          data: [
            { metric: 'HNSW Available', value: status.available ? output.success('Yes (@swarmvector/core)') : output.warning('No') },
        ...
        } else if (!status.available && !swarmvectorAvailable) {
          output.writeln();
          output.printWarning('@swarmvector/core not available');
          output.printInfo('Install: npm install @swarmvector/core');
        } else if (!status.available) {
          output.writeln();
          output.printWarning('HNSW index not initialized (but @swarmvector/core is installed)');
          output.printInfo('This usually means no embeddings have been stored yet.');
          output.printInfo('Run: swarmdo memory store -k "key" --value "text"');
        } else {
          output.writeln();
      ...
      if (action === 'build' || action === 'rebuild') {
        ...
        if (!index) {
          spinner.fail('@swarmvector/core not available');
          output.printInfo('Install: npm install @swarmvector/core');
          return { success: false, exitCode: 1 };
        }
        ...
        if (!collection && newStatus.entryCount === 0) {
          ...
          output.printInfo('  swarmdo memory store -k "key" --value "text" --namespace <ns>');
        }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings init', description: 'Initialize with defaults' },
    { command: 'swarmdo embeddings init --model all-mpnet-base-v2', description: 'Use higher quality model' },
    { command: 'swarmdo embeddings init --no-hyperbolic', description: 'Euclidean only' },
    { command: 'swarmdo embeddings init --curvature=-0.5', description: 'Custom curvature (use = for negative)' },
    { command: 'swarmdo embeddings init --force', description: 'Overwrite existing config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.swarmdo');
      ...
      if (download) {
        ...
        if (embeddings) {
          ...
          output.writeln(output.dim('  (Skipped — @swarmdo/embeddings not installed)'));
        }
      }
...
const providersCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo embeddings providers', description: 'List providers' },
  ],
...
const chunkCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings chunk -t "Long text..." -s 256', description: 'Chunk with 256 char limit' },
    { command: 'swarmdo embeddings chunk -f doc.txt --strategy paragraph', description: 'Chunk file by paragraph' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const embeddings = await getEmbeddings();
    const text = ctx.flags.text as string || '';
    ...
    if (!embeddings) {
      output.printWarning('@swarmdo/embeddings not installed, showing preview');
      output.writeln();
      output.printBox([
        `Strategy: ${strategy}`,
...
const normalizeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings normalize -i "[0.5, 0.3, 0.8]" -t l2', description: 'L2 normalize' },
    { command: 'swarmdo embeddings normalize --check -i "[...]"', description: 'Check if normalized' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const hyperbolicCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings hyperbolic -a convert -i "[0.5, 0.3]"', description: 'Convert to Poincaré' },
    { command: 'swarmdo embeddings hyperbolic -a distance', description: 'Compute hyperbolic distance' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const hyperbolic = await import('@swarmdo/embeddings').then(m => m).catch(() => null);

      if (!hyperbolic || !hyperbolic.euclideanToPoincare) {
        output.printWarning('@swarmdo/embeddings hyperbolic module not available');
        output.printInfo('Install with: npm install @swarmdo/embeddings');
        return { success: false, exitCode: 1 };
      }
...
const neuralCommand: Command = {
  name: 'neural',
  description: 'Neural substrate features (SwarmVector integration)',
  options: [
    { name: 'feature', short: 'f', type: 'string', description: 'Feature: drift, memory, swarm, coherence, all', default: 'all' },
    { name: 'init', type: 'boolean', description: 'Initialize neural substrate with SwarmVector' },
  ...
  examples: [
    { command: 'swarmdo embeddings neural --init', description: 'Initialize SwarmVector substrate' },
    { command: 'swarmdo embeddings neural -f drift', description: 'Semantic drift detection' },
    { command: 'swarmdo embeddings neural -f memory', description: 'Memory physics (hippocampal)' },
    { command: 'swarmdo embeddings neural -f coherence', description: 'Safety & alignment monitoring' },
    { command: 'swarmdo embeddings neural --drift-threshold=0.2', description: 'Custom drift threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Neural Embedding Substrate (SwarmVector)'));
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.swarmdo', 'embeddings.json');
    ...
    const swarmvector = (neuralConfig.swarmvector || {}) as Record<string, boolean>;
    ...
    output.writeln();
    output.writeln(output.bold('SwarmVector Integration'));
    output.printTable({
      columns: [
      ...
      data: [
        {
          ...
          status: swarmvector.sona ? output.success('Enabled') : output.dim('Disabled')
        },
        {
          component: 'Flash Attention',
          description: '2.49x-7.47x attention speedup',
          status: swarmvector.flashAttention ? output.success('Enabled') : output.dim('Disabled')
        },
        {
          component: 'EWC++',
          description: 'Elastic Weight Consolidation (anti-forgetting)',
          status: swarmvector.ewcPlusPlus ? output.success('Enabled') : output.dim('Disabled')
        },
        {
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .swarmdo/embeddings.json'));
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings models', description: 'List models' },
    { command: 'swarmdo embeddings models -d all-MiniLM-L6-v2', description: 'Download model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (download) {
      ...
      if (embeddings) {
        ...
        await new Promise(r => setTimeout(r, 500));
        spinner.succeed(`Download skipped — @swarmdo/embeddings not installed`);
      }
      return { success: true };
    }
...
const cacheCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings cache', description: 'Show cache stats' },
    { command: 'swarmdo embeddings cache -a clear', description: 'Clear cache' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const warmupCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings warmup', description: 'Preload model with test' },
    { command: 'swarmdo embeddings warmup -b', description: 'Background warmup' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings benchmark', description: 'Quick benchmark' },
    { command: 'swarmdo embeddings benchmark -n 50 -f', description: 'Full benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const embeddingsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo embeddings init', description: 'Initialize ONNX embedding system' },
    { command: 'swarmdo embeddings init --model all-mpnet-base-v2', description: 'Init with larger model' },
    { command: 'swarmdo embeddings generate -t "Hello"', description: 'Generate embedding' },
    { command: 'swarmdo embeddings search -q "error handling"', description: 'Semantic search' },
    { command: 'swarmdo embeddings chunk -t "Long doc..."', description: 'Chunk document' },
    { command: 'swarmdo embeddings hyperbolic -a convert', description: 'Hyperbolic space' },
    { command: 'swarmdo embed neural -f drift', description: 'Neural substrate' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Embeddings'));
