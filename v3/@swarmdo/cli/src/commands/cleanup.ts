...
/**
 * Artifact directories and files that swarmdo/swarmdo may create
 */
const ARTIFACT_DIRS = [
  { path: '.swarmdo', description: 'Capabilities and configuration' },
...
const ARTIFACT_FILES = [
  { path: 'swarmdo.config.json', description: 'Swarmdo configuration' },
];
...
export const cleanupCommand: Command = {
  name: 'cleanup',
  description: 'Remove project artifacts created by swarmdo/swarmdo',
  aliases: ['clean'],
  options: [
    {
      name: 'dry-run',
    ...
    },
    {
      name: 'keep-config',
      short: 'k',
      description: 'Preserve swarmdo.config.json and .claude/settings.json',
      type: 'boolean',
      default: false,
    },
  ],
  examples: [
    ...
    },
    {
      ...
      description: 'Remove all swarmdo artifacts',
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Scan swarmdo-owned subdirs within .claude/ (surgical — preserves Claude Code files)
    ...
    // Check if .claude/settings.json has swarmdo hooks/claudeFlow blocks to clean
    const settingsPath = join(cwd, '.claude', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.claude', 'settings.json'), description: 'Remove swarmdo hooks/claudeFlow blocks (preserves rest)', size: 0, type: 'file' });
    }
    ...
    if (found.length === 0) {
      output.writeln(output.info('No swarmdo artifacts found in the current directory.'));
