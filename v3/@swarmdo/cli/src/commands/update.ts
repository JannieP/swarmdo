 ...
 * Auto-update system for @swarmdo packages (ADR-025)
 */
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @swarmdo package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @swarmdo packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  swarmdo update all');
      }

      return { success: true };
    } finally {
      delete process.env.SWARMDO_FORCE_UPDATE;
    }
  },
};
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @swarmdo packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @swarmdo package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @swarmdo package updates with auto-update support.');
    output.writeln();
    ...
    output.printList([
      `${output.dim('SWARMDO_AUTO_UPDATE=false')}  - Disable auto-update`,
      `${output.dim('SWARMDO_FORCE_UPDATE=true')} - Force update check`,
    ]);
    output.writeln();
    output.writeln('Run "swarmdo update <subcommand> --help" for subcommand help');
