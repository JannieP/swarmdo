 ...
 * `scripts/benchmark-intelligence.mjs`, persists `.swarmdo/bench-results.json`,
 * and renders an honest summary (measured numbers + `null`/reason for the
 * unmeasurable, never fabricated). Shared persistence so `swarmdo doctor`
 * (Move 4) and `swarmdo demo` (Move 7) read the same file.
 */
async function runAuthoritativeBenchmarkCommand(
  ...
  output.writeln();
  output.writeln(output.dim(`Results: ${run.persistedPath} · run \`swarmdo doctor\` to surface these as health checks`));
  return { success: true, data: r };
}
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance benchmark', description: 'Authoritative harness — honest HNSW/quant/SONA numbers, persisted to .swarmdo/bench-results.json' },
    { command: 'swarmdo performance benchmark --sizes 1000,5000,20000', description: 'Authoritative harness across several HNSW sizes' },
    { command: 'swarmdo performance benchmark --in-process -s neural', description: 'In-process micro-benchmarks with per-op p95/p99' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // intelligence-system-audit and `swarmdo doctor`, and persist them for
    ...
    if (suite === 'all' || suite === 'search') {
      ...
      // @swarmvector/core is loadable and the index has data on disk.
...
const profileCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance profile -t cpu', description: 'Profile CPU usage' },
    { command: 'swarmdo performance profile -d 60', description: 'Profile for 60 seconds' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const metricsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance metrics -t 7d', description: 'Show 7-day metrics' },
    { command: 'swarmdo performance metrics -f prometheus', description: 'Export as Prometheus format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance optimize -t memory', description: 'Optimize memory usage' },
    { command: 'swarmdo performance optimize --apply', description: 'Apply all optimizations' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const bottleneckCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance bottleneck', description: 'Find bottlenecks' },
    { command: 'swarmdo performance bottleneck -d full', description: 'Full analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
export const performanceCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo performance benchmark', description: 'Run benchmarks' },
    { command: 'swarmdo performance profile', description: 'Profile application' },
    { command: 'swarmdo perf metrics', description: 'View metrics (alias)' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Performance Suite'));
