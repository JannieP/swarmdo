 ...
 * Session management for Swarmdo
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "swarmdo session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: swarmdo session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "swarmdo start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo session list', description: 'List all sessions' },
    { command: 'swarmdo session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'swarmdo session restore session-123', description: 'Restore a session' },
    { command: 'swarmdo session delete session-123', description: 'Delete a session' },
    { command: 'swarmdo session export -o backup.json', description: 'Export session to file' },
    { command: 'swarmdo session import backup.json', description: 'Import session from file' },
    { command: 'swarmdo session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: swarmdo session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "swarmdo session <subcommand> --help" for subcommand help');
