/**
 * V3 CLI Init Command
 * Comprehensive initialization for Swarmdo with Claude Code integration
 */
...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Swarmdo V3 for OpenAI Codex'));
  output.writeln();
  ...
  try {
    ...
    // Try multiple resolution strategies for the @swarmdo/codex package
    // Use a variable to prevent TypeScript from statically resolving the optional module
    const codexModuleId = '@swarmdo/codex';
    const resolutionStrategies = [
      ...
      async () => {
        const projectPath = path.join(ctx.cwd, 'node_modules', '@swarmdo', 'codex', 'dist', 'index.js');
      ...
      async () => {
        ...
        const codexPath = path.join(globalPath, '@swarmdo', 'codex', 'dist', 'index.js');
    ...
    if (!CodexInitializer) {
      throw new Error('Cannot find module @swarmdo/codex');
    }
    ...
    if (errorMessage.includes('Cannot find module') || errorMessage.includes('@swarmdo/codex')) {
      output.printError('The @swarmdo/codex package is not installed.');
      output.printInfo('Install it with: npm install @swarmdo/codex');
      output.writeln();
...
// Check if project is already initialized with swarmdo.
// #2207: .claude/settings.json alone is NOT a swarmdo marker — it's created by
// Claude Code itself and exists in every Claude Code project. We require a
// swarmdo-specific signal: either a claudeFlow section in settings.json, OR a
// .mcp.json with a 'swarmdo' or 'swarmdo' server key, OR the swarmdo-only
// .swarmdo/config.yaml. Using the bare file-existence check was causing
...
function isInitialized(cwd: string): { claude: boolean; claudeFlow: boolean } {
  const claudeFlowPath = path.join(cwd, '.swarmdo', 'config.yaml');
  const mcpJsonPath = path.join(cwd, '.mcp.json');
  const settingsPath = path.join(cwd, '.claude', 'settings.json');

  // Check .swarmdo/config.yaml — swarmdo-specific, always reliable
  const hasClaudeFlow = fs.existsSync(claudeFlowPath);

  // Check .claude/settings.json for swarmdo-specific content (claudeFlow section)
  let hasSwarmdoSettings = false;
  ...
  // Check .mcp.json for swarmdo/swarmdo server key
  let hasSwarmdoMcp = false;
  if (fs.existsSync(mcpJsonPath)) {
    try {
      ...
      hasSwarmdoMcp =
        ...
        ('swarmdo' in (parsed.mcpServers as Record<string, unknown>) ||
         'swarmdo' in (parsed.mcpServers as Record<string, unknown>));
    } catch { /* malformed — ignore */ }
  }

  return {
    claude: hasSwarmdoSettings || hasSwarmdoMcp,
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('Swarmdo appears to be already initialized');
    if (initialized.claude) output.printInfo('  Found: .claude/settings.json');
    if (initialized.claudeFlow) output.printInfo('  Found: .swarmdo/config.yaml');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Swarmdo V3'));
  output.writeln();
  ...
  if (presetFlag) {
    ...
    if (!cloudMcp) {
      options.mcp = { ...options.mcp, swarmdoSwarm: false };
    }
  } else {
  ...
  // #1744 — opt-out of the user-global ~/.claude/CLAUDE.md "Swarmdo Integration"
  ...
  try {
    ...
    spinner.succeed('Swarmdo V3 initialized successfully!');
    output.writeln();
    ...
    if (options.components.claudeMd || options.components.settings || options.components.skills || options.components.commands || options.components.agents) {
      output.printBox(
        [
          ...
          options.components.commands ? `Commands:    .claude/commands/sDo/ (${result.summary.commandsCount} commands)` : '',
    ...
    if (options.components.runtime) {
      output.printBox(
        [
          `Config:      .swarmdo/config.yaml`,
          `Data:        .swarmdo/data/`,
          `Logs:        .swarmdo/logs/`,
          `Sessions:    .swarmdo/sessions/`,
        ].join('\n'),
        'V3 Runtime'
      );
      output.writeln();
    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @swarmdo/cli@latest memory init 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync('npx @swarmdo/cli@latest daemon start 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @swarmdo/cli@latest swarm init --topology hierarchical 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execFileInit('npx', [
          '@swarmdo/cli@latest', 'embeddings', 'init',
    ...
    if (!startDaemon && !startAll) {
      const bin = (process.argv[1] || '').includes('swarmdo') ? 'swarmdo' : 'swarmdo';
      output.writeln(output.bold('Next steps:'));
      output.printList([
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo V3 Setup Wizard'));
      ...
      if (preset === 'minimal') {
        ...
        const components = await multiSelect({
          ...
          options: [
            ...
            { value: 'runtime', label: 'Runtime', hint: '.swarmdo/ directory for V3 runtime', selected: true },
          ],
        });
        ...
        if (options.components.settings) {
          const hooks = await multiSelect({
            ...
            options: [
              ...
              { value: 'permissionRequest', label: 'PermissionRequest', hint: 'Auto-allow swarmdo tools', selected: true },
            ],
          });
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if Swarmdo is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const result = {
      ...
      paths: {
        ...
        claudeFlowConfig: initialized.claudeFlow ? path.join(ctx.cwd, '.swarmdo', 'config.yaml') : null,
      },
    };
    ...
    if (result.initialized) {
      output.printSuccess('Swarmdo is initialized');
      if (initialized.claude) {
        output.printInfo(`  Claude Code: .claude/settings.json`);
      }
      if (initialized.claudeFlow) {
        output.printInfo(`  V3 Runtime: .swarmdo/config.yaml`);
      }
    } else {
      output.printWarning('Swarmdo is not initialized in this directory');
      output.printInfo('Run "swarmdo init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading Swarmdo'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Swarmdo in the current directory',
  ...
  options: [
    ...
    },
    {
      ...
      name: 'cloud-mcp',
      description: 'Register auth-gated cloud MCP servers (swarmdo-swarm) in .mcp.json (only relevant with --full)',
      type: 'boolean',
      default: false,
    },
    {
    ...
    {
      name: 'no-global',
      description: 'Skip the ~/.claude/CLAUDE.md "Swarmdo Integration" pointer block (#1744)',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'swarmdo init', description: 'Initialize with default configuration' },
    { command: 'swarmdo init --start-all', description: 'Initialize and start daemon, memory, swarm' },
    { command: 'swarmdo init --start-daemon', description: 'Initialize and start daemon only' },
    { command: 'swarmdo init --minimal', description: 'Initialize with minimal configuration' },
    { command: 'swarmdo init --full', description: 'Initialize with all components' },
    { command: 'swarmdo init --force', description: 'Reinitialize and overwrite existing config' },
    { command: 'swarmdo init --only-claude', description: 'Only create Claude Code integration' },
    { command: 'swarmdo init --skip-claude', description: 'Only create V3 runtime' },
    { command: 'swarmdo init wizard', description: 'Interactive setup wizard' },
    { command: 'swarmdo init --with-embeddings', description: 'Initialize with ONNX embeddings' },
    { command: 'swarmdo init --with-embeddings --embedding-model Xenova/all-mpnet-base-v2', description: 'Use larger embedding model' },
    { command: 'swarmdo init skills --all', description: 'Install all available skills' },
    { command: 'swarmdo init hooks --minimal', description: 'Create minimal hooks configuration' },
    { command: 'swarmdo init upgrade', description: 'Update helpers while preserving data' },
    { command: 'swarmdo init upgrade --settings', description: 'Update helpers and merge new settings (Agent Teams)' },
    { command: 'swarmdo init upgrade --verbose', description: 'Show detailed upgrade info' },
    { command: 'swarmdo init --codex', description: 'Initialize for OpenAI Codex (AGENTS.md)' },
    { command: 'swarmdo init --codex --full', description: 'Codex init with all 137+ skills' },
    { command: 'swarmdo init --dual', description: 'Initialize for both Claude Code and Codex' },
    { command: 'swarmdo init --all-agents', description: 'Install all agent categories (~89 agents; ADR-128 opt-in)' },
  ],
  action: initAction,
};
