...
// Train subcommand - REAL WASM training with SwarmVector
const trainCommand: Command = {
  ...
  options: [
    ...
    { name: 'wasm', short: 'w', type: 'boolean', description: 'Use SwarmVector WASM acceleration', default: 'true' },
  ...
  examples: [
    { command: 'swarmdo neural train -p coordination -e 100', description: 'Train coordination patterns' },
    { command: 'swarmdo neural train -d ./training-data.json --flash', description: 'Train from file with Flash Attention' },
    { command: 'swarmdo neural train -p security --wasm --contrastive', description: 'Security patterns with contrastive learning' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Neural Pattern Training (SwarmVector WASM)'));
    output.writeln(output.dim('─'.repeat(55)));

    const spinner = output.createSpinner({ text: 'Initializing SwarmVector training systems...', spinner: 'dots' });
    spinner.start();

    try {
      // Import SwarmVector training service
      const swarmvector = await import('../services/swarmvector-training.js');
      ...
      // Initialize SwarmVector WASM training
      let wasmFeatures: string[] = [];
      if (useWasm) {
        const initResult = await swarmvector.initializeTraining({
          dim,
          learningRate,
          alpha: 0.1,
        ...
        if (initResult.success) {
          ...
          spinner.setText(`SwarmVector initialized [${backendLabel}]: ${wasmFeatures.join(', ')}`);
        } else {
      ...
      const operatorMap: Record<string, number> = {
        coordination: swarmvector.OperatorType.COORDINATION,
        optimization: swarmvector.OperatorType.OPTIMIZATION,
        prediction: swarmvector.OperatorType.ROUTING,
        security: swarmvector.OperatorType.SECURITY,
        testing: swarmvector.OperatorType.TESTING,
        debugging: swarmvector.OperatorType.DEBUGGING,
        memory: swarmvector.OperatorType.MEMORY,
        reasoning: swarmvector.OperatorType.REASONING,
      };
      const operatorType = operatorMap[patternType] ?? swarmvector.OperatorType.GENERAL;
      ...
      for (let epoch = 0; epoch < epochs; epoch++) {
        ...
        const difficulty = useCurriculum ? swarmvector.getCurriculumDifficulty(epoch) : 1.0;
        ...
        if (useContrastive && batch.length >= 3 && useWasm && wasmFeatures.length > 0) {
          ...
          try {
            ...
            const { loss, gradient } = swarmvector.computeContrastiveLoss(anchor, positives, negatives);
            totalLoss += loss;
            ...
            await swarmvector.trainPattern(anchor, scaledGradient, operatorType);
            adaptations++;

            // Record trajectory for learning
            const baselineMs = 10; // Baseline execution time
            const executionMs = performance.now() - epochStart;
            swarmvector.recordTrajectory(anchor, operatorType, useFlash ? 1 : 0, executionMs, baselineMs);
          } catch {
      ...
      // Get SwarmVector stats
      const swarmvectorStats = useWasm && wasmFeatures.length > 0 ? swarmvector.getTrainingStats() : null;
      const trajectoryStats = swarmvectorStats?.trajectoryStats;
      ...
      if (useWasm && wasmFeatures.length > 0) {
        try {
          ...
          benchmark = await swarmvector.benchmarkTraining(dim, 100);
        } catch {
      ...
      // Save LoRA checkpoint via swarmllm TrainingPipeline if available
      try {
        const { LoRAAdapter } = await import('../swarmvector/lora-adapter.js');
        const path = await import('path');
        const cpDir = path.join(process.cwd(), '.swarmdo', 'neural');
      ...
      if (useWasm && wasmFeatures.length > 0) {
        const backendUsed = swarmvectorStats?.backend || 'unknown';
        tableData.push(
        ...
        if (swarmvectorStats?.microLoraStats) {
          tableData.push(
            { metric: 'MicroLoRA Delta Norm', value: swarmvectorStats.microLoraStats.deltaNorm.toFixed(6) }
          );
        }
      ...
      if (useWasm && wasmFeatures.length > 0) {
        ...
        const backendMsg = backendUsed === 'wasm'
          ? `SwarmVector WASM backend: ${wasmFeatures.join(', ')}`
          : `SwarmVector JS fallback (install @swarmvector/learning-wasm for native speed): ${wasmFeatures.join(', ')}`;
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural status', description: 'Show all neural status' },
    { command: 'swarmdo neural status -m model-123', description: 'Check specific model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Check SwarmVector WASM status
      const swarmvectorStats = swarmvector.getTrainingStats();
      const sonaAvailable = swarmvector.isSonaAvailable();
      ...
      output.printTable({
        ...
        data: [
          ...
          {
            component: 'SwarmVector Training',
            status: swarmvectorStats.initialized ? output.success('Active') : output.dim('Not loaded'),
            details: swarmvectorStats.initialized
              ? `${swarmvectorStats.backend === 'wasm' ? 'WASM' : 'JS fallback'} | MicroLoRA: ${swarmvectorStats.totalAdaptations} adapts`
              : 'Call neural train to initialize',
          },
          {
            component: 'SONA Engine',
            status: sonaAvailable ? output.success('Active') : output.dim('Not loaded'),
            details: sonaAvailable && swarmvectorStats.sonaStats
              ? `${swarmvectorStats.sonaStats.totalLearns} learns, ${swarmvectorStats.sonaStats.totalSearches} searches`
          ...
          },
          {
            ...
            // singleton — a false negative even when @swarmvector/core is present.
            ...
            details: hnswStatus.initialized
              ...
              : hnswStatus.available
                ? '@swarmvector/core installed (loads on first vector search)'
                : '@swarmvector/core not available',
          },
          {
          ...
          {
            component: 'swarmllm Coordinator',
            status: stats._swarmllmBackend === 'active' ? output.success('Active') : output.dim('Unavailable'),
            details: stats._swarmllmBackend === 'active'
              ? `SonaCoordinator | ${stats._swarmllmTrajectories} trajectories`
              : 'Install @swarmvector/swarmllm',
          },
          {
          ...
          },
          {
            ...
            status: stats._trainingBackend === 'swarmllm' ? output.success('Active') : output.dim(stats._trainingBackend || 'Unavailable'),
            details: stats._trainingBackend === 'swarmllm'
              ? 'swarmllm checkpoints enabled'
              : 'JS fallback (no checkpoints)',
          },
          await (async () => {
            try {
              const { getGraphStats } = await import('../swarmvector/graph-backend.js');
              ...
              return {
                ...
                details: gs.backend === 'graph-node'
                  ...
                  : 'Install @swarmvector/graph-node',
              };
      ...
      if (verbose) {
        ...
        // Add SwarmVector WASM metrics if initialized
        if (swarmvectorStats.initialized) {
          detailedData.push(
            { metric: 'SwarmVector Adaptations', value: String(swarmvectorStats.totalAdaptations) },
            { metric: 'SwarmVector Forwards', value: String(swarmvectorStats.totalForwards) },
          );
          if (swarmvectorStats.microLoraStats) {
            detailedData.push(
              { metric: 'MicroLoRA Delta Norm', value: swarmvectorStats.microLoraStats.deltaNorm.toFixed(6) },
              { metric: 'MicroLoRA Adapt Count', value: String(swarmvectorStats.microLoraStats.adaptCount) },
            );
          }
          if (sonaAvailable && swarmvectorStats.sonaStats?.stats) {
            const sonaStats = swarmvectorStats.sonaStats.stats as Record<string, unknown>;
            detailedData.push(
      ...
      return { success: true, data: { stats, hnswStatus, adaptBench, modelInfo, swarmvectorStats } };
    } catch (error) {
...
const patternsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural patterns --action list', description: 'List all patterns' },
    { command: 'swarmdo neural patterns -a analyze -q "error handling"', description: 'Analyze patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run: swarmdo neural train --pattern-type general'));
...
const predictCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural predict -i "implement authentication"', description: 'Predict routing for task' },
    { command: 'swarmdo neural predict -i "fix bug in login" -k 3', description: 'Get top 3 predictions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (matches.length === 0) {
        output.writeln(output.warning('No similar patterns found. Try training first: swarmdo neural train'));
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural optimize --method quantize', description: 'Quantize patterns to Int8' },
    { command: 'swarmdo neural optimize --method analyze -v', description: 'Analyze memory usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Trigger swarmllm background learning if available
      try {
      ...
      const patternDir = path.join(process.cwd(), '.swarmdo', 'neural');
      let beforeSize = 0;
      try {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural export -m security-patterns --ipfs', description: 'Export and pin to IPFS' },
    { command: 'swarmdo neural export -m code-review -o ./export.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const exportData = {
        ...
        version: '1.0.0',
        name: customName || `swarmdo-model-${Date.now()}`,
      ...
      const memoryDir = path.join(process.cwd(), '.swarmdo', 'memory');
      ...
      if (pinToIpfs) {
        ...
        output.writeln(output.dim(`Import command: swarmdo neural import --cid ${result.IpfsHash}`));
      }
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural list', description: 'List all available models' },
    { command: 'swarmdo neural list --category security', description: 'List only security models' },
    { command: 'swarmdo neural list -f json', description: 'Output as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        output.writeln(output.dim('  All models:      ') + `swarmdo neural import --cid ${registryCid}`);
        if (category) {
          output.writeln(output.dim(`  ${category} only: `) + `swarmdo neural import --cid ${registryCid} --category ${category}`);
        } else {
          output.writeln(output.dim('  By category:     ') + `swarmdo neural import --cid ${registryCid} --category <category>`);
        }
      }
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural import --cid QmXxx...', description: 'Import from IPFS' },
    { command: 'swarmdo neural import -f ./patterns.json --verify', description: 'Import from file' },
    { command: 'swarmdo neural import --cid QmNr1yYMK... --category security', description: 'Import only security patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run "swarmdo neural patterns --action list" to see imported patterns'));
...
const benchmarkCommand: Command = {
  name: 'benchmark',
  description: 'Benchmark SwarmVector WASM training performance',
  options: [
  ...
  examples: [
    { command: 'swarmdo neural benchmark', description: 'Run default benchmark' },
    { command: 'swarmdo neural benchmark -d 128 -i 5000', description: 'Custom benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const dim = Math.min(parseInt(ctx.flags.dim as string || '256', 10), 256);
    ...
    output.writeln();
    output.writeln(output.bold('SwarmVector WASM Benchmark'));
    output.writeln(output.dim('─'.repeat(50)));
    ...
    try {
      ...
      const attention = await import('@swarmvector/attention') as unknown as Record<string, new (...args: number[]) => { computeRaw: (q: Float32Array, k: Float32Array[], v: Float32Array[]) => Float32Array }>;
      ...
      const wasmPath = require.resolve('@swarmvector/learning-wasm/swarmvector_learning_wasm_bg.wasm');
      const wasmBuffer = fs.readFileSync(wasmPath);

      const learningWasm = await import('@swarmvector/learning-wasm');
...
const routerStatusCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router status', description: 'Show router state' },
    { command: 'SWARMDO_ROUTER_NEURAL=1 swarmdo neural router status', description: 'Show status with gate open' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const format = (ctx.flags.format as string) || 'table';
    const { neuralRouterStatus } = await import('../swarmvector/neural-router.js');
    const { getModelRouterStats } = await import('../swarmvector/model-router.js');
    ...
    output.writeln(output.dim('─'.repeat(60)));
    output.writeln(`  Gate (SWARMDO_ROUTER_NEURAL=1): ${status.enabled ? output.success('open') : output.warning('closed')}`);
...
const routerTrainCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router train', description: 'Train from the bundled seed and write to ./router.krr.json' },
    { command: 'swarmdo neural router train -c ./my-corpus.json -o ./my-router.krr.json', description: 'Train from a custom corpus' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim('Use: SWARMDO_ROUTER_NEURAL=1 SWARMDO_ROUTER_MODEL_PATH=' + outPath + ' …'));
    output.writeln();
...
const routerTrainFromTrajectoriesCommand: Command = {
  ...
  options: [
    { name: 'in', short: 'i', type: 'string', description: 'Trajectory JSONL path (default: $SWARMDO_ROUTER_TRAJECTORY_PATH or .swarm/model-router-trajectories.jsonl)' },
  ...
  examples: [
    { command: 'swarmdo neural router train-from-trajectories', description: 'Show pairing stats from default trajectory path' },
    { command: 'swarmdo neural router train-from-trajectories -w production-rows.json', description: 'Emit a corpus to feed `router train`' },
    { command: 'swarmdo neural router train-from-trajectories -u assets/model-router/seed-rows.json -w merged.json', description: 'Union production rows with the bundled seed corpus' },
    { command: 'swarmdo neural router train-from-trajectories --filter-source llm-judge -w high-signal.json', description: 'Keep only judge-graded rows (drop coarse agent-execute baseline)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    const { pairTrajectoryRows } = await import('../swarmvector/router-trajectory.js');

    const inPath = (ctx.flags.in as string | undefined)
      ?? process.env.SWARMDO_ROUTER_TRAJECTORY_PATH
    ...
    if (!fs.existsSync(inPath)) {
      ...
      if (fmt === 'json') {
        output.writeln(JSON.stringify({ error: msg, hint: 'Set SWARMDO_ROUTER_TRAJECTORY=1 and run any agent_spawn → executeAgentTask flow to accumulate rows.' }, null, 2));
      } else {
        output.printError(msg);
        output.writeln(output.dim('  Set SWARMDO_ROUTER_TRAJECTORY=1 to enable trajectory recording.'));
      }
      return { success: false, exitCode: 1 };
    }
    ...
    if (stats.totalRows === 0) {
      output.writeln(output.dim('  Empty trajectory file. Enable recording with `export SWARMDO_ROUTER_TRAJECTORY=1` and run agent_spawn flows.'));
    } else if (pairs.length > 0 && writePath) {
      output.writeln(output.dim(`  Next: swarmdo neural router train -c ${writePath} -o router.krr.json`));
    }
    output.writeln();
    return { success: true, data };
  },
...
const routerReloadCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo neural router reload', description: 'Refresh backend caches after retraining an artifact' },
  ],
  action: async (): Promise<CommandResult> => {
    const { __resetNeuralRouterForTests, neuralRouterStatus } = await import('../swarmvector/neural-router.js');
...
const routerModelsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router models', description: 'Show candidate registry with measured stats' },
    { command: 'swarmdo neural router models -f json', description: 'Machine-readable JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fmt = (ctx.flags.format as string) || 'table';
    const fs = await import('node:fs');
...
const routerDecisionsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router decisions', description: 'Aggregate stats + 20 most-recent decisions' },
    { command: 'swarmdo neural router decisions --since 24h', description: 'Last 24 hours only' },
    { command: 'swarmdo neural router decisions --routed-by bandit-fallback', description: 'Find decisions where neural backend failed' },
    { command: 'swarmdo neural router decisions --model haiku --format json', description: 'All haiku picks, JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    if (!fs.existsSync(inPath)) {
      const msg = `Trajectory file not found at ${inPath}`;
      if (fmt === 'json') {
        output.writeln(JSON.stringify({ error: msg, hint: 'Set SWARMDO_ROUTER_TRAJECTORY=1 to enable recording.' }, null, 2));
      } else {
        output.printError(msg);
        output.writeln(output.dim('  Set SWARMDO_ROUTER_TRAJECTORY=1 to enable trajectory recording.'));
      }
...
const routerDecideCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router decide "fix typo in cache.ts"', description: 'See the routing decision for a small task' },
    { command: 'swarmdo neural router decide -t "design distributed consensus" -f json', description: 'JSON output for the routing decision' },
    { command: 'SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK=5 swarmdo neural router decide -t "..."', description: 'See iter 29 cost-ceiling mode in action' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const { analyzeTaskComplexity, routeToModelFull } = await import('../swarmvector/model-router.js');
    const { embedTaskWithCache } = await import('../swarmvector/task-embedder.js');
    const { neuralRouterStatus, tryCostOptimalRoute } = await import('../swarmvector/neural-router.js');
    ...
    const payload = {
      ...
      activeEnv: {
        SWARMDO_ROUTER_NEURAL: process.env.SWARMDO_ROUTER_NEURAL ?? null,
        SWARMDO_ROUTER_CALIBRATE: process.env.SWARMDO_ROUTER_CALIBRATE ?? null,
        SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK: process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK ?? null,
        SWARMDO_ROUTER_LATENCY_BUDGET_MS: process.env.SWARMDO_ROUTER_LATENCY_BUDGET_MS ?? null,
        SWARMDO_ROUTER_QUALITY_BAR: process.env.SWARMDO_ROUTER_QUALITY_BAR ?? null,
        SWARMDO_ROUTER_BANDIT_PER_MODEL: process.env.SWARMDO_ROUTER_BANDIT_PER_MODEL ?? null,
        SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD: process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD ?? null,
      },
...
const routerCostSavingsCommand: Command = {
  ...
  options: [
    { name: 'in', short: 'i', type: 'string', description: 'Trajectory JSONL path (default: $SWARMDO_ROUTER_TRAJECTORY_PATH or .swarm/...)' },
  ...
  examples: [
    { command: 'swarmdo neural router cost-savings', description: 'All-time, all baselines (heuristic + Sonnet-always + Opus-always)' },
    { command: 'swarmdo neural router cost-savings --baseline always-sonnet', description: 'Compare only against Sonnet-always' },
    { command: 'swarmdo neural router cost-savings --window 24h', description: 'Daily savings trend — iter 34 drift detection' },
    { command: 'swarmdo neural router cost-savings --since 7d --format json | jq .baselines.heuristic.savings.totalUsd', description: 'Pipe-friendly headline' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    const { MODEL_PRICES } = await import('../swarmvector/model-prices.js');
...
const routerTrajectoryHealthCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router trajectory-health', description: 'Snapshot of trajectory log health' },
    { command: 'swarmdo neural router trajectory-health --format json | jq .pairJoinRatePct', description: 'Pipe-friendly pair-join rate for dashboards' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    const recorderEnabled = process.env.SWARMDO_ROUTER_TRAJECTORY === '1';
    const maxSizeBytes = parseInt(process.env.SWARMDO_ROUTER_TRAJECTORY_MAXSIZE ?? `${10 * 1024 * 1024}`, 10) | 0;
    const maxRotations = Math.max(0, parseInt(process.env.SWARMDO_ROUTER_TRAJECTORY_MAXROTATIONS ?? '3', 10) || 3);
    ...
    if (!fs.existsSync(inPath)) {
      const payload = {
        ...
        hint: recorderEnabled
          ...
          : 'Recorder is OFF. Set SWARMDO_ROUTER_TRAJECTORY=1 to enable.',
      };
      if (fmt === 'json') output.writeln(JSON.stringify(payload, null, 2));
        ...
        output.writeln(`  Recorder gate:  ${recorderEnabled ? output.success('ON') : output.warning('OFF (SWARMDO_ROUTER_TRAJECTORY=1 to enable)')}`);
...
// SWARMDO_ROUTER_* env vars accumulated across iters 12-53. Operators
...
const routerConfigCommand: Command = {
  name: 'config',
  description: 'List all SWARMDO_ROUTER_* env vars with current values + effects (ADR-149 iter 54)',
  options: [
    { name: 'format', short: 'f', type: 'string', description: 'Output format: table, json', default: 'table' },
    { name: 'only-overrides', type: 'boolean', description: 'Only show env vars that are explicitly set (hide defaults)', default: false },
  ],
  examples: [
    { command: 'swarmdo neural router config', description: 'All router env vars with current values' },
    { command: 'swarmdo neural router config --only-overrides', description: 'Just what the operator has set' },
    { command: 'swarmdo neural router config --format json | jq', description: 'Audit / diff against another deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fmt = (ctx.flags.format as string) || 'table';
    ...
    const rows: EnvVarRow[] = [
      // Core gate (iter 0)
      { name: 'SWARMDO_ROUTER_NEURAL',                       iter: 0,  defaultValue: 'unset (0)',   currentValue: process.env.SWARMDO_ROUTER_NEURAL ?? '',     isOverride: !!process.env.SWARMDO_ROUTER_NEURAL,                       effect: 'Gate. =1 enables neural router; otherwise pure-bandit heuristic.' },
      { name: 'SWARMDO_ROUTER_MODEL_PATH',                   iter: 0,  defaultValue: 'unset',       currentValue: process.env.SWARMDO_ROUTER_MODEL_PATH ?? '', isOverride: !!process.env.SWARMDO_ROUTER_MODEL_PATH,                   effect: 'Override the KRR artifact path. Defaults to bundled.' },
      { name: 'SWARMDO_ROUTER_QUALITY_BAR',                  iter: 0,  defaultValue: '0.50',        currentValue: process.env.SWARMDO_ROUTER_QUALITY_BAR ?? '',isOverride: !!process.env.SWARMDO_ROUTER_QUALITY_BAR,                  effect: 'Cost-optimal mode: minimum predicted quality to pick a candidate.' },
      // Trajectory (iter 17)
      { name: 'SWARMDO_ROUTER_TRAJECTORY',                   iter: 17, defaultValue: 'unset (0)',   currentValue: process.env.SWARMDO_ROUTER_TRAJECTORY ?? '', isOverride: !!process.env.SWARMDO_ROUTER_TRAJECTORY,                   effect: 'Gate. =1 writes decision+outcome rows to .swarm/model-router-trajectories.jsonl.' },
      { name: 'SWARMDO_ROUTER_TRAJECTORY_PATH',              iter: 17, defaultValue: '.swarm/model-router-trajectories.jsonl', currentValue: process.env.SWARMDO_ROUTER_TRAJECTORY_PATH ?? '', isOverride: !!process.env.SWARMDO_ROUTER_TRAJECTORY_PATH, effect: 'Override the trajectory JSONL path.' },
      { name: 'SWARMDO_ROUTER_TRAJECTORY_MAXSIZE',           iter: 17, defaultValue: '10485760 (10MB)', currentValue: process.env.SWARMDO_ROUTER_TRAJECTORY_MAXSIZE ?? '', isOverride: !!process.env.SWARMDO_ROUTER_TRAJECTORY_MAXSIZE, effect: 'Bytes before rotation. Set 0 to disable rotation.' },
      { name: 'SWARMDO_ROUTER_TRAJECTORY_MAXROTATIONS',      iter: 17, defaultValue: '3',           currentValue: process.env.SWARMDO_ROUTER_TRAJECTORY_MAXROTATIONS ?? '', isOverride: !!process.env.SWARMDO_ROUTER_TRAJECTORY_MAXROTATIONS, effect: 'Max .bak files to keep when rotating.' },
      { name: 'SWARMDO_ROUTER_TRAJECTORY_TASKLEN',           iter: 17, defaultValue: '500',         currentValue: process.env.SWARMDO_ROUTER_TRAJECTORY_TASKLEN ?? '', isOverride: !!process.env.SWARMDO_ROUTER_TRAJECTORY_TASKLEN, effect: 'Max chars of task text to persist per row (truncated above this).' },
      // Latency budget (iter 12)
      { name: 'SWARMDO_ROUTER_LATENCY_BUDGET_MS',            iter: 12, defaultValue: '0 (unbounded)', currentValue: process.env.SWARMDO_ROUTER_LATENCY_BUDGET_MS ?? '', isOverride: !!process.env.SWARMDO_ROUTER_LATENCY_BUDGET_MS, effect: 'Drop candidates whose measured p50 latency > this many ms before selection.' },
      // Per-modelId bandit (iter 14)
      { name: 'SWARMDO_ROUTER_BANDIT_PER_MODEL',             iter: 14, defaultValue: 'unset (0)',   currentValue: process.env.SWARMDO_ROUTER_BANDIT_PER_MODEL ?? '', isOverride: !!process.env.SWARMDO_ROUTER_BANDIT_PER_MODEL, effect: 'Gate. =1 enables per-modelId Thompson sampling perturbation of neural prediction.' },
      // k-NN backend (iter 0)
      { name: 'SWARMDO_ROUTER_KNN_K',                        iter: 0,  defaultValue: '5',           currentValue: process.env.SWARMDO_ROUTER_KNN_K ?? '',      isOverride: !!process.env.SWARMDO_ROUTER_KNN_K,                        effect: 'k for the k-NN backend when KRR is not loadable.' },
      { name: 'SWARMDO_ROUTER_SEED_CORPUS',                  iter: 0,  defaultValue: 'bundled seed-rows.json', currentValue: process.env.SWARMDO_ROUTER_SEED_CORPUS ?? '', isOverride: !!process.env.SWARMDO_ROUTER_SEED_CORPUS, effect: 'Override the DRACO seed corpus path.' },
      // Calibration (iter 22-25)
      { name: 'SWARMDO_ROUTER_CALIBRATE',                    iter: 24, defaultValue: 'unset (default ON)', currentValue: process.env.SWARMDO_ROUTER_CALIBRATE ?? '', isOverride: !!process.env.SWARMDO_ROUTER_CALIBRATE, effect: 'Isotonic calibration of KRR predictions. =0 opts out (recovers raw KRR). Default-on since iter 24 (OOS validated).' },
      { name: 'SWARMDO_ROUTER_CALIBRATOR_PATH',              iter: 22, defaultValue: 'bundled seed-router.calibrator.json', currentValue: process.env.SWARMDO_ROUTER_CALIBRATOR_PATH ?? '', isOverride: !!process.env.SWARMDO_ROUTER_CALIBRATOR_PATH, effect: 'Override the unified calibrator path. Per-tier files (low/med/high) load from the same dir.' },
      // Cost ceiling (iter 29)
      { name: 'SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK',    iter: 29, defaultValue: '0 (disabled)', currentValue: process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK ?? '', isOverride: !!process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK, effect: 'Orthogonal selector mode: pick BEST quality among candidates ≤ ceiling $/Mtok blended.' },
      // A/B mode (iter 5/37)
      { name: 'SWARMDO_ROUTER_AB',                           iter: 5,  defaultValue: 'unset (0)',   currentValue: process.env.SWARMDO_ROUTER_AB ?? '',         isOverride: !!process.env.SWARMDO_ROUTER_AB,                           effect: 'Legacy all-on A/B mode. Records bandit_pick + hybrid_pick on every decision.' },
      { name: 'SWARMDO_ROUTER_AB_SAMPLE_RATE',               iter: 37, defaultValue: '0 (disabled)', currentValue: process.env.SWARMDO_ROUTER_AB_SAMPLE_RATE ?? '', isOverride: !!process.env.SWARMDO_ROUTER_AB_SAMPLE_RATE, effect: 'Sampled A/B mode. 0..1 fraction of decisions to A/B (deterministic by task_hash). Overrides legacy AB=1.' },
      // Ensemble uncertainty (iter 44)
      { name: 'SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD', iter: 44, defaultValue: '0 (disabled)', currentValue: process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD ?? '', isOverride: !!process.env.SWARMDO_ROUTER_ENSEMBLE_UNCERTAINTY_THRESHOLD, effect: 'When > 0: if |unified_q - specialist_q| > threshold for picked model, fall back to bandit.' },
      // Bandit warmup (iter 52/53)
      { name: 'SWARMDO_ROUTER_BANDIT_WARMUP_RANGE',          iter: 52, defaultValue: '8',           currentValue: process.env.SWARMDO_ROUTER_BANDIT_WARMUP_RANGE ?? '', isOverride: !!process.env.SWARMDO_ROUTER_BANDIT_WARMUP_RANGE, effect: 'Continuous warmup denominator. Smaller = bandit ramps faster; larger = more conservative.' },
      { name: 'SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE',        iter: 53, defaultValue: 'unset (0)',   currentValue: process.env.SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE ?? '', isOverride: !!process.env.SWARMDO_ROUTER_BANDIT_FULL_INFLUENCE, effect: 'Gate. =1 uses asymptotic curve (samples-2)/(samples+WARMUP) — bandit dominates at scale.' },
      { name: 'SWARMDO_ROUTER_BANDIT_SHRINKAGE_LAMBDA',      iter: 57, defaultValue: '4',           currentValue: process.env.SWARMDO_ROUTER_BANDIT_SHRINKAGE_LAMBDA ?? '', isOverride: !!process.env.SWARMDO_ROUTER_BANDIT_SHRINKAGE_LAMBDA, effect: 'Cross-bucket shrinkage strength. λ=0 disables; higher λ = more bias toward marginal anchor for cold cells.' },
    ];
...
// flipping SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK on for production.
const routerCompareModesCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router compare-modes "refactor strategy pattern"', description: 'Compare both modes for a task' },
    { command: 'swarmdo neural router compare-modes -t "..." --ceiling 5', description: 'Cost-ceiling at $5 blended' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const task = (ctx.flags.task as string | undefined) ?? (ctx.args && ctx.args[0]) ?? null;
    if (!task) {
    ...
    const { embedTaskWithCache } = await import('../swarmvector/task-embedder.js');
    const { tryCostOptimalRoute, __resetNeuralRouterForTests } = await import('../swarmvector/neural-router.js');
    const { analyzeTaskComplexity } = await import('../swarmvector/model-router.js');
    ...
    delete process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK;
    __resetNeuralRouterForTests();
    const costOptimal = await tryCostOptimalRoute(embedding, { complexityBucket: bucket });

    // Mode 2: cost-ceiling
    process.env.SWARMDO_ROUTER_COST_CEILING_USD_PER_MTOK = String(ceiling);
    __resetNeuralRouterForTests();
...
const routerStatsSummaryCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router stats-summary', description: 'One-screen health check' },
    { command: 'swarmdo neural router stats-summary --format json | jq .', description: 'Pipe to dashboards / alerting' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    const trajectoryPath = process.env.SWARMDO_ROUTER_TRAJECTORY_PATH
      ?? path.resolve(process.cwd(), '.swarm', 'model-router-trajectories.jsonl');
    let trajectory: {
...
const routerBanditStateCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router bandit-state', description: 'Show all Beta priors per bucket × tier + per bucket × modelId' },
    { command: 'swarmdo neural router bandit-state --format json | jq .priorsById', description: 'Just the per-modelId matrix' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    if (!fs.existsSync(statePath)) {
      ...
      if (fmt === 'json') output.writeln(JSON.stringify({ error: msg, hint: 'State is created on first routing decision. Run any agent_spawn flow with SWARMDO_ROUTER_NEURAL=1.' }, null, 2));
      else {
        output.printError(msg);
...
const routerAbStatsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router ab-stats', description: 'All A/B comparisons recorded so far' },
    { command: 'swarmdo neural router ab-stats --since 7d --format json', description: 'Last 7 days, pipe-friendly' },
    { command: 'SWARMDO_ROUTER_AB_SAMPLE_RATE=0.05 ... && swarmdo neural router ab-stats', description: 'After running with iter 37 sampling on' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    if (!fs.existsSync(inPath)) {
      ...
      if (fmt === 'json') output.writeln(JSON.stringify({ error: msg }, null, 2));
        ...
        output.printError(msg);
        output.writeln(output.dim('  Set SWARMDO_ROUTER_TRAJECTORY=1 + SWARMDO_ROUTER_AB_SAMPLE_RATE=0.05 to start collecting ab_pair data.'));
      }
      return { success: false, exitCode: 1 };
    }
    ...
    if (abRows.length === 0) {
      ...
      output.writeln(output.dim('    export SWARMDO_ROUTER_TRAJECTORY=1'));
      output.writeln(output.dim('    export SWARMDO_ROUTER_AB_SAMPLE_RATE=0.05  # 5% sampling'));
      output.writeln('');
      return { success: true, data: payload };
    }
...
// previously the answer required reading src/swarmvector/model-prices.ts.
const routerPricesCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router prices', description: 'Show all models sorted by blended price' },
    { command: 'swarmdo neural router prices --sort name', description: 'Sort alphabetically' },
    { command: 'swarmdo neural router prices --format json | jq \'.[] | select(.id | contains("opus"))\'', description: 'Filter via jq' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const { MODEL_PRICES, blendedPrice } = await import('../swarmvector/model-prices.js');
    const fmt = (ctx.flags.format as string) || 'table';
...
const routerCostProjectionCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router cost-projection', description: 'Project from last 7d of data → 7d/30d/90d/365d horizons' },
    { command: 'swarmdo neural router cost-projection --window 24h --horizons 7d,30d', description: 'Project from last day only' },
    { command: 'swarmdo neural router cost-projection --format json | jq .horizons[1].projectedSavingsUsd', description: '30-day savings projection for dashboards' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    ...
    if (pairCount === 0) {
      ...
      const payload = { error: msg, windowSpec, hint: 'Enable SWARMDO_ROUTER_TRAJECTORY=1 and run some routed agent calls first.' };
      if (fmt === 'json') output.writeln(JSON.stringify(payload, null, 2));
      else {
        output.printError(msg);
...
const routerCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural router status', description: 'Show router state, gate, counters' },
    { command: 'swarmdo neural router models', description: 'List candidate registry with measured stats (ADR-149)' },
    { command: 'swarmdo neural router prices', description: 'Show the canonical $/Mtok price table (iter 43)' },
    { command: 'swarmdo neural router config', description: 'Inventory all SWARMDO_ROUTER_* env vars (iter 54)' },
    { command: 'swarmdo neural router train -o ./router.krr.json', description: 'Train a KRR artifact' },
    { command: 'swarmdo neural router train-from-trajectories -w production-rows.json', description: 'Pair production JSONL into a training corpus (iter 18)' },
    { command: 'swarmdo neural router decide "fix typo in cache.ts"', description: 'Inspect decision for a hypothetical task (iter 30)' },
    { command: 'swarmdo neural router compare-modes "task" --ceiling 5', description: 'Compare cost-optimal vs cost-ceiling side-by-side (iter 55)' },
    { command: 'swarmdo neural router decisions --since 24h', description: 'Query recorded decisions (iter 28)' },
    { command: 'swarmdo neural router cost-savings --since 7d', description: 'Actual vs heuristic-counterfactual spend (iter 32)' },
    { command: 'swarmdo neural router trajectory-health', description: 'JSONL log health: size, rotations, parse + pair-join rate (iter 36)' },
    { command: 'swarmdo neural router ab-stats', description: 'A/B disagreement matrix from sampled ab_pair (iter 37/38)' },
    { command: 'swarmdo neural router bandit-state', description: 'Persisted Beta priors per (bucket × model) (iter 48)' },
    { command: 'swarmdo neural router stats-summary', description: 'One-screen SRE dashboard (iter 49)' },
    { command: 'swarmdo neural router cost-projection', description: 'Project monthly/quarterly spend from measured rate (iter 41)' },
    { command: 'swarmdo neural router reload', description: 'Clear in-process backend cache' },
  ],
  action: async (): Promise<CommandResult> => {
...
export const neuralCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo neural status', description: 'Check neural system status' },
    { command: 'swarmdo neural train -p coordination', description: 'Train coordination patterns' },
    { command: 'swarmdo neural patterns --action list', description: 'List learned patterns' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Neural System'));
