 ...
 * System startup for Swarmdo orchestration
 */
...
function isInitialized(cwd: string): boolean {
  const configPath = path.join(cwd, '.swarmdo', 'config.yaml');
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('Swarmdo is not initialized in this directory');
    output.printInfo('Run "swarmdo init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting Swarmdo V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('Swarmdo V3 is running!');
    output.writeln();
    ...
    output.printList([
      `${output.highlight('swarmdo status')} - View system status`,
      `${output.highlight('swarmdo agent spawn -t coder')} - Spawn an agent`,
      `${output.highlight('swarmdo swarm status')} - View swarm details`,
      `${output.highlight('swarmdo stop')} - Stop the system`
    ]);

    // Daemon mode
    if (daemon) {
      output.writeln();
      output.printInfo('Running in daemon mode. Use "swarmdo stop" to stop.');

      // Store PID for daemon management
      const daemonPidPath = path.join(cwd, '.swarmdo', 'daemon.pid');
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the Swarmdo system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping Swarmdo'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop Swarmdo?',
        default: false
      });
    ...
    try {
      ...
      // Clean up daemon PID
      const daemonPidPath = path.join(ctx.cwd, '.swarmdo', 'daemon.pid');
      if (fs.existsSync(daemonPidPath)) {
        fs.unlinkSync(daemonPidPath);
      }

      output.writeln();
      output.printSuccess('Swarmdo stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the Swarmdo system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting Swarmdo'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the Swarmdo orchestration system',
  ...
  examples: [
    { command: 'swarmdo start', description: 'Start with configuration defaults' },
    { command: 'swarmdo start --daemon', description: 'Start as background daemon' },
    { command: 'swarmdo start --port 3001', description: 'Start MCP on custom port' },
    { command: 'swarmdo start --topology mesh', description: 'Start with mesh topology' },
    { command: 'swarmdo start --skip-mcp', description: 'Start without MCP server' },
    { command: 'swarmdo start quick', description: 'Quick start with defaults' },
    { command: 'swarmdo start stop', description: 'Stop the running system' }
  ],
  action: startAction
};
