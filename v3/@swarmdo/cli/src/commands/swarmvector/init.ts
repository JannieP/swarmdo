/**
 * V3 CLI SwarmVector Init Command
 * Initialize SwarmVector PostgreSQL Bridge
 */
...
function getConnectionConfig(ctx: CommandContext): {
  ...
  return {
    ...
    schema: validateSchemaName((ctx.flags.schema as string) || 'swarmdo'),
  };
}
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize SwarmVector in PostgreSQL',
  options: [
    {
      name: 'host',
      short: 'h',
    ...
    },
    {
      name: 'schema',
      short: 's',
      description: 'Schema name for SwarmVector tables',
      type: 'string',
      default: 'swarmdo',
    },
    {
  ...
  examples: [
    { command: 'swarmdo swarmvector init -d mydb', description: 'Initialize with database name' },
    { command: 'swarmdo swarmvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'swarmdo swarmvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'swarmdo swarmvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    let config = getConnectionConfig(ctx);

    output.writeln();
    output.writeln(output.bold('SwarmVector PostgreSQL Initialization'));
    ...
    try {
      ...
      // Detect vector extension: prefer swarmvector, fall back to pgvector
      ...
      // Check for swarmvector extension first (ships with swarmvector-postgres image)
      const swarmvectorResult = await client.query(`
        SELECT extname, extversion FROM pg_extension WHERE extname = 'swarmvector'
      `);

      if (swarmvectorResult.rows.length > 0) {
        vectorExtName = 'swarmvector';
        vectorTypeName = 'swarmvector';
        spinner.succeed(`swarmvector v${swarmvectorResult.rows[0].extversion} found`);
      } else {
        ...
        if (pgvectorResult.rows.length > 0) {
          ...
          // Neither installed -- try to create swarmvector first, then pgvector
          spinner.succeed('No vector extension found, attempting to create...');
          let created = false;
          try {
            await client.query("CREATE EXTENSION IF NOT EXISTS swarmvector");
            vectorExtName = 'swarmvector';
            vectorTypeName = 'swarmvector';
            spinner.succeed('swarmvector extension created');
            created = true;
          } catch {
            // swarmvector not available, try pgvector
          }
          if (!created) {
            try {
      ...
      const cosineOps = vectorExtName === 'swarmvector' ? 'swarmvector_cosine_ops' : 'vector_cosine_ops';
      ...
      // Create tables
      spinner.setText('Creating SwarmVector tables...'); spinner.start();
      ...
      await client.query(`
        ...
        VALUES ('swarmvector_version', '"1.0.0"')
      ...
      spinner.succeed('SwarmVector tables created');
      ...
      await client.query(`
        ...
        VALUES ('1.0.0', 'Initial SwarmVector setup')
      ...
      output.writeln();
      output.printSuccess('SwarmVector PostgreSQL Bridge initialized successfully!');
      output.writeln();
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: swarmdo swarmvector migrate --up',
        '  2. Check status: swarmdo swarmvector status --verbose',
        '  3. Run benchmark: swarmdo swarmvector benchmark',
