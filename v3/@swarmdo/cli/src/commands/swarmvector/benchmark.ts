/**
 * V3 CLI SwarmVector Benchmark Command
 * Performance benchmarking for SwarmVector PostgreSQL Bridge
 */
...
export const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo swarmvector benchmark', description: 'Run default benchmark' },
    { command: 'swarmdo swarmvector benchmark --vectors 50000', description: 'Benchmark with 50k vectors' },
    { command: 'swarmdo swarmvector benchmark --index ivfflat', description: 'Test IVFFlat index' },
    { command: 'swarmdo swarmvector benchmark --dimensions 768 --metric l2', description: 'Custom dimensions and metric' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('SwarmVector Performance Benchmark'));
    ...
    try {
      ...
      // Detect vector extension type: prefer swarmvector, fall back to pgvector
      let vectorTypeName = 'vector';
      const swarmvectorCheck = await client.query(`
        SELECT extname FROM pg_extension WHERE extname = 'swarmvector'
      `);
      if (swarmvectorCheck.rows.length > 0) {
      ...
      if (indexType !== 'none') {
        ...
        const opsPrefix = vectorTypeName === 'swarmvector' ? 'swarmvector' : 'vector';
