/**
 * V3 CLI SwarmVector Import Command
 * Import data from sql.js/JSON memory to SwarmVector PostgreSQL
 *
 * Usage:
...
/**
 * Format a swarmvector embedding array for PostgreSQL
...
function formatEmbedding(embedding: number[], dimensions: number = 384): string {
  ...
  return `'[${padded.join(',')}]'::swarmvector(${dimensions})`;
}
...
function generateInsertSQL(entry: MemoryEntry): string {
  ...
  return `INSERT INTO swarmdo.memory_entries (key, value, embedding, namespace, metadata, created_at, updated_at)
VALUES (
  '${key}',
  '${value}',
  ${embeddingClause},
...
/**
 * SwarmVector Import command - import from sql.js/JSON to PostgreSQL
 */
export const importCommand: Command = {
  name: 'import',
  description: 'Import data from sql.js/JSON memory to SwarmVector PostgreSQL',
  ...
  options: [
    ...
    {
      name: 'from-memory',
      description: 'Export from current Swarmdo memory and import',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'swarmdo swarmvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'swarmdo swarmvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'swarmdo swarmvector import --from-memory', description: 'Export current memory and import' },
    { command: 'swarmdo swarmvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const containerName = (ctx.flags.container as string) || 'swarmvector-postgres';
    const verbose = ctx.flags.verbose as boolean;

    output.writeln();
    output.writeln(output.bold('SwarmVector PostgreSQL Import'));
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  swarmdo swarmvector import --input memory-export.json');
      output.writeln('  swarmdo swarmvector import --from-memory');
    ...
    if (fromMemory) {
      output.printInfo('Exporting from current Swarmdo memory...');
      output.printWarning('Note: Run "npx swarmdo memory list --format json > memory-export.json" first');
      output.printInfo('Then use: npx swarmdo swarmvector import --input memory-export.json');
    ...
    sqlStatements.push('-- SwarmVector PostgreSQL Import');
    ...
    if (outputFile) {
      try {
        ...
        output.writeln(`  docker exec -i ${containerName} psql -U claude -d swarmdo < ${outputFile}`);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      const tempFile = path.join(process.cwd(), '.swarmvector-import-temp.sql');
      try {
        ...
        output.writeln(output.dim(`  docker exec -i ${containerName} psql -U claude -d swarmdo < ${tempFile}`));
        output.writeln();
        ...
        try {
          ...
          output.writeln(`  docker exec -i ${containerName} psql -U claude -d swarmdo < ${tempFile}`);
    ...
    output.writeln(`  docker exec ${containerName} psql -U claude -d swarmdo -c "SELECT COUNT(*) FROM swarmdo.memory_entries;"`);
    output.writeln();
