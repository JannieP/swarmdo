/**
 * V3 CLI SwarmVector Migrate Command
 * Database migration management for SwarmVector
 */
...
const MIGRATIONS: Migration[] = [
  {
    version: '1.0.0',
    name: 'Initial SwarmVector setup',
...
export const migrateCommand: Command = {
  ...
  examples: [
    ...
    { command: 'swarmdo swarmvector migrate --down', description: 'Rollback last migration' },
    { command: 'swarmdo swarmvector migrate --to 1.2.0', description: 'Migrate to version 1.2.0' },
    { command: 'swarmdo swarmvector migrate --dry-run', description: 'Preview migration SQL' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('SwarmVector Migration'));
    ...
    try {
      ...
      if (swarmvectorCheck.rows.length > 0) {
        ...
        cosineOps = 'swarmvector_cosine_ops';
      }
      ...
      if (schemaExists.rows.length === 0) {
        ...
        output.printError('Run `swarmdo swarmvector init` first');
        await client.end();
        return { success: false, exitCode: 1 };
      }
