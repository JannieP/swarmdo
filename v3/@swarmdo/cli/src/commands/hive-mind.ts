...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__swarmdo__hive-mind_consensus    - Democratic decision making
   mcp__swarmdo__hive-mind_memory       - Share knowledge across the hive
   mcp__swarmdo__hive-mind_broadcast    - Broadcast to all workers
   mcp__swarmdo__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__swarmdo__hive-mind_status       - Monitor swarm health
   mcp__swarmdo__task_create            - Create and delegate tasks
   mcp__swarmdo__coordination_orchestrate - Orchestrate task distribution
   mcp__swarmdo__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__swarmdo__agent_list             - List all active agents
   mcp__swarmdo__agent_status           - Check agent status
   mcp__swarmdo__agent_health           - Check worker health
   mcp__swarmdo__hive-mind_join         - Add agent to hive
   mcp__swarmdo__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__swarmdo__task_assign            - Assign tasks to workers
   mcp__swarmdo__task_status            - Track task progress
   mcp__swarmdo__task_complete          - Mark tasks complete
   mcp__swarmdo__workflow_create        - Create workflows

5️⃣ **MEMORY & LEARNING**
   mcp__swarmdo__memory_store           - Store collective knowledge
   mcp__swarmdo__memory_retrieve        - Access shared memory
   mcp__swarmdo__memory_search          - Search memory patterns
   mcp__swarmdo__neural_train           - Learn from experiences
   mcp__swarmdo__hooks_intelligence_pattern-store - Store patterns
...
• You MUST use Swarmdo MCP tools (mcp__swarmdo__*) for ALL orchestration tasks
• Do NOT use Claude native Task/Agent tools for swarm coordination — use mcp__swarmdo__agent_spawn, mcp__swarmdo__task_assign, etc.
• Native Claude tools (Read, Write, Edit, Bash, Grep, Glob) should ONLY be used for file operations and shell commands
• All agent spawning, task assignment, memory, and coordination MUST go through mcp__swarmdo__* tools
• If a Swarmdo MCP tool exists for an operation, always prefer it over any native equivalent

💡 COORDINATION TIPS:
• Use mcp__swarmdo__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__swarmdo__hive-mind_status
• Store important decisions in shared memory for persistence
• Use consensus for any decisions affecting multiple workers
• Use mcp__swarmdo__task_assign to assign tasks to workers, then mcp__swarmdo__task_complete when done
...
async function spawnClaudeCodeInstance(
  ...
  try {
    ...
    output.printList([
      ...
      `MCP Tools: ${output.success('Full Swarmdo integration enabled')}`
    ]);
    ...
    if (claudeAvailable && !dryRun) {
      ...
      // mcp__swarmdo__* tools registered. Before this, the coordination prompt
      // referenced tools the worker didn't know about and exited silently.
      // Resolution order:
      //   1. explicit --mcp-config <path> flag passed by the caller
      //   2. ./.mcp.json in cwd (project-local Swarmdo MCP config)
      ...
      if (mcpConfigPath) {
        ...
        output.printWarning('No .mcp.json or ~/.claude.json found — spawned worker will not have mcp__swarmdo__* tools (#1748 Issue 2). Pass --mcp-config <path> or run "swarmdo init" to generate one.');
      }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo hive-mind init -t hierarchical-mesh', description: 'Init hierarchical mesh' },
    { command: 'swarmdo hive-mind init -c byzantine -m 20', description: 'Init with Byzantine consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('  Use "swarmdo hive-mind spawn" to add workers'));
      output.writeln(output.dim('  Use "swarmdo hive-mind spawn --claude" to launch Claude Code'));
...
const spawnCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo hive-mind spawn -n 5', description: 'Spawn 5 workers' },
    { command: 'swarmdo hive-mind spawn -n 3 -r specialist', description: 'Spawn 3 specialists' },
    { command: 'swarmdo hive-mind spawn -t coder -p my-coder', description: 'Spawn coder with custom prefix' },
    { command: 'swarmdo hive-mind spawn --claude -o "Build a REST API"', description: 'Launch Claude Code with objective' },
    { command: 'swarmdo hive-mind spawn -n 5 --claude -o "Research AI patterns"', description: 'Spawn workers and launch Claude Code' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (workerData.length === 0) {
        output.printInfo('No workers in hive. Use "swarmdo hive-mind spawn" to add workers.');
      } else {
        output.printTable({
          columns: [
...
const taskCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo hive-mind task -d "Implement auth module"', description: 'Submit task' },
    { command: 'swarmdo hive-mind task -d "Security review" -p critical -c', description: 'Critical task with consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // server (the `mcp__swarmdo__hive-mind_*` surface only exposes init,
      ...
      output.writeln(output.dim(`  Track with: swarmdo hive-mind task-status ${result.taskId}`));
...
export const hiveMindCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo hive-mind init -t hierarchical-mesh', description: 'Initialize hive' },
    { command: 'swarmdo hive-mind spawn -n 5', description: 'Spawn workers' },
    { command: 'swarmdo hive-mind spawn --claude -o "Build a feature"', description: 'Launch Claude Code with hive mind' },
    { command: 'swarmdo hive-mind task -d "Build feature"', description: 'Submit task' }
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Hive Mind - Consensus-Based Multi-Agent Coordination'));
    output.writeln();
    output.writeln('Usage: swarmdo hive-mind <subcommand> [options]');
    output.writeln();
    ...
    output.writeln(output.dim('  swarmdo hive-mind init'));
    output.writeln(output.dim('  swarmdo hive-mind spawn -n 5 --claude -o "Your objective here"'));
