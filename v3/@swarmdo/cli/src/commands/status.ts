 ...
 * System status display for Swarmdo
 */
...
//
// #2120 — the old check required `.swarmdo/config.yaml`, which
// missed projects that were initialized via `swarmdo memory init` (writes
...
// and a running MCP, yet `swarmdo status` reported "not initialized".
//
// Now: any of these signals counts as initialized:
//   - `.swarmdo/config.yaml`   (the canonical `swarmdo init` output)
//   - `.swarmdo/config.json`   (same, alt format)
//   - `.swarm/memory.db`           (the `swarmdo memory init` output)
//   - `.claude/settings.json`      (the Claude Code hook surface)
function isInitialized(cwd: string): boolean {
  const candidates = [
    path.join(cwd, '.swarmdo', 'config.yaml'),
    path.join(cwd, '.swarmdo', 'config.json'),
...
function displayStatus(status: Awaited<ReturnType<typeof getSystemStatus>>): void {
  ...
  output.writeln(`${output.bold('Swarmdo V3')} ${statusIcon}`);
  output.writeln();
...
export const statusCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo status', description: 'Show current system status' },
    { command: 'swarmdo status --watch', description: 'Watch mode with live updates' },
    { command: 'swarmdo status --watch -i 5', description: 'Watch mode updating every 5 seconds' },
    { command: 'swarmdo status --health-check', description: 'Run health checks' },
    { command: 'swarmdo status --json', description: 'Output status as JSON' },
    { command: 'swarmdo status agents', description: 'Show detailed agent status' },
    { command: 'swarmdo status tasks', description: 'Show detailed task status' },
    { command: 'swarmdo status memory', description: 'Show detailed memory status' }
  ],
  action: statusAction
};
