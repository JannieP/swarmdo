 ...
 *
 * @module @swarmdo/cli/commands/mcp
 * @version 3.0.0
 */
...
const startCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo mcp start', description: 'Start with defaults (stdio)' },
    { command: 'swarmdo mcp start -p 8080 -t http', description: 'Start HTTP server' },
    { command: 'swarmdo mcp start -d', description: 'Start as daemon' },
    { command: 'swarmdo mcp start -f', description: 'Force restart (kill existing)' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (tools && tools !== 'all') {
      process.env.SWARMDO_TOOLS_GROUPS = tools;
    } else if (toolsProfile && toolsProfile !== 'full') {
      process.env.SWARMDO_TOOLS_PROFILE = toolsProfile;
    }
    ...
    if (existingStatus.running && !isSelfDetected) {
      ...
      if (!shouldForceRestart) {
        ...
        if (health.healthy) {
          ...
          output.writeln(output.dim('Use "swarmdo mcp stop" to stop the server first, or use --force'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (!status.running) {
        ...
        output.writeln();
        output.writeln(output.dim('Run "swarmdo mcp start" to start the server'));
        return { success: true, data: status };
      }
...
const execCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo mcp exec -t swarm_init -p \'{"topology":"mesh"}\'', description: 'Execute tool' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const mcpCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo mcp start', description: 'Start MCP server' },
    { command: 'swarmdo mcp start -t http -p 8080', description: 'Start HTTP server on port 8080' },
    { command: 'swarmdo mcp status', description: 'Show server status' },
    { command: 'swarmdo mcp tools', description: 'List tools' },
    { command: 'swarmdo mcp stop', description: 'Stop the server' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('MCP Server Management'));
    output.writeln();
    output.writeln('Usage: swarmdo mcp <subcommand> [options]');
    output.writeln();
