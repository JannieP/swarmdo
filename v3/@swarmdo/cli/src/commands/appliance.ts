...
async function loadModule<T>(path: string, exportName: string, label: string): Promise<T | null> {
  try {
    ...
    output.printError(`RVFA ${label} module not found`, 'Install with: npm install @swarmdo/appliance');
    return null;
  }
}
...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained swarmdo.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'swarmdo.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'swarmdo.rvf';
    ...
    const steps = [
      ...
      'Packaging swarmdo CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'swarmdo'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'swarmdo', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'swarmdo appliance inspect -f swarmdo.rvf', description: 'Inspect appliance contents' },
    { command: 'swarmdo appliance verify -f swarmdo.rvf', description: 'Verify integrity' },
    { command: 'swarmdo appliance extract -f swarmdo.rvf', description: 'Extract sections' },
    { command: 'swarmdo appliance run -f swarmdo.rvf', description: 'Boot and run appliance' },
    { command: 'swarmdo appliance sign -f swarmdo.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'swarmdo appliance publish -f swarmdo.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'swarmdo appliance update -f swarmdo.rvf -s swarmdo -d ./new-swarmdo.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full Swarmdo platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained swarmdo.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "swarmdo appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
