...
function getClaimsConfigPaths(): string[] {
  return [
    ...
    resolve(process.env.HOME || '~', '.config/swarmdo/claims.json'),
  ];
}
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims list', description: 'List all claims' },
    { command: 'swarmdo claims list -u user123', description: 'List user claims' },
  ],
...
const checkCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims check -c swarm:create', description: 'Check swarm creation permission' },
    { command: 'swarmdo claims check -c admin:delete -u user123', description: 'Check user permission' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const claimsConfigPaths = [
        path.resolve('.swarmdo/claims.json'),
        path.resolve('swarmdo.claims.json'),
        path.resolve(process.env.HOME || '~', '.config/swarmdo/claims.json'),
      ];
...
const grantCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims grant -c swarm:create -u user123', description: 'Grant to user' },
    { command: 'swarmdo claims grant -c agent:spawn -r developer', description: 'Grant to role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
...
const revokeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims revoke -c swarm:delete -u user123', description: 'Revoke from user' },
    { command: 'swarmdo claims revoke -c admin:* -r guest', description: 'Revoke from role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
    const user = ctx.flags.user as string;
...
const rolesCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims roles', description: 'List all roles' },
    { command: 'swarmdo claims roles -a show -n admin', description: 'Show role details' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const policiesCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims policies', description: 'List policies' },
    { command: 'swarmdo claims policies -a create -n rate-limit', description: 'Create policy' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags.action as string) || 'list';
    const name = ctx.flags.name as string;
...
export const claimsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo claims list', description: 'List all claims' },
    { command: 'swarmdo claims check -c swarm:create', description: 'Check permission' },
    { command: 'swarmdo claims grant -c agent:spawn -r developer', description: 'Grant claim' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Claims System'));
