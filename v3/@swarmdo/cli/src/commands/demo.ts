 ...
 * systems behind swarmdo work, on the user's machine, and writes
 * `.claude-flow/bench-results.json` so `swarmdo doctor` (Move 4) can surface
 * the measured numbers as honest health-check rows.
 *
 * Closes the audit gap where users couldn't tell which of swarmdo's 275 MCP
 * tools were real. A single `swarmdo demo` invocation now produces:
 ...
 *   4. Embedding backend in use (swarmvector ONNX vs hash-fallback)
 *
 ...
 * a reason if the script can't run (e.g. dist/ not built, swarmvector missing).
 */
...
async function measureEd25519(iterations = 1000): Promise<DemoResults['ed25519']> {
  ...
  try {
    ...
    const msg = new TextEncoder().encode('swarmdo-demo-canary');
...
async function measureAgentRun(verbose: boolean): Promise<DemoResults['agentRun']> {
  ...
  const result = await tool.handler({
    agentType: 'coder',
    prompt: 'Respond with exactly the three words: swarmdo demo ok',
    model: 'haiku',
    maxTokens: 32,
    temperature: 0,
    timeoutMs: 30_000,
...
async function measureEmbeddingBackend(): Promise<DemoResults['embeddingBackend']> {
  try {
    const rv = await import('swarmvector').catch(() => null) as unknown as {
    ...
    if (!rv?.embed || !rv?.isOnnxAvailable?.()) {
      return {
        ...
        reason: 'swarmvector ONNX not available — install swarmvector with native bindings, or accept the deterministic hash fallback used by neural-tools',
      };
    }
    if (typeof rv.initOnnxEmbedder === 'function') await rv.initOnnxEmbedder();
    const r = await rv.embed('swarmdo demo embedding probe');
    const v = (r as { embedding?: number[] })?.embedding ?? (r as number[]);
    const arr = Array.isArray(v) ? v : Array.from(v as ArrayLike<number>);
    return { backend: 'swarmvector (all-MiniLM-L6-v2)', dims: arr.length };
  } catch (err) {
    return {
...
function persistResults(results: DemoResults): string {
  // Write to the SHARED .swarmdo/bench-results.json in the doctor-compatible
  // shape (hnsw.entries[] + persistedAt) so `swarmdo doctor -c benchmarks`
  // surfaces these numbers — same file `swarmdo performance benchmark` writes.
 ...
 * (in order): SWARMDO_REPO_ROOT env var, walk-up from CWD, walk-up from this
...
export const demoCommand: Command = {
  name: 'demo',
  description: 'Run a 30-second capability tour — proves HNSW, Ed25519, agent_run, and the embedding backend really work on this machine. Writes .swarmdo/bench-results.json so `swarmdo doctor` can surface measured numbers.',
  options: [
    {
  ...
  examples: [
    { command: 'swarmdo demo', description: 'Full capability tour (~30s)' },
    { command: 'swarmdo demo --skip-llm', description: 'Skip LLM step — no provider key needed' },
    { command: 'swarmdo demo --json', description: 'Emit JSON only (for CI smoke tests)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!asJson) {
      output.writeln();
      output.writeln(output.bold('Swarmdo Demo'));
    ...
    output.writeln(output.dim('Tip: run `swarmdo doctor` to see these numbers surfaced as health checks (after Move 4 ships).'));
    output.writeln();
