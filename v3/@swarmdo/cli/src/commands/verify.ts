 ...
 *
 * Run via: swarmdo verify [--branch <branch>] [--manifest <local-path>]
 *
...
const DEFAULT_MANIFEST_URL = 'https://raw.githubusercontent.com/upstream/swarmdo/{branch}/verification.md.json';
...
async function verifySignature(witness: Witness): Promise<{
  ...
  const seed = createHash('sha256').update(witness.manifest.gitCommit + ':swarmdo-witness/v1').digest();
...
export const verifyCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo verify', description: 'Fetch latest manifest from main branch + verify' },
    { command: 'swarmdo verify --branch main', description: 'Verify against a specific branch' },
    { command: 'swarmdo verify --manifest ./verification.md.json', description: 'Use a local manifest copy' },
    { command: 'swarmdo verify --json', description: 'Machine-readable output for CI' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!asJson) {
      output.writeln();
      output.writeln(output.bold('Swarmdo Verification'));
