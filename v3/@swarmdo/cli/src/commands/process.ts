...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.swarmdo/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.swarmdo/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  examples: [
    { command: 'swarmdo process daemon --action start', description: 'Start the daemon' },
    { command: 'swarmdo process daemon --action stop', description: 'Stop the daemon' },
    { command: 'swarmdo process daemon --action restart --port 3850', description: 'Restart on different port' },
    { command: 'swarmdo process daemon --action status', description: 'Check daemon status' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags?.action as string) || 'status';
    const port = (ctx.flags?.port as number) || 3847;
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.swarmdo/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.swarmdo/daemon.log';
    ...
    switch (action) {
      ...
      case 'status':
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: swarmdo process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo process monitor', description: 'Show process dashboard' },
    { command: 'swarmdo process monitor --watch --interval 5', description: 'Watch mode' },
    { command: 'swarmdo process monitor --components agents,memory,tasks', description: 'Monitor specific components' },
    { command: 'swarmdo process monitor --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const agentStorePath = resolve('.swarmdo/agents/store.json');
    ...
    try {
      const taskStorePath = resolve('.swarmdo/tasks/store.json');
    ...
    const metrics = {
      ...
      agents: {
        total: agentCount,
        _note: agentCount === 0 ? 'No agent store found at .swarmdo/agents/store.json' : null,
      },
      tasks: {
        ...taskCounts,
        _note: (taskCounts.running + taskCounts.queued + taskCounts.completed + taskCounts.failed) === 0
          ? 'No task store found at .swarmdo/tasks/store.json' : null,
      },
      memory: {
...
const workersCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo process workers --action list', description: 'List all workers' },
    { command: 'swarmdo process workers --action spawn --type task --count 3', description: 'Spawn task workers' },
    { command: 'swarmdo process workers --action kill --id worker-123', description: 'Kill specific worker' },
    { command: 'swarmdo process workers --action scale --type memory --count 5', description: 'Scale memory workers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const signalsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo process signals --target daemon --signal graceful-shutdown', description: 'Graceful shutdown' },
    { command: 'swarmdo process signals --target workers --signal pause', description: 'Pause workers' },
    { command: 'swarmdo process signals --target all --signal reload-config', description: 'Reload all configs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo process logs', description: 'Show recent logs' },
    { command: 'swarmdo process logs --source daemon --tail 100', description: 'Daemon logs' },
    { command: 'swarmdo process logs --follow --level error', description: 'Follow error logs' },
    { command: 'swarmdo process logs --since 1h --grep "error"', description: 'Search logs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Read actual log files from .swarmdo/logs/ if they exist
    const logsDir = resolve('.swarmdo/logs');
...
export const processCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo process daemon --action start', description: 'Start daemon' },
    { command: 'swarmdo process monitor --watch', description: 'Watch processes' },
    { command: 'swarmdo process workers --action list', description: 'List workers' },
    { command: 'swarmdo process logs --follow', description: 'Follow logs' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  swarmdo process daemon --action start');
    console.log('  swarmdo process monitor --watch');
    console.log('  swarmdo process workers --action spawn --type task --count 3');
    console.log('  swarmdo process logs --follow --level error');
