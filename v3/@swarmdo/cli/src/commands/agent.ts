...
function updateSwarmActivityMetrics(agentCountDelta: number): void {
  try {
    const metricsDir = path.join(process.cwd(), '.swarmdo', 'metrics');
...
const spawnCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent spawn --type coder --name bot-1', description: 'Spawn a coder agent' },
    { command: 'swarmdo agent spawn -t researcher --task "Research React 19"', description: 'Spawn researcher with task' }
  ],
...
const poolCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent pool --size 5', description: 'Set pool size' },
    { command: 'swarmdo agent pool --min 2 --max 15', description: 'Configure auto-scaling' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const result = await callMCPTool<{
...
const healthCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent health', description: 'Show all agents health' },
    { command: 'swarmdo agent health -i agent-001 -d', description: 'Detailed health for specific agent' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo agent logs -i agent-001', description: 'Show agent logs' },
    { command: 'swarmdo agent logs -i agent-001 -f', description: 'Follow agent logs' },
    { command: 'swarmdo agent logs -l error --since 1h', description: 'Show errors from last hour' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const agentId = ctx.args[0] || ctx.flags.id as string;
...
export const agentCommand: Command = {
  ...
  examples: [
    ...
    { command: 'swarmdo agent list', description: 'List all agents' },
    { command: 'swarmdo agent status agent-001', description: 'Show agent status' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: swarmdo agent <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "swarmdo agent <subcommand> --help" for subcommand help');
