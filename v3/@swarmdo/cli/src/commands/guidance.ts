...
/** Task intent categories matching @swarmdo/guidance TaskIntent */
type TaskIntent = 'bug-fix' | 'feature' | 'refactor' | 'security' | 'performance' | 'testing' | 'docs' | 'deployment' | 'architecture' | 'debug' | 'general';

/** Gate evaluation result matching @swarmdo/guidance GateResult */
...
const compileCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo guidance compile', description: 'Compile default CLAUDE.md' },
    { command: 'swarmdo guidance compile -r ./CLAUDE.md -l ./CLAUDE.local.md', description: 'Compile with local overlay' },
    { command: 'swarmdo guidance compile --json', description: 'Output compiled bundle as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { GuidanceCompiler } = await import('@swarmdo/guidance/compiler');
...
const retrieveCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo guidance retrieve -t "Fix SQL injection in user search"', description: 'Retrieve guidance for a security task' },
    { command: 'swarmdo guidance retrieve -t "Add unit tests" -n 3', description: 'Retrieve top 3 shards for testing' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { GuidanceCompiler } = await import('@swarmdo/guidance/compiler');
      const { ShardRetriever, HashEmbeddingProvider } = await import('@swarmdo/guidance/retriever');
...
const gatesCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo guidance gates -c "rm -rf /tmp"', description: 'Check if a command is destructive' },
    { command: 'swarmdo guidance gates --content "api_key=sk-abc123..."', description: 'Check content for secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { EnforcementGates } = await import('@swarmdo/guidance/gates');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const statusData = {
        ...
        dataDir: existsSync('./.swarmdo/guidance') ? 'exists' : 'not created',
      };
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo guidance optimize', description: 'Analyze current CLAUDE.md and show suggestions' },
    { command: 'swarmdo guidance optimize --apply', description: 'Apply optimizations to CLAUDE.md' },
    { command: 'swarmdo guidance optimize -s compact --apply', description: 'Optimize for compact context window' },
    { command: 'swarmdo guidance optimize --target-score 95', description: 'Optimize until score reaches 95' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const rootPath = ctx.flags.root as string || './CLAUDE.md';
    const localPath = ctx.flags.local as string | undefined;
    ...
    try {
      ...
      const { analyze, formatReport, optimizeForSize, formatBenchmark } = await import('@swarmdo/guidance/analyzer');
...
const abTestCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo guidance ab-test', description: 'Run default A/B test (no guidance vs ./CLAUDE.md)' },
    { command: 'swarmdo guidance ab-test -a old.md -b new.md', description: 'Compare two CLAUDE.md versions' },
    { command: 'swarmdo guidance ab-test --tasks custom-tasks.json', description: 'Run with custom test tasks' },
    { command: 'swarmdo guidance ab-test --json', description: 'Output full report as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { abBenchmark, getDefaultABTasks } = await import('@swarmdo/guidance/analyzer');
...
export const guidanceCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo guidance compile', description: 'Compile CLAUDE.md into policy bundle' },
    { command: 'swarmdo guidance retrieve -t "Fix auth bug"', description: 'Retrieve relevant guidance' },
    { command: 'swarmdo guidance gates -c "rm -rf /"', description: 'Check enforcement gates' },
    { command: 'swarmdo guidance status', description: 'Show control plane status' },
    { command: 'swarmdo guidance optimize', description: 'Analyze and optimize CLAUDE.md' },
    { command: 'swarmdo guidance ab-test', description: 'Run A/B behavioral comparison' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    ...
    output.writeln();
    output.writeln(output.dim('Use swarmdo guidance <subcommand> --help for details'));
