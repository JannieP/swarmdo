...
const startCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo swarm start -o "Build REST API" -s development', description: 'Start development swarm' },
    { command: 'swarmdo swarm start -o "Analyze codebase" --parallel', description: 'Parallel analysis' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // #2370: the old hint referenced the deprecated `swarmdo@v3alpha`
      // dist-tag which now resolves to a pre-rename package. Use the current
      // `swarmdo@latest` and force a fresh fetch with `-y` so npx doesn't pick
      // a stale local install.
      output.writeln(output.dim('  The MCP server may not be running. Start it with: claude mcp add swarmdo -- npx -y swarmdo@latest mcp start'));
    }
    ...
    output.writeln(output.dim(`  Monitor: swarmdo swarm status ${swarmId}`));
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!status.hasActiveSwarm) {
      ...
      output.writeln(output.dim('  npx @swarmdo/cli@latest swarm init'));
      output.writeln(output.dim('  npx @swarmdo/cli@latest swarm start'));
      output.writeln();
      return { success: true, data: status };
    }
...
export const swarmCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo swarm init --v3-mode', description: 'Initialize V3 swarm' },
    { command: 'swarmdo swarm start -o "Build API" -s development', description: 'Start development swarm' },
    { command: 'swarmdo swarm coordinate --agents 15', description: 'V3 coordination' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarm Coordination Commands'));
    output.writeln();
    output.writeln('Usage: swarmdo swarm <subcommand> [options]');
    output.writeln();
