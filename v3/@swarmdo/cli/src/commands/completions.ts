...
import { getLazyCommandNames } from './index.js';

// Source the top-level command list from the real command registry so shell
// completions never drift from the actual CLI surface (#129g). This was a
// hand-maintained array that omitted ~30 commands (init, start, status, mcp,
// migrate, usage, hud, route, verify, and the rest). 'help'/'version' are
// global pseudo-commands with no entry in the loader registry, so add them here.
const TOP_LEVEL_COMMANDS = Array.from(
  new Set([...getLazyCommandNames(), 'help', 'version'])
).sort();
...
complete -F _swarmdo_completions swarmdo
complete -F _swarmdo_completions npx\\ @swarmdo/cli@latest
`;
}

// Generate zsh completion script
function generateZshCompletion(): string {
  return `#compdef swarmdo
# swarmdo zsh completion
# Generated by swarmdo completions zsh
# Add to ~/.zfunc/_swarmdo or ~/.zsh/completions/_swarmdo
...
_swarmdo() {
    ...
    case $state in
        command)
            _describe -t commands 'swarmdo commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# swarmdo fish completion
# Generated by swarmdo completions fish
# Save to ~/.config/fish/completions/swarmdo.fish

# Disable file completion by default
complete -c swarmdo -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c swarmdo -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c swarmdo -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c swarmdo -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# swarmdo PowerShell completion
# Generated by swarmdo completions powershell
...
Register-ArgumentCompleter -Native -CommandName swarmdo -ScriptBlock {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'swarmdo completions bash > ~/.bash_completion.d/swarmdo', description: 'Install bash completions' },
    { command: 'swarmdo completions zsh > ~/.zfunc/_swarmdo', description: 'Install zsh completions' },
    { command: 'swarmdo completions fish > ~/.config/fish/completions/swarmdo.fish', description: 'Install fish completions' },
    { command: 'swarmdo completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  swarmdo completions bash > ~/.bash_completion.d/swarmdo'));
    output.writeln(output.dim('  source ~/.bash_completion.d/swarmdo'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  swarmdo completions zsh > ~/.zfunc/_swarmdo'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  swarmdo completions fish > ~/.config/fish/completions/swarmdo.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  swarmdo completions powershell >> $PROFILE'));
