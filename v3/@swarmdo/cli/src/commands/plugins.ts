...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: swarmdo-official)' },
  ],
  examples: [
    { command: 'swarmdo plugins list', description: 'List all plugins from registry' },
    { command: 'swarmdo plugins list --installed', description: 'List installed only' },
    { command: 'swarmdo plugins list --official', description: 'List official plugins' },
    { command: 'swarmdo plugins list --category security', description: 'List security plugins' },
  ],
    ...
    if (installedOnly) {
      ...
      try {
        ...
        if (installed.length === 0) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "swarmdo plugins list" to see available plugins'));
          output.writeln(output.dim('Run "swarmdo plugins install -n <plugin>" to install'));
          return { success: true };
        }
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'swarmdo plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "swarmdo plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'swarmdo plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins info -n @swarmdo/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'swarmdo plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const upgradeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins upgrade -n @swarmdo/neural', description: 'Upgrade to latest' },
    { command: 'swarmdo plugins upgrade -n @swarmdo/neural -v 3.1.0', description: 'Upgrade to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'swarmdo plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const rateCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins rate -n @swarmdo/embeddings -r 5', description: 'Rate 5 stars' },
    { command: 'swarmdo plugins rate -n my-plugin -r 4', description: 'Rate 4 stars' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo plugins list', description: 'List plugins from IPFS registry' },
    { command: 'swarmdo plugins search -q neural', description: 'Search for plugins' },
    { command: 'swarmdo plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'swarmdo plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Plugin System'));
    ...
    output.printList([
      '@swarmdo/neural              - Neural patterns and inference (WASM SIMD)',
      '@swarmdo/security            - Security scanning and CVE detection',
      '@swarmdo/embeddings          - Vector embeddings with hyperbolic support',
      '@swarmdo/claims              - Claims-based authorization',
      '@swarmdo/performance         - Performance profiling and benchmarks',
      '@swarmdo/plugin-gastown-bridge - Gas Town orchestrator integration (WASM-accelerated)',
    ]);
    output.writeln();
