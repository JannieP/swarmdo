...
// Precedence: --path > SWARMDO_DB_PATH env var > default root
const DB_PATH_OPTION = {
  name: 'path',
  description:
    'Override DB file path (also: SWARMDO_DB_PATH env var). ' +
    'Precedence: --path > SWARMDO_DB_PATH > SWARMDO_MEMORY_PATH/memory.db > cwd/.swarm/memory.db',
...
const storeCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo memory store -k "api/auth" -v "JWT implementation"', description: 'Store text' },
    { command: 'swarmdo memory store -k "pattern/singleton" --vector', description: 'Store vector' },
    { command: 'swarmdo memory store -k "pattern" -v "updated" --upsert', description: 'Update existing' }
  ],
...
const searchCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (buildHnsw) {
      ...
      try {
        ...
        if (index) {
          ...
          output.printWarning('HNSW index not available (install @swarmvector/core for acceleration)');
        }
        output.writeln();
      } catch (error) {
    ...
    try {
      ...
      // @swarmdo/memory don't expose it. Fall through to plain
      ...
      if (useSmart) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const memMod: any = await import('@swarmdo/memory');
        ...
        if (!smartSearchFn) {
          output.printWarning(
            'Smart search requested but smartSearch is not available on the installed @swarmdo/memory build (#1846). Falling back to standard semantic search.',
          );
        }
      }
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: swarmdo memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: swarmdo memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'swarmdo memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'swarmdo memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
      ...
      try {
        ...
        output.printTable({
          ...
          data: [
            ...
            {
              metric: 'HNSW Index',
              // swarmdo#1989 / #1987: `hnsw.entryCount` is in-process JS state
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'swarmdo memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'swarmdo memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const compressCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo memory compress', description: 'Balanced compression' },
    { command: 'swarmdo memory compress --quantize --bits 4', description: '4-bit quantization (32x reduction)' },
    { command: 'swarmdo memory compress -l max -t vectors', description: 'Max compression on vectors' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo memory init', description: 'Initialize hybrid backend with all features' },
    { command: 'swarmdo memory init -b agentdb', description: 'Initialize AgentDB backend' },
    { command: 'swarmdo memory init -p ./data/memory.db --force', description: 'Reinitialize at custom path' },
    { command: 'swarmdo memory init --verbose --verify', description: 'Initialize with full verification' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('swarmdo memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('swarmdo memory search -q "query"')}`,
        `Train patterns: ${output.highlight('swarmdo neural train -p coordination')}`,
        `View stats: ${output.highlight('swarmdo memory stats')}`
      ]);
...
export const memoryCommand: Command = {
  ...
  examples: [
    ...
    { command: 'swarmdo memory search -q "auth patterns"', description: 'Search memory' },
    { command: 'swarmdo memory stats', description: 'Show statistics' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Memory Management Commands'));
    output.writeln();
    output.writeln('Usage: swarmdo memory <subcommand> [options]');
    output.writeln();
