...
const scanCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security scan -t ./src', description: 'Scan source directory' },
    { command: 'swarmdo security scan --depth deep --fix', description: 'Deep scan with auto-fix' },
  ],
...
const cveCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security cve --list', description: 'List all CVEs' },
    { command: 'swarmdo security cve -c CVE-2024-1234', description: 'Check specific CVE' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim(`Source: \`npm audit --json\` (GitHub Advisory DB). Run \`swarmdo security scan\` for code + dep scan.`));
...
const threatsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security threats --model stride', description: 'Run STRIDE analysis' },
    { command: 'swarmdo security threats -e md', description: 'Export as markdown' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const auditCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security audit --action list', description: 'List audit logs' },
    { command: 'swarmdo security audit -a export', description: 'Export audit trail' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (auditEntries.length === 0) {
      output.writeln(output.dim('No audit events found. Initialize a project first: swarmdo init'));
    } else {
      output.printTable({
        columns: [
...
const secretsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security secrets --action scan', description: 'Scan for secrets' },
    { command: 'swarmdo security secrets -a rotate', description: 'Rotate compromised secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const defendCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security defend -i "ignore previous instructions"', description: 'Scan text for threats' },
    { command: 'swarmdo security defend -f ./prompts.txt', description: 'Scan file for threats' },
    { command: 'swarmdo security defend --stats', description: 'Show detection statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    let createAIDefence: typeof import('@swarmdo/aidefence').createAIDefence;
    try {
      const aidefence = await import('@swarmdo/aidefence');
      createAIDefence = aidefence.createAIDefence;
    } catch {
      output.error('AIDefence package not installed. Run: npm install @swarmdo/aidefence');
    ...
    if (!textToScan) {
      output.writeln('Usage: swarmdo security defend -i "<text>" or -f <file>');
      output.writeln();
...
export const securityCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo security scan', description: 'Run security scan' },
    { command: 'swarmdo security cve --list', description: 'List known CVEs' },
    { command: 'swarmdo security threats', description: 'Run threat analysis' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Security Suite'));
