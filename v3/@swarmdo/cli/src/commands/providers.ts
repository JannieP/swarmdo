...
const listCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers list', description: 'List all providers' },
    { command: 'swarmdo providers list -t embedding', description: 'List embedding providers' },
  ],
...
const configureCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers configure -p openai -k sk-...', description: 'Set OpenAI key' },
    { command: 'swarmdo providers configure -p anthropic -m claude-3.5-sonnet', description: 'Set default model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
...
const testCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers test -p openai', description: 'Test OpenAI connection' },
    { command: 'swarmdo providers test --all', description: 'Test all providers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const provider = (ctx.flags.provider as string) || (ctx.args && ctx.args[0]) || '';
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers models', description: 'List all models' },
    { command: 'swarmdo providers models -p anthropic', description: 'List Anthropic models' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
const usageCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers usage', description: 'View all usage' },
    { command: 'swarmdo providers usage -t 30d', description: 'View 30-day usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const providersCommand: Command = {
  ...
  examples: [
    { command: 'swarmdo providers list', description: 'List all providers' },
    { command: 'swarmdo providers configure -p openai', description: 'Configure OpenAI' },
    { command: 'swarmdo providers test --all', description: 'Test all providers' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarmdo Provider Management'));
