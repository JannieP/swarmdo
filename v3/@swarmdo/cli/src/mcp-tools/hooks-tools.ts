...
// #1773 item 4 — moe-router migrated to @swarmdo/neural
let moeRouter: Awaited<ReturnType<typeof import('@swarmdo/neural').getMoERouter>> | null = null;
async function getMoERouter() {
  if (!moeRouter) {
    try {
      const { getMoERouter: getMoE } = await import('@swarmdo/neural');
...
let semanticRouter: import('../swarmvector/semantic-router.js').SemanticRouter | null = null;
...
const ROUTING_OUTCOMES_PATH = join(resolve('.'), '.swarmdo/routing-outcomes.json');
...
async function getSemanticRouter() {
  ...
  // STEP 1: Try native VectorDb from @swarmvector/router (HNSW-backed)
  ...
  try {
    ...
    const router = require('@swarmvector/router');
  ...
  try {
    const { SemanticRouter } = await import('../swarmvector/semantic-router.js');
...
// #1773 item 4 — flash-attention migrated to @swarmdo/neural
let flashAttention: Awaited<ReturnType<typeof import('@swarmdo/neural').getFlashAttention>> | null = null;
async function getFlashAttention() {
  if (!flashAttention) {
    try {
      const { getFlashAttention: getFlash } = await import('@swarmdo/neural');
...
let loraAdapter: Awaited<ReturnType<typeof import('../swarmvector/lora-adapter.js').getLoRAAdapter>> | null = null;
async function getLoRAAdapter() {
  if (!loraAdapter) {
    try {
      const { getLoRAAdapter: getLora } = await import('../swarmvector/lora-adapter.js');
...
const MEMORY_DIR = '.swarmdo/memory';
...
export const hooksPostEdit: MCPTool = {
  ...
  description: 'Record editing outcome for learning Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksPostCommand: MCPTool = {
  ...
  description: 'Record command execution outcome Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksMetrics: MCPTool = {
  name: 'hooks_metrics',
  description: 'View learning metrics dashboard Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksList: MCPTool = {
  name: 'hooks_list',
  description: 'List all registered hooks Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
export const hooksPostTask: MCPTool = {
  ...
  description: 'Record task completion for learning Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
      taskId: { type: 'string', description: 'Task identifier' },
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      const dataDir = join(getProjectCwd(), '.swarmdo', 'data');
...
export const hooksExplain: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      const outcomesPath = join(resolve('.'), '.swarmdo/routing-outcomes.json');
...
export const hooksSessionEnd: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    let insightCount = 0;
    try {
      const insightsPath = resolve(join('.swarmdo', 'data', 'pending-insights.jsonl'));
...
export const hooksSessionRestore: MCPTool = {
  ...
  description: 'Restore a previous session Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksNotify: MCPTool = {
  name: 'hooks_notify',
  description: 'Send cross-agent notification Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      components: {
        ...
        swarmllmCoordinator: await (async () => {
          try {
            const { getIntelligenceStats } = await import('../memory/intelligence.js');
            const s = getIntelligenceStats();
            return { status: s._swarmllmBackend || 'unavailable', trajectories: s._swarmllmTrajectories || 0, note: s._swarmllmBackend === 'active' ? 'SonaCoordinator forwarding trajectories' : '@swarmvector/swarmllm not loaded' };
        ...
        contrastiveTrainer: await (async () => {
          try {
            ...
            return { status: s._contrastiveTrainer !== 'unavailable' ? 'active' : 'unavailable', details: s._contrastiveTrainer, note: s._contrastiveTrainer !== 'unavailable' ? 'Agent embedding learning active' : '@swarmvector/swarmllm not loaded' };
        ...
        trainingPipeline: await (async () => {
          try {
            ...
            return { status: s?._trainingBackend || 'unavailable', note: s?._trainingBackend === 'swarmllm' ? 'Checkpoint save/load via swarmllm' : 'JS fallback' };
        ...
        graphDatabase: await (async () => {
          try {
            ...
            return { status: gs.backend, totalNodes: gs.totalNodes, totalEdges: gs.totalEdges, avgDegree: gs.avgDegree, note: gs.backend === 'graph-node' ? 'Native Rust graph with hyperedges and k-hop queries' : '@swarmvector/graph-node not loaded' };
      ...
      implementationStatus: {
        working: [
          ...
          'flash-attention', 'lora-adapter', 'swarmllm-coordinator', 'contrastive-trainer', 'training-pipeline', 'graph-database'
        ],
        partial: [],
        notImplemented: [],
      },
...
export const hooksIntelligenceReset: MCPTool = {
  ...
  description: 'Reset intelligence learning state Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
  handler: async () => {
    ...
    const dataFiles = [
      join(cwd, '.swarmdo', 'data', 'auto-memory-store.json'),
      join(cwd, '.swarmdo', 'data', 'graph-state.json'),
      join(cwd, '.swarmdo', 'data', 'ranked-context.json'),
    ];
    ...
    const neuralDir = join(cwd, '.swarmdo', 'neural');
...
export const hooksTrajectoryEnd: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    if (trajectory && persistResult.success) {
      ...
      // Trigger swarmllm background learning after trajectory end
      try {
...
export const hooksPatternSearch: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      searchTimeMs: 0,
      backend: 'unavailable',
      note: 'Real vector search not available. Initialize memory database with: swarmdo memory init',
    };
  },
};
...
export const hooksIntelligenceStats: MCPTool = {
  ...
  description: 'Get SwarmVector intelligence layer statistics Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    // swarmllm native backend stats
    let swarmllmStats = { coordinator: 'unavailable' as string, trajectories: 0, contrastiveTrainer: 'unavailable' as string | object, trainingBackend: 'unavailable' as string, graphDatabase: { backend: 'unavailable', totalNodes: 0, totalEdges: 0 } as Record<string, unknown> };
    try {
      const { getIntelligenceStats } = await import('../memory/intelligence.js');
      const iStats = getIntelligenceStats();
      swarmllmStats.coordinator = iStats._swarmllmBackend || 'unavailable';
      swarmllmStats.trajectories = iStats._swarmllmTrajectories || 0;
    } catch { /* not initialized */ }
    try {
      const { getSONAStats: getSONA } = await import('../memory/sona-optimizer.js');
      const sStats = await getSONA();
      swarmllmStats.contrastiveTrainer = sStats._contrastiveTrainer || 'unavailable';
    } catch { /* not initialized */ }
    if (lora) {
      const ls = lora.getStats();
      swarmllmStats.trainingBackend = ls._trainingBackend || 'unavailable';
    }
    try {
      const { getGraphStats } = await import('../swarmvector/graph-backend.js');
      const gs = await getGraphStats();
      swarmllmStats.graphDatabase = { backend: gs.backend, totalNodes: gs.totalNodes, totalEdges: gs.totalEdges, avgDegree: gs.avgDegree };
    } catch { /* not available */ }
    ...
    let routerStats: ReturnType<typeof import('../swarmvector/model-router.js').getModelRouterStats> | null = null;
    ...
    const stats = {
      ...
      // `swarmdo neural router cost-savings --since 7d` shape).
      costSavings,
...
export const hooksIntelligenceLearn: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      ...
      // Run background learning (swarmllm) pass as well — best-effort
      try {
...
export const hooksIntelligenceAttention: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    async function getQueryEmbedding(text: string, dims: number): Promise<{ embedding: Float32Array; source: 'onnx' | 'hash-fallback' }> {
      // Try ONNX via @swarmdo/embeddings
      try {
        const embeddingsModule = await import('@swarmdo/embeddings').catch(() => null);
    ...
    return {
      ...
      stats: {
        ...
        _note: implementation === 'none' ? 'No attention backend available. Install @swarmvector/attention for real computation.' : undefined,
        ...(embeddingSource === 'hash-fallback' && implementation !== 'none'
          ? { _embeddingNote: 'Query embeddings are hash-based (not semantic). Install @swarmdo/embeddings for real ONNX embeddings.' }
          : {}),
      },
      implementation,
    };
...
export const hooksWorkerDispatch: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const cwd = getProjectCwd();
    const pidFile = join(cwd, '.swarmdo', 'daemon.pid');
    ...
    if (!daemonAlive) {
      ...
      note = 'No worker daemon detected. Run `swarmdo daemon start` to enable real worker execution. The dispatch was recorded in-process but no actual work will run.';
    } else if (background) {
      ...
      const queueDir = join(cwd, '.swarmdo', 'daemon-queue');
      ...
      try {
        ...
        note = `Daemon detected (pid ${daemonPid}) but queue write to ${queuePath} failed: ${(err as Error).message}. Worker recorded in-process only; use \`swarmdo daemon trigger -w ${trigger}\` to run synchronously.`;
      }
      if (queueWritten) {
        reportedStatus = 'queued';
        note = `Worker queued for daemon (pid ${daemonPid}) at ${queuePath}. Daemon polls every 5s; processed entries move to .swarmdo/daemon-queue/.processed/. Poll hooks_worker-status until status === "completed".`;
      } else {
...
export const hooksModelRoute: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      // swarmvector/model-router.ts — not a neural network. See #2329.
...
export const hooksModelStats: MCPTool = {
  ...
  description: 'Get model routing statistics Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
export const hooksCodemod: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const { applyCodemod, isDeterministicCodemod } = await import('../swarmvector/codemods/engine.js');
...
export const hooksWorkerCancel: MCPTool = {
  ...
  description: 'Cancel a running worker Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Swarmdo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
// #1916: the `swarmdo hooks teammate-idle` / `swarmdo hooks task-completed` CLI
