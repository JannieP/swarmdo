/**
 * SwarmVector LLM WASM MCP Tools
 *
 * Exposes @swarmvector/swarmllm-wasm operations via MCP protocol.
 * All tools gracefully degrade when the WASM package is not installed.
 */

import type { MCPTool } from './types.js';
import { validateIdentifier, validateText } from './validate-input.js';
import type { ChatMessage } from '../swarmvector/swarmllm-wasm.js';

// #2086 — every swarmllm_* MCP handler that touches the WASM runtime calls
...
// MCP callers — they don't need a separate `swarmllm_init` tool. `_wasmReady`
// inside `initSwarmllmWasm` short-circuits on the second+ call, so the cost
// after the first invocation is one boolean check.
//
// `swarmllm_status` deliberately uses `loadSwarmllmWasmModule()` (no init) so a
// caller diagnosing why nothing works gets `initialized=false` instead of
// an error from a failed init.
async function loadSwarmllmWasm() {
  const mod = await loadSwarmllmWasmModule();
  await mod.initSwarmllmWasm();
  return mod;
}

async function loadSwarmllmWasmModule() {
  return import('../swarmvector/swarmllm-wasm.js');
}

export const swarmllmWasmTools: MCPTool[] = [
  {
    name: 'swarmllm_status',
    description: 'Get swarmllm-wasm availability and initialization status. Use when sending every prompt to the Anthropic API is wrong because you need local inference — air-gapped environments, MicroLoRA-fine-tuned per-task adapters, or sub-cent per-call cost. For general Claude work native Task is the right call.',
    inputSchema: { type: 'object' as const, properties: {} },
    handler: async () => {
      try {
        const mod = await loadSwarmllmWasmModule();
        const wasmStatus = await mod.getSwarmllmStatus();

        // Also include native swarmllm CJS backend status (ADR-086)
        ...
        try {
          ...
          nativeBackend = {
            available: iStats._swarmllmBackend === 'active',
            coordinator: iStats._swarmllmBackend || 'unavailable',
            trajectories: iStats._swarmllmTrajectories || 0,
  ...
  },
  {
    ...
    handler: async (args: Record<string, unknown>) => {
      try {
        const mod = await loadSwarmllmWasm();
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        routerId: { type: 'string', description: 'HNSW router ID from swarmllm_hnsw_create' },
  ...
  {
    name: 'swarmllm_microlora_create',
...
const hnswRouters = new Map<string, Awaited<ReturnType<typeof import('../swarmvector/swarmllm-wasm.js').createHnswRouter>>>();
const sonaInstances = new Map<string, Awaited<ReturnType<typeof import('../swarmvector/swarmllm-wasm.js').createSonaInstant>>>();
const loraInstances = new Map<string, Awaited<ReturnType<typeof import('../swarmvector/swarmllm-wasm.js').createMicroLora>>>();
