...
export const configTools: MCPTool[] = [
  {
    name: 'config_get',
    description: 'Get configuration value Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'config_set',
    description: 'Set configuration value Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'config_list',
    description: 'List configuration values Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'config_reset',
    description: 'Reset configuration to defaults Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'config_export',
    description: 'Export configuration to JSON Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'config_import',
    description: 'Import configuration from JSON Use when native settings.json edits are wrong because the values need to be read by the Swarmdo runtime (daemon, MCP server, neural router) — those load via the config_* path, not by re-reading settings.json. For .gitignore / .editorconfig style files, native Edit is fine.',
    category: 'config',
    inputSchema: {
      type: 'object',
      properties: {
