 ...
 *
 * @module @swarmdo/cli/mcp-tools/progress
 */
...
// CLI is at v3/@swarmdo/cli, so go up 2 levels from cli to get to v3
const CLI_ROOT = join(__dirname, '../../..');
const SWARMDO_DIR = join(CLI_ROOT, '..'); // @swarmdo directory
const V3_DIR = join(SWARMDO_DIR, '..'); // v3 directory
...
async function calculateProgress(): Promise<V3ProgressMetrics> {
  ...
  // Count V3 modules
  const modulesDir = join(V3_DIR, '@swarmdo');
  ...
  const commandsIndexPath = join(V3_DIR, '@swarmdo/cli/src/commands/index.ts');
  ...
  const toolsIndexPath = join(V3_DIR, '@swarmdo/cli/src/mcp-tools/index.ts');
  ...
  const hooksPath = join(V3_DIR, '@swarmdo/cli/src/commands/hooks.ts');
...
async function syncProgress(): Promise<V3ProgressMetrics> {
  ...
  // Persist to file
  const metricsDir = join(PROJECT_ROOT, '.swarmdo/metrics');
