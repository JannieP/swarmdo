...
function saveSession(session: SessionRecord): void {
  ...
  // only when SWARMDO_ENCRYPT_AT_REST is set; otherwise the legacy
...
export const sessionTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      // while the auto-session writer (swarmdo daemon) writes
  ...
  {
    // #1916: `swarmdo session current` referenced an unregistered
  ...
  {
    // #1916: `swarmdo session export <id> -o <file>` referenced an unregistered
