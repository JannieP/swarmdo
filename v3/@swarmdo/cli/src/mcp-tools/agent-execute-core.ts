 ...
 * SWARMDO_PROVIDER=ollama is explicitly set. Response shape is normalized
...
export async function callAnthropicMessages(input: AnthropicCallInput): Promise<AnthropicCallResult> {
  const explicitProvider = (process.env.SWARMDO_PROVIDER || '').toLowerCase();
  ...
  // explicit SWARMDO_PROVIDER=openrouter OR presence of OPENROUTER_API_KEY
...
async function callOllamaCompat(
  ...
  // (e.g. http://swarmultra:11434, http://localhost:11434) instead of
...
export async function executeAgentTask(input: AgentExecuteInput): Promise<AgentExecuteResult> {
  ...
  const systemPrompt = input.systemPrompt ||
    `You are a ${agent.agentType} agent operating as part of a Swarmdo swarm. ` +
  ...
  // bounded by SWARMDO_ROUTER_FALLBACK_MAX_RETRIES (default 1) so
  ...
  try {
    const { recordModelOutcome, recordModelOutcomeByModelId } = await import('../swarmvector/model-router.js');
    ...
    // The trajectory recorder (SWARMDO_ROUTER_TRAJECTORY=1) writes
    ...
    if (process.env.SWARMDO_ROUTER_TRAJECTORY === '1') {
      try {
        const { recordTrajectoryOutcome } = await import('../swarmvector/router-trajectory.js');
  ...
  return {
    ...
    ...(noProvider && {
      remediation:
        ...
        'Or set SWARMDO_PROVIDER=openrouter|ollama to force a specific provider.',
    }),
  };
}
