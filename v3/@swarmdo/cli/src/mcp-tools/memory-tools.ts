...
const LEGACY_MEMORY_DIR = '.swarmdo/memory';
...
/**
 * Check if legacy JSON store exists in old .swarmdo/memory/ location
 */
...
async function ensureInitialized(): Promise<void> {
  try {
    ...
    // Migrate legacy JSON data if exists (from old .swarmdo/memory/ location)
    if (hasLegacyStore()) {
...
export const memoryTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        // The export landed in @swarmdo/memory@>3.0.0-alpha.14 — older
        ...
        if (input.smart) {
          ...
          let memMod: any;
          try {
            memMod = await import('@swarmdo/memory');
          } catch (err) {
            smartFallbackReason = `@swarmdo/memory failed to load: ${(err as Error).message}`;
          }
          ...
          smartFallbackReason = smartFallbackReason
            ?? 'smartSearch is not exported by the installed @swarmdo/memory build (likely a release lag — see #1846). Falling back to standard semantic search.';
        }
...

  {
    ...
    handler: async (input) => {
      ...
      //   3. env var SWARMDO_MEMORY_SEARCH_NAMESPACES (CSV)
      ...
      } else if (process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES) {
        namespaces = process.env.SWARMDO_MEMORY_SEARCH_NAMESPACES.split(',').map(s => s.trim()).filter(Boolean);
        namespaceSource = 'env';
      } else {
  ...
  {
    // #1916: `swarmdo status memory` (the detailed view) referenced an
  ...
  {
    // #1916: `swarmdo memory cleanup` referenced an unregistered `memory_cleanup`
  ...
  {
    // #1916: `swarmdo memory compress` referenced an unregistered tool. The
  ...
  {
    // #1916: `swarmdo memory export -o <file>` referenced an unregistered tool.
