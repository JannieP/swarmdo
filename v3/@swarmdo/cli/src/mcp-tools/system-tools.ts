  ...
  try {
    ...
    for (const depth of ['../..', '../../..']) {
      ...
      if (existsSync(pkgPath)) {
        ...
        if (pkg.name?.includes('swarmdo') || pkg.name === 'swarmdo') {
...
export const systemTools: MCPTool[] = [
  {
    name: 'system_status',
    description: 'Get overall system status Use when native Bash is wrong because you need Swarmdo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_metrics',
    description: 'Get system metrics and performance data Use when native Bash is wrong because you need Swarmdo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_health',
    description: 'Perform system health check Use when native Bash is wrong because you need Swarmdo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input) => {
      ...
      // not just the legacy `.swarmdo/memory/*` triple.
      {
        const t0 = performance.now();
        const memoryCandidates = [
          join(projectCwd, '.swarmdo', 'memory', 'store.json'),       // legacy
          join(projectCwd, '.swarmdo', 'memory', 'agentdb.sqlite'),
          join(projectCwd, '.swarmdo', 'memory', 'store.rvf'),
          join(projectCwd, '.swarmdo', 'memory', 'swarmdo.db'),   // sql.js
          join(projectCwd, '.swarm', 'memory.db'),                        // swarm
          join(projectCwd, 'vector.db'),                                // swarmvector
      ...
      // #1843: also accept YAML config (.swarmdo/config.yaml) which
      // the rest of v3 treats as canonical; previous code only counted
      // .json variants and reported `degraded` when YAML was used.
      {
        const t0 = performance.now();
        const configCandidates = [
          join(projectCwd, '.swarmdo', 'config.json'),
          join(projectCwd, '.swarmdo', 'config.yaml'),
          join(projectCwd, '.swarmdo', 'config.yml'),
          join(projectCwd, 'swarmdo.config.json'),
          join(projectCwd, 'swarmdo.config.yaml'),
          join(projectCwd, 'swarmdo.config.yml'),
        ];
      ...
      if (input.deep) {
        ...
        {
          const t0 = performance.now();
          const coordPath = join(projectCwd, '.swarmdo', 'coordination', 'store.json');
  ...
  {
    name: 'system_info',
    description: 'Get system information Use when native Bash is wrong because you need Swarmdo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async () => {
      ...
      try {
        const { getFlashAttention } = await import('@swarmdo/neural');
  ...
  {
    name: 'system_reset',
    description: 'Reset system state Use when native Bash is wrong because you need Swarmdo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'mcp_status',
    description: 'Get MCP server status, including stdio mode detection Use when native Claude Code MCP status is wrong because you need Swarmdo-side server detail — tool counts per namespace, transport stats, MCP handshake errors. For just "is MCP up?", `claude mcp list` is fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {},
    ...
    handler: async () => {
      ...
      const isStdio = !process.stdin.isTTY;
      const transport = process.env.SWARMDO_MCP_TRANSPORT || (isStdio ? 'stdio' : 'http');
      const port = parseInt(process.env.SWARMDO_MCP_PORT || '3000', 10);
      ...
      const host = process.env.SWARMDO_MCP_HOST || 'localhost';
      try {
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const storePath = join(getProjectCwd(), '.swarmdo', 'tasks', 'store.json');
  ...
  {
    // #1916: `swarmdo start` referenced an unregistered `mcp_start` tool. MCP
    ...
    handler: async (input) => {
      ...
      return {
        ...
        port: typeof input.port === 'number' ? input.port : (parseInt(process.env.SWARMDO_MCP_PORT || '0', 10) || null),
        transport: (input.transport as string) || process.env.SWARMDO_MCP_TRANSPORT || (isStdio ? 'stdio' : 'in-process'),
        startedAt: new Date().toISOString(),
        note: 'MCP tools run in-process via the CLI; no separate server process was started. Use `swarmdo mcp start` for a standalone server.',
      };
    },
  },
  {
    ...
    handler: async () => {
      return {
        stopped: false,
        note: 'no separate MCP server process; nothing to stop. The in-process toolset goes away when the CLI process exits. Use `swarmdo mcp stop` for a standalone server.',
      };
    },
  },
];
