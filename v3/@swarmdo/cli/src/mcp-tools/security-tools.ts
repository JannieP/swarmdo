...
type AIDefenceInstance = ReturnType<typeof import('@swarmdo/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@swarmdo/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @swarmdo/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[swarmdo] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    if (instance) {
      aidefenceInstance = instance;
      console.error(`[swarmdo] ${packageName} loaded after install (resolver path)`);
      return instance;
    }
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[swarmdo] ${packageName} loaded after install (file:// path)`);
    return instance;
  } catch (retryError) {
    throw new Error(
      ...
      `(common with global installs of \`swarmdo\`). Recovery options:\n` +
      `  1. Run \`npm install --save @swarmdo/aidefence\` in your project's working directory.\n` +
      `  2. Or run \`npx swarmdo@latest mcp start\` from a directory whose node_modules contains the package.\n` +
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      ...
      const defender = await getAIDefence();
      const { isSafe } = await import('@swarmdo/aidefence');
