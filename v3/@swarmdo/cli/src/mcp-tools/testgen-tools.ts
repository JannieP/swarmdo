 ...
 *
 * Exposes `swarmdo-testgen` plugin scripts as first-class MCP tools so
 ...
 * Zero static `swarmdo-testgen/*` imports. All script invocation stays
 ...
 *
 * @module @swarmdo/cli/mcp-tools/testgen
 */
...
/**
 * Locate the swarmdo-testgen plugin's scripts directory.
 *
...
function locateTestgenScripts(): string | null {
  ...
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'swarmdo-testgen', 'scripts'));
    candidates.push(join(p, '..', 'plugins', 'swarmdo-testgen', 'scripts'));
    p = dirname(p);
  }
  const cwd = getProjectCwd();
  candidates.push(join(cwd, 'plugins', 'swarmdo-testgen', 'scripts'));
  candidates.push(join(cwd, 'node_modules', '@swarmdo', 'cli', 'plugins', 'swarmdo-testgen', 'scripts'));
...
export const testgenTools: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        maxAttempts: { type: 'number', description: '1..5 (swarmdo cap). Each attempt gets budgetUsd/maxAttempts of the total budget.', default: 1 },
