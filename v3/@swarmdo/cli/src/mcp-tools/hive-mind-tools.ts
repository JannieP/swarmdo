...
function loadAgentStore(): { agents: Record<string, unknown> } {
  const storePath = join(getProjectCwd(), '.swarmdo', 'agents.json');
  try {
...
function saveAgentStore(store: { agents: Record<string, unknown> }): void {
  const storeDir = join(getProjectCwd(), '.swarmdo');
...
export const hiveMindTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      const taskStorePath = join(getProjectCwd(), '.swarmdo', 'tasks', 'store.json');
  ...
  {
    // #1916: `swarmdo hive-mind optimize-memory` referenced an unregistered
