 ...
 * - Uses @swarmdo/embeddings for REAL ML embeddings when available
 ...
 *
 * Note: For production neural features, use @swarmdo/neural module
 */
...
try {
  ...
  // NOTE: swarmvector's embed() returns `{embedding, dimension, timeMs}` — we
  // unwrap to plain number[] for the shared interface.
  const rv = await import('swarmvector').catch(() => null) as any;
  if (rv?.embed && typeof rv.embed === 'function' && rv.isOnnxAvailable?.()) {
    try {
      ...
      // Handle both shapes: swarmvector wraps as {embedding, dimension, timeMs};
      ...
      if (probeVec && (Array.isArray(probeVec) || (probeVec as ArrayLike<number>).length > 0)) {
        ...
        embeddingServiceName = 'swarmvector@0.2.27 (bundled all-MiniLM-L6-v2)';
      }
    } catch {
      // swarmvector embed failed at runtime; fall through to next tier
    }
  }
  ...
  // Tier 2: @swarmdo/embeddings with agentic-flow provider
  if (!realEmbeddings) {
  ...
  // Tier 3: @swarmdo/embeddings with ONNX provider
  if (!realEmbeddings) {
    const embeddingsModule = await import('@swarmdo/embeddings').catch(() => null);
    if (embeddingsModule?.createEmbeddingService) {
      try {
...
export function getNeuralStoreStats(): {
  ...
  return {
    ...
    source: '.swarmdo/neural/patterns.json (loadNeuralStore)',
  };
}
    ...
    handler: async (input) => {
      ...
      return {
        ...
        ...(embeddingServiceName === 'hash-fallback' || embeddingServiceName === 'none' ? {
          platformNote: 'ONNX embeddings not available — using hash-based fallback. Install @swarmdo/embeddings and run "embeddings init --download" for semantic search.',
        } : {}),
      };
    },
  },
      ...
      return {
        ...
        features: {
          ...
          flashAttention: await (async () => {
            try {
              // #1773 item 4 — flash-attention now lives in @swarmdo/neural
