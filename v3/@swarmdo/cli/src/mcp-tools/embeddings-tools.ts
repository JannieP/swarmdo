...
export const embeddingsTools: MCPTool[] = [
  {
    ...
    description: 'Initialize the ONNX embedding subsystem (hyperbolic support). Use when meaning matters beyond keyword match — native Grep finds only exact strings. Pair with memory_store / agentdb_pattern-search; for literal symbols, native Grep is faster.',
    category: 'embeddings',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'Neural substrate operations (SwarmVector integration) Use when text similarity matters beyond keyword match — native Grep finds exact strings, embeddings find meaning. Pair with memory_store / agentdb_pattern-search to land the vector against your knowledge base. For literal symbol search, native Grep is faster.',
    category: 'embeddings',
    inputSchema: {
      type: 'object',
      properties: {
      ...
      switch (action) {
        case 'init':
          ...
          return {
            ...
            message: 'Neural substrate initialized with SwarmVector integration',
          };
          ...
          try {
            ...
            return {
              ...
              neural: {
                ...
                swarmvector: config.neural.swarmvector || { enabled: false },
...

  {
    ...
    handler: async () => {
      ...
      // ADR-093 F5: distinguish "@swarmvector/core installed" from "wired into
      // the embedding pipeline". Previously this collapsed both into a
      // single `swarmvector: boolean` field, which gave callers no way to
      // tell whether re-running embeddings_init would help (#1698 partial
      // regression on the MCP boundary).
      let swarmvectorAvailable = false;
      let swarmvectorVersion: string | undefined;
      try {
        const mod = await import('@swarmvector/core');
        swarmvectorAvailable = !!(mod as Record<string, unknown>);
        try {
          // Best-effort: many packages expose a `version` constant
          swarmvectorVersion = (mod as { version?: string }).version;
        } catch { /* ignore */ }
      } catch { /* not installed */ }

      const swarmvectorEnabled = config.neural.swarmvector?.enabled ?? false;
      ...
      return {
        ...
        config: {
          ...
          neural: {
            ...
            swarmvector: swarmvectorEnabled,
            ...
            swarmvectorStatus: {
              ...
              enabled: swarmvectorEnabled,
              version: swarmvectorVersion,
            },
          },
        },
        paths: {
...

  {
    ...
    description: 'Build a RaBitQ 1-bit quantized index (32× compression) from stored embeddings; Hamming pre-filter then exact rerank. Use when meaning matters beyond keyword match — native Grep finds only exact strings. Pair with memory_store / agentdb_pattern-search; for literal symbols, native Grep is faster.',
    category: 'embeddings',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'Search the RaBitQ quantized index (fast Hamming scan); returns candidate IDs for reranking. Use when meaning matters beyond keyword match — native Grep finds only exact strings. Pair with memory_store / agentdb_pattern-search; for literal symbols, native Grep is faster.',
    category: 'embeddings',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'RaBitQ quantized index status — availability, vector count, compression ratio. Use when meaning matters beyond keyword match — native Grep finds only exact strings. Pair with memory_store / agentdb_pattern-search; for literal symbols, native Grep is faster.',
    category: 'embeddings',
    inputSchema: {
      type: 'object',
      properties: {},
