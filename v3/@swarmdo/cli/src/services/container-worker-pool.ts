...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  ...
  statePath: '.swarmdo/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.swarmdo`,
   ...
   *
   * #2371: the old command spawned `npx swarmdo@v3alpha daemon trigger`,
   ...
   * locally-installed stale `swarmdo` without fetching the published
   * version. Workers were running pre-autopilot / pre-browser builds.
   * Use the current `swarmdo@latest` and force a fresh resolution with `-y`.
   */
  private buildWorkerCommand(options: ContainerExecutionOptions): string[] {
    return [
      'npx', '-y', 'swarmdo@latest',
      'daemon', 'trigger',
