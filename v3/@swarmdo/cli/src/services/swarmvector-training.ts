/**
 * SwarmVector Training Service
 * Real WASM-accelerated neural training using @swarmvector packages
 *
 * Features:
...
} from '@swarmvector/learning-wasm';

// @swarmvector/attention types — use any since the NAPI exports vary across versions
...
// SONA Engine type (from @swarmvector/sona)
...
/**
 * Initialize the SwarmVector training system.
 * Attempts to load @swarmvector/learning-wasm for WASM-accelerated training.
...
export async function initializeTraining(config: TrainingConfig = {}): Promise<{
  ...
  try {
    ...
    console.warn(`[swarmvector] WASM backend unavailable (${reason}), using JS fallback`);
  ...
  try {
    const attention: any = await importWithInterop('@swarmvector/attention');
    ...
    // @swarmvector/attention not available - attention features skipped
    const reason = attentionError instanceof Error ? attentionError.message : String(attentionError);
    console.warn(`[swarmvector] @swarmvector/attention unavailable (${reason}), attention features disabled`);
  }

  // --- SONA (optional, backward compatible) ---
  if (config.useSona !== false) {
    try {
      const sona = await importWithInterop('@swarmvector/sona');
