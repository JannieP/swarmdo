...
export interface DaemonConfig {
  ...
  // #2356 (carry-forward from pacphi/swarmdo-machine-ref token-leak findings):
...
// pacphi/swarmdo-machine-ref kit's proven value) heals a forgotten daemon within
// half a day; set SWARMDO_DAEMON_TTL_SECS=0 (or `--ttl 0`) to opt out. Idle
...
export class WorkerDaemon extends EventEmitter {
  ...
  // path, `swarmdo daemon trigger -w <worker>` runs before headlessAvailable
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const claudeFlowDir = join(projectRoot, '.swarmdo');

    // Read daemon config from .swarmdo/config.json (Layer B)
    ...
    this.config = {
      ...
      // env (SWARMDO_DAEMON_TTL_SECS) > built-in default. readEnvSecsAsMs folds
      // env-or-default and honors an explicit 0 (disable).
      ttlMs: config?.ttlMs ?? fileConfig.ttlMs ?? readEnvSecsAsMs('SWARMDO_DAEMON_TTL_SECS', DEFAULT_DAEMON_TTL_MS),
      idleShutdownMs: config?.idleShutdownMs ?? fileConfig.idleShutdownMs ?? readEnvSecsAsMs('SWARMDO_DAEMON_IDLE_SECS', DEFAULT_DAEMON_IDLE_SHUTDOWN_MS),
  ...
  /**
   * Read daemon-specific config from .swarmdo/config.{json,yaml,yml}.
  ...
  /**
   * Append a structured crash record to .swarmdo/logs/crash.log.
   * Inspectable by hand or via `swarmdo daemon status` follow-ups.
   */
  ...
  private get childrenFile(): string {
    return join(this.projectRoot, '.swarmdo', 'daemon-children.json');
  }
  ...
  private writeChildrenSnapshot(): void {
    ...
    try {
      ...
      const dir = join(this.projectRoot, '.swarmdo');
   ...
   * previous crashed lifetime. Reads `.swarmdo/daemon-children.json`,
  ...
  private get pidFile(): string {
    return join(this.projectRoot, '.swarmdo', 'daemon.pid');
  }
   ...
   * Each entry is a JSON file at `.swarmdo/daemon-queue/<id>.json`
   * with `{ workerId, trigger, context, enqueuedAt }`. We move processed
   * files to `.swarmdo/daemon-queue/.processed/` so the daemon never
   * re-runs the same dispatch and operators can inspect history.
   */
  private async processDispatchQueue(): Promise<void> {
    if (!this.running) return;
    const queueDir = join(this.projectRoot, '.swarmdo', 'daemon-queue');
    ...
    if (isHeadlessWorker(workerConfig.type) && this.headlessAvailable && this.headlessExecutor) {
      try {
        ...
        if (!ok) {
          ...
          // parsedOutput that lived only in `.swarmdo/logs/headless/*` and
          // never reached `.swarmdo/metrics/<name>.json` — `memory stats`
   ...
   * that lived only in `.swarmdo/logs/headless/*_result.log` and was
   * invisible to `npx swarmdo memory stats` or the metrics consumers.
   *
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const metricsFile = join(this.projectRoot, '.swarmdo', 'metrics', 'codebase-map.json');
    const metricsDir = join(this.projectRoot, '.swarmdo', 'metrics');
    ...
    const map = {
      ...
      structure: {
        ...
        hasClaudeFlow: existsSync(join(this.projectRoot, '.swarmdo')),
      },
      scannedAt: Date.now(),
    };
  ...
  private async runAuditWorkerLocal(): Promise<unknown> {
    ...
    const auditFile = join(this.projectRoot, '.swarmdo', 'metrics', 'security-audit.json');
    const metricsDir = join(this.projectRoot, '.swarmdo', 'metrics');
  ...
  private async runTestGapsWorkerLocal(): Promise<unknown> {
    ...
    const testGapsFile = join(this.projectRoot, '.swarmdo', 'metrics', 'test-gaps.json');
    const metricsDir = join(this.projectRoot, '.swarmdo', 'metrics');
  ...
  private async runBenchmarkWorkerLocal(): Promise<unknown> {
    const benchmarkFile = join(this.projectRoot, '.swarmdo', 'metrics', 'benchmark.json');
    const metricsDir = join(this.projectRoot, '.swarmdo', 'metrics');
