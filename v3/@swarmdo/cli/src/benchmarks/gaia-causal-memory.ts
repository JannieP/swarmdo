 ...
 *
 * This is one of swarmdo's 6 architectural primitives distinguishing it
 * from HAL: HAL is stateless across runs; swarmdo accumulates causal memory.
 *
 * Storage: JSONL file at ~/.cache/swarmdo/gaia/causal-edges.jsonl
 *   - Simple, portable, no runtime dependency on AgentDB
 *   - Production upgrade path: switch to AgentDB causal-edge MCP controller
 *     (`mcp__swarmdo__agentdb_causal-edge`) for cross-session persistence
...
export interface CausalMemoryOptions {
   ...
   * Default: ~/.cache/swarmdo/gaia/causal-edges.jsonl
   */
  storePath?: string;
  /**
   ...
   * hash-based implementation; reserved for future SwarmVector upgrade.
   * Default: 0.7
   */
...
const DEFAULT_STORE_SUFFIX = path.join('.cache', 'swarmdo', 'gaia', 'causal-edges.jsonl');
const DEFAULT_MAX_EDGES = 5;

/** Resolve the JSONL store path from options, defaulting to ~/.cache/swarmdo/…  */
 ...
 *
 * Future (v2, embedding-based): replace with SwarmVector cosine similarity so
