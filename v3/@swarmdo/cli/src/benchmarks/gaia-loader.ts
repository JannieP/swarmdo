 ...
 * validation split, caches it under ~/.cache/swarmdo/gaia/, and exposes
...
export interface LoadGaiaOptions {
  ...
  smokeOnly?: boolean;
  /** Override the cache directory (default: ~/.cache/swarmdo/gaia). */
  cacheDir?: string;
}
...
function defaultCacheDir(): string {
  return path.join(os.homedir(), '.cache', 'swarmdo', 'gaia');
}
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'swarmdo-gaia-loader/1.0',
        },
      },
      (res) => {
