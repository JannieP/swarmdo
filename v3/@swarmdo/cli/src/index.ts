...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    this.name = options.name || 'swarmdo';
    this.description = options.description || 'Swarmdo V3 - AI Agent Orchestration Platform';
  ...
  async run(args: string[] = process.argv.slice(2)): Promise<void> {
    try {
      ...
      // `flags.noColor`. Honor BOTH so `swarmdo --no-color` actually disables
  ...
  private async loadConfig(configPath?: string): Promise<V3Config | undefined> {
    try {
      ...
      const { loadConfig: loadSystemConfig } = await import('@swarmdo/shared');
