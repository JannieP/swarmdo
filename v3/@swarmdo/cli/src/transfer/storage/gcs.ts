 ...
 *
 * @module @swarmdo/cli/transfer/storage/gcs
 * @version 3.0.0
 */
...
export function getGCSConfig(): GCSConfig | null {
  ...
  return {
    ...
    prefix: process.env.GCS_PREFIX || 'swarmdo-patterns',
  };
}
...
export async function uploadToGCS(
  ...
  const tempFile = path.join(tempDir, `swarmdo-upload-${Date.now()}.json`);
...
export async function downloadFromGCS(
  ...
  const tempDir = process.env.TMPDIR || '/tmp';
  const tempFile = path.join(tempDir, `swarmdo-download-${Date.now()}.json`);
