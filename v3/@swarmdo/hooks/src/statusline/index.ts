...
export class StatuslineGenerator {
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.swarmdo', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.swarmdo', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.swarmdo', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    const patternsPath = join(this.projectRoot, '.swarmdo', 'learning', 'patterns.db');
    try {
