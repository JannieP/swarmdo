 ...
 *
 * Usage:
 *   npx @swarmdo/hooks session-context
 *   npx @swarmdo/hooks user-prompt "Fix authentication bug"
 *   npx @swarmdo/hooks pre-edit "/path/to/file.ts"
 *   npx @swarmdo/hooks route "Implement caching layer"
 *
 * @module @swarmdo/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@swarmdo/hooks - V3 Guidance System CLI

Usage: npx @swarmdo/hooks <command> [args]
...
Examples:
  ...
  npx @swarmdo/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @swarmdo/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @swarmdo/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @swarmdo/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @swarmdo/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @swarmdo/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @swarmdo/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
