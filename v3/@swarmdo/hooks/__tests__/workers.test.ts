...
const TEST_PROJECT_ROOT = path.join(os.tmpdir(), 'swarmdo-test-' + Date.now());

async function setupTestDir(): Promise<void> {
  await fs.mkdir(path.join(TEST_PROJECT_ROOT, '.swarmdo', 'metrics'), { recursive: true });
  await fs.mkdir(path.join(TEST_PROJECT_ROOT, 'v3', '@swarmdo', 'hooks', 'src'), { recursive: true });
}
...
describe('Statusline Integration', () => {
  ...
  it('should export statusline to file', async () => {
    ...
    const statuslinePath = path.join(TEST_PROJECT_ROOT, '.swarmdo', 'metrics', 'statusline.json');
...
describe('Persistence', () => {
  ...
  it('should save state to disk', async () => {
    ...
    const statePath = path.join(TEST_PROJECT_ROOT, '.swarmdo', 'metrics', 'workers-state.json');
