/**
 * AgentAdapter - Bridge between Swarmdo and agentic-flow Agents
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Swarmdo v3 DDD agent architecture
...
/**
 * AgentAdapter - Bridges Swarmdo and agentic-flow agents
 *
 ...
 *
 * // Convert agentic-flow agent to Swarmdo agent
 * const { agent, success } = adapter.fromAgenticFlow(agenticFlowAgent);
 *
 * // Create Swarmdo agent with agentic-flow delegation
...
export class AgentAdapter extends EventEmitter {
  ...
  /**
   * Convert agentic-flow agent to Swarmdo AgenticFlowAgent
   *
   * This method creates a Swarmdo agent wrapper around an existing
  ...
  fromAgenticFlow(
    ...
    try {
      // Map agentic-flow config to Swarmdo config
  ...
  /**
   * Create a Swarmdo agent with agentic-flow delegation
   *
  ...
  /**
   * Map agentic-flow agent type to Swarmdo AgentType
   */
  ...
  /**
   * Sync status between Swarmdo and agentic-flow agents
   */
  private syncStatus(
    agent: AgenticFlowAgent,
    agenticFlowAgent: AgenticFlowAgent_External
  ): void {
    // Map agentic-flow status to Swarmdo status
