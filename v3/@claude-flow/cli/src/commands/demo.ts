...
function persistResults(results: DemoResults): string {
  const dir = join(getProjectCwd(), '.claude-flow');
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  const path = join(dir, 'bench-results.json');
...
export const demoCommand: Command = {
  name: 'demo',
  description: 'Run a 30-second capability tour — proves HNSW, Ed25519, agent_run, and the embedding backend really work on this machine. Writes .claude-flow/bench-results.json so `ruflo doctor` can surface measured numbers.',
  options: [
    {
  ...
  examples: [
    { command: 'claude-flow demo', description: 'Full capability tour (~30s)' },
    { command: 'claude-flow demo --skip-llm', description: 'Skip LLM step — no provider key needed' },
    { command: 'claude-flow demo --json', description: 'Emit JSON only (for CI smoke tests)' },
  ],
