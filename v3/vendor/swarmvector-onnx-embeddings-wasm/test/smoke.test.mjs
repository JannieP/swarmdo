...
// swarmvector-onnx-embeddings-wasm — renamed fork of
// swarmvector-onnx-embeddings-wasm@0.1.2 (agentic-flow optional dep).
//
...
test('ships the wasm binary', () => {
  assert.ok(existsSync(join(pkgDir, 'swarmvector_onnx_embeddings_wasm_bg.wasm')));
});
...
test('main entry fails under ESM exactly like upstream (consumers fall back)', async () => {
  await assert.rejects(
    () => import('../swarmvector_onnx_embeddings_wasm.js'),
