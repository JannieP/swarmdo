/**
 * Benchmark CLI for swarmdo-swarm
...
class BenchmarkCLI {
  ...
  async run(args) {
    ...
    console.log('🚀 swarmdo-swarm Performance Benchmark\n');
    ...
    try {
      ...
      if (outputFile) {
        ...
        const defaultPath = path.join(process.cwd(), '.swarmdo-swarm', 'benchmarks', `benchmark-${Date.now()}.json`);
  ...
  async compare(args) {
    ...
    if (!file1 || !file2) {
      ...
      console.log('Usage: swarmdo-swarm benchmark compare file1.json file2.json');
      process.exit(1);
    }
