...
class WASMLoader {
  ...
  async loadModule() {
    ...
    try {
      const wasmJsPath = path.join(this.wasmPath, 'swarmdo_swarm_wasm.js');
    ...
    const moduleFile = this.useSIMD ? 'swarmdo_swarm_simd.wasm' : 'swarmdo_swarm_wasm_bg.wasm';
...
class SwarmdoSwarm {
  ...
  static async initialize(options = {}) {
    ...
    const wasmJsPath = path.join(loader.wasmPath, 'swarmdo_swarm_wasm.js');
    const bindings = await import(path.resolve(wasmJsPath));

    // Initialize WASM module with file buffer for Node.js
    if (bindings.default) {
      const wasmPath = path.join(loader.wasmPath, 'swarmdo_swarm_wasm_bg.wasm');
    ...
    return new SwarmdoSwarm(bindings, options);
  }
  ...
  static getRuntimeFeatures() {
    if (!wasmInstance) {
      throw new Error('SwarmdoSwarm not initialized. Call SwarmdoSwarm.initialize() first.');
    }
  ...
  async createSwarm(config) {
    try {
      const swarm = new this._wasmSwarm.SwarmdoSwarm(config);
