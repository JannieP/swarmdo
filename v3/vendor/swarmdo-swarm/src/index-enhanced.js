/**
 * Enhanced SwarmdoSwarm Main Class
...
class SwarmdoSwarm {
   ...
   */
  destroy() {
    console.log('🧹 Cleaning up SwarmdoSwarm instance...');
  ...
  static async initialize(options = {}) {
    ...
    // Register SwarmdoSwarm factory if not already registered
    if (!container.has('SwarmdoSwarm')) {
      container.register('SwarmdoSwarm', () => new SwarmdoSwarm(), {
        singleton: true,
        lazy: false,
      });
    }
    ...
    const instance = container.get('SwarmdoSwarm');
    ...
    if (instance.isInitialized) {
      if (debug) {
        console.log('[DEBUG] SwarmdoSwarm already initialized through container');
      }
      return instance;
    }

    console.log('🧠 Initializing swarmdo-swarm with WASM capabilities...');
    ...
    try {
      ...
      console.log('✅ swarmdo-swarm initialized successfully');
      ...
      console.error('❌ Failed to initialize swarmdo-swarm:', error);
      throw error;
    }
  }
  ...
  async detectFeatures(useSIMD = true) {
    try {
      ...
      // Detect SIMD support
      if (useSIMD) {
        this.features.simd_support = SwarmdoSwarm.detectSIMDSupport();
      }
  ...
  async createSwarm(config) {
    ...
    let wasmSwarm;
    if (coreModule.exports && coreModule.exports.SwarmdoSwarm) {
      try {
        wasmSwarm = new coreModule.exports.SwarmdoSwarm();
  ...
  static getRuntimeFeatures() {
    return {
      ...
      simd: SwarmdoSwarm.detectSIMDSupport(),
  ...
  detectSIMDSupport() {
    return SwarmdoSwarm.detectSIMDSupport();
  }
}

// Enhanced Swarm wrapper class
class Swarm {
  constructor(id, wasmInstance, swarmdoSwarmInstance) {
    this.id = id;
  ...
  async spawn(config) {
    ...
    if (enableNeuralNetwork && this.swarmdoSwarm.features.neural_networks) {
      await this.swarmdoSwarm.wasmLoader.loadModule('neural');
    }
    ...
    if (this.swarmdoSwarm.persistence && !id) {
      try {
        this.swarmdoSwarm.persistence.createAgent({
          id: agentId,
          swarmId: this.id,
          name: result.name,
          type,
  ...
  async orchestrate(taskConfig) {
    ...
    if (this.swarmdoSwarm.persistence) {
      await this.swarmdoSwarm.persistence.createTask({
        id: taskId,
        swarmId: this.id,
        description,
        priority,
  ...
  async terminate() {
    ...
    this.swarmdoSwarm.activeSwarms.delete(this.id);
  }
}
...
export { SwarmdoSwarm, Swarm, Agent, Task, DAAService, daaService };