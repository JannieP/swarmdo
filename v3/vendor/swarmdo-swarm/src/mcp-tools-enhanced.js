...
class EnhancedMCPTools {
  constructor(swarmdoSwarmInstance = null) {
    this.swarmdoSwarm = swarmdoSwarmInstance;
    ...
    try {
      ...
      for (const swarmData of swarmsArray) {
        try {
          ...
          const swarm = await this.swarmdoSwarm.createSwarm({
            id: swarmData.id,
            name: swarmData.name,
  ...
  async swarm_init(params) {
    ...
    try {
      ...
      // Ensure we have a SwarmdoSwarm instance (but don't re-initialize)
      if (!this.swarmdoSwarm) {
        try {
          this.logger.debug('SwarmdoSwarm not initialized, initializing now');
          await this.initialize();
          this.logger.debug('SwarmdoSwarm initialized successfully');
        } catch (error) {
          this.logger.error('Failed to initialize SwarmdoSwarm', { error });
          throw ErrorFactory.createError('wasm',
            'Failed to initialize SwarmdoSwarm WASM module',
      ...
      if (enableForecasting && this.swarmdoSwarm.features.forecasting) {
        await this.swarmdoSwarm.wasmLoader.loadModule('forecasting');
      }
      ...
      const result = {
        ...
        maxAgents,
        features: {
          cognitive_diversity: enableCognitiveDiversity && this.swarmdoSwarm.features.cognitive_diversity,
          neural_networks: enableNeuralAgents && this.swarmdoSwarm.features.neural_networks,
          forecasting: enableForecasting && this.swarmdoSwarm.features.forecasting,
          simd_support: this.swarmdoSwarm.features.simd_support,
        },
        created: new Date().toISOString(),
        performance: {
          initialization_time_ms: performance.now() - startTime,
          memory_usage_mb: this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
        },
      };
      ...
      } else if (!(error instanceof ValidationError || error instanceof SwarmdoSwarmError)) {
  ...
  async swarm_status(params) {
    ...
    try {
      ...
      if (swarmId) {
        ...
        status.wasm_metrics = {
          memory_usage_mb: this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          loaded_modules: this.swarmdoSwarm.wasmLoader.getModuleStatus(),
          features: this.swarmdoSwarm.features,
        };
      ...
      const globalMetrics = await this.swarmdoSwarm.getGlobalMetrics();
      const allSwarms = await this.swarmdoSwarm.getAllSwarms();
      ...
      const result = {
        ...
        runtime_info: {
          features: this.swarmdoSwarm.features,
          wasm_modules: this.swarmdoSwarm.wasmLoader.getModuleStatus(),
  ...
  async benchmark_run(params) {
    ...
    try {
      ...
      if (type === 'all' || type === 'neural') {
        if (includeNeuralBenchmarks && this.swarmdoSwarm.features.neural_networks) {
      ...
      const result = {
        ...
        environment: {
          features: this.swarmdoSwarm.features,
          memory_usage_mb: this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          runtime_features: SwarmdoSwarm.getRuntimeFeatures(),
        },
        performance: {
  ...
  async features_detect(params) {
    ...
    try {
      ...
      const features = {
        runtime: SwarmdoSwarm.getRuntimeFeatures(),
        wasm: {
          modules_loaded: this.swarmdoSwarm.wasmLoader.getModuleStatus(),
          total_memory_mb: this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          simd_support: this.swarmdoSwarm.features.simd_support,
        },
        swarmdo_swarm: this.swarmdoSwarm.features,
        neural_networks: {
          available: this.swarmdoSwarm.features.neural_networks,
          activation_functions: this.swarmdoSwarm.features.neural_networks ? 18 : 0,
          training_algorithms: this.swarmdoSwarm.features.neural_networks ? 5 : 0,
          cascade_correlation: this.swarmdoSwarm.features.neural_networks,
        },
        forecasting: {
          available: this.swarmdoSwarm.features.forecasting,
          models_available: this.swarmdoSwarm.features.forecasting ? 27 : 0,
          ensemble_methods: this.swarmdoSwarm.features.forecasting,
        },
        cognitive_diversity: {
          available: this.swarmdoSwarm.features.cognitive_diversity,
          patterns_available: this.swarmdoSwarm.features.cognitive_diversity ? 5 : 0,
          pattern_optimization: this.swarmdoSwarm.features.cognitive_diversity,
        },
      };
  ...
  async memory_usage(params) {
    ...
    try {
      ...
      const wasmMemory = this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage();
      const jsMemory = SwarmdoSwarm.getMemoryUsage();
      ...
      if (detail === 'detailed') {
        ...
        const moduleStatus = this.swarmdoSwarm.wasmLoader.getModuleStatus();
  ...
  async neural_status(params) {
    ...
    try {
      ...
      if (!this.swarmdoSwarm.features.neural_networks) {
        return {
          available: false,
      ...
      const result = {
        ...
        simd_acceleration: this.swarmdoSwarm.features.simd_support,
  ...
  async neural_train(params) {
    ...
    try {
      ...
      for (let i = 1; i <= iterations; i++) {
        ...
        if (this.swarmdoSwarm.wasmLoader.modules.get('core')?.neural_train) {
          try {
            this.swarmdoSwarm.wasmLoader.modules.get('core').neural_train({
      ...
      try {
        await this.persistence.updateNeuralNetwork(neuralNetwork.id, {
          ...
          weights: {
            ...
            values: this.swarmdoSwarm.wasmLoader.modules.get('core')?.get_neural_weights?.(neuralNetwork.id) || {},
          },
  ...
  async runWasmBenchmarks(iterations) {
    ...
    for (let i = 0; i < iterations; i++) {
      try {
        ...
        const coreModule = await this.swarmdoSwarm.wasmLoader.loadModule('core');
  ...
  async swarm_monitor(params) {
    ...
    try {
      ...
      monitoringData.system_metrics = {
        ...
        wasm_memory_usage_mb: this.swarmdoSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
        system_uptime_ms: Date.now() - (this.systemStartTime || Date.now()),
        features_available: Object.keys(this.swarmdoSwarm.features).filter(f => this.swarmdoSwarm.features[f]).length,
      };
