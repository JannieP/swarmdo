/**
 * Neural Network CLI for swarmdo-swarm
...
class NeuralCLI {
  ...
  async train(args) {
    ...
    try {
      ...
      const outputDir = path.join(process.cwd(), '.swarmdo-swarm', 'neural');
  ...
  async loadPersistenceInfo() {
    const neuralDir = path.join(process.cwd(), '.swarmdo-swarm', 'neural');
    const modelDetails = {};
    let totalSessions = 0;
    let savedModels = 0;
