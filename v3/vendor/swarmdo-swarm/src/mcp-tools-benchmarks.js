 ...
 * Contains all benchmark and testing functionality for swarmdo-swarm MCP tools
 */
...
export class MCPBenchmarks {
  constructor(swarmdoSwarm, persistence) {
    this.swarmdoSwarm = swarmdoSwarm;
  ...
  async runBenchmarks(type = 'all', iterations = 10) {
    ...
    const result = {
      ...
      environment: {
        ...
        memory_usage_mb: this.swarmdoSwarm?.wasmLoader?.getTotalMemoryUsage() / (1024 * 1024) || 0,
        runtime_features: this.swarmdoSwarm?.getRuntimeFeatures ? this.swarmdoSwarm.getRuntimeFeatures() : {},
      },
