/**
 * Claude Code Hooks Implementation for swarmdo-swarm
...
class SwarmdoSwarmHooks {
  ...
  async preEditHook(args) {
    ...
    if (!swarmStatus.initialized) {
      return {
        continue: false,
        reason: 'Swarm not initialized - run mcp__swarmdo-swarm__swarm_init first',
  ...
  async notificationHook(args) {
    ...
    if (sendTelemetry && process.env.SWARMDO_SWARM_TELEMETRY_ENABLED === 'true') {
  ...
  async mcpSwarmInitializedHook(args) {
    ...
    if (persistConfig) {
      const configDir = path.join(process.cwd(), '.swarmdo-swarm');
  ...
  async mcpAgentSpawnedHook(args) {
    ...
    if (updateRoster) {
      ...
      // Persist roster
      const rosterPath = path.join(process.cwd(), '.swarmdo-swarm', 'agent-roster.json');
  ...
  async mcpNeuralTrainedHook(args) {
    ...
    // Save neural weights
    if (saveWeights) {
      const weightsDir = path.join(process.cwd(), '.swarmdo-swarm', 'neural-weights');
  ...
  async agentCompleteHook(args) {
    ...
    try {
      ...
      let reportPath = null;
      if (generateReport) {
        const reportDir = path.join(process.cwd(), '.swarmdo-swarm', 'agent-reports');
...
---
*Generated by swarmdo-swarm agent coordination system*
`;
...
🤖 Generated by swarmdo-swarm agent coordination
Co-Authored-By: ${agentName} <agent@swarmdo-swarm.ai>`;

            // Pass the message as an argv element so no shell parsing can act
            // on its contents (an 'EOF' line could otherwise break out of the
            // heredoc). execFileSync uses no shell.
            ...
            if (pushToGithub && process.env.SWARMDO_SWARM_AUTO_PUSH === 'true') {
  ...
  async sessionRestoreHook(args) {
    ...
    try {
      const sessionDir = path.join(process.cwd(), '.swarmdo-swarm');
  ...
  async checkSwarmStatus() {
    try {
      ...
      const statusFile = path.join(process.cwd(), '.swarmdo-swarm', 'status.json');
  ...
  generateSessionSummary() {
    ...
    return `# swarmdo-swarm Session Summary
...
`;
  ...
  async updateKnowledgeBase(type, data) {
    const kbPath = path.join(process.cwd(), '.swarmdo-swarm', 'knowledge-base.json');
  ...
  async getSwarmStatus() {
    try {
      const statusPath = path.join(process.cwd(), '.swarmdo-swarm', 'status.json');
  ...
  sendTelemetry(event, data) {
    ...
    const telemetryPath = path.join(process.cwd(), '.swarmdo-swarm', 'telemetry.jsonl');
...
const hooksInstance = new SwarmdoSwarmHooks();
