...
\`\`\`json
{
  "command": "npx swarmdo-swarm hook pre-task --auto-spawn-agents --optimize-topology"
}
\`\`\``,
      },
...
## Monitoring
\`\`\`
Tool: mcp__swarmdo-swarm__swarm_monitor
...
\`\`\`
Tool: mcp__swarmdo-swarm__task_results
...
npx swarmdo-swarm hook session-end --export-metrics
...
\`\`\`
Tool: mcp__swarmdo-swarm__neural_status
Result: {
  "patterns": {
    "convergent": 0.92,
    "divergent": 0.87,
...
\`\`\`json
{
  "hooks": [{
    "matcher": "^Task$",
    "command": "npx swarmdo-swarm hook pre-task --auto-spawn-agents"
  }]
}
\`\`\`
...
\`\`\`json
{
  "PostToolUse": [{
    "matcher": "^Bash$",
    "command": "npx swarmdo-swarm hook post-bash --exit-code '\${tool.result.exitCode}' --auto-recover"
  }]
}
\`\`\`
...
npx swarmdo-swarm hook session-restore --session-id "sess-123"
\`\`\`
...
rm .swarmdo-swarm/session-*.json

# Disable memory
export SWARMDO_SWARM_MEMORY_PERSIST=false
\`\`\`
