...
class ClaudeIntegrationCore {
  ...
  /**
     * Add swarmdo-swarm MCP server to Claude Code
     */
  async addMcpServer() {
    ...
    try {
      // Add swarmdo-swarm MCP server using stdio (no port needed)
      const mcpCommand = 'claude mcp add swarmdo-swarm npx swarmdo-swarm mcp start';
      execSync(mcpCommand, { stdio: 'inherit', cwd: this.workingDir });
      return { success: true, message: 'Added swarmdo-swarm MCP server to Claude Code (stdio)' };
    } catch (error) {
