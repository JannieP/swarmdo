...
class WasmModuleLoader {
  constructor() {
    ...
    this.moduleManifest = {
      ...
      swarm: {
        path: './wasm/swarmdo-swarm-orchestration.wasm',
        size: 768 * 1024, // 768KB
        priority: 'high',
        dependencies: ['core'],
      ...
      persistence: {
        path: './wasm/swarmdo-swarm-persistence.wasm',
        size: 256 * 1024, // 256KB
        priority: 'high',
        dependencies: ['core'],
  ...
  async loadCoreModule() {
    ...
    try {
      ...
      const wasmJsUrl = new URL('../wasm/swarmdo_swarm_wasm.js', import.meta.url).href;
      ...
      if (bindings.default && typeof window === 'undefined') {
        const wasmPath = path.join(this.baseDir, 'wasm', 'swarmdo_swarm_wasm_bg.wasm');
        try {
