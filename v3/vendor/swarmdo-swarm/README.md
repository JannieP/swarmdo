...
**What if every task, every file, every function could truly think?** Just for a moment. No LLM required. That's what swarmdo-swarm makes real.
...
```bash
npx swarmdo-swarm@1.0.19 init --claude
```

swarmdo-swarm lets you spin up ultra-lightweight custom neural networks that exist just long enough to solve the problem. Tiny purpose-built brains dedicated to solving very specific challenges.
...
Get started with swarmdo-swarm in under 2 minutes:

```bash
# Try instantly with npx
npx swarmdo-swarm init mesh 5
npx swarmdo-swarm spawn researcher "AI Research Agent"
npx swarmdo-swarm orchestrate "Research the latest advances in neural architecture search"

# Use Claude Code hooks for automated coordination
npx swarmdo-swarm hook pre-task --description "Your task description"
npx swarmdo-swarm hook post-task --task-id "task-123" --analyze-performance true
```

Or use programmatically:

```javascript
import { SwarmdoSwarm } from 'swarmdo-swarm';

// Initialize with cognitive diversity
const swarm = await SwarmdoSwarm.initialize({
  topology: 'mesh',
...
npm install swarmdo-swarm

# Global CLI installation (recommended for servers)
npm install -g swarmdo-swarm

# Development installation
npm install swarmdo-swarm --save-dev
```

### 🤖 MCP (Model Context Protocol) Installation

swarmdo-swarm is available in the **official MCP Registry**! Add it to your AI clients:
...
```json
{
  "mcpServers": {
    "swarmdo-swarm": {
      "command": "npx",
      "args": ["swarmdo-swarm@1.0.19", "mcp", "start"],
      "env": {
        "SWARMDO_SWARM_LOG_LEVEL": "info",
        "SWARMDO_SWARM_ENABLE_SIMD": "true"
      }
    }
  }
}
...
```bash
# Start MCP server
npx swarmdo-swarm@1.0.19 mcp start
...
**Important**: swarmdo-swarm requires WebAssembly support. Ensure your environment meets these requirements:
...
npx swarmdo-swarm --help
npx swarmdo-swarm init --claude
npx swarmdo-swarm init mesh 10
npx swarmdo-swarm benchmark --test swe-bench

# Instant MCP server for Claude Code
npx swarmdo-swarm mcp start --port 3000

# Remote server deployment
ssh user@remote-server 'npx swarmdo-swarm init hierarchical 20'
```
...
```bash
# Install from source
cargo install swarmdo-swarm-cli
...
swarmdo-swarm implements cognitive diversity through specialized agent archetypes:
...
```javascript
const { SwarmdoSwarm } = require('swarmdo-swarm');
...
class AIProjectOrchestrator {
  ...
  async initialize(config: AIProjectConfig): Promise<void> {
    ...
    this.swarm = await SwarmdoSwarm.initialize(swarmConfig);
  }
...
├──────────────────────────────────────────┤
│         swarmdo-swarm Core (Rust)           │ ← Agent Logic & Orchestration
├──────────────────────────────────────────┤
...
swarmdo-swarm provides native integration with Claude Code through the Model Context Protocol (MCP):
...
# Check server status
npx swarmdo-swarm mcp status

# List available tools
npx swarmdo-swarm mcp tools
```
...
Add swarmdo-swarm to your Claude MCP configuration:
...
| Benchmark | swarmdo-swarm | Claude 3.7 Sonnet | GPT-4 | Improvement |
|-----------|-----------|-------------------|-------|-------------|
...
npx swarmdo-swarm benchmark --full --include-swe-bench

# Specific performance tests
npx swarmdo-swarm benchmark --test agent-spawn
npx swarmdo-swarm benchmark --test task-throughput  
npx swarmdo-swarm benchmark --test memory-usage
npx swarmdo-swarm benchmark --test wasm-performance
npx swarmdo-swarm benchmark --test swe-bench-solve-rate

# Model comparison
npx swarmdo-swarm benchmark --compare lstm,tcn,nbeats,claude-optimizer

# Cost analysis
npx swarmdo-swarm benchmark --test cost-efficiency --baseline claude-3.7-sonnet

# Custom benchmark
npx swarmdo-swarm benchmark --config ./custom-bench.json
```
...
npx swarmdo-swarm benchmark --test swe-bench --instances 100

# Performance comparison
npx swarmdo-swarm benchmark --compare-with claude-3.7-sonnet

# Token efficiency analysis
npx swarmdo-swarm benchmark --test token-efficiency --tasks 1000

# Multi-agent coordination test
npx swarmdo-swarm benchmark --test coordination --agents 5-50
```
...
const adaptiveSwarm = await SwarmdoSwarm.initialize({
  topology: 'adaptive',
  adaptationRules: {
...
const persistentSwarm = await SwarmdoSwarm.initialize({
  persistence: {
    backend: 'sqlite',
    path: './swarm-memory.db',
...
const scalableSwarm = await SwarmdoSwarm.initialize({
  scaling: {
    minAgents: 3,
    maxAgents: 50,
...
swarmdo-swarm provides comprehensive hooks for Claude Code operations:
...
export SWARMDO_SWARM_AUTO_COMMIT=true
export SWARMDO_SWARM_GENERATE_REPORTS=true

# Agent work is automatically committed
npx swarmdo-swarm orchestrate "Implement user authentication"
...
```json
{
  "hooks": {
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "npx swarmdo-swarm hook agent-complete --agent '${tool.params.description}' --commit-to-git true"
      }]
    }]
  }
}
...
class SwarmdoSwarm {
  // Static methods
  static initialize(config?: SwarmConfig): Promise<SwarmdoSwarm>;
...
const haSwarm = await SwarmdoSwarm.initialize({
  deployment: {
    mode: 'distributed',
...
const secureSwarm = await SwarmdoSwarm.initialize({
  security: {
...
```typescript
// Advanced analytics
const analyticsSwarm = await SwarmdoSwarm.initialize({
  analytics: {
...
```bash
# Unit tests
cargo test -p swarmdo-swarm-core
cargo test -p swarmdo-swarm-agents
...
const researchSwarm = await SwarmdoSwarm.initialize({
  topology: 'hierarchical',
...
const devSwarm = await SwarmdoSwarm.initialize({
  topology: 'agile_team',
  methodology: 'scrum'
});
...
const biSwarm = await SwarmdoSwarm.initialize({
...
const eduSwarm = await SwarmdoSwarm.initialize({
...
|---------|-------------|---------|
| `init <topology> [max-agents]` | Initialize swarm | `npx swarmdo-swarm init mesh 10` |
| `spawn <type> [name]` | Create agent | `npx swarmdo-swarm spawn researcher "AI Researcher"` |
| `orchestrate <task>` | Execute task | `npx swarmdo-swarm orchestrate "Build REST API"` |
| `status` | Show swarm state | `npx swarmdo-swarm status` |
| `monitor` | Real-time monitoring | `npx swarmdo-swarm monitor` |

### Advanced Commands

| Command | Description | Example |
|---------|-------------|---------|
| `cluster create <name>` | Create agent cluster | `npx swarmdo-swarm cluster create research-team` |
| `workflow run <file>` | Execute workflow | `npx swarmdo-swarm workflow run ./ai-project.yml` |
| `memory store <key> <data>` | Store persistent data | `npx swarmdo-swarm memory store project-spec "API requirements..."` |
| `benchmark [test]` | Run performance tests | `npx swarmdo-swarm benchmark --test throughput` |
| `export <format> <file>` | Export swarm data | `npx swarmdo-swarm export json ./swarm-state.json` |

### MCP Commands

| Command | Description | Example |
|---------|-------------|---------|
| `mcp start [--port]` | Start MCP server | `npx swarmdo-swarm mcp start --port 3000` |
| `mcp status` | Check MCP server | `npx swarmdo-swarm mcp status` |
| `mcp tools` | List MCP tools | `npx swarmdo-swarm mcp tools` |
...
Create `swarmdo-swarm.config.json`:
...
swarmdo-swarm is **fully compatible with remote servers** using npx:

```bash
# ✅ Works on any remote server with Node.js 14+
ssh user@remote-server 'npx swarmdo-swarm init mesh 10'

# ✅ Start MCP server remotely
ssh user@remote-server 'npx swarmdo-swarm mcp start --port 3000 &'

# ✅ Run benchmarks on remote hardware
ssh user@remote-server 'npx swarmdo-swarm benchmark --test swe-bench'

# ✅ Deploy with screen/tmux for persistence
ssh user@remote-server 'screen -S swarmdo-swarm -d -m npx swarmdo-swarm mcp start'
```
...
docker run -d -p 3000:3000 --name swarmdo-swarm \
...
kubectl run swarmdo-swarm --image=node:18-alpine \
  --port=3000 \
  --command -- npx swarmdo-swarm mcp start --port 3000

# PM2 process management
pm2 start 'npx swarmdo-swarm mcp start --port 3000' --name swarmdo-swarm
```
...
npx swarmdo-swarm --version  # Should show version without errors
npx swarmdo-swarm features   # Lists available features

# If you see "Invalid or unexpected token" error (v1.0.5 bug - fixed in v1.0.6)
npm update swarmdo-swarm@latest  # Update to v1.0.6+

# Force clean reinstall
npm cache clean --force
npm uninstall -g swarmdo-swarm
npm install -g swarmdo-swarm@latest

# Verify Node.js version
node --version  # Should be 14.0+ (v18+ recommended)

# Check WASM files are present
ls node_modules/swarmdo-swarm/wasm/  # Should contain .wasm files
```
...
npx swarmdo-swarm mcp start --port 3000 --host 0.0.0.0

# Test with curl
curl http://your-server:3000/health

# Enable debug logging
NODE_ENV=development npx swarmdo-swarm mcp start --verbose
```

**Agent Spawn Failures**
```bash
# Check system resources
npx swarmdo-swarm status --detailed

# Verify configuration
npx swarmdo-swarm config validate

# Check logs
npx swarmdo-swarm logs --level debug
```

**Performance Issues**
```bash
# Run diagnostics
npx swarmdo-swarm benchmark --quick

# Enable SIMD if supported
export RUVA_SWARM_WASM_SIMD=true

# Adjust agent limits
npx swarmdo-swarm config set maxAgents 10
```
...
# Verbose output
npx swarmdo-swarm --verbose <command>

# Performance profiling
npx swarmdo-swarm profile <command>
```
...
> "*swarmdo-swarm transformed our AI development workflow. The cognitive diversity and WASM performance made complex multi-agent coordination finally practical.*" - **Tech Lead, AI Research**
...
> "*Enterprise features like persistence and auto-scaling make swarmdo-swarm production-ready out of the box.*" - **DevOps Engineer**
