...
export class SwarmVectorBackend {
    name = 'swarmvector';
    db; // VectorDB from @swarmvector/core
    config;
    metadata = new Map();
    initialized = false;
    // Concurrency control
    ...
    async initialize() {
        ...
        try {
            ...
            if (errorMessage.includes('Path traversal') || errorMessage.includes('Invalid path')) {
                throw new Error(`SwarmVector does not support :memory: database paths.\n` +
                    `Use a file path instead, or SwarmVector will be skipped and fallback backend will be used.\n` +
                    `Original error: ${errorMessage}`);
            }
            throw new Error(`SwarmVector initialization failed. Please install: npm install swarmvector\n` +
    ...
    async initializeMmap() {
        ...
        try {
            ...
            console.debug(`[SwarmVectorBackend] Failed to initialize mmap: ${error.message}`);
...
//# sourceMappingURL=SwarmVectorBackend.js.map