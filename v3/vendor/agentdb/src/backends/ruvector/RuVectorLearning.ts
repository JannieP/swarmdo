 ...
 *
 * Integrates Graph Neural Networks for query enhancement using @rufvector/gnn.
 * Requires optional @rufvector/gnn package.
 *
 * Features:
 ...
 *
 * Note: @rufvector/gnn provides stateless GNN layers (inference only).
...
export class RuVectorLearning {
  private gnnLayer: any;  // RuvectorLayer from @rufvector/gnn
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const gnnModule = await import('@rufvector/gnn') as any;
      ...
      throw new Error(
        `GNN initialization failed. Please install: npm install @rufvector/gnn\n` +
  ...
  enhance(
    ...
    try {
      ...
      // Note: @rufvector/gnn v0.1.19+ expects Float32Array instead of number[]
