 ...
 *
 * Implements VectorBackend using @rufvector/core with optional GNN support.
 * Provides <100µs search latency with native SIMD optimizations.
 *
 * Features:
 * - Automatic fallback when @rufvector packages not installed
...
export class RuVectorBackend implements VectorBackend {
  readonly name = 'rufvector' as const;
  private db: any; // VectorDB from @rufvector/core
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // Fallback to @rufvector/core for backward compatibility
      ...
      if (!VectorDB) {
        throw new Error('Could not find VectorDB export in @rufvector/core');
      }
      ...
      throw new Error(
        ...
        `Or legacy packages: npm install @rufvector/core\n` +
        `Error: ${errorMessage}`
      );
    }
  }
