/**
 * SwarmVectorBackend - High-Performance Vector Storage
 *
 * Implements VectorBackend using @swarmvector/core with optional GNN support.
 * Provides <100µs search latency with native SIMD optimizations.
 *
 * Features:
 * - Automatic fallback when @swarmvector packages not installed
 ...
 */
export interface SwarmVectorConfig extends VectorConfig {
...
export class SwarmVectorBackend implements VectorBackend {
  readonly name = 'swarmvector' as const;
  private db: any; // VectorDB from @swarmvector/core
  ...
  constructor(config: VectorConfig | SwarmVectorConfig) {
    ...
    this.config = { ...config, dimension, dimensions: dimension } as SwarmVectorConfig;

    // Initialize concurrency control with configurable limit (bounded for security)
    const concurrency = Math.min(Math.max(1, (config as SwarmVectorConfig).parallelConcurrency ?? 4), 32);
    this.semaphore = new Semaphore(concurrency);

    // Initialize buffer pool with bounded size
    const bufferPoolSize = Math.min(Math.max(1, (config as SwarmVectorConfig).bufferPoolSize ?? 100), 1000);
    this.bufferPool = new BufferPool(bufferPoolSize);

    // Mmap configuration - validate path if provided
    this.mmapEnabled = (config as SwarmVectorConfig).enableMmap ?? false;
    if (this.mmapEnabled && (config as SwarmVectorConfig).mmapPath) {
      validatePath((config as SwarmVectorConfig).mmapPath!);
    }
  }
  ...
  /**
   * Initialize SwarmVector database with optional dependency handling
   */
  async initialize(): Promise<void> {
    if (this.initialized) return;

    try {
      // Try main swarmvector package first (includes core, gnn, graph)
      let VectorDB;
      try {
        const swarmvector = await import('swarmvector');
        VectorDB = swarmvector.VectorDB || swarmvector.default?.VectorDB;
      } catch {
        // Fallback to @swarmvector/core for backward compatibility
      ...
      if (!VectorDB) {
        throw new Error('Could not find VectorDB export in @swarmvector/core');
      }
      ...
      // SwarmVector VectorDB constructor signature
      ...
      // Special handling for path validation errors (from swarmvector package)
      // When using :memory:, swarmvector may reject it as a path traversal attempt
      // This is expected and not critical - users should use file-based paths for swarmvector persistence
      if (errorMessage.includes('Path traversal') || errorMessage.includes('Invalid path')) {
        throw new Error(
          `SwarmVector does not support :memory: database paths.\n` +
          `Use a file path instead, or SwarmVector will be skipped and fallback backend will be used.\n` +
          `Original error: ${errorMessage}`
        );
      }

      throw new Error(
        `SwarmVector initialization failed. Please install: npm install swarmvector\n` +
        `Or legacy packages: npm install @swarmvector/core\n` +
        `Error: ${errorMessage}`
      );
    }
  }
  ...
  private async initializeMmap(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.debug('[SwarmVectorBackend] mmap-io not available, using standard I/O');
        return;
      }
      ...
      console.debug(`[SwarmVectorBackend] Failed to initialize mmap: ${(error as Error).message}`);
  ...
  insert(id: string, embedding: Float32Array, metadata?: Record<string, any>): void {
    ...
    // SwarmVector v0.1.30+ uses object API with 'vector' field
  ...
  search(query: Float32Array, k: number, options?: SearchOptions): SearchResult[] {
    ...
    // SwarmVector v0.1.30+ supports both object API and legacy positional args
  ...
  async load(loadPath: string): Promise<void> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        ...
        console.debug(`[SwarmVectorBackend] No metadata file found at ${metadataPath}`);
      } else {
        throw error;
      }
    }
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error('SwarmVectorBackend not initialized. Call initialize() first.');
    }
  }
