...
// Lazy load swarmvector (only when needed, not for install/help commands)
let VectorDB, getVersion, getImplementationType;
let swarmvectorLoaded = false;

function loadSwarmvector() {
  if (swarmvectorLoaded) return true;
  try {
    const swarmvector = require('../dist/index.js');
    VectorDB = swarmvector.VectorDB;
    getVersion = swarmvector.getVersion;
    getImplementationType = swarmvector.getImplementationType;
    swarmvectorLoaded = true;
    return true;
  } catch (e) {
    return false;
  }
...
function requireSwarmvector() {
  if (!loadSwarmvector()) {
    console.error(chalk.red('Error: Failed to load swarmvector. Please run: npm run build'));
    console.error(chalk.yellow('Or install the package: npm install swarmvector'));
    process.exit(1);
  }
}

// Lazy load GNN (optional - loaded on first use, not at startup)
// Saves ~6ms startup time by deferring require('@swarmvector/gnn')
let _gnnModule = undefined; // undefined = not yet attempted, null = failed, object = loaded
let SwarmvectorLayer, TensorCompress, differentiableSearch, getCompressionLevel, hierarchicalForward;
let gnnAvailable = false;

function loadGnn() {
  if (_gnnModule !== undefined) return _gnnModule;
  try {
    const gnn = require('@swarmvector/gnn');
    SwarmvectorLayer = gnn.SwarmvectorLayer;
...
// Saves ~5ms startup time by deferring require('@swarmvector/attention')
...
// `@swarmvector/gnn@0.1.25` has a native-binding regression where every method
...
program
  .name('swarmvector')
  .description(`${chalk.cyan('swarmvector')} - High-performance vector database CLI`)
...
program
  ...
  .action((options) => {
    requireSwarmvector();
    console.log(chalk.cyan('\nswarmvector Performance Benchmark'));
    console.log(chalk.gray(`Implementation: ${getImplementationType()}\n`));
...
program
  ...
  .action(() => {
    ...
    if (loadSwarmvector()) {
      ...
      console.log(chalk.white(`  Core: ${chalk.gray('Not loaded (install @swarmvector/core)')}`));
    }
    ...
    if (!gnnAvailable) {
      console.log(chalk.gray('\n  Install GNN with: npx swarmvector install gnn'));
    }
  });
...
program
  ...
  .description('Install optional swarmvector packages')
  ...
  .action(async (packages, options) => {
    ...
    // Available optional packages - all swarmvector npm packages
    const availablePackages = {
      // Core packages
      core: {
        name: '@swarmvector/core',
        description: 'Core vector database with native Rust bindings (HNSW, SIMD)',
        installed: true, // Always installed with swarmvector
        category: 'core'
      },
      gnn: {
        name: '@swarmvector/gnn',
        description: 'Graph Neural Network layers, tensor compression, differentiable search',
        installed: gnnAvailable,
        category: 'core'
      },
      'graph-node': {
        name: '@swarmvector/graph-node',
        description: 'Native Node.js bindings for hypergraph database with Cypher queries',
        installed: false,
        category: 'core'
      },
      'agentic-synth': {
        name: '@swarmvector/agentic-synth',
        description: 'Synthetic data generator for AI/ML training, RAG, and agentic workflows',
        installed: false,
        category: 'tools'
      },
      extensions: {
        name: 'swarmvector-extensions',
        description: 'Advanced features: embeddings, UI, exports, temporal tracking, persistence',
        installed: false,
        category: 'tools'
      },
      // Platform-specific native bindings for @swarmvector/core
      'node-linux-x64': {
        name: '@swarmvector/node-linux-x64-gnu',
        description: 'Linux x64 native bindings for @swarmvector/core',
        installed: false,
        category: 'platform'
      },
      'node-linux-arm64': {
        name: '@swarmvector/node-linux-arm64-gnu',
        description: 'Linux ARM64 native bindings for @swarmvector/core',
        installed: false,
        category: 'platform'
      },
      'node-darwin-x64': {
        name: '@swarmvector/node-darwin-x64',
        description: 'macOS Intel x64 native bindings for @swarmvector/core',
        installed: false,
        category: 'platform'
      },
      'node-darwin-arm64': {
        name: '@swarmvector/node-darwin-arm64',
        description: 'macOS Apple Silicon native bindings for @swarmvector/core',
        installed: false,
        category: 'platform'
      },
      'node-win32-x64': {
        name: '@swarmvector/node-win32-x64-msvc',
        description: 'Windows x64 native bindings for @swarmvector/core',
        installed: false,
        category: 'platform'
      },
      // Platform-specific native bindings for @swarmvector/gnn
      'gnn-linux-x64': {
        name: '@swarmvector/gnn-linux-x64-gnu',
        description: 'Linux x64 native bindings for @swarmvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-linux-arm64': {
        name: '@swarmvector/gnn-linux-arm64-gnu',
        description: 'Linux ARM64 native bindings for @swarmvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-darwin-x64': {
        name: '@swarmvector/gnn-darwin-x64',
        description: 'macOS Intel x64 native bindings for @swarmvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-darwin-arm64': {
        name: '@swarmvector/gnn-darwin-arm64',
        description: 'macOS Apple Silicon native bindings for @swarmvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-win32-x64': {
        name: '@swarmvector/gnn-win32-x64-msvc',
        description: 'Windows x64 native bindings for @swarmvector/gnn',
        installed: false,
        category: 'platform'
      },
      ...
      'swarmvector-core': {
        name: 'swarmvector-core',
        description: 'Standalone vector database (legacy, use @swarmvector/core instead)',
        installed: false,
        category: 'legacy'
      }
    };
    ...
    if (options.list || (packages.length === 0 && !options.all && !options.interactive)) {
      console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
      console.log(chalk.cyan('                    Swarmvector Packages'));
      console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
      ...
      console.log(chalk.white('  npx swarmvector install gnn              # Install GNN package'));
      console.log(chalk.white('  npx swarmvector install graph-node       # Install graph database'));
      console.log(chalk.white('  npx swarmvector install agentic-synth    # Install data generator'));
      console.log(chalk.white('  npx swarmvector install --all            # Install all core packages'));
      console.log(chalk.white('  npx swarmvector install -i               # Interactive selection'));
      console.log(chalk.gray('\n  Note: Platform bindings are auto-detected by @swarmvector/core'));
      return;
    }
    ...
    for (const pkg of packages) {
      const key = pkg.toLowerCase().replace('@swarmvector/', '');
    ...
    function installPackages(pkgs) {
      ...
      try {
        ...
        console.log(chalk.cyan('\nRun "npx swarmvector info" to verify installation.'));
      } catch (error) {
...
function requireGnn() {
  ...
  if (!gnnAvailable) {
    ...
    console.error(chalk.yellow('Install it with: npm install @swarmvector/gnn'));
    process.exit(1);
  }
}
...
gnnCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      const layer = new SwarmvectorLayer(inputDim, hiddenDim, heads, dropout);
      ...
      if (options.test) {
        ...
        // The @swarmvector/gnn binding requires Float32Array — plain number[] surfaces
...
gnnCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      // @swarmvector/gnn's differentiableSearch needs Float32Array everywhere; plain
...
gnnCmd
  ...
  .action(() => {
    ...
    if (!gnnAvailable) {
      ...
      console.log(chalk.white('Install with: npm install @swarmvector/gnn'));
      return;
    }
    ...
    console.log(chalk.cyan('\nAvailable Features:'));
    console.log(chalk.white(`  • SwarmvectorLayer   - GNN layer with multi-head attention`));
...
function requireAttention() {
  ...
  if (!attentionAvailable) {
    console.error(chalk.red('Error: @swarmvector/attention is not installed'));
    console.error(chalk.yellow('Install it with: npm install @swarmvector/attention'));
    process.exit(1);
  }
}
...
attentionCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      // The native @swarmvector/attention bindings require Float32Array; passing
      ...
      // The native @swarmvector/attention bindings expose `compute(query, keys, values)`
...
attentionCmd
  ...
  .action(() => {
    ...
    if (!attentionAvailable) {
      ...
      console.log(chalk.white('Install with: npm install @swarmvector/attention'));
      return;
    }
...
attentionCmd
  ...
  .action((options) => {
    ...
    if (!attentionAvailable) {
      console.log(chalk.yellow('\n  Note: @swarmvector/attention not installed'));
      console.log(chalk.white('  Install with: npm install @swarmvector/attention'));
    }
    ...
    if (options.verbose) {
      ...
      console.log(chalk.white('    npx swarmvector attention compute -q "[1,2,3]" -k keys.json -t dot'));
      console.log(chalk.gray('\n    # Benchmark attention mechanisms'));
      console.log(chalk.white('    npx swarmvector attention benchmark -d 256 -n 100'));
      console.log(chalk.gray('\n    # Hyperbolic distance'));
      console.log(chalk.white('    npx swarmvector attention hyperbolic -a distance -v "[0.1,0.2]" -b "[0.3,0.4]"'));
    }
  });
...
program
  ...
  .action(async (options) => {
    ...
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Doctor'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    // SwarmVector Packages
    console.log(chalk.cyan('SwarmVector Packages:'));

    // Check @swarmvector/core
    let coreAvailable = false;
    try {
      require.resolve('@swarmvector/core');
      coreAvailable = true;
      console.log(chalk.green(`  ✓ @swarmvector/core installed`));
    } catch (e) {
      console.log(chalk.yellow(`  ! @swarmvector/core not found (using WASM fallback)`));
      warnings++;
    }

    // Check if native binding works
    if (coreAvailable && loadSwarmvector()) {
    ...
    // Check @swarmvector/gnn
    if (gnnAvailable) {
      console.log(chalk.green(`  ✓ @swarmvector/gnn installed`));
    } else {
      console.log(chalk.gray(`  ○ @swarmvector/gnn not installed (optional)`));
    }

    // Check @swarmvector/attention
    if (attentionAvailable) {
      console.log(chalk.green(`  ✓ @swarmvector/attention installed`));
    } else {
      console.log(chalk.gray(`  ○ @swarmvector/attention not installed (optional)`));
    }

    // Check @swarmvector/graph-node
    try {
      require.resolve('@swarmvector/graph-node');
      console.log(chalk.green(`  ✓ @swarmvector/graph-node installed`));
    } catch (e) {
      console.log(chalk.gray(`  ○ @swarmvector/graph-node not installed (optional)`));
    }
    console.log('');
    ...
    if (issues === 0 && warnings === 0) {
      console.log(chalk.green('\n  ✓ All checks passed! SwarmVector is ready to use.\n'));
    } else if (issues === 0) {
      console.log(chalk.yellow(`\n  ! ${warnings} warning(s) found. SwarmVector should work but may have limited features.\n`));
    } else {
      console.log(chalk.red(`\n  ✗ ${issues} issue(s) and ${warnings} warning(s) found.\n`));
      console.log(chalk.white('  Run "npx swarmvector setup" for installation instructions.\n'));
    }
  });
...
program
  ...
  .action((options) => {
    ...
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Setup Guide'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (showAll || options.npm) {
      ...
      console.log(chalk.white('  npm install -g swarmvector'));
      console.log(chalk.white('  npx swarmvector'));
      console.log('');

      console.log(chalk.yellow('Core packages:'));
      console.log(chalk.white('  npm install @swarmvector/core       # Vector database'));
      console.log(chalk.white('  npm install @swarmvector/gnn        # Graph Neural Networks'));
      console.log(chalk.white('  npm install @swarmvector/graph-node # Hypergraph database'));
      console.log('');

      console.log(chalk.yellow('Install all optional packages:'));
      console.log(chalk.white('  npx swarmvector install --all'));
      console.log('');

      console.log(chalk.yellow('List available packages:'));
      console.log(chalk.white('  npx swarmvector install'));
      console.log('');
    }
    ...
    if (showAll || options.rust) {
      ...
      console.log(chalk.yellow('3. Add SwarmVector crates to your project:'));
      console.log(chalk.white('  cargo add swarmvector-core          # Vector database'));
      console.log(chalk.white('  cargo add swarmvector-graph         # Hypergraph with Cypher'));
      console.log(chalk.white('  cargo add swarmvector-gnn           # Graph Neural Networks'));
      console.log('');

      console.log(chalk.yellow('4. Other available crates:'));
      console.log(chalk.white('  cargo add swarmvector-cluster       # Distributed clustering'));
      console.log(chalk.white('  cargo add swarmvector-raft          # Raft consensus'));
      console.log(chalk.white('  cargo add swarmvector-replication   # Data replication'));
      console.log(chalk.white('  cargo add swarmvector-tiny-dancer-core  # AI routing'));
      console.log(chalk.white('  cargo add swarmvector-router-core       # Semantic routing (Rust crate)'));
      console.log(chalk.white('  npm install @swarmvector/router         # Semantic routing (npm)'));
      console.log('');
    ...
    console.log(chalk.white('  npx swarmvector doctor     # Check system health'));
    console.log(chalk.white('  npx swarmvector info       # Show version info'));
    console.log(chalk.white('  npx swarmvector benchmark  # Run performance test'));
    console.log(chalk.white('  npx swarmvector install    # List available packages'));
    console.log('');
  });
...
program
  .command('graph')
  .description('Graph database operations (requires @swarmvector/graph-node)')
  ...
  .action(async (options) => {
    let graphNode;
    try {
      graphNode = require('@swarmvector/graph-node');
    } catch (e) {
      console.log(chalk.yellow('\n  @swarmvector/graph-node is not installed.\n'));
      console.log(chalk.cyan('  Install with:'));
      console.log(chalk.white('    npm install @swarmvector/graph-node\n'));
      ...
      console.log(chalk.white('    npx swarmvector graph --query "CREATE (n:Person {name: \'Alice\'})"'));
      console.log(chalk.white('    npx swarmvector graph --query "MATCH (n) RETURN n"'));
      console.log('');
      return;
    }

    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Graph'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));

    if (options.info) {
      console.log(chalk.green('  @swarmvector/graph-node is available!'));
    ...
    if (options.query) {
      ...
      console.log(chalk.gray('  Note: Full Cypher execution requires running swarmvector-server'));
      console.log(chalk.gray('  See: npx swarmvector server --help'));
    }
...
program
  .command('router')
  .description('AI semantic router operations (requires @swarmvector/router)')
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Router'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (options.info || (!options.route && !options.intents && !options.addIntent)) {
      ...
      console.log(chalk.gray('  npm package:  npm install @swarmvector/router'));
      console.log(chalk.gray('  Rust crate:   cargo add swarmvector-router-core'));
      console.log('');
      console.log(chalk.cyan('  Usage (when available):'));
      console.log(chalk.white('    npx swarmvector router --route "What is the weather?"'));
      console.log(chalk.white('    npx swarmvector router --intents intents.json --route "query"'));
      console.log('');
      return;
    }
...
const tinyDancer = program
  .command('tiny-dancer')
  .alias('td')
  .description('Cost-optimal FastGRNN model router — train from a DRACO dataset and route with it (requires @swarmvector/tiny-dancer)');

function loadTinyDancer() {
  try {
    return require('@swarmvector/tiny-dancer');
  } catch (e) {
    console.error(chalk.red('\n  This command requires @swarmvector/tiny-dancer'));
    console.error(chalk.yellow('  Install it:  npm install @swarmvector/tiny-dancer'));
...
tinyDancer
  ...
  .action(() => {
    try {
      ...
      console.log(chalk.green(`\n  @swarmvector/tiny-dancer ${td.version()} — ${td.hello()}`));
      console.log(chalk.gray('  train:  npx swarmvector tiny-dancer train <draco.json> --out model.safetensors'));
      console.log(chalk.gray('  score:  npx swarmvector tiny-dancer score <model.safetensors> --query <embedding.json>\n'));
    } catch {
      console.log(chalk.yellow('\n  @swarmvector/tiny-dancer not installed.  npm install @swarmvector/tiny-dancer\n'));
    }
  });
...
program
  .command('server')
  .description('Start SwarmVector HTTP/gRPC server')
  .option('-p, --port <number>', 'HTTP port', '8080')
  .option('-g, --grpc-port <number>', 'gRPC port', '50051')
  .option('-d, --data-dir <path>', 'Data directory', './swarmvector-data')
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Server'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (options.info || Object.keys(options).filter(k => k !== 'port' && k !== 'grpcPort' && k !== 'dataDir').length === 0) {
      ...
      console.log(chalk.white('    cargo install swarmvector-server  # When published'));
      console.log('');
...
program
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Cluster'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    console.log(chalk.white('    cargo add swarmvector-cluster      # Clustering'));
    console.log(chalk.white('    cargo add swarmvector-raft         # Raft consensus'));
    console.log(chalk.white('    cargo add swarmvector-replication  # Replication'));
    console.log('');
    console.log(chalk.cyan('  Commands (when available):'));
    console.log(chalk.white('    npx swarmvector cluster --status'));
    ...
    try {
      ...
      if (!vectors || vectors.length === 0 || !vectors[0].vector) {
        spinner.fail(chalk.yellow(
          ...
          '(the same format `swarmvector insert` accepts). Legacy _export.json ' +
  ...
  .action(async (text, opts) => {
    try {
      ...
      if (text) {
        ...
        console.log(chalk.yellow('Usage: swarmvector embed optimized <text>'));
...
program
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    SwarmVector Demo'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (showMenu) {
      ...
      console.log(chalk.white('    npx swarmvector demo --basic'));
      console.log(chalk.white('    npx swarmvector demo --gnn'));
      console.log('');
      return;
    }
    ...
    if (options.basic) {
      ...
      try {
        ...
        console.log(chalk.white('    npx swarmvector stats ./demo.db'));
        console.log(chalk.white('    npx swarmvector search ./demo.db --vector "[0.8, 0.6, 0, 0]"'));
      } catch (error) {
    ...
    if (options.gnn) {
      loadGnn();
      if (!gnnAvailable) {
        console.log(chalk.yellow('  @swarmvector/gnn not installed.'));
        console.log(chalk.white('  Install with: npm install @swarmvector/gnn'));
        console.log('');
        return;
      }
      ...
      try {
        ...
        // The native @swarmvector/gnn binding expects Float32Array typed arrays.
        ...
        // `@swarmvector/gnn@0.1.25`'s native binding has a regression where every
    ...
    if (options.graph) {
      ...
      try {
        ...
        console.log(chalk.yellow('  @swarmvector/graph-node not installed.'));
        console.log(chalk.white('  Install with: npm install @swarmvector/graph-node'));
        console.log('');
        return;
      }
      ...
      try {
        ...
        // HyperGraph / SwarmVectorGraph) with createNode / createEdge / query.
        const GraphDatabase = graphNode.GraphDatabase;
        if (typeof GraphDatabase !== 'function') {
          console.log(chalk.yellow('  @swarmvector/graph-node has no GraphDatabase constructor.'));
        ...
        // args) and are async — see @swarmvector/graph-node index.d.ts.
    ...
    if (options.benchmark) {
      ...
      try {
        // Note: swarmvector-core-linux-x64-gnu@0.1.29 (and current sister binaries)
        ...
        console.log(chalk.gray('  For deeper benchmarks: npx swarmvector benchmark'));
      } catch (error) {
...
// Full SwarmVector Stack: VectorDB + SONA + Attention
// ============================================
...
class Intelligence {
  ...
  getIntelPath() {
    const projectPath = path.join(process.cwd(), '.swarmvector', 'intelligence.json');
    const homePath = path.join(require('os').homedir(), '.swarmvector', 'intelligence.json');
  ...
  checkVectorWrite(active) {
    ...
    if (this.isLegacyVectorStore()) {
      ...
      const err = new Error(
        ...
        `${prov.describeProvenance(active)}. Run 'swarmvector hooks reembed' to re-embed and unlock it.`
      );
  ...
  warnRecallProvenance(active) {
    ...
    if (mismatches.length > 0) {
      ...
      console.error(
        `swarmvector: recall quality degraded — stored vectors are ${prov.describeProvenance(stored)} ` +
        `but the query was embedded as ${prov.describeProvenance(active)} (differs on: ${mismatches.join(', ')}). ` +
        `Run 'swarmvector hooks reembed' to fix.`
      );
    }
  }
...
function getPermissionsForProjectType(types) {
  const basePermissions = [
    ...
    'Bash(ls:*)', 'Bash(pwd)', 'Bash(cat:*)', 'Bash(mkdir:*)', 'Bash(which:*)', 'Bash(swarmvector:*)'
  ];
...
hooksCmd.command('init')
  ...
  // Workers configuration (native swarmvector workers + agentic-flow integration)
  if (!opts.minimal) {
  ...
  if (!opts.minimal && opts.statusline !== false) {
    if (!settings.statusLine) {
      if (isWindows) {
        ...
        const statuslineContent = `# SwarmVector Intelligence Statusline for Windows PowerShell
...
$IntelFile = Join-Path $CWD ".swarmvector\intelligence.json"
if (Test-Path $IntelFile) {
  ...
  Write-Host "$e[2m SwarmVector: run 'npx swarmvector hooks session-start'$e[0m"
}
`;
...
# SwarmVector Intelligence Statusline - Multi-line display
INPUT=\$(cat)
...
echo
INTEL_FILE=""
for P in "\$CWD/.swarmvector/intelligence.json" "\$CWD/npm/packages/swarmvector/.swarmvector/intelligence.json" "\$HOME/.swarmvector/intelligence.json"; do
...
if [ -n "\$INTEL_FILE" ]; then
  ...
  printf "\$MAGENTA🧠 SwarmVector\$RESET"
  ...
  printf "\$DIM🧠 SwarmVector: run 'npx swarmvector hooks session-start' to initialize\$RESET\\n"
fi
`;
        fs.writeFileSync(statuslineScript, statuslineContent);
  ...
  let hookCmd = 'npx swarmvector@latest';
  let fastTimeouts = { simple: 2000, complex: 2000, session: 5000 };
  if (opts.fast && !isWindows) {
    const fastWrapperPath = path.join(settingsDir, 'swarmvector-fast.sh');
    const fastWrapperContent = `#!/bin/bash
# Fast SwarmVector hooks wrapper - avoids npx overhead (20x faster)
...
elif command -v swarmvector &> /dev/null; then
  exec swarmvector "$@"
# Fallback to npx (slow but works)
else
  exec npx swarmvector@latest "$@"
fi
...
`;
    ...
    hookCmd = '.claude/swarmvector-fast.sh';
    fastTimeouts = { simple: 300, complex: 500, session: 1000 };
    // Add permission for fast wrapper
    if (settings.permissions && settings.permissions.allow) {
      if (!settings.permissions.allow.includes('Bash(.claude/swarmvector-fast.sh:*)')) {
        settings.permissions.allow.push('Bash(.claude/swarmvector-fast.sh:*)');
      }
    }
    console.log(chalk.blue('  ✓ Fast wrapper created (.claude/swarmvector-fast.sh) - 20x faster hooks'));
  }
...
## SwarmVector Self-Learning Intelligence v2.0

This project uses SwarmVector's self-learning intelligence hooks with advanced capabilities:
...
npx swarmvector hooks init

# View learning statistics
npx swarmvector hooks stats

# Route a task to best agent
npx swarmvector hooks route "implement feature X"

# Enhanced routing with AST/coverage/diff signals
npx swarmvector hooks route-enhanced "fix bug" --file src/api.ts

# Store context in vector memory
npx swarmvector hooks remember "important context" -t project

# Recall from memory (semantic search)
npx swarmvector hooks recall "context query"
\`\`\`
...
npx swarmvector hooks ast-analyze src/index.ts

# Get complexity metrics for multiple files
npx swarmvector hooks ast-complexity src/*.ts --threshold 15
\`\`\`
...
npx swarmvector hooks diff-analyze HEAD

# Classify change type (feature, bugfix, refactor, etc.)
npx swarmvector hooks diff-classify

# Find similar past commits
npx swarmvector hooks diff-similar -k 5

# Get risk score only
npx swarmvector hooks diff-analyze --risk-only
\`\`\`
...
npx swarmvector hooks coverage-route src/api.ts

# Suggest tests for files based on coverage
npx swarmvector hooks coverage-suggest src/*.ts
\`\`\`
...
npx swarmvector hooks graph-mincut src/*.ts

# Detect code communities (Louvain/Spectral clustering)
npx swarmvector hooks graph-cluster src/*.ts --method louvain
\`\`\`
...
npx swarmvector hooks security-scan src/*.ts

# RAG-enhanced context retrieval
npx swarmvector hooks rag-context "how does auth work"

# Git churn analysis (hot spots)
npx swarmvector hooks git-churn --days 30
\`\`\`

### MCP Tools (via Claude Code)

When using the SwarmVector MCP server, these tools are available:
...
SwarmVector includes 10 attention mechanisms:
...
Stored in \`.swarmvector/intelligence.json\`:
...
\`\`\`bash
npx swarmvector hooks init              # Full configuration with all capabilities
npx swarmvector hooks init --minimal    # Basic hooks only
npx swarmvector hooks init --fast       # Use fast local wrapper (20x faster)
npx swarmvector hooks init --pretrain   # Initialize + pretrain from git history
npx swarmvector hooks init --build-agents quality  # Generate optimized agents
npx swarmvector hooks init --force      # Overwrite existing configuration
\`\`\`
...
`;
  ...
  if (opts.gitignore !== false) {
    ...
    const entriesToAdd = ['.swarmvector/', '.claude/statusline.sh'];
    ...
    if (linesToAdd.length > 0) {
      const newContent = gitignoreContent.trim() + '\n\n# SwarmVector intelligence data\n' + linesToAdd.join('\n') + '\n';
      fs.writeFileSync(gitignorePath, newContent);
      console.log(chalk.blue('  ✓ .gitignore updated'));
    }
  }

  // Create .swarmvector directory for intelligence data
  const swarmvectorDir = path.join(process.cwd(), '.swarmvector');
  if (!fs.existsSync(swarmvectorDir)) {
    fs.mkdirSync(swarmvectorDir, { recursive: true });
    console.log(chalk.blue('  ✓ .swarmvector/ directory created'));
  }

  console.log(chalk.green('\n✅ SwarmVector hooks initialization complete!'));
  ...
  if (opts.pretrain) {
    ...
    const { execSync } = require('child_process');
    try {
      execSync('npx swarmvector hooks pretrain', { stdio: 'inherit' });
    } catch (e) {
  ...
  if (opts.buildAgents) {
    ...
    const { execSync } = require('child_process');
    try {
      execSync(`npx swarmvector hooks build-agents --focus ${focus} --include-prompts`, { stdio: 'inherit' });
    } catch (e) {
  ...
  if (!opts.pretrain && !opts.buildAgents) {
    console.log(chalk.dim('   Run `npx swarmvector hooks verify` to test the setup'));
    console.log(chalk.dim('   Run `npx swarmvector hooks pretrain` to bootstrap intelligence'));
    console.log(chalk.dim('   Run `npx swarmvector hooks build-agents` to generate optimized agents'));
  }
});
...
hooksCmd.command('session-start').description('Session start hook').option('--resume', 'Resume previous session').action(() => {
  ...
  intel.sessionStart();
  intel.save();
  console.log(chalk.bold.cyan('🧠 SwarmVector Intelligence Layer Active'));
...
hooksCmd.command('suggest-context').description('Suggest relevant context').action(() => {
  const intel = new Intelligence();
  const stats = intel.stats();
  console.log(`SwarmVector Intelligence: ${stats.total_patterns} learned patterns, ${stats.total_errors} error fixes available. Use 'swarmvector hooks route' for agent suggestions.`);
});
...
hooksCmd.command('learning-config')
  ...
  .action(async (opts) => {
    ...
    const dataPath = path.join(process.cwd(), '.swarmvector', 'intelligence.json');
    let data = {};
    try {
...
hooksCmd.command('verify')
  ...
  .action((opts) => {
    console.log(chalk.bold.cyan('\n🔍 SwarmVector Hooks Verification\n'));
    const checks = [];
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      checks.push({ name: 'Settings file', status: 'fail', detail: 'Run `npx swarmvector hooks init` first' });
    }

    // Check 2: .swarmvector directory
    const swarmvectorDir = path.join(process.cwd(), '.swarmvector');
    if (fs.existsSync(swarmvectorDir)) {
      checks.push({ name: 'Data directory', status: 'pass', detail: '.swarmvector/ exists' });
      const intelFile = path.join(swarmvectorDir, 'intelligence.json');
    ...
    try {
      const { execSync } = require('child_process');
      execSync('npx swarmvector hooks stats', { stdio: 'pipe', timeout: 5000 });
    ...
    if (failCount === 0) {
      ...
      console.log(chalk.dim('   Run `npx swarmvector hooks doctor` for detailed diagnostics'));
    }
  });
...
hooksCmd.command('doctor')
  ...
  .action((opts) => {
    ...
    const settingsPath = path.join(process.cwd(), '.claude', 'settings.json');
    if (!fs.existsSync(settingsPath)) {
      issues.push({ severity: 'error', message: 'No .claude/settings.json found', fix: 'Run `npx swarmvector hooks init`' });
    } else {
      try {
        const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
        ...
        const checkCommands = (hooks) => {
          ...
          hooks.forEach(h => {
            h.hooks?.forEach(hh => {
              if (hh.command && hh.command.includes('swarmvector') && !hh.command.startsWith('npx ') && !hh.command.includes('/bin/')) {
                issues.push({ severity: 'warning', message: `Command should use 'npx swarmvector' for portability`, fix: 'Update to use npx' });
              }
            });
          });
        };
        ...
        issues.push({ severity: 'error', message: 'Invalid JSON in settings file', fix: 'Re-run `npx swarmvector hooks init --force`' });
      }
    }
    ...
    if (fs.existsSync(gitignorePath)) {
      ...
      if (!content.includes('.swarmvector/')) {
        issues.push({ severity: 'warning', message: '.swarmvector/ not in .gitignore', fix: 'Add to prevent committing learning data' });
        if (opts.fix) {
          fs.appendFileSync(gitignorePath, '\n# SwarmVector intelligence data\n.swarmvector/\n');
          fixes.push('Added .swarmvector/ to .gitignore');
        }
      }
    }
...
hooksCmd.command('export')
  ...
  .option('-o, --output <file>', 'Output file path', 'swarmvector-export.json')
...
hooksCmd.command('import')
  ...
  .action((file, opts) => {
    ...
    try {
      ...
      intel.save();
      console.log(chalk.dim('   Data saved to .swarmvector/intelligence.json'));
    } catch (e) {
...
hooksCmd.command('pretrain')
  ...
  .action(async (opts) => {
    ...
    console.log(chalk.bold.cyan('\n🧠 SwarmVector Pretrain - Repository Intelligence Bootstrap\n'));
    ...
    try {
      ...
      try {
        ...
        console.log(chalk.dim('  ⏭️  @swarmvector/attention not installed'));
      }

      try {
        const gnn = require('@swarmvector/gnn');
        capabilities.gnn = true;
        console.log(chalk.green(`  ✓ GNN: SwarmvectorLayer, TensorCompress available`));
      } catch (e) {
        console.log(chalk.dim('  ⏭️  @swarmvector/gnn not installed'));
      }
...
${focus.description}` : null
    ...
    agents.forEach(agent => {
      ...
      if (opts.format === 'yaml') {
        const yaml = [
          `# Auto-generated by SwarmVector Agent Builder`,
          `# Focus: ${opts.focus}`,
    ...
    const indexContent = opts.format === 'yaml'
      ? `# SwarmVector Agent Configuration\n# Focus: ${opts.focus}\n\nagents:\n${agents.map(a => `  - ${a.name}`).join('\n')}`
...
// Native Workers command group - Deep swarmvector integration (no agentic-flow delegation)
...
nativeCmd.command('compare')
  .description('Compare swarmvector native vs agentic-flow workers')
...
// RVF (SwarmVector Format) commands
const rvfCmd = program.command('rvf').description('SwarmVector Format (.rvf) cognitive container operations');
...
const RVF_EXAMPLES = [
  ...
  { name: 'swarmbot', size: '51 KB', desc: 'Chatbot knowledge store' },
  ...
  { name: 'swarmllm_inference', size: '133 KB', desc: 'SwarmLLM inference cache' },
...
rvfCmd.command('examples')
  ...
  .action((opts) => {
    ...
    console.log(chalk.dim(`Download: npx swarmvector rvf download <name>\n`));
...
rvfCmd.command('download [names...]')
  ...
  .action(async (names, opts) => {
    ...
    if (opts.all) {
      ...
      console.error(chalk.red('Specify example names or use --all. Run `npx swarmvector rvf examples` to list.'));
      process.exit(1);
    }
    ...
    for (const name of toDownload) {
      ...
      if (!opts.all) {
        ...
        if (!RVF_EXAMPLES.some(e => e.name === baseName)) {
          console.log(chalk.red(`SKIPPED: Unknown example '${baseName}'. Run 'npx swarmvector rvf examples' to list.`));
          fail++;
          continue;
        }
      }
...
mcpCmd.command('start')
  .description('Start the SwarmVector MCP server')
  .action(() => {
...
mcpCmd.command('info')
  ...
  .action(() => {
    console.log(chalk.bold.cyan('\n🔌 SwarmVector MCP Server\n'));
    console.log(chalk.white('The SwarmVector MCP server provides self-learning intelligence'));
    ...
    console.log(chalk.bold('\n📦 Resources:'));
    console.log(chalk.dim('  swarmvector://intelligence/stats     - Current statistics'));
    console.log(chalk.dim('  swarmvector://intelligence/patterns  - Learned patterns'));
    console.log(chalk.dim('  swarmvector://intelligence/memories  - Vector memories'));

    console.log(chalk.bold.yellow('\n⚙️  Setup Instructions:\n'));
    console.log(chalk.white('Add to Claude Code:'));
    console.log(chalk.cyan('  claude mcp add swarmvector npx swarmvector mcp start\n'));
...
mcpCmd.command('test')
  ...
  .action(() => {
    ...
    console.log(chalk.bold.green('\n  All checks passed.\n'));
    console.log(chalk.dim('  Setup: claude mcp add swarmvector npx swarmvector mcp start\n'));
  });

// ============================================================================
// Brain Commands — Shared intelligence via @swarmvector/pi-brain (lazy-loaded)
// ============================================================================

// Lazy load pi-brain (optional peer dependency)
async function requirePiBrain() {
  try {
    return await import('@swarmvector/pi-brain');
  } catch {
    console.error(chalk.red('Brain commands require @swarmvector/pi-brain'));
    console.error(chalk.yellow('  npm install @swarmvector/pi-brain'));
    process.exit(1);
  }
}
...
brainAgiCmd
  ...
  .action(() => {
    console.log(chalk.cyan('\n  SONA subsystem'));
    console.log(chalk.dim('  Use `swarmvector sona status` for full SONA metrics.\n'));
  });
...
identityCmd
  ...
  .option('--save', 'Save the key to ~/.swarmvector/pi-key')
  .action(async (cmdOpts) => {
    ...
    if (cmdOpts.save) {
      const keyDir = path.join(require('os').homedir(), '.swarmvector');
...
identityCmd
  ...
  .action(async () => {
    ...
    if (!key) {
      const keyPath = path.join(require('os').homedir(), '.swarmvector', 'pi-key');
      if (fs.existsSync(keyPath)) {
        key = fs.readFileSync(keyPath, 'utf8').trim();
      }
    }
    if (!key) {
      console.error(chalk.red('No pi key found. Set PI env var or run: swarmvector identity generate --save'));
      process.exit(1);
    }
    let pseudonym;
    try {
    ...
    if (isJsonOutput(opts)) {
      console.log(JSON.stringify({
        ...
        source: process.env.PI ? 'PI env var' : '~/.swarmvector/pi-key',
      }, null, 2));
    } else {
      console.log(chalk.bold.cyan('\n  Pi Identity\n'));
      console.log(chalk.white(`  Pseudonym: ${pseudonym}`));
      console.log(chalk.gray(`  Key:       ${key.substring(0, 8)}...${key.substring(key.length - 8)}`));
      console.log(chalk.gray(`  Source:    ${process.env.PI ? 'PI env var' : '~/.swarmvector/pi-key'}\n`));
    }
  });
...
identityCmd
  ...
  .option('--save', 'Save the imported key to ~/.swarmvector/pi-key')
...
llmCmd.command('embed')
  ...
  .action(async (text, opts) => {
    let swarmllm;
    try {
      swarmllm = require('@swarmvector/swarmllm');
    } catch (e) {
      console.error(chalk.red('LLM commands require @swarmvector/swarmllm'));
      console.error(chalk.yellow('  npm install @swarmvector/swarmllm'));
      console.error(chalk.dim('  or: npx swarmvector install swarmllm'));
      console.error(chalk.dim('\n  Tip: use "swarmvector embed text <text>" for built-in ONNX embeddings'));
      process.exit(1);
    }
    ...
    try {
      const { performance } = require('perf_hooks');
      const start = performance.now();
      const llm = new swarmllm.SwarmLLM({ embeddingDim: 384 });
...
llmCmd.command('models')
  ...
  .action(async (opts) => {
    ...
    console.log(chalk.bold.cyan('\n  SwarmLLM Available Models\n'));
    if (!models || models.length === 0) {
      console.log(chalk.yellow('  No models registered in the registry.'));
      console.log(chalk.dim('  Upgrade @swarmvector/swarmllm for model registry support:'));
      console.log(chalk.dim('    npm install @swarmvector/swarmllm@latest\n'));
    } else {
...
llmCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    const version = typeof swarmllm.version === 'function' ? swarmllm.version() : (swarmllm.version || 'unknown');
    const hasSIMD = swarmllm.hasSimdSupport ? swarmllm.hasSimdSupport() : false;
    const models = swarmllm.listModels ? swarmllm.listModels() : [];
    const info = {
      package: '@swarmvector/swarmllm',
      version,
      simd: hasSIMD,
...
sonaCmd.command('status')
  ...
  .action(async (opts) => {
    try {
      ...
      try {
        const { SonaCoordinator } = require('@swarmvector/swarmllm');
        ...
        console.error(chalk.yellow('  Install @swarmvector/swarmllm for JS fallback: npm install @swarmvector/swarmllm'));
...
sonaCmd.command('train')
  ...
  .action(async (data, opts) => {
    ...
    try {
      ...
      console.error(chalk.dim('\n  Example: swarmvector sona train \'{"steps":[{"type":"query","input":"test","output":"result","confidence":0.9}],"outcome":"success"}\''));
      process.exit(1);
    }
    ...
    try {
      ...
      try {
        const { SonaCoordinator, TrajectoryBuilder } = require('@swarmvector/swarmllm');
        const sona = new SonaCoordinator();
...
sonaCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    try { const sona = require('@swarmvector/sona'); nativeAvailable = !!sona.SonaEngine; nativeVersion = sona.version || '0.1.4'; } catch (e) { /* not available */ }
    let jsAvailable = false, jsVersion = 'N/A';
    try { const swarmllm = require('@swarmvector/swarmllm'); jsAvailable = !!swarmllm.SonaCoordinator; jsVersion = typeof swarmllm.version === 'function' ? swarmllm.version() : (swarmllm.version || 'unknown'); } catch (e) { /* not available */ }
    const info = { package: '@swarmvector/sona', nativeBinding: nativeAvailable, nativeVersion, jsFallback: jsAvailable, jsFallbackVersion: jsVersion, features: ['LoRA adaptive weights', 'EWC++ memory protection', 'ReasoningBank pattern storage', 'Trajectory tracking', 'Continual learning', 'Sub-millisecond overhead'] };
    ...
    console.log(chalk.white(`  JS fallback:     ${jsAvailable ? chalk.green('Available') + chalk.dim(` (swarmllm v${jsVersion})`) : chalk.yellow('Not available')}`));
    console.log(chalk.white('\n  Features:'));
...
// =============================================================================
// Route Commands - Semantic routing (lazy: @swarmvector/router)
// =============================================================================
...
routeCmd.command('classify')
  ...
  .action(async (input, opts) => {
    let router;
    try { router = require('@swarmvector/router'); } catch (e) {
      console.error(chalk.red('Route commands require @swarmvector/router'));
      console.error(chalk.yellow('  npm install @swarmvector/router'));
      console.error(chalk.dim('  or: npx swarmvector install router'));
      process.exit(1);
    }
    ...
    try {
      ...
      if (!results || results.length === 0) {
        ...
        if (!routesDef) { console.log(chalk.dim('  Provide a routes file with -r/--routes to define intents.')); console.log(chalk.dim('  Example: swarmvector route classify "hello" -r routes.json')); }
      } else {
        for (let i = 0; i < results.length; i++) {
          const r = results[i];
...
routeCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    try { const router = require('@swarmvector/router'); routerAvailable = true; routerVersion = router.version || '0.1.28'; const sr = new router.SemanticRouter({ dimension: 16 }); metrics = { distanceMetrics: Object.keys(router.DistanceMetric || {}), hasSemanticRouter: true }; } catch (e) { /* Not available */ }
    const info = { package: '@swarmvector/router', available: routerAvailable, version: routerVersion, backend: 'Rust NAPI (HNSW + SIMD)', features: ['Semantic intent matching', 'HNSW indexing', 'SIMD-accelerated search', 'Multiple distance metrics', 'Save/load state', 'Async embedding support'], ...metrics };
    if (opts.json) { console.log(JSON.stringify(info, null, 2)); return; }
...
const decompileCmd = program
  ...
  .action(async (target, opts) => {
    ...
    if (!target) {
      console.log(chalk.cyan('\nUsage:'));
      console.log(chalk.white('  swarmvector decompile <package>           Decompile npm package'));
      console.log(chalk.white('  swarmvector decompile <pkg>@<ver>         Specific version'));
      console.log(chalk.white('  swarmvector decompile ./bundle.js         Local file'));
      console.log(chalk.white('  swarmvector decompile https://unpkg.com/x URL'));
      console.log(chalk.white('  swarmvector decompile --model <file.gguf> LLM weight file'));
      console.log(chalk.white('  swarmvector decompile --api <model-id>    Probe remote API'));
      console.log(chalk.dim('\nOptions:'));
...
const optimizeCmd = program.command('optimize')
  ...
  .action(async (opts) => {
    ...
    if (opts.list) {
      ...
      console.log('');
      console.log(chalk.dim('  Usage: swarmvector optimize --profile <type>'));
      console.log(chalk.dim('         swarmvector optimize --generate-settings --profile coding'));
      console.log('');
      return;
    }
...
// Borrows metaharness concepts using primitives swarmvector already ships:
...
function buildHarnessSurface() {
  ...
  try {
    ...
    primitives.costRouter = {
      ...
      role: 'cost-optimal model routing (cheap vs strong)',
      available: false,
      install: 'npm install @swarmvector/tiny-dancer',
    };
  }

  // Semantic intent router — @swarmvector/router / swarmvector-router-core
  let semanticAvailable = false;
  try { require.resolve('@swarmvector/router'); semanticAvailable = true; } catch { semanticAvailable = false; }
  primitives.semanticRouter = {
    name: '@swarmvector/router',
    role: 'semantic intent routing',
    available: semanticAvailable,
    ...(semanticAvailable ? { usage: 'npx swarmvector router --route "<text>"' } : { install: 'npm install @swarmvector/router' }),
  };
  ...
  primitives.hooksRouting = {
    ...
    available: true,
    usage: 'npx swarmvector hooks route "<task>"',
  };
  ...
  return {
    adr: 'ADR-256',
    decision: 'borrow metaharness concepts using primitives swarmvector already ships',
    primitives,
    summary: {
...
function printHarnessStatus(opts) {
  ...
  console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
  console.log(chalk.cyan('              SwarmVector Harness Router (ADR-256)'));
  console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
...
// Bare `swarmvector harness` defaults to status
