 ...
 *
 * Model Context Protocol server for SwarmVector hooks
 * Provides self-learning intelligence tools for Claude Code
 *
 * Usage:
 *   npx swarmvector mcp start
...
// .swarmvector/intelligence.json enforce the same contract instead of bypassing
...
// Intelligence class with full SwarmVector stack support
class Intelligence {
  constructor() {
  ...
  // (vectors without provenance) are read-only until `swarmvector hooks reembed`,
  ...
  checkVectorWrite(active) {
    ...
    if (this.isLegacyVectorStore()) {
      ...
      const err = new Error(
        ...
        `${provenanceMod.describeProvenance(active)}. Run 'swarmvector hooks reembed' to re-embed and unlock it.`
      );
  ...
  warnRecallProvenance(active) {
    ...
    if (mismatches.length > 0) {
      ...
      console.error(
        `swarmvector: recall quality degraded — stored vectors are ${provenanceMod.describeProvenance(stored)} ` +
...
const TOOLS = [
  {
    name: 'hooks_stats',
    description: 'Get SwarmVector intelligence statistics including learned patterns, memories, and trajectories',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'hooks_init',
    description: 'Initialize SwarmVector hooks in the current project',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get SwarmVector engine capabilities (VectorDB, SONA, Attention)',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'rvf_examples',
    description: 'List available example .rvf files with download URLs from the swarmvector repository',
    inputSchema: {
      type: 'object',
      properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'hooks_init': {
        let cmd = 'npx swarmvector hooks init';
      ...
      case 'hooks_pretrain': {
        let cmd = 'npx swarmvector hooks pretrain';
      ...
      case 'hooks_build_agents': {
        let cmd = 'npx swarmvector hooks build-agents';
      ...
      case 'hooks_verify': {
        try {
          const output = execSync('npx swarmvector hooks verify', { encoding: 'utf-8', timeout: 15000 });
          return {
            content: [{
              type: 'text',
              text: JSON.stringify({ success: true, output }, null, 2)
      ...
      case 'hooks_doctor': {
        let cmd = 'npx swarmvector hooks doctor';
        if (args.fix) cmd += ' --fix';
            ...
            if (withVectors.length > 0 && provenanceMod) {
              if (intel.isLegacyVectorStore()) {
                const err = new Error(
                  ...
                  `Run 'swarmvector hooks reembed' before importing vector memories.`
                );
                err.code = 'ERR_LEGACY_STORE_READONLY';
                throw err;
              }
      ...
      case 'hooks_ast_analyze': {
        try {
          ...
          const output = execSync(`npx swarmvector hooks ast-analyze "${safeFile}" --json`, { encoding: 'utf-8', timeout: 30000 });
      ...
      case 'hooks_ast_complexity': {
        try {
          ...
          const output = execSync(`npx swarmvector hooks ast-complexity ${filesArg} --threshold ${threshold}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_diff_analyze': {
        try {
          const cmd = args.commit ? `npx swarmvector hooks diff-analyze "${sanitizeShellArg(args.commit)}" --json` : 'npx swarmvector hooks diff-analyze --json';
          const output = execSync(cmd, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
      ...
      case 'hooks_diff_classify': {
        try {
          const cmd = args.commit ? `npx swarmvector hooks diff-classify "${sanitizeShellArg(args.commit)}"` : 'npx swarmvector hooks diff-classify';
          const output = execSync(cmd, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
      ...
      case 'hooks_diff_similar': {
        try {
          ...
          const output = execSync(`npx swarmvector hooks diff-similar -k ${topK} --commits ${commits}`, { encoding: 'utf-8', timeout: 120000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_coverage_route': {
        try {
          const safeFile = sanitizeShellArg(args.file);
          const output = execSync(`npx swarmvector hooks coverage-route "${safeFile}"`, { encoding: 'utf-8', timeout: 15000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_coverage_suggest': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx swarmvector hooks coverage-suggest ${filesArg}`, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_graph_mincut': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx swarmvector hooks graph-mincut ${filesArg}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_graph_cluster': {
        try {
          ...
          const output = execSync(`npx swarmvector hooks graph-cluster ${filesArg} --method ${method} --clusters ${clusters}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_security_scan': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx swarmvector hooks security-scan ${filesArg}`, { encoding: 'utf-8', timeout: 120000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_rag_context': {
        try {
          ...
          const topK = parseInt(args.top_k, 10) || 5;
          let cmd = `npx swarmvector hooks rag-context "${safeQuery}" -k ${topK}`;
      ...
      case 'hooks_git_churn': {
        try {
          ...
          const output = execSync(`npx swarmvector hooks git-churn --days ${days} --top ${top}`, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_route_enhanced': {
        // Issue #463/#422: previously shelled out via `npx swarmvector hooks
      ...
      case 'hooks_gnn_info': {
        ...
        try {
          ...
          gnnInfo = {
            ...
            layers: [
              { name: 'SwarmvectorLayer', description: 'Differentiable vector search layer' },
      ...
      case 'rvf_create': {
        try {
          ...
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message, hint: 'Install @swarmvector/rvf: npm install @swarmvector/rvf' }, null, 2) }], isError: true };
        }
      }
      ...
      case 'brain_search': {
        try {
          ...
          if (e.code === 'MODULE_NOT_FOUND' || e.code === 'ERR_REQUIRE_ESM' || e.code === 'ERR_PACKAGE_PATH_NOT_EXPORTED') {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Brain tools require @swarmvector/pi-brain', hint: 'npm install @swarmvector/pi-brain' }, null, 2) }] };
          }
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }], isError: true };
        }
      }
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  return {
    resources: [
      {
        uri: 'swarmvector://intelligence/stats',
        name: 'Intelligence Stats',
        description: 'Current SwarmVector intelligence statistics',
        mimeType: 'application/json'
      },
      {
        uri: 'swarmvector://intelligence/patterns',
        name: 'Learned Patterns',
        description: 'Q-learning patterns for agent routing',
        mimeType: 'application/json'
      },
      {
        uri: 'swarmvector://intelligence/memories',
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  switch (uri) {
    case 'swarmvector://intelligence/stats':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
    ...
    case 'swarmvector://intelligence/patterns':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
    ...
    case 'swarmvector://intelligence/memories':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
...
async function main() {
  ...
  console.error('SwarmVector MCP server running on stdio');
