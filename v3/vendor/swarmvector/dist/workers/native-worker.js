"use strict";
/**
 * Native Worker Runner for SwarmVector
 *
 ...
 *
 * No delegation to external tools - pure swarmvector execution.
 */
...
async function loadOptionalDeps() {
    try {
        const core = await Promise.resolve().then(() => __importStar(require('@swarmvector/core')));
...
class NativeWorker {
    ...
    async init() {
        ...
        if (this.config.capabilities?.vectorDb && VectorDb) {
            const dbPath = path.join(process.cwd(), '.swarmvector', 'workers', `${this.config.name}.db`);
