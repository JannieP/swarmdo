...
if (rvfRequested) {
    ...
    try {
        implementation = require('@swarmvector/rvf');
        implementationType = 'rvf';
    }
    catch (e) {
        throw new Error('@swarmvector/rvf is not installed.\n' +
            '  Run: npm install @swarmvector/rvf\n' +
    ...
    try {
        ...
        implementation = require('@swarmvector/core');
        ...
        try {
            ...
            console.warn('[SwarmVector] Native module not available:', e.message);
            console.warn('[SwarmVector] RVF module not available:', rvfErr.message);
            console.warn('[SwarmVector] Vector operations will be limited. Install @swarmvector/core or @swarmvector/rvf for full functionality.');
            // Create a stub implementation that provides basic functionality
            implementation = {
                VectorDb: class StubVectorDb {
                    constructor() {
                        console.warn('[SwarmVector] Using stub VectorDb - install @swarmvector/core for native performance');
                    }
...
class VectorIndex {
    constructor(opts) {
        ...
        this._storagePath = require('os').tmpdir() + `/swarmvector-idx-${Date.now()}-${Math.random().toString(36).slice(2)}.db`;
    ...
    async clear() {
        ...
        const newPath = require('os').tmpdir() + `/swarmvector-idx-${Date.now()}-${Math.random().toString(36).slice(2)}.db`;
