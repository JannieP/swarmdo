"use strict";
/**
 * GNN Wrapper - Safe wrapper around @swarmvector/gnn with automatic array conversion
 *
 ...
 *
 * The native @swarmvector/gnn requires Float32Array for maximum performance.
...
exports.TensorCompress = exports.SwarmvectorLayer = void 0;
...
function getGnnModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        gnnModule = require('@swarmvector/gnn');
        return gnnModule;
    }
    catch (e) {
        loadError = new Error(`@swarmvector/gnn is not installed or failed to load: ${e.message}\n` +
            `Install with: npm install @swarmvector/gnn`);
        throw loadError;
    }
}
/**
 ...
 * @example
 * ```typescript
 * import { differentiableSearch } from 'swarmvector/core/gnn-wrapper';
 *
 ...
 */
class SwarmvectorLayer {
    /**
     * Create a new Swarmvector GNN layer
     *
    ...
    constructor(inputDim, hiddenDim, heads, dropout = 0.1) {
        const gnn = getGnnModule();
        this.inner = new gnn.SwarmvectorLayer(inputDim, hiddenDim, heads, dropout);
    }
    /**
    ...
    static fromJson(json) {
        const gnn = getGnnModule();
        const layer = new SwarmvectorLayer(1, 1, 1, 0); // Dummy constructor
        layer.inner = gnn.SwarmvectorLayer.fromJson(json);
        return layer;
    }
}
exports.SwarmvectorLayer = SwarmvectorLayer;
/**
