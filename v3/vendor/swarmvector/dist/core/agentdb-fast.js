 ...
 * - Vector similarity search using @swarmvector/core
...
// Lazy load swarmvector core
let coreModule = null;
function getCoreModule() {
    if (coreModule)
        return coreModule;
    try {
        coreModule = require('@swarmvector/core');
        return coreModule;
    }
    catch {
        // Fallback to swarmvector if core not available
        try {
            coreModule = require('swarmvector');
            return coreModule;
        }
        catch (e) {
            throw new Error(`Neither @swarmvector/core nor swarmvector is available: ${e.message}`);
        }
    }
}
/**
