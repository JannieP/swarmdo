 ...
 *
 * Wraps @swarmvector/graph-node for dependency analysis, co-edit patterns,
...
function getGraphModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        graphModule = require('@swarmvector/graph-node');
        return graphModule;
    }
    catch (e) {
        loadError = new Error(`@swarmvector/graph-node not installed: ${e.message}\n` +
            `Install with: npm install @swarmvector/graph-node`);
        throw loadError;
    }
}
