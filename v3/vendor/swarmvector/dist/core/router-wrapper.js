 ...
 *
 * Wraps @swarmvector/router for vector-based intent classification.
...
function getRouterModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        routerModule = require('@swarmvector/router');
        return routerModule;
    }
    catch (e) {
        loadError = new Error(`@swarmvector/router not installed: ${e.message}\n` +
            `Install with: npm install @swarmvector/router`);
        throw loadError;
    }
}
