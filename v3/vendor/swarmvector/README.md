...
[![npm version](https://badge.fury.io/js/swarmvector.svg)](https://www.npmjs.com/package/swarmvector)
...
**Self-learning intelligence for Claude Code** — SwarmVector provides optimized hooks with ONNX embeddings, AST analysis, and coverage-aware routing.

```bash
# One-command setup with pretrain and agent generation
npx swarmvector hooks init --pretrain --build-agents quality
```
...
SwarmVector includes an MCP server for Claude Code with 97 tools:
...
See `npx swarmvector mcp tools` for the full, authoritative tool list.
...
`npx swarmvector harness status --json` (see `mcp.accessControl`).

### Harness Router (ADR-256)

`swarmvector harness` surfaces the unified routing/agentic primitives swarmvector ships —
...
```bash
npx swarmvector harness status          # human-readable surface + availability
npx swarmvector harness status --json   # structured, for tooling/CI
```
...
npx swarmvector brain agi sona            # SONA patterns, trajectories, ticks
npx swarmvector brain agi temporal        # Knowledge evolution velocity
npx swarmvector brain agi explore         # Meta-learning curiosity & regret
npx swarmvector brain agi midstream       # Scheduler, attractor, solver, strange-loop
npx swarmvector brain agi flags           # Feature flag state
```
...
```bash
npx swarmvector midstream status          # Platform overview
npx swarmvector midstream attractor       # Lyapunov attractor analysis
npx swarmvector midstream scheduler       # Nanosecond scheduler metrics
npx swarmvector midstream benchmark       # Latency benchmark (p50/p90/p99)
```
...
npm install swarmvector --ignore-scripts
...
```javascript
const { VectorDb } = require('swarmvector');
...
npx swarmvector create mydb.vec --dimensions 384 --metric cosine
...
npx swarmvector insert mydb.vec vectors.json
...
npx swarmvector search mydb.vec --vector "[0.1,0.2,0.3,...]" --top-k 10
...
npx swarmvector stats mydb.vec
...
npx swarmvector benchmark --num-vectors 10000 --num-queries 1000
...
#              npm: @swarmvector/core

# Install specific package
npx swarmvector install gnn

# Install all optional packages
npx swarmvector install --all

# Interactive selection
npx swarmvector install -i
```
...
npx swarmvector gnn layer -i 128 -h 256 --test
...
npx swarmvector gnn compress -f embeddings.json -l pq8 -o compressed.json
...
npx swarmvector gnn search -q "[1.0,0.0,0.0]" -c candidates.json -k 5
...
npm install @swarmvector/attention
```
...
npx swarmvector attention info
...
npx swarmvector attention list

# With verbose details
npx swarmvector attention list -v
```
...
npx swarmvector attention benchmark -d 256 -n 100 -i 100
...
npx swarmvector attention hyperbolic -a distance -v "[0.1,0.2,0.3]" -b "[0.4,0.5,0.6]"

# Project vector to Poincaré ball
npx swarmvector attention hyperbolic -a project -v "[1.5,2.0,0.8]"

# Möbius addition in hyperbolic space
npx swarmvector attention hyperbolic -a mobius-add -v "[0.1,0.2]" -b "[0.3,0.4]"

# Exponential map (tangent space → Poincaré ball)
npx swarmvector attention hyperbolic -a exp-map -v "[0.1,0.2,0.3]"
...
SwarmVector includes a pure JavaScript ONNX runtime for local embeddings - no Python, no API calls, no build tools required.
...
npx swarmvector hooks session-start

# End a session (save learned patterns)
npx swarmvector hooks session-end
```
...
npx swarmvector hooks pre-edit src/index.ts
# Output: 🤖 Recommended: typescript-developer (85% confidence)

# After editing - record success/failure for learning
npx swarmvector hooks post-edit src/index.ts --success
npx swarmvector hooks post-edit src/index.ts --error "Type error on line 42"
```
...
npx swarmvector hooks pre-command "npm test"
# Output: ✅ Risk: LOW, Category: test

# After running - record outcome
npx swarmvector hooks post-command "npm test" --success
npx swarmvector hooks post-command "npm test" --error "3 tests failed"
```
...
npx swarmvector hooks route "fix the authentication bug in login.ts"
# Output: 🤖 Recommended: security-specialist (92% confidence)

npx swarmvector hooks route "add unit tests for the API"
...
npx swarmvector hooks remember "API uses JWT tokens with 1h expiry" --type decision
npx swarmvector hooks remember "Database schema in docs/schema.md" --type reference

# Semantic search memory
npx swarmvector hooks recall "authentication mechanism"
...
npx swarmvector hooks suggest-context
...
npx swarmvector hooks swarm-recommend "code-review"
...
npx swarmvector hooks ast-analyze src/index.ts --json
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{ "type": "command", "command": "npx swarmvector hooks pre-edit \"$TOOL_INPUT_file_path\"" }]
      },
      {
        "matcher": "Bash",
        "hooks": [{ "type": "command", "command": "npx swarmvector hooks pre-command \"$TOOL_INPUT_command\"" }]
      }
    ],
    "PostToolUse": [
      {
        "matcher": "Edit|Write|MultiEdit",
        "hooks": [{ "type": "command", "command": "npx swarmvector hooks post-edit \"$TOOL_INPUT_file_path\"" }]
      }
    ],
    "SessionStart": [{ "hooks": [{ "type": "command", "command": "npx swarmvector hooks session-start" }] }],
    "Stop": [{ "hooks": [{ "type": "command", "command": "npx swarmvector hooks session-end" }] }]
  }
}
```
...
**Prerequisites:**
```bash
npm install swarmvector openai
...
**Prerequisites:**
```bash
npm install swarmvector @xenova/transformers
```
...
- **[swarmvector-core](https://www.npmjs.com/package/swarmvector-core)** - Core native bindings (lower-level API)
- **[swarmvector-wasm](https://www.npmjs.com/package/swarmvector-wasm)** - WebAssembly implementation for browsers
- **[swarmvector-cli](https://www.npmjs.com/package/swarmvector-cli)** - Standalone CLI tools
- **[@swarmvector/rvf](https://www.npmjs.com/package/@swarmvector/rvf)** - RVF cognitive container SDK
- **[@swarmvector/rvf-wasm](https://www.npmjs.com/package/@swarmvector/rvf-wasm)** - RVF WASM build for browsers, Deno, and edge
...
- **[swarmvector-core-linux-x64-gnu](https://www.npmjs.com/package/swarmvector-core-linux-x64-gnu)**
- **[swarmvector-core-linux-arm64-gnu](https://www.npmjs.com/package/swarmvector-core-linux-arm64-gnu)**
- **[swarmvector-core-darwin-x64](https://www.npmjs.com/package/swarmvector-core-darwin-x64)**
- **[swarmvector-core-darwin-arm64](https://www.npmjs.com/package/swarmvector-core-darwin-arm64)**
- **[swarmvector-core-win32-x64-msvc](https://www.npmjs.com/package/swarmvector-core-win32-x64-msvc)**
...
```typescript
import { getImplementationType, isRvf } from 'swarmvector';
...
8 RVF-specific subcommands are available through the swarmvector CLI:

```bash
# Create an RVF store
npx swarmvector rvf create mydb.rvf -d 384 --metric cosine

# Ingest vectors from JSON
npx swarmvector rvf ingest mydb.rvf --input vectors.json --format json

# Query nearest neighbors
npx swarmvector rvf query mydb.rvf --vector "[0.1,0.2,...]" --k 10

# File status and segment listing
npx swarmvector rvf status mydb.rvf
npx swarmvector rvf segments mydb.rvf

# COW branching — derive a child file
npx swarmvector rvf derive mydb.rvf --output child.rvf

# Compact and reclaim space
npx swarmvector rvf compact mydb.rvf

# Export to JSON
npx swarmvector rvf export mydb.rvf --output dump.json
```
...
| Any | Deno | WASM (`@swarmvector/rvf-wasm`) |
| Any | Browser | WASM (`@swarmvector/rvf-wasm`) |
| Any | Cloudflare Workers | WASM (`@swarmvector/rvf-wasm`) |
...
If you see "Cannot find module 'swarmvector-core-*'":

```bash
# Reinstall with optional dependencies
npm install --include=optional swarmvector
...
2. **Rebuild native module**: `npm rebuild swarmvector`
3. **Verify native**: `npx swarmvector info` should show "native (Rust)"
