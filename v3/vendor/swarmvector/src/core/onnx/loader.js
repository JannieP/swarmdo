/**
 * Model Loader for SwarmVector ONNX Embeddings WASM
 *
...
export class ModelLoader {
    constructor(options = {}) {
        ...
        this.cacheStorage = options.cacheStorage ?? 'swarmvector-models';
...
export async function createEmbedder(modelName = DEFAULT_MODEL, wasmModule = null) {
    ...
    if (!wasmModule) {
        ...
        // (pkg/swarmvector_onnx_embeddings_wasm.js) BOTH starts with
        ...
        if (typeof process !== 'undefined' && process.versions?.node) {
            ...
            const bgJs = path.join(pkgDir, 'swarmvector_onnx_embeddings_wasm_bg.js');
            const bgWasm = path.join(pkgDir, 'swarmvector_onnx_embeddings_wasm_bg.wasm');
            ...
            wasmModule = await import('./pkg/swarmvector_onnx_embeddings_wasm.js');
