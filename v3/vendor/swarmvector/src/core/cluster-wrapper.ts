...
function getClusterModule() {
  ...
  try {
    ...
    loadError = new Error(
      `@swarmvector/cluster not installed: ${e.message}\n` +
 ...
 */
export class SwarmvectorCluster {
  private inner: any;
 ...
 */
export function createCluster(config: ClusterConfig): SwarmvectorCluster {
  return new SwarmvectorCluster(config);
}

export default SwarmvectorCluster;
