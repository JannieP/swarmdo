/**
 * RVF Wrapper - Persistent vector store via @swarmvector/rvf
 *
 * Wraps @swarmvector/rvf RvfDatabase through thin convenience functions.
...
function getRvfModule() {
  ...
  try {
    rvfModule = require('@swarmvector/rvf');
    return rvfModule;
  } catch (e: any) {
    loadError = new Error(
      '@swarmvector/rvf is not installed. Run: npm install @swarmvector/rvf'
    );
    throw loadError;
  }
}
...
// ---------------------------------------------------------------------------
// Minimal inline types (mirrors @swarmvector/rvf/types when package absent)
// ---------------------------------------------------------------------------
