...
class ClaudeIntegrationCore {
  ...
  /**
     * Add ruf-swarm MCP server to Claude Code
     */
  async addMcpServer() {
    ...
    try {
      // Add ruf-swarm MCP server using stdio (no port needed)
      const mcpCommand = 'claude mcp add ruf-swarm npx ruf-swarm mcp start';
      execSync(mcpCommand, { stdio: 'inherit', cwd: this.workingDir });
      return { success: true, message: 'Added ruf-swarm MCP server to Claude Code (stdio)' };
    } catch (error) {
