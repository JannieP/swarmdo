...
class ClaudeDocsGenerator {
  ...
  async generateClaudeMd(options = {}) {
    ...
    const content = `# Claude Code Configuration for ruf-swarm
...
[Single Message with BatchTool]:
  mcp__ruf-swarm__swarm_init { topology: "mesh", maxAgents: 6 }
  mcp__ruf-swarm__agent_spawn { type: "researcher" }
  mcp__ruf-swarm__agent_spawn { type: "coder" }
  mcp__ruf-swarm__agent_spawn { type: "analyst" }
  mcp__ruf-swarm__agent_spawn { type: "tester" }
  mcp__ruf-swarm__agent_spawn { type: "coordinator" }
...
\`\`\`javascript
Message 1: mcp__ruf-swarm__swarm_init
Message 2: mcp__ruf-swarm__agent_spawn 
Message 3: mcp__ruf-swarm__agent_spawn
...
\`\`\`bash
# Add ruf-swarm MCP server to Claude Code using stdio
claude mcp add ruf-swarm npx ruf-swarm mcp start
\`\`\`

### 2. Use MCP Tools for Coordination in Claude Code
Once configured, ruf-swarm MCP tools enhance Claude Code's coordination:

**Initialize a swarm:**
- Use the \`mcp__ruf-swarm__swarm_init\` tool to set up coordination topology
- Choose: mesh, hierarchical, ring, or star
- This creates a coordination framework for Claude Code's work

**Spawn agents:**
- Use \`mcp__ruf-swarm__agent_spawn\` tool to create specialized coordinators
- Agent types represent different thinking patterns, not actual coders
- They help Claude Code approach problems from different angles

**Orchestrate tasks:**
- Use \`mcp__ruf-swarm__task_orchestrate\` tool to coordinate complex workflows
...
### Coordination Tools:
- \`mcp__ruf-swarm__swarm_init\` - Set up coordination topology for Claude Code
- \`mcp__ruf-swarm__agent_spawn\` - Create cognitive patterns to guide Claude Code
- \`mcp__ruf-swarm__task_orchestrate\` - Break down and coordinate complex tasks

### Monitoring Tools:
- \`mcp__ruf-swarm__swarm_status\` - Monitor coordination effectiveness
- \`mcp__ruf-swarm__agent_list\` - View active cognitive patterns
- \`mcp__ruf-swarm__agent_metrics\` - Track coordination performance
- \`mcp__ruf-swarm__task_status\` - Check workflow progress
- \`mcp__ruf-swarm__task_results\` - Review coordination outcomes

### Memory & Neural Tools:
- \`mcp__ruf-swarm__memory_usage\` - Persistent memory across sessions
- \`mcp__ruf-swarm__neural_status\` - Neural pattern effectiveness
- \`mcp__ruf-swarm__neural_train\` - Improve coordination patterns
- \`mcp__ruf-swarm__neural_patterns\` - Analyze thinking approaches

### System Tools:
- \`mcp__ruf-swarm__benchmark_run\` - Measure coordination efficiency
- \`mcp__ruf-swarm__features_detect\` - Available capabilities
- \`mcp__ruf-swarm__swarm_monitor\` - Real-time coordination tracking
...
- Tool: \`mcp__ruf-swarm__swarm_init\`
- Parameters: \`{"topology": "mesh", "maxAgents": 5, "strategy": "balanced"}\`
- Result: Creates a mesh topology for comprehensive exploration

**Step 2:** Define research perspectives
- Tool: \`mcp__ruf-swarm__agent_spawn\`
...
- Tool: \`mcp__ruf-swarm__task_orchestrate\`
- Parameters: \`{"task": "Research neural architecture search papers", "strategy": "adaptive"}\`
- Result: Claude Code systematically searches, reads, and analyzes papers

**What Actually Happens:**
1. The swarm sets up a coordination framework
2. Each agent MUST use ruf-swarm hooks for coordination:
   - \`npx ruf-swarm hook pre-task\` before starting
   - \`npx ruf-swarm hook post-edit\` after each file operation
   - \`npx ruf-swarm hook notification\` to share decisions
...
4. Agents share progress through ruf-swarm memory
...
When using ruf-swarm coordination with Claude Code:
...
ruf-swarm includes powerful hooks that automate coordination:
...
4. **ALL AGENTS MUST USE COORDINATION TOOLS** - Every spawned agent MUST use ruf-swarm hooks and memory
...
npx ruf-swarm hook pre-task --description "[agent task]" --auto-spawn-agents false
npx ruf-swarm hook session-restore --session-id "swarm-[id]" --load-memory true
\`\`\`

**2️⃣ DURING Work (After EVERY Major Step):**
\`\`\`bash
# Store progress in memory after each file operation
npx ruf-swarm hook post-edit --file "[filepath]" --memory-key "swarm/[agent]/[step]"

# Store decisions and findings
npx ruf-swarm hook notification --message "[what was done]" --telemetry true

# Check coordination with other agents
npx ruf-swarm hook pre-search --query "[what to check]" --cache-results true
\`\`\`

**3️⃣ AFTER Completing Work:**
\`\`\`bash
# Save all results and learnings
npx ruf-swarm hook post-task --task-id "[task]" --analyze-performance true
npx ruf-swarm hook session-end --export-metrics true --generate-summary true
\`\`\`
...
1. START: Run \`npx ruf-swarm hook pre-task --description "[your task]"\`
2. DURING: After EVERY file operation, run \`npx ruf-swarm hook post-edit --file "[file]" --memory-key "agent/[step]"\`
3. MEMORY: Store ALL decisions using \`npx ruf-swarm hook notification --message "[decision]"\`
4. END: Run \`npx ruf-swarm hook post-task --task-id "[task]" --analyze-performance true\`
...
Message 1: [BatchTool]
  ...
  - mcp__ruf-swarm__agent_spawn (researcher)
  - mcp__ruf-swarm__agent_spawn (coder)
  - mcp__ruf-swarm__agent_spawn (analyst)
  - mcp__ruf-swarm__agent_spawn (tester)
  - mcp__ruf-swarm__agent_spawn (coordinator)
...
[BatchTool]:
  - mcp__ruf-swarm__swarm_init { topology: "hierarchical", maxAgents: 8, strategy: "parallel" }
  - mcp__ruf-swarm__agent_spawn { type: "architect", name: "System Designer" }
  - mcp__ruf-swarm__agent_spawn { type: "coder", name: "API Developer" }
  - mcp__ruf-swarm__agent_spawn { type: "coder", name: "Frontend Dev" }
  - mcp__ruf-swarm__agent_spawn { type: "analyst", name: "DB Designer" }
  - mcp__ruf-swarm__agent_spawn { type: "tester", name: "QA Engineer" }
  - mcp__ruf-swarm__agent_spawn { type: "researcher", name: "Tech Lead" }
  - mcp__ruf-swarm__agent_spawn { type: "coordinator", name: "PM" }
  - TodoWrite { todos: [multiple todos at once] }

STEP 2: PARALLEL TASK EXECUTION (Single Message!)
[BatchTool]:
  - mcp__ruf-swarm__task_orchestrate { task: "main task", strategy: "parallel" }
  - mcp__ruf-swarm__memory_usage { action: "store", key: "init", value: {...} }
...
[BatchTool - Message 1]:
  ...
  mcp__ruf-swarm__agent_spawn { type: "architect", name: "System Designer" }
  mcp__ruf-swarm__agent_spawn { type: "coder", name: "API Developer" }
  mcp__ruf-swarm__agent_spawn { type: "coder", name: "Auth Expert" }
  mcp__ruf-swarm__agent_spawn { type: "analyst", name: "DB Designer" }
  mcp__ruf-swarm__agent_spawn { type: "tester", name: "Test Engineer" }
  mcp__ruf-swarm__agent_spawn { type: "coordinator", name: "Lead" }
  ...
  // Start orchestration
  mcp__ruf-swarm__task_orchestrate { task: "Build REST API", strategy: "parallel" }
  
  // Store initial memory
  mcp__ruf-swarm__memory_usage { action: "store", key: "project/init", value: { started: Date.now() } }
...
Message 1: mcp__ruf-swarm__swarm_init
Message 2: mcp__ruf-swarm__agent_spawn (just one agent)
Message 3: mcp__ruf-swarm__agent_spawn (another agent)
...
4. **Monitor Progress**: Use mcp__ruf-swarm__swarm_monitor for real-time tracking
...
- Documentation: https://github.com/ruvnet/ruv-FANN/tree/main/ruf-swarm
- Issues: https://github.com/ruvnet/ruv-FANN/issues
- Examples: https://github.com/ruvnet/ruv-FANN/tree/main/ruf-swarm/examples

---

Remember: **ruf-swarm coordinates, Claude Code creates!** Start with \`mcp__ruf-swarm__swarm_init\` to enhance your development workflow.
`;
...
The orchestrator creates a plan that Claude Code follows using its native tools.`,
      ...
      'monitoring/status.md': {
        ...
        tool: 'mcp__ruf-swarm__swarm_status',
        params: '{"verbose": true}',
...
- Overall system health`,
      ...
      'monitoring/agents.md': {
        ...
        tool: 'mcp__ruf-swarm__agent_list',
...
- **busy**: Patterns actively coordinating tasks`,
      ...
      'memory/usage.md': {
        ...
        tool: 'mcp__ruf-swarm__memory_usage',
...
- Store coordination strategies that worked well`,
      ...
      'memory/neural.md': {
        ...
        tool: 'mcp__ruf-swarm__neural_train',
...
Remember: All code is written by Claude Code using its native tools!`,
      ...
      'hooks/overview.md': {
        ...
        content: `# Claude Code Hooks for ruf-swarm
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruf-swarm hook pre-edit --file '\${tool.params.file_path}'"
        }]
      }
    ]
  }
...
- [Session End Hook](./session-end.md)`,
      ...
      'hooks/setup.md': {
        ...
        content: `# Setting Up ruf-swarm Hooks

## Quick Start

### 1. Initialize with Hooks
\`\`\`bash
npx ruf-swarm init --claude --force
\`\`\`
...
\`\`\`bash
# Test pre-edit hook
npx ruf-swarm hook pre-edit --file test.js --ensure-coordination

# Test session summary
npx ruf-swarm hook session-end --generate-summary
\`\`\`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruf-swarm hook custom-pre-write --file '\${tool.params.file_path}'"
        }]
      }
    ]
  }
...
\`\`\`bash
# Enable debug output
export RUF_SWARM_HOOK_DEBUG=true

# Test specific hook
npx ruf-swarm hook pre-edit --file app.js --debug
\`\`\`
...
\`\`\`json
{
  ...
  "hooks": [{
    "type": "command",
    "command": "npx ruf-swarm hook check-protected --file '\${tool.params.file_path}'"
  }]
}
\`\`\`
...
`;
  ...
  async generateSettingsJson() {
    const settings = {
      env: {
        RUF_SWARM_AUTO_COMMIT: 'false',
        RUF_SWARM_AUTO_PUSH: 'false',
        RUF_SWARM_HOOKS_ENABLED: 'false',
  ...
  /**
     * Merge ruf-swarm content with existing CLAUDE.md
     */
  async mergeClaudeMd(filePath, noBackup = false) {
    try {
      const existingContent = await fs.readFile(filePath, 'utf8');

      console.log('📝 Merging ruf-swarm configuration with existing CLAUDE.md');
      ...
      // Generate new ruf-swarm content
      const rufSwarmContent = this.getRufSwarmContent();

      // Intelligent merging
      const mergedContent = this.intelligentMerge(existingContent, rufSwarmContent);

      // Write merged content
      await fs.writeFile(filePath, mergedContent);

      console.log('✅ Successfully merged ruf-swarm configuration with existing CLAUDE.md');
  ...
  /**
     * Get the ruf-swarm specific content (full content from generateClaudeMd)
     */
  getRufSwarmContent() {
    // Return the complete ruf-swarm configuration content
    const content = `# Claude Code Configuration for ruf-swarm
...
Remember: **ruf-swarm coordinates, Claude Code creates!** Start with \`mcp__ruf-swarm__swarm_init\` to enhance your development workflow.`;
  ...
  /**
     * Intelligently combine ruf-swarm content with existing content
     */
  intelligentMerge(existingContent, rufSwarmContent) {
    const existingLines = existingContent.split('\n');
    const newLines = rufSwarmContent.split('\n');

    // Check if ruf-swarm content already exists
    const rufSwarmSectionIndex = this.findRufSwarmSection(existingLines);

    if (rufSwarmSectionIndex !== -1) {
      // Replace existing ruf-swarm section
      console.log('📝 Updating existing ruf-swarm section in CLAUDE.md');
      const sectionEnd = this.findSectionEnd(existingLines, rufSwarmSectionIndex);

      // Replace the section
      const beforeSection = existingLines.slice(0, rufSwarmSectionIndex);
      const afterSection = existingLines.slice(sectionEnd);

      return [...beforeSection, ...newLines, '', ...afterSection].join('\n');
    }
    // Intelligently insert ruf-swarm content
    console.log('📝 Integrating ruf-swarm configuration into existing CLAUDE.md');
    return this.intelligentInsert(existingLines, newLines);

  }

  /**
     * Find existing ruf-swarm section in content
     */
  findRufSwarmSection(lines) {
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].toLowerCase();
      if (line.includes('ruf-swarm') && (line.startsWith('#') || line.includes('claude code configuration'))) {
        return i;
      }
    }
    return -1;
  ...
  findSectionEnd(lines, startIndex) {
    ...
    for (let i = startIndex + 1; i < lines.length; i++) {
      if (lines[i].startsWith('# ') && !lines[i].includes('ruf-swarm')) {
        return i;
      }
