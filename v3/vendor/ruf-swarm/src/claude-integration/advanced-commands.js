...
\`\`\`json
{
  "command": "npx ruf-swarm hook pre-task --auto-spawn-agents --optimize-topology"
}
\`\`\``,
      },
...
## Monitoring
\`\`\`
Tool: mcp__ruf-swarm__swarm_monitor
...
\`\`\`
Tool: mcp__ruf-swarm__task_results
...
npx ruf-swarm hook session-end --export-metrics
...
\`\`\`
Tool: mcp__ruf-swarm__neural_status
Result: {
  "patterns": {
    "convergent": 0.92,
    "divergent": 0.87,
...
\`\`\`json
{
  "hooks": [{
    "matcher": "^Task$",
    "command": "npx ruf-swarm hook pre-task --auto-spawn-agents"
  }]
}
\`\`\`
...
\`\`\`json
{
  "PostToolUse": [{
    "matcher": "^Bash$",
    "command": "npx ruf-swarm hook post-bash --exit-code '\${tool.result.exitCode}' --auto-recover"
  }]
}
\`\`\`
...
npx ruf-swarm hook session-restore --session-id "sess-123"
\`\`\`
...
rm .ruf-swarm/session-*.json

# Disable memory
export RUF_SWARM_MEMORY_PERSIST=false
\`\`\`
