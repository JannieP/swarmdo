#!/usr/bin/env node
/**
 * Diagnostic CLI for ruf-swarm
 * Usage: npx ruf-swarm diagnose [options]
 */
...
function showLoggingConfig(logger) {
  ...
  console.log('\n💡 Examples:');
  console.log('   LOG_LEVEL=DEBUG npx ruf-swarm mcp start');
  console.log('   MCP_LOG_LEVEL=TRACE TOOLS_LOG_LEVEL=DEBUG npx ruf-swarm mcp start');
  console.log('   LOG_TO_FILE=true LOG_DIR=./mylogs npx ruf-swarm mcp start');
}
...
function showHelp() {
  console.log(`
🔍 ruf-swarm Diagnostics

Usage: npx ruf-swarm diagnose <command> [options]
...
Examples:
  npx ruf-swarm diagnose test
  npx ruf-swarm diagnose report --output=report.json
  npx ruf-swarm diagnose monitor --duration=120
  npx ruf-swarm diagnose logs --pattern="connection.*failed"
  npx ruf-swarm diagnose config
`);
}
