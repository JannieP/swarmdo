...
class WASMLoader {
  ...
  async loadModule() {
    ...
    try {
      const wasmJsPath = path.join(this.wasmPath, 'ruf_swarm_wasm.js');
    ...
    const moduleFile = this.useSIMD ? 'ruf_swarm_simd.wasm' : 'ruf_swarm_wasm_bg.wasm';
...
class RufSwarm {
  ...
  static async initialize(options = {}) {
    ...
    const wasmJsPath = path.join(loader.wasmPath, 'ruf_swarm_wasm.js');
    const bindings = await import(path.resolve(wasmJsPath));

    // Initialize WASM module with file buffer for Node.js
    if (bindings.default) {
      const wasmPath = path.join(loader.wasmPath, 'ruf_swarm_wasm_bg.wasm');
    ...
    return new RufSwarm(bindings, options);
  }
  ...
  static getRuntimeFeatures() {
    if (!wasmInstance) {
      throw new Error('RufSwarm not initialized. Call RufSwarm.initialize() first.');
    }
  ...
  async createSwarm(config) {
    try {
      const swarm = new this._wasmSwarm.RufSwarm(config);
