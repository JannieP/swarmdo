...
class WasmModuleLoader {
  constructor() {
    ...
    this.moduleManifest = {
      ...
      swarm: {
        path: './wasm/ruf-swarm-orchestration.wasm',
        size: 768 * 1024, // 768KB
        priority: 'high',
        dependencies: ['core'],
      ...
      persistence: {
        path: './wasm/ruf-swarm-persistence.wasm',
        size: 256 * 1024, // 256KB
        priority: 'high',
        dependencies: ['core'],
  ...
  async loadCoreModule() {
    ...
    try {
      ...
      const wasmJsUrl = new URL('../wasm/ruf_swarm_wasm.js', import.meta.url).href;
      ...
      if (bindings.default && typeof window === 'undefined') {
        const wasmPath = path.join(this.baseDir, 'wasm', 'ruf_swarm_wasm_bg.wasm');
        try {
