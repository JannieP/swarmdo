 ...
 * Contains all benchmark and testing functionality for ruf-swarm MCP tools
 */
...
export class MCPBenchmarks {
  constructor(rufSwarm, persistence) {
    this.rufSwarm = rufSwarm;
  ...
  async runBenchmarks(type = 'all', iterations = 10) {
    ...
    const result = {
      ...
      environment: {
        ...
        memory_usage_mb: this.rufSwarm?.wasmLoader?.getTotalMemoryUsage() / (1024 * 1024) || 0,
        runtime_features: this.rufSwarm?.getRuntimeFeatures ? this.rufSwarm.getRuntimeFeatures() : {},
      },
