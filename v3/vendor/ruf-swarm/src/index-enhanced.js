...
class RufSwarm {
   ...
   */
  destroy() {
    console.log('🧹 Cleaning up RufSwarm instance...');
  ...
  static async initialize(options = {}) {
    ...
    // Register RufSwarm factory if not already registered
    if (!container.has('RufSwarm')) {
      container.register('RufSwarm', () => new RufSwarm(), {
        singleton: true,
        lazy: false,
      });
    }
    ...
    const instance = container.get('RufSwarm');
    ...
    if (instance.isInitialized) {
      if (debug) {
        console.log('[DEBUG] RufSwarm already initialized through container');
      }
      return instance;
    }

    console.log('🧠 Initializing ruf-swarm with WASM capabilities...');
    ...
    try {
      ...
      console.log('✅ ruf-swarm initialized successfully');
      ...
      console.error('❌ Failed to initialize ruf-swarm:', error);
      throw error;
    }
  }
  ...
  async detectFeatures(useSIMD = true) {
    try {
      ...
      // Detect SIMD support
      if (useSIMD) {
        this.features.simd_support = RufSwarm.detectSIMDSupport();
      }
  ...
  async createSwarm(config) {
    ...
    let wasmSwarm;
    if (coreModule.exports && coreModule.exports.RufSwarm) {
      try {
        wasmSwarm = new coreModule.exports.RufSwarm();
  ...
  static getRuntimeFeatures() {
    return {
      ...
      simd: RufSwarm.detectSIMDSupport(),
  ...
  detectSIMDSupport() {
    return RufSwarm.detectSIMDSupport();
  }
}

// Enhanced Swarm wrapper class
class Swarm {
  constructor(id, wasmInstance, rufSwarmInstance) {
    this.id = id;
  ...
  async spawn(config) {
    ...
    if (enableNeuralNetwork && this.rufSwarm.features.neural_networks) {
      await this.rufSwarm.wasmLoader.loadModule('neural');
    }
    ...
    if (this.rufSwarm.persistence && !id) {
      try {
        this.rufSwarm.persistence.createAgent({
          id: agentId,
          swarmId: this.id,
          name: result.name,
          type,
  ...
  async orchestrate(taskConfig) {
    ...
    if (this.rufSwarm.persistence) {
      await this.rufSwarm.persistence.createTask({
        id: taskId,
        swarmId: this.id,
        description,
        priority,
  ...
  async terminate() {
    ...
    this.rufSwarm.activeSwarms.delete(this.id);
  }
}
...
export { RufSwarm, Swarm, Agent, Task, DAAService, daaService };