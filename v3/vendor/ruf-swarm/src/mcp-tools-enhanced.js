...
class EnhancedMCPTools {
  constructor(rufSwarmInstance = null) {
    this.rufSwarm = rufSwarmInstance;
  ...
  async initialize(rufSwarmInstance = null) {
    ...
    try {
      ...
      for (const swarmData of swarmsArray) {
        try {
          ...
          const swarm = await this.rufSwarm.createSwarm({
            id: swarmData.id,
            name: swarmData.name,
  ...
  async swarm_init(params) {
    ...
    try {
      ...
      // Ensure we have a RufSwarm instance (but don't re-initialize)
      if (!this.rufSwarm) {
        try {
          this.logger.debug('RufSwarm not initialized, initializing now');
          await this.initialize();
          this.logger.debug('RufSwarm initialized successfully');
        } catch (error) {
          this.logger.error('Failed to initialize RufSwarm', { error });
          throw ErrorFactory.createError('wasm',
            'Failed to initialize RufSwarm WASM module',
      ...
      if (enableForecasting && this.rufSwarm.features.forecasting) {
        await this.rufSwarm.wasmLoader.loadModule('forecasting');
      }
      ...
      const result = {
        ...
        maxAgents,
        features: {
          cognitive_diversity: enableCognitiveDiversity && this.rufSwarm.features.cognitive_diversity,
          neural_networks: enableNeuralAgents && this.rufSwarm.features.neural_networks,
          forecasting: enableForecasting && this.rufSwarm.features.forecasting,
          simd_support: this.rufSwarm.features.simd_support,
        },
        created: new Date().toISOString(),
        performance: {
          initialization_time_ms: performance.now() - startTime,
          memory_usage_mb: this.rufSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
        },
      };
      ...
      } else if (!(error instanceof ValidationError || error instanceof RufSwarmError)) {
  ...
  async swarm_status(params) {
    ...
    try {
      ...
      if (swarmId) {
        ...
        status.wasm_metrics = {
          memory_usage_mb: this.rufSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          loaded_modules: this.rufSwarm.wasmLoader.getModuleStatus(),
          features: this.rufSwarm.features,
        };
      ...
      const globalMetrics = await this.rufSwarm.getGlobalMetrics();
      const allSwarms = await this.rufSwarm.getAllSwarms();
      ...
      const result = {
        ...
        runtime_info: {
          features: this.rufSwarm.features,
          wasm_modules: this.rufSwarm.wasmLoader.getModuleStatus(),
  ...
  async benchmark_run(params) {
    ...
    try {
      ...
      if (type === 'all' || type === 'neural') {
        if (includeNeuralBenchmarks && this.rufSwarm.features.neural_networks) {
      ...
      const result = {
        ...
        environment: {
          features: this.rufSwarm.features,
          memory_usage_mb: this.rufSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          runtime_features: RufSwarm.getRuntimeFeatures(),
        },
        performance: {
  ...
  async features_detect(params) {
    ...
    try {
      ...
      const features = {
        runtime: RufSwarm.getRuntimeFeatures(),
        wasm: {
          modules_loaded: this.rufSwarm.wasmLoader.getModuleStatus(),
          total_memory_mb: this.rufSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
          simd_support: this.rufSwarm.features.simd_support,
        },
        ruf_swarm: this.rufSwarm.features,
        neural_networks: {
          available: this.rufSwarm.features.neural_networks,
          activation_functions: this.rufSwarm.features.neural_networks ? 18 : 0,
          training_algorithms: this.rufSwarm.features.neural_networks ? 5 : 0,
          cascade_correlation: this.rufSwarm.features.neural_networks,
        },
        forecasting: {
          available: this.rufSwarm.features.forecasting,
          models_available: this.rufSwarm.features.forecasting ? 27 : 0,
          ensemble_methods: this.rufSwarm.features.forecasting,
        },
        cognitive_diversity: {
          available: this.rufSwarm.features.cognitive_diversity,
          patterns_available: this.rufSwarm.features.cognitive_diversity ? 5 : 0,
          pattern_optimization: this.rufSwarm.features.cognitive_diversity,
        },
      };
  ...
  async memory_usage(params) {
    ...
    try {
      ...
      const wasmMemory = this.rufSwarm.wasmLoader.getTotalMemoryUsage();
      const jsMemory = RufSwarm.getMemoryUsage();
      ...
      if (detail === 'detailed') {
        ...
        const moduleStatus = this.rufSwarm.wasmLoader.getModuleStatus();
  ...
  async neural_status(params) {
    ...
    try {
      ...
      if (!this.rufSwarm.features.neural_networks) {
        return {
          available: false,
      ...
      const result = {
        ...
        simd_acceleration: this.rufSwarm.features.simd_support,
  ...
  async neural_train(params) {
    ...
    try {
      ...
      for (let i = 1; i <= iterations; i++) {
        ...
        if (this.rufSwarm.wasmLoader.modules.get('core')?.neural_train) {
          try {
            this.rufSwarm.wasmLoader.modules.get('core').neural_train({
      ...
      try {
        await this.persistence.updateNeuralNetwork(neuralNetwork.id, {
          ...
          weights: {
            ...
            values: this.rufSwarm.wasmLoader.modules.get('core')?.get_neural_weights?.(neuralNetwork.id) || {},
          },
  ...
  async runWasmBenchmarks(iterations) {
    ...
    for (let i = 0; i < iterations; i++) {
      try {
        ...
        const coreModule = await this.rufSwarm.wasmLoader.loadModule('core');
  ...
  async swarm_monitor(params) {
    ...
    try {
      ...
      monitoringData.system_metrics = {
        ...
        wasm_memory_usage_mb: this.rufSwarm.wasmLoader.getTotalMemoryUsage() / (1024 * 1024),
        system_uptime_ms: Date.now() - (this.systemStartTime || Date.now()),
        features_available: Object.keys(this.rufSwarm.features).filter(f => this.rufSwarm.features[f]).length,
      };
