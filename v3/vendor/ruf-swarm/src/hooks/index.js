/**
 * Claude Code Hooks Implementation for ruf-swarm
...
class RufSwarmHooks {
  ...
  async preEditHook(args) {
    ...
    if (!swarmStatus.initialized) {
      return {
        continue: false,
        reason: 'Swarm not initialized - run mcp__ruf-swarm__swarm_init first',
  ...
  async notificationHook(args) {
    ...
    if (sendTelemetry && process.env.RUF_SWARM_TELEMETRY_ENABLED === 'true') {
  ...
  async mcpSwarmInitializedHook(args) {
    ...
    if (persistConfig) {
      const configDir = path.join(process.cwd(), '.ruf-swarm');
  ...
  async mcpAgentSpawnedHook(args) {
    ...
    if (updateRoster) {
      ...
      // Persist roster
      const rosterPath = path.join(process.cwd(), '.ruf-swarm', 'agent-roster.json');
  ...
  async mcpNeuralTrainedHook(args) {
    ...
    // Save neural weights
    if (saveWeights) {
      const weightsDir = path.join(process.cwd(), '.ruf-swarm', 'neural-weights');
  ...
  async agentCompleteHook(args) {
    ...
    try {
      ...
      let reportPath = null;
      if (generateReport) {
        const reportDir = path.join(process.cwd(), '.ruf-swarm', 'agent-reports');
...
---
*Generated by ruf-swarm agent coordination system*
`;
...
🤖 Generated by ruf-swarm agent coordination
Co-Authored-By: ${agentName} <agent@ruf-swarm.ai>`;
...
EOF
            ...
            if (pushToGithub && process.env.RUF_SWARM_AUTO_PUSH === 'true') {
  ...
  async sessionRestoreHook(args) {
    ...
    try {
      const sessionDir = path.join(process.cwd(), '.ruf-swarm');
  ...
  async checkSwarmStatus() {
    try {
      ...
      const statusFile = path.join(process.cwd(), '.ruf-swarm', 'status.json');
  ...
  generateSessionSummary() {
    ...
    return `# ruf-swarm Session Summary
...
`;
  ...
  async updateKnowledgeBase(type, data) {
    const kbPath = path.join(process.cwd(), '.ruf-swarm', 'knowledge-base.json');
  ...
  async getSwarmStatus() {
    try {
      const statusPath = path.join(process.cwd(), '.ruf-swarm', 'status.json');
  ...
  sendTelemetry(event, data) {
    ...
    const telemetryPath = path.join(process.cwd(), '.ruf-swarm', 'telemetry.jsonl');
...
const hooksInstance = new RufSwarmHooks();
