...
**What if every task, every file, every function could truly think?** Just for a moment. No LLM required. That's what ruf-swarm makes real.

[![npm version](https://badge.fury.io/js/ruf-swarm.svg)](https://www.npmjs.com/package/ruf-swarm)
[![npm downloads](https://img.shields.io/npm/dt/ruf-swarm.svg)](https://www.npmjs.com/package/ruf-swarm)
[![npm downloads per month](https://img.shields.io/npm/dm/ruf-swarm.svg)](https://www.npmjs.com/package/ruf-swarm)
[![MCP Registry](https://img.shields.io/badge/MCP%20Registry-io.github.ruvnet%2Fruv--swarm-blue)](https://registry.modelcontextprotocol.io/servers/io.github.ruvnet/ruf-swarm)
...
```bash
npx ruf-swarm@1.0.19 init --claude
```

ruf-swarm lets you spin up ultra-lightweight custom neural networks that exist just long enough to solve the problem. Tiny purpose-built brains dedicated to solving very specific challenges.
...
Get started with ruf-swarm in under 2 minutes:

```bash
# Try instantly with npx
npx ruf-swarm init mesh 5
npx ruf-swarm spawn researcher "AI Research Agent"
npx ruf-swarm orchestrate "Research the latest advances in neural architecture search"

# Use Claude Code hooks for automated coordination
npx ruf-swarm hook pre-task --description "Your task description"
npx ruf-swarm hook post-task --task-id "task-123" --analyze-performance true
```

Or use programmatically:

```javascript
import { RufSwarm } from 'ruf-swarm';

// Initialize with cognitive diversity
const swarm = await RufSwarm.initialize({
  topology: 'mesh',
...
npm install ruf-swarm

# Global CLI installation (recommended for servers)
npm install -g ruf-swarm

# Development installation
npm install ruf-swarm --save-dev
```

### 🤖 MCP (Model Context Protocol) Installation

ruf-swarm is available in the **official MCP Registry**! Add it to your AI clients:
...
```json
{
  "mcpServers": {
    "ruf-swarm": {
      "command": "npx",
      "args": ["ruf-swarm@1.0.19", "mcp", "start"],
      "env": {
        "RUF_SWARM_LOG_LEVEL": "info",
        "RUF_SWARM_ENABLE_SIMD": "true"
      }
    }
  }
}
...
```bash
# Start MCP server
npx ruf-swarm@1.0.19 mcp start

# View available MCP tools
npx ruf-swarm@1.0.19 mcp tools
```

🔗 **Registry**: [io.github.ruvnet/ruf-swarm](https://registry.modelcontextprotocol.io/servers/io.github.ruvnet/ruf-swarm)

### ⚠️ WASM Requirements

**Important**: ruf-swarm requires WebAssembly support. Ensure your environment meets these requirements:
...
npx ruf-swarm --help
npx ruf-swarm init --claude
npx ruf-swarm init mesh 10
npx ruf-swarm benchmark --test swe-bench

# Instant MCP server for Claude Code
npx ruf-swarm mcp start --port 3000

# Remote server deployment
ssh user@remote-server 'npx ruf-swarm init hierarchical 20'
```
...
docker run -p 3000:3000 ruvnet/ruf-swarm:latest

# With MCP server
docker run -p 3000:3000 -e MCP_ENABLED=true ruvnet/ruf-swarm:latest
```
...
ruf-swarm implements cognitive diversity through specialized agent archetypes:
...
```javascript
const { RufSwarm } = require('ruf-swarm');
...
class AIProjectOrchestrator {
  ...
  async initialize(config: AIProjectConfig): Promise<void> {
    ...
    this.swarm = await RufSwarm.initialize(swarmConfig);
  }
...
├──────────────────────────────────────────┤
│         ruf-swarm Core (Rust)           │ ← Agent Logic & Orchestration
├──────────────────────────────────────────┤
...
ruf-swarm provides native integration with Claude Code through the Model Context Protocol (MCP):
...
# Check server status
npx ruf-swarm mcp status

# List available tools
npx ruf-swarm mcp tools
```
...
Add ruf-swarm to your Claude MCP configuration:
...
| Benchmark | ruf-swarm | Claude 3.7 Sonnet | GPT-4 | Improvement |
|-----------|-----------|-------------------|-------|-------------|
...
npx ruf-swarm benchmark --full --include-swe-bench

# Specific performance tests
npx ruf-swarm benchmark --test agent-spawn
npx ruf-swarm benchmark --test task-throughput  
npx ruf-swarm benchmark --test memory-usage
npx ruf-swarm benchmark --test wasm-performance
npx ruf-swarm benchmark --test swe-bench-solve-rate

# Model comparison
npx ruf-swarm benchmark --compare lstm,tcn,nbeats,claude-optimizer

# Cost analysis
npx ruf-swarm benchmark --test cost-efficiency --baseline claude-3.7-sonnet

# Custom benchmark
npx ruf-swarm benchmark --config ./custom-bench.json
```
...
npx ruf-swarm benchmark --test swe-bench --instances 100

# Performance comparison
npx ruf-swarm benchmark --compare-with claude-3.7-sonnet

# Token efficiency analysis
npx ruf-swarm benchmark --test token-efficiency --tasks 1000

# Multi-agent coordination test
npx ruf-swarm benchmark --test coordination --agents 5-50
```
...
const adaptiveSwarm = await RufSwarm.initialize({
  topology: 'adaptive',
  adaptationRules: {
...
const persistentSwarm = await RufSwarm.initialize({
  persistence: {
    backend: 'sqlite',
    path: './swarm-memory.db',
...
const scalableSwarm = await RufSwarm.initialize({
  scaling: {
    minAgents: 3,
    maxAgents: 50,
...
ruf-swarm provides comprehensive hooks for Claude Code operations:
...
export RUF_SWARM_AUTO_COMMIT=true
export RUF_SWARM_GENERATE_REPORTS=true

# Agent work is automatically committed
npx ruf-swarm orchestrate "Implement user authentication"
...
```json
{
  "hooks": {
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "npx ruf-swarm hook agent-complete --agent '${tool.params.description}' --commit-to-git true"
      }]
    }]
  }
}
...
class RufSwarm {
  // Static methods
  static initialize(config?: SwarmConfig): Promise<RufSwarm>;
...
const haSwarm = await RufSwarm.initialize({
  deployment: {
    mode: 'distributed',
...
const secureSwarm = await RufSwarm.initialize({
  security: {
...
```typescript
// Advanced analytics
const analyticsSwarm = await RufSwarm.initialize({
  analytics: {
...
```bash
# Unit tests
cargo test -p ruf-swarm-core
cargo test -p ruf-swarm-agents
...
const researchSwarm = await RufSwarm.initialize({
  topology: 'hierarchical',
...
const devSwarm = await RufSwarm.initialize({
  topology: 'agile_team',
  methodology: 'scrum'
});
...
const biSwarm = await RufSwarm.initialize({
...
const eduSwarm = await RufSwarm.initialize({
...
|---------|-------------|---------|
| `init <topology> [max-agents]` | Initialize swarm | `npx ruf-swarm init mesh 10` |
| `spawn <type> [name]` | Create agent | `npx ruf-swarm spawn researcher "AI Researcher"` |
| `orchestrate <task>` | Execute task | `npx ruf-swarm orchestrate "Build REST API"` |
| `status` | Show swarm state | `npx ruf-swarm status` |
| `monitor` | Real-time monitoring | `npx ruf-swarm monitor` |

### Advanced Commands

| Command | Description | Example |
|---------|-------------|---------|
| `cluster create <name>` | Create agent cluster | `npx ruf-swarm cluster create research-team` |
| `workflow run <file>` | Execute workflow | `npx ruf-swarm workflow run ./ai-project.yml` |
| `memory store <key> <data>` | Store persistent data | `npx ruf-swarm memory store project-spec "API requirements..."` |
| `benchmark [test]` | Run performance tests | `npx ruf-swarm benchmark --test throughput` |
| `export <format> <file>` | Export swarm data | `npx ruf-swarm export json ./swarm-state.json` |

### MCP Commands

| Command | Description | Example |
|---------|-------------|---------|
| `mcp start [--port]` | Start MCP server | `npx ruf-swarm mcp start --port 3000` |
| `mcp status` | Check MCP server | `npx ruf-swarm mcp status` |
| `mcp tools` | List MCP tools | `npx ruf-swarm mcp tools` |
...
ruf-swarm is **fully compatible with remote servers** using npx:

```bash
# ✅ Works on any remote server with Node.js 14+
ssh user@remote-server 'npx ruf-swarm init mesh 10'

# ✅ Start MCP server remotely
ssh user@remote-server 'npx ruf-swarm mcp start --port 3000 &'

# ✅ Run benchmarks on remote hardware
ssh user@remote-server 'npx ruf-swarm benchmark --test swe-bench'

# ✅ Deploy with screen/tmux for persistence
ssh user@remote-server 'screen -S ruf-swarm -d -m npx ruf-swarm mcp start'
```
...
docker run -d -p 3000:3000 --name ruf-swarm \
...
kubectl run ruf-swarm --image=node:18-alpine \
  --port=3000 \
  --command -- npx ruf-swarm mcp start --port 3000

# PM2 process management
pm2 start 'npx ruf-swarm mcp start --port 3000' --name ruf-swarm
```
...
npx ruf-swarm --version  # Should show version without errors
npx ruf-swarm features   # Lists available features

# If you see "Invalid or unexpected token" error (v1.0.5 bug - fixed in v1.0.6)
npm update ruf-swarm@latest  # Update to v1.0.6+

# Force clean reinstall
npm cache clean --force
npm uninstall -g ruf-swarm
npm install -g ruf-swarm@latest

# Verify Node.js version
node --version  # Should be 14.0+ (v18+ recommended)

# Check WASM files are present
ls node_modules/ruf-swarm/wasm/  # Should contain .wasm files
```
...
npx ruf-swarm mcp start --port 3000 --host 0.0.0.0

# Test with curl
curl http://your-server:3000/health

# Enable debug logging
NODE_ENV=development npx ruf-swarm mcp start --verbose
```

**Agent Spawn Failures**
```bash
# Check system resources
npx ruf-swarm status --detailed

# Verify configuration
npx ruf-swarm config validate

# Check logs
npx ruf-swarm logs --level debug
```

**Performance Issues**
```bash
# Run diagnostics
npx ruf-swarm benchmark --quick

# Enable SIMD if supported
export RUVA_SWARM_WASM_SIMD=true

# Adjust agent limits
npx ruf-swarm config set maxAgents 10
```
...
# Verbose output
npx ruf-swarm --verbose <command>

# Performance profiling
npx ruf-swarm profile <command>
```
...
- 🚀 [Getting Started Guide](https://github.com/ruvnet/ruv-FANN/blob/main/ruf-swarm/guide/README.md)
- 📖 [API Reference](https://docs.rs/ruf-swarm)
- 🎯 [Examples Repository](https://github.com/ruvnet/ruv-FANN/tree/main/ruf-swarm/examples)
...
- 🔧 [Architecture Overview](https://github.com/ruvnet/ruv-FANN/blob/main/ruf-swarm/plans/ruf-swarm-architecture.md)

---

## 🌟 Showcase

> "*ruf-swarm transformed our AI development workflow. The cognitive diversity and WASM performance made complex multi-agent coordination finally practical.*" - **Tech Lead, AI Research**
...
> "*Enterprise features like persistence and auto-scaling make ruf-swarm production-ready out of the box.*" - **DevOps Engineer**

---

<div align="center">

**[⭐ Star us on GitHub](https://github.com/ruvnet/ruv-FANN)** | **[📦 NPM Package](https://www.npmjs.com/package/ruf-swarm)** | **[💬 Join Community](https://github.com/ruvnet/ruv-FANN/discussions)**
...
[![NPM downloads](https://img.shields.io/npm/dm/ruf-swarm)](https://www.npmjs.com/package/ruf-swarm)
