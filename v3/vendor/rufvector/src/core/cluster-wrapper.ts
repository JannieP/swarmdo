...
function getClusterModule() {
  ...
  try {
    ...
    loadError = new Error(
      `@rufvector/cluster not installed: ${e.message}\n` +
 ...
 */
export function createCluster(config: ClusterConfig): RufvectorCluster {
  return new RufvectorCluster(config);
}

export default RufvectorCluster;
