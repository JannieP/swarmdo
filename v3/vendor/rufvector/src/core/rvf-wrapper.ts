/**
 * RVF Wrapper - Persistent vector store via @rufvector/rvf
 *
 * Wraps @rufvector/rvf RvfDatabase through thin convenience functions.
...
function getRvfModule() {
  ...
  try {
    rvfModule = require('@rufvector/rvf');
    return rvfModule;
  } catch (e: any) {
    loadError = new Error(
      '@rufvector/rvf is not installed. Run: npm install @rufvector/rvf'
    );
    throw loadError;
  }
}
...
// ---------------------------------------------------------------------------
// Minimal inline types (mirrors @rufvector/rvf/types when package absent)
// ---------------------------------------------------------------------------
