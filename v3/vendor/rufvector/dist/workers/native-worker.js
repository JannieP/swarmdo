"use strict";
/**
 * Native Worker Runner for RufVector
 *
 ...
 *
 * No delegation to external tools - pure rufvector execution.
 */
...
async function loadOptionalDeps() {
    try {
        const core = await Promise.resolve().then(() => __importStar(require('@rufvector/core')));
...
class NativeWorker {
    ...
    async init() {
        ...
        if (this.config.capabilities?.vectorDb && VectorDb) {
            const dbPath = path.join(process.cwd(), '.rufvector', 'workers', `${this.config.name}.db`);
