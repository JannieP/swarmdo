"use strict";
/**
 * GNN Wrapper - Safe wrapper around @rufvector/gnn with automatic array conversion
 *
 ...
 *
 * The native @rufvector/gnn requires Float32Array for maximum performance.
...
exports.TensorCompress = exports.RufvectorLayer = void 0;
...
function getGnnModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        gnnModule = require('@rufvector/gnn');
        return gnnModule;
    }
    catch (e) {
        loadError = new Error(`@rufvector/gnn is not installed or failed to load: ${e.message}\n` +
            `Install with: npm install @rufvector/gnn`);
        throw loadError;
    }
}
/**
 ...
 * @example
 * ```typescript
 * import { differentiableSearch } from 'rufvector/core/gnn-wrapper';
 *
 ...
 */
class RufvectorLayer {
    /**
     * Create a new Rufvector GNN layer
     *
    ...
    constructor(inputDim, hiddenDim, heads, dropout = 0.1) {
        const gnn = getGnnModule();
        this.inner = new gnn.RufvectorLayer(inputDim, hiddenDim, heads, dropout);
    }
    /**
    ...
    static fromJson(json) {
        const gnn = getGnnModule();
        const layer = new RufvectorLayer(1, 1, 1, 0); // Dummy constructor
        layer.inner = gnn.RufvectorLayer.fromJson(json);
        return layer;
    }
}
exports.RufvectorLayer = RufvectorLayer;
/**
