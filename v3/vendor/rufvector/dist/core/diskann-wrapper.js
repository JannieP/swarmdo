 ...
 *
 * Wraps @rufvector/diskann for SSD-friendly approximate nearest neighbor search.
...
function getDiskAnnModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        diskannModule = require('@rufvector/diskann');
        return diskannModule;
    }
    catch (e) {
        loadError = new Error(`@rufvector/diskann not installed: ${e.message}\n` +
            `Install with: npm install @rufvector/diskann`);
        throw loadError;
    }
}
