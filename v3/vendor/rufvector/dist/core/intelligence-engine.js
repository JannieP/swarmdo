"use strict";
/**
 * IntelligenceEngine - Full RufVector Intelligence Stack
 *
 * Integrates all RufVector capabilities for self-learning hooks:
...
function getVectorDB() {
    ...
    if (vectorDbError)
        throw vectorDbError;
    try {
        const core = require('@rufvector/core');
        VectorDB = core.VectorDb || core.VectorDB;
        return VectorDB;
    }
    catch {
        try {
            const pkg = require('rufvector');
...
function getAttention() {
    ...
    try {
        attentionModule = require('@rufvector/attention');
        return attentionModule;
    }
    catch (e) {
        attentionError = e;
...
// ============================================================================
/**
 * Full-stack intelligence engine using all RufVector capabilities
 */
        ...
        if (selection === 'hash') {
            ...
            if (!(0, onnx_embedder_1.isOnnxAvailable)()) {
                throw new Error('RUVECTOR_EMBEDDER=minilm (or RUVECTOR_ONNX=1) hard-requires the ONNX embedder, ' +
                    'but the bundled WASM files are missing. Reinstall rufvector or unset the flag.');
            }
            useOnnx = true;
