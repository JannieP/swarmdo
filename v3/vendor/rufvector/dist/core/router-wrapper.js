 ...
 *
 * Wraps @rufvector/router for vector-based intent classification.
...
function getRouterModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        routerModule = require('@rufvector/router');
        return routerModule;
    }
    catch (e) {
        loadError = new Error(`@rufvector/router not installed: ${e.message}\n` +
            `Install with: npm install @rufvector/router`);
        throw loadError;
    }
}
