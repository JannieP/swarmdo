 ...
 *
 * Wraps @rufvector/graph-node for dependency analysis, co-edit patterns,
...
function getGraphModule() {
    ...
    if (loadError)
        throw loadError;
    try {
        graphModule = require('@rufvector/graph-node');
        return graphModule;
    }
    catch (e) {
        loadError = new Error(`@rufvector/graph-node not installed: ${e.message}\n` +
            `Install with: npm install @rufvector/graph-node`);
        throw loadError;
    }
}
