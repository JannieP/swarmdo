 ...
 * - Vector similarity search using @rufvector/core
...
// Lazy load rufvector core
let coreModule = null;
function getCoreModule() {
    if (coreModule)
        return coreModule;
    try {
        coreModule = require('@rufvector/core');
        return coreModule;
    }
    catch {
        // Fallback to rufvector if core not available
        try {
            coreModule = require('rufvector');
            return coreModule;
        }
        catch (e) {
            throw new Error(`Neither @rufvector/core nor rufvector is available: ${e.message}`);
        }
    }
}
/**
