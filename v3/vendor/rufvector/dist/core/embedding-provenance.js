 ...
 * read-only for vector writes until re-embedded (`rufvector hooks reembed`).
 *
...
class ProvenanceMismatchError extends Error {
    constructor(store, active, mismatches, storeName) {
        super(`Embedding-provenance mismatch (ADR-210): refusing vector write to ${storeName}. ` +
            ...
            `Mixed stores are never created — re-embed the store ('rufvector hooks reembed') ` +
...
function warnHashFallbackOnce(reason) {
    ...
    process.stderr.write(`rufvector: ONNX semantic embedder unavailable — using deterministic hash-fallback embeddings ` +
