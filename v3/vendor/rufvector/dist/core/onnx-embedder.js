 ...
 * or `isReady()`. See https://github.com/ruvnet/RufVector/issues/523.
 */
 ...
 * external `rufvector-onnx-embeddings-wasm/parallel` package, which was rejected
 * in ADR-194. See https://github.com/ruvnet/RufVector/issues/531.
 */
 ...
 * `rufvector-onnx-embeddings-wasm/parallel` was never published and was rejected
 * in ADR-194; the bundled pool (`onnx/bundled-parallel.mjs`) is the only
 * parallel implementation. See https://github.com/ruvnet/RufVector/issues/531.
 */
    ...
    loadPromise = (async () => {
        try {
            ...
            const modelLoader = new ModelLoader({
                cache: true,
                cacheDir: config.cacheDir || path.join(process.env.HOME || '/tmp', '.rufvector', 'models'),
            });
            ...
            if (config.enableParallel === true) {
                ...
                const isCLI = process.argv[1]?.includes('cli.js') ||
                    process.argv[1]?.includes('bin/rufvector') ||
 ...
 * https://github.com/ruvnet/RufVector/issues/523.
 */
 ...
 * runtime upgrade in the rufvector-onnx-embeddings-wasm crate (tracked as
    ...
    if (texts.length >= threshold) {
        try {
            ...
            if (!bulkPoolFallbackWarned) {
                ...
                console.error(`rufvector: parallel bulk-embed pool unavailable (${e?.message ?? e}); ` +
