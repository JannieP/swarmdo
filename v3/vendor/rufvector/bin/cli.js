...
// Lazy load rufvector (only when needed, not for install/help commands)
let VectorDB, getVersion, getImplementationType;
let ruvectorLoaded = false;

function loadRufvector() {
  if (ruvectorLoaded) return true;
  try {
    const rufvector = require('../dist/index.js');
    VectorDB = rufvector.VectorDB;
    getVersion = rufvector.getVersion;
    getImplementationType = rufvector.getImplementationType;
    ruvectorLoaded = true;
    return true;
  } catch (e) {
    return false;
...
function requireRufvector() {
  if (!loadRufvector()) {
    console.error(chalk.red('Error: Failed to load rufvector. Please run: npm run build'));
    console.error(chalk.yellow('Or install the package: npm install rufvector'));
    process.exit(1);
  }
}

// Lazy load GNN (optional - loaded on first use, not at startup)
// Saves ~6ms startup time by deferring require('@rufvector/gnn')
let _gnnModule = undefined; // undefined = not yet attempted, null = failed, object = loaded
let RufvectorLayer, TensorCompress, differentiableSearch, getCompressionLevel, hierarchicalForward;
let gnnAvailable = false;

function loadGnn() {
  if (_gnnModule !== undefined) return _gnnModule;
  try {
    const gnn = require('@rufvector/gnn');
    RufvectorLayer = gnn.RufvectorLayer;
...
// Saves ~5ms startup time by deferring require('@rufvector/attention')
...
// `@rufvector/gnn@0.1.25` has a native-binding regression where every method
...
function reportGnnBindingError(error) {
  ...
  if (msg.includes('Given napi value is not an array') || msg.includes('TypedArray info failed')) {
    console.error(chalk.yellow('  Note: this is a known regression in the @rufvector/gnn native binding,'));
    console.error(chalk.yellow('  not in the CLI. Track at:'));
    console.error(chalk.white('    https://github.com/ruvnet/rufvector/issues/402'));
  }
}
...
program
  .name('rufvector')
  .description(`${chalk.cyan('rufvector')} - High-performance vector database CLI`)
...
program
  .command('info')
  .description('Show rufvector information')
  .action(() => {
    ...
    // Try to load rufvector for implementation info
    if (loadRufvector()) {
      ...
      console.log(chalk.white(`  Core: ${chalk.gray('Not loaded (install @rufvector/core)')}`));
    }
    ...
    if (!gnnAvailable) {
      console.log(chalk.gray('\n  Install GNN with: npx rufvector install gnn'));
    }
  });
...
program
  ...
  .description('Install optional rufvector packages')
  ...
  .action(async (packages, options) => {
    ...
    // Available optional packages - all rufvector npm packages
    const availablePackages = {
      // Core packages
      core: {
        name: '@rufvector/core',
        description: 'Core vector database with native Rust bindings (HNSW, SIMD)',
        installed: true, // Always installed with rufvector
        category: 'core'
      },
      gnn: {
        name: '@rufvector/gnn',
      ...
      'graph-node': {
        name: '@rufvector/graph-node',
        description: 'Native Node.js bindings for hypergraph database with Cypher queries',
        installed: false,
        category: 'core'
      },
      'agentic-synth': {
        name: '@rufvector/agentic-synth',
        description: 'Synthetic data generator for AI/ML training, RAG, and agentic workflows',
        installed: false,
        category: 'tools'
      },
      extensions: {
        name: 'rufvector-extensions',
        description: 'Advanced features: embeddings, UI, exports, temporal tracking, persistence',
        installed: false,
        category: 'tools'
      },
      // Platform-specific native bindings for @rufvector/core
      'node-linux-x64': {
        name: '@rufvector/node-linux-x64-gnu',
        description: 'Linux x64 native bindings for @rufvector/core',
        installed: false,
        category: 'platform'
      },
      'node-linux-arm64': {
        name: '@rufvector/node-linux-arm64-gnu',
        description: 'Linux ARM64 native bindings for @rufvector/core',
        installed: false,
        category: 'platform'
      },
      'node-darwin-x64': {
        name: '@rufvector/node-darwin-x64',
        description: 'macOS Intel x64 native bindings for @rufvector/core',
        installed: false,
        category: 'platform'
      },
      'node-darwin-arm64': {
        name: '@rufvector/node-darwin-arm64',
        description: 'macOS Apple Silicon native bindings for @rufvector/core',
        installed: false,
        category: 'platform'
      },
      'node-win32-x64': {
        name: '@rufvector/node-win32-x64-msvc',
        description: 'Windows x64 native bindings for @rufvector/core',
        installed: false,
        category: 'platform'
      },
      // Platform-specific native bindings for @rufvector/gnn
      'gnn-linux-x64': {
        name: '@rufvector/gnn-linux-x64-gnu',
        description: 'Linux x64 native bindings for @rufvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-linux-arm64': {
        name: '@rufvector/gnn-linux-arm64-gnu',
        description: 'Linux ARM64 native bindings for @rufvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-darwin-x64': {
        name: '@rufvector/gnn-darwin-x64',
        description: 'macOS Intel x64 native bindings for @rufvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-darwin-arm64': {
        name: '@rufvector/gnn-darwin-arm64',
        description: 'macOS Apple Silicon native bindings for @rufvector/gnn',
        installed: false,
        category: 'platform'
      },
      'gnn-win32-x64': {
        name: '@rufvector/gnn-win32-x64-msvc',
        description: 'Windows x64 native bindings for @rufvector/gnn',
        installed: false,
        category: 'platform'
      },
      ...
      'rufvector-core': {
        name: 'rufvector-core',
        description: 'Standalone vector database (legacy, use @rufvector/core instead)',
        installed: false,
        category: 'legacy'
      }
    };
    ...
    if (options.list || (packages.length === 0 && !options.all && !options.interactive)) {
      console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
      console.log(chalk.cyan('                    Rufvector Packages'));
      console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
      ...
      console.log(chalk.white('  npx rufvector install gnn              # Install GNN package'));
      console.log(chalk.white('  npx rufvector install graph-node       # Install graph database'));
      console.log(chalk.white('  npx rufvector install agentic-synth    # Install data generator'));
      console.log(chalk.white('  npx rufvector install --all            # Install all core packages'));
      console.log(chalk.white('  npx rufvector install -i               # Interactive selection'));
      console.log(chalk.gray('\n  Note: Platform bindings are auto-detected by @rufvector/core'));
      return;
    }
    ...
    for (const pkg of packages) {
      const key = pkg.toLowerCase().replace('@rufvector/', '');
    ...
    function installPackages(pkgs) {
      ...
      try {
        ...
        console.log(chalk.cyan('\nRun "npx rufvector info" to verify installation.'));
      } catch (error) {
...
function requireGnn() {
  ...
  if (!gnnAvailable) {
    ...
    console.error(chalk.yellow('Install it with: npm install @rufvector/gnn'));
    process.exit(1);
  }
}
...
gnnCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      const layer = new RufvectorLayer(inputDim, hiddenDim, heads, dropout);
      ...
      if (options.test) {
        ...
        // The @rufvector/gnn binding requires Float32Array — plain number[] surfaces
...
gnnCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      // @rufvector/gnn's differentiableSearch needs Float32Array everywhere; plain
...
gnnCmd
  ...
  .action(() => {
    ...
    if (!gnnAvailable) {
      ...
      console.log(chalk.white('Install with: npm install @rufvector/gnn'));
      return;
    }
    ...
    console.log(chalk.cyan('\nAvailable Features:'));
    console.log(chalk.white(`  • RufvectorLayer   - GNN layer with multi-head attention`));
...
function requireAttention() {
  ...
  if (!attentionAvailable) {
    console.error(chalk.red('Error: @rufvector/attention is not installed'));
    console.error(chalk.yellow('Install it with: npm install @rufvector/attention'));
    process.exit(1);
  }
}
...
attentionCmd
  ...
  .action((options) => {
    ...
    try {
      ...
      // The native @rufvector/attention bindings require Float32Array; passing
      ...
      // The native @rufvector/attention bindings expose `compute(query, keys, values)`
...
attentionCmd
  ...
  .action(() => {
    ...
    if (!attentionAvailable) {
      ...
      console.log(chalk.white('Install with: npm install @rufvector/attention'));
      return;
    }
...
attentionCmd
  ...
  .action((options) => {
    ...
    if (!attentionAvailable) {
      console.log(chalk.yellow('\n  Note: @rufvector/attention not installed'));
      console.log(chalk.white('  Install with: npm install @rufvector/attention'));
    }
    ...
    if (options.verbose) {
      ...
      console.log(chalk.white('    npx rufvector attention compute -q "[1,2,3]" -k keys.json -t dot'));
      console.log(chalk.gray('\n    # Benchmark attention mechanisms'));
      console.log(chalk.white('    npx rufvector attention benchmark -d 256 -n 100'));
      console.log(chalk.gray('\n    # Hyperbolic distance'));
      console.log(chalk.white('    npx rufvector attention hyperbolic -a distance -v "[0.1,0.2]" -b "[0.3,0.4]"'));
    }
  });
...
program
  ...
  .action(async (options) => {
    ...
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Doctor'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    // Check @rufvector/core
    let coreAvailable = false;
    try {
      require.resolve('@rufvector/core');
      coreAvailable = true;
      console.log(chalk.green(`  ✓ @rufvector/core installed`));
    } catch (e) {
      console.log(chalk.yellow(`  ! @rufvector/core not found (using WASM fallback)`));
      warnings++;
    }

    // Check if native binding works
    if (coreAvailable && loadRufvector()) {
    ...
    // Check @rufvector/gnn
    if (gnnAvailable) {
      console.log(chalk.green(`  ✓ @rufvector/gnn installed`));
    } else {
      console.log(chalk.gray(`  ○ @rufvector/gnn not installed (optional)`));
    }

    // Check @rufvector/attention
    if (attentionAvailable) {
      console.log(chalk.green(`  ✓ @rufvector/attention installed`));
    } else {
      console.log(chalk.gray(`  ○ @rufvector/attention not installed (optional)`));
    }

    // Check @rufvector/graph-node
    try {
      require.resolve('@rufvector/graph-node');
      console.log(chalk.green(`  ✓ @rufvector/graph-node installed`));
    } catch (e) {
      console.log(chalk.gray(`  ○ @rufvector/graph-node not installed (optional)`));
    }
    console.log('');
    ...
    if (issues === 0 && warnings === 0) {
      console.log(chalk.green('\n  ✓ All checks passed! RufVector is ready to use.\n'));
    } else if (issues === 0) {
      console.log(chalk.yellow(`\n  ! ${warnings} warning(s) found. RufVector should work but may have limited features.\n`));
    } else {
      console.log(chalk.red(`\n  ✗ ${issues} issue(s) and ${warnings} warning(s) found.\n`));
      console.log(chalk.white('  Run "npx rufvector setup" for installation instructions.\n'));
    }
  });
...
program
  ...
  .action((options) => {
    ...
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Setup Guide'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));

    // Quick install
    console.log(chalk.cyan('Quick Install (one-liner):'));
    console.log(chalk.white('  curl -fsSL https://raw.githubusercontent.com/ruvnet/rufvector/main/install.sh | bash'));
    console.log('');
    ...
    if (showAll || options.npm) {
      ...
      console.log(chalk.white('  npm install -g rufvector'));
      console.log(chalk.white('  npx rufvector'));
      console.log('');

      console.log(chalk.yellow('Core packages:'));
      console.log(chalk.white('  npm install @rufvector/core       # Vector database'));
      console.log(chalk.white('  npm install @rufvector/gnn        # Graph Neural Networks'));
      console.log(chalk.white('  npm install @rufvector/graph-node # Hypergraph database'));
      console.log('');

      console.log(chalk.yellow('Install all optional packages:'));
      console.log(chalk.white('  npx rufvector install --all'));
      console.log('');

      console.log(chalk.yellow('List available packages:'));
      console.log(chalk.white('  npx rufvector install'));
      console.log('');
    }
    ...
    if (showAll || options.rust) {
      ...
      console.log(chalk.yellow('3. Add RufVector crates to your project:'));
      console.log(chalk.white('  cargo add rufvector-core          # Vector database'));
      console.log(chalk.white('  cargo add rufvector-graph         # Hypergraph with Cypher'));
      console.log(chalk.white('  cargo add rufvector-gnn           # Graph Neural Networks'));
      console.log('');

      console.log(chalk.yellow('4. Other available crates:'));
      console.log(chalk.white('  cargo add rufvector-cluster       # Distributed clustering'));
      console.log(chalk.white('  cargo add rufvector-raft          # Raft consensus'));
      console.log(chalk.white('  cargo add rufvector-replication   # Data replication'));
      console.log(chalk.white('  cargo add rufvector-tiny-dancer-core  # AI routing'));
      console.log(chalk.white('  cargo add rufvector-router-core       # Semantic routing (Rust crate)'));
      console.log(chalk.white('  npm install @rufvector/router         # Semantic routing (npm)'));
      console.log('');
    ...
    console.log(chalk.white('  GitHub:     https://github.com/ruvnet/rufvector'));
    console.log(chalk.white('  npm:        https://www.npmjs.com/package/rufvector'));
    console.log(chalk.white('  crates.io:  https://crates.io/crates/rufvector-core'));
    console.log(chalk.white('  Issues:     https://github.com/ruvnet/rufvector/issues'));
    console.log('');

    console.log(chalk.cyan('Quick Commands:'));
    console.log(chalk.white('  npx rufvector doctor     # Check system health'));
    console.log(chalk.white('  npx rufvector info       # Show version info'));
    console.log(chalk.white('  npx rufvector benchmark  # Run performance test'));
    console.log(chalk.white('  npx rufvector install    # List available packages'));
    console.log('');
  });
...
program
  .command('graph')
  .description('Graph database operations (requires @rufvector/graph-node)')
  ...
  .action(async (options) => {
    let graphNode;
    try {
      graphNode = require('@rufvector/graph-node');
    } catch (e) {
      console.log(chalk.yellow('\n  @rufvector/graph-node is not installed.\n'));
      console.log(chalk.cyan('  Install with:'));
      console.log(chalk.white('    npm install @rufvector/graph-node\n'));
      ...
      console.log(chalk.white('    npx rufvector graph --query "CREATE (n:Person {name: \'Alice\'})"'));
      console.log(chalk.white('    npx rufvector graph --query "MATCH (n) RETURN n"'));
      console.log('');
      return;
    }

    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Graph'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));

    if (options.info) {
      console.log(chalk.green('  @rufvector/graph-node is available!'));
    ...
    if (options.query) {
      ...
      console.log(chalk.gray('  Note: Full Cypher execution requires running rufvector-server'));
      console.log(chalk.gray('  See: npx rufvector server --help'));
    }
...
program
  .command('router')
  .description('AI semantic router operations (requires @rufvector/router)')
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Router'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (options.info || (!options.route && !options.intents && !options.addIntent)) {
      ...
      console.log(chalk.gray('  npm package:  npm install @rufvector/router'));
      console.log(chalk.gray('  Rust crate:   cargo add rufvector-router-core'));
      console.log('');
      console.log(chalk.cyan('  Usage (when available):'));
      console.log(chalk.white('    npx rufvector router --route "What is the weather?"'));
      console.log(chalk.white('    npx rufvector router --intents intents.json --route "query"'));
      console.log('');
      return;
    }
...
const tinyDancer = program
  .command('tiny-dancer')
  .alias('td')
  .description('Cost-optimal FastGRNN model router — train from a DRACO dataset and route with it (requires @rufvector/tiny-dancer)');

function loadTinyDancer() {
  try {
    return require('@rufvector/tiny-dancer');
  } catch (e) {
    console.error(chalk.red('\n  This command requires @rufvector/tiny-dancer'));
    console.error(chalk.yellow('  Install it:  npm install @rufvector/tiny-dancer'));
...
tinyDancer
  ...
  .action(() => {
    try {
      ...
      console.log(chalk.green(`\n  @rufvector/tiny-dancer ${td.version()} — ${td.hello()}`));
      console.log(chalk.gray('  train:  npx rufvector tiny-dancer train <draco.json> --out model.safetensors'));
      console.log(chalk.gray('  score:  npx rufvector tiny-dancer score <model.safetensors> --query <embedding.json>\n'));
    } catch {
      console.log(chalk.yellow('\n  @rufvector/tiny-dancer not installed.  npm install @rufvector/tiny-dancer\n'));
    }
  });
...
program
  .command('server')
  .description('Start RufVector HTTP/gRPC server')
  .option('-p, --port <number>', 'HTTP port', '8080')
  .option('-g, --grpc-port <number>', 'gRPC port', '50051')
  .option('-d, --data-dir <path>', 'Data directory', './rufvector-data')
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Server'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (options.info || Object.keys(options).filter(k => k !== 'port' && k !== 'grpcPort' && k !== 'dataDir').length === 0) {
      ...
      console.log(chalk.white('    cargo install rufvector-server  # When published'));
      console.log('');
      ...
      console.log('');
      console.log(chalk.gray('  Track progress: https://github.com/ruvnet/rufvector/issues/20'));
      console.log('');
      return;
    }
...
program
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Cluster'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    console.log(chalk.white('    cargo add rufvector-cluster      # Clustering'));
    console.log(chalk.white('    cargo add rufvector-raft         # Raft consensus'));
    console.log(chalk.white('    cargo add rufvector-replication  # Replication'));
    console.log('');
    console.log(chalk.cyan('  Commands (when available):'));
    console.log(chalk.white('    npx rufvector cluster --status'));
    console.log(chalk.white('    npx rufvector cluster --join 192.168.1.10:7000'));
    console.log(chalk.white('    npx rufvector cluster --nodes'));
    console.log('');
    console.log(chalk.gray('  Track progress: https://github.com/ruvnet/rufvector/issues/20'));
    console.log('');
  });
    ...
    try {
      ...
      if (!vectors || vectors.length === 0 || !vectors[0].vector) {
        spinner.fail(chalk.yellow(
          ...
          '(the same format `rufvector insert` accepts). Legacy _export.json ' +
  ...
  .action(async (text, opts) => {
    try {
      ...
      if (text) {
        ...
        console.log(chalk.yellow('Usage: rufvector embed optimized <text>'));
...
program
  ...
  .action(async (options) => {
    console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
    console.log(chalk.cyan('                    RufVector Demo'));
    console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
    ...
    if (showMenu) {
      ...
      console.log(chalk.white('    npx rufvector demo --basic'));
      console.log(chalk.white('    npx rufvector demo --gnn'));
      console.log('');
      return;
    }
    ...
    if (options.basic) {
      ...
      try {
        ...
        console.log(chalk.white('    npx rufvector stats ./demo.db'));
        console.log(chalk.white('    npx rufvector search ./demo.db --vector "[0.8, 0.6, 0, 0]"'));
      } catch (error) {
    ...
    if (options.gnn) {
      loadGnn();
      if (!gnnAvailable) {
        console.log(chalk.yellow('  @rufvector/gnn not installed.'));
        console.log(chalk.white('  Install with: npm install @rufvector/gnn'));
        console.log('');
        return;
      }
      ...
      try {
        ...
        // The native @rufvector/gnn binding expects Float32Array typed arrays.
        ...
        // `@rufvector/gnn@0.1.25`'s native binding has a regression where every
        ...
        if (msg.includes('not an array') || msg.includes('TypedArray')) {
          ...
          console.error(chalk.yellow('\n  This looks like a regression in the @rufvector/gnn native binding,'));
    ...
    if (options.graph) {
      ...
      try {
        ...
        console.log(chalk.yellow('  @rufvector/graph-node not installed.'));
        console.log(chalk.white('  Install with: npm install @rufvector/graph-node'));
        console.log('');
        return;
      }
      ...
      try {
        ...
        // HyperGraph / RufVectorGraph) with createNode / createEdge / query.
        const GraphDatabase = graphNode.GraphDatabase;
        if (typeof GraphDatabase !== 'function') {
          console.log(chalk.yellow('  @rufvector/graph-node has no GraphDatabase constructor.'));
        ...
        // args) and are async — see @rufvector/graph-node index.d.ts.
    ...
    if (options.benchmark) {
      ...
      try {
        // Note: rufvector-core-linux-x64-gnu@0.1.29 (and current sister binaries)
        ...
        console.log(chalk.gray('  For deeper benchmarks: npx rufvector benchmark'));
      } catch (error) {
...
// Full RufVector Stack: VectorDB + SONA + Attention
// ============================================
...
class Intelligence {
  ...
  getIntelPath() {
    const projectPath = path.join(process.cwd(), '.rufvector', 'intelligence.json');
    const homePath = path.join(require('os').homedir(), '.rufvector', 'intelligence.json');
  ...
  checkVectorWrite(active) {
    ...
    if (this.isLegacyVectorStore()) {
      ...
      const err = new Error(
        ...
        `${prov.describeProvenance(active)}. Run 'rufvector hooks reembed' to re-embed and unlock it.`
      );
    ...
    try {
      ...
      if (policy === 'warn') {
        if (!Intelligence._reembedWarned) {
          ...
          console.error(`rufvector: ${e.message} (RUVECTOR_REEMBED=warn: store stays read-only, write skipped)`);
        }
        return { ok: false, skipped: true, error: e.message };
      }
      // 'auto' without an async re-embed path behaves like refuse, with a hint.
      if (policy === 'auto') e.message += ` (RUVECTOR_REEMBED=auto: run 'rufvector hooks reembed' — in-place re-embedding needs the async path)`;
      throw e;
    }
  }
  ...
  warnRecallProvenance(active) {
    ...
    if (mismatches.length > 0) {
      ...
      console.error(
        `rufvector: recall quality degraded — stored vectors are ${prov.describeProvenance(stored)} ` +
        `but the query was embedded as ${prov.describeProvenance(active)} (differs on: ${mismatches.join(', ')}). ` +
        `Run 'rufvector hooks reembed' to fix.`
      );
    }
  }
...
function getPermissionsForProjectType(types) {
  const basePermissions = [
    ...
    'Bash(ls:*)', 'Bash(pwd)', 'Bash(cat:*)', 'Bash(mkdir:*)', 'Bash(which:*)', 'Bash(rufvector:*)'
  ];
...
hooksCmd.command('init')
  ...
  // Workers configuration (native rufvector workers + agentic-flow integration)
  if (!opts.minimal) {
  ...
  if (!opts.minimal && opts.statusline !== false) {
    if (!settings.statusLine) {
      if (isWindows) {
        ...
        const statuslineContent = `# RufVector Intelligence Statusline for Windows PowerShell
...
$IntelFile = Join-Path $CWD ".rufvector\intelligence.json"
if (Test-Path $IntelFile) {
  ...
  Write-Host "$e[2m RufVector: run 'npx rufvector hooks session-start'$e[0m"
}
`;
...
# RufVector Intelligence Statusline - Multi-line display
INPUT=\$(cat)
...
echo
INTEL_FILE=""
for P in "\$CWD/.rufvector/intelligence.json" "\$CWD/npm/packages/rufvector/.rufvector/intelligence.json" "\$HOME/.rufvector/intelligence.json"; do
...
if [ -n "\$INTEL_FILE" ]; then
  ...
  printf "\$MAGENTA🧠 RufVector\$RESET"
  ...
  printf "\$DIM🧠 RufVector: run 'npx rufvector hooks session-start' to initialize\$RESET\\n"
fi
`;
        fs.writeFileSync(statuslineScript, statuslineContent);
  ...
  let hookCmd = 'npx rufvector@latest';
  let fastTimeouts = { simple: 2000, complex: 2000, session: 5000 };
  if (opts.fast && !isWindows) {
    const fastWrapperPath = path.join(settingsDir, 'rufvector-fast.sh');
    const fastWrapperContent = `#!/bin/bash
# Fast RufVector hooks wrapper - avoids npx overhead (20x faster)
# Usage: .claude/rufvector-fast.sh hooks <command> [args...]

# Find rufvector CLI - check local first, then global
RUVECTOR_CLI=""

# Check local npm package (for development)
if [ -f "$PWD/npm/packages/rufvector/bin/cli.js" ]; then
  RUVECTOR_CLI="$PWD/npm/packages/rufvector/bin/cli.js"
# Check node_modules
elif [ -f "$PWD/node_modules/rufvector/bin/cli.js" ]; then
  RUVECTOR_CLI="$PWD/node_modules/rufvector/bin/cli.js"
# Check global npm installation
elif [ -f "$PWD/node_modules/.bin/rufvector" ]; then
  exec "$PWD/node_modules/.bin/rufvector" "$@"
elif command -v rufvector &> /dev/null; then
  exec rufvector "$@"
# Fallback to npx (slow but works)
else
  exec npx rufvector@latest "$@"
fi
...
`;
    ...
    hookCmd = '.claude/rufvector-fast.sh';
    fastTimeouts = { simple: 300, complex: 500, session: 1000 };
    // Add permission for fast wrapper
    if (settings.permissions && settings.permissions.allow) {
      if (!settings.permissions.allow.includes('Bash(.claude/rufvector-fast.sh:*)')) {
        settings.permissions.allow.push('Bash(.claude/rufvector-fast.sh:*)');
      }
    }
    console.log(chalk.blue('  ✓ Fast wrapper created (.claude/rufvector-fast.sh) - 20x faster hooks'));
  }
...
## RufVector Self-Learning Intelligence v2.0

This project uses RufVector's self-learning intelligence hooks with advanced capabilities:
...
npx rufvector hooks init

# View learning statistics
npx rufvector hooks stats

# Route a task to best agent
npx rufvector hooks route "implement feature X"

# Enhanced routing with AST/coverage/diff signals
npx rufvector hooks route-enhanced "fix bug" --file src/api.ts

# Store context in vector memory
npx rufvector hooks remember "important context" -t project

# Recall from memory (semantic search)
npx rufvector hooks recall "context query"
\`\`\`
...
npx rufvector hooks ast-analyze src/index.ts

# Get complexity metrics for multiple files
npx rufvector hooks ast-complexity src/*.ts --threshold 15
\`\`\`
...
npx rufvector hooks diff-analyze HEAD

# Classify change type (feature, bugfix, refactor, etc.)
npx rufvector hooks diff-classify

# Find similar past commits
npx rufvector hooks diff-similar -k 5

# Get risk score only
npx rufvector hooks diff-analyze --risk-only
\`\`\`
...
npx rufvector hooks coverage-route src/api.ts

# Suggest tests for files based on coverage
npx rufvector hooks coverage-suggest src/*.ts
\`\`\`
...
npx rufvector hooks graph-mincut src/*.ts

# Detect code communities (Louvain/Spectral clustering)
npx rufvector hooks graph-cluster src/*.ts --method louvain
\`\`\`
...
npx rufvector hooks security-scan src/*.ts

# RAG-enhanced context retrieval
npx rufvector hooks rag-context "how does auth work"

# Git churn analysis (hot spots)
npx rufvector hooks git-churn --days 30
\`\`\`

### MCP Tools (via Claude Code)

When using the RufVector MCP server, these tools are available:
...
RufVector includes 10 attention mechanisms:
...
Stored in \`.rufvector/intelligence.json\`:
...
\`\`\`bash
npx rufvector hooks init              # Full configuration with all capabilities
npx rufvector hooks init --minimal    # Basic hooks only
npx rufvector hooks init --fast       # Use fast local wrapper (20x faster)
npx rufvector hooks init --pretrain   # Initialize + pretrain from git history
npx rufvector hooks init --build-agents quality  # Generate optimized agents
npx rufvector hooks init --force      # Overwrite existing configuration
\`\`\`

---
*Powered by [RufVector](https://github.com/ruvnet/rufvector) self-learning intelligence v2.0*
`;
  ...
  if (opts.gitignore !== false) {
    ...
    const entriesToAdd = ['.rufvector/', '.claude/statusline.sh'];
    ...
    if (linesToAdd.length > 0) {
      const newContent = gitignoreContent.trim() + '\n\n# RufVector intelligence data\n' + linesToAdd.join('\n') + '\n';
      fs.writeFileSync(gitignorePath, newContent);
      console.log(chalk.blue('  ✓ .gitignore updated'));
    }
  }

  // Create .rufvector directory for intelligence data
  const ruvectorDir = path.join(process.cwd(), '.rufvector');
  if (!fs.existsSync(ruvectorDir)) {
    fs.mkdirSync(ruvectorDir, { recursive: true });
    console.log(chalk.blue('  ✓ .rufvector/ directory created'));
  }

  console.log(chalk.green('\n✅ RufVector hooks initialization complete!'));
  ...
  if (opts.pretrain) {
    ...
    const { execSync } = require('child_process');
    try {
      execSync('npx rufvector hooks pretrain', { stdio: 'inherit' });
    } catch (e) {
  ...
  if (opts.buildAgents) {
    ...
    const { execSync } = require('child_process');
    try {
      execSync(`npx rufvector hooks build-agents --focus ${focus} --include-prompts`, { stdio: 'inherit' });
    } catch (e) {
  ...
  if (!opts.pretrain && !opts.buildAgents) {
    console.log(chalk.dim('   Run `npx rufvector hooks verify` to test the setup'));
    console.log(chalk.dim('   Run `npx rufvector hooks pretrain` to bootstrap intelligence'));
    console.log(chalk.dim('   Run `npx rufvector hooks build-agents` to generate optimized agents'));
  }
});
...
hooksCmd.command('session-start').description('Session start hook').option('--resume', 'Resume previous session').action(() => {
  ...
  intel.sessionStart();
  intel.save();
  console.log(chalk.bold.cyan('🧠 RufVector Intelligence Layer Active'));
...
hooksCmd.command('suggest-context').description('Suggest relevant context').action(() => {
  const intel = new Intelligence();
  const stats = intel.stats();
  console.log(`RufVector Intelligence: ${stats.total_patterns} learned patterns, ${stats.total_errors} error fixes available. Use 'rufvector hooks route' for agent suggestions.`);
});
...
hooksCmd.command('remember').description('Store in memory').requiredOption('-t, --type <type>', 'Memory type').option('--silent', 'Suppress output').option('--semantic', 'Use ONNX semantic embeddings (slower, better quality)').argument('<content...>', 'Content').action(async (content, opts) => {
  ...
  try {
    ...
    if (id === null) {
      ...
      if (!opts.silent) {
        console.log(JSON.stringify({ success: false, skipped: true, reason: 'store is read-only for vector writes (embedding provenance, ADR-210); run `rufvector hooks reembed`' }));
      }
      return;
    }
    intel.save();
...
hooksCmd.command('learning-config')
  ...
  .action(async (opts) => {
    ...
    const dataPath = path.join(process.cwd(), '.rufvector', 'intelligence.json');
    let data = {};
    try {
...
hooksCmd.command('verify')
  ...
  .action((opts) => {
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      checks.push({ name: 'Settings file', status: 'fail', detail: 'Run `npx rufvector hooks init` first' });
    }

    // Check 2: .rufvector directory
    const ruvectorDir = path.join(process.cwd(), '.rufvector');
    if (fs.existsSync(ruvectorDir)) {
      checks.push({ name: 'Data directory', status: 'pass', detail: '.rufvector/ exists' });
    ...
    try {
      const { execSync } = require('child_process');
      execSync('npx rufvector hooks stats', { stdio: 'pipe', timeout: 5000 });
    ...
    if (failCount === 0) {
      ...
      console.log(chalk.dim('   Run `npx rufvector hooks doctor` for detailed diagnostics'));
    }
  });
...
hooksCmd.command('doctor')
  ...
  .action((opts) => {
    ...
    const settingsPath = path.join(process.cwd(), '.claude', 'settings.json');
    if (!fs.existsSync(settingsPath)) {
      issues.push({ severity: 'error', message: 'No .claude/settings.json found', fix: 'Run `npx rufvector hooks init`' });
    } else {
      try {
        const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
        ...
        const checkCommands = (hooks) => {
          ...
          hooks.forEach(h => {
            h.hooks?.forEach(hh => {
              if (hh.command && hh.command.includes('rufvector') && !hh.command.startsWith('npx ') && !hh.command.includes('/bin/')) {
                issues.push({ severity: 'warning', message: `Command should use 'npx rufvector' for portability`, fix: 'Update to use npx' });
              }
            });
          });
        };
        ...
        issues.push({ severity: 'error', message: 'Invalid JSON in settings file', fix: 'Re-run `npx rufvector hooks init --force`' });
      }
    }
    ...
    if (fs.existsSync(gitignorePath)) {
      ...
      if (!content.includes('.rufvector/')) {
        issues.push({ severity: 'warning', message: '.rufvector/ not in .gitignore', fix: 'Add to prevent committing learning data' });
        if (opts.fix) {
          fs.appendFileSync(gitignorePath, '\n# RufVector intelligence data\n.rufvector/\n');
          fixes.push('Added .rufvector/ to .gitignore');
        }
      }
    }
...
hooksCmd.command('export')
  ...
  .option('-o, --output <file>', 'Output file path', 'rufvector-export.json')
...
hooksCmd.command('import')
  ...
  .action((file, opts) => {
    ...
    try {
      ...
      intel.save();
      console.log(chalk.dim('   Data saved to .rufvector/intelligence.json'));
    } catch (e) {
...
hooksCmd.command('pretrain')
  ...
  .action(async (opts) => {
    ...
    console.log(chalk.bold.cyan('\n🧠 RufVector Pretrain - Repository Intelligence Bootstrap\n'));
    ...
    try {
      ...
      try {
        ...
        console.log(chalk.dim('  ⏭️  @rufvector/attention not installed'));
      }

      try {
        const gnn = require('@rufvector/gnn');
        capabilities.gnn = true;
        console.log(chalk.green(`  ✓ GNN: RufvectorLayer, TensorCompress available`));
      } catch (e) {
        console.log(chalk.dim('  ⏭️  @rufvector/gnn not installed'));
      }
...
${focus.description}` : null
    ...
    agents.forEach(agent => {
      ...
      if (opts.format === 'yaml') {
        const yaml = [
          `# Auto-generated by RufVector Agent Builder`,
          `# Focus: ${opts.focus}`,
...
// Native Workers command group - Deep rufvector integration (no agentic-flow delegation)
...
nativeCmd.command('compare')
  .description('Compare rufvector native vs agentic-flow workers')
...
// RVF (RufVector Format) commands
const rvfCmd = program.command('rvf').description('RufVector Format (.rvf) cognitive container operations');
...
const RVF_EXAMPLES = [
  ...
  { name: 'rufllm_inference', size: '133 KB', desc: 'RuvLLM inference cache' },
...
const RVF_BASE_URL = 'https://raw.githubusercontent.com/ruvnet/rufvector/main/examples/rvf/output';
...
rvfCmd.command('examples')
  ...
  .action((opts) => {
    ...
    console.log(chalk.dim(`Download: npx rufvector rvf download <name>\n`));
    ...
    console.log(chalk.dim(`\nFull catalog: https://github.com/ruvnet/rufvector/tree/main/examples/rvf/output\n`));
  });
...
rvfCmd.command('download [names...]')
  ...
  .action(async (names, opts) => {
    ...
    if (opts.all) {
      ...
      console.error(chalk.red('Specify example names or use --all. Run `npx rufvector rvf examples` to list.'));
      process.exit(1);
    }
    ...
    for (const name of toDownload) {
      ...
      if (!opts.all) {
        ...
        if (!RVF_EXAMPLES.some(e => e.name === baseName)) {
          console.log(chalk.red(`SKIPPED: Unknown example '${baseName}'. Run 'npx rufvector rvf examples' to list.`));
          fail++;
          continue;
        }
      }
...
mcpCmd.command('start')
  .description('Start the RufVector MCP server')
  .action(() => {
...
mcpCmd.command('info')
  ...
  .action(() => {
    console.log(chalk.bold.cyan('\n🔌 RufVector MCP Server\n'));
    console.log(chalk.white('The RufVector MCP server provides self-learning intelligence'));
    ...
    console.log(chalk.bold('\n📦 Resources:'));
    console.log(chalk.dim('  rufvector://intelligence/stats     - Current statistics'));
    console.log(chalk.dim('  rufvector://intelligence/patterns  - Learned patterns'));
    console.log(chalk.dim('  rufvector://intelligence/memories  - Vector memories'));

    console.log(chalk.bold.yellow('\n⚙️  Setup Instructions:\n'));
    console.log(chalk.white('Add to Claude Code:'));
    console.log(chalk.cyan('  claude mcp add rufvector npx rufvector mcp start\n'));
...
mcpCmd.command('test')
  ...
  .action(() => {
    ...
    console.log(chalk.bold.green('\n  All checks passed.\n'));
    console.log(chalk.dim('  Setup: claude mcp add rufvector npx rufvector mcp start\n'));
  });

// ============================================================================
// Brain Commands — Shared intelligence via @rufvector/pi-brain (lazy-loaded)
// ============================================================================

// Lazy load pi-brain (optional peer dependency)
async function requirePiBrain() {
  try {
    return await import('@rufvector/pi-brain');
  } catch {
    console.error(chalk.red('Brain commands require @rufvector/pi-brain'));
    console.error(chalk.yellow('  npm install @rufvector/pi-brain'));
    process.exit(1);
  }
}
...
brainAgiCmd
  ...
  .action(() => {
    console.log(chalk.cyan('\n  SONA subsystem'));
    console.log(chalk.dim('  Use `rufvector sona status` for full SONA metrics.\n'));
  });
...
identityCmd
  ...
  .option('--save', 'Save the key to ~/.rufvector/pi-key')
  .action(async (cmdOpts) => {
    ...
    if (cmdOpts.save) {
      const keyDir = path.join(require('os').homedir(), '.rufvector');
...
identityCmd
  ...
  .action(async () => {
    ...
    if (!key) {
      const keyPath = path.join(require('os').homedir(), '.rufvector', 'pi-key');
      if (fs.existsSync(keyPath)) {
        key = fs.readFileSync(keyPath, 'utf8').trim();
      }
    }
    if (!key) {
      console.error(chalk.red('No pi key found. Set PI env var or run: rufvector identity generate --save'));
      process.exit(1);
    }
    let pseudonym;
    try {
    ...
    if (isJsonOutput(opts)) {
      console.log(JSON.stringify({
        ...
        source: process.env.PI ? 'PI env var' : '~/.rufvector/pi-key',
      }, null, 2));
    } else {
      console.log(chalk.bold.cyan('\n  Pi Identity\n'));
      console.log(chalk.white(`  Pseudonym: ${pseudonym}`));
      console.log(chalk.gray(`  Key:       ${key.substring(0, 8)}...${key.substring(key.length - 8)}`));
      console.log(chalk.gray(`  Source:    ${process.env.PI ? 'PI env var' : '~/.rufvector/pi-key'}\n`));
    }
  });
...
identityCmd
  ...
  .option('--save', 'Save the imported key to ~/.rufvector/pi-key')
...
// =============================================================================
// LLM Commands - LLM orchestration (lazy: @rufvector/rufllm)
// =============================================================================
...
llmCmd.command('embed')
  ...
  .action(async (text, opts) => {
    let rufllm;
    try {
      rufllm = require('@rufvector/rufllm');
    } catch (e) {
      console.error(chalk.red('LLM commands require @rufvector/rufllm'));
      console.error(chalk.yellow('  npm install @rufvector/rufllm'));
      console.error(chalk.dim('  or: npx rufvector install rufllm'));
      console.error(chalk.dim('\n  Tip: use "rufvector embed text <text>" for built-in ONNX embeddings'));
      process.exit(1);
    }
    ...
    try {
      const { performance } = require('perf_hooks');
      const start = performance.now();
      const llm = new rufllm.RuvLLM({ embeddingDim: 384 });
...
llmCmd.command('models')
  ...
  .action(async (opts) => {
    ...
    const models = typeof rufllm.listModels === 'function' ? rufllm.listModels() :
      (rufllm.RUVLTRA_MODELS ? Object.values(rufllm.RUVLTRA_MODELS) : []);
    if (opts.json) {
    ...
    if (!models || models.length === 0) {
      ...
      console.log(chalk.dim('  Upgrade @rufvector/rufllm for model registry support:'));
      console.log(chalk.dim('    npm install @rufvector/rufllm@latest\n'));
    } else {
...
llmCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    const version = typeof rufllm.version === 'function' ? rufllm.version() : (rufllm.version || 'unknown');
    const hasSIMD = rufllm.hasSimdSupport ? rufllm.hasSimdSupport() : false;
    const models = rufllm.listModels ? rufllm.listModels() : [];
    const info = {
      package: '@rufvector/rufllm',
      version,
      simd: hasSIMD,
...
sonaCmd.command('status')
  ...
  .action(async (opts) => {
    try {
      const { SonaEngine } = require('@rufvector/sona');
      ...
      try {
        const { SonaCoordinator } = require('@rufvector/rufllm');
        ...
        console.error(chalk.yellow('  Install @rufvector/rufllm for JS fallback: npm install @rufvector/rufllm'));
...
sonaCmd.command('train')
  ...
  .action(async (data, opts) => {
    ...
    try {
      ...
      console.error(chalk.dim('\n  Example: rufvector sona train \'{"steps":[{"type":"query","input":"test","output":"result","confidence":0.9}],"outcome":"success"}\''));
      process.exit(1);
    }
    ...
    try {
      ...
      try {
        const { SonaCoordinator, TrajectoryBuilder } = require('@rufvector/rufllm');
        const sona = new SonaCoordinator();
...
sonaCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    try { const sona = require('@rufvector/sona'); nativeAvailable = !!sona.SonaEngine; nativeVersion = sona.version || '0.1.4'; } catch (e) { /* not available */ }
    let jsAvailable = false, jsVersion = 'N/A';
    try { const rufllm = require('@rufvector/rufllm'); jsAvailable = !!rufllm.SonaCoordinator; jsVersion = typeof rufllm.version === 'function' ? rufllm.version() : (rufllm.version || 'unknown'); } catch (e) { /* not available */ }
    const info = { package: '@rufvector/sona', nativeBinding: nativeAvailable, nativeVersion, jsFallback: jsAvailable, jsFallbackVersion: jsVersion, features: ['LoRA adaptive weights', 'EWC++ memory protection', 'ReasoningBank pattern storage', 'Trajectory tracking', 'Continual learning', 'Sub-millisecond overhead'] };
    ...
    console.log(chalk.white(`  JS fallback:     ${jsAvailable ? chalk.green('Available') + chalk.dim(` (rufllm v${jsVersion})`) : chalk.yellow('Not available')}`));
    console.log(chalk.white('\n  Features:'));
...
// =============================================================================
// Route Commands - Semantic routing (lazy: @rufvector/router)
// =============================================================================
...
routeCmd.command('classify')
  ...
  .action(async (input, opts) => {
    let router;
    try { router = require('@rufvector/router'); } catch (e) {
      console.error(chalk.red('Route commands require @rufvector/router'));
      console.error(chalk.yellow('  npm install @rufvector/router'));
      console.error(chalk.dim('  or: npx rufvector install router'));
      process.exit(1);
    }
    ...
    try {
      ...
      if (!results || results.length === 0) {
        ...
        if (!routesDef) { console.log(chalk.dim('  Provide a routes file with -r/--routes to define intents.')); console.log(chalk.dim('  Example: rufvector route classify "hello" -r routes.json')); }
      } else {
        for (let i = 0; i < results.length; i++) {
          const r = results[i];
...
routeCmd.command('info')
  ...
  .action(async (opts) => {
    ...
    try { const router = require('@rufvector/router'); routerAvailable = true; routerVersion = router.version || '0.1.28'; const sr = new router.SemanticRouter({ dimension: 16 }); metrics = { distanceMetrics: Object.keys(router.DistanceMetric || {}), hasSemanticRouter: true }; } catch (e) { /* Not available */ }
    const info = { package: '@rufvector/router', available: routerAvailable, version: routerVersion, backend: 'Rust NAPI (HNSW + SIMD)', features: ['Semantic intent matching', 'HNSW indexing', 'SIMD-accelerated search', 'Multiple distance metrics', 'Save/load state', 'Async embedding support'], ...metrics };
    if (opts.json) { console.log(JSON.stringify(info, null, 2)); return; }
...
const decompileCmd = program
  ...
  .action(async (target, opts) => {
    ...
    if (!target) {
      console.log(chalk.cyan('\nUsage:'));
      console.log(chalk.white('  rufvector decompile <package>           Decompile npm package'));
      console.log(chalk.white('  rufvector decompile <pkg>@<ver>         Specific version'));
      console.log(chalk.white('  rufvector decompile ./bundle.js         Local file'));
      console.log(chalk.white('  rufvector decompile https://unpkg.com/x URL'));
      console.log(chalk.white('  rufvector decompile --model <file.gguf> LLM weight file'));
      console.log(chalk.white('  rufvector decompile --api <model-id>    Probe remote API'));
      console.log(chalk.dim('\nOptions:'));
...
const optimizeCmd = program.command('optimize')
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      console.error(chalk.yellow('\n  rufvector optimize: not yet shipped in this release.\n'));
      ...
      console.error(chalk.white('    https://github.com/ruvnet/rufvector/issues/401\n'));
      process.exit(1);
    }
    ...
    if (opts.list) {
      ...
      console.log('');
      console.log(chalk.dim('  Usage: rufvector optimize --profile <type>'));
      console.log(chalk.dim('         rufvector optimize --generate-settings --profile coding'));
      console.log('');
      return;
    }
...
// Borrows metaharness concepts using primitives rufvector already ships:
...
function buildHarnessSurface() {
  ...
  try {
    ...
    primitives.costRouter = {
      ...
      role: 'cost-optimal model routing (cheap vs strong)',
      available: false,
      install: 'npm install @rufvector/tiny-dancer',
    };
  }

  // Semantic intent router — @rufvector/router / rufvector-router-core
  let semanticAvailable = false;
  try { require.resolve('@rufvector/router'); semanticAvailable = true; } catch { semanticAvailable = false; }
  primitives.semanticRouter = {
    name: '@rufvector/router',
    role: 'semantic intent routing',
    available: semanticAvailable,
    ...(semanticAvailable ? { usage: 'npx rufvector router --route "<text>"' } : { install: 'npm install @rufvector/router' }),
  };
  ...
  primitives.hooksRouting = {
    ...
    available: true,
    usage: 'npx rufvector hooks route "<task>"',
  };
  ...
  primitives.memory = {
    ...
    available: true,
    namespace: (process.env.RUVECTOR_MEMORY_NAMESPACE || 'rufvector').trim() || 'rufvector',
  };

  const values = Object.values(primitives);
  return {
    adr: 'ADR-256',
    decision: 'borrow metaharness concepts using primitives rufvector already ships',
    primitives,
    summary: {
...
function printHarnessStatus(opts) {
  ...
  console.log(chalk.cyan('\n═══════════════════════════════════════════════════════════════'));
  console.log(chalk.cyan('              RufVector Harness Router (ADR-256)'));
  console.log(chalk.cyan('═══════════════════════════════════════════════════════════════\n'));
...
// Bare `rufvector harness` defaults to status
