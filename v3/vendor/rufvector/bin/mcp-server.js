 ...
 *
 * Model Context Protocol server for RufVector hooks
 * Provides self-learning intelligence tools for Claude Code
 *
 * Usage:
 *   npx rufvector mcp start
...
// .rufvector/intelligence.json enforce the same contract instead of bypassing
...
// Intelligence class with full RufVector stack support
class Intelligence {
  constructor() {
  ...
  // (vectors without provenance) are read-only until `rufvector hooks reembed`,
  ...
  checkVectorWrite(active) {
    ...
    if (this.isLegacyVectorStore()) {
      ...
      const err = new Error(
        ...
        `${provenanceMod.describeProvenance(active)}. Run 'rufvector hooks reembed' to re-embed and unlock it.`
      );
  ...
  guardVectorWrite(active) {
    try {
      ...
      if (policy === 'auto') e.message += ` (RUVECTOR_REEMBED=auto: run 'rufvector hooks reembed' — in-place re-embedding needs the CLI)`;
      throw e;
    }
  }
  ...
  warnRecallProvenance(active) {
    ...
    if (mismatches.length > 0) {
      ...
      console.error(
        `rufvector: recall quality degraded — stored vectors are ${provenanceMod.describeProvenance(stored)} ` +
...
const TOOLS = [
  {
    name: 'hooks_stats',
    description: 'Get RufVector intelligence statistics including learned patterns, memories, and trajectories',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'hooks_init',
    description: 'Initialize RufVector hooks in the current project',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get RufVector engine capabilities (VectorDB, SONA, Attention)',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'rvf_examples',
    description: 'List available example .rvf files with download URLs from the rufvector repository',
    inputSchema: {
      type: 'object',
      properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'hooks_init': {
        let cmd = 'npx rufvector hooks init';
      ...
      case 'hooks_pretrain': {
        let cmd = 'npx rufvector hooks pretrain';
      ...
      case 'hooks_build_agents': {
        let cmd = 'npx rufvector hooks build-agents';
      ...
      case 'hooks_verify': {
        try {
          const output = execSync('npx rufvector hooks verify', { encoding: 'utf-8', timeout: 15000 });
          return {
            content: [{
              type: 'text',
              text: JSON.stringify({ success: true, output }, null, 2)
      ...
      case 'hooks_doctor': {
        let cmd = 'npx rufvector hooks doctor';
        if (args.fix) cmd += ' --fix';
            ...
            if (withVectors.length > 0 && provenanceMod) {
              if (intel.isLegacyVectorStore()) {
                const err = new Error(
                  ...
                  `Run 'rufvector hooks reembed' before importing vector memories.`
                );
                err.code = 'ERR_LEGACY_STORE_READONLY';
                throw err;
              }
      ...
      case 'hooks_ast_analyze': {
        try {
          ...
          const output = execSync(`npx rufvector hooks ast-analyze "${safeFile}" --json`, { encoding: 'utf-8', timeout: 30000 });
      ...
      case 'hooks_ast_complexity': {
        try {
          ...
          const output = execSync(`npx rufvector hooks ast-complexity ${filesArg} --threshold ${threshold}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_diff_analyze': {
        try {
          const cmd = args.commit ? `npx rufvector hooks diff-analyze "${sanitizeShellArg(args.commit)}" --json` : 'npx rufvector hooks diff-analyze --json';
          const output = execSync(cmd, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
      ...
      case 'hooks_diff_classify': {
        try {
          const cmd = args.commit ? `npx rufvector hooks diff-classify "${sanitizeShellArg(args.commit)}"` : 'npx rufvector hooks diff-classify';
          const output = execSync(cmd, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
      ...
      case 'hooks_diff_similar': {
        try {
          ...
          const output = execSync(`npx rufvector hooks diff-similar -k ${topK} --commits ${commits}`, { encoding: 'utf-8', timeout: 120000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_coverage_route': {
        try {
          const safeFile = sanitizeShellArg(args.file);
          const output = execSync(`npx rufvector hooks coverage-route "${safeFile}"`, { encoding: 'utf-8', timeout: 15000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_coverage_suggest': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx rufvector hooks coverage-suggest ${filesArg}`, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_graph_mincut': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx rufvector hooks graph-mincut ${filesArg}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_graph_cluster': {
        try {
          ...
          const output = execSync(`npx rufvector hooks graph-cluster ${filesArg} --method ${method} --clusters ${clusters}`, { encoding: 'utf-8', timeout: 60000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_security_scan': {
        try {
          const filesArg = args.files.map(f => `"${sanitizeShellArg(f)}"`).join(' ');
          const output = execSync(`npx rufvector hooks security-scan ${filesArg}`, { encoding: 'utf-8', timeout: 120000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_rag_context': {
        try {
          ...
          const topK = parseInt(args.top_k, 10) || 5;
          let cmd = `npx rufvector hooks rag-context "${safeQuery}" -k ${topK}`;
      ...
      case 'hooks_git_churn': {
        try {
          ...
          const output = execSync(`npx rufvector hooks git-churn --days ${days} --top ${top}`, { encoding: 'utf-8', timeout: 30000 });
          return { content: [{ type: 'text', text: output }] };
        } catch (e) {
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }] };
        }
      ...
      case 'hooks_route_enhanced': {
        // Issue #463/#422: previously shelled out via `npx rufvector hooks
      ...
      case 'hooks_gnn_info': {
        ...
        try {
          ...
          gnnInfo = {
            ...
            layers: [
              { name: 'RufvectorLayer', description: 'Differentiable vector search layer' },
      ...
      case 'rvf_create': {
        try {
          ...
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message, hint: 'Install @rufvector/rvf: npm install @rufvector/rvf' }, null, 2) }], isError: true };
        }
      }
      ...
      case 'rvf_examples': {
        const BASE_URL = 'https://raw.githubusercontent.com/ruvnet/rufvector/main/examples/rvf/output';
        const examples = [
        ...
        return { content: [{ type: 'text', text: JSON.stringify({
          ...
          catalog: 'https://github.com/ruvnet/rufvector/tree/main/examples/rvf/output'
        }, null, 2) }] };
      }
      ...
      case 'brain_search': {
        try {
          const piBrain = require('@rufvector/pi-brain');
          ...
          if (e.code === 'MODULE_NOT_FOUND' || e.code === 'ERR_REQUIRE_ESM' || e.code === 'ERR_PACKAGE_PATH_NOT_EXPORTED') {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: 'Brain tools require @rufvector/pi-brain', hint: 'npm install @rufvector/pi-brain' }, null, 2) }] };
          }
          return { content: [{ type: 'text', text: JSON.stringify({ success: false, error: e.message }, null, 2) }], isError: true };
        }
      }
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  return {
    resources: [
      {
        uri: 'rufvector://intelligence/stats',
        name: 'Intelligence Stats',
        description: 'Current RufVector intelligence statistics',
        mimeType: 'application/json'
      },
      {
        uri: 'rufvector://intelligence/patterns',
        name: 'Learned Patterns',
        description: 'Q-learning patterns for agent routing',
        mimeType: 'application/json'
      },
      {
        uri: 'rufvector://intelligence/memories',
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  switch (uri) {
    case 'rufvector://intelligence/stats':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
    ...
    case 'rufvector://intelligence/patterns':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
    ...
    case 'rufvector://intelligence/memories':
      return {
        contents: [{
          uri,
          mimeType: 'application/json',
...
async function main() {
  ...
  console.error('RufVector MCP server running on stdio');
