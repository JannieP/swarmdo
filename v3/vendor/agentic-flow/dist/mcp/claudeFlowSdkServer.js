...
import { execMemoryStore, execMemoryRetrieve, execMemorySearch, execAgentSpawn, safeExecNpx, } from '../utils/safe-exec.js';
let _AgentBooster = null;
async function loadAgentBooster() {
    if (_AgentBooster)
        return _AgentBooster;
    try {
        ...
        throw new Error(`Agent Booster is unavailable (optional package 'agent-booster' not installed). ` +
        ...
        tool('agent_booster_batch_edit', 'Apply multiple code edits in parallel using Agent Booster. Perfect for multi-file refactoring.', {
            ...
            try {
                ...
                for (const edit of edits) {
                    ...
                    results.push(`  ${result.success ? '✅' : '❌'} ${safePath} (${result.latency}ms, ${(result.confidence * 100).toFixed(0)}%)`);
                }
