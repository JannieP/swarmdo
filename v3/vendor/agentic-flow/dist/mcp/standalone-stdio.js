...
server.addTool({
    ...
    execute: async ({ agent, task, priority, max_cost }) => {
        try {
            const args = ['--yes', 'agentic-flow', '--agent', agent, '--task', task, '--optimize'];
...
server.addTool({
    ...
    execute: async ({ sessionId, task, reward, success, critique, input, output, latencyMs, tokensUsed }) => {
        try {
            ...
            if (critique)
                cliArgs.push(critique);
            if (input)
                cliArgs.push(input);
            if (output)
                cliArgs.push(output);
            if (latencyMs !== undefined)
                cliArgs.push(latencyMs.toString());
            if (tokensUsed !== undefined)
                cliArgs.push(tokensUsed.toString());
...
server.addTool({
    ...
    execute: async ({ task, k, minReward, onlySuccesses, onlyFailures }) => {
        try {
            const cliArgs = ['agentdb', 'reflexion', 'retrieve', task, (k || 5).toString()];
            if (minReward !== undefined)
                cliArgs.push(minReward.toString());
            if (onlyFailures)
                cliArgs.push('true', 'false');
            else if (onlySuccesses)
                cliArgs.push('false', 'true');
            const result = execFileSync('npx', cliArgs, {
                ...NPX_EXEC_OPTS,
                encoding: 'utf-8',
                maxBuffer: 10 * 1024 * 1024,
