    ...
    execute: async ({ depth, incremental: _incremental, skipGit, skipFiles, verbose }, { onProgress }) => {
        ...
        if (!skipGit) {
            ...
            try {
                ...
                const gitLog = execFileSync('git', ['log', '--name-only', '--pretty=format:COMMIT:%H', '-n', String(safeDepth)], { shell: false, encoding: 'utf-8', maxBuffer: 50 * 1024 * 1024, stdio: ['pipe', 'pipe', 'ignore'] });
