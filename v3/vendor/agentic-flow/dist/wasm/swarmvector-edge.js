 ...
 */
export async function initSwarmVectorWasm() {
    if (wasmInitialized)
        return true;
    if (!isWasmSupported()) {
    ...
    try {
        // Dynamic import of @swarmvector/edge-full/edge (replaces @swarmvector/edge)
        const swarmvectorEdge = await import('@swarmvector/edge-full/edge');
        // Initialize WASM
        if (swarmvectorEdge.default) {
            await swarmvectorEdge.default();
        }
        // Export WASM classes
        WasmIdentity = swarmvectorEdge.WasmIdentity;
        WasmHnswIndex = swarmvectorEdge.WasmHnswIndex;
        WasmSemanticMatcher = swarmvectorEdge.WasmSemanticMatcher;
        wasmModule = swarmvectorEdge;
        wasmInitialized = true;
        logger.info('SwarmVector Edge WASM initialized', {
            features: ['WasmIdentity', 'WasmHnswIndex', 'WasmSemanticMatcher'],
        });
        return true;
    }
    catch (error) {
        logger.debug('Failed to initialize SwarmVector WASM', { error });
        return false;
    }
}
/**
 ...
 */
export class SwarmVectorHnswIndex {
    wasmIndex = null;
    jsVectors = [];
    dimensions;
    m;
 ...
 */
export class SwarmVectorSemanticMatcher {
    wasmMatcher = null;
    jsAgents = new Map();
    constructor() {
...
export default {
    ...
    signData,
    verifySignature,
    SwarmVectorHnswIndex,
    SwarmVectorSemanticMatcher,
};
//# sourceMappingURL=swarmvector-edge.js.map