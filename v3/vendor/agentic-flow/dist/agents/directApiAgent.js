...
async function executeToolCall(toolName, toolInput) {
    try {
        ...
        const runClaudeFlow = (args) => execFileSync('npx', ['claude-flow@alpha', ...args], { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
        switch (toolName) {
            case 'memory_store': {
                ...
                if (ttl !== undefined)
                    args.push('--ttl', String(Number(ttl)));
                runClaudeFlow(args);
            ...
            case 'memory_search': {
                ...
                if (namespace)
                    args.push('--namespace', String(namespace));
            ...
            case 'agent_spawn': {
                ...
                if (capabilities)
                    args.push('--capabilities', capabilities.map(String).join(','));
                if (name)
            ...
            case 'task_orchestrate': {
                ...
                if (maxAgents !== undefined)
                    args.push('--max-agents', String(Number(maxAgents)));
                const result = runClaudeFlow(args);
