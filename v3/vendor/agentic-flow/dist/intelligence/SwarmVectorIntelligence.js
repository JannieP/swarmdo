/**
 * SwarmVector Unified Intelligence Layer
 *
 * Integrates the FULL power of SwarmVector ecosystem:
 *
 * @swarmvector/sona - Self-Learning:
 ...
 *
 * @swarmvector/attention - Advanced Attention:
 ...
 *
 * @swarmvector/core - Vector Database:
...
// Try to load @swarmvector/sona
try {
    const sonaModule = await import('@swarmvector/sona');
    SonaEngine = sonaModule.SonaEngine;
    sonaAvailable = true;
}
catch {
    console.warn('[SwarmVectorIntelligence] @swarmvector/sona not available - self-learning disabled');
}
// Try to load @swarmvector/attention
try {
    const attentionModule = await import('@swarmvector/attention');
    ...
    console.warn('[SwarmVectorIntelligence] @swarmvector/attention not available - attention features disabled');
}
// Import from swarmvector core (for HNSW)
import swarmvector from 'swarmvector';
// Export availability flags
export { sonaAvailable, attentionAvailable };
export class SwarmVectorIntelligence {
    config;
    initialized = false;
    initPromise = null;
    ...
    async initializeAsync() {
        ...
        if (this.config.enableSona) {
            try {
                ...
                console.warn('[SwarmVectorIntelligence] SONA init failed, using fallback:', err);
                this.sona = null;
            }
        }
        ...
        try {
            ...
            console.warn('[SwarmVectorIntelligence] Attention init failed, using fallback:', err);
        }
        ...
        try {
            ...
            console.warn('[SwarmVectorIntelligence] Optimizer init failed:', err);
        }
        ...
        if (this.config.enableHnsw) {
            try {
                ...
                console.warn('[SwarmVectorIntelligence] HNSW init failed:', err);
            }
        }
    ...
    initializeHnsw() {
        try {
            // Use swarmvector core HNSW
            this.hnswIndex = new swarmvector.HnswIndex({
...
export function createIntelligenceLayer(config) {
    return new SwarmVectorIntelligence(config);
}
/**
...
//# sourceMappingURL=SwarmVectorIntelligence.js.map