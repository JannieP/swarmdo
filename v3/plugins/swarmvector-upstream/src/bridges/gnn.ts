 ...
 *
 * Bridge to swarmvector-gnn-wasm for graph-based learning and inference.
 */
...
export class GnnBridge implements WasmBridge<GnnModule> {
  readonly name = 'swarmvector-gnn-wasm';
  ...
  async init(): Promise<void> {
    ...
    try {
      const wasmModule = await import('@swarmvector/gnn-wasm').catch(() => null);
