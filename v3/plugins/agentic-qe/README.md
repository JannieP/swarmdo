...
This plugin adds 58 AI agents to Swarmdo that handle all aspects of software quality:
...
**Via Swarmdo CLI (recommended):**
```bash
npx swarmdo plugins install --name @swarmdo/plugin-agentic-qe
```
...
```bash
npx swarmdo@v3alpha mcp call aqe/generate-tests \
...
```bash
npx swarmdo@v3alpha mcp call aqe/tdd-cycle \
...
```bash
npx swarmdo@v3alpha mcp call aqe/security-scan \
  --targetPath ./src \
  --scanType sast \
...
```bash
npx swarmdo@v3alpha mcp call aqe/predict-defects \
...
npx swarmdo@v3alpha mcp call aqe/chaos-inject \
