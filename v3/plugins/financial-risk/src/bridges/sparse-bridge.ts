 ...
 * swarmvector-sparse-inference-wasm for anomaly detection and
...
/**
 * WASM module interface for swarmvector-sparse-inference-wasm
 */
...
export class FinancialSparseBridge implements SparseBridge {
  ...
  private async resolveWasmPath(): Promise<string | null> {
    try {
      const module = await import(/* webpackIgnore: true */ 'swarmvector-sparse-inference-wasm' as string) as { default?: string };
