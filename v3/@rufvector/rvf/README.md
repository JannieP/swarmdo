...
**Browser**: The `@rufvector/rvf-wasm` package provides a ~46 KB control-plane WASM module plus a ~5.5 KB tile-compute module. Works in any browser with WebAssembly support.
...
npm install @rufvector/rvf-wasm
```
...
```typescript
import { RvfDatabase } from '@rufvector/rvf';
...
```html
<script type="module">
import init, { RvfStore } from '@rufvector/rvf-wasm';
...
import init, { RvfStore } from "npm:@rufvector/rvf-wasm";
...
```bash
npm install ruvector @rufvector/rvf
...
```bash
npm install rvlite @rufvector/rvf-wasm
```

When `@rufvector/rvf-wasm` is installed, rvlite can use RVF as a persistent storage backend:

```typescript
import { createRvLite } from 'rvlite';

// rvlite auto-detects @rufvector/rvf-wasm for persistence
...
|---------|-------------|---------|
| `@rufvector/rvf` | Unified SDK (this package) | Node.js |
| `@rufvector/rvf-node` | Native N-API bindings | Node.js |
| `@rufvector/rvf-wasm` | WASM build (~46 KB + ~5.5 KB tile) | Browser, Deno, Edge |
| `@rufvector/rvf-mcp-server` | MCP server for AI agents | Node.js |
