/**
 * @rufvector/learning-wasm smoke test (node:test).
 * Fork of the upstream @ruvector/learning-wasm (prebuilt WASM/dist; package renamed
...
test('package loads via its published entry', async () => {
  const mod = await import('@rufvector/learning-wasm');
...
test('package.json name is the @rufvector fork name', async () => {
  ...
  assert.equal(pkg.name, '@rufvector/learning-wasm', 'name renamed to fork');
});
