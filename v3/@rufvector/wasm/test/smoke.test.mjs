/**
 * @rufvector/wasm smoke test (node:test).
 * Fork of the upstream @ruvector/wasm (prebuilt WASM/dist; package renamed
...
test('package loads via its published entry', async () => {
  const mod = await import('@rufvector/wasm');
...
test('package.json name is the @rufvector fork name', async () => {
  ...
  assert.equal(pkg.name, '@rufvector/wasm', 'name renamed to fork');
});
