...
[![npm version](https://img.shields.io/npm/v/@rufvector/wasm.svg)](https://www.npmjs.com/package/@rufvector/wasm)
...
```bash
npm install @rufvector/wasm
```

### Yarn

```bash
yarn add @rufvector/wasm
```
...
```html
<script type="module">
  import init, { VectorDB } from 'https://unpkg.com/@rufvector/wasm/pkg/ruvector_wasm.js';
...
```javascript
import init, { VectorDB } from '@rufvector/wasm';
...
`@rufvector/wasm/adapter` wraps `VectorDB` with a metadata sidecar and a real
`similarity = 1 - distance` so the documented "higher is better" contract holds.

```javascript
import { RuvectorWasmAdapter } from '@rufvector/wasm/adapter';
...
```javascript
// main.js
import { WorkerPool } from '@rufvector/wasm/worker-pool';
...
```javascript
import { IndexedDBPersistence } from '@rufvector/wasm/indexeddb';
import init, { VectorDB } from '@rufvector/wasm';
...
```javascript
import { detectSIMD } from '@rufvector/wasm';

if (detectSIMD()) {
  console.log('SIMD acceleration available!');
  // Load SIMD-optimized build
  await import('@rufvector/wasm/pkg-simd/ruvector_wasm.js');
} else {
  console.log('Standard build');
  // Load standard build
  await import('@rufvector/wasm');
}
```
...
const loadVectorDB = async () => {
  const { default: init, VectorDB } = await import('@rufvector/wasm');
  await init();
  return VectorDB;
};
