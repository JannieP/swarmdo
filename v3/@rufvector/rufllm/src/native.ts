/**
 * Native bindings loader for RufLLM
 *
...
let nativeModule: NativeRufLLM | null = null;

interface NativeRufLLM {
  // Native exports RuvLlmEngine (camelCase), we normalize to RufLLMEngine
  RufLLMEngine: new (config?: NativeConfig) => NativeEngine;
...
const PLATFORM_PACKAGES: Record<string, string> = {
  'darwin-x64': '@rufvector/rufllm-darwin-x64',
  'darwin-arm64': '@rufvector/rufllm-darwin-arm64',
  'linux-x64': '@rufvector/rufllm-linux-x64-gnu',
  'linux-arm64': '@rufvector/rufllm-linux-arm64-gnu',
  'win32-x64': '@rufvector/rufllm-win32-x64-msvc',
};
...
function loadNativeModule(): NativeRufLLM | null {
  if (nativeModule) {
    return nativeModule;
  }
  ...
  const attempts = [
    // Rufflo fork: bundled native binary shipped with this package
    () => require(join(__dirname, '..', '..', 'platforms', platformKey, 'rufllm.node')),
    // Try the platform-specific package
    () => require(packageName),
    // Try loading from local .node file (CJS build)
    () => require(join(__dirname, '..', '..', 'rufllm.node')),
  ...
  for (const attempt of attempts) {
    try {
      ...
      // Normalize: native exports RuvLlmEngine, we expose as RufLLMEngine
      nativeModule = {
        RufLLMEngine: raw.RufLLMEngine ?? raw.RuvLlmEngine!,
...
export function getNativeModule(): NativeRufLLM | null {
