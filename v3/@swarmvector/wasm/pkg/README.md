...
```bash
npm install @swarmvector/wasm
```

### Yarn

```bash
yarn add @swarmvector/wasm
```
...
```html
<script type="module">
  import init, { VectorDB } from 'https://unpkg.com/@swarmvector/wasm/pkg/swarmvector_wasm.js';
...
```javascript
import init, { VectorDB } from '@swarmvector/wasm';
...
`@swarmvector/wasm/adapter` wraps `VectorDB` with a metadata sidecar and a real
...
```javascript
// main.js
import { WorkerPool } from '@swarmvector/wasm/worker-pool';

const pool = new WorkerPool(
  '/worker.js',
  '/pkg/swarmvector_wasm.js',
  {
...
importScripts('/pkg/swarmvector_wasm.js');
...
self.onmessage = async (e) => {
  ...
  switch (type) {
    case 'init':
      await wasm_bindgen('/pkg/swarmvector_wasm_bg.wasm');
...
```javascript
import { IndexedDBPersistence } from '@swarmvector/wasm/indexeddb';
import init, { VectorDB } from '@swarmvector/wasm';
...
```javascript
import { detectSIMD } from '@swarmvector/wasm';

if (detectSIMD()) {
  console.log('SIMD acceleration available!');
  // Load SIMD-optimized build
  await import('@swarmvector/wasm/pkg-simd/swarmvector_wasm.js');
} else {
  console.log('Standard build');
  // Load standard build
  await import('@swarmvector/wasm');
}
```
...
const loadVectorDB = async () => {
  const { default: init, VectorDB } = await import('@swarmvector/wasm');
  await init();
  return VectorDB;
};
...
const wasmUrl = new URL('./pkg/swarmvector_wasm_bg.wasm', import.meta.url);
...
- **[Live Demo](https://swarmvector-demo.vercel.app)** - Try it in your browser
- **[CodeSandbox](https://codesandbox.io/s/swarmvector-wasm)** - Interactive playground
