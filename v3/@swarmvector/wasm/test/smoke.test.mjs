/**
 * @swarmvector/wasm smoke test (node:test).
 * Fork of the upstream @swarmvector/wasm (prebuilt WASM/dist; package renamed
...
test('package loads via its published entry', async () => {
  const mod = await import('@swarmvector/wasm');
...
test('package.json name is the @swarmvector fork name', async () => {
  ...
  assert.equal(pkg.name, '@swarmvector/wasm', 'name renamed to fork');
});
