/**
 * @swarmvector/core smoke test (node:test — no extra deps).
 *
 ...
 *   3. the default on-disk store is `vector.db` (NOT the legacy `swarmvector.db`) —
...
test('construct / insert / search / len', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'swarmvector-'));
  try {
...
test('default on-disk store is vector.db, never swarmvector.db', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'swarmvector-default-'));
  ...
  try {
    ...
    assert.ok(!existsSync(join(dir, 'swarmvector.db')), 'legacy swarmvector.db must NOT appear');
  } finally {
    process.chdir(cwd);
    rmSync(dir, { recursive: true, force: true });
  }
