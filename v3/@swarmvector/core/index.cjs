"use strict";
/**
 * @swarmvector/core - CommonJS wrapper
 *
 * Swarmdo in-repo fork of @swarmvector/core. Loads the bundled native redb+HNSW
...
function loadNativeBinding() {
  ...
  if (triple) {
    ...
    try {
      return require(join(__dirname, "platforms", triple, "swarmvector.node"));
    } catch (e) {
      errors.push(`bundled: ${e.message}`);
    }
    // 2) platform package (for future multi-platform publishing)
    try {
      return require(`@swarmvector/core-${triple}`);
    } catch (e) {
      errors.push(`@swarmvector/core-${triple}: ${e.message}`);
    }
  }
  throw new Error(
    `@swarmvector/core: no native binding for ${key}.\n` + errors.join("\n")
  );
}
...
let attention = null;
try {
  attention = require("@swarmvector/attention");
} catch {
  /* optional */
}
