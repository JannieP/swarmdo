 ...
 */
function toNativeConfig(config?: SwarmLLMConfig): NativeConfig | undefined {
 ...
 * ```
 */
export class SwarmLLM {
  private native: NativeEngine | null = null;
  private config: SwarmLLMConfig;
  ...
  constructor(config?: SwarmLLMConfig) {
    ...
    if (mod) {
      try {
        this.native = new mod.SwarmLLMEngine(toNativeConfig(config));
      } catch {
  ...
  generate(prompt: string, config?: GenerationConfig): string {
    ...
    return `[SwarmLLM JavaScript Fallback Mode]
...
1. Install the native bindings: npm install @swarmvector/swarmllm-${process.platform}-${process.arch}
