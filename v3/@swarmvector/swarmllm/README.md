...
```typescript
import { SwarmLLM, SwarmLLMConfig } from '@swarmvector/swarmllm';
...
```typescript
import { simd } from '@swarmvector/swarmllm/simd';
...
```bash
# Query a model
swarmllm query "What is machine learning?"
...
swarmllm eval --model ./models/model.gguf --subset lite --max-tasks 50
```
...
```typescript
import { SwarmLLM, EvaluationHarness, AblationMode } from '@swarmvector/swarmllm';
...
```typescript
import { SwarmLLM, MistralBackend, PagedAttentionConfig } from '@swarmvector/swarmllm';
...
const llm = new SwarmLLM({ backend });
