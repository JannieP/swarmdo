...
Unified TypeScript/JavaScript SDK for the **SwarmVector Format (RVF)** — a cognitive container that stores vectors, carries models, boots compute kernels, and proves everything in a single `.rvf` file.
...
**Browser**: The `@swarmvector/rvf-wasm` package provides a ~46 KB control-plane WASM module plus a ~5.5 KB tile-compute module. Works in any browser with WebAssembly support.
...
npm install @swarmvector/rvf-wasm
```
...
```html
<script type="module">
import init, { RvfStore } from '@swarmvector/rvf-wasm';
...
import init, { RvfStore } from "npm:@swarmvector/rvf-wasm";
...
RVF (SwarmVector Format) is a universal binary substrate that merges database, model, graph engine, kernel, and attestation into a single deployable file. A `.rvf` file is segmented — each segment carries a different payload type, and unknown segments are preserved by all tools.
...
| `swarmbot.rvf` | 51 KB | Chatbot knowledge store |
...
| `swarmllm_inference.rvf` | 133 KB | SwarmLLM inference cache |
...
### With `swarmvector` (npx swarmvector)

The `swarmvector` npm package includes 8 RVF CLI commands:
...
npx swarmvector rvf create mydb.rvf -d 384
npx swarmvector rvf ingest mydb.rvf --input data.json
npx swarmvector rvf query mydb.rvf --vector "[0.1,...]" --k 10
...
```bash
npm install rvlite @swarmvector/rvf-wasm
```

When `@swarmvector/rvf-wasm` is installed, rvlite can use RVF as a persistent storage backend:

```typescript
import { createRvLite } from 'rvlite';

// rvlite auto-detects @swarmvector/rvf-wasm for persistence
...
|---------|-------------|---------|
| `@swarmvector/rvf` | Unified SDK (this package) | Node.js |
| `@swarmvector/rvf-node` | Native N-API bindings | Node.js |
| `@swarmvector/rvf-wasm` | WASM build (~46 KB + ~5.5 KB tile) | Browser, Deno, Edge |
| `@swarmvector/rvf-mcp-server` | MCP server for AI agents | Node.js |
