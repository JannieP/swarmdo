...
// ---------------------------------------------------------------------------
// NodeBackend — wraps @swarmvector/rvf-node (N-API)
// ---------------------------------------------------------------------------
/**
 * Backend that delegates to the `@swarmvector/rvf-node` native N-API addon.
 *
...
class NodeBackend {
    ...
    async loadNative() {
        ...
        try {
            ...
            const mod = await Promise.resolve().then(() => __importStar(require('@swarmvector/rvf-node')));
            this.native = mod.RvfDatabase ?? mod.default?.RvfDatabase ?? mod;
        }
        catch {
            throw new errors_1.RvfError(errors_1.RvfErrorCode.BackendNotFound, 'Could not load @swarmvector/rvf-node — is it installed?');
        }
    }
    ensureHandle() {
        if (!this.handle) {
...
// ---------------------------------------------------------------------------
// WasmBackend — wraps @swarmvector/rvf-wasm
// ---------------------------------------------------------------------------
/**
 * Backend that delegates to the `@swarmvector/rvf-wasm` WASM build.
 *
...
class WasmBackend {
    ...
    async loadWasm() {
        if (this.wasm)
            return;
        try {
            const mod = await Promise.resolve().then(() => __importStar(require('@swarmvector/rvf-wasm')));
            ...
            throw new errors_1.RvfError(errors_1.RvfErrorCode.BackendNotFound, 'Could not load @swarmvector/rvf-wasm — is it installed?');
        }
    }
    ensureHandle() {
        if (!this.handle) {
