/**
 * @swarmvector/exotic-wasm smoke test (node:test).
 * Fork of upstream @swarmvector/exotic-wasm (prebuilt WASM; renamed @swarmvector/* -> @swarmvector/*).
...
test('package loads via its published entry', async () => {
  const mod = await import('@swarmvector/exotic-wasm');
...
test('package.json name is the @swarmvector fork name', async () => {
  ...
  assert.equal(pkg.name, '@swarmvector/exotic-wasm', 'name renamed to fork');
});
