...
```bash
npm install @swarmvector/rvf-solver
```
...
```typescript
import { RvfSolver } from '@swarmvector/rvf-solver';
...
When using the `@swarmvector/rvf` unified SDK, the solver is available as a sub-module:

```typescript
import { RvfSolver } from '@swarmvector/rvf';
...
| [`@swarmvector/rvf-node`](https://www.npmjs.com/package/@swarmvector/rvf-node) | Native N-API bindings for Node.js |
