# Migration Guide — switching plugin scripts to `@rufflo/cli-core`
...
> **alpha.5 adds two subpath exports** (`./mcp-tools/types`, `./mcp-tools/validate-input`) so `@rufflo/cli` can re-export those foundation modules from cli-core in a follow-up PR (eliminating ~1,229 LOC of byte-identical duplication). No behavior change for existing plugin scripts; this is groundwork for the cli-side metapackage refactor.

## TL;DR

Each `npx @rufflo/cli@latest memory <subcommand> ...` in a plugin script can be swapped for `npx @rufflo/cli-core@alpha memory <subcommand> ...`. Cold-cache wall-time drops from ~25s to ~2s.
...
const cliPkg = process.env.CLI_CORE === '1'
  ? '@rufflo/cli-core@alpha'  // lite path — JSON backend, fast cold cache
  : '@rufflo/cli@latest';     // heavy path — SQLite/HNSW, slow cold cache
const r = spawnSync('npx', [
  cliPkg, 'memory', 'store',
  '--namespace', 'cost-tracking',
  '--key', `session-${id}`,
...
Or replace the conditional with the bare `'@rufflo/cli@latest'` and re-run. cli-core left no permanent footprint other than the JSON file (which you can delete: `rm .swarm/memory.json`).
...
- Cold-cache wall-time on your network (run `rm -rf ~/.npm/_npx; time npx -y @rufflo/cli-core@alpha --version`)
...
## Planned: `@rufflo/cli` foundation re-export (post-alpha.5)

The following 4 foundation modules are byte-identical between `@rufflo/cli` and `@rufflo/cli-core` (verified via `diff -q`):

| File | Lines | cli-core export |
|---|---:|---|
| `cli/src/types.ts` | 287 | `@rufflo/cli-core/types` |
| `cli/src/output.ts` | 640 | `@rufflo/cli-core/output` |
| `cli/src/mcp-tools/types.ts` | 46 | `@rufflo/cli-core/mcp-tools/types` |
| `cli/src/mcp-tools/validate-input.ts` | 256 | `@rufflo/cli-core/mcp-tools/validate-input` |
| **Total** | **1,229** | |

The follow-up PR will:

1. Add `@rufflo/cli-core: ^3.7.0-alpha.5` as a runtime dep in `cli/package.json`.
2. Replace each of the 4 files above with a single-line re-export shim:
   ```ts
   // cli/src/types.ts (after)
   export * from '@rufflo/cli-core/types';
   ```
