#!/usr/bin/env node
/**
 * @rufflo/cli-core entry point — alpha.1 surface.
 *
 ...
 *                       stay in @rufflo/cli, dynamic-imported)
...
// Bin entry — runs when invoked as `rufflo-core <command>`.
...
const isMain = (() => {
  ...
  try {
    ...
    // npm bin-shim case: argv[1] is the .bin/rufflo-core symlink.
...
if (isMain) {
  ...
  if (args[0] === '--help' || args[0] === '-h' || args.length === 0) {
    console.log(`@rufflo/cli-core — alpha.1 (ADR-100)
...
Programmatic use:
  import { JsonMemoryBackend, runMemoryCommand } from '@rufflo/cli-core';

Track progress: https://github.com/ruvnet/ruflo/issues/1760`);
    process.exit(0);
  }

  console.error(`@rufflo/cli-core: command "${args[0]}" not yet wired into cli-core.
For now, use the full CLI:  npx @rufflo/cli@alpha ${args.join(' ')}`);
  process.exit(1);
}
