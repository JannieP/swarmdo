 ...
 *
 * Loads @rufflo/security validators when available
...
// Try to load the full @rufflo/security module for enhanced validation
...
async function getSecurityModule(): Promise<Record<string, any> | null> {
  ...
  try {
    // Dynamic import — @rufflo/security is an optional dependency
    _securityModule = await (Function('return import("@rufflo/security")')() as Promise<Record<string, any>>);
  } catch {
    // @rufflo/security is optional — fallback to inline validation above
  }
  return _securityModule;
}

/**
 * Enhanced validation using @rufflo/security Zod schemas when available.
...
export async function validateAgentSpawn(input: Record<string, unknown>): Promise<{ valid: boolean; errors: string[] }> {
  ...
  // Fix for #1567: @rufflo/security's SpawnAgentSchema expects `type` and
