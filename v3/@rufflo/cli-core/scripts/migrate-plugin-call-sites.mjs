 ...
 *
 * Scans a plugin (or any .mjs/.js) directory for `npx @rufflo/cli`
...
for (const f of files) {
  ...
  // Pattern 1: spawnSync('npx', ['@rufflo/cli@latest', 'memory', 'store', ...])
  ...
  for (const op of SAFE_MEM_OPS) {
    const pattern = new RegExp(
      `(['"\`])@rufflo/cli@latest\\1(\\s*,\\s*['"\`]${op}\\b)`,
      'g',
    );
    if (pattern.test(updated)) {
      updated = updated.replace(
        pattern,
        `$1\${process.env.CLI_CORE === '1' ? '@rufflo/cli-core@alpha' : '@rufflo/cli@latest'}\$1$2`,
      );
      ...
      changed = false;
      needsReview++;
      console.error(`[review] ${f}: contains @rufflo/cli@latest with ${op} — apply MIGRATION.md env-flag pattern manually`);
      break;
    }
  }

  // Pattern 2: bare 'npx @rufflo/cli@latest <safe-op>' in shell-style.
  ...
  for (const op of SAFE_MEM_OPS) {
    const re = new RegExp(`npx @rufflo/cli@latest ${op}\\b`, 'g');
    if (re.test(updated)) {
      // Check next-token-is-hooks etc. excluded by SAFE list — ok to rewrite.
      const before = updated;
      updated = updated.replace(re, `npx \${CLI_CORE:-@rufflo/cli@latest=@rufflo/cli-core@alpha} ${op}`);
  ...
  for (const op of NEEDS_REVIEW) {
    const re = new RegExp(`@rufflo/cli@latest['"\`]?\\s*,?\\s*['"\`]?${op}\\b`, 'g');
    if (re.test(text)) {
