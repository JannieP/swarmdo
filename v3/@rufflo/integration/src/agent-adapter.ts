/**
 * AgentAdapter - Bridge between Rufflo and agentic-flow Agents
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Rufflo v3 DDD agent architecture
...
/**
 * AgentAdapter - Bridges Rufflo and agentic-flow agents
 *
 ...
 *
 * // Convert agentic-flow agent to Rufflo agent
 * const { agent, success } = adapter.fromAgenticFlow(agenticFlowAgent);
 *
 * // Create Rufflo agent with agentic-flow delegation
...
export class AgentAdapter extends EventEmitter {
  ...
  /**
   * Convert agentic-flow agent to Rufflo AgenticFlowAgent
   *
   * This method creates a Rufflo agent wrapper around an existing
  ...
  fromAgenticFlow(
    ...
    try {
      // Map agentic-flow config to Rufflo config
  ...
  /**
   * Create a Rufflo agent with agentic-flow delegation
   *
  ...
  /**
   * Map agentic-flow agent type to Rufflo AgentType
   */
  ...
  /**
   * Sync status between Rufflo and agentic-flow agents
   */
  private syncStatus(
    agent: AgenticFlowAgent,
    agenticFlowAgent: AgenticFlowAgent_External
  ): void {
    // Map agentic-flow status to Rufflo status
