...
export function getMemoryRoot(): string {
  ...
  const configCandidates = [
    path.resolve(process.cwd(), 'rufflo.config.json'),
    path.resolve(process.cwd(), '.rufflo', 'config.json'),
  ];
 ...
 *
 * Failures are isolated: if @rufflo/memory or agentdb is not installed,
...
export async function loadEmbeddingModel(options?: {
  ...
  try {
    ...
    // Inlined here rather than depending on @rufflo/embeddings to
    // avoid a circular optional-dep at install time; the logic mirrors
    // @rufflo/embeddings/src/transformers-loader.ts.
...
export async function storeEntry(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      return { success: false, id: '', error: 'Database not initialized. Run: rufflo memory init' };
    }
