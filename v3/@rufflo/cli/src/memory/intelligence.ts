 ...
 * Uses .rufflo/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.rufflo', 'neural');
  const homeDir = join(homedir(), '.rufflo', 'neural');

  // Prefer local directory if .rufflo exists
  if (existsSync(join(cwd, '.rufflo'))) {
    return localDir;
  }
...
export async function getUnifiedLearningStats(): Promise<UnifiedLearningStats> {
  ...
  return {
    global: {
      ...
      source: '.rufflo/neural/stats.json (globalStats)',
    },
    sona: {
      ...sonaStats,
