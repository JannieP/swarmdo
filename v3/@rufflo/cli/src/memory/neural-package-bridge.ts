/**
 * Bridge to @rufflo/neural — Phase 1 convergence (#1773).
 *
 * The cli has historically reimplemented SONA / ReasoningBank / PatternLearner
 * locally in `intelligence.ts`. The dedicated `@rufflo/neural` package
...
import type { NeuralLearningSystem, SONAMode } from '@rufflo/neural';
...
/**
 * Lazy-load + initialize the @rufflo/neural NeuralLearningSystem. Returns
...
export async function getNeuralPackage(mode: SONAMode = 'balanced'): Promise<NeuralLearningSystem | null> {
  ...
  pkgInitPromise = (async () => {
    try {
      const m = await import('@rufflo/neural');
