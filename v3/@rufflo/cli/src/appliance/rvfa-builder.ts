 ...
 *
 * Creates a single binary containing kernel, runtime, Rufflo CLI, models/keys,
...
type SectionId = 'kernel' | 'runtime' | 'rufflo' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      ruffloVersion: options.ruffloVersion || detectRuffloVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'rufflo',   raw: this.buildRuffloSection(),   label: 'Rufflo CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> rufflo-init (PID 1)',
  ...
  private buildRuffloSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack rufflo@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'rufflo', version: this.opts.ruffloVersion,
      package: packageMeta ?? { name: 'rufflo', version: this.opts.ruffloVersion },
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'RUFLO_CMD="${RUFLO_CMD:-rufflo}"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    return {
      magic: 'RVFA', version: 1, name: 'rufflo-appliance', appVersion: this.opts.ruffloVersion,
...
function detectRuffloVersion(): string {
  try {
