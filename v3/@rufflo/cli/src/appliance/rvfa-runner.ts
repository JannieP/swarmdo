...
export class RvfaRunner {
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const rufflo = tryExtract(this.reader, 'rufflo');
      if (!rufflo) return fail('RVFA appliance does not contain a "rufflo" section');

      const entryFile = join(workDir, 'rufflo-bundle.js');
      await writeFile(entryFile, rufflo);
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const rufflo = tryExtract(this.reader, 'rufflo');
      if (!rufflo) return fail('RVFA appliance does not contain a "rufflo" section');
      await writeFile(join(workDir, 'rufflo-bundle.js'), rufflo);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY rufflo-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "rufflo-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
