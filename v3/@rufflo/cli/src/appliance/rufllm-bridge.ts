/**
 * rufLLM Bridge -- Local Language Model Inference from RufVector
 *
 ...
 *   Tier 2: Local model via rufLLM (~200ms) -- routing, classification
 ...
 *
 * @module @rufflo/cli/appliance/rufllm-bridge
 */
...
const DEFAULT_CONFIG: Required<RufllmConfig> = {
...
export interface BridgeStatus {
  available: boolean;
  rufvectorCore: boolean;
  rufvectorRouter: boolean;
  rufvectorSona: boolean;
  modelsLoaded: string[];
  kvCacheSize: number;
}
...
export class RufllmBridge {
  private config: Required<RufllmConfig>;
  private models: Map<string, ModelInfo> = new Map();
  private activeModel: string | null = null;
  private kvCacheEntries = 0;
  private rufvectorCore: any = null;
  private rufvectorRouter: any = null;
  private rufvectorSona: any = null;
  private ggufEngine: GgufEngineType | null = null;

  constructor(config: RufllmConfig) {
    if (!config.modelsDir) throw new Error('RufllmConfig.modelsDir is required');
    this.config = { ...DEFAULT_CONFIG, ...config };
  }

  /** Probe optional @rufvector packages, initialize GGUF engine, and scan modelsDir. */
  async initialize(): Promise<void> {
    this.rufvectorCore = await this.tryImport('@rufvector/core');
    this.rufvectorRouter = await this.tryImport('@rufvector/router');
    this.rufvectorSona = await this.tryImport('@rufvector/sona');
    ...
    if (this.config.verbose) {
      const pkgs = [
        this.rufvectorCore && '@rufvector/core',
        this.rufvectorRouter && '@rufvector/router',
        this.rufvectorSona && '@rufvector/sona',
        this.ggufEngine && 'gguf-engine',
      ].filter(Boolean);
      if (pkgs.length) console.log(`[rufLLM] Loaded: ${pkgs.join(', ')}`);
      console.log(`[rufLLM] ${this.models.size} model(s) in ${this.config.modelsDir}`);
    }
  }
  ...
  async loadModel(name: string): Promise<void> {
    ...
    } else if (this.rufvectorCore?.loadModel) {
      await this.rufvectorCore.loadModel(info.path, { contextSize: this.config.contextSize });
    }
    info.loaded = true;
  ...
  async generate(request: GenerateRequest): Promise<GenerateResponse> {
    ...
    if (info?.loaded) {
      try {
        ...
        } else if (this.rufvectorCore?.generate) {
          const r = await this.rufvectorCore.generate({
        ...
        if (this.config.verbose) console.warn('[rufLLM] Local generation failed, tier 3 fallback:', err);
      }
    }
  ...
  async routeTask(description: string): Promise<TierRouting> {
    if (this.rufvectorRouter?.route) {
      try {
        const r = await this.rufvectorRouter.route(description);
  ...
  async getStatus(): Promise<BridgeStatus> {
    return {
      available: this.models.size > 0 || this.rufvectorCore !== null,
      rufvectorCore: this.rufvectorCore !== null,
      rufvectorRouter: this.rufvectorRouter !== null,
      rufvectorSona: this.rufvectorSona !== null,
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.config.kvCachePath && this.rufvectorCore?.persistKvCache) {
      try { await this.rufvectorCore.persistKvCache(this.config.kvCachePath); }
      catch (e) { if (this.config.verbose) console.warn('[rufLLM] KV-cache persist failed:', e); }
    }
    if (this.rufvectorCore?.unloadAll) await this.rufvectorCore.unloadAll();
...
let instance: RufllmBridge | null = null;

/** Get or create the singleton RufllmBridge. Config required on first call. */
export function getRufllmBridge(config?: RufllmConfig): RufllmBridge {
  if (!instance && config) instance = new RufllmBridge(config);
  if (!instance) throw new Error('rufLLM bridge not initialized. Call with config first.');
  return instance;
}

/** Reset the singleton (useful for tests). */
export function resetRufllmBridge(): void { instance = null; }

/** Check whether @rufvector/core is importable without loading the bridge. */
export async function isRufllmAvailable(): Promise<boolean> {
