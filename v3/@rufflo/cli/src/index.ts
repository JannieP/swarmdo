...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    this.name = options.name || 'rufflo';
    this.description = options.description || 'Rufflo V3 - AI Agent Orchestration Platform';
  ...
  private async loadConfig(configPath?: string): Promise<V3Config | undefined> {
    try {
      ...
      const { loadConfig: loadSystemConfig } = await import('@rufflo/shared');
