...
async function getEmbeddings() {
  try {
    return await import('@rufflo/embeddings');
  } catch {
    return null;
  }
}
...
const generateCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings generate -t "Hello world"', description: 'Generate embedding' },
    { command: 'rufflo embeddings generate -t "Test" -o json', description: 'Output as JSON' },
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings search -q "error handling"', description: 'Search for similar' },
    { command: 'rufflo embeddings search -q "test" -l 5', description: 'Limit results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!fs.existsSync(fullDbPath)) {
        ...
        output.printInfo('Run: rufflo memory init');
        return { success: false, exitCode: 1 };
      }
      ...
      if (topResults.length === 0) {
        ...
        output.printInfo(`Try: rufflo memory store -k "key" --value "your data"`);
...
const compareCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings compare --text1 "Hello" --text2 "Hi there"', description: 'Compare texts' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const collectionsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings collections', description: 'List collections' },
    { command: 'rufflo embeddings collections -a stats', description: 'Show detailed stats' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (collections.length === 0) {
        ...
        output.writeln(output.highlight('  rufflo memory store -k "key" --value "data"'));
        return { success: true, data: [] };
      }
...
const indexCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings index', description: 'Show index status' },
    { command: 'rufflo embeddings index -a build', description: 'Build index from all namespaces' },
    { command: 'rufflo embeddings index -a rebuild -c project', description: 'Rebuild (label as `project`)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (action === 'build' || action === 'rebuild') {
        ...
        if (!collection && newStatus.entryCount === 0) {
          ...
          output.printInfo('  rufflo memory store -k "key" --value "text" --namespace <ns>');
        }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings init', description: 'Initialize with defaults' },
    { command: 'rufflo embeddings init --model all-mpnet-base-v2', description: 'Use higher quality model' },
    { command: 'rufflo embeddings init --no-hyperbolic', description: 'Euclidean only' },
    { command: 'rufflo embeddings init --curvature=-0.5', description: 'Custom curvature (use = for negative)' },
    { command: 'rufflo embeddings init --force', description: 'Overwrite existing config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.rufflo');
      ...
      if (download) {
        ...
        if (embeddings) {
          ...
          output.writeln(output.dim('  (Skipped — @rufflo/embeddings not installed)'));
        }
      }
...
const providersCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo embeddings providers', description: 'List providers' },
  ],
...
const chunkCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings chunk -t "Long text..." -s 256', description: 'Chunk with 256 char limit' },
    { command: 'rufflo embeddings chunk -f doc.txt --strategy paragraph', description: 'Chunk file by paragraph' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const embeddings = await getEmbeddings();
    const text = ctx.flags.text as string || '';
    ...
    if (!embeddings) {
      output.printWarning('@rufflo/embeddings not installed, showing preview');
      output.writeln();
      output.printBox([
        `Strategy: ${strategy}`,
...
const normalizeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings normalize -i "[0.5, 0.3, 0.8]" -t l2', description: 'L2 normalize' },
    { command: 'rufflo embeddings normalize --check -i "[...]"', description: 'Check if normalized' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const hyperbolicCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings hyperbolic -a convert -i "[0.5, 0.3]"', description: 'Convert to Poincaré' },
    { command: 'rufflo embeddings hyperbolic -a distance', description: 'Compute hyperbolic distance' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const hyperbolic = await import('@rufflo/embeddings').then(m => m).catch(() => null);

      if (!hyperbolic || !hyperbolic.euclideanToPoincare) {
        output.printWarning('@rufflo/embeddings hyperbolic module not available');
        output.printInfo('Install with: npm install @rufflo/embeddings');
        return { success: false, exitCode: 1 };
      }
...
const neuralCommand: Command = {
  ...
  examples: [
    ...
    { command: 'rufflo embeddings neural --drift-threshold=0.2', description: 'Custom drift threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.rufflo', 'embeddings.json');
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .rufflo/embeddings.json'));
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings models', description: 'List models' },
    { command: 'rufflo embeddings models -d all-MiniLM-L6-v2', description: 'Download model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (download) {
      ...
      if (embeddings) {
        ...
        await new Promise(r => setTimeout(r, 500));
        spinner.succeed(`Download skipped — @rufflo/embeddings not installed`);
      }
      return { success: true };
    }
...
const cacheCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings cache', description: 'Show cache stats' },
    { command: 'rufflo embeddings cache -a clear', description: 'Clear cache' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const warmupCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings warmup', description: 'Preload model with test' },
    { command: 'rufflo embeddings warmup -b', description: 'Background warmup' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings benchmark', description: 'Quick benchmark' },
    { command: 'rufflo embeddings benchmark -n 50 -f', description: 'Full benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const embeddingsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo embeddings init', description: 'Initialize ONNX embedding system' },
    { command: 'rufflo embeddings init --model all-mpnet-base-v2', description: 'Init with larger model' },
    { command: 'rufflo embeddings generate -t "Hello"', description: 'Generate embedding' },
    { command: 'rufflo embeddings search -q "error handling"', description: 'Semantic search' },
    { command: 'rufflo embeddings chunk -t "Long doc..."', description: 'Chunk document' },
    { command: 'rufflo embeddings hyperbolic -a convert', description: 'Hyperbolic space' },
    { command: 'rufflo embed neural -f drift', description: 'Neural substrate' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Embeddings'));
