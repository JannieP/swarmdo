/**
 * V3 CLI RufVector Migrate Command
 * Database migration management for RufVector
 */
...
export const migrateCommand: Command = {
  ...
  examples: [
    ...
    { command: 'rufflo rufvector migrate --down', description: 'Rollback last migration' },
    { command: 'rufflo rufvector migrate --to 1.2.0', description: 'Migrate to version 1.2.0' },
    { command: 'rufflo rufvector migrate --dry-run', description: 'Preview migration SQL' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('RufVector Migration'));
    ...
    try {
      ...
      if (schemaExists.rows.length === 0) {
        ...
        output.printError('Run `rufflo rufvector init` first');
        await client.end();
        return { success: false, exitCode: 1 };
      }
