/**
 * V3 CLI RufVector Setup Command
 * Outputs Docker files and SQL for easy RufVector PostgreSQL setup
 *
 * Usage:
 *   npx rufflo rufvector setup              # Output to ./rufvector-postgres/
 *   npx rufflo rufvector setup --output /path/to/dir
 *   npx rufflo rufvector setup --print      # Print to stdout only
 *
...
/**
 * Docker Compose template for RufVector PostgreSQL
 */
const DOCKER_COMPOSE_TEMPLATE = `# RufVector PostgreSQL Testing Environment
# Official RufVector extension from ruvnet/rufvector-postgres
#
# Features:
...
services:
  postgres:
    image: ruvnet/rufvector-postgres:latest
  ...
  pgadmin:
    ...
    container_name: rufvector-pgadmin
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@rufflo.local
...
/**
 * Init SQL template for RufVector PostgreSQL
 */
const INIT_SQL_TEMPLATE = `-- ============================================
-- RUFVECTOR POSTGRESQL INITIALIZATION SCRIPT
-- ============================================
--
-- This script initializes RufVector PostgreSQL extension
-- from ruvnet/rufvector-postgres with Rufflo V3 integration.
--
-- RufVector provides 77+ SQL functions including:
...
-- IMPORTANT: RufVector requires explicit VERSION
-- The control file says 2.0.0 but only 0.1.0 SQL exists
CREATE EXTENSION IF NOT EXISTS rufvector VERSION '0.1.0';
...
CREATE TABLE IF NOT EXISTS claude_flow.agents (
    ...
    state JSONB DEFAULT '{}',
    memory_embedding rufvector(384),
...
RETURNS real[] AS $$
BEGIN
    RETURN rufvector_exp_map(ARRAY_FILL(0.0::real, ARRAY[array_length(euclidean, 1)]), euclidean, curvature);
END;
...
RETURNS FLOAT AS $$
BEGIN
    RETURN rufvector_poincare_distance(x, y, curvature);
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
BEGIN
    RETURN QUERY
    SELECT rufvector_version(), rufvector_simd_info();
END;
$$ LANGUAGE plpgsql STABLE;
...
DECLARE
    ...
    v_simd TEXT;
BEGIN
    SELECT rufvector_version() INTO v_version;
    SELECT rufvector_simd_info() INTO v_simd;

    RAISE NOTICE '';
    RAISE NOTICE '============================================';
    RAISE NOTICE 'RufVector PostgreSQL Initialization Complete!';
    RAISE NOTICE '============================================';
    RAISE NOTICE '';
 ...
 * README template
 */
const README_TEMPLATE = `# RufVector PostgreSQL Setup

This directory contains the Docker configuration for RufVector PostgreSQL with Rufflo V3.
...
### Vector Type
\`\`\`sql
-- Use rufvector(384), NOT vector(384)
...
\`\`\`bash
# Export current Rufflo memory
npx rufflo memory list --format json > memory-export.json

# Import to RufVector PostgreSQL
npx rufflo rufvector import --input memory-export.json
\`\`\`
...
Use explicit version: \`CREATE EXTENSION rufvector VERSION '0.1.0';\`
...
## Learn More
- [RufVector Docker Hub](https://hub.docker.com/r/ruvnet/rufvector-postgres)
- [Rufflo Documentation](https://github.com/ruvnet/claude-flow)
`;

/**
 * RufVector Setup command - outputs Docker files and SQL
 */
export const setupCommand: Command = {
  name: 'setup',
  description: 'Output Docker files and SQL for RufVector PostgreSQL setup',
  aliases: ['scaffold', 'docker'],
  options: [
    {
      name: 'output',
      short: 'o',
      description: 'Output directory (default: ./rufvector-postgres)',
      type: 'string',
      default: './rufvector-postgres',
    },
    {
      name: 'print',
      short: 'p',
  ...
  examples: [
    { command: 'rufflo rufvector setup', description: 'Output files to ./rufvector-postgres/' },
    { command: 'rufflo rufvector setup --output /path/to/dir', description: 'Output to custom directory' },
    { command: 'rufflo rufvector setup --print', description: 'Print files to stdout' },
    { command: 'rufflo rufvector setup --force', description: 'Overwrite existing files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const outputDir = (ctx.flags.output as string) || './rufvector-postgres';
    const printOnly = ctx.flags.print as boolean;
    const force = ctx.flags.force as boolean;

    output.writeln();
    output.writeln(output.bold('RufVector PostgreSQL Setup'));
    ...
    try {
      ...
      output.writeln();
      output.printSuccess('RufVector PostgreSQL setup files created!');
      output.writeln();
