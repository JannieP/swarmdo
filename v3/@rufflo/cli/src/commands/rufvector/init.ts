/**
 * V3 CLI RufVector Init Command
 * Initialize RufVector PostgreSQL Bridge
 */
...
/**
 * Initialize RufVector in PostgreSQL
 */
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize RufVector in PostgreSQL',
  options: [
    {
      name: 'host',
      short: 'h',
    ...
    },
    {
      name: 'schema',
      short: 's',
      description: 'Schema name for RufVector tables',
      type: 'string',
      default: 'claude_flow',
    },
    {
  ...
  examples: [
    { command: 'rufflo rufvector init -d mydb', description: 'Initialize with database name' },
    { command: 'rufflo rufvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'rufflo rufvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'rufflo rufvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    let config = getConnectionConfig(ctx);

    output.writeln();
    output.writeln(output.bold('RufVector PostgreSQL Initialization'));
    ...
    try {
      ...
      // Detect vector extension: prefer rufvector, fall back to pgvector
      ...
      if (rufvectorResult.rows.length > 0) {
        ...
        if (pgvectorResult.rows.length > 0) {
          ...
          // Neither installed -- try to create rufvector first, then pgvector
          ...
          try {
            ...
            // rufvector not available, try pgvector
          }
          if (!created) {
            try {
              ...
              output.printError('Please install rufvector or pgvector manually.');
              output.printError('  rufvector: https://hub.docker.com/r/ruvnet/rufvector-postgres');
      ...
      const cosineOps = vectorExtName === 'rufvector' ? 'rufvector_cosine_ops' : 'vector_cosine_ops';
      ...
      // Create tables
      spinner.setText('Creating RufVector tables...'); spinner.start();
      ...
      await client.query(`
        ...
        VALUES ('rufvector_version', '"1.0.0"')
      ...
      spinner.succeed('RufVector tables created');
      ...
      await client.query(`
        ...
        VALUES ('1.0.0', 'Initial RufVector setup')
      ...
      output.writeln();
      output.printSuccess('RufVector PostgreSQL Bridge initialized successfully!');
      output.writeln();
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: rufflo rufvector migrate --up',
        '  2. Check status: rufflo rufvector status --verbose',
        '  3. Run benchmark: rufflo rufvector benchmark',
