/**
 * V3 CLI RufVector Import Command
 * Import data from sql.js/JSON memory to RufVector PostgreSQL
 *
 * Usage:
 *   npx rufflo rufvector import --input memory-export.json
 *   npx rufflo rufvector import --from-memory
 *   npx rufflo rufvector import --input data.json --batch-size 100
 *
...
/**
 * Format a rufvector embedding array for PostgreSQL
...
/**
 * RufVector Import command - import from sql.js/JSON to PostgreSQL
 */
export const importCommand: Command = {
  name: 'import',
  description: 'Import data from sql.js/JSON memory to RufVector PostgreSQL',
  ...
  options: [
    ...
    {
      name: 'from-memory',
      description: 'Export from current Rufflo memory and import',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'rufflo rufvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'rufflo rufvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'rufflo rufvector import --from-memory', description: 'Export current memory and import' },
    { command: 'rufflo rufvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const containerName = (ctx.flags.container as string) || 'rufvector-postgres';
    const verbose = ctx.flags.verbose as boolean;

    output.writeln();
    output.writeln(output.bold('RufVector PostgreSQL Import'));
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  rufflo rufvector import --input memory-export.json');
      output.writeln('  rufflo rufvector import --from-memory');
    ...
    if (fromMemory) {
      output.printInfo('Exporting from current Rufflo memory...');
      output.printWarning('Note: Run "npx rufflo memory list --format json > memory-export.json" first');
      output.printInfo('Then use: npx rufflo rufvector import --input memory-export.json');
    ...
    sqlStatements.push('-- RufVector PostgreSQL Import');
    ...
    if (outputFile) {
      ...
      const tempFile = path.join(process.cwd(), '.rufvector-import-temp.sql');
      try {
