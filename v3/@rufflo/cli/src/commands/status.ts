 ...
 * System status display for Rufflo
 */
...
//
// #2120 — the old check required `.rufflo/config.yaml`, which
// missed projects that were initialized via `rufflo memory init` (writes
...
// and a running MCP, yet `rufflo status` reported "not initialized".
//
// Now: any of these signals counts as initialized:
//   - `.rufflo/config.yaml`   (the canonical `rufflo init` output)
//   - `.rufflo/config.json`   (same, alt format)
//   - `.swarm/memory.db`           (the `rufflo memory init` output)
//   - `.claude/settings.json`      (the Claude Code hook surface)
function isInitialized(cwd: string): boolean {
  const candidates = [
    path.join(cwd, '.rufflo', 'config.yaml'),
    path.join(cwd, '.rufflo', 'config.json'),
...
function displayStatus(status: Awaited<ReturnType<typeof getSystemStatus>>): void {
  ...
  output.writeln(`${output.bold('Rufflo V3')} ${statusIcon}`);
  output.writeln();
...
export const statusCommand: Command = {
  ...
  examples: [
    { command: 'rufflo status', description: 'Show current system status' },
    { command: 'rufflo status --watch', description: 'Watch mode with live updates' },
    { command: 'rufflo status --watch -i 5', description: 'Watch mode updating every 5 seconds' },
    { command: 'rufflo status --health-check', description: 'Run health checks' },
    { command: 'rufflo status --json', description: 'Output status as JSON' },
    { command: 'rufflo status agents', description: 'Show detailed agent status' },
    { command: 'rufflo status tasks', description: 'Show detailed task status' },
    { command: 'rufflo status memory', description: 'Show detailed memory status' }
  ],
  action: statusAction
};
