...
const startCommand: Command = {
  ...
  examples: [
    { command: 'rufflo daemon start', description: 'Start daemon in background (default)' },
    { command: 'rufflo daemon start --foreground', description: 'Start in foreground (blocks terminal)' },
    { command: 'rufflo daemon start -w map,audit,optimize', description: 'Start with specific workers' },
    { command: 'rufflo daemon start --headless --sandbox strict', description: 'Start with headless workers in strict sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!isDaemonProcess) {
      const stateDir = join(resolve(projectRoot), '.rufflo');
    ...
    try {
      const stateDir = join(projectRoot, '.rufflo');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.rufflo') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean, forwarded: ForwardedDaemonFlags = {}): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.rufflo');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  const forkOpts: Record<string, unknown> = {
    ...
    // daemon writes its own logs via appendFileSync to .rufflo/logs/.
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: rufflo daemon stop`);
  }
...
const stopCommand: Command = {
  ...
  examples: [
    { command: 'rufflo daemon stop', description: 'Stop the daemon' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.rufflo', 'daemon.pid');
...
async function killStaleDaemonsPosix(projectRoot: string, quiet: boolean): Promise<void> {
  try {
    ...
    for (const line of lines) {
      ...
      if (!line.includes('rufflo') && !line.includes('@rufflo/cli')) continue;
...
/**
 * #2356: enumerate every running rufflo daemon across ALL workspaces. Reuses
...
async function renderAllDaemonsStatus(): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    output.printBox(
      'No rufflo daemons are running in any workspace.',
      'Rufflo Daemons (all workspaces)'
    );
  ...
  const rows = daemons.map(d => {
    ...
    if (d.workspace) {
      try {
        const statePath = join(d.workspace, '.rufflo', 'daemon-state.json');
  ...
  if (staleCount > 0) {
    output.printWarning(
      `${staleCount} daemon(s) have outlived their TTL (or have run >12h). Stop one with: cd <workspace> && rufflo daemon stop`
    );
  } else {
...
const statusCommand: Command = {
  ...
  options: [
    ...
    // leaked in another project is invisible. --all scans every running rufflo
    // daemon across all workspaces (the global view that surfaces leaks).
    { name: 'all', short: 'a', type: 'boolean', description: 'List rufflo daemons across ALL workspaces (global view — surfaces leaked daemons)' },
  ],
  examples: [
    { command: 'rufflo daemon status', description: 'Show daemon status' },
    { command: 'rufflo daemon status -v', description: 'Show detailed status' },
    { command: 'rufflo daemon status --show-modes', description: 'Show worker execution modes' },
    { command: 'rufflo daemon status --all', description: 'List daemons across all workspaces' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox(
        [
          ...
          '',
          'Run "rufflo daemon start" to start the daemon',
        ].join('\n'),
        'Rufflo Daemon'
      );
...
const triggerCommand: Command = {
  ...
  examples: [
    { command: 'rufflo daemon trigger -w map', description: 'Trigger the map worker' },
    { command: 'rufflo daemon trigger -w audit', description: 'Trigger security audit' },
    { command: 'rufflo daemon trigger -w audit --headless', description: 'Trigger audit in headless sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const enableCommand: Command = {
  ...
  examples: [
    { command: 'rufflo daemon enable -w predict', description: 'Enable predict worker' },
    { command: 'rufflo daemon enable -w document --disable', description: 'Disable document worker' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerType = ctx.flags.worker as WorkerType;
...
const installSupervisorCommand: Command = {
  ...
  examples: [
    { command: 'rufflo daemon install-supervisor', description: 'Install + load (auto-restart enabled)' },
    { command: 'rufflo daemon install-supervisor --no-load', description: 'Write unit file but do not enable yet' },
    { command: 'rufflo daemon install-supervisor --dry-run', description: 'Preview the unit file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      ...
      const plistPath = join(plistDir, 'io.ruv.rufflo.daemon.plist');
      const logDir = join(projectRoot, '.rufflo', 'logs');
      const plist = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key><string>io.ruv.rufflo.daemon</string>
...
`;
    ...
    const unitPath = join(unitDir, 'rufflo-daemon.service');
    const unit = `[Unit]
Description=Rufflo background worker daemon
...
`;
    ...
    if (load) {
      try {
        ...
        execFileSync('systemctl', ['--user', 'enable', '--now', 'rufflo-daemon.service'], { encoding: 'utf-8', timeout: 10000 });
        ...
        output.printInfo(`Run manually: systemctl --user daemon-reload && systemctl --user enable --now rufflo-daemon.service`);
      }
    } else {
      output.printInfo(`Run when ready:  systemctl --user daemon-reload && systemctl --user enable --now rufflo-daemon.service`);
    }
    return { success: true };
  },
};
...
const uninstallSupervisorCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      const plistPath = join(home, 'Library', 'LaunchAgents', 'io.ruv.rufflo.daemon.plist');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('launchctl', ['unload', plistPath], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
    ...
    if (platform === 'linux') {
      const unitPath = join(home, '.config', 'systemd', 'user', 'rufflo-daemon.service');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('systemctl', ['--user', 'disable', '--now', 'rufflo-daemon.service'], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
...
export const daemonCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo daemon start', description: 'Start the daemon' },
    { command: 'rufflo daemon start --headless', description: 'Start with headless workers (E2B sandbox)' },
    { command: 'rufflo daemon status', description: 'Check daemon status' },
    { command: 'rufflo daemon stop', description: 'Stop the daemon' },
    { command: 'rufflo daemon trigger -w audit', description: 'Run security audit' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Daemon - Background Task Management'));
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "rufflo daemon <subcommand> --help" for details');
