  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "rufflo session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: rufflo session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "rufflo start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo session list', description: 'List all sessions' },
    { command: 'rufflo session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'rufflo session restore session-123', description: 'Restore a session' },
    { command: 'rufflo session delete session-123', description: 'Delete a session' },
    { command: 'rufflo session export -o backup.json', description: 'Export session to file' },
    { command: 'rufflo session import backup.json', description: 'Import session from file' },
    { command: 'rufflo session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: rufflo session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "rufflo session <subcommand> --help" for subcommand help');
