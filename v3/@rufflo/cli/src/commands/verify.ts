 ...
 *
 * Run via: rufflo verify [--branch <branch>] [--manifest <local-path>]
 *
...
async function verifySignature(witness: Witness): Promise<{
  ...
  const seed = createHash('sha256').update(witness.manifest.gitCommit + ':rufflo-witness/v1').digest();
...
export const verifyCommand: Command = {
  ...
  examples: [
    { command: 'rufflo verify', description: 'Fetch latest manifest from main branch + verify' },
    { command: 'rufflo verify --branch main', description: 'Verify against a specific branch' },
    { command: 'rufflo verify --manifest ./verification.md.json', description: 'Use a local manifest copy' },
    { command: 'rufflo verify --json', description: 'Machine-readable output for CI' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!asJson) {
      output.writeln();
      output.writeln(output.bold('Rufflo Verification'));
