...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  // pre-v3 init flows; v3 init writes only `.rufflo/config.yaml`.
  const jsonPaths = [
    '.rufflo/config.json',
    'rufflo.config.json',
    '.rufflo.json'
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'rufflo config init' };
}

// Check daemon status
/**
 * #2448 — Detect the runaway `npx @rufflo/cli@latest` statusLine / hook
 ...
 * and never re-ran `rufflo init` still have it.
 *
 * Detection only — does not modify settings. Fix path is `rufflo init` (the
...
async function checkStaleSettingsNpx(): Promise<HealthCheck> {
  ...
  const BROKEN_RE = /npx\s+(?:--?\S+\s+)*@?rufflo\/cli@latest\s+hooks\s+(?:statusline|\S+)/;
  ...
  return {
    ...
    status: 'fail',
    message: `CRITICAL — runaway \`npx @rufflo/cli@latest\` commands detected: ${summary}`,
    fix: 'Re-run `npx rufflo init` to migrate (the v3.13.3+ init migrator regenerates these to local-helper form). On macOS this prevents the process-storm / kernel-panic class reported in #2448.',
  };
}

async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.rufflo/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .rufflo/daemon.pid && rufflo daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'rufflo daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'rufflo daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'rufflo memory configure --backend hybrid' };
}
...
// `@rufflo/aidefence` to be installed and loadable. The package is an
// optional dependency — present in some installs (project-local) but
// missing in others (npm-global of `rufflo`). Without it, every
...
async function checkAIDefence(): Promise<HealthCheck> {
  try {
    ...
    await import('@rufflo/aidefence');
    return {
      name: 'AIDefence',
      status: 'pass',
      message: '@rufflo/aidefence loadable — aidefence_* MCP tools functional',
    };
  } catch {
    return {
      name: 'AIDefence',
      status: 'warn',
      message: '@rufflo/aidefence not loadable — aidefence_* MCP tools will fail (optional package)',
      fix: 'npm install --save @rufflo/aidefence  (in your project), or run `rufflo mcp start` from a directory that has it installed',
    };
  }
}
...
async function checkFederationBreaker(): Promise<HealthCheck> {
  try {
    // Optional plugin — not a hard dep of @rufflo/cli. Build the
    ...
    const specifier = ['@rufflo', 'plugin-agent-federation'].join('/');
    ...
    if (!mod.FederationNodeState) {
      return {
        ...
        status: 'warn',
        message:
          '@rufflo/plugin-agent-federation loaded but FederationNodeState export missing — version older than ADR-097 Phase 2',
        fix: 'Upgrade: npm install @rufflo/plugin-agent-federation@alpha',
      };
    }
    return {
      name: 'Federation Breaker',
    ...
    return {
      ...
      fix: 'npm install --save @rufflo/plugin-agent-federation@alpha',
    };
  }
}
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  // parsed.projects[<projectPath>].mcpServers.rufflo, in addition to any
  ...
  const isRuffloKey = (k: string) =>
    k === 'rufflo' || k === 'rufflo_alpha' || k === 'rufflo' || k === 'rufflo_alpha';
  ...
  for (const configPath of mcpConfigPaths) {
    ...
    try {
      ...
      const topHasRufflo = topServerKeys.some(isRuffloKey);

      // Project-scoped (Claude Code shape): projects[*].mcpServers.rufflo
      let projectHits = 0;
      ...
      if (content.projects && typeof content.projects === 'object') {
        for (const projectVal of Object.values(content.projects)) {
          ...
          if (pm && typeof pm === 'object') {
            ...
            if (keys.some(isRuffloKey)) projectHits += 1;
          }
        }
      }

      const totalServers = topServerKeys.length + projectScannedServers;
      if (topHasRufflo || projectHits > 0) {
        const where = topHasRufflo
          ? 'top-level'
          : `${projectHits} project-scoped`;
        return {
          name: 'MCP Servers',
          status: 'pass',
          message: `${totalServers} servers (rufflo configured: ${where})`,
        };
      }
      if (totalServers > 0) {
        return {
          name: 'MCP Servers',
          status: 'warn',
          message: `${totalServers} servers (rufflo not found)`,
          fix: 'claude mcp add rufflo -- npx -y rufflo@latest mcp start',
        };
      }
    } catch {
...
async function checkVersionFreshness(): Promise<HealthCheck> {
  try {
    ...
    try {
      ...
      // package.json that belongs to @rufflo/cli (or rufflo/cli).
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@rufflo/cli' || pkg.name === 'rufflo' || pkg.name === 'rufflo')
            ) {
    ...
    try {
      const npmInfo = await runCommand('npm view @rufflo/cli@alpha version', 5000);
    ...
    if (isOutdated) {
      const fix = isNpx
        ? 'rm -rf ~/.npm/_npx/* && npx -y @rufflo/cli@latest'
        : 'npm update @rufflo/cli';
...
// the standard rufflo doctor flow. Graceful degradation: when metaharness
// is not installed (no network, optionalDep skipped), the check returns
// `warn` with a hint instead of `fail` — rufflo continues to function.
/**
 * iter 45 — verify the rufflo-side MetaHarness integration is intact.
 *
 ...
 * layer is shipped with rufflo — missing files mean rufflo's install is
 ...
 *   - plugins/rufflo-metaharness/scripts/_harness.mjs                (subprocess bridge)
 *   - plugins/rufflo-metaharness/scripts/_similarity.mjs             (ADR-152 §3.1 module, iter 36)
 *   - plugins/rufflo-metaharness/scripts/similarity.mjs              (CLI skill, iter 36)
 *   - plugins/rufflo-metaharness/scripts/_spike-similarity.mjs       (regression anchor, iter 35)
 *   - plugins/rufflo-metaharness/scripts/drift-from-history.mjs      (1-command primitive, iter 53)
 *   - plugins/rufflo-metaharness/skills/harness-similarity/SKILL.md
 *   - plugins/rufflo-metaharness/skills/harness-drift-from-history/SKILL.md  (iter 53)
 */
async function checkMetaharnessIntegration(): Promise<HealthCheck> {
  ...
  // hard-coded `<cwd>/node_modules/@rufflo/cli/...` candidate. That
  // missed the two cases users actually run from:
  //   (a) `npx @rufflo/cli@<tag>` → resolves to a per-version cache
  //       under `~/.npm/_npx/<hash>/node_modules/@rufflo/cli/...`
  //   (b) `npm install -g @rufflo/cli` → lives at
  //       `$(npm prefix -g)/lib/node_modules/@rufflo/cli/...`
  //
  ...
  // (dev) until we find a directory containing `plugins/rufflo-metaharness/`.
  const candidates: string[] = [];
  ...
  try {
    ...
    let q = selfDir;
    for (let i = 0; i < 8; i++) {
      candidates.push(join(q, 'plugins', 'rufflo-metaharness'));
      q = dirname(q);
    }
  } catch {
  ...
  let p = process.cwd();
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'rufflo-metaharness'));
    p = dirname(p);
  }

  // Strategy 3: explicit node_modules path relative to cwd — covers project-local install.
  candidates.push(join(process.cwd(), 'node_modules', '@rufflo', 'cli', 'plugins', 'rufflo-metaharness'));
  ...
  if (!pluginDir) {
    ...
    return {
      ...
      status: 'warn',
      message: 'plugins/rufflo-metaharness/ not found — MetaHarness skills will degrade gracefully',
      fix: 'Optional: install via `npm i -D @metaharness/darwin metaharness` or run `rufflo plugins install rufflo-metaharness`',
    };
  }
  ...
  if (missing.length > 0) {
    return {
      ...
      fix: 'Reinstall rufflo or restore from git: `git checkout HEAD -- plugins/rufflo-metaharness/`',
    };
  }
  ...
  try {
    ...
    if (typeof mod.similarity !== 'function') {
      return {
        ...
        fix: 'Reinstall rufflo or restore the file from git',
      };
    }
    ...
    if (typeof harnessMod.parseMcpScanText !== 'function') {
      return {
        ...
        fix: 'Reinstall rufflo or restore _harness.mjs from git',
      };
    }
...
async function checkMetaharness(): Promise<HealthCheck> {
  try {
    ...
    return {
      name: 'MetaHarness (ADR-150)',
      status: 'pass',
      message: `v${versionMatch[1]} — run \`npx rufflo metaharness score\` for the full scorecard`,
    };
  } catch {
    return {
      name: 'MetaHarness (ADR-150)',
      status: 'warn',
      message: 'Not installed — `npx rufflo metaharness *` commands will degrade gracefully',
...
async function checkEncryptionAtRest(): Promise<HealthCheck> {
  ...
  const stores: Array<{ label: string; path: string }> = [
    { label: 'sessions/', path: join(cwd, '.rufflo', 'sessions') },
    { label: 'terminals', path: join(cwd, '.rufflo', 'terminals', 'store.json') },
...
export const doctorCommand: Command = {
  ...
  examples: [
    { command: 'rufflo doctor', description: 'Run full health check' },
    { command: 'rufflo doctor --fix', description: 'Print suggested fix commands (does not auto-apply)' },
    { command: 'rufflo doctor --install', description: 'Auto-install missing dependencies' },
    { command: 'rufflo doctor -c version', description: 'Check for stale npx cache' },
    { command: 'rufflo doctor -c claude', description: 'Check Claude Code CLI only' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Rufflo Doctor'));
