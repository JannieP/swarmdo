...
export const ruvectorCommand: Command = {
  ...
  examples: [
    { command: 'rufflo ruvector setup', description: 'Output Docker files and SQL for setup' },
    { command: 'rufflo ruvector import --input memory.json', description: 'Import from sql.js/JSON export' },
    { command: 'rufflo ruvector init --database mydb', description: 'Initialize RuVector in PostgreSQL' },
    { command: 'rufflo ruvector status --verbose', description: 'Check connection and schema status' },
    { command: 'rufflo ruvector migrate --up', description: 'Run pending migrations' },
    { command: 'rufflo ruvector benchmark --vectors 10000', description: 'Run performance benchmark' },
    { command: 'rufflo ruvector optimize --analyze', description: 'Analyze and suggest optimizations' },
    { command: 'rufflo ruvector backup --output backup.sql', description: 'Backup RuVector data' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printBox([
      'RuVector provides PostgreSQL integration for Rufflo with:',
      '',
    ...
    output.writeln();
    output.printInfo('Run `rufflo ruvector <command> --help` for details');
    output.writeln();
