 ...
 *   npx rufflo ruvector import --from-memory
 *   npx rufflo ruvector import --input data.json --batch-size 100
 *
...
export const importCommand: Command = {
  ...
  examples: [
    { command: 'rufflo ruvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'rufflo ruvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'rufflo ruvector import --from-memory', description: 'Export current memory and import' },
    { command: 'rufflo ruvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  rufflo ruvector import --input memory-export.json');
      output.writeln('  rufflo ruvector import --from-memory');
    ...
    if (fromMemory) {
      ...
      output.printInfo('Then use: npx rufflo ruvector import --input memory-export.json');
