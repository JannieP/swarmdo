/**
 * V3 CLI Init Command
 * Comprehensive initialization for Rufflo with Claude Code integration
 */
...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Rufflo V3 for OpenAI Codex'));
  output.writeln();
  ...
  try {
    ...
    // Try multiple resolution strategies for the @rufflo/codex package
    // Use a variable to prevent TypeScript from statically resolving the optional module
    const codexModuleId = '@rufflo/codex';
    const resolutionStrategies = [
      ...
      async () => {
        const projectPath = path.join(ctx.cwd, 'node_modules', '@rufflo', 'codex', 'dist', 'index.js');
      ...
      async () => {
        ...
        const codexPath = path.join(globalPath, '@rufflo', 'codex', 'dist', 'index.js');
    ...
    if (!CodexInitializer) {
      throw new Error('Cannot find module @rufflo/codex');
    }
    ...
    if (errorMessage.includes('Cannot find module') || errorMessage.includes('@rufflo/codex')) {
      output.printError('The @rufflo/codex package is not installed.');
      output.printInfo('Install it with: npm install @rufflo/codex');
      output.writeln();
...
// Check if project is already initialized with rufflo.
// #2207: .claude/settings.json alone is NOT a rufflo marker — it's created by
// Claude Code itself and exists in every Claude Code project. We require a
// rufflo-specific signal: either a claudeFlow section in settings.json, OR a
// .mcp.json with a 'rufflo' or 'rufflo' server key, OR the rufflo-only
// .rufflo/config.yaml. Using the bare file-existence check was causing
...
function isInitialized(cwd: string): { claude: boolean; claudeFlow: boolean } {
  const claudeFlowPath = path.join(cwd, '.rufflo', 'config.yaml');
  const mcpJsonPath = path.join(cwd, '.mcp.json');
  const settingsPath = path.join(cwd, '.claude', 'settings.json');

  // Check .rufflo/config.yaml — rufflo-specific, always reliable
  const hasClaudeFlow = fs.existsSync(claudeFlowPath);

  // Check .claude/settings.json for rufflo-specific content (claudeFlow section)
  let hasRuffloSettings = false;
  ...
  // Check .mcp.json for rufflo/rufflo server key
  let hasRuffloMcp = false;
  if (fs.existsSync(mcpJsonPath)) {
    try {
      ...
      hasRuffloMcp =
        ...
        ('rufflo' in (parsed.mcpServers as Record<string, unknown>) ||
         'rufflo' in (parsed.mcpServers as Record<string, unknown>));
    } catch { /* malformed — ignore */ }
  }

  return {
    claude: hasRuffloSettings || hasRuffloMcp,
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('Rufflo appears to be already initialized');
    if (initialized.claude) output.printInfo('  Found: .claude/settings.json');
    if (initialized.claudeFlow) output.printInfo('  Found: .rufflo/config.yaml');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Rufflo V3'));
  output.writeln();
  ...
  // #1744 — opt-out of the user-global ~/.claude/CLAUDE.md "Rufflo Integration"
  ...
  try {
    ...
    spinner.succeed('Rufflo V3 initialized successfully!');
    output.writeln();
    ...
    if (options.components.runtime) {
      output.printBox(
        [
          `Config:      .rufflo/config.yaml`,
          `Data:        .rufflo/data/`,
          `Logs:        .rufflo/logs/`,
          `Sessions:    .rufflo/sessions/`,
        ].join('\n'),
        'V3 Runtime'
      );
      output.writeln();
    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @rufflo/cli@latest memory init 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync('npx @rufflo/cli@latest daemon start 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @rufflo/cli@latest swarm init --topology hierarchical 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execFileInit('npx', [
          '@rufflo/cli@latest', 'embeddings', 'init',
    ...
    if (!startDaemon && !startAll) {
      const bin = (process.argv[1] || '').includes('rufflo') ? 'rufflo' : 'rufflo';
      output.writeln(output.bold('Next steps:'));
      output.printList([
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo V3 Setup Wizard'));
      ...
      if (preset === 'minimal') {
        ...
        const components = await multiSelect({
          ...
          options: [
            ...
            { value: 'runtime', label: 'Runtime', hint: '.rufflo/ directory for V3 runtime', selected: true },
          ],
        });
        ...
        if (options.components.settings) {
          const hooks = await multiSelect({
            ...
            options: [
              ...
              { value: 'permissionRequest', label: 'PermissionRequest', hint: 'Auto-allow rufflo tools', selected: true },
            ],
          });
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if Rufflo is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const result = {
      ...
      paths: {
        ...
        claudeFlowConfig: initialized.claudeFlow ? path.join(ctx.cwd, '.rufflo', 'config.yaml') : null,
      },
    };
    ...
    if (result.initialized) {
      output.printSuccess('Rufflo is initialized');
      if (initialized.claude) {
        output.printInfo(`  Claude Code: .claude/settings.json`);
      }
      if (initialized.claudeFlow) {
        output.printInfo(`  V3 Runtime: .rufflo/config.yaml`);
      }
    } else {
      output.printWarning('Rufflo is not initialized in this directory');
      output.printInfo('Run "rufflo init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading Rufflo'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Rufflo in the current directory',
  ...
  options: [
    ...
    {
      name: 'no-global',
      description: 'Skip the ~/.claude/CLAUDE.md "Rufflo Integration" pointer block (#1744)',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'rufflo init', description: 'Initialize with default configuration' },
    { command: 'rufflo init --start-all', description: 'Initialize and start daemon, memory, swarm' },
    { command: 'rufflo init --start-daemon', description: 'Initialize and start daemon only' },
    { command: 'rufflo init --minimal', description: 'Initialize with minimal configuration' },
    { command: 'rufflo init --full', description: 'Initialize with all components' },
    { command: 'rufflo init --force', description: 'Reinitialize and overwrite existing config' },
    { command: 'rufflo init --only-claude', description: 'Only create Claude Code integration' },
    { command: 'rufflo init --skip-claude', description: 'Only create V3 runtime' },
    { command: 'rufflo init wizard', description: 'Interactive setup wizard' },
    { command: 'rufflo init --with-embeddings', description: 'Initialize with ONNX embeddings' },
    { command: 'rufflo init --with-embeddings --embedding-model Xenova/all-mpnet-base-v2', description: 'Use larger embedding model' },
    { command: 'rufflo init skills --all', description: 'Install all available skills' },
    { command: 'rufflo init hooks --minimal', description: 'Create minimal hooks configuration' },
    { command: 'rufflo init upgrade', description: 'Update helpers while preserving data' },
    { command: 'rufflo init upgrade --settings', description: 'Update helpers and merge new settings (Agent Teams)' },
    { command: 'rufflo init upgrade --verbose', description: 'Show detailed upgrade info' },
    { command: 'rufflo init --codex', description: 'Initialize for OpenAI Codex (AGENTS.md)' },
    { command: 'rufflo init --codex --full', description: 'Codex init with all 137+ skills' },
    { command: 'rufflo init --dual', description: 'Initialize for both Claude Code and Codex' },
    { command: 'rufflo init --all-agents', description: 'Install all agent categories (~89 agents; ADR-128 opt-in)' },
  ],
  action: initAction,
};
