...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.rufflo/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.rufflo/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  examples: [
    { command: 'rufflo process daemon --action start', description: 'Start the daemon' },
    { command: 'rufflo process daemon --action stop', description: 'Stop the daemon' },
    { command: 'rufflo process daemon --action restart --port 3850', description: 'Restart on different port' },
    { command: 'rufflo process daemon --action status', description: 'Check daemon status' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags?.action as string) || 'status';
    const port = (ctx.flags?.port as number) || 3847;
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.rufflo/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.rufflo/daemon.log';
    ...
    switch (action) {
      ...
      case 'status':
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: rufflo process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  examples: [
    { command: 'rufflo process monitor', description: 'Show process dashboard' },
    { command: 'rufflo process monitor --watch --interval 5', description: 'Watch mode' },
    { command: 'rufflo process monitor --components agents,memory,tasks', description: 'Monitor specific components' },
    { command: 'rufflo process monitor --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const agentStorePath = resolve('.rufflo/agents/store.json');
    ...
    try {
      const taskStorePath = resolve('.rufflo/tasks/store.json');
    ...
    const metrics = {
      ...
      agents: {
        total: agentCount,
        _note: agentCount === 0 ? 'No agent store found at .rufflo/agents/store.json' : null,
      },
      tasks: {
        ...taskCounts,
        _note: (taskCounts.running + taskCounts.queued + taskCounts.completed + taskCounts.failed) === 0
          ? 'No task store found at .rufflo/tasks/store.json' : null,
      },
      memory: {
...
const workersCommand: Command = {
  ...
  examples: [
    { command: 'rufflo process workers --action list', description: 'List all workers' },
    { command: 'rufflo process workers --action spawn --type task --count 3', description: 'Spawn task workers' },
    { command: 'rufflo process workers --action kill --id worker-123', description: 'Kill specific worker' },
    { command: 'rufflo process workers --action scale --type memory --count 5', description: 'Scale memory workers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const signalsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo process signals --target daemon --signal graceful-shutdown', description: 'Graceful shutdown' },
    { command: 'rufflo process signals --target workers --signal pause', description: 'Pause workers' },
    { command: 'rufflo process signals --target all --signal reload-config', description: 'Reload all configs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo process logs', description: 'Show recent logs' },
    { command: 'rufflo process logs --source daemon --tail 100', description: 'Daemon logs' },
    { command: 'rufflo process logs --follow --level error', description: 'Follow error logs' },
    { command: 'rufflo process logs --since 1h --grep "error"', description: 'Search logs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Read actual log files from .rufflo/logs/ if they exist
    const logsDir = resolve('.rufflo/logs');
...
export const processCommand: Command = {
  ...
  examples: [
    { command: 'rufflo process daemon --action start', description: 'Start daemon' },
    { command: 'rufflo process monitor --watch', description: 'Watch processes' },
    { command: 'rufflo process workers --action list', description: 'List workers' },
    { command: 'rufflo process logs --follow', description: 'Follow logs' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  rufflo process daemon --action start');
    console.log('  rufflo process monitor --watch');
    console.log('  rufflo process workers --action spawn --type task --count 3');
    console.log('  rufflo process logs --follow --level error');
