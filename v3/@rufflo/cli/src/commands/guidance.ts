...
/** Task intent categories matching @rufflo/guidance TaskIntent */
type TaskIntent = 'bug-fix' | 'feature' | 'refactor' | 'security' | 'performance' | 'testing' | 'docs' | 'deployment' | 'architecture' | 'debug' | 'general';

/** Gate evaluation result matching @rufflo/guidance GateResult */
...
const compileCommand: Command = {
  ...
  examples: [
    { command: 'rufflo guidance compile', description: 'Compile default CLAUDE.md' },
    { command: 'rufflo guidance compile -r ./CLAUDE.md -l ./CLAUDE.local.md', description: 'Compile with local overlay' },
    { command: 'rufflo guidance compile --json', description: 'Output compiled bundle as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { GuidanceCompiler } = await import('@rufflo/guidance/compiler');
...
const retrieveCommand: Command = {
  ...
  examples: [
    { command: 'rufflo guidance retrieve -t "Fix SQL injection in user search"', description: 'Retrieve guidance for a security task' },
    { command: 'rufflo guidance retrieve -t "Add unit tests" -n 3', description: 'Retrieve top 3 shards for testing' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { GuidanceCompiler } = await import('@rufflo/guidance/compiler');
      const { ShardRetriever, HashEmbeddingProvider } = await import('@rufflo/guidance/retriever');
...
const gatesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo guidance gates -c "rm -rf /tmp"', description: 'Check if a command is destructive' },
    { command: 'rufflo guidance gates --content "api_key=sk-abc123..."', description: 'Check content for secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { EnforcementGates } = await import('@rufflo/guidance/gates');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const statusData = {
        ...
        dataDir: existsSync('./.rufflo/guidance') ? 'exists' : 'not created',
      };
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo guidance optimize', description: 'Analyze current CLAUDE.md and show suggestions' },
    { command: 'rufflo guidance optimize --apply', description: 'Apply optimizations to CLAUDE.md' },
    { command: 'rufflo guidance optimize -s compact --apply', description: 'Optimize for compact context window' },
    { command: 'rufflo guidance optimize --target-score 95', description: 'Optimize until score reaches 95' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const rootPath = ctx.flags.root as string || './CLAUDE.md';
    const localPath = ctx.flags.local as string | undefined;
    ...
    try {
      ...
      const { analyze, formatReport, optimizeForSize, formatBenchmark } = await import('@rufflo/guidance/analyzer');
...
const abTestCommand: Command = {
  ...
  examples: [
    { command: 'rufflo guidance ab-test', description: 'Run default A/B test (no guidance vs ./CLAUDE.md)' },
    { command: 'rufflo guidance ab-test -a old.md -b new.md', description: 'Compare two CLAUDE.md versions' },
    { command: 'rufflo guidance ab-test --tasks custom-tasks.json', description: 'Run with custom test tasks' },
    { command: 'rufflo guidance ab-test --json', description: 'Output full report as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { abBenchmark, getDefaultABTasks } = await import('@rufflo/guidance/analyzer');
...
export const guidanceCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo guidance compile', description: 'Compile CLAUDE.md into policy bundle' },
    { command: 'rufflo guidance retrieve -t "Fix auth bug"', description: 'Retrieve relevant guidance' },
    { command: 'rufflo guidance gates -c "rm -rf /"', description: 'Check enforcement gates' },
    { command: 'rufflo guidance status', description: 'Show control plane status' },
    { command: 'rufflo guidance optimize', description: 'Analyze and optimize CLAUDE.md' },
    { command: 'rufflo guidance ab-test', description: 'Run A/B behavioral comparison' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    ...
    output.writeln();
    output.writeln(output.dim('Use rufflo guidance <subcommand> --help for details'));
