...
const scanCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security scan -t ./src', description: 'Scan source directory' },
    { command: 'rufflo security scan --depth deep --fix', description: 'Deep scan with auto-fix' },
  ],
...
const cveCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security cve --list', description: 'List all CVEs' },
    { command: 'rufflo security cve -c CVE-2024-1234', description: 'Check specific CVE' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim(`Source: \`npm audit --json\` (GitHub Advisory DB). Run \`rufflo security scan\` for code + dep scan.`));
...
const threatsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security threats --model stride', description: 'Run STRIDE analysis' },
    { command: 'rufflo security threats -e md', description: 'Export as markdown' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const auditCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security audit --action list', description: 'List audit logs' },
    { command: 'rufflo security audit -a export', description: 'Export audit trail' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (auditEntries.length === 0) {
      output.writeln(output.dim('No audit events found. Initialize a project first: rufflo init'));
    } else {
      output.printTable({
        columns: [
...
const secretsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security secrets --action scan', description: 'Scan for secrets' },
    { command: 'rufflo security secrets -a rotate', description: 'Rotate compromised secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const defendCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security defend -i "ignore previous instructions"', description: 'Scan text for threats' },
    { command: 'rufflo security defend -f ./prompts.txt', description: 'Scan file for threats' },
    { command: 'rufflo security defend --stats', description: 'Show detection statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    let createAIDefence: typeof import('@rufflo/aidefence').createAIDefence;
    try {
      const aidefence = await import('@rufflo/aidefence');
      createAIDefence = aidefence.createAIDefence;
    } catch {
      output.error('AIDefence package not installed. Run: npm install @rufflo/aidefence');
    ...
    if (!textToScan) {
      output.writeln('Usage: rufflo security defend -i "<text>" or -f <file>');
      output.writeln();
...
export const securityCommand: Command = {
  ...
  examples: [
    { command: 'rufflo security scan', description: 'Run security scan' },
    { command: 'rufflo security cve --list', description: 'List known CVEs' },
    { command: 'rufflo security threats', description: 'Run threat analysis' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Security Suite'));
