...
const storeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory store -k "api/auth" -v "JWT implementation"', description: 'Store text' },
    { command: 'rufflo memory store -k "pattern/singleton" --vector', description: 'Store vector' },
    { command: 'rufflo memory store -k "pattern" -v "updated" --upsert', description: 'Update existing' }
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory search -q "authentication patterns"', description: 'Semantic search' },
    { command: 'rufflo memory search -q "JWT" -t keyword', description: 'Keyword search' },
    { command: 'rufflo memory search -q "test" --build-hnsw', description: 'Build HNSW index and search' },
    { command: 'rufflo memory search -q "auth patterns" --smart', description: 'SmartRetrieval with RRF + MMR' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // @rufflo/memory don't expose it. Fall through to plain
      ...
      if (useSmart) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const memMod: any = await import('@rufflo/memory');
        ...
        if (!smartSearchFn) {
          output.printWarning(
            'Smart search requested but smartSearch is not available on the installed @rufflo/memory build (#1846). Falling back to standard semantic search.',
          );
        }
      }
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: rufflo memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: rufflo memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'rufflo memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'rufflo memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
      ...
      try {
        ...
        output.printTable({
          ...
          data: [
            ...
            {
              metric: 'HNSW Index',
              // rufflo#1989 / #1987: `hnsw.entryCount` is in-process JS state
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'rufflo memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'rufflo memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const compressCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory compress', description: 'Balanced compression' },
    { command: 'rufflo memory compress --quantize --bits 4', description: '4-bit quantization (32x reduction)' },
    { command: 'rufflo memory compress -l max -t vectors', description: 'Max compression on vectors' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory export -o ./backup.json', description: 'Export all to JSON' },
    { command: 'rufflo memory export -o ./data.csv -f csv', description: 'Export to CSV' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory import -i ./backup.json', description: 'Import from file' },
    { command: 'rufflo memory import -i ./data.json -n archive', description: 'Import to namespace' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  examples: [
    { command: 'rufflo memory init', description: 'Initialize hybrid backend with all features' },
    { command: 'rufflo memory init -b agentdb', description: 'Initialize AgentDB backend' },
    { command: 'rufflo memory init -p ./data/memory.db --force', description: 'Reinitialize at custom path' },
    { command: 'rufflo memory init --verbose --verify', description: 'Initialize with full verification' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('rufflo memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('rufflo memory search -q "query"')}`,
        `Train patterns: ${output.highlight('rufflo neural train -p coordination')}`,
        `View stats: ${output.highlight('rufflo memory stats')}`
      ]);
...
export const memoryCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo memory store -k "key" -v "value"', description: 'Store data' },
    { command: 'rufflo memory search -q "auth patterns"', description: 'Search memory' },
    { command: 'rufflo memory stats', description: 'Show statistics' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Memory Management Commands'));
    output.writeln();
    output.writeln('Usage: rufflo memory <subcommand> [options]');
    output.writeln();
