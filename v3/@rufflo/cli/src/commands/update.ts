...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @rufflo package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @rufflo packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  rufflo update all');
      }

      return { success: true };
    } finally {
      delete process.env.RUFFLO_FORCE_UPDATE;
    }
  },
};
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @rufflo packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @rufflo package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @rufflo package updates with auto-update support.');
    output.writeln();
    ...
    output.printList([
      `${output.dim('RUFFLO_AUTO_UPDATE=false')}  - Disable auto-update`,
      `${output.dim('RUFFLO_FORCE_UPDATE=true')} - Force update check`,
    ]);
    output.writeln();
    output.writeln('Run "rufflo update <subcommand> --help" for subcommand help');
