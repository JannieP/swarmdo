...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: rufflo-official)' },
  ],
  examples: [
    { command: 'rufflo plugins list', description: 'List all plugins from registry' },
    { command: 'rufflo plugins list --installed', description: 'List installed only' },
    { command: 'rufflo plugins list --official', description: 'List official plugins' },
    { command: 'rufflo plugins list --category security', description: 'List security plugins' },
  ],
    ...
    if (installedOnly) {
      ...
      try {
        ...
        if (installed.length === 0) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "rufflo plugins list" to see available plugins'));
          output.writeln(output.dim('Run "rufflo plugins install -n <plugin>" to install'));
          return { success: true };
        }
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'rufflo plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "rufflo plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'rufflo plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins info -n @rufflo/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'rufflo plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const upgradeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins upgrade -n @rufflo/neural', description: 'Upgrade to latest' },
    { command: 'rufflo plugins upgrade -n @rufflo/neural -v 3.1.0', description: 'Upgrade to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'rufflo plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const rateCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins rate -n @rufflo/embeddings -r 5', description: 'Rate 5 stars' },
    { command: 'rufflo plugins rate -n my-plugin -r 4', description: 'Rate 4 stars' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo plugins list', description: 'List plugins from IPFS registry' },
    { command: 'rufflo plugins search -q neural', description: 'Search for plugins' },
    { command: 'rufflo plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'rufflo plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Plugin System'));
    ...
    output.printList([
      '@rufflo/neural              - Neural patterns and inference (WASM SIMD)',
      '@rufflo/security            - Security scanning and CVE detection',
      '@rufflo/embeddings          - Vector embeddings with hyperbolic support',
      '@rufflo/claims              - Claims-based authorization',
      '@rufflo/performance         - Performance profiling and benchmarks',
      '@rufflo/plugin-gastown-bridge - Gas Town orchestrator integration (WASM-accelerated)',
    ]);
    output.writeln();
    output.writeln(output.dim('Run "rufflo plugins list --official" to see all official plugins'));
