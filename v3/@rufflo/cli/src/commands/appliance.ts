...
async function loadModule<T>(path: string, exportName: string, label: string): Promise<T | null> {
  try {
    ...
    output.printError(`RVFA ${label} module not found`, 'Install with: npm install @rufflo/appliance');
    return null;
  }
}
...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained rufflo.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'rufflo.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'rufflo.rvf';
    ...
    const steps = [
      ...
      'Packaging rufflo CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'rufflo'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'rufflo', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'rufflo appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'rufflo appliance inspect -f rufflo.rvf', description: 'Inspect appliance contents' },
    { command: 'rufflo appliance verify -f rufflo.rvf', description: 'Verify integrity' },
    { command: 'rufflo appliance extract -f rufflo.rvf', description: 'Extract sections' },
    { command: 'rufflo appliance run -f rufflo.rvf', description: 'Boot and run appliance' },
    { command: 'rufflo appliance sign -f rufflo.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'rufflo appliance publish -f rufflo.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'rufflo appliance update -f rufflo.rvf -s rufflo -d ./new-rufflo.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full Rufflo platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained rufflo.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "rufflo appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
