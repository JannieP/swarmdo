 ...
 * Removes project artifacts created by rufflo/rufflo
 *
 * Created with ruv.io
 */
...
/**
 * Rufflo-owned subdirectories within .claude/ that are safe to delete.
...
const CLAUDE_OWNED_SUBDIRS = [
  { path: join('.claude', 'helpers'), description: 'Rufflo hook scripts' },
];

/**
 * Artifact directories and files that rufflo/rufflo may create
 */
const ARTIFACT_DIRS = [
  { path: '.rufflo', description: 'Capabilities and configuration' },
...
const ARTIFACT_FILES = [
  { path: 'rufflo.config.json', description: 'Rufflo configuration' },
];
...
export const cleanupCommand: Command = {
  name: 'cleanup',
  description: 'Remove project artifacts created by rufflo/rufflo',
  aliases: ['clean'],
  options: [
    {
      name: 'dry-run',
    ...
    },
    {
      name: 'keep-config',
      short: 'k',
      description: 'Preserve rufflo.config.json and .claude/settings.json',
      type: 'boolean',
      default: false,
    },
  ],
  examples: [
    ...
    },
    {
      ...
      description: 'Remove all rufflo artifacts',
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Scan rufflo-owned subdirs within .claude/ (surgical — preserves Claude Code files)
    ...
    // Check if .claude/settings.json has rufflo hooks/claudeFlow blocks to clean
    const settingsPath = join(cwd, '.claude', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.claude', 'settings.json'), description: 'Remove rufflo hooks/claudeFlow blocks (preserves rest)', size: 0, type: 'file' });
    }
    ...
    if (found.length === 0) {
      output.writeln(output.info('No rufflo artifacts found in the current directory.'));
