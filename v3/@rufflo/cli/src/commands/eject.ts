 ...
 *
 * Lifts the calling rufflo/rufflo project into a renamed standalone
 * harness using `metaharness --from-existing <dir>`. Attribution to
 * rufflo is preserved via the upstream's `<!-- rufflo-attribution-block -->`
 * convention.
 *
 *   npx rufflo eject --name my-harness                  # dry-run plan
 *   npx rufflo eject --name my-harness --confirm        # actually eject
 *   npx rufflo eject --name my-harness --target /abs/out --confirm
 *
 ...
 *      is `/tmp/rufflo-eject-<ts>-<name>/`. Writing to the calling repo
 *      is refused with exit 2 — protects the user from `cwd: $rufflo`
 *      eject accidents.
 ...
 *   exits 0 with a structured "feature not available" message. Rufflo
...
export const ejectCommand: Command = {
  name: 'eject',
  description:
    'Lift the calling rufflo project into a renamed standalone harness via metaharness --from-existing (ADR-150 Phase 2). Dry-run by default; --confirm required to write.',
  options: [
    {
      name: 'name',
    ...
    {
      name: 'target',
      description: 'Absolute output dir (default: /tmp/rufflo-eject-<ts>-<name>/); refused if inside the calling repo',
      type: 'string' as const,
    },
    {
      name: 'confirm',
  ...
  examples: [
    { command: 'npx rufflo eject --name my-harness', description: 'Dry-run; prints what would happen' },
    {
      command: 'npx rufflo eject --name my-harness --confirm',
      description: 'Eject to /tmp/rufflo-eject-<ts>-my-harness/',
    },
    {
      command: 'npx rufflo eject --name my-harness --target /abs/out --confirm',
  ...
  async action(context: CommandContext): Promise<CommandResult> {
    ...
    if (!opts.name) {
      ...
      output.writeln('');
      output.writeln('Example: npx rufflo eject --name my-harness');
    ...
    const target = opts.target
      ...
      : resolvePath(tmpdir(), `rufflo-eject-${Date.now()}-${opts.name}`);
    ...
    if (!opts.confirm) {
      if (opts.format === 'json') {
        ...
        output.writeln(output.bold('# rufflo eject (dry-run)'));
        output.writeln('');
    ...
    // Actually run.
    output.writeln(output.bold('# rufflo eject — running'));
    output.writeln('');
    ...
    if (r.degraded) {
      ...
      output.writeln(output.dim('(ADR-150 graceful degradation: rufflo runs without it; install with `npm i -D metaharness`.)'));
