...
const startCommand: Command = {
  ...
  examples: [
    { command: 'rufflo swarm start -o "Build REST API" -s development', description: 'Start development swarm' },
    { command: 'rufflo swarm start -o "Analyze codebase" --parallel', description: 'Parallel analysis' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // #2370: the old hint referenced the deprecated `rufflo@v3alpha`
      // dist-tag which now resolves to a pre-rename package. Use the current
      // `rufflo@latest` and force a fresh fetch with `-y` so npx doesn't pick
      // a stale local install.
      output.writeln(output.dim('  The MCP server may not be running. Start it with: claude mcp add rufflo -- npx -y rufflo@latest mcp start'));
    }
    ...
    output.writeln(output.dim(`  Monitor: rufflo swarm status ${swarmId}`));
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!status.hasActiveSwarm) {
      ...
      output.writeln(output.dim('  npx @rufflo/cli@latest swarm init'));
      output.writeln(output.dim('  npx @rufflo/cli@latest swarm start'));
      output.writeln();
      return { success: true, data: status };
    }
...
export const swarmCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo swarm init --v3-mode', description: 'Initialize V3 swarm' },
    { command: 'rufflo swarm start -o "Build API" -s development', description: 'Start development swarm' },
    { command: 'rufflo swarm coordinate --agents 15', description: 'V3 coordination' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarm Coordination Commands'));
    output.writeln();
    output.writeln('Usage: rufflo swarm <subcommand> [options]');
    output.writeln();
