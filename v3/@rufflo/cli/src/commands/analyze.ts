 ...
 *
 * Features:
 * - AST analysis using rufvector (tree-sitter) with graceful fallback
...
async function getASTAnalyzer() {
  try {
    return await import('../rufvector/ast-analyzer.js');
  } catch {
    return null;
  }
}
...
async function getGraphAnalyzer() {
  try {
    return await import('../rufvector/graph-analyzer.js');
  } catch {
    return null;
  }
}
...
const diffCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze diff --risk', description: 'Analyze current diff with risk assessment' },
    { command: 'rufflo analyze diff HEAD~1 --classify', description: 'Classify changes from last commit' },
    { command: 'rufflo analyze diff main..feature --format json', description: 'Compare branches with JSON output' },
    { command: 'rufflo analyze diff --reviewers', description: 'Get recommended reviewers for changes' },
  ],
...
const codeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze code -p ./src', description: 'Analyze source directory' },
    { command: 'rufflo analyze code --type complexity', description: 'Run complexity analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
// ============================================================================
// AST Analysis Subcommands (using rufvector tree-sitter with fallback)
// ============================================================================
...
const astCommand: Command = {
  name: 'ast',
  description: 'Analyze code using AST parsing (tree-sitter via rufvector)',
  options: [
    {
      name: 'complexity',
      short: 'c',
  ...
  examples: [
    { command: 'rufflo analyze ast src/', description: 'Analyze all files in src/' },
    { command: 'rufflo analyze ast src/index.ts --complexity', description: 'Analyze with complexity' },
    { command: 'rufflo analyze ast . --format json', description: 'JSON output' },
    { command: 'rufflo analyze ast src/ --symbols', description: 'Extract symbols' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const complexityAstCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze complexity src/', description: 'Analyze complexity' },
    { command: 'rufflo analyze complexity src/ --threshold 15', description: 'Flag high complexity' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const symbolsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze symbols src/', description: 'Extract all symbols' },
    { command: 'rufflo analyze symbols src/ --type function', description: 'Only functions' },
    { command: 'rufflo analyze symbols src/ --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const importsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze imports src/', description: 'Analyze all imports' },
    { command: 'rufflo analyze imports src/ --external', description: 'Only npm packages' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
    const formatType = (ctx.flags.format as string) || 'text';
...
/**
 * Fallback analysis when rufvector is not available
 */
...
const depsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze deps --outdated', description: 'Show outdated dependencies' },
    { command: 'rufflo analyze deps --security', description: 'Check for vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const boundariesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze boundaries src/', description: 'Find code boundaries in src/' },
    { command: 'rufflo analyze boundaries -p 3 src/', description: 'Find 3 partitions' },
    { command: 'rufflo analyze boundaries -f dot -o graph.dot src/', description: 'Export to DOT format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const modulesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze modules src/', description: 'Detect module communities' },
    { command: 'rufflo analyze modules -f dot -o modules.dot src/', description: 'Export colored DOT graph' },
    { command: 'rufflo analyze modules -m 3 src/', description: 'Only show communities with 3+ files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const dependenciesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze dependencies src/', description: 'Build dependency graph' },
    { command: 'rufflo analyze dependencies -f dot -o deps.dot src/', description: 'Export to DOT' },
    { command: 'rufflo analyze dependencies -i .ts,.tsx src/', description: 'Only TypeScript files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const circularCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze circular src/', description: 'Find circular dependencies' },
    { command: 'rufflo analyze circular -s high src/', description: 'Only high severity cycles' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
export const analyzeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo analyze ast src/', description: 'Analyze code with AST parsing' },
    { command: 'rufflo analyze complexity src/ --threshold 15', description: 'Find high-complexity files' },
    { command: 'rufflo analyze symbols src/ --type function', description: 'Extract all functions' },
    { command: 'rufflo analyze imports src/ --external', description: 'List npm dependencies' },
    { command: 'rufflo analyze diff --risk', description: 'Analyze diff with risk assessment' },
    { command: 'rufflo analyze boundaries src/', description: 'Find code boundaries using MinCut' },
    { command: 'rufflo analyze modules src/', description: 'Detect module communities with Louvain' },
    { command: 'rufflo analyze dependencies src/ --format dot', description: 'Export dependency graph as DOT' },
    { command: 'rufflo analyze circular src/', description: 'Find circular dependencies' },
    { command: 'rufflo analyze deps --security', description: 'Check dependency vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(`  ${output.dim('rufflo analyze ast src/')}                  # Full AST analysis`);
    output.writeln(`  ${output.dim('rufflo analyze ast src/index.ts -c')}       # Include complexity`);
    output.writeln(`  ${output.dim('rufflo analyze complexity src/ -t 15')}     # Flag high complexity`);
    output.writeln(`  ${output.dim('rufflo analyze symbols src/ --type fn')}    # Extract functions`);
    output.writeln(`  ${output.dim('rufflo analyze imports src/ --external')}   # Only npm imports`);
    output.writeln();

    output.writeln(output.bold('Graph Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('rufflo analyze boundaries src/')}            # Find natural code boundaries`);
    output.writeln(`  ${output.dim('rufflo analyze modules src/')}               # Detect module communities`);
    output.writeln(`  ${output.dim('rufflo analyze dependencies -f dot src/')}   # Export to DOT format`);
    output.writeln(`  ${output.dim('rufflo analyze circular src/')}              # Find circular deps`);
    output.writeln();

    output.writeln(output.bold('Diff Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('rufflo analyze diff --risk')}              # Risk assessment`);
    output.writeln(`  ${output.dim('rufflo analyze diff HEAD~1 --classify')}   # Classify changes`);
    output.writeln(`  ${output.dim('rufflo analyze diff main..feature')}       # Compare branches`);
    output.writeln();
