 ...
 * `plugins/rufflo-metaharness/scripts/<name>.mjs` via spawnSync. This is
 ...
 *
 *   npx rufflo metaharness score
 *   npx rufflo metaharness mcp-scan --fail-on high
 *   npx rufflo metaharness mint --name foo --template vertical:coding
 *
 * instead of:
 *
 *   node plugins/rufflo-metaharness/scripts/score.mjs
 *
...
/**
 * Walk up from the current dirname to find the rufflo repo root that
 * contains plugins/rufflo-metaharness/. Handles three install layouts:
 *   1. rufflo dev tree   (cwd / ../plugins/...)
 *   2. rufflo wrapper    (rufflo/node_modules/@rufflo/cli/...)
...
function locatePluginScripts(): string | null {
  ...
  // Also try from cwd (covers the "npx rufflo" case where the user is
  // sitting in their own repo and `npx rufflo metaharness score` should
...
export const metaharnessCommand: Command = {
  ...
  async action(context: CommandContext): Promise<CommandResult> {
    ...
    if (!subcommand || subcommand === 'help' || subcommand === '--help' || subcommand === '-h') {
      output.writeln(output.bold('npx rufflo metaharness <subcommand> [options]'));
      output.writeln('');
    ...
    if (!scriptDir) {
      output.writeln(
        output.warning(
          'metaharness: plugins/rufflo-metaharness/scripts/ not found. Install rufflo with `npm i rufflo` or run from the rufflo repo.'
        )
      );
      output.writeln(
