...
const routeTaskCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route task "implement authentication"', description: 'Route task to best agent' },
    { command: 'rufflo route task "write unit tests" --q-learning', description: 'Use Q-Learning routing' },
    { command: 'rufflo route task "review code" --agent reviewer', description: 'Force specific agent' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!taskDescription) {
      ...
      output.writeln(output.dim('Usage: rufflo route task "task description"'));
...
const listAgentsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route list-agents', description: 'List all agents' },
    { command: 'rufflo route agents --json', description: 'List agents as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const jsonOutput = ctx.flags.json as boolean;
...
const statsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route stats', description: 'Show routing statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const jsonOutput = ctx.flags.json as boolean;
    ...
    try {
      ...
      const rufvectorAvailable = await isRufvectorAvailable();

      const rufvectorStatus = {
        available: rufvectorAvailable,
        wasmAccelerated: stats.useNative === 1,
        backend: stats.useNative === 1 ? 'rufvector-native' : 'fallback',
      };

      if (jsonOutput) {
        output.printJson({ stats, rufvector: rufvectorStatus });
      } else {
        output.writeln();
        ...
        output.writeln();
        output.writeln(output.bold('RufVector Status'));
        output.printList([
          `Available: ${rufvectorStatus.available ? output.success('Yes') : output.warning('No (using fallback)')}`,
          `WASM Accelerated: ${rufvectorStatus.wasmAccelerated ? output.success('Yes') : 'No'}`,
          `Backend: ${rufvectorStatus.backend}`,
        ]);
      }

      return { success: true, data: { stats, rufvector: rufvectorStatus } };
    } catch (error) {
      output.printError(error instanceof Error ? error.message : String(error));
      return { success: false, exitCode: 1 };
    }
...
const feedbackCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route feedback -t "implement auth" -a coder -r 0.9', description: 'Positive feedback' },
    { command: 'rufflo route feedback -t "write tests" -a tester -r -0.5', description: 'Negative feedback' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const resetCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route reset', description: 'Reset router state' },
    { command: 'rufflo route reset --force', description: 'Force reset' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route export', description: 'Export Q-table to stdout' },
    { command: 'rufflo route export -f qtable.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route import -f qtable.json', description: 'Import Q-table from file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const coverageRouteCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route coverage', description: 'Analyze coverage and suggest routing' },
    { command: 'rufflo route coverage --suggest', description: 'Get improvement suggestions' },
    { command: 'rufflo route coverage --gaps', description: 'List coverage gaps by agent' },
    { command: 'rufflo route coverage -p src/auth -t 90', description: 'Analyze specific path with threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { coverageRoute, coverageSuggest, coverageGaps } = await import('../rufvector/coverage-router.js');
...
export const routeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo route "implement feature"', description: 'Route task to best agent' },
    { command: 'rufflo route "write tests" --q-learning', description: 'Use Q-Learning routing' },
    { command: 'rufflo route --agent coder "fix bug"', description: 'Force specific agent' },
    { command: 'rufflo route list-agents', description: 'List available agents' },
    { command: 'rufflo route stats', description: 'Show routing statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln('Usage: rufflo route <task> [options]');
    output.writeln('       rufflo route <subcommand>');
    output.writeln();
    ...
    output.printList([
      `RufVector: ${rufvectorAvailable ? output.success('Available') : output.warning('Fallback mode')}`,
      `Backend: ${rufvectorAvailable ? 'rufvector-native' : 'JavaScript fallback'}`,
    ]);
    output.writeln();

    output.writeln(output.dim('Run "rufflo route <subcommand> --help" for more info'));
