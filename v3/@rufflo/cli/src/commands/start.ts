 ...
 * System startup for Rufflo orchestration
 */
...
function isInitialized(cwd: string): boolean {
  const configPath = path.join(cwd, '.rufflo', 'config.yaml');
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('Rufflo is not initialized in this directory');
    output.printInfo('Run "rufflo init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting Rufflo V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('Rufflo V3 is running!');
    output.writeln();
    ...
    output.printList([
      `${output.highlight('rufflo status')} - View system status`,
      `${output.highlight('rufflo agent spawn -t coder')} - Spawn an agent`,
      `${output.highlight('rufflo swarm status')} - View swarm details`,
      `${output.highlight('rufflo stop')} - Stop the system`
    ]);

    // Daemon mode
    if (daemon) {
      output.writeln();
      output.printInfo('Running in daemon mode. Use "rufflo stop" to stop.');

      // Store PID for daemon management
      const daemonPidPath = path.join(cwd, '.rufflo', 'daemon.pid');
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the Rufflo system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping Rufflo'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop Rufflo?',
        default: false
      });
    ...
    try {
      ...
      // Clean up daemon PID
      const daemonPidPath = path.join(ctx.cwd, '.rufflo', 'daemon.pid');
      if (fs.existsSync(daemonPidPath)) {
        fs.unlinkSync(daemonPidPath);
      }

      output.writeln();
      output.printSuccess('Rufflo stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the Rufflo system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting Rufflo'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the Rufflo orchestration system',
  ...
  examples: [
    { command: 'rufflo start', description: 'Start with configuration defaults' },
    { command: 'rufflo start --daemon', description: 'Start as background daemon' },
    { command: 'rufflo start --port 3001', description: 'Start MCP on custom port' },
    { command: 'rufflo start --topology mesh', description: 'Start with mesh topology' },
    { command: 'rufflo start --skip-mcp', description: 'Start without MCP server' },
    { command: 'rufflo start quick', description: 'Quick start with defaults' },
    { command: 'rufflo start stop', description: 'Stop the running system' }
  ],
  action: startAction
};
