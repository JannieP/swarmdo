...
const trainCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural train -p coordination -e 100', description: 'Train coordination patterns' },
    { command: 'rufflo neural train -d ./training-data.json --flash', description: 'Train from file with Flash Attention' },
    { command: 'rufflo neural train -p security --wasm --contrastive', description: 'Security patterns with contrastive learning' },
  ],
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural status', description: 'Show all neural status' },
    { command: 'rufflo neural status -m model-123', description: 'Check specific model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const patternsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural patterns --action list', description: 'List all patterns' },
    { command: 'rufflo neural patterns -a analyze -q "error handling"', description: 'Analyze patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run: rufflo neural train --pattern-type general'));
...
const predictCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural predict -i "implement authentication"', description: 'Predict routing for task' },
    { command: 'rufflo neural predict -i "fix bug in login" -k 3', description: 'Get top 3 predictions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (matches.length === 0) {
        output.writeln(output.warning('No similar patterns found. Try training first: rufflo neural train'));
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural optimize --method quantize', description: 'Quantize patterns to Int8' },
    { command: 'rufflo neural optimize --method analyze -v', description: 'Analyze memory usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const patternDir = path.join(process.cwd(), '.rufflo', 'neural');
      let beforeSize = 0;
      try {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural export -m security-patterns --ipfs', description: 'Export and pin to IPFS' },
    { command: 'rufflo neural export -m code-review -o ./export.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const exportData = {
        ...
        version: '1.0.0',
        name: customName || `rufflo-model-${Date.now()}`,
      ...
      const memoryDir = path.join(process.cwd(), '.rufflo', 'memory');
      ...
      if (pinToIpfs) {
        ...
        output.writeln(output.dim(`Import command: rufflo neural import --cid ${result.IpfsHash}`));
      }
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural list', description: 'List all available models' },
    { command: 'rufflo neural list --category security', description: 'List only security models' },
    { command: 'rufflo neural list -f json', description: 'Output as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        output.writeln(output.dim('  All models:      ') + `rufflo neural import --cid ${registryCid}`);
        if (category) {
          output.writeln(output.dim(`  ${category} only: `) + `rufflo neural import --cid ${registryCid} --category ${category}`);
        } else {
          output.writeln(output.dim('  By category:     ') + `rufflo neural import --cid ${registryCid} --category <category>`);
        }
      }
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural import --cid QmXxx...', description: 'Import from IPFS' },
    { command: 'rufflo neural import -f ./patterns.json --verify', description: 'Import from file' },
    { command: 'rufflo neural import --cid QmNr1yYMK... --category security', description: 'Import only security patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run "rufflo neural patterns --action list" to see imported patterns'));
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural benchmark', description: 'Run default benchmark' },
    { command: 'rufflo neural benchmark -d 128 -i 5000', description: 'Custom benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const dim = Math.min(parseInt(ctx.flags.dim as string || '256', 10), 256);
...
const routerTrainCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router train', description: 'Train from the bundled seed and write to ./router.krr.json' },
    { command: 'rufflo neural router train -c ./my-corpus.json -o ./my-router.krr.json', description: 'Train from a custom corpus' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const routerTrainFromTrajectoriesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router train-from-trajectories', description: 'Show pairing stats from default trajectory path' },
    { command: 'rufflo neural router train-from-trajectories -w production-rows.json', description: 'Emit a corpus to feed `router train`' },
    { command: 'rufflo neural router train-from-trajectories -u assets/model-router/seed-rows.json -w merged.json', description: 'Union production rows with the bundled seed corpus' },
    { command: 'rufflo neural router train-from-trajectories --filter-source llm-judge -w high-signal.json', description: 'Keep only judge-graded rows (drop coarse agent-execute baseline)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
...
const routerModelsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router models', description: 'Show candidate registry with measured stats' },
    { command: 'rufflo neural router models -f json', description: 'Machine-readable JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fmt = (ctx.flags.format as string) || 'table';
    const fs = await import('node:fs');
...
const routerDecisionsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router decisions', description: 'Aggregate stats + 20 most-recent decisions' },
    { command: 'rufflo neural router decisions --since 24h', description: 'Last 24 hours only' },
    { command: 'rufflo neural router decisions --routed-by bandit-fallback', description: 'Find decisions where neural backend failed' },
    { command: 'rufflo neural router decisions --model haiku --format json', description: 'All haiku picks, JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerCostSavingsCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router cost-savings', description: 'All-time, all baselines (heuristic + Sonnet-always + Opus-always)' },
    { command: 'rufflo neural router cost-savings --baseline always-sonnet', description: 'Compare only against Sonnet-always' },
    { command: 'rufflo neural router cost-savings --window 24h', description: 'Daily savings trend — iter 34 drift detection' },
    { command: 'rufflo neural router cost-savings --since 7d --format json | jq .baselines.heuristic.savings.totalUsd', description: 'Pipe-friendly headline' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerTrajectoryHealthCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router trajectory-health', description: 'Snapshot of trajectory log health' },
    { command: 'rufflo neural router trajectory-health --format json | jq .pairJoinRatePct', description: 'Pipe-friendly pair-join rate for dashboards' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerConfigCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router config', description: 'All router env vars with current values' },
    { command: 'rufflo neural router config --only-overrides', description: 'Just what the operator has set' },
    { command: 'rufflo neural router config --format json | jq', description: 'Audit / diff against another deployment' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fmt = (ctx.flags.format as string) || 'table';
...
const routerCompareModesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router compare-modes "refactor strategy pattern"', description: 'Compare both modes for a task' },
    { command: 'rufflo neural router compare-modes -t "..." --ceiling 5', description: 'Cost-ceiling at $5 blended' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const task = (ctx.flags.task as string | undefined) ?? (ctx.args && ctx.args[0]) ?? null;
    if (!task) {
...
const routerStatsSummaryCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router stats-summary', description: 'One-screen health check' },
    { command: 'rufflo neural router stats-summary --format json | jq .', description: 'Pipe to dashboards / alerting' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerBanditStateCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router bandit-state', description: 'Show all Beta priors per bucket × tier + per bucket × modelId' },
    { command: 'rufflo neural router bandit-state --format json | jq .priorsById', description: 'Just the per-modelId matrix' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerPricesCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router prices', description: 'Show all models sorted by blended price' },
    { command: 'rufflo neural router prices --sort name', description: 'Sort alphabetically' },
...
const routerCostProjectionCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural router cost-projection', description: 'Project from last 7d of data → 7d/30d/90d/365d horizons' },
    { command: 'rufflo neural router cost-projection --window 24h --horizons 7d,30d', description: 'Project from last day only' },
    { command: 'rufflo neural router cost-projection --format json | jq .horizons[1].projectedSavingsUsd', description: '30-day savings projection for dashboards' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const fs = await import('node:fs');
    const path = await import('node:path');
...
const routerCommand: Command = {
  ...
  examples: [
    ...
    { command: 'rufflo neural router compare-modes "task" --ceiling 5', description: 'Compare cost-optimal vs cost-ceiling side-by-side (iter 55)' },
    { command: 'rufflo neural router decisions --since 24h', description: 'Query recorded decisions (iter 28)' },
    { command: 'rufflo neural router cost-savings --since 7d', description: 'Actual vs heuristic-counterfactual spend (iter 32)' },
    { command: 'rufflo neural router trajectory-health', description: 'JSONL log health: size, rotations, parse + pair-join rate (iter 36)' },
    { command: 'rufflo neural router ab-stats', description: 'A/B disagreement matrix from sampled ab_pair (iter 37/38)' },
    { command: 'rufflo neural router bandit-state', description: 'Persisted Beta priors per (bucket × model) (iter 48)' },
    { command: 'rufflo neural router stats-summary', description: 'One-screen SRE dashboard (iter 49)' },
    { command: 'rufflo neural router cost-projection', description: 'Project monthly/quarterly spend from measured rate (iter 41)' },
    { command: 'rufflo neural router reload', description: 'Clear in-process backend cache' },
  ],
  action: async (): Promise<CommandResult> => {
...
export const neuralCommand: Command = {
  ...
  examples: [
    { command: 'rufflo neural status', description: 'Check neural system status' },
    { command: 'rufflo neural train -p coordination', description: 'Train coordination patterns' },
    { command: 'rufflo neural patterns --action list', description: 'List learned patterns' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Rufflo Neural System'));
