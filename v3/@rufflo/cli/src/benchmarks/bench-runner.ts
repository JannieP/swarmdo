 ...
 * `<cwd>/.rufflo/bench-results.json`.
 *
 * Consumed by:
 *   - `rufflo performance benchmark` (Sprint 2 Move 3) — default path
 *   - `rufflo doctor` (Sprint 2 Move 4) — reads the persisted file
 *   - `rufflo demo` (Sprint 1 Move 7) — should refactor onto this post-merge
 *
...
export const BENCH_RESULTS_DIR = '.rufflo';
 ...
 * Tries RUFFLO_REPO_ROOT, then walks up from cwd, then from this module.
...
export function findBenchmarkScript(cwd: string = process.cwd()): string | null {
  ...
  const envRoot = process.env.RUFFLO_REPO_ROOT;
...
/** Absolute path to the persisted bench-results.json under `<cwd>/.rufflo/`. */
...
/** Persist results to `<cwd>/.rufflo/bench-results.json`. Returns the path. */
...
export function runAuthoritativeBenchmark(opts: RunBenchmarkOptions = {}): Promise<RunBenchmarkResult> {
  ...
  if (!script) {
    return Promise.resolve({
      ok: false,
      error: 'benchmark-intelligence.mjs not found (set RUFFLO_REPO_ROOT or run from the repo).',
    });
  }
