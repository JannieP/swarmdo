 ...
 *
 * This is one of rufflo's 6 architectural primitives distinguishing it
 * from HAL: HAL is stateless across runs; rufflo accumulates causal memory.
 *
 * Storage: JSONL file at ~/.cache/rufflo/gaia/causal-edges.jsonl
 *   - Simple, portable, no runtime dependency on AgentDB
 *   - Production upgrade path: switch to AgentDB causal-edge MCP controller
 *     (`mcp__rufflo__agentdb_causal-edge`) for cross-session persistence
...
export interface CausalMemoryOptions {
   ...
   * Default: ~/.cache/rufflo/gaia/causal-edges.jsonl
   */
  storePath?: string;
  /**
...
const DEFAULT_STORE_SUFFIX = path.join('.cache', 'rufflo', 'gaia', 'causal-edges.jsonl');
const DEFAULT_MAX_EDGES = 5;

/** Resolve the JSONL store path from options, defaulting to ~/.cache/rufflo/…  */
