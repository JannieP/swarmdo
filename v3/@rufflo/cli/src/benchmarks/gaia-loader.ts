 ...
 * validation split, caches it under ~/.cache/rufflo/gaia/, and exposes
...
export interface LoadGaiaOptions {
  ...
  smokeOnly?: boolean;
  /** Override the cache directory (default: ~/.cache/rufflo/gaia). */
  cacheDir?: string;
}
...
function defaultCacheDir(): string {
  return path.join(os.homedir(), '.cache', 'rufflo', 'gaia');
}
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'rufflo-gaia-loader/1.0',
        },
      },
      (res) => {
