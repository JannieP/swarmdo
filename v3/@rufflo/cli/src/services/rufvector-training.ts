/**
 * RufVector Training Service
 * Real WASM-accelerated neural training using @rufvector packages
 *
 * Features:
...
/**
 * Initialize the RufVector training system.
...
export async function initializeTraining(config: TrainingConfig = {}): Promise<{
  ...
  try {
    ...
    console.warn(`[rufvector] WASM backend unavailable (${reason}), using JS fallback`);
  ...
  try {
    ...
    console.warn(`[rufvector] @rufvector/attention unavailable (${reason}), attention features disabled`);
  }
