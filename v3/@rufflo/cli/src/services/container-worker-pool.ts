...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  ...
  statePath: '.rufflo/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.rufflo`,
   ...
   *
   * #2371: the old command spawned `npx rufflo@v3alpha daemon trigger`,
   ...
   * locally-installed stale `rufflo` without fetching the published
   * version. Workers were running pre-autopilot / pre-browser builds.
   * Use the current `rufflo@latest` and force a fresh resolution with `-y`.
   */
