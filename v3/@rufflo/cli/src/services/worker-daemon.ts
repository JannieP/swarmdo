...
export interface DaemonConfig {
  ...
  // #2356 (carry-forward from pacphi/rufflo-machine-ref token-leak findings):
...
// pacphi/rufflo-machine-ref kit's proven value) heals a forgotten daemon within
...
export class WorkerDaemon extends EventEmitter {
  ...
  // path, `rufflo daemon trigger -w <worker>` runs before headlessAvailable
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const claudeFlowDir = join(projectRoot, '.rufflo');

    // Read daemon config from .rufflo/config.json (Layer B)
  ...
  /**
   * Read daemon-specific config from .rufflo/config.{json,yaml,yml}.
  ...
  /**
   * Append a structured crash record to .rufflo/logs/crash.log.
   * Inspectable by hand or via `rufflo daemon status` follow-ups.
   */
  ...
  private get childrenFile(): string {
    return join(this.projectRoot, '.rufflo', 'daemon-children.json');
  }
  ...
  private writeChildrenSnapshot(): void {
    ...
    try {
      ...
      const dir = join(this.projectRoot, '.rufflo');
   ...
   * previous crashed lifetime. Reads `.rufflo/daemon-children.json`,
  ...
  private get pidFile(): string {
    return join(this.projectRoot, '.rufflo', 'daemon.pid');
  }
   ...
   * Each entry is a JSON file at `.rufflo/daemon-queue/<id>.json`
   * with `{ workerId, trigger, context, enqueuedAt }`. We move processed
   * files to `.rufflo/daemon-queue/.processed/` so the daemon never
   * re-runs the same dispatch and operators can inspect history.
   */
  private async processDispatchQueue(): Promise<void> {
    if (!this.running) return;
    const queueDir = join(this.projectRoot, '.rufflo', 'daemon-queue');
    ...
    if (isHeadlessWorker(workerConfig.type) && this.headlessAvailable && this.headlessExecutor) {
      try {
        ...
        if (!ok) {
          ...
          // parsedOutput that lived only in `.rufflo/logs/headless/*` and
          // never reached `.rufflo/metrics/<name>.json` — `memory stats`
   ...
   * that lived only in `.rufflo/logs/headless/*_result.log` and was
   * invisible to `npx rufflo memory stats` or the metrics consumers.
   *
  ...
  private persistHeadlessResult(
    ...
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const metricsFile = join(this.projectRoot, '.rufflo', 'metrics', 'codebase-map.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
    ...
    const map = {
      ...
      structure: {
        ...
        hasClaudeFlow: existsSync(join(this.projectRoot, '.rufflo')),
      },
      scannedAt: Date.now(),
    };
  ...
  private async runAuditWorkerLocal(): Promise<unknown> {
    ...
    const auditFile = join(this.projectRoot, '.rufflo', 'metrics', 'security-audit.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
  ...
  private async runOptimizeWorkerLocal(): Promise<unknown> {
    ...
    const optimizeFile = join(this.projectRoot, '.rufflo', 'metrics', 'performance.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
  ...
  private async runConsolidateWorker(): Promise<unknown> {
    ...
    const consolidateFile = join(this.projectRoot, '.rufflo', 'metrics', 'consolidation.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
  ...
  private async runTestGapsWorkerLocal(): Promise<unknown> {
    ...
    const testGapsFile = join(this.projectRoot, '.rufflo', 'metrics', 'test-gaps.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
  ...
  private async runBenchmarkWorkerLocal(): Promise<unknown> {
    const benchmarkFile = join(this.projectRoot, '.rufflo', 'metrics', 'benchmark.json');
    const metricsDir = join(this.projectRoot, '.rufflo', 'metrics');
