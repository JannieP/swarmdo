/**
 * One-time storage-directory migration: `.claude-flow/` -> `.rufflo/`.
 *
 * The clean-break rename (chore/rename-to-rufflo) changed every STORAGE_DIR
 * constant and path join from `.claude-flow` to `.rufflo`. Existing users
 ...
 *   - If `.rufflo/` already exists -> do nothing (already migrated, or fresh).
 *   - Else if `.claude-flow/` exists -> rename it to `.rufflo/`.
 *   - On a cross-device or permission error, fall back to a recursive copy
 *     and leave the original in place (so no data is lost).
 *   - Idempotent by STATE (presence of `.rufflo/`), not a process flag — so a
...
const NEW_DIR = '.rufflo';

/**
 * Migrate `<cwd>/.claude-flow` to `<cwd>/.rufflo` if needed.
...
function logMigration(msg: string): void {
  ...
  if (process.env.RUFFLO_QUIET_MIGRATION === '1') return;
  try {
    process.stderr.write(`[rufflo] ${msg}\n`);
  } catch {
    /* ignore */
  }
}
