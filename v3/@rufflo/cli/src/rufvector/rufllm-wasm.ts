/**
 * RufVector LLM WASM Integration
 *
 ...
 *
 * Published API (v2.0.2): RufLLMWasm, ChatTemplateWasm, ChatMessageWasm,
 ...
 *
 * @module @rufflo/cli/rufvector/rufllm-wasm
 */
 ...
 */
export async function isRufllmWasmAvailable(): Promise<boolean> {
  try {
    const mod = await import('@rufvector/rufllm-wasm');
    return typeof mod.RufLLMWasm === 'function';
  } catch {
    return false;
  }
}
 ...
 */
export async function initRufllmWasm(): Promise<void> {
  if (_wasmReady) return;
  try {
    const mod = await import('@rufvector/rufllm-wasm');
    const require_ = createRequire(import.meta.url);
    const wasmPath = require_.resolve('@rufvector/rufllm-wasm/rufllm_wasm_bg.wasm');
...
/**
 * Get rufllm-wasm status.
 */
export async function getRufllmStatus(): Promise<RufllmStatus> {
  ...
  try {
    const mod = await import('@rufvector/rufllm-wasm');
    // version is a standalone function, not on RufLLMWasm class
