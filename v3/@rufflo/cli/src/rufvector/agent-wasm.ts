/**
 * RufVector Agent WASM Integration
 *
 ...
 *
 * @module @rufflo/cli/rufvector/agent-wasm
 */
...
export async function promptWasmAgent(agentId: string, input: string): Promise<string> {
  ...
  try {
    ...
    const systemPrompt = entry.info.config.instructions || 'You are a helpful coding assistant running in a Rufflo WASM agent sandbox.';
 ...
 * composed agents can declare which of rufflo's 314 MCP tools they need.
 */
