 ...
 *
 * Provides optional rufvector WASM-accelerated vector operations for:
 * - Semantic similarity search
 * - HNSW indexing (150x faster)
 * - Embedding generation
 *
 * Gracefully degrades when rufvector is not installed.
 *
...
export interface RufVectorModule {
...
// ============================================================================
// Fallback Implementation (when rufvector not available)
// ============================================================================
...
/**
 * Generate a simple hash-based embedding (fallback when rufvector not available)
...
function generateHashEmbedding(text: string, dimensions: number = 768): Float32Array {
  if (!hashEmbeddingWarned) {
    ...
    console.warn(
      ...
      'Install rufvector or @rufflo/embeddings for real ML embeddings.'
    );
  }
...
let rufvectorModule: RufVectorModule | null = null;
...
/**
 * Attempt to load the rufvector module
 * Returns true if successfully loaded, false otherwise
 */
export async function loadRufVector(): Promise<boolean> {
  if (loadAttempted) {
    return isAvailable;
  }
  ...
  try {
    ...
    // rufvector exports VectorDB class, not createVectorDB function
    if (rufvector && (typeof rufvector.VectorDB === 'function' || typeof rufvector.VectorDb === 'function')) {
      // Create adapter module that matches our expected interface
      const VectorDBClass = rufvector.VectorDB || rufvector.VectorDb;
      rufvectorModule = {
        createVectorDB: async (dimensions: number): Promise<VectorDB> => {
          ...
          // These map onto rufvector's DbOptions.hnsw { m, efConstruction }.
          //
          // storagePath: the native rufvector DB takes an exclusive file lock on
          ...
          // Wrap rufvector's VectorDB to match our interface
          return {
        ...
        generateEmbedding: (text: string, dimensions: number = 768): Float32Array => {
          // rufvector may not have this - use fallback
    ...
    // Silently fail - rufvector is optional
  }
...
/**
 * Check if rufvector is available
 */
export function isRufVectorAvailable(): boolean {
  return isAvailable;
}
...
export function isWASMAccelerated(): boolean {
  if (rufvectorModule && typeof rufvectorModule.isWASMAccelerated === 'function') {
    return rufvectorModule.isWASMAccelerated();
  }
  return false;
}
 ...
 * Uses rufvector HNSW if available, falls back to brute-force search
 */
export async function createVectorDB(dimensions: number = 768): Promise<VectorDB> {
  await loadRufVector();

  if (rufvectorModule && typeof rufvectorModule.createVectorDB === 'function') {
    try {
      return await rufvectorModule.createVectorDB(dimensions);
    } catch (err) {
      ...
      console.warn(
        `[vector-db] rufvector HNSW init failed, falling back to brute-force search: ${(err as Error)?.message ?? err}`
      );
    }
  }
 ...
 * Uses rufvector if available, falls back to hash-based embedding
 *
...
export function generateEmbedding(text: string, dimensions: number = 768): Float32Array {
  if (rufvectorModule && typeof rufvectorModule.generateEmbedding === 'function') {
    try {
      return rufvectorModule.generateEmbedding(text, dimensions);
    } catch {
...
export function computeSimilarity(a: Float32Array, b: Float32Array): number {
  if (rufvectorModule && typeof rufvectorModule.cosineSimilarity === 'function') {
    try {
      return rufvectorModule.cosineSimilarity(a, b);
    } catch {
...
/**
 * Get status information about the rufvector module
 */
export function getStatus(): {
  available: boolean;
  wasmAccelerated: boolean;
  backend: 'rufvector-stub-search-disabled' | 'rufvector-native' | 'fallback';
} {
  if (!isAvailable) {
    return {
      available: false,
  ...
  // rufvector's `isWasm()` does NOT mean "WASM-accelerated" — there is no WASM
  ...
  return {
    ...
    backend: stubActive ? 'rufvector-stub-search-disabled' : 'rufvector-native',
  };
}
