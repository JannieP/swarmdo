# RufVector NPM Package API Documentation
...
RufVector is a high-performance vector database for Node.js with automatic native/WASM fallback. It provides self-learning intelligence for Claude Code with Q-learning optimization, vector memory, and automatic agent routing.
...
```bash
claude mcp add rufvector-mcp -- npx rufvector mcp-server
```
...
// CLI Usage
npx rufvector hooks route "implement user login"
```
...
// CLI Usage
npx rufvector hooks ast-analyze src/api/routes.ts --json
```
...
// CLI Usage
npx rufvector hooks diff-analyze --json
npx rufvector hooks diff-analyze abc123 --json
```
...
// CLI Usage
npx rufvector hooks coverage-route src/services/auth.ts
```
...
npx rufvector hooks graph-cluster src/**/*.ts --method spectral --clusters 5
```
...
```typescript
import { VectorDb } from 'rufvector';
...
|----------|---------|-------------|
| `RUFVECTOR_INTELLIGENCE_ENABLED` | `true` | Enable/disable intelligence |
| `RUFVECTOR_LEARNING_RATE` | `0.1` | Q-learning rate (0.0-1.0) |
| `RUFVECTOR_MEMORY_BACKEND` | `rvlite` | Memory storage backend |
...
```bash
# Initialize hooks
npx rufvector hooks init --pretrain --build-agents quality

# Verify setup
npx rufvector hooks verify
npx rufvector hooks doctor --fix

# Analysis
npx rufvector hooks ast-analyze <file> --json
npx rufvector hooks ast-complexity <files> --threshold 10
npx rufvector hooks diff-analyze [commit] --json
npx rufvector hooks diff-classify [commit]
npx rufvector hooks coverage-route <file>
npx rufvector hooks coverage-suggest <files>
npx rufvector hooks graph-mincut <files>
npx rufvector hooks graph-cluster <files> --method louvain

# Memory
npx rufvector hooks remember "context" -t project
npx rufvector hooks recall "query"
npx rufvector hooks route "task description"

# Stats and export
npx rufvector hooks stats
npx rufvector hooks export -o backup.json
npx rufvector hooks import backup.json --merge
```
...
- `@rufvector/attention`: ^0.1.3
- `@rufvector/core`: ^0.1.25
- `@rufvector/gnn`: ^0.1.22
- `@rufvector/sona`: ^0.1.4
...
- [npm Package](https://www.npmjs.com/package/rufvector)
- [GitHub Repository](https://github.com/ruvnet/rufvector)
- [Hooks Documentation](https://github.com/ruvnet/rufvector/blob/main/npm/packages/rufvector/HOOKS.md)
