/**
 * RufVector Integration Module for Rufflo CLI
 *
 * Provides integration with @rufvector packages for:
...
// ── RufVector LLM WASM (inference utilities) ─────────────────
export {
  isRufllmWasmAvailable,
  initRufllmWasm,
  getRufllmStatus,
  createHnswRouter,
  createSonaInstant,
...
/**
 * Check if rufvector packages are available
 */
export async function isRufvectorAvailable(): Promise<boolean> {
  try {
...
/**
 * Get rufvector version if available
 */
export async function getRufvectorVersion(): Promise<string | null> {
  try {
    const rufvector = await import('@rufvector/core');
    return (rufvector as any).version || '1.0.0';
  } catch {
    return null;
  }
}
