...
export class ASTAnalyzer {
  ...
  private rufvectorEngine: unknown = null;
  ...
  async initialize(): Promise<void> {
    try {
      // @rufvector/ast is optional - gracefully fallback if not installed
      const rufvector = await import('@rufvector/ast' as string).catch(() => null);
      if (rufvector) {
        this.rufvectorEngine = (rufvector as any).createASTAnalyzer?.(this.config);
        this.useNative = !!this.rufvectorEngine;
      }
    } catch {
