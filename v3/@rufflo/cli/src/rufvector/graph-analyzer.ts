 ...
 *
 * Provides code dependency graph analysis using rufvector's graph algorithms:
 ...
 *
 * @module @rufflo/cli/rufvector/graph-analyzer
 */
...
// ============================================================================
// RufVector Integration (with graceful fallback)
// ============================================================================

/**
 * Interface for rufvector graph operations
 */
interface IRufVectorGraph {
...
let rufVectorGraph: IRufVectorGraph | null = null;
let rufVectorLoadAttempted = false;

/**
 * Attempt to load rufvector graph algorithms
 */
async function loadRufVector(): Promise<IRufVectorGraph | null> {
  if (rufVectorLoadAttempted) return rufVectorGraph;
  rufVectorLoadAttempted = true;

  // Use dynamic module names to bypass TypeScript static analysis
  // These modules are optional and may not be installed
  const rufvectorModule = 'rufvector';
  const wasmModule = '@rufvector/wasm';

  try {
    // Try to load rufvector's graph module
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const rufvector: any = await import(/* webpackIgnore: true */ rufvectorModule).catch(() => null);

    if (rufvector && typeof rufvector.hooks_graph_mincut === 'function' && typeof rufvector.hooks_graph_cluster === 'function') {
      rufVectorGraph = {
        mincut: (nodes, edges) => rufvector.hooks_graph_mincut(nodes, edges),
        louvain: (nodes, edges) => rufvector.hooks_graph_cluster(nodes, edges),
      };
      return rufVectorGraph;
    }
  } catch {
...
/**
 * Stoer-Wagner MinCut algorithm (fallback when rufvector not available)
...
/**
 * Louvain community detection algorithm (fallback when rufvector not available)
...
export async function analyzeMinCutBoundaries(
  ...
  // Try to use rufvector, fallback to built-in
  const rufVector = await loadRufVector();

  // Get initial partition
  let result: ReturnType<typeof fallbackMinCut>;
  if (rufVector) {
    result = rufVector.mincut(nodes, edges);
  } else {
  ...
  if (numPartitions > 2 && result.partition1.length > 2) {
    ...
    const subResult = rufVector
      ? rufVector.mincut(result.partition1, subEdges)
...
export async function analyzeModuleCommunities(graph: DependencyGraph): Promise<ModuleCommunity[]> {
  ...
  // Try to use rufvector, fallback to built-in
  const rufVector = await loadRufVector();
  const result = rufVector ? rufVector.louvain(nodes, edges) : fallbackLouvain(nodes, edges);
