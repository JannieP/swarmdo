...
function getConfig(): NeuralRouterConfig {
  ...
  // dist layout:  dist/src/rufvector/neural-router.js → assets at dist/assets/...
  // src layout:   src/rufvector/neural-router.ts     → assets at assets/...
  // We probe both candidate locations.
  let assetsDir: string;
  try {
    const here = dirname(fileURLToPath(import.meta.url));
    const candidates = [
      resolvePath(here, '..', '..', 'assets', 'model-router'),       // src/rufvector → src/assets/...
      resolvePath(here, '..', '..', '..', 'assets', 'model-router'), // dist/src/rufvector → dist/assets/...
      resolvePath(here, '..', '..', '..', '..', 'assets', 'model-router'), // safety net
    ];
    assetsDir = candidates.find(existsSync) ?? candidates[0];
  } catch {
    assetsDir = resolvePath(process.cwd(), 'v3', '@rufflo', 'cli', 'assets', 'model-router');
  }
 ...
 * Used by the bench and by `rufflo neural router status` (future CLI).
 */
