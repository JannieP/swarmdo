...
export interface LoRAStats {
  ...
  /** Training backend in use ('rufllm' | 'js-fallback') */
...
async function loadTrainingPipeline(adapter: LoRAAdapter): Promise<any> {
  ...
  try {
    ...
    // Create a rufllm LoraAdapter that mirrors our JS adapter's config
    const nativeAdapter = new (rufllm as any).LoraAdapter(config.inputDim, config.outputDim, config.rank);
    rufllmPipeline = new (rufllm as any).TrainingPipeline({
      adapter: nativeAdapter,
      learningRate: 0.001,
      batchSize: 8,
    });
    return rufllmPipeline;
  } catch {
    return null;
  }
}
...
export class LoRAAdapter {
  ...
  /**
   * Eagerly load the rufllm TrainingPipeline backend.
  ...
  getStats(): LoRAStats {
    ...
    return {
      ...
      _trainingBackend: pipelineLoaded
        ? (rufllmPipeline ? 'rufllm' : 'js-fallback')
        : 'js-fallback',
    };
  }
  ...
  /**
   * Save a training checkpoint via rufllm TrainingPipeline.
   * Falls back to writing our own weight JSON if rufllm is unavailable.
   */
  async saveCheckpoint(path: string): Promise<boolean> {
    ...
    if (pipeline) {
      try {
        ...
        // Verify rufllm actually wrote the file (some versions are no-op)
  ...
  /**
   * Load a training checkpoint via rufllm TrainingPipeline.
   * Falls back to reading our own weight JSON if rufllm is unavailable.
   */
