...
export class CoverageRouter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const rufvector = await import('@rufvector/coverage' as string).catch(() => null);
      if (rufvector) {
        this.rufvectorEngine = (rufvector as any).createCoverageRouter?.(this.config);
