...
const COMMANDS_MAP: Record<string, string[]> = {
  core: ['rufflo-help.md', 'rufflo-swarm.md', 'rufflo-memory.md'],
  analysis: ['analysis'],
...
function mergeSettingsForUpgrade(existing: Record<string, unknown>): Record<string, unknown> {
  ...
  //
  // #2448 — Detect and REGENERATE the runaway `npx @rufflo/cli@latest`
  ...
  // Matches any invocation of `rufflo hooks statusline` — either via npx
  // (`npx [--prefer-offline] [@]rufflo[/cli][@<tag>] hooks statusline …`)
  // or as a bare command (`rufflo hooks statusline …`). Both forms cold-
  ...
  const BROKEN_STATUSLINE_RE = /(?:npx\s+(?:--?\S+\s+)*)?@?rufflo(?:\/cli)?(?:@\S+)?\s+hooks\s+statusline/;
  ...
  // the runaway `npx @rufflo/cli@latest hooks <sub>` form. These fire
  ...
  const BROKEN_HOOK_RE = /npx\s+(?:--?\S+\s+)*@?rufflo\/cli@latest\s+hooks\s+(\S+)/;
...
export async function executeUpgrade(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    const metricsDir = path.join(targetDir, '.rufflo', 'metrics');
    const securityDir = path.join(targetDir, '.rufflo', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use Rufflo'
      };
      fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
      result.created.push('.rufflo/metrics/v3-progress.json');
    } else {
      result.preserved.push('.rufflo/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.rufflo/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.rufflo/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Rufflo'
      };
      fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
      result.created.push('.rufflo/metrics/learning.json');
    } else {
      result.preserved.push('.rufflo/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      const audit = {
        ...
        totalCves: 3,
        lastScan: null,
        _note: 'Run: npx @rufflo/cli@latest security scan'
      };
      fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
      result.created.push('.rufflo/security/audit-status.json');
    } else {
      result.preserved.push('.rufflo/security/audit-status.json');
    }
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add rufflo allow rules)
 ...
 * that already declares a `rufflo`-keyed MCP server. We use this to skip
 * writing our own `rufflo`-keyed entry when the user has already
 ...
 * issue describes.
 *
 * Returns the path of the file that already declares `rufflo` (so we can
 * surface it in the skipped-message), or null if none found.
 */
function detectExistingRuffloMCP(targetDir: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // #2207: accept BOTH the old 'rufflo' key AND the new 'rufflo' key so that
      ...
      // 'rufflo', a second `rufflo init` must still recognise the existing install.
      if (parsed.mcpServers && typeof parsed.mcpServers === 'object') {
        const servers = parsed.mcpServers as Record<string, unknown>;
        // #2369: also recognise the legacy dist-tag keys generated by
        // pre-rename installs (rufflo ≤ 2.x). Without these the
        // detection walks parent dirs, doesn't match, and writes a NEW
        // rufflo-keyed config — both servers then run under
        // different prefixes producing duplicate-tool noise.
        if (
          'rufflo' in servers ||
          'rufflo' in servers ||
          'rufflo@alpha' in servers ||
          'rufflo@v3alpha' in servers
        ) return candidate;
      }
      ...
      //     a `claude mcp add rufflo` (or legacy `rufflo`) in this repo is
...
async function writeMCPConfig(
  ...
  if (fs.existsSync(mcpPath) && !options.force) {
    ...
    try {
      ...
      if (servers && typeof servers === 'object') {
        const staleKeys = ['rufflo@alpha', 'rufflo@v3alpha'].filter(k => k in servers);
  ...
  // #1779 — Skip writing if the user already has a `rufflo`-keyed MCP
  // server registered elsewhere (parent .mcp.json, ~/.claude.json, etc).
  // Writing our `rufflo`-keyed entry on top of that produces the
  ...
  if (!options.force) {
    const existingRuffloPath = detectExistingRuffloMCP(targetDir);
    if (existingRuffloPath) {
      result.skipped.push(`.mcp.json (existing 'rufflo' MCP registration found at ${existingRuffloPath} — would create duplicate; pass --force to write anyway)`);
      return;
    }
  }
...
function findSourceHelpersDir(sourceBaseDir?: string): string | null {
  ...
  try {
    ...
    const pkgJsonPath = esmRequire.resolve('@rufflo/cli/package.json');
...
async function writeHelpers(
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    // #2132: Always generate rufflo-hook.cjs regardless of source copy path.
    // The source helpers dir may not contain this file (it lives in
    // plugins/rufflo-core/scripts/, not .claude/helpers/), but it must
    // always be present so Windows users can use the node-based shim.
    const ruffloHookDest = path.join(helpersDir, 'rufflo-hook.cjs');
    if (!fs.existsSync(ruffloHookDest) || options.force) {
      fs.writeFileSync(ruffloHookDest, generateRuffloHookCjs(), 'utf-8');
      result.created.files.push('.claude/helpers/rufflo-hook.cjs');
    } else {
      result.skipped.push('.claude/helpers/rufflo-hook.cjs');
    }
...
/**
 * Write runtime configuration (.rufflo/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.rufflo', 'config.yaml');

  if (fs.existsSync(configPath) && !options.force) {
    result.skipped.push('.rufflo/config.yaml');
    return;
  }

  const config = `# Rufflo V3 Runtime Configuration
...
`;
  ...
  result.created.files.push('.rufflo/config.yaml');

  // Write .gitignore
  const gitignorePath = path.join(targetDir, '.rufflo', '.gitignore');
  const gitignore = `# Rufflo runtime files
data/
logs/
sessions/
neural/
...
`;
  ...
  if (!fs.existsSync(gitignorePath) || options.force) {
    ...
    result.created.files.push('.rufflo/.gitignore');
  }
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.rufflo', 'metrics');
  const learningDir = path.join(targetDir, '.rufflo', 'learning');
  const securityDir = path.join(targetDir, '.rufflo', 'security');
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Rufflo. Run: npx rufflo@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
    result.created.files.push('.rufflo/metrics/v3-progress.json');
  }
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.rufflo/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.rufflo/metrics/learning.json');
  }
  ...
  if (!fs.existsSync(auditPath) || options.force) {
    ...
    fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
    result.created.files.push('.rufflo/security/audit-status.json');
  }
}

/**
 * Write CAPABILITIES.md - comprehensive overview of all Rufflo features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.rufflo', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.rufflo/CAPABILITIES.md');
    return;
  }

  const capabilities = `# Rufflo V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
...
Rufflo V3 is a domain-driven design architecture for multi-agent AI coordination with:
...
\`\`\`bash
# Initialize swarm
npx @rufflo/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized

# Check status
npx @rufflo/cli@latest swarm status

# Monitor activity
npx @rufflo/cli@latest swarm monitor
\`\`\`
...
\`\`\`bash
# Initialize
npx @rufflo/cli@latest init --wizard

# Spawn agent
npx @rufflo/cli@latest agent spawn -t coder --name my-coder

# Memory operations
npx @rufflo/cli@latest memory store --key "pattern" --value "data" --namespace patterns
npx @rufflo/cli@latest memory search --query "authentication"

# Diagnostics
npx @rufflo/cli@latest doctor --fix
\`\`\`
...
\`\`\`bash
# Store pattern
npx @rufflo/cli@latest memory store --key "name" --value "data" --namespace patterns

# Semantic search
npx @rufflo/cli@latest memory search --query "authentication"

# List entries
npx @rufflo/cli@latest memory list --namespace patterns

# Initialize database
npx @rufflo/cli@latest memory init --force
\`\`\`
...
\`\`\`bash
# Initialize
npx @rufflo/cli@latest hive-mind init --queen-type strategic

# Status
npx @rufflo/cli@latest hive-mind status

# Spawn workers
npx @rufflo/cli@latest hive-mind spawn --count 5 --type worker

# Consensus
npx @rufflo/cli@latest hive-mind consensus --propose "task"
\`\`\`
...
# Swarm
npx rufflo@latest swarm init --topology hierarchical --max-agents 8
npx rufflo@latest swarm status
...
# Memory
npx rufflo@latest memory search --query "patterns"

# Hooks
npx rufflo@latest hooks pre-task --description "task"
npx rufflo@latest hooks worker dispatch --trigger optimize
\`\`\`
...
`;
  ...
  result.created.files.push('.rufflo/CAPABILITIES.md');
}
...
async function writeClaudeMd(
  ...
  if (fs.existsSync(claudeMdPath) && !options.force) {
    ...
    if (fs.existsSync(claudeMdPath)) {
      const backupBase = `${claudeMdPath}.pre-rufflo`;
      ...
      console.warn(`[rufflo init] Existing CLAUDE.md backed up to ${path.basename(backupPath)} before overwrite`);
    }
  ...
  // Also write/append global ~/.claude/CLAUDE.md so rufflo tools are used automatically (#1497).
  ...
  if (homeDir && !options.skipGlobalClaudeMd) {
    ...
    const ruffloBlock = [
      '',
      '# Rufflo Integration (auto-generated by rufflo init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke rufflo MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('Rufflo Integration')) {
          fs.appendFileSync(globalClaudeMd, ruffloBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended rufflo block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, ruffloBlock.trimStart(), 'utf-8');
...
function findSourceDir(type: 'skills' | 'commands' | 'agents', sourceBaseDir?: string): string | null {
  ...
  // __dirname is typically /path/to/node_modules/@rufflo/cli/dist/src/init
