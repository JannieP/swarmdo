...
export function generateSettings(options: InitOptions): object {
  ...
  // #1670 — Rufflo attribution (Co-Authored-By trailer + PR footer) is now
  ...
  // `rufflo-bot@users.noreply.github.com` is GitHub's no-reply convention and
  // is excluded from contributor graphs / mapped to a tool identity.
  if (options.attribution === true) {
    settings.attribution = {
      commit: 'Co-Authored-By: rufflo-bot <rufflo-bot@users.noreply.github.com>',
      pr: '🤖 Generated with [Rufflo](https://github.com/ruvnet/ruflo)',
    };
  }
 ...
 * install layout. `rufflo init` can land helpers either project-locally
...
function hookCmd(script: string, subcommand: string): string {
  ...
  // The fallback handles `rufflo init`'s global-install path where helpers
...
function generateStatusLineConfig(_options: InitOptions): object {
  ...
  //
  // rufflo#1948 + #1973: the previous `sh -c 'D="${CLAUDE_PROJECT_DIR:-.}"; …'`
