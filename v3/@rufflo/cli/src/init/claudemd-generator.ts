...
### Before Any Task
\`\`\`bash
npx @rufflo/cli@latest memory search --query "[task keywords]" --namespace patterns
npx @rufflo/cli@latest hooks route --task "[task description]"
\`\`\`

### After Success
\`\`\`bash
npx @rufflo/cli@latest memory store --namespace patterns --key "[name]" --value "[what worked]"
npx @rufflo/cli@latest hooks post-task --task-id "[id]" --success true --store-results true
\`\`\`
...
\`\`\`bash
npx @rufflo/cli@latest init --wizard           # Setup
npx @rufflo/cli@latest swarm init --v3-mode     # Start swarm
npx @rufflo/cli@latest memory search --query "" # Vector search
npx @rufflo/cli@latest hooks route --task ""    # Route to agent
npx @rufflo/cli@latest doctor --fix             # Diagnostics
npx @rufflo/cli@latest security scan            # Security scan
npx @rufflo/cli@latest performance benchmark    # Benchmarks
\`\`\`
...
> if you want those sweeps: \`npx rufflo@latest daemon start\` (self-stops after 12h
...
npx @rufflo/cli@latest security audit --report
\`\`\`
...
npx @rufflo/cli@latest performance profile --target "[component]"
\`\`\`
...
\`\`\`bash
npx @rufflo/cli@latest federation init
npx @rufflo/cli@latest federation join wss://peer:8443
npx @rufflo/cli@latest federation send --to peer --type task-request --message "..."
npx @rufflo/cli@latest federation status
\`\`\`
...
export function generateClaudeMd(options: InitOptions, template?: ClaudeMdTemplate): string {
  ...
  const header = `# Rufflo — Claude Code Configuration\n`;
