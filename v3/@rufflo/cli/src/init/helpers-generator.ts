...
export const ATTRIBUTION_FOOTER =
  '🤖 Generated with [Rufflo](https://github.com/ruvnet/ruflo)';
...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# Rufflo Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running Rufflo pre-commit checks..."
...
for FILE in $STAGED_FILES; do
  if [[ "$FILE" =~ \\.(ts|js|tsx|jsx)$ ]]; then
    ...
    npx @rufflo/cli hooks pre-edit --file "$FILE" --validate-syntax 2>/dev/null || true
  fi
done
...
# Notify rufflo of commit
npx rufflo@latest hooks notify \\
 ...
 * @rufflo/memory is not installed. Gets overwritten when source copy succeeds.
 */
...
async function loadMemoryPackage() {
  ...
  // Strategy 2: ESM import (works when @rufflo/memory is a direct dependency)
  try { return await import('@rufflo/memory'); } catch { /* fall through */ }

  // Strategy 3: Walk up from PROJECT_ROOT looking for the package in any node_modules
  let searchDir = PROJECT_ROOT;
  const { parse } = await import('path');
  while (searchDir !== parse(searchDir).root) {
    const candidate = join(searchDir, 'node_modules', '@rufflo', 'memory', 'dist', 'index.js');
...
export function generateWindowsDaemonManager(): string {
  return `# Rufflo V3 Daemon Manager for Windows
...
$ClaudeFlowDir = Join-Path $PWD '.rufflo'
...
function Start-SwarmMonitor {
    ...
    $process = Start-Process -FilePath 'node' -ArgumentList @(
        '-e',
        'setInterval(() => { require("fs").writeFileSync(".rufflo/metrics/swarm-activity.json", JSON.stringify({swarm:{active:true,agent_count:0},timestamp:Date.now()})) }, 5000)'
...
function Show-Status {
    Write-Host ""
    Write-Host "Rufflo V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM Rufflo V3 - Windows Batch Wrapper
...
/**
 * Generate cross-platform Node.js port of rufflo-hook.sh (#2132).
 *
 ...
 *   - Both: .claude/helpers/rufflo-hook.cjs available as a canonical cross-platform shim
 */
export function generateRuffloHookCjs(): string {
  return `#!/usr/bin/env node
/**
 * rufflo-hook.cjs — cross-platform Node.js port of rufflo-hook.sh (#2132)
 *
 * Deployed to .claude/helpers/ during rufflo init. On Windows, the
 * generated .claude/settings.json hooks point here instead of the
 * plugin's bash-only rufflo-hook.sh.
 *
...
function main() {
  ...
  if (commandExists('rufflo')) { invokeHook('rufflo', [], hookArgs, stdinData); done(); }
  if (commandExists('rufflo')) { invokeHook('rufflo', [], hookArgs, stdinData); done(); }
