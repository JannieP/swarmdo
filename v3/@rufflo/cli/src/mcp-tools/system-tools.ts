  ...
  try {
    ...
    for (const depth of ['../..', '../../..']) {
      ...
      if (existsSync(pkgPath)) {
        ...
        if (pkg.name?.includes('rufflo') || pkg.name === 'rufflo') {
...
export const systemTools: MCPTool[] = [
  {
    name: 'system_status',
    description: 'Get overall system status Use when native Bash is wrong because you need Rufflo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_metrics',
    description: 'Get system metrics and performance data Use when native Bash is wrong because you need Rufflo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_health',
    description: 'Perform system health check Use when native Bash is wrong because you need Rufflo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input) => {
      ...
      // not just the legacy `.rufflo/memory/*` triple.
      {
        const t0 = performance.now();
        const memoryCandidates = [
          join(projectCwd, '.rufflo', 'memory', 'store.json'),       // legacy
          join(projectCwd, '.rufflo', 'memory', 'agentdb.sqlite'),
      ...
      // #1843: also accept YAML config (.rufflo/config.yaml) which
      // the rest of v3 treats as canonical; previous code only counted
      // .json variants and reported `degraded` when YAML was used.
      {
        const t0 = performance.now();
        const configCandidates = [
          join(projectCwd, '.rufflo', 'config.json'),
          join(projectCwd, '.rufflo', 'config.yaml'),
          join(projectCwd, '.rufflo', 'config.yml'),
          join(projectCwd, 'rufflo.config.json'),
          join(projectCwd, 'rufflo.config.yaml'),
          join(projectCwd, 'rufflo.config.yml'),
        ];
      ...
      if (input.deep) {
        ...
        {
          const t0 = performance.now();
          const coordPath = join(projectCwd, '.rufflo', 'coordination', 'store.json');
  ...
  {
    name: 'system_info',
    description: 'Get system information Use when native Bash is wrong because you need Rufflo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async () => {
      ...
      try {
        const { getFlashAttention } = await import('@rufflo/neural');
  ...
  {
    name: 'system_reset',
    description: 'Reset system state Use when native Bash is wrong because you need Rufflo runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'mcp_status',
    description: 'Get MCP server status, including stdio mode detection Use when native Claude Code MCP status is wrong because you need Rufflo-side server detail — tool counts per namespace, transport stats, MCP handshake errors. For just "is MCP up?", `claude mcp list` is fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {},
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const storePath = join(getProjectCwd(), '.rufflo', 'tasks', 'store.json');
  ...
  {
    // #1916: `rufflo start` referenced an unregistered `mcp_start` tool. MCP
    ...
    // responds, MCP is already up. (`rufflo mcp start` runs a standalone
    // stdio/HTTP server; that's a process command, not an MCP tool.)
    name: 'mcp_start',
    description: 'Report that the in-process MCP toolset is available (no-op "start" — if this tool responds, MCP is up). Use when native `claude mcp list` is wrong because you want Rufflo-side confirmation that the in-process registry loaded. For a standalone stdio/HTTP MCP server, run `rufflo mcp start` (a process command, not this tool). Pair with mcp_status for detail.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    // #1916: `rufflo stop` referenced an unregistered `mcp_stop` tool. Same
    // story as mcp_start — nothing to stop for the in-process registry.
    name: 'mcp_stop',
    description: 'No-op "stop" for the in-process MCP toolset (there is no separate server process to stop from inside an MCP call). Use when native process-kill is wrong because you mistakenly think Rufflo runs a daemon — it does not, the tools live in the CLI process. To stop a standalone server run `rufflo mcp stop` or terminate that process. Pair with mcp_status.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async () => {
      return {
        stopped: false,
        note: 'no separate MCP server process; nothing to stop. The in-process toolset goes away when the CLI process exits. Use `rufflo mcp stop` for a standalone server.',
      };
    },
  },
];
