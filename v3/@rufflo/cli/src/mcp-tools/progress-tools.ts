 ...
 *
 * @module @rufflo/cli/mcp-tools/progress
 */
...
async function calculateProgress(): Promise<V3ProgressMetrics> {
  ...
  const commandsIndexPath = join(V3_DIR, '@rufflo/cli/src/commands/index.ts');
  ...
  const toolsIndexPath = join(V3_DIR, '@rufflo/cli/src/mcp-tools/index.ts');
  ...
  const hooksPath = join(V3_DIR, '@rufflo/cli/src/commands/hooks.ts');
