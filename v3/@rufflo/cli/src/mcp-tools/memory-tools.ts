...
const LEGACY_MEMORY_DIR = '.rufflo/memory';
...
/**
 * Check if legacy JSON store exists in old .rufflo/memory/ location
 */
...
async function ensureInitialized(): Promise<void> {
  try {
    ...
    // Migrate legacy JSON data if exists (from old .rufflo/memory/ location)
    if (hasLegacyStore()) {
...
export const memoryTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        // The export landed in @rufflo/memory@>3.0.0-alpha.14 — older
        ...
        if (input.smart) {
          ...
          let memMod: any;
          try {
            memMod = await import('@rufflo/memory');
          } catch (err) {
            smartFallbackReason = `@rufflo/memory failed to load: ${(err as Error).message}`;
          }
          ...
          smartFallbackReason = smartFallbackReason
            ?? 'smartSearch is not exported by the installed @rufflo/memory build (likely a release lag — see #1846). Falling back to standard semantic search.';
        }
  ...
  {
    // #1916: `rufflo status memory` (the detailed view) referenced an
  ...
  {
    // #1916: `rufflo memory cleanup` referenced an unregistered `memory_cleanup`
  ...
  {
    // #1916: `rufflo memory compress` referenced an unregistered tool. The
  ...
  {
    // #1916: `rufflo memory export -o <file>` referenced an unregistered tool.
    ...
    handler: async (input) => {
      ...
      const payload = {
        schema: 'rufflo-memory-export/v1',
  ...
  {
    // #1916: `rufflo memory import <file>` referenced an unregistered tool.
    // Reads a rufflo-memory-export JSON and re-stores each entry.
    name: 'memory_import',
