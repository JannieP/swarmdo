...
type AIDefenceInstance = ReturnType<typeof import('@rufflo/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@rufflo/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @rufflo/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[rufflo] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    if (instance) {
      aidefenceInstance = instance;
      console.error(`[rufflo] ${packageName} loaded after install (resolver path)`);
      return instance;
    }
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[rufflo] ${packageName} loaded after install (file:// path)`);
    return instance;
  } catch (retryError) {
    throw new Error(
      ...
      `(common with global installs of \`rufflo\`). Recovery options:\n` +
      `  1. Run \`npm install --save @rufflo/aidefence\` in your project's working directory.\n` +
      `  2. Or run \`npx rufflo@latest mcp start\` from a directory whose node_modules contains the package.\n` +
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      ...
      const defender = await getAIDefence();
      const { isSafe } = await import('@rufflo/aidefence');
