 ...
 *
 * Exposes `rufflo-testgen` plugin scripts as first-class MCP tools so
 ...
 * Zero static `rufflo-testgen/*` imports. All script invocation stays
 ...
 *
 * @module @rufflo/cli/mcp-tools/testgen
 */
...
/**
 * Locate the rufflo-testgen plugin's scripts directory.
 *
...
function locateTestgenScripts(): string | null {
  ...
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'rufflo-testgen', 'scripts'));
    candidates.push(join(p, '..', 'plugins', 'rufflo-testgen', 'scripts'));
    p = dirname(p);
  }
  const cwd = getProjectCwd();
  candidates.push(join(cwd, 'plugins', 'rufflo-testgen', 'scripts'));
  candidates.push(join(cwd, 'node_modules', '@rufflo', 'cli', 'plugins', 'rufflo-testgen', 'scripts'));
...
export const testgenTools: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        maxAttempts: { type: 'number', description: '1..5 (rufflo cap). Each attempt gets budgetUsd/maxAttempts of the total budget.', default: 1 },
