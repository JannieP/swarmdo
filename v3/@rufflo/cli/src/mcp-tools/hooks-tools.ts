...
// #1773 item 4 — moe-router migrated to @rufflo/neural
let moeRouter: Awaited<ReturnType<typeof import('@rufflo/neural').getMoERouter>> | null = null;
async function getMoERouter() {
  if (!moeRouter) {
    try {
      const { getMoERouter: getMoE } = await import('@rufflo/neural');
...
const ROUTING_OUTCOMES_PATH = join(resolve('.'), '.rufflo/routing-outcomes.json');
...
// #1773 item 4 — flash-attention migrated to @rufflo/neural
let flashAttention: Awaited<ReturnType<typeof import('@rufflo/neural').getFlashAttention>> | null = null;
async function getFlashAttention() {
  if (!flashAttention) {
    try {
      const { getFlashAttention: getFlash } = await import('@rufflo/neural');
...
const MEMORY_DIR = '.rufflo/memory';
...
export const hooksPostEdit: MCPTool = {
  ...
  description: 'Record editing outcome for learning Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksPreCommand: MCPTool = {
  ...
  description: 'Assess risk before executing a command Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksPostCommand: MCPTool = {
  ...
  description: 'Record command execution outcome Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksMetrics: MCPTool = {
  name: 'hooks_metrics',
  description: 'View learning metrics dashboard Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksList: MCPTool = {
  name: 'hooks_list',
  description: 'List all registered hooks Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
export const hooksPostTask: MCPTool = {
  ...
  description: 'Record task completion for learning Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
      taskId: { type: 'string', description: 'Task identifier' },
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      const dataDir = join(getProjectCwd(), '.rufflo', 'data');
...
export const hooksExplain: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      const outcomesPath = join(resolve('.'), '.rufflo/routing-outcomes.json');
...
export const hooksSessionEnd: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    let insightCount = 0;
    try {
      const insightsPath = resolve(join('.rufflo', 'data', 'pending-insights.jsonl'));
...
export const hooksSessionRestore: MCPTool = {
  ...
  description: 'Restore a previous session Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksNotify: MCPTool = {
  name: 'hooks_notify',
  description: 'Send cross-agent notification Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksIntelligenceReset: MCPTool = {
  ...
  description: 'Reset intelligence learning state Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
  handler: async () => {
    ...
    const dataFiles = [
      join(cwd, '.rufflo', 'data', 'auto-memory-store.json'),
      join(cwd, '.rufflo', 'data', 'graph-state.json'),
      join(cwd, '.rufflo', 'data', 'ranked-context.json'),
    ];
    ...
    const neuralDir = join(cwd, '.rufflo', 'neural');
...
export const hooksPatternSearch: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      searchTimeMs: 0,
      backend: 'unavailable',
      note: 'Real vector search not available. Initialize memory database with: rufflo memory init',
    };
  },
};
...
export const hooksIntelligenceStats: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const stats = {
      ...
      // `rufflo neural router cost-savings --since 7d` shape).
      costSavings,
...
export const hooksIntelligenceAttention: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    async function getQueryEmbedding(text: string, dims: number): Promise<{ embedding: Float32Array; source: 'onnx' | 'hash-fallback' }> {
      // Try ONNX via @rufflo/embeddings
      try {
        const embeddingsModule = await import('@rufflo/embeddings').catch(() => null);
...
export const hooksWorkerDispatch: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const cwd = getProjectCwd();
    const pidFile = join(cwd, '.rufflo', 'daemon.pid');
    ...
    if (!daemonAlive) {
      ...
      note = 'No worker daemon detected. Run `rufflo daemon start` to enable real worker execution. The dispatch was recorded in-process but no actual work will run.';
    } else if (background) {
      ...
      const queueDir = join(cwd, '.rufflo', 'daemon-queue');
      ...
      try {
        ...
        note = `Daemon detected (pid ${daemonPid}) but queue write to ${queuePath} failed: ${(err as Error).message}. Worker recorded in-process only; use \`rufflo daemon trigger -w ${trigger}\` to run synchronously.`;
      }
      if (queueWritten) {
        reportedStatus = 'queued';
        note = `Worker queued for daemon (pid ${daemonPid}) at ${queuePath}. Daemon polls every 5s; processed entries move to .rufflo/daemon-queue/.processed/. Poll hooks_worker-status until status === "completed".`;
      } else {
...
export const hooksModelStats: MCPTool = {
  ...
  description: 'Get model routing statistics Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
export const hooksWorkerCancel: MCPTool = {
  ...
  description: 'Cancel a running worker Use when native Bash hooks (via Claude Code\'s settings.json) are wrong because you need Rufflo-side state — pattern persistence, neural training signals, model-routing learning, cost tracking, audit chain. For one-off shell commands, plain Bash hooks are fine.',
  inputSchema: {
    type: 'object',
    properties: {
...
// #1916: the `rufflo hooks teammate-idle` / `rufflo hooks task-completed` CLI
