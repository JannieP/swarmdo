...
import type { ChatMessage } from '../rufvector/rufllm-wasm.js';

// #2086 — every rufllm_* MCP handler that touches the WASM runtime calls
...
// MCP callers — they don't need a separate `rufllm_init` tool. `_wasmReady`
// inside `initRufllmWasm` short-circuits on the second+ call, so the cost
// after the first invocation is one boolean check.
//
// `rufllm_status` deliberately uses `loadRufllmWasmModule()` (no init) so a
// caller diagnosing why nothing works gets `initialized=false` instead of
// an error from a failed init.
async function loadRufllmWasm() {
  const mod = await loadRufllmWasmModule();
  await mod.initRufllmWasm();
  return mod;
}

async function loadRufllmWasmModule() {
  return import('../rufvector/rufllm-wasm.js');
}

export const rufllmWasmTools: MCPTool[] = [
  {
    name: 'rufllm_status',
    description: 'Get rufllm-wasm availability and initialization status. Use when sending every prompt to the Anthropic API is wrong because you need local inference — air-gapped environments, MicroLoRA-fine-tuned per-task adapters, or sub-cent per-call cost. For general Claude work native Task is the right call.',
    inputSchema: { type: 'object' as const, properties: {} },
    handler: async () => {
      try {
        const mod = await loadRufllmWasmModule();
        const wasmStatus = await mod.getRufllmStatus();

        // Also include native rufllm CJS backend status (ADR-086)
        ...
        try {
          ...
          nativeBackend = {
            available: iStats._rufllmBackend === 'active',
            coordinator: iStats._rufllmBackend || 'unavailable',
            trajectories: iStats._rufllmTrajectories || 0,
  ...
  },
  {
    ...
    handler: async (args: Record<string, unknown>) => {
      try {
        const mod = await loadRufllmWasm();
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        routerId: { type: 'string', description: 'HNSW router ID from rufllm_hnsw_create' },
...
const hnswRouters = new Map<string, Awaited<ReturnType<typeof import('../rufvector/rufllm-wasm.js').createHnswRouter>>>();
const sonaInstances = new Map<string, Awaited<ReturnType<typeof import('../rufvector/rufllm-wasm.js').createSonaInstant>>>();
const loraInstances = new Map<string, Awaited<ReturnType<typeof import('../rufvector/rufllm-wasm.js').createMicroLora>>>();
