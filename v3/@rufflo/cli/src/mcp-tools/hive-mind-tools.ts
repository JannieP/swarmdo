...
function loadAgentStore(): { agents: Record<string, unknown> } {
  const storePath = join(getProjectCwd(), '.rufflo', 'agents.json');
  try {
...
function saveAgentStore(store: { agents: Record<string, unknown> }): void {
  const storeDir = join(getProjectCwd(), '.rufflo');
...
export const hiveMindTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      const taskStorePath = join(getProjectCwd(), '.rufflo', 'tasks', 'store.json');
  ...
  {
    // #1916: `rufflo hive-mind optimize-memory` referenced an unregistered
