...
export async function autoInstallPackage(
  ...
  if (!validPackageName.test(packageName)) {
    if (!silent) {
      console.error(`[rufflo] Invalid package name: ${packageName}`);
    }
    return false;
  }
  ...
  try {
    if (!silent) {
      console.error(`[rufflo] Auto-installing ${packageName}...`);
    }
    ...
    if (!silent) {
      console.error(`[rufflo] Successfully installed ${packageName}`);
    }
    return true;
  } catch (error) {
    if (!silent) {
      console.error(`[rufflo] Failed to auto-install ${packageName}: ${error}`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (installed) {
      try {
        ...
        console.error(`[rufflo] ${packageName} installed but failed to load. Restart MCP server.`);
        return null;
      }
    }
    return null;
