 ...
 * - Uses @rufflo/embeddings for REAL ML embeddings when available
 ...
 *
 * Note: For production neural features, use @rufflo/neural module
 */
...
try {
  ...
  // Tier 2: @rufflo/embeddings with agentic-flow provider
  if (!realEmbeddings) {
  ...
  // Tier 3: @rufflo/embeddings with ONNX provider
  if (!realEmbeddings) {
    const embeddingsModule = await import('@rufflo/embeddings').catch(() => null);
    if (embeddingsModule?.createEmbeddingService) {
      try {
...
export function getNeuralStoreStats(): {
  ...
  return {
    ...
    source: '.rufflo/neural/patterns.json (loadNeuralStore)',
  };
}
    ...
    handler: async (input) => {
      ...
      return {
        ...
        ...(embeddingServiceName === 'hash-fallback' || embeddingServiceName === 'none' ? {
          platformNote: 'ONNX embeddings not available — using hash-based fallback. Install @rufflo/embeddings and run "embeddings init --download" for semantic search.',
        } : {}),
      };
    },
  },
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      return {
        ...
        embeddingProvider: realEmbeddings ? `@rufflo/embeddings (${embeddingServiceName})` : 'hash-based (deterministic)',
        models: {
          total: models.length,
        ...
        features: {
          ...
          flashAttention: await (async () => {
            try {
              // #1773 item 4 — flash-attention now lives in @rufflo/neural
