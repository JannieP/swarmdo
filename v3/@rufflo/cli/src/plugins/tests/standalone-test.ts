...
async function main() {
  ...
  try {
    const similar = findSimilarPlugins(registry, '@rufflo/neural', 3);
    console.log(`  ✅ Found ${similar.length} similar plugins to @rufflo/neural`);
    passed++;
  } catch (e) {
    console.log(`  ❌ Failed: ${(e as Error).message}`);
    failed++;
  ...
  if (failed === 0) {
    ...
    console.log('    npx @rufflo/cli plugins list');
    console.log('    npx @rufflo/cli plugins list --official');
    console.log('    npx @rufflo/cli plugins search -q "plugin creator"');
    console.log('    npx @rufflo/cli plugins info -n plugin-creator');
    console.log('');
  }
