...
export class PluginManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    const pluginsDir = path.join(baseDir, '.rufflo', 'plugins');
    this.config = {
      pluginsDir,
  ...
  async installFromNpm(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Check for rufflo plugin metadata
        if (pkg['rufflo']) {
          commands = pkg['rufflo'].commands || [];
          hooks = pkg['rufflo'].hooks || [];
        }
      }
  ...
  async upgrade(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        existing.commands = pkg['rufflo']?.commands || existing.commands;
        existing.hooks = pkg['rufflo']?.hooks || existing.hooks;
      }
...
export function getPluginManager(baseDir?: string): PluginManager {
  ...
  } else if (baseDir && defaultManager.getPluginsDir() !== path.join(baseDir, '.rufflo', 'plugins')) {
