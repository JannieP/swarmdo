...
```bash
# Create new project
gcloud projects create rufflo-registry --name="Rufflo Registry"

# Set as active project
gcloud config set project rufflo-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link rufflo-registry \
...
gcloud storage buckets create gs://rufflo-plugin-registry \
  --location=US \
  --uniform-bucket-level-access

# Enable versioning for rollback
gsutil versioning set on gs://rufflo-plugin-registry

# Make registry.json publicly readable (optional)
gsutil iam ch allUsers:objectViewer gs://rufflo-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding rufflo-registry \
  --member="serviceAccount:plugin-registry-publisher@rufflo-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/rufflo-registry/secrets/${name}/versions/latest`,
  });
...
export async function publishRegistry(req, res) {
  try {
    ...
    const bucket = storage.bucket('rufflo-plugin-registry');
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@rufflo-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=rufflo-registry
```
...
# Upload to GCS
gsutil cp registry.json gs://rufflo-plugin-registry/registry.json
```
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://rufflo-plugin-registry/registry.json']
...
substitutions:
  _FUNCTION_URL: 'https://us-central1-rufflo-registry.cloudfunctions.net/publish-registry'
...
gcloud scheduler jobs create http publish-registry-daily \
  ...
  --schedule="0 2 * * *" \
  --uri="https://us-central1-rufflo-registry.cloudfunctions.net/publish-registry" \
  --http-method=POST
```

## Step 6: Update Rufflo CLI
