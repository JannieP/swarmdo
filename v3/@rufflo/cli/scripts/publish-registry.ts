...
async function generateRegistry(): Promise<PluginRegistry> {
  ...
  for (const pkg of officialPackages) {
    ...
    plugins.push({
      id: pkg,
      name: pkg,
      displayName: pkg.replace('@rufflo/plugin-', '').replace('@rufflo/', ''),
      description: `Official Rufflo plugin: ${pkg}`,
  ...
  return {
    ...
    plugins,
    categories: [
      { id: 'official', name: 'Official', description: 'Official Rufflo plugins', pluginCount: plugins.length },
    ],
...
async function main() {
  ...
  try {
    const result = await pinToIPFS(registry, 'rufflo-plugin-registry', jwt);
    ...
    console.log('   3. Test with: npx rufflo@latest plugins list');
  } catch (error) {
