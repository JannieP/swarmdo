/**
 * Rufflo Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@rufflo.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'rufflo';
const BUCKET_NAME = 'rufflo-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `rufflo-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'rufflo-plugin-registry', pinataJwt);
