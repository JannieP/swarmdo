#!/bin/bash
# Deploy Rufflo Registry Cloud Function

set -e

PROJECT_ID="${GCP_PROJECT:-rufflo}"
...
gsutil mb -p $PROJECT_ID gs://rufflo-plugin-registry 2>/dev/null || true
...
echo ""
echo "Rate a plugin:"
echo "  curl -X POST '$FUNCTION_URL?action=rate' -H 'Content-Type: application/json' -d '{\"itemId\":\"@rufflo/embeddings\",\"rating\":5}'"
