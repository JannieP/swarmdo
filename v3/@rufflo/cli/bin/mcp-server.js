#!/usr/bin/env node
/**
 * @rufflo/cli - MCP Server Entry Point
 *
  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] ERROR [rufflo-mcp] Failed to parse:`,
...
process.on('SIGINT', () => {
  console.error(`[${new Date().toISOString()}] INFO [rufflo-mcp] Received SIGINT`);
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.error(`[${new Date().toISOString()}] INFO [rufflo-mcp] Received SIGTERM`);
  process.exit(0);
});
  ...
  try {
    switch (message.method) {
      case 'initialize':
        return {
          ...
          result: {
            ...
            serverInfo: { name: 'rufflo', version: VERSION },
            capabilities: {
      ...
      case 'notifications/initialized':
        console.error(`[${new Date().toISOString()}] INFO [rufflo-mcp] Client initialized`);
    ...
    console.error(`[${new Date().toISOString()}] ERROR [rufflo-mcp] ${message.method}:`, error);
    return {
      jsonrpc: '2.0',
      id: message.id,
      error: {
