...
hooks:
  pre: |
    ...
    # Check agent claims
    npx rufflo@v3alpha claims check --agent "$AGENT_ID" --resource "$RESOURCE" --action "$ACTION"
  post: |
    echo "✅ Authorization complete"
    # Log authorization decision
    mcp__rufflo__memory_usage --action="store" --namespace="audit" --key="auth:$(date +%s)" --value="$AUTH_DECISION"
---
...
npx rufflo@v3alpha claims check \
  --agent "agent-123" \
  --resource "memory:patterns" \
  --action "write"

# Grant claim to agent
npx rufflo@v3alpha claims grant \
  --agent "agent-123" \
  --claim "scope:write" \
  --resource "memory:*"

# Revoke claim
npx rufflo@v3alpha claims revoke \
  --agent "agent-123" \
  --claim "scope:admin"

# List agent claims
npx rufflo@v3alpha claims list --agent "agent-123"
```
...
{
  "PreToolUse": [{
    "matcher": "^mcp__rufflo__.*$",
    "hooks": [{
      "type": "command",
      "command": "npx rufflo@v3alpha claims check --agent $AGENT_ID --tool $TOOL_NAME --auto-deny"
    }]
  }],
  "PermissionRequest": [{
    "matcher": ".*",
    "hooks": [{
      "type": "command",
      "command": "npx rufflo@v3alpha claims evaluate --request '$PERMISSION_REQUEST'"
    }]
  }]
}
```
...
# Query audit log
mcp__rufflo__memory_search --pattern="auth:*" --namespace="audit" --limit=100
```
