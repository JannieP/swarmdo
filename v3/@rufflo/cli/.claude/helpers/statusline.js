#!/usr/bin/env node
/**
 * Rufflo Statusline Generator
 * Displays real-time V3 implementation progress and system status.
 * Version is read from the installed @rufflo/cli package.json at
...
// #1892 — derive Rufflo banner version from the installed cli package.json
// so the statusline never drifts from `rufflo doctor`. Falls back to a
// generic "Rufflo" label only if every resolution path fails.
function resolveBannerVersion() {
  const candidates = [
    // Local-checkout / monorepo case
    path.join(__dirname, '..', '..', 'package.json'),
    // npm-installed-as-dep case
    path.join(__dirname, '..', '..', '..', '@rufflo', 'cli', 'package.json'),
  ...
  for (const p of candidates) {
    try {
      ...
      if (pkg.name && pkg.name.includes('rufflo') && typeof pkg.version === 'string') {
  ...
  return ''; // empty → header just says "Rufflo"
}
...
function getSystemMetrics() {
  ...
  try {
    const agents = execSync('ps aux 2>/dev/null | grep -c "rufflo.*agent" || echo "0"', { encoding: 'utf-8' });
...
function generateStatusline() {
  ...
  let header = `${c.bold}${c.brightPurple}▊ Rufflo${BANNER_VERSION ? ' ' + BANNER_VERSION : ''} ${c.reset}`;
