#!/usr/bin/env node
/**
 * Rufflo V3.5 Statusline Generator
...
// install location first (`~/.claude/plugins/marketplaces/rufflo/package.json`),
// then npm-style installs, then the source-checkout location. The previous
// code hardcoded `Rufflo V3.5` in the header — so users on alpha.27+ still
// saw V3.5 in the statusline even though `rufflo doctor` reported the real
// version (#1951).
...
function getLearningStats() {
  ...
  const dataDir = path.join(process.cwd(), '.rufflo', 'data');
...
function getSystemMetrics() {
  ...
  try {
    if (isWindows) {
      ...
      const agents = execSync('tasklist 2>NUL | findstr /I "rufflo" 2>NUL | find /C /V "" 2>NUL || echo 0', { encoding: 'utf-8' });
...
function generateStatusline() {
  ...
  // Header Line
  let header = `${c.bold}${c.brightPurple}▊ Rufflo V${RUFLO_VERSION} ${c.reset}`;
...
function generateSingleLine() {
  ...
  return `${c.brightPurple}Rufflo${c.reset} ${c.dim}|${c.reset} ` +
