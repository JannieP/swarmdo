...
List all active agents in the Rufflo system with filtering options.

## Usage

```bash
npx rufflo agent list [options]
npx rufflo agent ls [options]  # Alias
```
...
npx rufflo agent list

# List all agents including inactive
npx rufflo agent list --all

# Filter by type
npx rufflo agent list -t coder

# Filter by status
npx rufflo agent list -s idle

# JSON output for scripting
npx rufflo agent list --format json

# Combined filters
npx rufflo agent list -t researcher -s active
```
