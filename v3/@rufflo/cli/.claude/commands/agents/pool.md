...
npx rufflo agent pool

# Set pool size
npx rufflo agent pool --size 5

# Configure auto-scaling
npx rufflo agent pool --min 2 --max 15 --auto-scale

# Pre-warm agents for fast response
npx rufflo agent pool --warmup

# Disable auto-scaling
npx rufflo agent pool --auto-scale false
```
...
Configure in `.rufflo/config.yaml`:
...
- `npx rufflo agent spawn` - Manual agent spawning
- `npx rufflo agent list` - View active agents
- `npx rufflo swarm scale` - Swarm-level scaling
