...
```bash
npx rufflo agent health [agent-id] [options]
```
...
npx rufflo agent health coder-lx7m9k2

# Continuous monitoring
npx rufflo agent health --watch

# Custom interval
npx rufflo agent health -w -i 10

# JSON output
npx rufflo agent health --format json
```
...
- `npx rufflo agent status` - Detailed agent info
- `npx rufflo agent metrics` - Performance metrics
- `npx rufflo doctor` - System-wide health
