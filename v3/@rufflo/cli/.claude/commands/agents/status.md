...
```bash
npx rufflo agent status <agent-id>
npx rufflo agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx rufflo agent status coder-lx7m9k2

# Using --id flag
npx rufflo agent status --id researcher-abc123

# JSON output
npx rufflo agent status coder-lx7m9k2 --format json
```
...
- `npx rufflo agent metrics` - Aggregate metrics
- `npx rufflo agent health` - Health check
- `npx rufflo agent logs <id>` - Activity logs
