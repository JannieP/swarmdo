...
```bash
npx rufflo auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx rufflo auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx rufflo auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx rufflo auto agent -t "Fix bug in login" -s minimal
```
