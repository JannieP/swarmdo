...
```bash
rufflo pair --switch <session-id>
```

### Merge Sessions
```bash
rufflo pair --merge <session-1> <session-2>
```
...
### Manual Recovery
```bash
rufflo pair --recover [--point <timestamp>]
```

### Crash Recovery
```bash
rufflo pair --crash-recovery
```
...
### Share with Team
```bash
rufflo pair --share <session-id> \
  --team <team-id>
```

### Export for Review
```bash
rufflo pair --export-review <session-id>
```

### Create Learning Material
```bash
rufflo pair --create-tutorial <session-id>
```
