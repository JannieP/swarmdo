...
Main configuration file: `.rufflo/pair-config.json`
...
rufflo pair config reset verification
```
...
### Project-Specific
`.rufflo/pair-config.json` in project root

### User-Specific
`~/.rufflo/pair-config.json`

### Global
`/etc/rufflo/pair-config.json`
...
rufflo pair config validate

# Test configuration
rufflo pair config test
```
...
### From Version 1.x
```bash
rufflo pair config migrate --from 1.x
```

### Export/Import
```bash
# Export current config
rufflo pair config export > my-config.json

# Import config
rufflo pair config import my-config.json
```
