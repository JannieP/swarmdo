...
rufflo truth --period 7d

# Export to HTML report
rufflo truth --export report.html --format html
```
...
rufflo truth --threshold 0.8

# Agent-specific metrics
rufflo truth --agent coder --period 24h

# JSON for processing
rufflo truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    rufflo truth --format json > truth.json
...
Set truth display preferences in `.rufflo/config.json`:
