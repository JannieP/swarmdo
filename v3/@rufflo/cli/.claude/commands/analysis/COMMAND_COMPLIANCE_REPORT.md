...
- `mcp__rufflo__*` tools (preferred)
- `npx rufflo` commands (as fallback)
...
**Status**: ✅ Updated
**Changes Made**:
- Replaced `npx ruf-swarm hook session-end --export-metrics` with proper MCP tool call
- Updated to: `Tool: mcp__rufflo__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__rufflo__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__rufflo__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__rufflo__*` format
...
The analysis directory is now fully compliant with the Rufflo command standards.