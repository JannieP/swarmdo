...
npx rufflo sparc run mcp "integrate with external API"

# For alpha features
npx rufflo@alpha sparc run mcp "integrate with external API"

# With namespace
npx rufflo sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx rufflo sparc run mcp "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If rufflo is installed locally
./rufflo sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx rufflo memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx rufflo memory query "mcp" --limit 5
```
