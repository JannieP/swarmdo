# Claude Code Configuration - Rufflo V3
...
```bash
npx @rufflo/cli@latest hooks pre-task --description "[task description]"
```
...
npx @rufflo/cli@latest swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
Bash("npx @rufflo/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("npx @rufflo/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @rufflo/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @rufflo/cli@latest hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("npx @rufflo/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @rufflo/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @rufflo/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @rufflo/cli@latest hooks worker dispatch --trigger optimize")
```
...
npx @rufflo/cli@latest daemon start

# Spawn an agent
npx @rufflo/cli@latest agent spawn -t coder --name my-coder

# Initialize swarm
npx @rufflo/cli@latest swarm init --v3-mode

# Search memory (HNSW-indexed)
npx @rufflo/cli@latest memory search --query "authentication patterns"

# System diagnostics
npx @rufflo/cli@latest doctor --fix

# Security scan
npx @rufflo/cli@latest security scan --depth full

# Performance benchmark
npx @rufflo/cli@latest performance benchmark --suite all
```
...
```bash
# Core hooks
npx @rufflo/cli@latest hooks pre-task --description "[task]"
npx @rufflo/cli@latest hooks post-task --task-id "[id]" --success true
npx @rufflo/cli@latest hooks post-edit --file "[file]" --train-neural true

# Session management
npx @rufflo/cli@latest hooks session-start --session-id "[id]"
npx @rufflo/cli@latest hooks session-end --export-metrics true
npx @rufflo/cli@latest hooks session-restore --session-id "[id]"

# Intelligence routing
npx @rufflo/cli@latest hooks route --task "[task]"
npx @rufflo/cli@latest hooks explain --topic "[topic]"

# Neural learning
npx @rufflo/cli@latest hooks pretrain --model-type moe --epochs 10
npx @rufflo/cli@latest hooks build-agents --agent-types coder,tester

# Background workers
npx @rufflo/cli@latest hooks worker list
npx @rufflo/cli@latest hooks worker dispatch --trigger audit
npx @rufflo/cli@latest hooks worker status

# Coverage-aware routing
npx @rufflo/cli@latest hooks coverage-gaps --format table
npx @rufflo/cli@latest hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
npx @rufflo/cli@latest hooks statusline
npx @rufflo/cli@latest hooks statusline --json
```
...
npx @rufflo/cli@latest migrate status

# Run migration with backup
npx @rufflo/cli@latest migrate run --backup

# Rollback if needed
npx @rufflo/cli@latest migrate rollback
...
Bash("npx @rufflo/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @rufflo/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @rufflo/cli@latest performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("npx @rufflo/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @rufflo/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("npx @rufflo/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @rufflo/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @rufflo/cli@latest neural patterns --list")
```
...
Run `npx @rufflo/cli@latest doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
- **Swarm init**: `npx @rufflo/cli@latest swarm init --topology <type>`
- **Swarm status**: `npx @rufflo/cli@latest swarm status`
- **Agent spawn**: `npx @rufflo/cli@latest agent spawn -t <type> --name <name>`
- **Memory store**: `npx @rufflo/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `npx @rufflo/cli@latest memory search --query "search terms"`
- **Memory list**: `npx @rufflo/cli@latest memory list --namespace patterns`
- **Memory retrieve**: `npx @rufflo/cli@latest memory retrieve --key "mykey" --namespace patterns`
- **Hooks**: `npx @rufflo/cli@latest hooks <hook-name> [options]`
...
npx @rufflo/cli@latest memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx @rufflo/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
npx @rufflo/cli@latest memory search --query "authentication patterns"
npx @rufflo/cli@latest memory search --query "error handling" --namespace patterns --limit 5
```

### List Entries
```bash
# OPTIONAL: --namespace, --limit
npx @rufflo/cli@latest memory list
npx @rufflo/cli@latest memory list --namespace patterns --limit 10
```
...
npx @rufflo/cli@latest memory retrieve --key "pattern-auth"
npx @rufflo/cli@latest memory retrieve --key "pattern-auth" --namespace patterns
```

### Initialize Memory Database
```bash
npx @rufflo/cli@latest memory init --force --verbose
```
...
For a comprehensive overview of all Rufflo V3 features, agents, commands, and integrations, see:

**`.rufflo/CAPABILITIES.md`** - Complete reference generated during init
...
- Integration ecosystem (agentic-flow, agentdb, ruf-swarm, flow-nexus, agentic-jujutsu)
...
Remember: **Rufflo CLI coordinates, Claude Code Task tool creates!**
