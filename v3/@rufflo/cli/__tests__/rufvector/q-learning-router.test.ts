...
import { QLearningRouter, createQLearningRouter, type RouteDecision } from '../../src/rufvector/q-learning-router';
...
describe('QLearningRouter', () => {
  ...
  describe('initialize', () => {
    it('should initialize without rufvector (fallback mode)', async () => {
      await router.initialize();
      const stats = router.getStats();
      expect(stats.useNative).toBe(0);
    });

    it('should handle rufvector import failure gracefully', async () => {
