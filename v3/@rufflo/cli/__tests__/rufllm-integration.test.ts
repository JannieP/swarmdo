...
// Mock createRequire to return a rufllm-like module
...
vi.mock('module', () => ({
  createRequire: vi.fn().mockReturnValue(
    vi.fn().mockImplementation((specifier: string) => {
      if (specifier === '@rufvector/rufllm') return mockRufllm;
...
describe.skipIf(__SKIP_WASM_TESTS)('ADR-086: rufllm native intelligence backend', () => {
  describe('SonaCoordinator via intelligence.ts', () => {
    ...
    it('exposes _rufllmBackend in stats', async () => {
      const intel = await import('../src/memory/intelligence.js');
      const stats = intel.getIntelligenceStats();
      // Stats should include rufllm fields
      expect(stats).toHaveProperty('_rufllmBackend');
      expect(stats).toHaveProperty('_rufllmTrajectories');
    });
  });
  ...
  describe('TrainingPipeline via lora-adapter.ts', () => {
    it('exposes initBackend method', async () => {
      const lora = await import('../src/rufvector/lora-adapter.js');
  ...
  describe('CJS import pattern (createRequire)', () => {
    it('uses createRequire instead of ESM dynamic import', async () => {
      ...
      const files = [
        ...
        'rufvector/lora-adapter.ts',
      ];

      for (const file of files) {
        const content = readFileSync(join(srcDir, file), 'utf8');
        // Should NOT have bare ESM import of rufllm
        expect(content).not.toMatch(/await import\(['"]@rufvector\/rufllm['"]\)/);
  ...
  describe('graceful degradation', () => {
    it('intelligence falls back to null coordinator when rufllm unavailable', async () => {
      // The loadRufllmCoordinator function catches errors and returns null
      // Stats show 'unavailable' in that case
      const intel = await import('../src/memory/intelligence.js');
      const stats = intel.getIntelligenceStats();
      // Just verify the field exists — actual value depends on module loading order
      expect(['active', 'unavailable']).toContain(stats._rufllmBackend);
    });
    ...
    it('lora-adapter falls back to js-fallback when pipeline unavailable', async () => {
      ...
      const adapter = new lora.LoRAAdapter({ inputDim: 64, outputDim: 64, rank: 4 });
      const stats = adapter.getStats();
      expect(['rufllm', 'js-fallback']).toContain(stats._trainingBackend);
    });
  });
});
