 ...
 *
 * #2206 — mcp-generator must register the server under the 'rufflo' key
 *          (not 'rufflo') so all plugins resolve as mcp__rufflo__*.
 * #2207 — detectExistingRuffloMCP must accept both 'rufflo' and 'rufflo' keys;
 *          a bare .claude/settings.json must NOT be a false positive.
 * #2208 — writeClaudeMd (--force) must back up an existing CLAUDE.md to
 *          CLAUDE.md.pre-rufflo before overwriting it.
 */
...
describe('#2206 — mcp-generator server key', () => {
  it('registers the rufflo MCP server under the rufflo key', () => {
  ...
  it('still invokes rufflo@latest mcp start as the command args', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    const entry = config.mcpServers['rufflo'] as { command: string; args: string[] };
    expect(entry.args).toContain('rufflo@latest');
...
// ─── #2207: detectExistingRuffloMCP — both keys + no false positive ───────────
...
// calls writeMCPConfig → detectExistingRuffloMCP.
...
describe('#2207 — init detector accepts both server keys', () => {
  ...
  it('detects a prior install registered under the new rufflo key', async () => {
    // Write a parent .mcp.json with the 'rufflo' key (post-#2206 install)
    const parentDir = mkdtempSync(path.join(tmpdir(), 'wizard-2207-parent-'));
    try {
      writeFileSync(
        path.join(parentDir, '.mcp.json'),
        JSON.stringify({ mcpServers: { 'rufflo': { command: 'npx', args: ['-y', 'rufflo@latest', 'mcp', 'start'] } } }),
      );
      ...
      // Because a 'rufflo' key was found in the parent .mcp.json, init should
  ...
  it('detects a prior install registered under the legacy rufflo key', async () => {
  ...
  it('does NOT false-positive on a bare .claude/settings.json with no MCP registration', async () => {
    ...
    try {
      ...
      try {
        const result = await executeInit(opts);
        // No pre-existing rufflo/rufflo key → init must NOT skip .mcp.json
...
describe('#2208 — CLAUDE.md backup before overwrite', () => {
  ...
  it('backs up existing CLAUDE.md to CLAUDE.md.pre-rufflo when --force is used', async () => {
    ...
    const backupPath = path.join(tmp, 'CLAUDE.md.pre-rufflo');
    expect(existsSync(backupPath)).toBe(true);
    expect(readFileSync(backupPath, 'utf-8')).toBe(sentinel);
  });

  it('uses a timestamped backup name when CLAUDE.md.pre-rufflo already exists', async () => {
    writeFileSync(path.join(tmp, 'CLAUDE.md'), '# First content\n', 'utf-8');
    writeFileSync(path.join(tmp, 'CLAUDE.md.pre-rufflo'), '# Previous backup\n', 'utf-8');
    ...
    // The original .pre-rufflo must still have its old content
    expect(readFileSync(path.join(tmp, 'CLAUDE.md.pre-rufflo'), 'utf-8')).toBe('# Previous backup\n');
    // A timestamped backup must have been created
    const entries = readdirSync(tmp);
    const timestamped = entries.find(e => e.startsWith('CLAUDE.md.pre-rufflo.') && e !== 'CLAUDE.md.pre-rufflo');
  ...
  it('does NOT create a backup when no pre-existing CLAUDE.md exists', async () => {
    ...
    expect(existsSync(path.join(tmp, 'CLAUDE.md.pre-rufflo'))).toBe(false);
