...
describe('Copilot example mcp.json', () => {
  ...
  it('uses the VS Code MCP schema (root `servers`, stdio rufflo server)', () => {
    const cfg = JSON.parse(readFileSync(configPath, 'utf8'));
    expect(cfg.servers).toBeDefined();
    expect(cfg.mcpServers).toBeUndefined(); // common wrong key
    const rufflo = cfg.servers.rufflo;
    expect(rufflo).toBeDefined();
    expect(rufflo.type).toBe('stdio');
    expect(rufflo.command).toBe('npx');
    expect(rufflo.args).toEqual(expect.arrayContaining(['rufflo', 'mcp', 'start', '--tools-profile']));
  });

  it('references a REAL tools-profile (ties the doc to profiles.ts)', () => {
    const cfg = JSON.parse(readFileSync(configPath, 'utf8'));
    const args: string[] = cfg.servers.rufflo.args;
