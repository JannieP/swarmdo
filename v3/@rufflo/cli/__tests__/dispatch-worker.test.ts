...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'rufflo-dw-'));
  daemon = null;
});
afterEach(async () => {
...
function seedPendingTask(): void {
  ...
  writeFileSync(join(dir, '.rufflo', 'tasks', 'store.json'), JSON.stringify({
    tasks: {
...
describe('WorkerDaemon dispatch worker', () => {
  ...
  it('drains the queue: a no-agent task is skipped (no LLM call) and left pending', async () => {
    ...
    const tasks = JSON.parse(readFileSync(join(dir, '.rufflo', 'tasks', 'store.json'), 'utf-8')).tasks;
