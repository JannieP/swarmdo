 ...
 *
 * All three bugs are the same shape: the rename from `rufflo` to `rufflo`
 * left three runtime call sites referencing the deprecated dist-tag
 * `rufflo@v3alpha` (or `rufflo@alpha`), each of which silently
...
describe('#2369 — stale MCP key handling in init', () => {
  ...
  it('detectExistingRuffloMCP recognises the legacy `rufflo@alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'rufflo@alpha'\s+in\s+servers/);
  });

  it('detectExistingRuffloMCP recognises the legacy `rufflo@v3alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'rufflo@v3alpha'\s+in\s+servers/);
  });

  it('detectExistingRuffloMCP also recognises legacy keys in project-scoped registrations', () => {
    // The project-scoped path needs the same widening — pin both.
    expect(executorSrc).toMatch(/'rufflo@alpha'\s+in\s+mcp/);
    expect(executorSrc).toMatch(/'rufflo@v3alpha'\s+in\s+mcp/);
  });
  ...
  it('writeMCPConfig surfaces a loud message naming the deprecated key when an existing local .mcp.json uses one (Scenario A)', () => {
    ...
    expect(executorSrc).toMatch(/rufflo@alpha/);
    expect(executorSrc).toMatch(/rufflo@v3alpha/);
  });
});
...
describe('#2370 — swarm.ts MCP-down hint references the current package', () => {
  ...
  it('does not reference the deprecated `rufflo@v3alpha` dist-tag in code (comments OK)', () => {
    expect(swarmCode).not.toMatch(/rufflo@v3alpha/);
  });

  it('points users at `rufflo@latest` with the `-y` flag (forces fresh fetch)', () => {
    expect(swarmSrc).toMatch(/npx -y rufflo@latest/);
  });

  it('uses the `--` separator before the npx invocation (avoids claude-mcp flag ambiguity)', () => {
    expect(swarmSrc).toMatch(/claude mcp add rufflo -- npx -y rufflo@latest/);
  });
});
...
describe('#2371 — ContainerWorkerPool spawns workers with the current package', () => {
  ...
  it('does not spawn workers via the deprecated `rufflo@v3alpha` dist-tag in code (comments OK)', () => {
    expect(poolCode).not.toMatch(/rufflo@v3alpha/);
  });

  it('spawns workers via `rufflo@latest` with `-y` so npx never falls back to a stale local install', () => {
    ...
    expect(poolSrc).toMatch(/['"]npx['"]\s*,\s*['"]-y['"]\s*,\s*['"]rufflo@latest['"]/);
  });
});
...
describe('Sanity — no other runtime source references the deprecated dist-tags', () => {
  ...
  it('no runtime .ts/.js code references `rufflo@v3alpha` (comments and legacy-key recognition lists are OK)', () => {
    const offenders: string[] = [];
    for (const file of walk(srcRoot)) {
      const text = fs.readFileSync(file, 'utf-8');
      const code = stripComments(text);
      // Strip the legitimate appearances:
      // (a) the legacy-key recognition list in detectExistingRuffloMCP
      // (b) the staleKeys array in writeMCPConfig
      const stripped = code
        .replace(/'rufflo@v3alpha'\s+in\s+\w+/g, '')
        .replace(/\['rufflo@alpha',\s*'rufflo@v3alpha'\]/g, '');
      if (/rufflo@v3alpha/.test(stripped)) offenders.push(path.relative(srcRoot, file));
    }
