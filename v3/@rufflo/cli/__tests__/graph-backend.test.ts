...
describe('ADR-087: graph-node backend', () => {
  describe('module exports', () => {
    it('exports all expected functions', async () => {
      const mod = await import('../src/rufvector/graph-backend.js');
  ...
  describe('graceful degradation', () => {
    it('getGraphStats returns unavailable when graph-node not loaded', async () => {
      ...
      const { getGraphStats } = await import('../src/rufvector/graph-backend.js');
    ...
    it('addNode returns string or null', async () => {
      const { addNode } = await import('../src/rufvector/graph-backend.js');
    ...
    it('getNeighbors returns array', async () => {
      const { getNeighbors } = await import('../src/rufvector/graph-backend.js');
      const result = await getNeighbors('nonexistent', 2);
      expect(Array.isArray(result)).toBe(true);
    });

    it('recordCausalEdge returns result with backend', async () => {
      const { recordCausalEdge } = await import('../src/rufvector/graph-backend.js');
    ...
    it('recordSwarmTeam returns result', async () => {
      const { recordSwarmTeam } = await import('../src/rufvector/graph-backend.js');
  ...
  describe('CJS import pattern', () => {
    it('source uses createRequire for @rufvector/graph-node', async () => {
      ...
      const content = readFileSync(join(import.meta.dirname, '..', 'src', 'rufvector', 'graph-backend.ts'), 'utf8');
      ...
      expect(content).not.toMatch(/await import\(['"]@rufvector\/graph-node['"]\)/);
    });
  });
  ...
  describe('type exports', () => {
    ...
    it('exports GraphEdgeData interface shape', async () => {
      const { addEdge } = await import('../src/rufvector/graph-backend.js');
