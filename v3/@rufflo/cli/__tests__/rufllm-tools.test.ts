/**
 * Tests for rufllm-wasm MCP tools.
...
vi.mock('../src/rufvector/rufllm-wasm.js', () => ({
  isRufllmWasmAvailable: vi.fn().mockResolvedValue(true),
  initRufllmWasm: vi.fn().mockResolvedValue(undefined),
  getRufllmStatus: vi.fn().mockResolvedValue({ available: true, initialized: true, version: '2.0.1' }),
...
import { rufllmWasmTools } from '../src/mcp-tools/rufllm-tools.js';

function findTool(name: string) {
  const tool = rufllmWasmTools.find(t => t.name === name);
  if (!tool) throw new Error(`Tool not found: ${name}`);
  return tool;
}

// Same WASM-init issue as rufllm-wasm.test.ts — mocks intercept the
...
describe.skipIf(__SKIP_WASM_TESTS)('rufllm-wasm MCP tools', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should export 10 tools', () => {
    expect(rufllmWasmTools).toHaveLength(10);
  });

  describe('rufllm_status', () => {
    it('should return status', async () => {
      const tool = findTool('rufllm_status');
  ...
  describe('rufllm_hnsw_create', () => {
    it('should create router and return ID', async () => {
      const tool = findTool('rufllm_hnsw_create');
  ...
  describe('rufllm_hnsw_add', () => {
    it('should add pattern to router', async () => {
      // First create
      const createTool = findTool('rufllm_hnsw_create');
      ...
      const addTool = findTool('rufllm_hnsw_add');
  ...
  describe('rufllm_sona_create', () => {
    it('should create SONA instance', async () => {
      const tool = findTool('rufllm_sona_create');
  ...
  describe('rufllm_microlora_create', () => {
    it('should create MicroLoRA', async () => {
      const tool = findTool('rufllm_microlora_create');
  ...
  describe('rufllm_microlora_adapt', () => {
    it('should adapt with feedback', async () => {
      const createTool = findTool('rufllm_microlora_create');
      ...
      const adaptTool = findTool('rufllm_microlora_adapt');
  ...
  describe('rufllm_chat_format', () => {
    it('should format with preset', async () => {
      const tool = findTool('rufllm_chat_format');
  ...
  describe('rufllm_generate_config', () => {
    it('should create config', async () => {
      const tool = findTool('rufllm_generate_config');
