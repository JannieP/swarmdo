...
describe('isSafePackageSpec (audit_1776853149979)', () => {
  ...
  describe('shell injection in version', () => {
    it.each([
      ...
      expect(isSafePackageSpec('rufflo', version)).toBe(false);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('rejects an empty version', () => {
      expect(isSafePackageSpec('rufflo', '')).toBe(false);
    });
    ...
    it('rejects a version longer than 64 chars', () => {
      expect(isSafePackageSpec('rufflo', '1.'.repeat(40))).toBe(false);
    });
