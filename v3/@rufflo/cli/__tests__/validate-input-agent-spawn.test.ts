 ...
 * `{ agentType, name }` to `@rufflo/security`'s SpawnAgentSchema, which
...
describe('validateAgentSpawn (#1567)', () => {
  ...
  it('accepts a custom agent type not in the SpawnAgentSchema enum', async () => {
    ...
    // @rufflo/security's AgentTypeSchema enumerates, but it's a valid
    // Rufflo agent type and should NOT be rejected.
