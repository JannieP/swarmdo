 ...
 *
 *  (a) `agent_logs` MCP tool must be registered — the `rufflo agent logs <id>`
 ...
 *      `.rufflo/agents.json`, a *different* file from the canonical
 *      `.rufflo/agents/store.json` the agent tools read, so
...
describe('#1916 — agent_logs registered + hive-worker resolution', () => {
  ...
  it('canonical store wins on id collision', async () => {
    ...
    writeFileSync(
      join(dir, '.rufflo', 'agents', 'store.json'),
      JSON.stringify({
        version: '3.0.0',
        agents: {
          'hive-worker-1716-abcd': {
