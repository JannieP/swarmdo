 ...
 * `@rufflo/cli`'s package.json (only in sibling workspace packages), so under
 * pnpm's strict isolation a clean `npx rufflo` install couldn't resolve it and
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'rufflo-sqljs-'));
  prev = process.env.RUFFLO_MEMORY_PATH;
  process.env.RUFFLO_MEMORY_PATH = dir; // isolated, fresh store
});
afterEach(() => {
  if (prev === undefined) delete process.env.RUFFLO_MEMORY_PATH;
  else process.env.RUFFLO_MEMORY_PATH = prev;
