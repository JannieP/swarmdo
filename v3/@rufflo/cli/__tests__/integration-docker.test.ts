 ...
 *
 * Validates the Rufflo Docker-based deployment stack without running Docker.
...
const ROOT = resolve(__dirname, '..', '..', '..', '..');            // /workspaces/rufflo
const CLI_DIR = resolve(__dirname, '..');                             // v3/@rufflo/cli
const RUFLO_DIR = join(ROOT, 'rufflo');
...
describe('Nginx Configuration', () => {
  ...
  it('injects Rufflo welcome.js script via sub_filter', () => {
    expect(nginxContent).toMatch(/sub_filter\s+'<\/head>'\s+'<script src="\/rufflo\/welcome\.js"/);
  });
  ...
  it('serves /rufflo/ static assets from /etc/nginx/static/', () => {
    expect(nginxContent).toMatch(/location\s+\/rufflo\//);
  });
...
describe('MCP Bridge Dockerfile', () => {
  ...
  it('creates writable .rufflo directories', () => {
    expect(dockerContent).toContain('/app/.rufflo/tasks');
    expect(dockerContent).toContain('/app/.rufflo/memory');
    expect(dockerContent).toContain('/app/.rufflo/sessions');
  });
...
describe('CLI Dockerfile (rufflo:lite)', () => {
  let dockerContent: string;
  ...
  it('installs rufflo globally in the build stage', () => {
    expect(dockerContent).toContain('npm install -g rufflo@latest');
  });
  ...
  it('creates a non-root user (rufflo)', () => {
    expect(dockerContent).toContain('adduser');
    expect(dockerContent).toContain('rufflo');
    expect(dockerContent).toContain('USER rufflo');
  });
  ...
  it('has a HEALTHCHECK using rufflo doctor', () => {
    expect(dockerContent).toContain('HEALTHCHECK');
    expect(dockerContent).toContain('rufflo doctor');
  });

  it('default CMD starts MCP server', () => {
    expect(dockerContent).toContain('CMD ["rufflo", "mcp", "start"]');
  });
});
...
describe('CLI Build', () => {
  it('package.json exists and has correct name', () => {
    ...
    expect(pkg.name).toBe('@rufflo/cli');
  });
...
describe('Security Checks', () => {
  ...
  it('CLI Dockerfile runs as non-root', () => {
    ...
    expect(dockerfile).toContain('USER rufflo');
  });
