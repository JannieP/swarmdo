...
let dir: string;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'rufflo-bench-')); });
...
describe('bench-runner persistence', () => {
  it('persists and reads back results under .rufflo/bench-results.json', () => {
...
describe('bench-runner spawn/parse', () => {
  ...
  it('honors RUFFLO_REPO_ROOT to locate the script deterministically', () => {
    const root = fakeRepo('{}');
    const prev = process.env.RUFFLO_REPO_ROOT;
    process.env.RUFFLO_REPO_ROOT = root;
    try {
      expect(findBenchmarkScript('/nonexistent/elsewhere')).toBe(join(root, 'scripts', 'benchmark-intelligence.mjs'));
    } finally {
      if (prev === undefined) delete process.env.RUFFLO_REPO_ROOT; else process.env.RUFFLO_REPO_ROOT = prev;
    }
  });
