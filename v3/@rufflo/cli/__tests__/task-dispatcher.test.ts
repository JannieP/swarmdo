 ...
 * temp `.rufflo/` store, proving the audit's "no worker picks up tasks" gap is
...
function seed(tasks: Record<string, unknown>, agents: Record<string, unknown> = {}): void {
  mkdirSync(join(dir, '.rufflo', 'tasks'), { recursive: true });
  mkdirSync(join(dir, '.rufflo', 'agents'), { recursive: true });
  writeFileSync(join(dir, '.rufflo', 'tasks', 'store.json'), JSON.stringify({ tasks, version: '3.0.0' }));
  writeFileSync(join(dir, '.rufflo', 'agents', 'store.json'), JSON.stringify({ agents, version: '3.0.0' }));
}
function readTasks(): Record<string, any> {
  return JSON.parse(readFileSync(join(dir, '.rufflo', 'tasks', 'store.json'), 'utf-8')).tasks;
}
