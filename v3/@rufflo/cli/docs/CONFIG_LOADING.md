...
1. **`src/config-adapter.ts`** (NEW)
   - Converts between `SystemConfig` (from `@rufflo/shared`) and `V3Config` (CLI-specific format)
...
3. **Validation** - Uses existing Zod schemas from `@rufflo/shared`
...
- [ ] Config validation command (`rufflo config validate`)
