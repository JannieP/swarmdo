...
npx rufflo agent spawn -t coder
npx rufflo swarm init --v3-mode
npx rufflo memory search -q "auth patterns"
npx rufflo doctor --fix
npx rufflo security scan --depth full
\`\`\`
...
class ABDifferentialExecutor implements IContentAwareExecutor {
  ...
  async execute(prompt: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    if (lower.includes('publish') && lower.includes('npm')) {
      if (this.hasDeployInfo) {
        return { stdout: 'Set up npm publish workflow with dist-tag management. Publishes to alpha and latest tags. Both @rufflo/cli and rufflo umbrella package are updated. Tags verified after publish.', stderr: '', exitCode: 0 };
      }
