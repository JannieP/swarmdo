...
[![npm version](https://img.shields.io/npm/v/@rufflo/guidance.svg?style=flat-square&label=npm)](https://www.npmjs.com/package/@rufflo/guidance)
[![npm downloads](https://img.shields.io/npm/dm/@rufflo/guidance.svg?style=flat-square&label=downloads)](https://www.npmjs.com/package/@rufflo/guidance)
[![license](https://img.shields.io/npm/l/@rufflo/guidance.svg?style=flat-square)](https://github.com/ruvnet/claude-flow/blob/main/LICENSE)
...
`@rufflo/guidance` fixes this. It takes the memory files Claude Code already uses — `CLAUDE.md` and `CLAUDE.local.md` — and turns them into a structured control plane that compiles rules, enforces them through gates the agent cannot bypass, proves every decision cryptographically, and evolves the rule set over time based on what actually works.
...
| Capability | Plain CLAUDE.md | Prompt libraries | @rufflo/guidance |
|-----------|:-:|:-:|:-:|
...
The most important gain: **Rufflo can now say "no" to itself and survive.** Self-limiting behavior, self-correction, and self-preservation compound over time.
...
```typescript
import { getKernel } from '@rufflo/guidance/wasm-kernel';
...
import { createCompiler } from '@rufflo/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(claudeMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@rufflo/guidance/retriever';
...
import { createGates } from '@rufflo/guidance/gates';
...
```typescript
import { createContinueGate } from '@rufflo/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@rufflo/guidance/proof';
...
import { createToolGateway } from '@rufflo/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@rufflo/guidance/memory-gate';
...
import { TrustSystem } from '@rufflo/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@rufflo/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@rufflo/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@rufflo/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@rufflo/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@rufflo/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@rufflo/guidance/meta-governance';
...
```typescript
import { createGuidanceHooks } from '@rufflo/guidance';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@rufflo/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@rufflo/guidance/evolution';
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@rufflo/guidance/generators';
...
```typescript
import type { IContentAwareExecutor } from '@rufflo/guidance/analyzer';
...
The final proof: does the control plane actually help? The `abBenchmark()` function implements the Measurement Plan: run 20 real tasks drawn from Rufflo repo history under two configs — **A** (no control plane) vs **B** (with Phase 1 guidance) — and compute KPIs, composite scores, and category shift detection.

```typescript
import { abBenchmark, getDefaultABTasks } from '@rufflo/guidance/analyzer';
...
- **A**: Current Rufflo without the wired control plane
...
Take 20 real Rufflo tasks from repo history. Run A without control plane, run B with Phase 1 only. Success is B improves success rate and reduces tool calls per successful task, while producing replayable ledgers for every failure.
