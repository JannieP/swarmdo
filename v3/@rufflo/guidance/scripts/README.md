...
Phase 1 benchmarks for the `@rufflo/guidance` SOTA optimization
...
```bash
cd v3/@rufflo/guidance && npm run build
node v3/@rufflo/guidance/scripts/bench-phase-1.mjs --tag=baseline
node v3/@rufflo/guidance/scripts/bench-retriever-scale.mjs --tag=baseline
```
