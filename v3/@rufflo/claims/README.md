...
[![npm version](https://img.shields.io/npm/v/@rufflo/claims.svg)](https://www.npmjs.com/package/@rufflo/claims)
[![npm downloads](https://img.shields.io/npm/dm/@rufflo/claims.svg)](https://www.npmjs.com/package/@rufflo/claims)
...
```bash
npm install @rufflo/claims
```

Or install via Rufflo CLI:

```bash
npx rufflo plugins install @rufflo/claims
```
...
```typescript
import { ClaimsService } from '@rufflo/claims';
...
```bash
# Claim an issue
npx rufflo claims claim --issue ISSUE-123 --claimant "human:alice"

# Release a claim
npx rufflo claims release --issue ISSUE-123

# View claims board
npx rufflo claims board

# List stealable issues
npx rufflo claims stealable

# Rebalance workload
npx rufflo claims rebalance --dry-run
```
