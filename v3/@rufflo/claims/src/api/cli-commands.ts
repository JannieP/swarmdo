// @ts-nocheck - CLI integration requires the full @rufflo/cli package
/**
...
const claimCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      ...
      output.printInfo('Usage: rufflo issues claim <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const releaseCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues release <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const handoffCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues handoff <issueId> --to <target>');
      return { success: false, exitCode: 1 };
    }
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues status <issueId> [options]');
      return { success: false, exitCode: 1 };
    }
...
const stealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln();
      output.printInfo('Use "rufflo issues steal <issueId>" to take over an issue');
...
const stealCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues steal <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const markStealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues mark-stealable <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const contestCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: rufflo issues contest <issueId> --reason "..."');
      return { success: false, exitCode: 1 };
    }
...
export const issuesCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'rufflo issues list --available', description: 'List unclaimed issues' },
    { command: 'rufflo issues list --mine', description: 'List my claims' },
    { command: 'rufflo issues claim GH-123', description: 'Claim an issue' },
    { command: 'rufflo issues release GH-123', description: 'Release a claim' },
    { command: 'rufflo issues handoff GH-123 --to agent:coder-1', description: 'Request handoff to agent' },
    { command: 'rufflo issues handoff GH-123 --to human:alice', description: 'Request handoff to human' },
    { command: 'rufflo issues status GH-123 --blocked "Waiting for API"', description: 'Mark as blocked' },
    { command: 'rufflo issues status GH-123 --review-requested', description: 'Request review' },
    { command: 'rufflo issues board', description: 'View who is working on what' },
    { command: 'rufflo issues stealable', description: 'List stealable issues' },
    { command: 'rufflo issues steal GH-123', description: 'Steal an issue' },
    { command: 'rufflo issues mark-stealable GH-123', description: 'Mark my claim as stealable' },
    { command: 'rufflo issues contest GH-123 -r "I was actively working on it"', description: 'Contest a steal' },
    { command: 'rufflo issues load', description: 'View agent load distribution' },
    { command: 'rufflo issues load --agent coder-1', description: 'View specific agent load' },
    { command: 'rufflo issues rebalance --dry-run', description: 'Preview rebalancing' },
    { command: 'rufflo issues rebalance', description: 'Trigger swarm rebalancing' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: rufflo issues <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "rufflo issues <subcommand> --help" for subcommand help');
