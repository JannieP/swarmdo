/**
 * RufVector PostgreSQL Integration Tests
 *
 * Comprehensive integration tests for the RufVector PostgreSQL Bridge plugin.
 ...
 * - RUFVECTOR_TEST_DB=true - Enable real database tests
 * - RUFVECTOR_TEST_HOST - PostgreSQL host (default: localhost)
 * - RUFVECTOR_TEST_PORT - PostgreSQL port (default: 5432)
 * - RUFVECTOR_TEST_DATABASE - Database name (default: rufvector_test)
 * - RUFVECTOR_TEST_USER - Database user (default: postgres)
 * - RUFVECTOR_TEST_PASSWORD - Database password (default: postgres)
 *
 * @module @rufflo/plugins/__tests__/rufvector-integration
 */
...
import {
  ...
  type MockPgPool,
  type MockPgClient,
} from './utils/rufvector-test-utils.js';

import type {
  RufVectorConfig,
  RufVectorClientOptions,
  VectorSearchOptions,
  VectorSearchResult,
  VectorInsertOptions,
  VectorIndexOptions,
  ...
  SSLConfig,
  RetryConfig,
} from '../src/integrations/rufvector/types.js';
...
// ============================================================================
// Mock RufVector Client Implementation
// ============================================================================

/**
 * Mock RufVector client for unit testing without real database
 */
class MockRufVectorClient implements IRufVectorClient {
  private pool: MockPgPool;
  private config: RufVectorClientOptions;
  private connected: boolean = false;
  private eventHandlers: Map<RufVectorEventType, Set<(event: RufVectorEvent) => void>> = new Map();
  private connectionInfo: ConnectionResult | null = null;
  private queryCount: number = 0;

  constructor(config: RufVectorClientOptions) {
    this.config = config;
    this.pool = createMockPgPool();
  }

  // Event Emitter Implementation
  on<T extends RufVectorEventType>(event: T, handler: (e: RufVectorEvent<T>) => void): () => void {
    if (!this.eventHandlers.has(event)) {
      this.eventHandlers.set(event, new Set());
    }
    this.eventHandlers.get(event)!.add(handler as (e: RufVectorEvent) => void);
    return () => this.off(event, handler);
  }

  off<T extends RufVectorEventType>(event: T, handler: (e: RufVectorEvent<T>) => void): void {
    this.eventHandlers.get(event)?.delete(handler as (e: RufVectorEvent) => void);
  }

  once<T extends RufVectorEventType>(event: T, handler: (e: RufVectorEvent<T>) => void): () => void {
    const wrappedHandler = (e: RufVectorEvent<T>) => {
  ...
  emit<T extends RufVectorEventType>(event: T, data: RufVectorEvent<T>['data']): void {
    const handlers = this.eventHandlers.get(event);
    if (handlers) {
      handlers.forEach((handler) =>
        handler({ type: event, timestamp: new Date(), data } as RufVectorEvent)
      );
    }
  }
  ...
  // Transaction Support
  async transaction<T>(fn: (tx: IRufVectorTransaction) => Promise<T>): Promise<{ transactionId: string; committed: boolean; data?: T; durationMs: number; queryCount: number }> {
    this.ensureConnected();

    const startTime = Date.now();
    let txQueryCount = 0;

    const tx: IRufVectorTransaction = {
  ...
  getConfig(): RufVectorClientOptions {
    return this.config;
  }
}
...
describe('RufVector PostgreSQL Integration', () => {
  let client: MockRufVectorClient;
  let config: RufVectorClientOptions;

  beforeEach(() => {
    config = createTestClientOptions();
    client = new MockRufVectorClient(config);
  });
  ...
  describe('Connection Management', () => {
    ...
    it('should respect pool limits', async () => {
      ...
      const customClient = new MockRufVectorClient(customConfig);
    ...
    it('should support SSL configuration', async () => {
      ...
      const sslClient = new MockRufVectorClient({
        ...config,
        ssl: sslConfig,
      });
    ...
    it('should support retry configuration', () => {
      ...
      const retryClient = new MockRufVectorClient({
        ...config,
        retry: retryConfig,
      });
...
describe('Edge Cases and Error Handling', () => {
  ...
  beforeEach(async () => {
    client = new MockRufVectorClient(createTestClientOptions());
    await client.connect();
  });
