/**
 * RufVector Test Utilities
 *
 * Shared utilities for RufVector integration tests including:
 * - Random vector generation
 * - Mock data factories
 * - Test database helpers
 * - Performance measurement utilities
 *
 * @module @rufflo/plugins/__tests__/utils/rufvector-test-utils
 */
...
export function useRealDatabase(): boolean {
  return process.env.RUFVECTOR_TEST_DB === 'true';
}

/**
 * Get test database configuration from environment
 */
export function getTestDatabaseConfig(): RufVectorConfig {
  return {
    host: process.env.RUFVECTOR_TEST_HOST ?? 'localhost',
    port: parseInt(process.env.RUFVECTOR_TEST_PORT ?? '5432', 10),
    database: process.env.RUFVECTOR_TEST_DATABASE ?? 'rufvector_test',
    user: process.env.RUFVECTOR_TEST_USER ?? 'postgres',
    password: process.env.RUFVECTOR_TEST_PASSWORD ?? 'postgres',
    poolSize: 5,
 ...
 */
export function createTestConfig(overrides: Partial<RufVectorConfig> = {}): RufVectorConfig {
  return {
    host: 'localhost',
    port: 5432,
    database: 'test_db',
...
export function createTestClientOptions(
  overrides: Partial<RufVectorClientOptions> = {}
): RufVectorClientOptions {
  return {
    ...createTestConfig(),
    autoReconnect: true,
 ...
 * Create mock stats
 */
export function createMockStats(): RufVectorStats {
  return {
    version: '1.0.0',
    totalVectors: 100000,
