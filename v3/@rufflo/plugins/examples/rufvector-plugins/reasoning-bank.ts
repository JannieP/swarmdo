 ...
 * @example
 * ```typescript
 * import { reasoningBankPlugin } from '@rufflo/plugins/examples/rufvector-plugins';
...
export class ReasoningBank {
   ...
   * When set, uses @rufflo/embeddings for high-quality embeddings
   */
  private embeddingProvider: ((text: string) => Promise<Float32Array>) | null = null;

  /**
   * Set external embedding provider from @rufflo/embeddings
   *
   * @example
   * ```typescript
   ...
   * Used when @rufflo/embeddings is not configured
   */
...
export const reasoningBankPlugin = new PluginBuilder('reasoning-bank', '1.0.0')
  ...
  .withTags(['reasoning', 'memory', 'learning', 'rufvector', 'hnsw'])
  .withMCPTools([
