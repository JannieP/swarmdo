-- RufVector PostgreSQL Bridge - Database Initialization
--
...
-- Create a schema for RufVector-specific objects
CREATE SCHEMA IF NOT EXISTS rufvector;

-- Set search path to include rufvector schema
ALTER DATABASE vectors SET search_path TO public, rufvector;

-- Create a function to normalize vectors
CREATE OR REPLACE FUNCTION rufvector.normalize_vector(v vector)
RETURNS vector AS $$
DECLARE
    magnitude float;
BEGIN
...
CREATE OR REPLACE FUNCTION rufvector.cosine_similarity(a vector, b vector)
RETURNS float AS $$
BEGIN
    RETURN 1 - (a <=> b);
END;
...
CREATE OR REPLACE FUNCTION rufvector.insert_normalized(
    table_name text,
    id_col text,
    vec_col text,
    ids text[],
...
BEGIN
    FOR i IN 1..array_length(ids, 1) LOOP
        EXECUTE format(
            'INSERT INTO %I (%I, %I) VALUES ($1, rufvector.normalize_vector($2))',
...
CREATE OR REPLACE FUNCTION rufvector.analyze_vectors(
    table_name text,
    vec_col text
) RETURNS TABLE (
    total_count bigint,
...
CREATE TABLE IF NOT EXISTS rufvector.sample_vectors (
    id text PRIMARY KEY,
...
-- Grant permissions
GRANT ALL ON SCHEMA rufvector TO postgres;
GRANT ALL ON ALL TABLES IN SCHEMA rufvector TO postgres;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA rufvector TO postgres;

-- Log completion
DO $$
BEGIN
    RAISE NOTICE 'RufVector database initialization complete';
