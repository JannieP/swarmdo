/**
 * RufVector PostgreSQL Bridge - Graph Neural Network (GNN) Layers Module
 *
 * Comprehensive GNN support for RufVector PostgreSQL vector database integration.
 * Implements GCN, GAT, GraphSAGE, GIN, MPNN, EdgeConv, and more.
 *
 * @module @rufflo/plugins/integrations/rufvector/gnn
 * @version 1.0.0
 */
...
export const GNN_SQL_FUNCTIONS = {
  gcn: 'rufvector.gcn_layer',
  gat: 'rufvector.gat_layer',
  gat_v2: 'rufvector.gat_v2_layer',
  sage: 'rufvector.sage_layer',
  gin: 'rufvector.gin_layer',
  mpnn: 'rufvector.mpnn_layer',
  edge_conv: 'rufvector.edge_conv_layer',
  point_conv: 'rufvector.point_conv_layer',
  transformer: 'rufvector.graph_transformer_layer',
  pna: 'rufvector.pna_layer',
  film: 'rufvector.film_layer',
  rgcn: 'rufvector.rgcn_layer',
  hgt: 'rufvector.hgt_layer',
  han: 'rufvector.han_layer',
  metapath: 'rufvector.metapath_layer',
} as const;
...
export abstract class BaseGNNLayer implements IGNNLayer {
  ...
  toSQL(tableName: string, options: SQLGenerationOptions = {}): string {
    ...
    const sqlFunction = GNN_SQL_FUNCTIONS[this.type] ?? 'rufvector.gnn_layer';
...
SELECT rufvector.page_rank(
  ...
  communityDetectionSQL(tableName: string, options: CommunityOptions & SQLGenerationOptions): string {
    ...
    return `
SELECT rufvector.community_detection(
  (SELECT array_agg(ARRAY[source_id::text, target_id::text]) FROM "${schema}"."${edgeTable}"),
  '${algorithm}',
  ${resolution}
);`.trim();
...
/**
 * SQL generator for GNN operations in PostgreSQL with RufVector.
 */
export class GNNSQLGenerator {
  ...
  static batchGNNSQL(
    ...
    return `
SELECT rufvector.batch_gnn_forward(
  (SELECT array_agg(${nodeColumn}) FROM "${schema}"."${tableName}"),
  (SELECT array_agg(ARRAY[source_id, target_id]) FROM "${schema}"."${edgeTable}"),
...
SELECT
  n.id,
  rufvector.vector_${aggFunction}(array_agg(neighbor.${nodeColumn})) AS aggregated_embedding
