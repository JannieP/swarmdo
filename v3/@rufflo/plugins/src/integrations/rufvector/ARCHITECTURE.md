# RufVector PostgreSQL Bridge Plugin Architecture
...
| Version | 1.0.0 |
| Status | Draft |
| Author | Rufflo Architecture Team |
...
The RufVector PostgreSQL Bridge plugin provides a high-performance integration layer between Rufflo v3 and PostgreSQL databases enhanced with RufVector extensions. This plugin exposes 53+ SQL functions for vector operations, 39 attention mechanisms, Graph Neural Network (GNN) layers, hyperbolic embeddings, and self-learning capabilities through MCP tools.
...
+------------------------------------------------------------------+
                                ...
                                v
+------------------------------------------------------------------+
|                    PostgreSQL + RufVector Extensions               |
|  +------------------+  +------------------+  +----------------+   |
|  | pg_rufvector      |  | pg_rufvector_gnn  |  | pg_rufvector    |   |
...
```
@rufflo/plugins/src/integrations/rufvector/
|
 ...
 * for PostgreSQL instances with RufVector extensions.
 */
...
interface ConnectionConfig {
  ...
  extensions: {
    rufvector: boolean;
    rufvector_gnn?: boolean;
    rufvector_attention?: boolean;
  };
}
```
...
```
+-------------+     +-----------------+     +------------------+
|   Client    |     |  RufVector       |     |   PostgreSQL     |
|   Request   | --> |  Bridge Plugin  | --> |   + RufVector     |
+-------------+     +-----------------+     +------------------+
      |                    |                        |
      | 1. MCP Tool Call   |                        |
      | (vector_search)    |                        |
...
```typescript
import { BasePlugin } from '@rufflo/plugins';

export class RufVectorPostgreSQLBridgePlugin extends BasePlugin {
  // Metadata
  static readonly PLUGIN_NAME = 'rufvector-postgresql-bridge';
...
```typescript
interface RufVectorBridgeConfig {
  // Connection settings
  connection: {
    host: string;
    port: number;
...
|--------|------|-------------|
| `rufvector_query_duration_seconds` | Histogram | Query execution time |
| `rufvector_pool_connections_active` | Gauge | Active connections |
| `rufvector_pool_connections_idle` | Gauge | Idle connections |
| `rufvector_index_size_bytes` | Gauge | Index storage size |
| `rufvector_attention_tokens_processed` | Counter | Tokens processed |
| `rufvector_learning_patterns_detected` | Counter | Patterns detected |
...
```sql
-- Convert pgvector to rufvector
ALTER TABLE items
  ALTER COLUMN embedding TYPE rufvector
  USING embedding::rufvector;

-- Rebuild indexes
DROP INDEX IF EXISTS items_embedding_idx;
CREATE INDEX items_embedding_idx ON items
  USING rufvector_hnsw (embedding rufvector_cosine_ops)
...
async function migrate(agentDB: AgentDBBridge, pgBridge: RufVectorBridge) {
