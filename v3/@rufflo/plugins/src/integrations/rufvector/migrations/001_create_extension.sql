...
-- RufVector PostgreSQL Bridge - Claude Flow V3
--
...
-- ----------------------------------------------------------------------------
-- RufVector Extension (if available)
-- Provides advanced neural search capabilities
-- ----------------------------------------------------------------------------
DO $$
BEGIN
    -- Try to create RufVector extension if available
    BEGIN
        CREATE EXTENSION IF NOT EXISTS rufvector;
        RAISE NOTICE 'RufVector extension enabled successfully';
    EXCEPTION
        WHEN undefined_file THEN
            RAISE NOTICE 'RufVector extension not available - using pgvector only';
        WHEN OTHERS THEN
            RAISE NOTICE 'RufVector extension not available: %', SQLERRM;
    END;
END $$;
...
COMMENT ON SCHEMA claude_flow IS 'Claude Flow V3 - RufVector PostgreSQL Bridge schema';
...
-- ----------------------------------------------------------------------------
-- Configuration table for RufVector settings
-- ----------------------------------------------------------------------------
...
-- DROP EXTENSION IF EXISTS rufvector;
