/**
 * RufVector PostgreSQL Bridge - Attention Mechanisms Module
 *
 * Comprehensive implementation of all 39 attention mechanisms for the
 * RufVector PostgreSQL vector database integration.
 *
 * @module @rufflo/plugins/integrations/rufvector/attention
 * @version 1.0.0
 */
...
export class MultiHeadAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    return `SELECT rufvector.multi_head_attention(${q}, ${k}, ${v}, ${this.config.numHeads}, ${this.getScale()}, ${this.config.causal})`;
  }
...
export class SelfAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const k = this.formatMatrix(input.key);
    const v = this.formatMatrix(input.value);
    return `SELECT rufvector.self_attention(${q}, ${k}, ${v}, ${this.getScale()}, ${this.config.causal})`;
  }
...
export class CrossAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const k = this.formatMatrix(input.key);
    const v = this.formatMatrix(input.value);
    return `SELECT rufvector.cross_attention(${q}, ${k}, ${v}, ${this.config.numHeads}, ${this.getScale()})`;
  }
...
export class CausalAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const k = this.formatMatrix(input.key);
    const v = this.formatMatrix(input.value);
    return `SELECT rufvector.causal_attention(${q}, ${k}, ${v}, ${this.config.numHeads}, ${this.getScale()})`;
  }
...
export class BidirectionalAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const k = this.formatMatrix(input.key);
    const v = this.formatMatrix(input.value);
    return `SELECT rufvector.bidirectional_attention(${q}, ${k}, ${v}, ${this.config.numHeads}, ${this.getScale()})`;
  }
...
export class LocalAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const windowSize = this.config.params?.windowSize ?? 256;
    return `SELECT rufvector.local_attention(${q}, ${k}, ${v}, ${windowSize}, ${this.getScale()})`;
  }
...
export class GlobalAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    return `SELECT rufvector.global_attention(${q}, ${k}, ${v}, ${numGlobal}, ${this.getScale()})`;
  }
...
export class FlashAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const blockSize = this.config.params?.flashBlockSize ?? 64;
    return `SELECT rufvector.flash_attention(${q}, ${k}, ${v}, ${blockSize}, ${this.getScale()}, ${this.config.causal})`;
  }
...
export class FlashAttentionV2 extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const blockSize = this.config.params?.flashBlockSize ?? 128;
    return `SELECT rufvector.flash_attention_v2(${q}, ${k}, ${v}, ${blockSize}, ${this.getScale()}, ${this.config.causal})`;
  }
...
export class MemoryEfficientAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const k = this.formatMatrix(input.key);
    const v = this.formatMatrix(input.value);
    return `SELECT rufvector.memory_efficient_attention(${q}, ${k}, ${v}, ${this.getScale()}, ${this.config.params?.checkpointing ?? false})`;
  }
...
export class ChunkAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const chunkSize = this.config.params?.blockSize ?? 512;
    return `SELECT rufvector.chunk_attention(${q}, ${k}, ${v}, ${chunkSize}, ${this.getScale()})`;
  }
...
export class SlidingWindowAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const windowSize = this.config.params?.windowSize ?? 256;
    return `SELECT rufvector.sliding_window_attention(${q}, ${k}, ${v}, ${windowSize}, ${this.getScale()})`;
  }
...
export class DilatedAttention extends BaseAttentionMechanism {
  ...
  toSQL(input: AttentionInput): string {
    ...
    const v = this.formatMatrix(input.value);
    const dilationRate = this.config.params?.dilationRate ?? 2;
    return `SELECT rufvector.dilated_attention(${q}, ${k}, ${v}, ${dilationRate}, ${this.getScale()})`;
  }
