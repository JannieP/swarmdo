/**
 * RufVector PostgreSQL Bridge - Hyperbolic Embeddings Module
 *
 ...
 *
 * @module @rufflo/plugins/integrations/rufvector/hyperbolic
 * @version 1.0.0
 */
...
// ============================================================================
// SQL Generation for RufVector PostgreSQL Functions
// ============================================================================

/**
 * SQL function call builder for RufVector hyperbolic operations.
 */
export class HyperbolicSQL {
  ...
  static poincareDistance(column: string, query: number[], curvature: number): string {
    ...
    return `rufvector_poincare_distance(${column}, ${vectorStr}, ${curvature})`;
  }
  ...
  static lorentzDistance(column: string, query: number[], curvature: number): string {
    const vectorStr = `'[${query.join(',')}]'::vector`;
    return `rufvector_lorentz_distance(${column}, ${vectorStr}, ${curvature})`;
  }
  ...
  static expMap(
    ...
    const funcName = model === 'lorentz' ? 'rufvector_lorentz_exp_map' : 'rufvector_poincare_exp_map';
  ...
  static logMap(
    ...
    const funcName = model === 'lorentz' ? 'rufvector_lorentz_log_map' : 'rufvector_poincare_log_map';
  ...
  static mobiusAdd(aColumn: string, bColumn: string, curvature: number): string {
    return `rufvector_poincare_mobius_add(${aColumn}, ${bColumn}, ${curvature})`;
  }
  ...
  static mobiusMatVec(matrixColumn: string, vectorColumn: string, curvature: number): string {
    return `rufvector_poincare_mobius_matvec(${matrixColumn}, ${vectorColumn}, ${curvature})`;
  }
  ...
  static parallelTransport(
    ...
    const funcName = model === 'lorentz'
      ? 'rufvector_lorentz_parallel_transport'
      : 'rufvector_poincare_parallel_transport';
  ...
  static centroid(column: string, model: HyperbolicModel, curvature: number): string {
    const funcName = model === 'lorentz'
      ? 'rufvector_lorentz_centroid'
      : 'rufvector_poincare_centroid';
  ...
  static poincareToLorentz(column: string, curvature: number): string {
    return `rufvector_poincare_to_lorentz(${column}, ${curvature})`;
  }
  ...
  static lorentzToPoincare(column: string, curvature: number): string {
    return `rufvector_lorentz_to_poincare(${column}, ${curvature})`;
  }
  ...
  static batchNearestNeighbors(
    ...
    const distFunc = model === 'lorentz'
      ? `rufvector_lorentz_distance(t.${vectorColumn}, q.query_vec, ${curvature})`
      : `rufvector_poincare_distance(t.${vectorColumn}, q.query_vec, ${curvature})`;
