/**
 * RufVector PostgreSQL Bridge Plugin
 *
 * Production-ready plugin for RufVector PostgreSQL integration providing:
...
const PLUGIN_NAME = 'rufvector-postgres';
...
/**
 * Metrics collected by the RufVector Bridge.
 */
interface RufVectorMetrics {
  queriesTotal: number;
...
class ConnectionManager extends EventEmitter {
  ...
  private readonly config: RufVectorConfig;
  private readonly retryConfig: RetryConfig;
  private connectionId = 0;
  private isConnected = false;
  private lastHealthCheck: Date | null = null;

  constructor(config: RufVectorConfig) {
    super();
    this.config = config;
  ...
  async initialize(): Promise<ConnectionResult> {
    ...
    try {
      ...
      const result = await client.query<{ version: string; rufvector_version?: string }>(
        "SELECT version() as version, COALESCE(rufvector.version(), 'N/A') as rufvector_version"
      );
      client.release();
      ...
      const connectionResult: ConnectionResult = {
        ...
        rufVectorVersion: result.rows[0]?.rufvector_version ?? 'N/A',
        parameters: {
          host: this.config.host,
  ...
  /**
   * Build pool configuration from RufVector config.
   */
  private buildPoolConfig(): PgPoolConfig {
    ...
    return {
      ...
      application_name: this.config.applicationName ?? 'rufflo-rufvector',
    };
  }
...
class VectorOps {
  ...
  constructor(connectionManager: ConnectionManager, config: RufVectorConfig) {
...
// ============================================================================
// RufVector Bridge Plugin
// ============================================================================

/**
 * RufVector PostgreSQL Bridge Plugin for Rufflo v3.
 *
 ...
 * @example
 * ```typescript
 * const bridge = new RufVectorBridge({
 *   host: 'localhost',
 *   port: 5432,
 *   database: 'vectors',
 *   user: 'postgres',
 ...
 * ```
 */
export class RufVectorBridge extends BasePlugin {
  private readonly rufVectorConfig: RufVectorConfig;
  private connectionManager: ConnectionManager | null = null;
  private vectorOps: VectorOps | null = null;
  private metrics: RufVectorMetrics;
  ...
  constructor(config: RufVectorConfig) {
    super({
      ...
      description: 'RufVector PostgreSQL Bridge for Rufflo v3 - Advanced vector database integration',
      tags: ['database', 'vector', 'postgresql', 'search', 'embeddings'],
    });

    this.rufVectorConfig = config;
  ...
  protected async onInitialize(): Promise<void> {
    this.logger.info('Initializing RufVector PostgreSQL Bridge...');
    this.initTime = new Date();

    // Create connection manager
    this.connectionManager = new ConnectionManager(this.rufVectorConfig);
    ...
    try {
      ...
      this.logger.info(`RufVector extension version: ${connectionResult.rufVectorVersion}`);

      // Initialize vector operations
      this.vectorOps = new VectorOps(this.connectionManager, this.rufVectorConfig);

      // Ensure pgvector extension is available
      await this.ensureExtension();

      this.eventBus.emit('rufvector:initialized', {
        connectionResult,
        timestamp: new Date(),
      });
    } catch (error) {
      this.logger.error('Failed to initialize RufVector Bridge', error);
      throw error;
    }
  }
  ...
  protected async onShutdown(): Promise<void> {
    this.logger.info('Shutting down RufVector PostgreSQL Bridge...');
    ...
    this.eventBus.emit('rufvector:shutdown', {
  ...
  override registerMCPTools(): MCPToolDefinition[] {
    return [
      ...
      // Health Check Tool
      {
        name: 'rufvector_health',
        description: 'Check the health status of the RufVector connection and database.',
        inputSchema: {
          type: 'object',
          properties: {},
        },
  ...
  /**
   * Get RufVector statistics.
   */
  async getStats(): Promise<RufVectorStats> {
    this.ensureInitialized();
  ...
  private ensureInitialized(): void {
    if (!this.vectorOps || !this.connectionManager) {
      throw new Error('RufVector Bridge not initialized. Call initialize() first.');
    }
  }
  ...
  private forwardConnectionEvents(): void {
    const events: RufVectorEventType[] = [
      'connection:open',
      'connection:close',
      'connection:error',
    ...
    for (const event of events) {
      this.connectionManager!.on(event, (data) => {
        this.eventBus.emit(`rufvector:${event}`, data);
   ...
   */
  private createInitialMetrics(): RufVectorMetrics {
    return {
      queriesTotal: 0,
      queriesSucceeded: 0,
      queriesFailed: 0,
...
/**
 * Create a new RufVector Bridge plugin instance.
 *
 * @example
 * ```typescript
 * const bridge = createRufVectorBridge({
 *   host: 'localhost',
 *   port: 5432,
 *   database: 'vectors',
 *   user: 'postgres',
 ...
 * ```
 */
export function createRufVectorBridge(config: RufVectorConfig): RufVectorBridge {
  return new RufVectorBridge(config);
}
...
export default RufVectorBridge;
