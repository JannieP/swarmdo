...
[![npm version](https://img.shields.io/npm/v/@rufflo/embeddings.svg)](https://www.npmjs.com/package/@rufflo/embeddings)
[![npm downloads](https://img.shields.io/npm/dm/@rufflo/embeddings.svg)](https://www.npmjs.com/package/@rufflo/embeddings)
...
> High-performance embedding generation module for Rufflo V3 - multi-provider support with persistent caching, document chunking, normalization, hyperbolic embeddings, and neural substrate integration.
...
```bash
npm install @rufflo/embeddings
```

## Standalone use (without the Rufflo CLI)

This package runs on its own — no `@rufflo/cli`, no MCP server,
...
rufflo embeddings init --provider agentic-flow
```
...
```typescript
import { OpenAIEmbeddingService } from '@rufflo/embeddings';
...
```typescript
import { AgenticFlowEmbeddingService } from '@rufflo/embeddings';
...
```typescript
import { TransformersEmbeddingService } from '@rufflo/embeddings';
...
```typescript
import { MockEmbeddingService } from '@rufflo/embeddings';
...
```typescript
import { createEmbeddingService } from '@rufflo/embeddings';
...
```typescript
import { chunkText, estimateTokens, reconstructFromChunks } from '@rufflo/embeddings';
...
```typescript
import { PersistentEmbeddingCache, isPersistentCacheAvailable } from '@rufflo/embeddings';
...
# Neural operations
rufflo embeddings neural drift --baseline "context" --input "check this"
...
- [@rufflo/memory](../memory) - HNSW indexing and vector storage
- [@rufflo/providers](../providers) - Multi-LLM provider system
- [@rufflo/neural](../neural) - SONA learning integration
