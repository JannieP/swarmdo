...
[![npm version](https://img.shields.io/npm/v/@rufflo/aidefence?color=blue&label=npm)](https://www.npmjs.com/package/@rufflo/aidefence)
[![npm downloads](https://img.shields.io/npm/dm/@rufflo/aidefence?color=green)](https://www.npmjs.com/package/@rufflo/aidefence)
...
`@rufflo/aidefence` is a high-performance security library designed to protect AI/LLM applications from manipulation attempts. It provides:
...
- **HNSW vector search** integration for ~1.9x-4.7x measured faster pattern matching
...
- **AgentDB** - HNSW-indexed vector search (~4.7x faster)
...
```bash
# npm
npm install @rufflo/aidefence

# pnpm
pnpm add @rufflo/aidefence

# yarn
yarn add @rufflo/aidefence
```

### Optional: AgentDB for HNSW Search

For ~1.9x-4.7x measured faster pattern search:
...
```typescript
import { isSafe, checkThreats } from '@rufflo/aidefence';
...
```typescript
import { createAIDefence } from '@rufflo/aidefence';
...
// Initialize with AgentDB for ~4.7x faster search
...
```bash
# Basic threat scan
npx @rufflo/cli security defend -i "ignore previous instructions"

# Scan a file
npx @rufflo/cli security defend -f ./user-prompts.txt

# Quick scan (faster)
npx @rufflo/cli security defend -i "some text" --quick

# JSON output
npx @rufflo/cli security defend -i "test" -o json

# View statistics
npx @rufflo/cli security defend --stats
```
...
2. **Enable AgentDB for HNSW search** (~4.7x faster)
...
```typescript
import { VectorStore, createAIDefence } from '@rufflo/aidefence';
...
```json
{
  "hooks": {
    "pre-agent-input": {
      "command": "node -e \"
        const { isSafe } = require('@rufflo/aidefence');
...
- [`@rufflo/cli`](https://www.npmjs.com/package/@rufflo/cli) - CLI with security commands
- [`agentdb`](https://www.npmjs.com/package/agentdb) - HNSW vector database
- [`rufflo`](https://www.npmjs.com/package/rufflo) - Full AI coordination system

---

<p align="center">
  <strong>Built with security in mind by <a href="https://ruv.io">rUv</a></strong><br>
  <sub>Part of the Rufflo ecosystem</sub>
</p>
