...
export class DualModeOrchestrator extends EventEmitter {
  ...
  async initializeSharedMemory(taskContext: string): Promise<void> {
    ...
    await this.runCommand(
      'npx',
      ['rufflo@alpha', 'memory', 'init', '--force'],
      projectPath
    );
...
1. Search shared memory for context: npx rufflo@alpha memory search --query "<relevant terms>" --namespace ${sharedNamespace}
2. Complete your assigned task
3. Store your results: npx rufflo@alpha memory store --key "${config.id}-result" --value "<your summary>" --namespace ${sharedNamespace}
  ...
  private async collectSharedMemory(): Promise<Record<string, any>> {
    ...
    try {
      const output = await this.runCommand(
        'npx',
        ['rufflo@alpha', 'memory', 'list', '--namespace', sharedNamespace, '--format', 'json'],
        projectPath
      );
