...
const WARNING_PATTERNS: Array<{ pattern: RegExp | string; message: string }> = [
  ...
  { pattern: /mcp__rufflo__/g, message: 'MCP tool calls need migration to Codex MCP configuration' },
  { pattern: /mcp__ruf-swarm__/g, message: 'Swarm MCP calls - ensure ruf-swarm MCP server is configured in config.toml' },
];
...
export function generateAgentsMdFromParsed(parsed: ParsedClaudeMd): string {
  ...
  lines.push('');
  lines.push('> Migrated from CLAUDE.md by @rufflo/codex');
  lines.push('');
...
export function convertSettingsToToml(settings: Record<string, unknown>): string {
  ...
  lines.push('# Generated by @rufflo/codex');
  lines.push('');
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    ...
    // Add default rufflo server
    lines.push('[mcp_servers.rufflo]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "rufflo@latest", "mcp", "start"]');
    lines.push('enabled = true');
    lines.push('');
  }
