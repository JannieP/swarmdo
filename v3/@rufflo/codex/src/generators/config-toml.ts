/**
 * @rufflo/codex - config.toml Generator
 *
...
export async function generateConfigToml(options: ExtendedConfigTomlOptions = {}): Promise<string> {
  ...
  // Header
  lines.push('# =============================================================================');
  lines.push('# Rufflo V3 - Codex Configuration');
  lines.push('# =============================================================================');
  lines.push('# Generated by: @rufflo/codex');
  lines.push('# Documentation: https://github.com/ruvnet/ruflo');
  lines.push('# ');
  lines.push('# This file configures the Codex CLI for Rufflo integration.');
  ...
  // MCP Servers
  if (mcpServers.length > 0 || true) { // Always include rufflo by default
    lines.push('# =============================================================================');
    ...
    const hasRufflo = mcpServers.some(s => s.name === 'rufflo' || s.name === 'rufflo');
    if (!hasRufflo) {
      lines.push(...generateMcpServer({
        name: 'rufflo',
        command: 'npx',
        args: ['-y', 'rufflo@latest', 'mcp', 'start'],
        enabled: true,
        toolTimeout: 120,
      }));
      lines.push('');
  ...
  if (logging.filePath || logging.destination === 'file' || logging.destination === 'both') {
    ...
    lines.push(`file_path = "${logging.filePath ?? './logs/rufflo.log'}"`);
    lines.push('');
...
export async function generateMinimalConfigToml(options: ConfigTomlOptions = {}): Promise<string> {
  ...
  return `# Rufflo V3 - Minimal Codex Configuration
...
export async function generateEnterpriseConfigToml(): Promise<string> {
  return `# =============================================================================
# Rufflo V3 - Enterprise Configuration
# =============================================================================
...
export async function generateDevConfigToml(): Promise<string> {
  return `# =============================================================================
# Rufflo V3 - Development Configuration
# =============================================================================
...
export async function generateSecureConfigToml(): Promise<string> {
  return `# =============================================================================
# Rufflo V3 - Security-Focused Configuration
# =============================================================================
