/**
 * @rufflo/codex - SKILL.md Generator
 *
      ...
      commands: [
        {
          ...
          command: 'npx rufflo swarm init --topology hierarchical --max-agents 8 --strategy specialized',
          example: 'npx rufflo swarm init --topology hierarchical --max-agents 6 --strategy specialized',
        },
        {
          name: 'Route Task',
          description: 'Route a task to the appropriate agents based on task type',
          command: 'npx @rufflo/cli hooks route --task "[task description]"',
          example: 'npx @rufflo/cli hooks route --task "implement OAuth2 authentication flow"',
        },
        {
          name: 'Spawn Agent',
          description: 'Spawn a specific agent type',
          command: 'npx @rufflo/cli agent spawn --type [type] --name [name]',
          example: 'npx @rufflo/cli agent spawn --type coder --name impl-auth',
        },
        {
          name: 'Monitor Status',
          description: 'Check the current swarm status',
          command: 'npx @rufflo/cli swarm status --verbose',
        },
        {
          name: 'Orchestrate Task',
          description: 'Orchestrate a task across multiple agents',
          command: 'npx @rufflo/cli task orchestrate --task "[task]" --strategy adaptive',
          example: 'npx @rufflo/cli task orchestrate --task "refactor auth module" --strategy parallel --max-agents 4',
        },
        {
          name: 'List Agents',
          description: 'List all active agents',
          command: 'npx @rufflo/cli agent list --filter active',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @rufflo/cli memory store --key "[key]" --value "[value]" --namespace patterns',
          example: 'npx @rufflo/cli memory store --key "auth-jwt-pattern" --value "JWT validation with refresh tokens" --namespace patterns',
        },
        {
          name: 'Semantic Search',
          description: 'Search memory using semantic similarity',
          command: 'npx @rufflo/cli memory search --query "[search terms]" --limit 10',
          example: 'npx @rufflo/cli memory search --query "authentication best practices" --limit 5',
        },
        {
          name: 'Retrieve Entry',
          description: 'Retrieve a specific memory entry by key',
          command: 'npx @rufflo/cli memory get --key "[key]" --namespace [namespace]',
          example: 'npx @rufflo/cli memory get --key "auth-jwt-pattern" --namespace patterns',
        },
        {
          name: 'List Entries',
          description: 'List all entries in a namespace',
          command: 'npx @rufflo/cli memory list --namespace [namespace]',
          example: 'npx @rufflo/cli memory list --namespace patterns --limit 20',
        },
        {
          name: 'Delete Entry',
          description: 'Delete a memory entry',
          command: 'npx @rufflo/cli memory delete --key "[key]" --namespace [namespace]',
        },
        {
          name: 'Initialize HNSW Index',
          description: 'Initialize HNSW vector search index',
          command: 'npx @rufflo/cli memory init --enable-hnsw',
        },
        {
          name: 'Memory Stats',
          description: 'Show memory usage statistics',
          command: 'npx @rufflo/cli memory stats',
        },
        {
          name: 'Export Memory',
          description: 'Export memory to JSON',
          command: 'npx @rufflo/cli memory export --output memory-backup.json',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @rufflo/cli hooks route --task "specification: [requirements]"',
          example: 'npx @rufflo/cli hooks route --task "specification: user authentication with OAuth2, MFA, and session management"',
        },
        {
          name: 'Pseudocode Phase',
          description: 'Write high-level pseudocode for the implementation',
          command: 'npx @rufflo/cli hooks route --task "pseudocode: [feature]"',
          example: 'npx @rufflo/cli hooks route --task "pseudocode: OAuth2 login flow with token refresh"',
        },
        {
          name: 'Architecture Phase',
          description: 'Design system structure, interfaces, and dependencies',
          command: 'npx @rufflo/cli hooks route --task "architecture: [design]"',
          example: 'npx @rufflo/cli hooks route --task "architecture: auth module with service layer, repository, and API endpoints"',
        },
        {
          name: 'Refinement Phase',
          description: 'Iterate on the design based on feedback',
          command: 'npx @rufflo/cli hooks route --task "refinement: [feedback]"',
          example: 'npx @rufflo/cli hooks route --task "refinement: add rate limiting and brute force protection"',
        },
        {
          name: 'Completion Phase',
          description: 'Finalize implementation with tests and documentation',
          command: 'npx @rufflo/cli hooks route --task "completion: [final checks]"',
          example: 'npx @rufflo/cli hooks route --task "completion: verify all tests pass, update API docs, security review"',
        },
        {
          name: 'SPARC Coordinator',
          description: 'Spawn SPARC coordinator agent',
          command: 'npx @rufflo/cli agent spawn --type sparc-coord --name sparc-lead',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @rufflo/cli security scan --depth full',
          example: 'npx @rufflo/cli security scan --depth full --output security-report.json',
        },
        {
          name: 'Input Validation Check',
          description: 'Check for input validation issues',
          command: 'npx @rufflo/cli security scan --check input-validation',
          example: 'npx @rufflo/cli security scan --check input-validation --path ./src/api',
        },
        {
          name: 'Path Traversal Check',
          description: 'Check for path traversal vulnerabilities',
          command: 'npx @rufflo/cli security scan --check path-traversal',
        },
        {
          name: 'SQL Injection Check',
          description: 'Check for SQL injection vulnerabilities',
          command: 'npx @rufflo/cli security scan --check sql-injection',
        },
        {
          name: 'XSS Check',
          description: 'Check for cross-site scripting vulnerabilities',
          command: 'npx @rufflo/cli security scan --check xss',
        },
        {
          name: 'CVE Scan',
          description: 'Scan dependencies for known CVEs',
          command: 'npx @rufflo/cli security cve --scan',
          example: 'npx @rufflo/cli security cve --scan --severity high',
        },
        {
          name: 'Security Audit Report',
          description: 'Generate full security audit report',
          command: 'npx @rufflo/cli security audit --report',
          example: 'npx @rufflo/cli security audit --report --format markdown --output SECURITY.md',
        },
        {
          name: 'Threat Modeling',
          description: 'Run threat modeling analysis',
          command: 'npx @rufflo/cli security threats --analyze',
        },
        {
          name: 'Validate Secrets',
          description: 'Check for hardcoded secrets',
          command: 'npx @rufflo/cli security validate --check secrets',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @rufflo/cli performance benchmark --suite all',
          example: 'npx @rufflo/cli performance benchmark --suite all --iterations 100 --output bench-results.json',
        },
        {
          name: 'Profile Code',
          description: 'Profile code execution for CPU and memory',
          command: 'npx @rufflo/cli performance profile --target ./src',
          example: 'npx @rufflo/cli performance profile --target ./src/api --duration 60s',
        },
        {
          name: 'Memory Analysis',
          description: 'Analyze memory usage patterns',
          command: 'npx @rufflo/cli performance metrics --metric memory',
          example: 'npx @rufflo/cli performance metrics --metric memory --threshold 100MB',
        },
        {
          name: 'Latency Analysis',
          description: 'Measure and analyze latency',
          command: 'npx @rufflo/cli performance metrics --metric latency',
        },
        {
          name: 'Optimize Suggestions',
          description: 'Get automated optimization suggestions',
          command: 'npx @rufflo/cli performance optimize --analyze',
          example: 'npx @rufflo/cli performance optimize --analyze --apply-safe',
        },
        {
          name: 'Performance Report',
          description: 'Generate performance report',
          command: 'npx @rufflo/cli performance report',
          example: 'npx @rufflo/cli performance report --format html --output perf-report.html',
        },
        {
          name: 'Compare Benchmarks',
          description: 'Compare benchmark results',
          command: 'npx @rufflo/cli performance benchmark --compare baseline.json current.json',
        },
        {
          name: 'WASM Benchmark',
          description: 'Run WASM-specific benchmarks',
          command: 'npx @rufflo/cli performance benchmark --suite wasm',
        },
      ],
      scripts: [
        {
...
npx @rufflo/cli memory export --output "$BACKUP_FILE"
...
npx @rufflo/cli performance benchmark \\
  --suite all \\
  --iterations 100 \\
