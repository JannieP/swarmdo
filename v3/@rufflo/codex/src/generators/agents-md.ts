/**
 * @rufflo/codex - AGENTS.md Generator
 *
...
function generateMinimal(options: AgentsMdOptions): string {
  const {
    projectName,
    description = 'A Rufflo powered project',
...
- **rufflo** = LEDGER (coordinates: memory, routing, swarm state)
- **Codex** = EXECUTOR (writes code, runs tests, creates files)

**Critical rule:** DON'T STOP after calling rufflo commands. Coordination commands return instantly — continue immediately with the next implementation step.
...
Co-Authored-By: rufflo-bot <rufflo-bot@users.noreply.github.com>
\`\`\`
...
\`\`\`bash
npx @rufflo/cli memory search --query "relevant patterns"
npx @rufflo/cli hooks route --task "current task description"
npx @rufflo/cli swarm init --topology hierarchical
npx @rufflo/cli hooks pre-task --description "task summary"
\`\`\`
...
Rufflo exposes tools via Model Context Protocol:
...
Rufflo uses hooks for lifecycle automation:
...
npx @rufflo/cli hooks pre-task \\
  --description "implementing authentication"

# After completing a task
npx @rufflo/cli hooks post-task \\
  --task-id "task-123" \\
  --success true

# Route a task to agents
npx @rufflo/cli hooks route \\
...
\`\`\`bash
# List workers
npx @rufflo/cli hooks worker list

# Trigger specific worker
npx @rufflo/cli hooks worker dispatch --trigger audit

# Check worker status
npx @rufflo/cli hooks worker status
\`\`\`
...
\`\`\`bash
# Run diagnostics
npx @rufflo/cli doctor --fix

# Check system status
npx @rufflo/cli status
\`\`\`
`;
...
npx @rufflo/cli logs --type agent-actions --last 1h

# Export audit log
npx @rufflo/cli logs export --format json --output audit.json
\`\`\`
...
\`\`\`bash
# Check current role
npx @rufflo/cli claims list

# Request elevated permissions
npx @rufflo/cli claims request --permission deploy:production
\`\`\`
...
1. **Contain** - Immediately stop affected agents
   \`\`\`bash
   npx @rufflo/cli agent stop --all --force
   \`\`\`
...
2. **Decide** - Roll back if safe, or forward-fix
   \`\`\`bash
   # Rollback
   npx @rufflo/cli deployment rollback --env production

   # Or forward-fix
   npx @rufflo/cli workflow run hotfix
   \`\`\`
...
\`\`\`bash
# Restore from backup
npx @rufflo/cli memory restore --snapshot latest

# Restore specific checkpoint
npx @rufflo/cli session restore --checkpoint <id>
\`\`\`
...
- [ ] Run \`npx @rufflo/cli doctor\` to verify setup
