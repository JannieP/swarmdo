...
const SAMPLE_CLAUDE_MD = `# Rufflo V3 Project
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('args = ["-y", "@rufflo/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default rufflo server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default rufflo server when none specified
    expect(result).toContain('[mcp_servers.rufflo]');
    expect(result).toContain('command = "npx"');
  });
