...
<p align="center">
  <strong>OpenAI Codex CLI Adapter for Rufflo V3</strong><br/>
  <em>Self-learning multi-agent orchestration following the <a href="https://agentics.org">Agentics Foundation</a> standard</em>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@rufflo/codex"><img src="https://img.shields.io/npm/v/@rufflo/codex?label=npm&color=blue" alt="npm version"></a>
  <a href="https://www.npmjs.com/package/@rufflo/codex"><img src="https://img.shields.io/npm/dm/@rufflo/codex?label=downloads&color=cb3837" alt="npm downloads"></a>
...
Transform OpenAI Codex CLI into a **self-improving AI development system**. While Codex executes code, rufflo orchestrates, coordinates, and **learns from every interaction**.

| Traditional Codex | With Rufflo |
|-------------------|------------------|
...
npx rufflo@alpha init --codex

# Full setup with all 137+ skills
npx rufflo@alpha init --codex --full

# Dual mode (both Claude Code and Codex)
npx rufflo@alpha init --dual
```
...
| **HNSW Search** | ~1.9x-4.7x measured faster pattern matching |
...
| `.rufflo/data/memory.db` | Vector memory database |
...
npx rufflo@alpha init --codex --minimal
...
| V3 Performance Optimization | `$v3-performance-optimization` | Achieve unverified (no benchmark) speedup targets |
...
Both platforms share the same `.rufflo/` runtime:
...
The `@rufflo/codex` package now includes built-in dual-mode orchestration:
...
npx rufflo-codex dual run --template feature --task "Add user authentication"
...
Codex does not expose Claude Code's `ScheduleWakeup`, so `@rufflo/codex` provides a process-based equivalent:

```bash
# Run Codex repeatedly until it creates .codex/loop/default.complete or reaches 10 iterations
npx rufflo-codex loop run "Fix failing tests and create the completion marker when done"

# Use command mode for recurring Rufflo workers or custom scripts
npx rufflo-codex loop run --name testgaps --interval 270 --max-iterations 0 \
  --command "npx rufflo hooks worker dispatch --trigger testgaps"

# Inspect or stop a loop from another terminal
npx rufflo-codex loop status --name testgaps
npx rufflo-codex loop stop --name testgaps
```
...
| Speed Improvement | ~1.9x-4.7x measured faster |
...
```typescript
import { initializeCodexProject } from '@rufflo/codex';
...
```typescript
import { CodexInitializer, initializeCodexProject } from '@rufflo/codex';
...
```typescript
import { migrate } from '@rufflo/codex';
...
5. **Register MCP**: `codex mcp add rufflo -- npx rufflo mcp start`
...
npx rufflo memory list

# Manually add a test pattern
npx rufflo memory store --key "test" --value "test pattern" --namespace patterns
```
...
# Rebuild skills
npx rufflo@alpha init --codex --force
```
...
# Rebuild index
npx rufflo memory optimize --rebuild-index
```
...
| Package | Description |
|---------|-------------|
| [@rufflo/cli](https://www.npmjs.com/package/@rufflo/cli) | Main CLI (26 commands, 140+ subcommands) |
| [rufflo](https://www.npmjs.com/package/rufflo) | Umbrella package |
| [@rufflo/memory](https://www.npmjs.com/package/@rufflo/memory) | AgentDB with HNSW vector search |
| [@rufflo/security](https://www.npmjs.com/package/@rufflo/security) | Security module |
