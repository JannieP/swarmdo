...
// Persist local private key (mode 0600) for operator to move to .rufflo/federation/
writeFileSync(
...
writeFileSync(join(stageDir, 'rufflo-fed.conf'), interfaceConfig);

// Firewall projection per OS
const fw = new wgFirewall.WgFirewallService();
const fwFile = process.platform === 'linux' ? 'rufflo-fed.nft' : 'rufflo-fed.pf';
...
console.log(`  ${stageDir}/rufflo-fed.conf               (wg-quick config)`);
...
console.log('  [ ] Review rufflo-fed.conf for unexpected [Peer] blocks (defense vs compromised manifest)');
...
console.log(`  sudo install -m 0600 ${stageDir}/rufflo-fed.conf /etc/wireguard/rufflo-fed.conf`);
if (process.platform === 'linux') {
  console.log(`  sudo nft -f ${stageDir}/${fwFile}`);
} else {
  console.log(`  sudo pfctl -a rufflo-fed -f ${stageDir}/${fwFile}`);
}
console.log('  sudo wg-quick up rufflo-fed');
console.log();
console.log('=== Verification ===');
console.log('  sudo wg show rufflo-fed');
console.log(`  ping ${peerMeshIP.replace('/32', '')}    # mesh reachability`);
console.log();
console.log('=== Rollback ===');
console.log('  sudo wg-quick down rufflo-fed');
if (process.platform === 'linux') {
  console.log('  sudo nft delete table inet rufflo_fed');
} else {
  console.log('  sudo pfctl -a rufflo-fed -F all');
}
