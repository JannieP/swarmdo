 ...
 * plugins/rufflo-cost-tracker/scripts/federation.mjs:
 *
 ...
 * persists to rufflo memory / Redis / Datadog / their accounting system.
 */
 ...
 * rufflo memory, Redis, DynamoDB, file-backed JSON, etc. The federation
...
/** Default namespace per cost-tracker's `plugins/rufflo-cost-tracker/scripts/federation.mjs`. */
 ...
 * (`plugins/rufflo-cost-tracker/scripts/federation.mjs`) `memory list`s
 ...
 *   });
 *
 *   // rufflo memory CLI shell-out
 *   new MemorySpendReporter({
 *     memoryStore: {
 *       store: async ({namespace, key, value, ttl}) => {
 *         await execFile('npx', ['rufflo', 'memory', 'store',
