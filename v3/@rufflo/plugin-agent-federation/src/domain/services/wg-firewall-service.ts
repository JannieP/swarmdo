 ...
 *   - `pf` for macOS — `pfctl -a rufflo-fed -f <file>`, anchor-scoped so
 *     rufflo's rules don't collide with the operator's main pf ruleset.
 *
...
export interface WgFirewallServiceConfig {
  ...
  /** WG interface name to scope rules to. Defaults to `rufflo-fed`. */
  readonly interfaceName?: string;
  /** Where the operator should write the rule file. Used for the load-command string. */
  readonly rulePath?: string;
  /** pf anchor (macOS only). Defaults to `rufflo-fed`. */
...
const DEFAULT_INTERFACE = 'rufflo-fed';
const DEFAULT_PF_ANCHOR = 'rufflo-fed';
...
export class WgFirewallService {
  ...
  private renderNftables(projections: ReadonlyArray<{
    ...
    lines.push('# Generated by rufflo federation plugin — ADR-111 Phase 4.');
    ...
    lines.push('');
    lines.push('table inet rufflo_fed {');
