import type { MCPToolDefinition } from '@rufflo/shared/src/plugin-interface.js';
import type { PluginContext } from '@rufflo/shared/src/plugin-interface.js';
  ...
  return [
    {
      ...
      pluginName: '@rufflo/plugin-agent-federation',
    ...
    },
    {
      ...
      description: 'ADR-111 Phase 6 (witness chain): emit an operator-signed attestation entry for a coordination change. Returns the canonical bytes the operator signs + the witness entry; operator appends it to .rufflo/federation/wg-changes.log. Idempotent — re-attesting the same change just appends another entry.',
      pluginName: '@rufflo/plugin-agent-federation',
      version: '1.0.0-alpha.1',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      ...
      handler: async (params) => {
        ...
        return textResult(JSON.stringify({
          ...
          nextSteps: [
            'Write the new private key to .rufflo/federation/wg-key-<nodeId>.json (mode 0600)',
