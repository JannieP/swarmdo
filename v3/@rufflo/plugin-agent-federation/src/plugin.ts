import type {
  ...
  AgentTypeDefinition,
} from '@rufflo/shared/src/plugin-interface.js';
...
export class AgentFederationPlugin implements ClaudeFlowPlugin {
  readonly name = '@rufflo/plugin-agent-federation';
  ...
  readonly dependencies = ['@rufflo/security', '@rufflo/aidefence'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    // signatures. Persist to .rufflo/federation/key-<nodeId>.json so
    ...
    const keyDir = join(process.cwd(), '.rufflo', 'federation');
    ...
    return [
      {
        ...
        defaultConfig: {
          ...
          timeout: 300_000,
          metadata: {
            pluginSource: '@rufflo/plugin-agent-federation',
          },
        },
