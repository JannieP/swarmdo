#!/usr/bin/env node
/**
 * Standalone CLI shim for @rufflo/plugin-agent-federation.
 * Maps `rufflo-federation <subcommand> [args]` -> CLICommandDefinition handlers.
 *
...
function printHelp(commandNames: string[]): void {
  console.log('rufflo-federation — Cross-installation agent federation CLI');
  console.log('');
  console.log('Usage: rufflo-federation <subcommand> [options]');
  console.log('');
  ...
  console.log('');
  console.log('Run "rufflo-federation <subcommand> --help" for command-specific options.');
}
...
async function main(): Promise<void> {
  ...
  if (!resolved) {
    ...
    console.error('Run "rufflo-federation --help" to list available subcommands.');
    process.exitCode = 1;
    return;
  }
