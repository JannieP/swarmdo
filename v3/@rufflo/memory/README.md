...
[![npm version](https://img.shields.io/npm/v/@rufflo/memory.svg)](https://www.npmjs.com/package/@rufflo/memory)
...
- **Graceful retrieval.** `service.search('query')` no longer throws when `@rufflo/embeddings` is unavailable — it degrades to FTS5 keyword search and emits `health.embedder = 'degraded'`. The full hybrid path also adds real Reciprocal Rank Fusion + MMR diversity rerank.
...
```bash
npm install @rufflo/memory
```

## Standalone use (without the Rufflo CLI)

This package works on its own — you don't need `@rufflo/cli` or the
full Rufflo install. Use it any time you want HNSW vector search, an
...
```typescript
import { ControllerRegistry } from '@rufflo/memory';
...
```typescript
import { createHybridService } from '@rufflo/memory';

async function embedder(text: string): Promise<Float32Array> {
  // Use any embedding provider — OpenAI, @rufflo/embeddings, your own model.
...
import { MemoryService } from '@rufflo/memory';
...
```typescript
import { MemoryService, HNSWIndex } from '@rufflo/memory';
...
```typescript
import { MemoryConsolidator } from '@rufflo/memory';
...
`MemoryService.search()` and `semanticSearch()` no longer hard-fail when `@rufflo/embeddings` (or whatever embedder you injected) is unavailable. They:
...
```typescript
import { AgentDBAdapter } from '@rufflo/memory';
...
```typescript
import { CacheManager } from '@rufflo/memory';
...
```typescript
import { MemoryMigrator, createMigrator } from '@rufflo/memory';
...
```typescript
import { AutoMemoryBridge } from '@rufflo/memory';
...
Connects insights to the `@rufflo/neural` learning pipeline. When neural is unavailable, all operations degrade to no-ops.

### Quick Start

```typescript
import { AutoMemoryBridge, LearningBridge } from '@rufflo/memory';
...
```typescript
import { LearningBridge } from '@rufflo/memory';

const lb = new LearningBridge(backend, {
  // Optional: inject neural loader for custom setups
  neuralLoader: async () => {
    const { NeuralLearningSystem } = await import('@rufflo/neural');
...
```typescript
import { AutoMemoryBridge, MemoryGraph } from '@rufflo/memory';
...
```typescript
import { MemoryGraph } from '@rufflo/memory';
...
```typescript
import { createAgentBridge, transferKnowledge } from '@rufflo/memory';
...
- `@rufflo/embeddings` — **Optional** for semantic search. When absent or failing, `search()` automatically falls back to FTS5 keyword (see [Graceful retrieval](#graceful-retrieval-fts5-fallback))
- `@rufflo/neural` — **Optional** peer dependency for self-learning (graceful fallback when unavailable)

## Related Packages

- [@rufflo/neural](../neural) - Neural learning integration (SONA, ReasoningBank, EWC++)
- [@rufflo/shared](../shared) - Shared types and utilities
- [@rufflo/hooks](../hooks) - Session lifecycle hooks for auto memory sync
