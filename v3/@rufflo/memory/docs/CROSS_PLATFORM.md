...
The `@rufflo/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@rufflo/memory';
...
```typescript
import { createDatabase } from '@rufflo/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@rufflo/memory';
...
npm install @rufflo/memory --no-optional
