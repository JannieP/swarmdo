...
describe('V2 API Compatibility', () => {
  ...
  describe('Import Path Aliases', () => {
    ...
    it('should resolve rufflo/hive-mind to @rufflo/swarm', () => {
      const resolved = mockRegistry.resolveImport('rufflo/hive-mind');

      expect(resolved).toBe('@rufflo/swarm');
    });

    it('should resolve rufflo/memory to @rufflo/memory', () => {
      const resolved = mockRegistry.resolveImport('rufflo/memory');

      expect(resolved).toBe('@rufflo/memory');
    });
