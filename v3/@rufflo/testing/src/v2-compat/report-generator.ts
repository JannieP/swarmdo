...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| rufflo/hive-mind | @rufflo/swarm |');
  lines.push('| rufflo/swarm | @rufflo/swarm |');
  lines.push('| rufflo/memory | @rufflo/memory |');
  lines.push('| rufflo/agents | @rufflo/agent-lifecycle |');
  lines.push('| rufflo/tasks | @rufflo/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx rufflo hive-mind status');
  lines.push('');
  lines.push('# V3 (recommended)');
  lines.push('npx @rufflo/cli swarm init');
  lines.push('npx @rufflo/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { MemoryManager } from 'rufflo/memory';");
  lines.push('');
  lines.push('// V3 imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@rufflo/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@rufflo/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @rufflo/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @rufflo/cli migrate config --input .rufflo/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @rufflo/cli migrate memory --input .rufflo/memory.db');
  lines.push('```');
  lines.push('');
