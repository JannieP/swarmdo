#!/usr/bin/env node
/**
 * Rufflo V3 - Metrics Database Manager
...
const DB_PATH = join(PROJECT_ROOT, '.rufflo', 'metrics.db');
...
function checkSecurityFile(filename, minLines = 100) {
  const filePath = join(V3_DIR, '@rufflo/security/src', filename);
...
async function syncMetrics() {
  ...
  // Count V3 modules
  const modulesDir = join(V3_DIR, '@rufflo');
  let modules = [];
  let totalProgress = 0;
  ...
  if (existsSync(modulesDir)) {
    ...
    for (const entry of entries) {
      // Skip hidden directories (like .agentic-flow, .rufflo)
...
function exportToJSON() {
  ...
  const metricsDir = join(PROJECT_ROOT, '.rufflo/metrics');
  const securityDir = join(PROJECT_ROOT, '.rufflo/security');
