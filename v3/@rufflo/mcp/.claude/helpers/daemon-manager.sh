#!/bin/bash
# Rufflo V3 - Daemon Manager
# Manages background services for real-time statusline updates

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
PID_DIR="$PROJECT_ROOT/.rufflo/pids"
LOG_DIR="$PROJECT_ROOT/.rufflo/logs"
...
# Start all daemons
start_all() {
    log "Starting all Rufflo daemons..."
...
# Stop all daemons
stop_all() {
    log "Stopping all Rufflo daemons..."
...
show_status() {
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════${RESET}"
    echo -e "${CYAN}       Rufflo V3 Daemon Status${RESET}"
    echo -e "${CYAN}═══════════════════════════════════════════════════${RESET}"
    echo ""
