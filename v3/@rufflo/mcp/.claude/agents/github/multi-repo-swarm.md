...
tools:
  ...
  - mcp__rufflo__github_sync_coord
...
npx rufflo@v3alpha github multi-repo-init \
...
npx rufflo@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx rufflo@v3alpha github task-execute \
...
npx rufflo@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx rufflo@v3alpha github multi-repo-refactor \
...
npx rufflo@v3alpha github multi-repo-security \
  --scan-all \
...
npx rufflo@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx rufflo@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx rufflo@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx rufflo@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx rufflo@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx rufflo@v3alpha github health-check \
  --repos "org/*" \
...
npx rufflo@v3alpha github microservices \
...
npx rufflo@v3alpha github lib-update \
...
npx rufflo@v3alpha github org-policy \
...
npx rufflo@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx rufflo@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx rufflo@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx rufflo@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx rufflo@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx rufflo@v3alpha github perf-analysis \
  --profile-operations \
...
npx rufflo@v3alpha github fullstack-update \
...
npx rufflo@v3alpha github cross-team \
