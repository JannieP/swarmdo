...
hooks:
  pre: |
    ...
    mcp__rufflo__memory_namespace --namespace="${NAMESPACE:-default}" --action="init"
    # Check HNSW index status
    mcp__rufflo__memory_analytics --timeframe="1h"
    # Store initialization event
    mcp__rufflo__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:init:${TASK_ID}" --value="$(date -Iseconds): Memory specialist session started"
  post: |
    echo "Memory optimization complete"
    # Persist memory state
    mcp__rufflo__memory_persist --sessionId="${SESSION_ID}"
    # Compress and optimize namespaces
    mcp__rufflo__memory_compress --namespace="${NAMESPACE:-default}"
    # Generate memory analytics report
    mcp__rufflo__memory_analytics --timeframe="24h"
    # Store completion metrics
    mcp__rufflo__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:complete:${TASK_ID}" --value="$(date -Iseconds): Memory optimization completed"
---
...
### 1. HNSW Indexing Optimization (~1.9x-4.7x measured Faster Search)
...
mcp__rufflo__memory_usage --action="store" --namespace="patterns" --key="auth:jwt-strategy" --value='{"pattern": "jwt-auth", "embedding": [...]}' --ttl=604800000

# Semantic search with HNSW
mcp__rufflo__memory_search --pattern="authentication strategies" --namespace="patterns" --limit=10

# Namespace management
mcp__rufflo__memory_namespace --namespace="project:myapp" --action="create"

# Memory analytics
mcp__rufflo__memory_analytics --timeframe="7d"

# Memory compression
mcp__rufflo__memory_compress --namespace="default"

# Cross-session persistence
mcp__rufflo__memory_persist --sessionId="session-12345"

# Memory backup
mcp__rufflo__memory_backup --path="./backups/memory-$(date +%Y%m%d).bak"

# Distributed sync
mcp__rufflo__memory_sync --target="peer-agent-1"
```
...
npx rufflo@v3alpha memory init --backend=hybrid --hnsw-enabled

# Memory health check
npx rufflo@v3alpha memory health

# Search memories
npx rufflo@v3alpha memory search -q "authentication patterns" --namespace="patterns"

# Consolidate memories
npx rufflo@v3alpha memory consolidate --strategy=hybrid --retention=0.7

# Export/import namespaces
npx rufflo@v3alpha memory export --namespace="project:myapp" --format=json
npx rufflo@v3alpha memory import --file="backup.json" --namespace="project:myapp"

# Memory statistics
npx rufflo@v3alpha memory stats --namespace="default"

# Quantization
npx rufflo@v3alpha memory quantize --namespace="embeddings" --method=int8
```
...
|--------|-------------|-----------|-------------|
| Vector Search | 1000ms | 0.8-6.7ms | ~1.9x-4.7x measured |
