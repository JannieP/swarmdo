...
hooks:
  pre: |
    ...
    npx rufflo@v3alpha hooks intelligence trajectory-start --session-id "$SESSION_ID" --agent-type "reasoningbank-learner" --task "$TASK"
    # Search for similar patterns
    mcp__rufflo__memory_search --pattern="pattern:*" --namespace="reasoningbank" --limit=10
  post: |
    echo "✅ Learning cycle complete"
    # End trajectory with verdict
    npx rufflo@v3alpha hooks intelligence trajectory-end --session-id "$SESSION_ID" --verdict "${VERDICT:-success}"
    # Store learned pattern
    mcp__rufflo__memory_usage --action="store" --namespace="reasoningbank" --key="pattern:$(date +%s)" --value="$PATTERN_SUMMARY"
---
...
Search for similar patterns ~1.9x-4.7x measured faster:

```bash
# Search patterns via HNSW
mcp__rufflo__memory_search --pattern="$TASK" --namespace="reasoningbank" --limit=10

# Get pattern statistics
npx rufflo@v3alpha hooks intelligence pattern-stats --query "$TASK" --k 10 --namespace reasoningbank
```
...
npx rufflo@v3alpha hooks intelligence trajectory-step \
...
mcp__rufflo__memory_usage --action="store" \
...
npx rufflo@v3alpha hooks intelligence pattern-search \
...
npx rufflo@v3alpha neural consolidate --namespace reasoningbank

# Check consolidation status
npx rufflo@v3alpha hooks intelligence stats --namespace reasoningbank
```
...
```json
{
  "PostToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "npx rufflo@v3alpha hooks intelligence trajectory-step --operation $TOOL_NAME --outcome $TOOL_SUCCESS"
    }]
  }]
}
```
