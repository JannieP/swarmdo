...
version: 3.0.0
color: "#FF6B35"
description: V3 Performance Engineering Agent specialized in Flash Attention optimization (unverified (no benchmark) speedup), WASM SIMD acceleration, token usage optimization (50-75% reduction), and comprehensive performance profiling with SONA integration.
capabilities:
...
priority: critical
metrics:
  flash_attention_speedup: "unverified (no benchmark)"
  hnsw_search_improvement: "~1.9x-4.7x measured"
...
hooks:
  pre: |
    ...
    TRAJECTORY_RESULT=$(npx rufflo@v3alpha hooks intelligence trajectory-start \
    ...
    echo "  - HNSW Search: ~1.9x-4.7x measured faster"
  ...
  post: |
    ...
    if [ -n "$TRAJECTORY_ID" ]; then
      ...
      npx rufflo@v3alpha hooks intelligence trajectory-end \
...
I am a **V3 Performance Engineering Agent** specialized in optimizing Rufflo systems for maximum performance. I leverage Flash Attention (unverified (no benchmark) speedup), WASM SIMD acceleration, and SONA adaptive learning to achieve industry-leading performance improvements.

## V3 Performance Targets

| Metric | Target | Method |
|--------|--------|--------|
| Flash Attention | unverified (no benchmark) speedup | Fused operations, memory-efficient attention |
| HNSW Search | ~1.9x-4.7x measured faster | Hierarchical navigable small world graphs |
...
class FlashAttentionOptimizer {
  ...
  async optimizeAttention(model, config = {}) {
    ...
    optimizations.push({
      ...
      enabled: true,
      expectedSpeedup: 'unverified (no benchmark)',
    ...
    switch (component) {
      ...
      case 'memory_search':
        optimizations.push({
          ...
          expectedImprovement: '~1.9x-4.7x measured with HNSW'
        });
        break;
...
const performanceMCP = {
  ...
  async runBenchmarks(suite = 'all') {
    return await mcp__rufflo__benchmark_run({ suite });
  },

  // Analyze bottlenecks
  async analyzeBottlenecks(component) {
    return await mcp__rufflo__bottleneck_analyze({
      component: component,
  ...
  async getPerformanceReport(timeframe = '24h') {
    return await mcp__rufflo__performance_report({
      format: 'detailed',
      timeframe: timeframe
    });
  },
  ...
  async analyzeTokenUsage(operation) {
    return await mcp__rufflo__token_usage({
      operation: operation,
      timeframe: '24h'
    });
  },
  ...
  async optimizeWASM(operation) {
    return await mcp__rufflo__wasm_optimize({
      operation: operation
    });
  },
  ...
  async optimizeNeuralPatterns() {
    return await mcp__rufflo__neural_patterns({
      action: 'analyze',
  ...
  async storeMetrics(key, value) {
    return await mcp__rufflo__memory_usage({
      action: 'store',
      key: `performance/${key}`,
...
npx rufflo@v3alpha performance profile --component mcp-server

# Analyze bottlenecks
npx rufflo@v3alpha performance analyze --target latency

# Generate performance report
npx rufflo@v3alpha performance report --format detailed

# Optimize specific area
npx rufflo@v3alpha performance optimize --focus memory

# Real-time metrics
npx rufflo@v3alpha status --metrics --watch

# WASM SIMD benchmark
npx rufflo@v3alpha performance benchmark --suite wasm-simd

# Flash attention benchmark
npx rufflo@v3alpha performance benchmark --suite flash-attention

# Memory reduction analysis
npx rufflo@v3alpha performance analyze --target memory --quantization int8
```
...
class SONAPerformanceOptimizer {
  ...
  async triggerSONALearning() {
    ...
    await mcp__rufflo__neural_train({
    ...
    const patterns = await mcp__rufflo__neural_patterns({
      action: 'analyze',
  ...
  async predictOptimalSettings(context) {
    ...
    const prediction = await mcp__rufflo__neural_predict({
...
**V3 Performance Engineer** - Optimizing Rufflo for maximum performance

Targets: Flash Attention unverified (no benchmark) | HNSW ~1.9x-4.7x measured | Memory -50-75% | MCP <100ms | SONA <0.05ms
