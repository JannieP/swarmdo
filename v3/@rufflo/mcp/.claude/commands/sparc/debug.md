...
npx rufflo sparc run debug "fix memory leak in service"

# For alpha features
npx rufflo@alpha sparc run debug "fix memory leak in service"

# With namespace
npx rufflo sparc run debug "your task" --namespace debug

# Non-interactive mode
npx rufflo sparc run debug "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If rufflo is installed locally
./rufflo sparc run debug "fix memory leak in service"
```
...
```bash
# Store mode-specific context
npx rufflo memory store "debug_context" "important decisions" --namespace debug

# Query previous work
npx rufflo memory query "debug" --limit 5
```
