...
npx rufflo sparc run post-deployment-monitoring-mode "monitor production metrics"

# For alpha features
npx rufflo@alpha sparc run post-deployment-monitoring-mode "monitor production metrics"

# With namespace
npx rufflo sparc run post-deployment-monitoring-mode "your task" --namespace post-deployment-monitoring-mode

# Non-interactive mode
npx rufflo sparc run post-deployment-monitoring-mode "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If rufflo is installed locally
./rufflo sparc run post-deployment-monitoring-mode "monitor production metrics"
```
...
```bash
# Store mode-specific context
npx rufflo memory store "post-deployment-monitoring-mode_context" "important decisions" --namespace post-deployment-monitoring-mode

# Query previous work
npx rufflo memory query "post-deployment-monitoring-mode" --limit 5
```
