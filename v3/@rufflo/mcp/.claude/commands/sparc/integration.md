...
npx rufflo sparc run integration "connect payment service"

# For alpha features
npx rufflo@alpha sparc run integration "connect payment service"

# With namespace
npx rufflo sparc run integration "your task" --namespace integration

# Non-interactive mode
npx rufflo sparc run integration "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If rufflo is installed locally
./rufflo sparc run integration "connect payment service"
```
...
```bash
# Store mode-specific context
npx rufflo memory store "integration_context" "important decisions" --namespace integration

# Query previous work
npx rufflo memory query "integration" --limit 5
```
