...
describe('CredentialGenerator', () => {
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['rufflo/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['rufflo/service-password']).toBe(credentials.servicePassword);
      expect(parsed['rufflo/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['rufflo/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['rufflo/encryption-key']).toBe(credentials.encryptionKey);
    });
  });
