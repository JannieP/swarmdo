...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# Rufflo V3 - Generated Credentials
...
`;
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'rufflo/admin-password': credentials.adminPassword,
      'rufflo/service-password': credentials.servicePassword,
      'rufflo/jwt-secret': credentials.jwtSecret,
      'rufflo/session-secret': credentials.sessionSecret,
      'rufflo/encryption-key': credentials.encryptionKey,
      'rufflo/generated-at': credentials.generatedAt.toISOString(),
      'rufflo/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}
