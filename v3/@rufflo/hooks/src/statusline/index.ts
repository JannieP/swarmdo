...
export class StatuslineGenerator {
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.rufflo', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.rufflo', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.rufflo', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    const patternsPath = join(this.projectRoot, '.rufflo', 'learning', 'patterns.db');
    try {
