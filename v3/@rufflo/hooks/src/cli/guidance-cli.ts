 ...
 *
 * Usage:
 *   npx @rufflo/hooks session-context
 *   npx @rufflo/hooks user-prompt "Fix authentication bug"
 *   npx @rufflo/hooks pre-edit "/path/to/file.ts"
 *   npx @rufflo/hooks route "Implement caching layer"
 *
 * @module @rufflo/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@rufflo/hooks - V3 Guidance System CLI

Usage: npx @rufflo/hooks <command> [args]
...
Examples:
  ...
  npx @rufflo/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @rufflo/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @rufflo/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @rufflo/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @rufflo/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @rufflo/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @rufflo/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
