 ...
 * - ONNX embeddings via @rufflo/embeddings (MiniLM-L6 384-dim)
 * - AgentDB backend for persistence
 * - Pattern promotion from short-term to long-term memory
 *
 * @module @rufflo/hooks/reasoningbank
 */
...
export class ReasoningBank extends EventEmitter {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    const memoryModule = await dynamicImport('@rufflo/memory');
    if (memoryModule) {
      AgentDBAdapter = memoryModule.AgentDBAdapter;
      HNSWIndex = memoryModule.HNSWIndex;
    }

    const embeddingsModule = await dynamicImport('@rufflo/embeddings');
    if (embeddingsModule) {
...
/**
 * Real embedding service using @rufflo/embeddings
 */
