...
**Example:**
```typescript
import { detectPlatform } from '@swarmdo/cli/init';
...
```typescript
import { generateSettings, generateSettingsJson } from '@swarmdo/cli/init';
...
```typescript
import { generateMCPConfig, generateMCPJson, generateMCPCommands } from '@swarmdo/cli/init';
...
// ['claude mcp add swarmdo -- npx @swarmdo/cli mcp start', ...]
```
...
```typescript
import { generateStatuslineScript, generateStatuslineHook } from '@swarmdo/cli/init';
...
const result = await executeInit({
  ...
  sourceBaseDir: '/path/to/swarmdo', // Source for skills/commands/agents
  force: true,
  components: {
...
```typescript
import { executeInit, detectPlatform, DEFAULT_INIT_OPTIONS } from '@swarmdo/cli/init';
...
```typescript
import { executeInit, MINIMAL_INIT_OPTIONS } from '@swarmdo/cli/init';
