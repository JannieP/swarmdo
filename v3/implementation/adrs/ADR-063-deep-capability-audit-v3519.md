...
**Context:** Comprehensive 4-agent parallel audit of all 98 CLI capabilities in Docker environment (swarmdo@latest). Covers CLI commands, memory/neural, hooks/sessions, MCP/hive-mind, and known platform limitations.
...
- **Environment:** Docker (multi-stage build from swarmdo@latest on npm)
...
- **Files:** `v3/@swarmdo/cli/src/parser.ts` (added `buildScopedAliases()`, `getScopedBooleanFlags()`)
...
- **File:** `v3/@swarmdo/cli/src/commands/neural.ts`
...
- **File:** `v3/@swarmdo/cli/src/memory/intelligence.ts`
...
- **File:** `v3/@swarmdo/cli/src/commands/hooks.ts`
...
- **File:** `v3/@swarmdo/cli/src/commands/agent.ts`
...
- **Mitigation:** Lowered default similarity threshold to 0.1 for hash-fallback (C-3 fix). For production, install `@swarmdo/embeddings` with ONNX model.
- **Status:** By design — Docker prioritizes image size over model quality

### L-2: Flash Attention at 0.40x (below 2.49x target)
- **Context:** WASM runtime (@swarmvector/learning-wasm) not available in Docker
...
### L-3: @swarmvector/learning-wasm missing
...
- **Context:** `@swarmdo/embeddings` package is pruned from Docker image
- **Impact:** `embeddings init` reports success but uses hash-fallback backend
- **Mitigation:** Hash-fallback provides basic functionality; full embeddings available via `plugins install @swarmdo/embeddings`
