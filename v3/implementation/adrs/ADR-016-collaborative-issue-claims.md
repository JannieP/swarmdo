...
swarmdo issues list --available

# Claim an issue (as current user/agent)
swarmdo issues claim 123

# Release a claim
swarmdo issues release 123

# Request handoff to specific agent/human
swarmdo issues handoff 123 --to agent:coder-1
swarmdo issues handoff 123 --to human:alice

# Update status
swarmdo issues status 123 --blocked "Waiting for API spec"
swarmdo issues status 123 --review-requested

# View who's working on what
swarmdo issues board

# Work stealing commands
swarmdo issues stealable              # List stealable issues
swarmdo issues steal 123              # Steal an issue
swarmdo issues mark-stealable 123     # Mark your claim as stealable
swarmdo issues contest 123            # Contest a steal

# Load balancing
swarmdo issues load                   # View agent load distribution
swarmdo issues load --agent coder-1   # View specific agent load
swarmdo issues rebalance              # Trigger swarm rebalancing
swarmdo issues rebalance --dry-run    # Preview rebalancing without applying
```
...
- [x] Claim service implemented (`@swarmdo/cli/src/services/claim-service.ts`)
...
- [x] CLI commands functional (`@swarmdo/cli/src/commands/issues.ts`)
...
|------|-------|-------------|
| `@swarmdo/cli/src/services/claim-service.ts` | ~600 | Full claims service with work stealing |
| `@swarmdo/cli/src/commands/issues.ts` | ~450 | CLI commands for issue claims |

### CLI Commands Implemented

```bash
# Issue claim commands (10 subcommands)
swarmdo issues list          # List all claims
swarmdo issues claim <id>    # Claim an issue
swarmdo issues release <id>  # Release a claim
swarmdo issues handoff       # Request handoff
swarmdo issues status <id>   # Get claim status
swarmdo issues stealable     # List stealable issues
swarmdo issues steal <id>    # Steal an issue
swarmdo issues load          # View agent load
swarmdo issues rebalance     # Rebalance swarm
swarmdo issues board         # Visual claim board
```
...
6. **Persistence**: File-based storage in `.swarmdo/claims/claims.json`
