...
- Package: `@swarmdo/codex` (npm)
- Future umbrella: `coflow` (npm/npx coflow)
- Current umbrella: `swarmdo` (maintained for compatibility)
...
We will create a **parallel Codex integration** in swarmdo that:
...
5. **Maps swarmdo concepts** to Codex equivalents
...
└── .swarmdo/                # Runtime (shared between both)
    ├── config.yaml
    └── data/
```
...
| swarmdo Concept | Claude Code Output | Codex Output |
|---------------------|-------------------|--------------|
| Project instructions | `CLAUDE.md` | `AGENTS.md` |
...
swarmdo init --codex --from-claude

# Convert existing Codex setup to Claude Code
swarmdo init --from-codex
```
...
### 1. Initialize Swarm
```bash
npx swarmdo@v3alpha swarm init --topology hierarchical
```
...
```bash
npx swarmdo@v3alpha memory store --key "[pattern]" --value "[learned]"
```
```
...
5. Wire up `@swarmdo/codex` as dependency
...
4. Publish skills as part of `@swarmdo/codex`

### Phase 4: Automation Integration (Week 5)
1. Map swarmdo hooks to Codex Automations
...
3. Maintain `swarmdo` as alias for compatibility
...
### AGENTS.md Sections (Swarmdo Template)
...
| Security | From @swarmdo/security patterns |
...
These features were discovered through binary analysis and can be leveraged for deep swarmdo integration.
...
**Integration opportunity**: Map to swarmdo swarm coordination.
...
**Integration opportunity**: Register swarmdo tools dynamically.
...
```toml
# Swarmdo V3 - Codex Configuration (Enhanced)
