...
- Package: `@rufflo/codex` (npm)
- Future umbrella: `coflow` (npm/npx coflow)
- Current umbrella: `rufflo` (maintained for compatibility)
...
We will create a **parallel Codex integration** in rufflo that:
...
5. **Maps rufflo concepts** to Codex equivalents
...
└── .rufflo/                # Runtime (shared between both)
    ├── config.yaml
    └── data/
```
...
| rufflo Concept | Claude Code Output | Codex Output |
|---------------------|-------------------|--------------|
| Project instructions | `CLAUDE.md` | `AGENTS.md` |
...
rufflo init --codex --from-claude

# Convert existing Codex setup to Claude Code
rufflo init --from-codex
```
...
### 1. Initialize Swarm
```bash
npx rufflo@v3alpha swarm init --topology hierarchical
```
...
```bash
npx rufflo@v3alpha memory store --key "[pattern]" --value "[learned]"
```
```
...
5. Wire up `@rufflo/codex` as dependency
...
4. Publish skills as part of `@rufflo/codex`

### Phase 4: Automation Integration (Week 5)
1. Map rufflo hooks to Codex Automations
...
3. Maintain `rufflo` as alias for compatibility
...
### AGENTS.md Sections (Rufflo Template)
...
| Security | From @rufflo/security patterns |
...
These features were discovered through binary analysis and can be leveraged for deep rufflo integration.
...
**Integration opportunity**: Map to rufflo swarm coordination.
...
**Integration opportunity**: Register rufflo tools dynamically.
...
```toml
# Rufflo V3 - Codex Configuration (Enhanced)
