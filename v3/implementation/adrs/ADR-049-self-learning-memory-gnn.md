...
1. **No learning pipeline**: The `@rufflo/neural` package has a fully implemented `NeuralLearningSystem` with SONA, ReasoningBank (4-step RETRIEVE/JUDGE/DISTILL/CONSOLIDATE pipeline), and PatternLearner — but these are completely disconnected from the memory bridge.
...
- **Optional dependency**: `@rufflo/neural` is loaded dynamically; when unavailable, all learning operations degrade to no-ops (confidence remains static).
...
│           │                                                   │
│  ┌────────▼─────────────────┐                                │
│  │  @rufflo/neural     │  (optional peer dependency)    │
...
- **Graceful degradation**: Tests verify no-op behavior when `@rufflo/neural` unavailable
...
- `@rufflo/neural` becomes an optional peer dependency
