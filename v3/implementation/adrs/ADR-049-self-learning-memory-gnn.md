# ADR-049: Self-Learning Memory with GNN & SwarmVector Integration
...
1. **No learning pipeline**: The `@swarmdo/neural` package has a fully implemented `NeuralLearningSystem` with SONA, ReasoningBank (4-step RETRIEVE/JUDGE/DISTILL/CONSOLIDATE pipeline), and PatternLearner — but these are completely disconnected from the memory bridge.
...
- **Optional dependency**: `@swarmdo/neural` is loaded dynamically; when unavailable, all learning operations degrade to no-ops (confidence remains static).
...
│           │                                                   │
│  ┌────────▼─────────────────┐                                │
│  │  @swarmdo/neural     │  (optional peer dependency)    │
...
- **Graceful degradation**: Tests verify no-op behavior when `@swarmdo/neural` unavailable
...
- `@swarmdo/neural` becomes an optional peer dependency
