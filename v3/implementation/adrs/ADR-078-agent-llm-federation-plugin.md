...
Rufflo v3.5 operates within single installations. Agents spawned on one machine cannot collaborate with agents on another machine, user, or organization. As multi-agent AI development matures, the need for cross-installation coordination becomes critical -- organizations running separate Rufflo installations need agents that can discover each other, share context safely, and coordinate work across trust boundaries without leaking PII or allowing prompt injection attacks from untrusted peers.
...
If Rufflo ships this, it becomes the reference implementation. Federation becomes the standard.
...
**Agent marketplaces that don't leak data** -- Safely expose MCP tools, agent capabilities, and partial memory without giving away full context. This is the Rufflo marketplace path.
...
Build `@rufflo/plugin-agent-federation` as a first-class Rufflo plugin that enables cross-installation agent collaboration with security-first design.
...
- Discovery uses three mechanisms:
  1. **Static peers** -- configured in `rufflo.config.json` under `federation.peers[]`
  2. **DNS-SD** -- `_claude-federation._tcp` SRV records for LAN discovery
  3. **Registry** -- optional IPFS-pinned registry (extends existing Pinata infrastructure in `v3/@rufflo/cli/src/plugins/store/discovery.ts`)

**Phase B: Handshake (mTLS + Challenge-Response)**
- Nodes establish mTLS connections using ed25519 keypairs generated by `@rufflo/security/TokenGenerator`.
...
| Decision | Rationale |
|----------|-----------|
| **Plugin, not core** | Federation is opt-in; many installations are single-node. Ships as `@rufflo/plugin-agent-federation`. |
...
| **Claims-based authorization** | Every cross-boundary operation requires a claim. Extends `@rufflo/claims` with federation-specific claim types. |
...
All primitives reuse existing `@rufflo/security`:
...
The existing PII patterns in `@rufflo/aidefence` detect email, SSN, credit card, API key, GitHub token, and hardcoded passwords. The federation plugin extends this with:
...
```
v3/@rufflo/plugin-agent-federation/
  package.json
  tsconfig.json
  vitest.config.ts
  src/
...
export class AgentFederationPlugin implements ClaudeFlowPlugin {
  ...
  readonly dependencies = ['@rufflo/security', '@rufflo/aidefence', '@rufflo/claims'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    // 1. Register federation claim types with @rufflo/claims
...
**Integration Points:**
- `@rufflo/aidefence` -- `hasPII()`, `detect()`, PII_PATTERNS
- `@rufflo/security` -- `TokenGenerator`, `PasswordHasher`
- `@rufflo/memory` -- AgentDB for audit storage
...
**Integration Points:**
- `@rufflo/swarm/federation-hub` -- heartbeat, registration
- `@rufflo/security` -- mTLS certificate validation
- `@rufflo/guidance/authority` -- `AuthorityGate` for peer acceptance
...
**Integration Points:**
- `@rufflo/aidefence` -- `detect()`, `hasPII()`, `learnFromDetection()`
- `@rufflo/claims` -- `ClaimService` for federation claims
- `@rufflo/hooks` -- hook registration for send/receive lifecycle
...
- npm publish as `@rufflo/plugin-agent-federation`
...
**Positive:**
- Rufflo becomes the first multi-agent framework with production-grade federated security
