...
Swarmdo v3.5 operates within single installations. Agents spawned on one machine cannot collaborate with agents on another machine, user, or organization. As multi-agent AI development matures, the need for cross-installation coordination becomes critical -- organizations running separate Swarmdo installations need agents that can discover each other, share context safely, and coordinate work across trust boundaries without leaking PII or allowing prompt injection attacks from untrusted peers.
...
If Swarmdo ships this, it becomes the reference implementation. Federation becomes the standard.
...
**Agent marketplaces that don't leak data** -- Safely expose MCP tools, agent capabilities, and partial memory without giving away full context. This is the Swarmdo marketplace path.
...
Build `@swarmdo/plugin-agent-federation` as a first-class Swarmdo plugin that enables cross-installation agent collaboration with security-first design.
...
- Discovery uses three mechanisms:
  1. **Static peers** -- configured in `swarmdo.config.json` under `federation.peers[]`
  2. **DNS-SD** -- `_claude-federation._tcp` SRV records for LAN discovery
  3. **Registry** -- optional IPFS-pinned registry (extends existing Pinata infrastructure in `v3/@swarmdo/cli/src/plugins/store/discovery.ts`)

**Phase B: Handshake (mTLS + Challenge-Response)**
- Nodes establish mTLS connections using ed25519 keypairs generated by `@swarmdo/security/TokenGenerator`.
...
| Decision | Rationale |
|----------|-----------|
| **Plugin, not core** | Federation is opt-in; many installations are single-node. Ships as `@swarmdo/plugin-agent-federation`. |
...
| **Claims-based authorization** | Every cross-boundary operation requires a claim. Extends `@swarmdo/claims` with federation-specific claim types. |
...
All primitives reuse existing `@swarmdo/security`:
...
The existing PII patterns in `@swarmdo/aidefence` detect email, SSN, credit card, API key, GitHub token, and hardcoded passwords. The federation plugin extends this with:
...
```
v3/@swarmdo/plugin-agent-federation/
  package.json
  tsconfig.json
  vitest.config.ts
  src/
...
export class AgentFederationPlugin implements ClaudeFlowPlugin {
  ...
  readonly dependencies = ['@swarmdo/security', '@swarmdo/aidefence', '@swarmdo/claims'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    // 1. Register federation claim types with @swarmdo/claims
...
**Integration Points:**
- `@swarmdo/aidefence` -- `hasPII()`, `detect()`, PII_PATTERNS
- `@swarmdo/security` -- `TokenGenerator`, `PasswordHasher`
- `@swarmdo/memory` -- AgentDB for audit storage
...
**Integration Points:**
- `@swarmdo/swarm/federation-hub` -- heartbeat, registration
- `@swarmdo/security` -- mTLS certificate validation
- `@swarmdo/guidance/authority` -- `AuthorityGate` for peer acceptance
...
**Integration Points:**
- `@swarmdo/aidefence` -- `detect()`, `hasPII()`, `learnFromDetection()`
- `@swarmdo/claims` -- `ClaimService` for federation claims
- `@swarmdo/hooks` -- hook registration for send/receive lifecycle
...
- npm publish as `@swarmdo/plugin-agent-federation`
...
## 9. Stretch Direction: SwarmVector / MinCut / RVF Integration
...
### 9.1 Trust Graph Analysis via SwarmVector

The trust scores computed in Section 2.1.1 produce a weighted directed graph. SwarmVector's HNSW indexing can surface structural properties:
...
The `.rvf` (SwarmVector Format) file format provides a signed, versioned container for agent state. In the federation context:
...
**Positive:**
- Swarmdo becomes the first multi-agent framework with production-grade federated security
