...
Swarmdo v3 has multiple extension mechanisms scattered across different packages:
- Plugin interfaces in `@swarmdo/shared`
- Worker system in `@swarmdo/integration`
- Hooks system in `@swarmdo/hooks`
...
Create a unified `@swarmdo/plugins` package that consolidates all plugin development capabilities into a single, coherent SDK.
...
1. **Single Import**: Plugin authors import from `@swarmdo/plugins` only
...
```typescript
// Before
import { IPlugin, PluginMetadata } from '@swarmdo/shared';

// After
import { IPlugin, PluginMetadata } from '@swarmdo/plugins';
```
...
```typescript
// Before
import { HookEvent, HookHandler } from '@swarmdo/hooks';

// After
import { HookEvent, HookHandler, HookRegistry } from '@swarmdo/plugins';
```
...
```typescript
import { pluginCreatorPlugin } from '@swarmdo/plugins/examples/plugin-creator';
...
- [Plugin Interface Design](../../@swarmdo/plugins/src/core/plugin-interface.ts)
- [Base Plugin Implementation](../../@swarmdo/plugins/src/core/base-plugin.ts)
- [Security Module](../../@swarmdo/plugins/src/security/index.ts)
- [agentic-flow@alpha Integration](../../@swarmdo/plugins/src/integrations/agentic-flow.ts)
- [Plugin Creator Example](../../@swarmdo/plugins/examples/plugin-creator/index.ts)
- [README.md](../../@swarmdo/plugins/README.md)
