...
`npx rufflo init`, and real-time data feeds from 8+ subsystems.
...
2. **Generated statusline** (`.claude/helpers/statusline.cjs`) — CommonJS script
   created by `npx rufflo init`, comprehensive with 12+ metric panels
...
5. **Generator** (`v3/@rufflo/cli/src/init/statusline-generator.ts`) — TypeScript
...
|-------------|-----------|---------|------------|
| `.rufflo/data/autopilot-state.json` | Context Autopilot (ADR-051) | Token %, token count, prune cycles, growth trend | `context-persistence-hook.mjs` on every `UserPromptSubmit` |
| `.rufflo/metrics/v3-progress.json` | DDD Architecture | Domain count, DDD progress %, active agents | `init` command, manual updates |
| `.rufflo/security/audit-status.json` | Security | CVE count, audit status (CLEAN/PENDING) | `security scan` command |
| `.rufflo/metrics/performance.json` | Performance | Flash Attention speedup | `performance benchmark` command |
| `.rufflo/metrics/learning.json` | Intelligence | Score (0-100), routing accuracy, SONA status | `hooks post-task`, neural training |
| `.rufflo/learning/patterns.db` | Pattern DB (SQLite) | Short/long-term pattern counts, avg quality | `hooks intelligence`, neural training |
| `.rufflo/data/transcript-archive.db` | Context Archive (SQLite) | Entry count, session count | `context-persistence-hook.mjs` |
| `.rufflo/metrics/swarm-activity.json` | Swarm Monitor | Active agent count, swarm state | Swarm monitor daemon |
...
```
Line 0: ▊ Rufflo V3 ● user  │  ⎇ branch  │  Model
Line -: ─────────────────────────────────────────
...
Created by `npx rufflo init`. CommonJS for ES module project compatibility.
...
#### Generator — `v3/@rufflo/cli/src/init/statusline-generator.ts` (1,317 lines)

Produces the `.cjs` script during `npx rufflo init`:
...
#### Settings Generator — `v3/@rufflo/cli/src/init/settings-generator.ts`
...
#### Executor — `v3/@rufflo/cli/src/init/executor.ts`
...
#### Types — `v3/@rufflo/cli/src/init/types.ts`
...
| `v3/@rufflo/cli/src/init/statusline-generator.ts` | 1,317 | TypeScript | Generator for `.cjs` during init |
| `v3/@rufflo/cli/src/init/settings-generator.ts` | ~20 | TypeScript | Wires statusLine into settings.json |
| `v3/@rufflo/cli/src/init/executor.ts` | ~60 | TypeScript | Copy/generate during init |
| `v3/@rufflo/cli/src/init/types.ts` | ~20 | TypeScript | `StatuslineConfig` interface |
