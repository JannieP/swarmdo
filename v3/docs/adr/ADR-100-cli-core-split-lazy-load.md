# ADR-100: Split `@swarmdo/cli` into `cli-core` + lazy-loaded extras
...
**Version**: `@swarmdo/cli-core@3.7.0-alpha.5` published; `@swarmdo/cli@3.7.0-alpha.1` metapackage released
**Supersedes**: nothing
**Related**: ADR-098 (plugin capability sync and optimization), issue [#1748](the upstream project (see NOTICE)) Issue 3 (cold-cache 30s MCP-startup race), [#1747](the upstream project (see NOTICE)) (hooks shell injection — fixed in 3.6.28; orthogonal to this ADR)
...
> **Issue 3:** First-time invocation of `npx -y swarmdo@latest mcp start` from a cold npx cache hits a Claude Code MCP-startup timeout. Logged as `Starting connection with timeout of 30000ms` followed by the server staying in "still connecting" state for the entire session. Zero swarmdo tools register; the model falls through to native tools.
>
> **Diagnosis:** The `swarmdo@latest` package is roughly 1.8 MB across 999 files. Cold npx download + extraction + spawn can exceed 30 seconds.

We confirmed the bug is reproducible. The same cold-cache penalty hits every plugin skill that falls back to `npx @swarmdo/cli ...` for memory/hooks operations when MCP tools aren't registered. Today's reality:
...
- The remaining ~95% of the package (swarm, neural, federation, browser, daa, hive-mind, claims, performance, security, embeddings, swarmvector, intelligence, autopilot, …) is paid as a download cost on every cold cache, even when never invoked.
...
Split `@swarmdo/cli` into two packages with a backwards-compatible metapackage facade:

### 1. `@swarmdo/cli-core` (new, ~150–200 KB packed)
...
### 2. `@swarmdo/cli` (existing, becomes a metapackage)

```js
// v3/@swarmdo/cli/src/index.ts (after split)
export * from '@swarmdo/cli-core';
...
The metapackage:
- depends on `@swarmdo/cli-core` (as a regular dep — no dynamic resolution needed for core path)
...
Existing users (`npx @swarmdo/cli@latest <anything>`) continue to work unchanged. The cold-cache penalty for `memory` / `hooks` invocations drops because they're served from cli-core (small) — but only if the user installs cli-core directly. Users who install the metapackage still pay the full footprint (because npx pulls the whole thing); the win is that **plugin skills can opt to invoke `npx @swarmdo/cli-core@latest memory store ...`** for the hot path.
...
```diff
- npx @swarmdo/cli@latest memory store --namespace cost-tracking ...
+ npx @swarmdo/cli-core@latest memory store --namespace cost-tracking ...
```
...
- **Backwards compatibility risk.** Anyone importing from internal cli paths (e.g. `import x from '@swarmdo/cli/dist/src/commands/memory.js'`) will need to switch to `cli-core`. We control all known consumers (the plugins) so this is auditable.
...
- **No changes to the published `swarmdo` umbrella.** It continues to depend on `@swarmdo/cli` and gets the lazy-load benefits transparently.
- **No changes to `swarmdo` umbrella.** Same.
...
rm -rf ~/.npm/_npx
time npx @swarmdo/cli-core@alpha memory store --namespace test --key x --value 1
# Expected: < 5s wall-time on typical connection

# Compare to current cli
rm -rf ~/.npm/_npx
time npx @swarmdo/cli@alpha memory store --namespace test --key x --value 1
# Expected: > 30s on typical connection (matches the bug)
```

The smoke contract for `cli-core` mirrors the existing one in spirit: every command parses, every MCP tool definition has the canonical fields, no wildcard tool grants. Existing `@swarmdo/cli` smoke contract is preserved.
...
1. **Plugins update their script Bash blocks** to invoke `cli-core` for memory/hooks operations. Backwards-compatible — `cli` still works, just slower. Sample diff:
   ...
   ```diff
   - npx @swarmdo/cli@latest memory store ...
   + npx @swarmdo/cli-core@latest memory store ...
   ```

2. **README install matrix simplifies** — the "Plugin install (lite, slash commands only)" caveat becomes a "Plugin install + cli-core (fast, registers MCP via npx-warm fallback)" entry that approaches parity with full `npx swarmdo init` for the common case.

## Implementation status (2026-05-09)

The split is live in alpha. `@swarmdo/cli-core@3.7.0-alpha.5` is published and proven 38× faster cold-cache than `@swarmdo/cli`. Steps 3–5 of the plan of work are complete; steps 7–8 are partially complete. `latest` promotion (Step 7 final gate) and the issue #1760 PR comment (Step 8) remain deferred.

| Step | What | Status | Commit(s) |
|---|---|---|---|
| 1 | Branch `feat/cli-core-split` + ADR-100 + scaffold | Implemented | `9b42ca71e feat(cli-core): scaffold @swarmdo/cli-core package + ADR-100` |
...
| 5 | `@swarmdo/cli/src/index.ts` re-exports 4 foundation modules from cli-core | Implemented (alpha.5) | `c63319e3d feat(cli): re-export 4 foundation modules from cli-core@alpha.5` |
...
|---|---|---|---|
| `@swarmdo/cli-core@3.7.0-alpha.0` | 671 ms | 22.3 KB | 22 |
| `@swarmdo/cli@3.6.30` | 25.5 s | 1.8 MB | 999 |
...
| 3 | Backend abstraction: extract a lite memory backend (JSON-only, no sql.js/HNSW/ONNX) so `mcp-tools/memory-tools.ts` can copy cleanly into cli-core. Heavy backend stays in @swarmdo/cli. | ✅ done — alpha.1 |
...
| 5 | Update `@swarmdo/cli/src/index.ts` to re-export from cli-core + register lazy-loaded extras for non-foundation commands. | ✅ done — alpha.5 (4 foundation modules re-exported) |
...
- `hooks-tools.ts` (146 KB top-level) → 9 transitive imports across `memory/{sona-optimizer, ewc-consolidation, memory-bridge}` (the SONA/EWC++ neural surface) and `swarmvector/{moe-router, semantic-router, flash-attention, lora-adapter, enhanced-model-router}` (the ML routing layer)
