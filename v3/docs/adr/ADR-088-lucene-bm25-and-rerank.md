...
**Status**: Accepted — Implemented in swarmdo 3.10.28
**Date**: 2026-05-30
...
| **swarmdo Lucene RRF + CE rerank (BGE-base 110M)** | **0.521** |
| Multi-field RRF + CE rerank | 0.520 |
| Lucene BM25 alone | 0.505 |
| BM25 (published Lucene) | 0.502 |
| dense alone (BGE-base) | 0.489 |

**Acceptance test from the climb plan ("swarmdo beats BM25 on both datasets") PASSES.** With RRF+CE rerank we're 0.521 on the 2-dataset mean — beats published BM25 (0.502), beats every other published baseline except SPLADE++ (0.526, 1 percentage point above us) and BGE-large (0.551, 3 percentage points above).
...
- Did not switch the runtime retrieval defaults to Lucene BM25. Runtime stays on multi-field BM25 (better for swarmdo's commit-history corpora). The Lucene BM25 module is BEIR-runner-scoped.
...
- **swarmvector BGE bundling** (upstream/swarmvector#524) — kills the silent-fallback bug at source.
...
USE_LUCENE_BM25=1 RERANK=1 node /path/to/v3/@swarmdo/cli/scripts/run-beir-hybrid.mjs
# → nDCG@10 0.358, rank 2/11 on NFCorpus

cd /tmp/beir-scifact
USE_LUCENE_BM25=1 RERANK=1 BEIR_DATA_DIR=/tmp/beir-scifact/scifact \
  node /path/to/v3/@swarmdo/cli/scripts/run-beir-hybrid.mjs
