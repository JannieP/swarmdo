# ADR-114 — Adopt DSPy.ts as a Rufflo plugin for declarative prompt-program optimization
...
The key observation: **`dspy.ts` is built on the same substrate rufflo already ships** (`agentdb`, ReasoningBank, HNSW, RaBitQ). Integrating it is not bolting on a foreign dependency with its own storage and lifecycle — it is wiring a complementary capability onto infrastructure that is already present.

### What this would give rufflo that it does not have

| Capability | Today in rufflo | With a `dspy` plugin |
|---|---|---|
...
`dspy.ts` and rufflo's intelligence system *both* lean on AgentDB + ReasoningBank, and both are "self-learning". Without a sharp boundary, this becomes two systems doing fuzzily-similar things, confusing both Claude (which tool to call — see ADR-112) and contributors (where does this logic live?).
...
- **Rufflo intelligence = run-time, cross-task.** "Across *all* tasks this agent has ever done, what patterns predict success here?" It is always-on, ambient, and not tied to a single signature/metric.

A DSPy-compiled module can *write into* ReasoningBank (its trials are patterns), and rufflo intelligence can *retrieve* those — but they are not the same loop, and the plugin must not re-implement the intelligence pipeline.

## Decision

**Adopt `dspy.ts` as an optional Rufflo plugin — `@rufflo/plugin-dspy` — that wraps the published `dspy.ts` npm package and exposes declarative prompt-program optimization through MCP tools, a slash skill, and an optional agent-loop hook. It shares rufflo's AgentDB instance rather than standing up its own store.**
...
**Storage**: the plugin receives rufflo's AgentDB handle via the plugin context. It uses dedicated namespaces (`dspy/signatures`, `dspy/modules`, `dspy/trials`) so its data is inspectable and prunable independently, and so the `agentdb-curator` consolidation pipeline can see DSPy trials as candidate patterns.
...
- It does **not** become a required dependency of any core package. `@rufflo/cli` and the core packages must build and run with the plugin absent (the runtime pattern is `await import('@rufflo/plugin-dspy').catch(() => null)` — and per ADR-114's own guard, the published `package.json` must not declare it as a hard dep of anything).
- It does **not** introduce a second LM-provider abstraction. `dspy.ts`'s `configureLM` is bridged to rufflo's existing provider config (`@rufflo/providers`) so there is one place to set keys/models.
...
- `@rufflo/*` references in the plugin's `package.json` are `peerDependenciesMeta.optional` with `>=X.Y.Z-0` ranges (so prerelease publishes resolve).
...
- **First-party, MIT, same author.** No third-party licensing or maintenance-risk concerns; the package and rufflo move together.
...
- **Another package to keep version-aligned** with the `agentdb` line (if `dspy.ts` and rufflo pin different `agentdb` majors, the shared-handle story breaks). Mitigated by both being first-party.
...
- `scripts/audit-plugin-packages.mjs` — plugin packaging guard the `@rufflo/plugin-dspy` package must pass (#1902/#1903/#1904)
