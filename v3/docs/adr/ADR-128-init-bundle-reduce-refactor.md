...
**Related**: ADR-102 (supply-chain CI guards), ADR-103 (witness temporal history), ADR-127 (GitHub stack modernization), swarmdo issues #2078, #2079, #2086
...
`v3/@swarmdo/cli/.claude/` contains three directories — `agents/`, `commands/`, `helpers/` — plus `settings.json`. The `package.json` `"files"` array (`v3/@swarmdo/cli/package.json`) includes `.claude`, so everything in those directories ships in every published tarball.
...
`DEFAULT_INIT_OPTIONS.skills.core`, `.agentdb`, `.github`, and `.v3` are all `true`. On a fresh install, all four default to copying skills — none of which copy. The `copySkills()` error path pushes to `result.errors` but `executeInit()` wraps the call in a try/catch that continues on error without surfacing the failure to the CLI output. A user who runs `swarmdo init` on a clean machine gets no skills installed and no warning.
...
|---|---|---|---|
| `coder.md` | `agents/core/` | `swarmdo-core` | 460 diff lines |
| `researcher.md` | `agents/core/` | `swarmdo-core` | 411 diff lines |
| `reviewer.md` | `agents/core/` | `swarmdo-core` | 527 diff lines |
| `tester.md` | `agents/core/` | `swarmdo-testgen` | 527 diff lines |
| `memory-specialist.md` | `agents/v3/` | `swarmdo-rag-memory` | 1,049 diff lines |
| `security-auditor.md` | `agents/v3/` | `swarmdo-security-audit` | 785 diff lines |
| `sparc-orchestrator.md` | `agents/v3/` | `swarmdo-sparc` | 261 diff lines |
| `goal-planner.md` | `agents/goal/` | `swarmdo-goals` | 68 diff lines |
| `adr-architect.md` | `agents/v3/` | `swarmdo-adr` | 191 diff lines |

There is no deduplication policy. A user who runs `swarmdo init` and then `swarmdo plugins install swarmdo-core` ends up with two competing definitions of `coder.md` in their workspace. No collision warning is issued; the last-writer wins silently.
...
ADR-127 established one explicit init-template constraint that applies here: *"anything added to `swarmdo init` output must work in an empty directory with only `node` and `gh` available."* ADR-127's Phase 4 also established the opt-in pattern for init defaults: `settings-generator.ts` lines 55–60 and `#2079` showed that init defaults should be opt-out-friendly. ADR-127's plug-and-play discipline (each phase ships a runnable artifact independently) applies here too.
...
- Core commands: the three loose `swarmdo-*.md` files and the 8 `CommandsConfig` categories already covered — `analysis`, `automation`, `github`, `hooks`, `monitoring`, `optimization`, `sparc` (85 files).
...
- Any agent that is the canonical definition for a plugin's own subject matter: `swarmdo-core` owns `coder.md`, `swarmdo-rag-memory` owns `memory-specialist.md`, etc.
...
**Rule for orphaned command dirs**: every subdirectory under `v3/@swarmdo/cli/.claude/commands/` must have a corresponding key in `COMMANDS_MAP` and `CommandsConfig`. Directories without a key are either deleted or promoted to a named key. Dead directories (no key, not wanted) are deleted.

### Deduplication algorithm (net-new — needs separate consideration for plugin installer)

When `swarmdo plugins install <plugin>` runs and the plugin ships a file at `.claude/agents/X.md`:
...
**Acceptance**: `npx @swarmdo/cli@latest init` on a machine with an empty `~/.claude/` installs all `SKILLS_MAP.core` skills without errors. `find v3/@swarmdo/cli/.claude/skills -name 'SKILL.md' | wc -l` >= 29 in CI.
...
**Delete from `v3/@swarmdo/cli/.claude/agents/`**:
- `core/coder.md`, `core/researcher.md`, `core/reviewer.md` (forked from `swarmdo-core`)
- `core/tester.md` (forked from `swarmdo-testgen`)
- `v3/memory-specialist.md` (forked from `swarmdo-rag-memory`)
- `v3/security-auditor.md` (forked from `swarmdo-security-audit`)
- `v3/sparc-orchestrator.md` (forked from `swarmdo-sparc`)
- `goal/goal-planner.md` (forked from `swarmdo-goals`)
- `v3/adr-architect.md` (forked from `swarmdo-adr`)
...
No behavioral change for users who have the relevant plugins installed; they already have the plugin's version. Users who do not have the plugins lose those 9 agents — they are available via `swarmdo plugins install swarmdo-core swarmdo-rag-memory swarmdo-security-audit swarmdo-sparc swarmdo-goals swarmdo-adr swarmdo-testgen`.

**Acceptance**: `comm -12 <(find v3/@swarmdo/cli/.claude/agents -name '*.md' -exec basename {} \;) <(find plugins -name '*.md' -path '*/agents/*' -exec basename {} \;)` returns empty. `smoke-init-bundle-dedup.mjs` (Phase 5 script, run locally) passes assertion 1.
...
- `agents/data/` (2 files) — generic enough to either delete or move to `swarmdo-core`.
...
**Acceptance**: `find v3/@swarmdo/cli/.claude/commands -mindepth 1 -maxdepth 1 -type d` lists only directories that have a corresponding `COMMANDS_MAP` key. `agents.all: true` produces <= 85 agents. `commands.all: true` produces no `flow-nexus` commands from the init template.
...
**Expected outcome**: a default `swarmdo init` installs ~24 agents (`core:5 + consensus:7 + swarm:3 + sparc:4 + testing:2 + browser:0 = ~21`; browser dir is currently empty) rather than 98. The full 85-agent set remains accessible via `swarmdo init --agents=all`.

The `--all` flag and the `agents.all` key are NOT removed — `swarmdo init --all` remains a supported pattern.

**Acceptance**: `swarmdo init` (no flags) installs <= 30 agents. `swarmdo init --agents=all` installs all agents in the template. Existing users who relied on `agents.all: true` are informed via a deprecation notice in the `swarmdo migrate` command output.
...
1. **No filename collision**: for every `.md` file in `v3/@swarmdo/cli/.claude/{agents,commands,skills}/`, assert its basename does not appear in any `plugins/*/{agents,commands,skills}/` directory. Fail on first violation, print the conflicting pair.

2. **SKILLS_MAP completeness**: for every skill name in `SKILLS_MAP` (all arrays), assert `v3/@swarmdo/cli/.claude/skills/{name}/SKILL.md` exists. Fail on any missing skill.

3. **COMMANDS_MAP coverage**: for every subdirectory in `v3/@swarmdo/cli/.claude/commands/`, assert its basename is a key in `COMMANDS_MAP` (parsed from `executor.ts` via regex). Fail on any orphaned directory.

**`v3-ci.yml`** (one new job, gated on path filter):
```yaml
paths:
  - 'v3/@swarmdo/cli/.claude/**'
  - 'plugins/**'
  - 'scripts/smoke-init-bundle-dedup.mjs'
  - 'v3/@swarmdo/cli/src/init/executor.ts'
```
...
**Init-template constraint (from ADR-127)**: anything added or modified in `swarmdo init` output must work in an empty directory with only `node` and `gh` available. All five phases satisfy this: bundled skill directories are static markdown files with no runtime deps; agent deletions reduce rather than add requirements; `COMMANDS_MAP` additions are code-path changes in the CLI itself.

## Consequences

### Positive

- Phase 1 closes a silent regression that has affected every user who ran `swarmdo init` on a machine without a prior `~/.claude/skills/` — which is every fresh CI runner and every first-time user.
...
- Phase 3 reduces the default `swarmdo init` command count from 88 to 88 (no change for defaults; orphaned dirs get proper keys or are deleted) and removes domain-specific clutter from the base install.
...
- Phase 2 removes 9 agents from the init template. Users who have `swarmdo init`-initialized projects and rely on, say, `coder.md` from the init template without having `swarmdo-core` installed will find those agents missing after upgrading. Mitigation: `swarmdo migrate` should detect removed agents and print install suggestions.
...
- Phase 4's `agents.all: false` change is a breaking default change. Users who relied on the current all-agents behavior must pass `--agents=all` explicitly. The `swarmdo migrate` command must warn on upgrade.
...
|---|---|---:|---|
| 1 — Bundle skills | `v3/@swarmdo/cli/.claude/skills/` (new, 29 dirs) + CI assertion | small | none |
