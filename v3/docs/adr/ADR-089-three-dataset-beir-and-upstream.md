# ADR-089 — 3-Dataset BEIR + Upstream swarmvector + User Bug Fixes

**Status**: Accepted — Implemented in swarmdo 3.10.29
**Date**: 2026-05-30
**Tracking**: continuation of BEIR climb (ADR-085, 086, 087, 088) + #2246 + upstream/swarmvector#523-524
...
3. **swarmvector@0.2.27 Tier-0 wiring** — kills the silent-fallback bug (ADR-086) at source.
4. **4 user-reported bugs from #2246** — 3 fixed in swarmdo, 1 forwarded to upstream/agentdb#7.

ADR-090 (separate) covers the BGE query-prefix experiment.

## 3-dataset BEIR results

Best swarmdo config per dataset (no per-dataset tuning of the config; we ship the same `run-beir-hybrid.mjs` pipeline + flags everywhere):

| Dataset | Best swarmdo nDCG@10 | Best swarmdo pipeline | Rank | Best Listed Baseline |
|---|---:|---|---:|---:|
...
| swarmdo Lucene BM25 alone | — | 0.328 | 0.681 | _n/a_ | (2-dataset 0.505) |
...
## swarmvector@0.2.27 Tier-0 wiring (closes ADR-086 silent-fallback at source)

Updated `src/mcp-tools/neural-tools.ts` embedder cascade:

```
Tier 0 (NEW): swarmvector@0.2.27.embed() — bundled, no sharp dep, disk-cache hit
Tier 1: agentic-flow/reasoningbank (broken on darwin-arm64 without sharp)
Tier 2: @swarmdo/embeddings + agentic-flow provider
Tier 3: @swarmdo/embeddings + onnx provider
(no Tier 4 — leave realEmbeddings null, force hash-fallback with explicit _embeddingNote)
```

Verified active: probe returns `embedder: swarmvector@0.2.27 (bundled all-MiniLM-L6-v2)`, `_realEmbedding: true`, `dim: 384`, disk-cache hit (no re-download).
...
| Finding | Status | Fix |
|---|---|---|
| **#1** memory_search_unified hardcoded 6 namespaces (silently misses ~95% of an 8789-entry store) | **FIXED** | New `namespaces: string[]` param + `SWARMDO_MEMORY_SEARCH_NAMESPACES` env + dynamic enumeration via `listEntries({})` as the new default + `namespaceSource` audit field. 9 regression tests covering all 5 priority paths. |
| **#2** `npm install -g swarmdo` silently overwrites `dist/` patches | **acknowledged** | Tracked for a separate release (postinstall checksum + warning). |
| **#3** agentdb `addCausalEdge()` silently orphans edges when `NodeIdMapper.getNodeId()` returns undefined | **forwarded** | Filed as [upstream/agentdb#7](the upstream project (see NOTICE)). Will pull when agentdb pin bumps. |
...
- `src/mcp-tools/neural-tools.ts` — Tier-0 swarmvector probe with content-vs-shape-safe unwrap
...
- `package.json` adds `swarmvector: ^0.2.27` + root override
...
- **CPU-bound.** The remaining BEIR datasets (TREC-COVID 171k, FiQA 57k, HotpotQA 5M, NQ 2.7M, DBPedia 4.6M) are not feasible on this hardware. The user offered Tailscale swarmultra GPU but didn't enable ssh access for me to bootstrap there.
...
- **swarmvector BGE bundling** ([upstream/swarmvector#524](the upstream project (see NOTICE))) — kills the @xenova/transformers dependency entirely.
...
for ds in nfcorpus scifact arguana; do
  ...
  BEIR_DATA_DIR=/tmp/beir-$ds/$ds node /path/to/v3/@swarmdo/cli/scripts/run-beir-bge.mjs        # dense alone
  USE_LUCENE_BM25=1 RERANK=1 BEIR_DATA_DIR=/tmp/beir-$ds/$ds node /path/to/v3/@swarmdo/cli/scripts/run-beir-hybrid.mjs  # full pipeline
done

# #2246 tests
( cd v3/@swarmdo/cli && npx vitest run __tests__/memory-search-unified-2246.test.ts )
```
