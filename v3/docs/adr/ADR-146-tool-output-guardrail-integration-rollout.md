...
**Where**: `v3/@swarmdo/cli/src/mcp-tools/dispatch.ts` (the single chokepoint every tool result flows through).
...
**Where**: `v3/@swarmdo/cli/src/memory/memory-bridge.ts` (every `bridgeRetrieve` / `bridgeSearch` return).

**Shape**: Same `scanAndEnforce` call applied to each result's content field. Per-namespace policy overrides supported via `memory.guardrail.<namespace>.<severity>` in `swarmdo.config.json` (e.g., a sandbox namespace might `allow` content that production would `redact`).
...
**Where**: hive-mind proposal pipeline (`v3/@swarmdo/cli/src/hive-mind/*` — exact file pinned during P4 PR).
...
P4 is the highest-risk phase. It ships behind `SWARMDO_STRICT_CONSENSUS_GUARDRAIL=true` (default off) until at least two weeks of P2/P3 production telemetry are available to tune the swarm-layer pattern set.

### P5 — Per-tool policy overrides + structured telemetry

**Where**: `v3/@swarmdo/cli/src/hooks/*` for policy resolution; new `v3/@swarmdo/security/src/telemetry/guardrail-events.ts` for the sink.
