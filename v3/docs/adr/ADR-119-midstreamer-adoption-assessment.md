# ADR-119 — `midstreamer` npm package: adoption assessment for swarmdo & federation

**Status**: Accepted (2026-05-14) — **Decision: Wait. Partial adoption gated on specific upstream work.**
**Date**: 2026-05-14
**Authors**: claude (via `swarmdo-goals:deep-researcher` investigation, drafted with the upstream author)
**Related**: ADR-097 (federation budget breaker) · ADR-104 (federation transport — WebSocket today + clean QUIC upgrade path) · ADR-108 (`agentic-flow` native QUIC binding plan) · ADR-111 (federation WireGuard mesh) · ADR-118 (`aidefence@2.3.0` — sibling package in `upstream/midstream`) · upstream [`upstream/midstream`](the upstream project (see NOTICE)) · npm [`midstreamer@0.2.5`](https://www.npmjs.com/package/midstreamer)
**Supersedes**: nothing
**Investigation:** deep-researcher session `a70c6ee07aeab17bf` (88k tokens, 34 tool uses, ~4 min)

## Context

`midstreamer` is an npm package from `upstream` (same author as swarmdo and the AIMDS / `aidefence` line covered in ADR-118). The package name suggests it could provide pieces of two stated federation problems — **QUIC networking** (ADR-108) and **Tailscale-style mesh** (ADR-111) — plus a third theme the project keeps returning to: **in-flight agentics** (agent traffic flowing through inline gates). This ADR records what `midstreamer` actually provides, what it does not, and where (if anywhere) swarmdo should pick it up.

The investigation was carried out by the `swarmdo-goals:deep-researcher` agent. Evidence is graded inline: claims tied to file inspection, tarball contents, or `npm view` output are verified; anything else is marked inferred.
...
The **upstream `upstream/midstream` repo** is a Rust workspace with six crates (`midstreamer-temporal-compare`, `midstreamer-scheduler`, `midstreamer-attractor`, `midstreamer-neural-solver`, `midstreamer-strange-loop`, `midstreamer-quic`) plus the AIMDS subdirectory. The native `midstreamer-quic` crate does use `quinn` (the canonical Rust QUIC impl) and would provide real QUIC — but **it is not part of the npm package** and would require an N-API binding that doesn't exist yet.
...
The `QuicMultistream` class is a counter-tracking stub identical in nature to the stub `agentic-flow/transport/quic` build that ADR-104 already documented. ADR-108 already specifies the loader pattern (`AGENTIC_FLOW_QUIC_NATIVE=1`, same `AgentTransport` interface) that lets swarmdo swap to a real QUIC implementation when one exists in `agentic-flow`. If the real `midstreamer-quic` Rust crate were instead exposed via N-API, ADR-108's plan would still apply — the decision architecture is transport-agnostic. **There is no adoption value here until someone builds the N-API wrapper, which is upstream work in either repo.**
...
The **AIMDS sibling component** (same upstream repo, exposed via the `aidefence` npm package) **is** the inline threat-detection surface. swarmdo already adopted it via ADR-118 today. The 3-gate pattern (`aidefence_is_safe` → `aidefence_scan` → `aidefence_has_pii`) is the canonical in-flight agentic gate in swarmdo. `midstreamer` and `aidefence` are sibling packages from the same repo with non-overlapping purposes; adopting `aidefence` does not imply adopting `midstreamer`.
...
| midstreamer feature | Plausible swarmdo use | File(s) that would change | Already done? |
|---|---|---|---|
| `TemporalCompare` (DTW/LCS) | Trajectory similarity in `swarmdo-agentdb` / ReasoningBank — compare agent execution sequences for drift detection or past-run matching | `plugins/swarmdo-agentdb/` adapters, `v3/@swarmdo/memory/` | Not done; no current equivalent for sequence-level trajectory comparison |
| `StrangeLoop` | Online policy adjustment in `swarmdo-intelligence` — replace/augment the static pattern-weight update in `hooks_intelligence_learn` | `plugins/swarmdo-intelligence/` or `v3/@swarmdo/hooks/` | Partially redundant with `aimds-response@0.1.1` meta-learning + `ReasoningBank` distillation |
| `QuicMultistream` | Nothing — it is a stub | — | — |
| `NanoScheduler` | Nothing in Node.js server contexts — requires browser `window` | — | — |

A naming coincidence: `swarmvector@0.2.25` once mentioned a removed `midstream` subcommand. That is unrelated to `upstream/midstream` — different project, same word.

### Risks and gaps

- **Nothing in swarmdo currently depends on `midstreamer`.** Adopting adds a 64 KB WASM init cost (~50-100 ms cold start per package docs).
...
**Wait. Partial adoption is gated on specific upstream work or specific swarmdo requirements.**
...
| `NanoScheduler` | **Don't adopt** | Browser-only. Not usable in swarmdo's Node.js plugin processes. |
| `TemporalCompare` (DTW/LCS) | **Wait** | Genuine use case (trajectory drift detection, ReasoningBank similarity), but no current plugin has it as a stated gap. Adopt when a concrete trajectory-comparison need surfaces in `swarmdo-agentdb` or `swarmdo-intelligence` and the WASM init cost is acceptable in that plugin's latency budget. |
...
- **Concrete trigger for future revisit.** If `swarmdo-agentdb` or `swarmdo-intelligence` surfaces a trajectory-comparison requirement, the `TemporalCompare` adoption path is mapped out and bounded.
...
2. A swarmdo plugin (likely `swarmdo-agentdb` or `swarmdo-intelligence`) lands a stated requirement for sequence-level trajectory comparison (DTW / LCS / edit distance).
3. The upstream `upstream/midstream` repo publishes a Node-compatible scheduler (replacing `NanoScheduler`'s `window`-only design) that fits swarmdo's hook scheduling model.
...
- Upstream: [`upstream/midstream`](the upstream project (see NOTICE)) (Rust workspace + npm-wasm/ + AIMDS/)
