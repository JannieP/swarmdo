# ADR-150 — MetaHarness Integration Surfaces in `npx rufflo`
...
**External reference**: [`ruvnet/agent-harness-generator`](https://github.com/ruvnet/agent-harness-generator) — the upstream that publishes `metaharness` + `@metaharness/*`. Same author (rUv), explicitly designed around rufflo primitives.
**Research dossier**: published as a gist (linked from the tracking issue) with full graded-evidence sourcing.

## Context

We just shipped `rufflo@3.11.0` (also `@rufflo/cli@3.11.0`, `rufflo@3.11.0`). ADR-148/149 already wired `@metaharness/router` as an `optionalDependency` for cost-optimal model routing behind a triple gate. The remaining MetaHarness surface — twenty-plus `@metaharness/*` packages: kernel, host adapters (9), verticals (13), scaffold/eject CLI — is unused by rufflo despite being authored by the same maintainer specifically around rufflo's architecture.
...
| Both packages MIT-licensed, same maintainer as rufflo | npm registry | HIGH |
...
**MetaHarness may augment rufflo. MetaHarness must never become a required runtime dependency for core orchestration, memory, routing, MCP dispatch, agent execution, or federation.**

Every integration in this ADR, and every future ADR that extends it, must satisfy:

1. **Removable**: rufflo's `npm ls` with all `@metaharness/*` packages removed must still produce a working CLI. The triple-gate pattern used for `@metaharness/router` (env flag + artifact + import success) is the reference implementation.
...
4. **CI coverage of the absent path**: at least one CI job must run `--ignore-optional` (or equivalent) and assert rufflo still passes its smoke contract. This is the only structural defense against accidentally promoting an optional dep to required.

The intent of this constraint is to prevent rufflo from becoming a hidden second orchestration framework wrapped around its sibling's runtime. Reviewer's framing: *"Rufflo remains operational if every MetaHarness package is removed."* That sentence is now part of the API surface contract — any PR that breaks it is a breaking change requiring its own ADR.
...
1. **`plugins/rufflo-metaharness/`** with three skills (`harness-score`, `harness-genome`, `harness-mint`), conventional structure (`plugin.json`, `skills/*/SKILL.md` with `allowed-tools: Bash`, `scripts/smoke.sh`). Skills shell out to `npx metaharness` / `npx harness` — no library imports. Covered by the fleet meta-smoke and the three existing audits (exit-bypass, frontmatter, manifest).
...
Exit criteria: `plugins/rufflo-metaharness/scripts/smoke.sh` passes; meta-smoke shows 33/33 plugins green; CI score + mcp-scan jobs green on main; new bench run shows `routedBy: 'metaharness-krr'` for ≥ 1 routing decision driven by measured-seed KRR.
...
- `npx rufflo eject` command wrapping `metaharness --from-existing ./` for one-shot harness extraction (attribution preserved via the `<!-- rufflo-attribution-block -->` convention).
...
- Harness entries in the rufflo plugin registry — accept `type: 'harness'` in `discovery.ts`; surface via `npx rufflo plugins list --type harness`.
...
- Closes the "what is the relationship between rufflo and MetaHarness?" question by answering it in the UX rather than the docs.
- Rufflo gains a continuous, machine-readable readiness score and MCP threat-model on every PR — the same primitives we use to score third-party repos for harness viability.
...
- Three integration channels match MetaHarness's three contributions — analysis, routing, hosts — without forcing the kernel's full surface into a position where its 0.x stability would block rufflo releases.
...
- **Bus factor**: same maintainer as rufflo, MetaHarness, and ruvector. No change from today, but the dependency edge is now explicit.
...
- **GCP dependency**: `harness validate` uses GCP Secret Manager via `gcloud`. Rufflo CI must skip those subcommands (or mock them) — explicit `--skip-gcp` flag from the `harness validate` command surface handles this.
- **Phase-1 MCP plugin spawns subprocesses**: subprocess crashes, timeouts, and stdout-parsing edge cases are now in rufflo's failure surface. Mitigation: hard timeout (60s) per invocation, captured stderr in error responses, structured-JSON output enforced via `--json` flag everywhere.
...
- Maintaining a `rufflo-metaharness` plugin doubles documentation surface for two sibling tools. Mitigation: skill descriptions explicitly point to the upstream MetaHarness docs as canonical for the underlying functionality; the plugin only documents the rufflo-side adaptation.

## Alternatives Considered

**Alternative A: Ignore MetaHarness, build all scaffolding/score/genome natively in rufflo.**
Rejected. `buildRepoScorecard()`, `buildGenomeReport()`, `scanMcp()`, `buildThreatModel()` are already-tested implementations exposing clean TypeScript APIs. Reimplementing them in rufflo is pure duplication cost with no advantage. The eject path's `rewriteContent()` with attribution-block preservation is subtle.
...
- Should the Phase-1 plugin's `harness-mint` skill require explicit user confirmation in the Claude Code UI before writing any files? Lean yes — destructive-action-confirmation matches rufflo's "executing actions with care" principle.
...
- Does the `oia-audit` background worker (Phase 2) belong in `rufflo-loop-workers` or in `rufflo-metaharness`? Probably the latter, since the audit output is MetaHarness-specific.
...
- Rufflo baseline scorecard captured: harnessFit 82/100, compileConfidence
...
- Rufflo genome: repo_type `node_mcp_ci`, risk_score 0.27 (low),
  publish_readiness 0.9, mcp_surface `remote`.

### Phase 1 — MVP ✅ DONE (iters 1–3)
- `plugins/rufflo-metaharness/` with 6 skills (one more than ADR-150
...
- `npx rufflo metaharness <subcommand>` top-level dispatcher in
  `v3/@rufflo/cli/src/commands/metaharness.ts` (iter 3).
- `metaharness@~0.1.11` and `@metaharness/router@~0.3.2` in
  optionalDependencies of BOTH `@rufflo/cli/package.json` and
  `rufflo/package.json` (iter 3). Tilde pin, not caret — per
...
- ✅ `npx rufflo eject` (iters 4–5) — Phase-2 differentiator wrapping
...
- ✅ `'harness'` PluginType in plugin registry (iter 6) — schema
  extension only, zero runtime overhead. `npx rufflo plugins list
...
- ✅ `SelfEvolvingRouter` parallel-logging — BOTH HALVES LANDED:
  - **Analyzer** (iter 10):
  `plugins/rufflo-metaharness/scripts/router-parallel-analyze.mjs` reads
  ...
  added to `optionalDependencies` of `@rufflo/cli` AND
  `rufflo/package.json` so the future Recording side can dynamic-import
...
- 33 plugins in `scripts/smoke-all-plugins.mjs` (was 32; +1 for
  rufflo-metaharness)
- 19 structural invariants in `plugins/rufflo-metaharness/scripts/smoke.sh`
...
- Total rufflo-metaharness smoke surface: ~260 assertions
...
- iter 45 separates rufflo-side integration health from upstream-dep
...
- ~290 total rufflo-metaharness smoke assertions
...
The single non-test rufflo source file that statically imports a
`@metaharness/*` package is:

```
v3/@rufflo/cli/src/ruvector/neural-router.ts  ← @metaharness/router
                                                     (dynamic import,
                                                      triple-gated)
```

All other rufflo code reaches MetaHarness exclusively through the
