...
**Related**: [`agentic-flow@2.0.11`](https://www.npmjs.com/package/agentic-flow), [upstream/agentic-flow](the upstream project (see NOTICE)), ADR-118 (AIDefence 2.3.0), ADR-121 (embeddings SwarmVector upgrade — long-term Xenova migration), ADR-122 (browser substrate), supply-chain hardening PR #2050, swarmdo issue #2046
**Supersedes**: nothing (upstream-targeted)

## Context

After landing the supply-chain hardening in PR #2050 (`scripts/audit-supply-chain.mjs` + `.github/supply-chain/allowed-deps.json` + dependency-review-action), the audit caught a real HIGH CVE in `@swarmdo/browser`'s fresh-install dep graph:

```
swarmdo-browser-consumer
  └── @swarmdo/browser
        └── agentic-flow ^2.0.11
...
The `agentdb → @opentelemetry/sdk-node` chain was fixed in [PR #2050 commit `9a8c9c464`](the upstream project (see NOTICE)) by bumping agentdb to `3.0.0-alpha.14` + npm overrides pinning `@opentelemetry/sdk-node ≥ 0.218.0`. Overrides do NOT cascade across separate npm projects, so the same block was duplicated in `v3/@swarmdo/browser/package.json` to keep standalone installs clean.

What remains: **`@xenova/transformers` is a direct dependency of `agentic-flow`**, and `agentic-flow` is a direct dependency of `@swarmdo/browser`. Overrides on the consumer side can pin to a newer Xenova version — but the latest is 2.17.2, and the only fix-available is `@xenova/transformers@2.0.1` (a *downgrade* via a major version-bump). Upgrading would break Xenova's runtime API in ways the agentic-flow embedding code paths aren't prepared for, and downgrading to 2.0.1 sheds three years of ONNX-runtime improvements.
...
- Bump `agentic-flow` to `^2.0.12` in `v3/@swarmdo/browser/package.json`
...
- The fresh-install end-user audit on `@swarmdo/browser` drops from **7 HIGH** (after PR #2050) to **0 HIGH** when `--omit=optional` is used, or remains the same when the user opts into the embedding feature explicitly
...
## Update swarmdo after upstream ships

1. Bump `v3/@swarmdo/browser/package.json` → `agentic-flow ^2.0.12`
...
- **Full Xenova retirement** — ADR-121 Phase 4 tracks the larger migration to `swarmvector-onnx-embeddings-wasm`. This ADR is a tactical patch that lets swarmdo ship a clean supply-chain audit *now*; the strategic migration continues independently.
...
- **Touching `@swarmdo/embeddings`** — also covered by ADR-121.
...
- [`agentic-flow` repo](the upstream project (see NOTICE))
...
- swarmdo PR #2050 (supply-chain hardening that surfaced this)
- swarmdo issue #2046
