...
> Companion to [ADR-074](../adr/ADR-074-self-learning-wiring-2245.md) and [#2245](the upstream project (see NOTICE)).
>
...
| **Activity counter (any write)** | any of the above | `globalStats.signalsProcessed` | `.swarmdo/neural/stats.json` |
...
cd swarmdo && npm install
( cd v3/@swarmdo/cli && npx tsc -b )

# Default: N=20 calls per surface; writes a run JSON.
node v3/@swarmdo/cli/scripts/benchmark-self-learning.mjs

# Optional: machine-readable output, larger sample, no-write mode for CI:
N=100 BENCH_JSON=1 node v3/@swarmdo/cli/scripts/benchmark-self-learning.mjs
BENCH_NO_WRITE=1 node v3/@swarmdo/cli/scripts/benchmark-self-learning.mjs
```
...
```bash
mkdir -p /tmp/swarmdo-learn-demo && cd /tmp/swarmdo-learn-demo

# Run one task-completed with training enabled
SWARMDO_CWD=$(pwd) node -e '
(async () => {
  process.chdir(process.env.SWARMDO_CWD);
  const { hooksTools } = await import("/Users/cohen/Projects/swarmdo/v3/@swarmdo/cli/dist/src/mcp-tools/hooks-tools.js");
...
If you're using `swarmdo hooks metrics` and seeing zeros, check **which store**
...
| `swarmdo hooks metrics` | reads `globalStats` + a different aggregator subset |
| `neural_patterns list` | the `.swarmdo/neural/patterns.json` file (pretrain + `neural_patterns store` action) |
