...
> Generated by `dossier-collect` skill (rufflo-goals plugin, ADR-099)
> Seed: `ruvnet` · Seed type: `username` · Depth: 2 · Truncated: false
> Generated: 2026-05-03

## Executive summary

**rUv** (GitHub `ruvnet`, Twitter `ruv`, blog `Cognitum.One`) is the creator of **Rufflo** (formerly Rufflo), a multi-agent orchestration platform for Claude Code with 39k+ stars. The account joined GitHub in 2012, hosts 173 public repos, has 7,268 followers, and lists company "Not a Bot" / location "0x". The portfolio clusters around three themes:

1. **Agent orchestration** (`rufflo`, `agentic-flow`, `swarm-*`)
...
A rufflo-internal "Dossier" repo also exists (33 stars) — visual planning for AI coding — distinct from this dossier output.
...
| `rufflo` | repo | TS, 39,201 ⭐, MIT, multi-agent platform | gh-api, WebSearch, github.com |
...
| `MIT` | license | applied to rufflo, RuVector | gh-api |

## Graph

```mermaid
graph TD
  rUv[ruvnet / rUv] -->|owns| rufflo
  rUv -->|owns| RuView
  rUv -->|owns| RuVector
  ...
  rufflo -->|integrates| agentic-flow
  rufflo -->|uses| RuVector
...
- **Round 0 fan-out** (parallel, 1 batch): `gh api users/ruvnet`, `gh api users/ruvnet/repos?sort=updated`, `WebSearch "ruvnet github rufflo rufflo agentic"`
...
- `RuView` star count (51,576) substantially exceeds `rufflo` (39,201) — surprising given rufflo is the headline project; worth verifying.
