...
Example outputs from the `rufflo-goals` plugin's `dossier-collect` skill (ADR-099). Each subdirectory contains:
...
| [rufflo-goals/](./rufflo-goals/) | `rufflo-goals` | concept (plugin) | Glob, Read, Grep |

## How to generate your own

```
/rufflo-goals:dossier-collect <seed> [--max-depth N] [--max-breadth N] [--budget-usd N]
```
