...
Example outputs from the `swarmdo-goals` plugin's `dossier-collect` skill (ADR-099). Each subdirectory contains:
...
|---|---|---|---|
| [upstream/](./upstream/) | `upstream` (the upstream author) | username | gh API, WebSearch, profile fetch |
| [adr-088/](./adr-088/) | `ADR-088` | adr-id | Read, Grep, git log, codebase |
| [swarmdo-goals/](./swarmdo-goals/) | `swarmdo-goals` | concept (plugin) | Glob, Read, Grep |

## How to generate your own

```
/swarmdo-goals:dossier-collect <seed> [--max-depth N] [--max-breadth N] [--budget-usd N]
```
