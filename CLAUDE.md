...
| `usage` | 10 | Claude Code token/cost analytics from local transcripts (daily, monthly, models, projects, sessions, 5h `blocks` w/ live burn, `errors` tool-failure analytics, `cache` prompt-cache efficiency + $ saved, `guard` budget policy, `reflect` wrapped-style retrospective, `limits` quota-cap forecaster; alias: `cost`) |
