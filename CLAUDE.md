...
> **Swarmdo v1.7.1** (2026-07-07) — every swarmdo slash command namespaced under `sDo` (`/sDo:swarm:init`, skills `/sdo-ponytail`) so they group together in Claude Code's `/` menu; legacy unprefixed copies auto-migrated on init/upgrade.
> 6,000+ commits, 314 MCP tools, 16 agent roles + custom types, 19 AgentDB controllers, 21 native plugins.
> Packages: `swarmdo@1.7.1` (umbrella), `@swarmdo/cli@1.7.1`, `swarmdo-bridge@1.7.1` (wrapper in `swarmdo/`)
