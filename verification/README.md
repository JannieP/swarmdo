# Rufflo Verification System

A cross-platform, cryptographically-signed regression-protection toolkit. Every documented fix in rufflo gets attested by a SHA-256 + marker substring + Ed25519 signature. Anyone with the same git commit can re-derive the public key and verify it independently.
...
The witness toolkit lives in `plugins/rufflo-core/scripts/witness/` — copy it into your project (or install the rufflo-core plugin).
...
In rufflo's monorepo there's a wrapper that hard-codes paths:
...
| **Ed25519 signature** | Deterministic seed `sha256(gitCommit + ':rufflo-witness/v1')` | Manifest tampering, key-derivation drift |
...
{
  "manifest": {
    "schema": "rufflo-witness/v1",
    "issuedAt": "2026-05-09T00:00:00.000Z",
    "gitCommit": "<full sha>",
    "branch": "main",
    "os": "macos",                              // matches the folder
    "releases": { "rufflo": "3.7.0-alpha.21", "@rufflo/plugin-agent-federation": "1.0.0-alpha.15" },
    "summary": { "totalFixes": 102, "verified": 102, "missing": 0 },
    "fixes": [
      {
        "id": "F1",
        "desc": "...",
        "file": "v3/@rufflo/cli/dist/...",
        "sha256": "<64 hex>",
  ...
  "integrity": {
    ...
    "seedDerivation": "sha256(gitCommit + ':rufflo-witness/v1')"
  }
}
```
...
## How it integrates with the rest of rufflo's regression stack

| Layer | Where | What it catches |
|---|---|---|
| Layer 1 — install smoke | `v3/@rufflo/memory/scripts/smoke-no-bsqlite.mjs` + CI `smoke-install-no-bsqlite` | `npm install` failures on platforms without prebuilds |
| Layer 1 — hook smoke | `plugins/rufflo-core/scripts/test-hooks.mjs` + CI `plugin-hooks-smoke` | Plugin/CLI flag drift, parser ambiguity |
| Layer 1 — MCP protocol smoke | `plugins/rufflo-core/scripts/test-mcp-protocol.mjs` + CI `mcp-protocol-smoke` | HTTP MCP wire-format compliance |
| Layer 1 — memory-import smoke | `plugins/rufflo-core/scripts/test-memory-import.mjs` + CI `memory-import-smoke` | Memory_import_claude WSL path + key sanitization regressions |
...
- [`plugins/rufflo-core/skills/witness/SKILL.md`](../plugins/rufflo-core/skills/witness/SKILL.md) — Claude Code skill workflow
- [`plugins/rufflo-core/agents/witness-curator.md`](../plugins/rufflo-core/agents/witness-curator.md) — curator agent definition
